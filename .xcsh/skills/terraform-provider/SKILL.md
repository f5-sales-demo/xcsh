...
Pool ref: set pool.name to existing origin pool name in same namespace. HTTPS: replace http { port=80 } with https_auto_cert { http_redirect=true default_header {} tls_config { default_security {} } no_mtls {} }. WAF: add disable_waf {} or app_firewall { name="waf" namespace="demo-app" }. Import: terraform import xcsh_http_loadbalancer.example ns/name
...
Healthcheck ref: add healthcheck { name="hc" namespace="demo-app" }. Import: terraform import xcsh_origin_pool.example ns/name
...
namespace: demo-app"xcsh_namespace" "example" { name="staging" }
