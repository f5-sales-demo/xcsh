...
# default_pool + routes coexist (pool as default backend, routes for specific paths)
check_constraint "pool_plus_routes_accept" \
...
# Inline pool with empty origin_servers rejected (min_items=1)
check_constraint "pool_empty_servers_reject" \
    '{"metadata":{"name":"xcsh-uat-eos","namespace":"'${NS}'"},"spec":{"domains":["eos-test.example.com"],"https_auto_cert":{},"default_pool":{"port":80,"origin_servers":[],"no_tls":{}}}}' \
    "400"

# default_pool port=0 rejected (unlike https_auto_cert which accepts 0 as default=443)
check_constraint "pool_port_0_reject" \
    '{"metadata":{"name":"xcsh-uat-pp0","namespace":"'${NS}'"},"spec":{"domains":["pp0-test.example.com"],"https_auto_cert":{},"default_pool":{"port":0,"origin_servers":[{"public_name":{"dns_name":"neverssl.com"}}],"no_tls":{}}}}' \
    "400"

# default_pool port=65536 rejected (same upper limit as LB port)
check_constraint "pool_port_65536_reject" \
...
# default_pool requires port_choice (port is mandatory even with https_auto_cert)
check_constraint "pool_no_port_reject" \
    '{"metadata":{"name":"xcsh-uat-pnp","namespace":"'${NS}'"},"spec":{"domains":["pnp-test.example.com"],"https_auto_cert":{},"default_pool":{"origin_servers":[{"public_name":{"dns_name":"neverssl.com"}}],"no_tls":{}}}}' \
    "400"

# host_name is read-only: value sent but server returns empty string
check_constraint "hostname_ignored_accept" \
...
# default_pool + default_route_pools coexist (composable, NOT mutually exclusive)
check_constraint "composable_routing_accept" \
    '{"metadata":{"name":"xcsh-uat-comp","namespace":"'${NS}'"},"spec":{"domains":["comp-test.example.com"],"https_auto_cert":{},"default_pool":{"port":80,"origin_servers":[{"public_name":{"dns_name":"neverssl.com"}}],"no_tls":{}},"default_route_pools":[{"pool":{"tenant":"nferreira-cuxnbbdn","namespace":"'${NS}'","name":"'${POOL_NAME}'"},"weight":50}]}}' \
    "200"
