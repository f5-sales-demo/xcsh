...
Ce document décrit comment l'agent de codage charge actuellement les modèles, applique les surcharges, résout les identifiants et choisit les modèles à l'exécution.

## Ce qui contrôle le comportement des modèles

Fichiers d'implémentation principaux :

- `src/config/model-registry.ts` — charge les modèles intégrés + personnalisés, surcharges de fournisseurs, découverte à l'exécution, intégration de l'authentification
- `src/config/model-resolver.ts` — analyse les patrons de modèles et sélectionne les modèles initial/smol/slow
...
Comportement hérité encore présent :
...
`provider-id` est la clé canonique du fournisseur utilisée dans la sélection et la recherche d'authentification.
...
### Valeurs `api` autorisées pour fournisseur/modèle
...
3. Appliquer les surcharges de fournisseur (`baseUrl`, `headers`) aux modèles intégrés.
...
Les heuristiques actuelles sont intentionnellement étroites :

- les préfixes en amont intégrés peuvent être supprimés lorsqu'ils sont présents, par exemple `anthropic/...` ou `openai/...`
...
- `modelOverrides` peut surcharger les métadonnées du modèle (`name`, `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`, `headers`, `compat`, `contextPromotionTarget`).
...
La découverte à l'exécution appelle `GET /api/tags` sur Ollama et synthétise des entrées de modèle avec des valeurs par défaut locales.
...
La découverte à l'exécution appelle `GET models` sur llama.cpp et synthétise des entrées de modèle avec des valeurs par défaut locales.
...
La découverte à l'exécution récupère les modèles (`GET /models`) et synthétise des entrées de modèle avec des valeurs par défaut locales.
...
- enregistrement de gestionnaires de flux personnalisés pour de nouveaux identifiants d'API
...
- `getAvailable()` filtre les modèles qui sont sans clé ou dont l'authentification est résoluble.
...
### CLI et analyse de patrons

`model-resolver.ts` prend en charge :

- `provider/modelId` exact
- identifiant canonique de modèle exact
- identifiant de modèle exact (fournisseur inféré)
- correspondance floue/sous-chaîne
- patrons glob de portée dans `--models` (par ex. `openai/*`, `*sonnet*`)
...
2. l'identifiant canonique exact se résout via l'index canonique
...
2. premier modèle de portée (si pas en reprise)
...
Les alias de rôle comme `pi/smol` s'expandent via `settings.modelRoles`. Chaque valeur de rôle peut également ajouter un sélecteur de réflexion tel que `:minimal`, `:low`, `:medium`, ou `:high`.

Si un rôle pointe vers un autre rôle, le modèle cible hérite normalement et tout suffixe explicite sur le rôle référençant l'emporte pour cette utilisation spécifique au rôle.

Paramètres associés :

- `modelRoles` (enregistrement)
- `enabledModels` (liste de patrons de portée)
...
Pour `enabledModels` et CLI `--models` :

- les identifiants canoniques exacts s'expandent vers toutes les variantes concrètes de ce groupe canonique
...
Les deux interfaces conservent les modèles préfixés par le fournisseur visibles et sélectionnables.

Elles exposent désormais aussi les modèles canoniques/coalescés :

- `/model` inclut une vue canonique aux côtés des onglets de fournisseur
...
2. Si une cible est trouvée, basculer vers elle et réessayer la requête — pas de compaction nécessaire.
...
Lors du basculement depuis/vers `openai-codex-responses`, la clé d'état du fournisseur de session `openai-codex-responses` est fermée avant le changement de modèle. Cela supprime l'état de transport websocket pour que le prochain tour démarre proprement sur le modèle promu.
...
### Configurer des chaînes de repli explicites
...
Le générateur de modèles intégré assigne également ceci automatiquement pour les modèles `*-spark` lorsqu'un modèle de base du même fournisseur existe.
...
### Surcharger la route du fournisseur intégré + métadonnées du modèle
...
### Auto-génération au premier lancement
...
| Condition | Action |
|-----------|--------|
| `models.yml` manquant | Auto-génération depuis les variables d'environnement |
...
La plupart des configurations de modèles passent désormais par `models.yml` via `ModelRegistry`.

Un chemin hérité notable subsiste : la résolution d'authentification Anthropic pour la recherche web lit toujours directement `~/.xcsh/agent/models.json` dans `src/web/search/auth.ts`.

Si vous dépendez de ce chemin spécifique, gardez la compatibilité JSON à l'esprit jusqu'à la migration de ce module.
...
- L'erreur est exposée via `ModelRegistry.getError()` et affichée dans l'interface/notifications.
