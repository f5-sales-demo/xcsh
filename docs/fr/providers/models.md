...
## Emplacement du fichier de configuration et comportement legacy

Chemin de configuration par défaut :

- `~/.xcsh/agent/models.yml`

Comportement legacy encore présent :

- Si `models.yml` est absent et que `models.json` existe au même emplacement, il est migré vers `models.yml`.
- Les chemins de configuration explicites `.json` / `.jsonc` sont toujours supportés lorsqu'ils sont passés de manière programmatique à `ModelRegistry`.
...
`configVersion` est un entier optionnel écrit par le système d'auto-configuration. Lorsqu'il est présent, xcsh l'utilise pour détecter les configurations obsolètes et les mettre à jour automatiquement.

`provider-id` est la clé canonique du fournisseur utilisée à travers la sélection et la recherche d'authentification.
...
### Fournisseur de surcharge uniquement (`models` absent ou vide)
...
### Vérifications des valeurs de modèle
...
6. Appliquer les modèles découverts à l'exécution (actuellement Ollama et LM Studio), puis ré-appliquer les surcharges de modèles.
...
2. correspondances d'identifiants officiels intégrés depuis les métadonnées des modèles intégrés
3. normalisation heuristique conservatrice pour les variantes de passerelle/fournisseur
4. repli vers l'identifiant propre du modèle concret

Les heuristiques actuelles sont intentionnellement restreintes :

- les préfixes amont intégrés peuvent être supprimés lorsqu'ils sont présents, par exemple `anthropic/...` ou `openai/...`
- les variantes de version avec points et tirets ne peuvent se normaliser que lorsqu'elles correspondent à un identifiant officiel existant, par exemple `4.6 -> 4-6`
- les familles ou versions ambiguës ne sont pas fusionnées sans correspondance intégrée ou surcharge explicite

### Comportement de résolution canonique

Lorsque plusieurs variantes concrètes partagent un identifiant canonique, la résolution utilise :

1. la disponibilité et l'authentification
2. `modelProviderOrder` de `config.yml`
3. l'ordre existant du registre/fournisseur si `modelProviderOrder` n'est pas défini
...
La découverte à l'exécution appelle `GET /api/tags` sur Ollama et synthétise des entrées de modèles avec les valeurs par défaut locales.
...
Note : il utilise la plus récente API de messages anthropic au lieu d'openai-completions.
...
La découverte à l'exécution appelle `GET models` sur llama.cpp et synthétise des entrées de modèles avec les valeurs par défaut locales.
...
La découverte à l'exécution récupère les modèles (`GET /models`) et synthétise des entrées de modèles avec les valeurs par défaut locales.
...
- le remplacement/ajout de modèles pour un fournisseur
- l'enregistrement d'un gestionnaire de flux personnalisé pour de nouveaux identifiants d'API
- l'enregistrement d'un fournisseur OAuth personnalisé
...
5. Résolveur de repli de ModelRegistry (`apiKey` du fournisseur depuis `models.yml`, sémantique nom-env-ou-littéral)
...
- Les fournisseurs marqués `auth: none` sont traités comme disponibles sans identifiants.
- `getApiKey*` retourne `kNoAuth` pour eux.
...
1. fournisseur+modèle explicite via CLI
2. premier modèle dans la portée (si pas de reprise)
...
### Alias de rôle et paramètres

Rôles de modèle supportés :

- `default`, `smol`, `slow`, `plan`, `commit`

Les alias de rôle comme `pi/smol` s'étendent via `settings.modelRoles`. Chaque valeur de rôle peut également ajouter un sélecteur de réflexion tel que `:minimal`, `:low`, `:medium` ou `:high`.

Si un rôle pointe vers un autre rôle, le modèle cible hérite toujours normalement et tout suffixe explicite sur le rôle référant l'emporte pour cette utilisation spécifique au rôle.

Paramètres associés :

- `modelRoles` (enregistrement)
- `enabledModels` (liste de patterns avec portée)
- `modelProviderOrder` (priorité globale fournisseur-canonique)
...
- `provider/modelId` pour épingler une variante concrète de fournisseur
- un identifiant canonique tel que `gpt-5.3-codex` pour permettre la coalescence de fournisseurs

Pour `enabledModels` et `--models` en CLI :

- les identifiants canoniques exacts s'étendent à toutes les variantes concrètes de ce groupe canonique
...
- `--list-models` affiche une section canonique plus les lignes de fournisseurs concrets

Sélectionner une entrée canonique enregistre le sélecteur canonique. Sélectionner une ligne de fournisseur enregistre le `provider/modelId` explicite.

## Promotion de contexte (chaînes de repli au niveau du modèle)

La promotion de contexte est un mécanisme de récupération de dépassement pour les variantes à petit contexte (par exemple `*-spark`) qui promeut automatiquement vers un modèle apparenté à plus grand contexte lorsque l'API rejette une requête avec une erreur de longueur de contexte.
...
2. Si une cible est trouvée, basculer vers celle-ci et réessayer la requête — pas de compaction nécessaire.
...
Les candidats sont ignorés sauf si les identifiants se résolvent (`ModelRegistry.getApiKey(...)`).

### Transfert websocket OpenAI Codex

Si l'on bascule depuis/vers `openai-codex-responses`, la clé d'état du fournisseur de session `openai-codex-responses` est fermée avant le changement de modèle. Cela supprime l'état du transport websocket afin que le prochain tour démarre proprement sur le modèle promu.
...
- ne réécrit pas le mappage de rôle sauvegardé
...
Le générateur de modèles intégrés assigne également ceci automatiquement pour les modèles `*-spark` lorsqu'un modèle de base sur le même fournisseur existe.

## Champs de compatibilité et de routage

`models.yml` supporte ce sous-ensemble `compat` :
...
### Surcharge de la route du fournisseur intégré + métadonnées du modèle
...
## Auto-configuration du proxy LiteLLM
...
Un `config.yml` par défaut est également généré avec des paramètres de fournisseur d'images sensés.
...
| Condition | Action |
|-----------|--------|
| `models.yml` manquant | Génération automatique depuis les variables d'environnement |
...
| `configVersion` manquant ou obsolète | Sauvegarde en `.bak`, régénération avec la version actuelle |
...
Si l'une des variables n'est pas définie, l'auto-configuration est silencieusement ignorée.

### Versionnement de la configuration

Les configurations générées incluent un champ `configVersion`. Lorsque le format généré change dans les versions futures, xcsh détecte les configurations obsolètes et les met automatiquement à jour (avec sauvegarde).

## Avertissement concernant les consommateurs legacy

La plupart de la configuration des modèles passe désormais par `models.yml` via `ModelRegistry`.

Un chemin legacy notable subsiste : la résolution de l'authentification Anthropic pour la recherche web lit encore directement `~/.xcsh/agent/models.json` dans `src/web/search/auth.ts`.
...
- Si `LITELLM_BASE_URL` et `LITELLM_API_KEY` sont définis, la vérification de santé au démarrage tente une auto-réparation (sauvegarde du fichier corrompu, régénération depuis les variables d'environnement). Si la réparation réussit, le registre recharge la configuration corrigée.
