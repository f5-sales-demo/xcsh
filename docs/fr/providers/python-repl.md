...
Il couvre le comportement de l'outil, le cycle de vie du noyau et de la passerelle, la gestion de l'environnement, la sémantique d'exécution, le rendu des sorties et les modes de défaillance opérationnels.

## Périmètre et fichiers clés

- Interface de l'outil : `src/tools/python.ts`
...
L'outil `python` exécute une ou plusieurs cellules Python via un noyau reposant sur Jupyter Kernel Gateway (et non en invoquant `python -c` directement par cellule).
...
2. **Passerelle locale partagée** (chemin par défaut)
   - Utilise un seul processus partagé coordonné sous `~/.xcsh/agent/python-gateway`.
...
- Démarre une nouvelle passerelle lorsqu'aucune passerelle saine n'existe.
...
6. Chargement des modules d'extension depuis :
   ...
   - projet : `<cwd>/.xcsh/modules/*.py` (remplace le module utilisateur du même nom)
...
- `session` (par défaut)
  ...
  - Un redémarrage automatique est autorisé une fois ; un crash répété entraîne une défaillance définitive.
...
- La liste de refus supprime les clés d'API courantes (OpenAI/Anthropic/Gemini/etc.)
...
L'initialisation de l'environnement du noyau en Python effectue également :
...
En cas d'échec du contrôle préalable Python, la création de l'outil se dégrade en bash-only pour cette session.

## Flux d'exécution et annulation/délai d'attente

### Délai d'attente au niveau de l'outil

Le délai d'attente de l'outil `python` est exprimé en secondes, par défaut 30, limité à `1..600`.
...
Les résultats de l'outil peuvent inclure des métadonnées de troncature et `artifact://<id>` pour la récupération complète des sorties.
...
- Rendu de l'outil (`python.ts`) :
  ...
  - prend en charge le mode étendu pour la sortie complète et des détails de statut plus riches
- Rendu interactif (`python-execution.ts`) :
  ...
  - limite les lignes individuelles très longues à 4000 caractères pour la sécurité de l'affichage
...
- Pas de fichiers de verrou/informations de passerelle locale
...
- Les échecs d'authentification sont exposés avec des indications explicites sur le jeton

## Dépannage opérationnel (modes de défaillance actuels)

- **Outil Python non disponible**
  - Vérifier `python.toolMode` / `PI_PY`.
  - En cas d'échec du contrôle préalable, le runtime se dégrade en bash-only.
...
- **L'exécution se bloque puis expire**
  ...
  - Pour le code bloqué, l'annulation déclenche une interruption du noyau mais le code utilisateur peut nécessiter une refactorisation.

- **Invites stdin/input dans le code Python**
  - `input()` n'est pas pris en charge de manière interactive dans ce chemin de runtime ; passer les données de manière programmatique.
...
- `PI_PY` — substitution de l'exposition de l'outil (correspondance `bash-only`/`ipy-only`/`both` ci-dessus)
