...
1. `streamSimple()` (`packages/ai/src/stream.ts`) mappe les options génériques et dispatche vers une fonction de streaming du fournisseur.
2. Les fonctions de streaming des fournisseurs (`anthropic.ts`, `openai-responses.ts`, `google.ts`) traduisent les événements natifs du fournisseur en la séquence unifiée `AssistantMessageEvent`.
3. Chaque fournisseur pousse les événements dans `AssistantMessageEventStream` (`packages/ai/src/utils/event-stream.ts`), qui régule les événements delta et expose :
   - l'itération asynchrone pour les mises à jour incrémentales
...
5. `AgentSession` (`packages/coding-agent/src/session/agent-session.ts`) s'abonne aux événements de l'agent, persiste les messages, pilote les hooks d'extension et applique les comportements de session (réessai, compaction, TTSR, vérifications d'abandon de l'édition en streaming).

## Contrat de streaming unifié dans `@f5xc-salesdemos/pi-ai`
...
- les deltas en tampon sont vidés avant les événements non-delta et avant la complétion
...
- les deltas en tampon ne sont fusionnés que lorsque le **type + contentIndex** correspondent
- la fusion conserve le dernier instantané `partial`
- les événements non-delta forcent un vidage immédiat

Cela lisse les flux de fournisseurs à haute fréquence pour les consommateurs TUI/événements, mais ce n'est pas de la contre-pression fournisseur : les fournisseurs produisent toujours à pleine vitesse, tandis que le flux local met en tampon.
...
- `content_block_start` est mappé vers les débuts texte/réflexion/appel d'outil
- `content_block_delta` est mappé :
...
- les `arguments` sont re-analysés à chaque delta via `parseStreamingJson()`
- `toolcall_end` re-analyse une dernière fois, puis supprime `partialJson`
...
- les fournisseurs qui n'envoient que `response.function_call_arguments.done` remplissent tout de même les arguments finaux
...
- aucun analyseur JSON partiel nécessaire pour Google dans ce chemin
...
2. se rabattre sur l'analyseur `partial-json` pour les fragments incomplets
3. si les deux échouent, retourner `{}`

Implications :

- les deltas d'arguments malformés ou tronqués ne font pas planter le traitement du flux immédiatement
- les `arguments` en cours peuvent temporairement être `{}`
- des deltas valides ultérieurs peuvent récupérer les arguments structurés car l'analyse est retentée à chaque ajout
...
2. **Défaillance de transport/runtime** (exceptions réseau/client/analyseur/abandon)

Si le flux du fournisseur lance une exception ou signale une défaillance, chaque wrapper de fournisseur intercepte et émet un événement terminal `error` avec :
...
Pour ces chemins de fournisseurs, le cadrage chunk/SSE est géré par les flux SDK des fournisseurs (SDK Anthropic, SDK OpenAI, SDK Google). Ce code n'implémente pas de décodeur SSE personnalisé ici.

Comportement observé dans l'implémentation actuelle :

- une analyse chunk/SSE malformée au niveau du SDK se manifeste comme une exception ou un événement `error` du flux
- le wrapper du fournisseur convertit cela en événement terminal `error` unifié
- pas de reprise/réessai spécifique au fournisseur à l'intérieur de la fonction de streaming elle-même
- les réessais de niveau supérieur sont gérés par la logique de réessai automatique d'`AgentSession` (réessai au niveau du message, pas relecture de chunk de flux)

## Limites d'annulation

L'annulation est structurée en couches :

- Requête au fournisseur IA : `options.signal` est passé dans l'appel de flux du client fournisseur.
- Wrapper du fournisseur : après la boucle de flux, un signal d'abandon force le chemin d'erreur (`"Request was aborted"`).
...
L'annulation de l'exécution des outils est séparée de l'annulation du flux du modèle :

- les exécuteurs d'outils utilisent `AbortSignal.any([agentSignal, steeringAbortSignal])`
- les interruptions de pilotage peuvent abandonner l'exécution restante des outils tout en préservant les résultats d'outils déjà produits

## Limites de contre-pression

Il n'y a pas de mécanisme de contre-pression strict entre le flux SDK du fournisseur et les consommateurs en aval :

- `EventStream` utilise des files d'attente en mémoire sans taille maximale
- la régulation réduit le taux de mise à jour de l'interface mais ne ralentit pas l'ingestion du fournisseur
...
## Comment les événements de flux apparaissent comme événements agent/session

`agentLoop.streamAssistantResponse()` fait le pont entre `AssistantMessageEvent` et `AgentEvent` :

- sur `start` : pousse un message d'assistant de remplacement et émet `message_start`
...
- le garde-fou d'édition en streaming inspecte `toolcall_delta`/`toolcall_end` sur les appels `edit` et peut abandonner prématurément
- la persistance écrit les messages finalisés au `message_end`
- le réessai automatique examine `stopReason === "error"` de l'assistant plus les heuristiques d'`errorMessage`

## Responsabilités unifiées vs spécifiques au fournisseur

Unifiées (contrat commun) :

- forme de l'événement (`AssistantMessageEvent`)
...
- taxonomies d'événements en amont et logique de mapping
- tables de traduction des raisons d'arrêt
- conventions d'identifiants d'appel d'outil
- sémantique et signatures des blocs de raisonnement/réflexion
...
- [`../../ai/src/utils/event-stream.ts`](../../packages/ai/src/utils/event-stream.ts) — file d'attente de flux générique + régulation des deltas assistant.
- [`../../ai/src/utils/json-parse.ts`](../../packages/ai/src/utils/json-parse.ts) — analyse JSON partielle pour les arguments d'outils en streaming.
...
- [`../../agent/src/agent-loop.ts`](../../packages/agent/src/agent-loop.ts) — consommation du flux fournisseur et pont `message_update`.
