...
description: >-
  Format d'extension de manifeste Gemini pour la compatibilité inter-plateformes
  des compétences et agents.
sidebar:
  order: 7
...
Ce document couvre la manière dont le coding-agent découvre et analyse les extensions de manifeste de style Gemini (`gemini-extension.json`) dans la capacité `extensions`.
...
Le fournisseur de priorité supérieure l'emporte sur les noms d'extension en doublon.
...
Parce que la déduplication fonctionne en « premier vu l'emporte », l'ordre local des éléments du fournisseur est important.

- Le chargeur Gemini ajoute **l'utilisateur en premier**, puis le **projet**.
- Par conséquent, les noms en doublon entre `~/.gemini/extensions` et `<cwd>/.gemini/extensions` conservent l'entrée utilisateur et masquent l'entrée projet.

En revanche, le fournisseur native construit l'ordre des répertoires de configuration différemment (`project` puis `user` dans `getConfigDirs()`), donc le masquage intra-fournisseur du native va dans la direction opposée.
...
- Les noms en doublon au sein de la source Gemini se résolvent en faveur de l'utilisateur en premier.
- Les noms en doublon face à des fournisseurs de priorité supérieure (notamment native) perdent par priorité.
...
- Elles ne sont pas, en elles-mêmes, exécutées comme modules d'extension à l'exécution par le pipeline du chargeur d'extensions.
