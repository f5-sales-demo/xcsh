...
description: >-
  ...
  collections de plugins sélectionnées.
sidebar:
  order: 4
  label: Marketplace
i18n:
...
Le système de marketplace vous permet de découvrir, installer et gérer des plugins à partir de catalogues hébergés sur Git. Il est compatible avec le format de registre de plugins Claude Code.
...
Ou tapez simplement `/marketplace` sans arguments pour ouvrir le navigateur interactif de plugins.
...
Un **plugin** est un répertoire contenant des compétences, des commandes, des hooks, des serveurs MCP ou des serveurs LSP. Les plugins sont identifiés par `nom@marketplace` (par exemple `code-review@f5xc-salesdemos-marketplace`).

**Portées** : les plugins peuvent être installés à deux niveaux de portée :

- **utilisateur** (par défaut) -- disponible dans tous les projets, stocké dans `~/.xcsh/plugins/installed_plugins.json`
- **projet** -- disponible uniquement dans le projet en cours, stocké dans `.xcsh/installed_plugins.json`
...
| Commande | Effet |
|---|---|
| `/marketplace add <source>` | Ajouter une source de marketplace |
| `/marketplace remove <name>` | Supprimer un marketplace |
| `/marketplace update [name]` | Re-télécharger le(s) catalogue(s) ; omettre le nom pour tout mettre à jour |
...
| `/marketplace installed` | Lister les plugins de marketplace installés |
...
| `https://...*.json` | URL directe de catalogue | `https://example.com/marketplace.json` |
...
Un catalogue de marketplace se trouve à l'emplacement `.xcsh-plugin/marketplace.json` à la racine du dépôt :
...
| Champ | Description |
|---|---|
| `name` | Nom du marketplace. Alphanumérique en minuscules, tirets et points. Doit commencer et finir par un caractère alphanumérique. 64 caractères maximum. |
...
### Formats de source des plugins

Le champ `source` supporte plusieurs formats :
...
~/.xcsh/
  ...
  plugins/
    ...
    cache/
      marketplaces/            # Catalogues de marketplace en cache
      plugins/                 # Répertoires de plugins en cache
...
- Commencer et finir par une lettre minuscule ou un chiffre
- Contenir uniquement des lettres minuscules, des chiffres, des tirets et des points
- Faire au maximum 64 caractères

Les identifiants de plugin (`nom@marketplace`) doivent faire au maximum 128 caractères au total.
