...
description: >-
  Détails internes du gestionnaire de plugins couvrant l'installation, la
...
# Mécanismes internes du gestionnaire et de l'installateur de plugins

Ce document décrit comment les opérations `xcsh plugin` modifient l'état des plugins sur le disque et comment les plugins installés deviennent des capacités à l'exécution (outils aujourd'hui, résolution de chemins pour les hooks/commandes disponible).
...
- `src/commands/plugin.ts` définit les commandes/drapeaux et les transmet à `runPluginCommand`.
- `src/cli/plugin-cli.ts` associe les sous-commandes aux méthodes de `PluginManager` :
  - `install`, `uninstall`, `list`, `link`, `doctor`, `features`, `config`, `enable`, `disable`
- Aucune action `update` explicite n'existe ; la mise à jour s'effectue en relançant `install` avec un nouveau paquet/spécification de version.
...
Les surcharges locales au projet résident dans :
...
- `pkg[*]` -> active toutes les fonctionnalités du manifeste
- `pkg[]` -> n'active aucune fonctionnalité optionnelle
- `pkg[a,b]` -> active les fonctionnalités nommées
...
- Un paquet sans `xcsh`/`pi` reste installable et listable.
...
Un JSON `package.json` mal formé provoque une erreur fatale à la lecture ; une forme de manifeste mal structurée peut échouer plus tard uniquement lorsque des champs spécifiques sont consommés.
...
2. Valider le nom du paquet avec une regex + liste de refus de métacaractères shell.
...
6. Résoudre le manifeste et calculer `enabledFeatures` :
   ...
   - spécification nue : `null` (utilise la politique par défaut ultérieurement dans le chargeur)
...
Comme la mise à jour est pilotée par l'installation :

- `xcsh plugin install pkg@newVersion` met à jour la dépendance et la version du fichier de verrouillage.
- Les paramètres existants sont préservés ; l'entrée d'état est écrasée pour version/fonctionnalités/activé.
...
3. Charger les surcharges du projet (`<cwd>/.xcsh/plugin-overrides.json`, erreurs d'analyse/lecture -> objet vide avec avertissement).
...
Mise en garde : le `PluginManager.link` actuel n'applique pas la vérification de limite de chemin `cwd` présente dans l'ancien `installer.ts` (`normalizedPath.startsWith(normalizedCwd)`), la responsabilité de confiance incombe donc à l'appelant.
...
- ignoré si pas de package.json de plugin
- ignoré si le manifeste (`xcsh`/`pi`) est absent
- ignoré si désactivé globalement dans le fichier de verrouillage
- ignoré si désactivé par le projet
...
- liste explicite de fonctionnalités -> uniquement les fonctionnalités sélectionnées
- `enabledFeatures === null` -> active les fonctionnalités marquées `default: true`
...
## Détails de la gestion du verrou/état
...
- écrit le JSON complet du fichier de verrouillage avec mise en forme à chaque mutation
...
- Le code du plugin s'exécute dans le processus lorsque les modules d'outils personnalisés sont importés ; pas de sandboxing.
- Les chemins relatifs du manifeste sont joints au répertoire du paquet du plugin et font uniquement l'objet d'une vérification d'existence.
- Le paquet du plugin lui-même est du code de confiance une fois installé.

## Vérifications propres à l'installateur historique
...
- protections supplémentaires contre la traversée de nom de paquet/chemin pour le nommage de la cible du lien symbolique

Comme le CLI utilise `PluginManager`, ces protections de lien plus strictes ne sont pas actuellement sur le chemin principal.
...
| L'installation réussit, puis la validation du manifeste/fonctionnalités échoue | la commande échoue | Pas de rollback de désinstallation ; la dépendance peut rester dans `node_modules`/`package.json` |
| L'installation réussit, puis l'écriture du fichier de verrouillage échoue | la commande échoue | Pas de rollback du paquet installé |
| `bun uninstall` réussit, l'écriture du fichier de verrouillage échoue | la commande échoue | Paquet supprimé, un état d'exécution obsolète peut persister |
...
Opérationnellement, `doctor --fix` peut réparer certaines dérives (`bun install`, nettoyage de configuration orpheline, nettoyage de fonctionnalités invalides), mais c'est au mieux un effort.

## Résumé du comportement en cas de manifeste mal formé/manquant
...
- `--json` : formatage de sortie uniquement, aucun changement de comportement.
...
- [`src/commands/plugin.ts`](../../packages/coding-agent/src/commands/plugin.ts) — Déclaration de commande CLI et mappage des drapeaux
...
- [`src/extensibility/plugins/manager.ts`](../../packages/coding-agent/src/extensibility/plugins/manager.ts) — Implémentation active d'installation/suppression/listage/lien/état/diagnostic
- [`src/extensibility/plugins/installer.ts`](../../packages/coding-agent/src/extensibility/plugins/installer.ts) — Fonctions auxiliaires d'installation historiques et vérifications de sécurité supplémentaires pour les liens
...
- [`src/extensibility/plugins/parser.ts`](../../packages/coding-agent/src/extensibility/plugins/parser.ts) — Fonctions d'analyse des spécifications d'installation et des noms de paquets
