...
- des gestionnaires d'événements (`pi.on(...)`)
- des outils appelables par le LLM (`pi.registerTool(...)`)
- des commandes slash (`pi.registerCommand(...)`)
- des raccourcis clavier et des drapeaux
- un rendu personnalisé des messages
- des API d'injection de session/message (`sendMessage`, `sendUserMessage`, `appendEntry`)
...
3. `ExtensionRunner.initialize(...)` connecte les actions/contextes actifs pour le mode courant.
...
Contrainte importante provenant de `loader.ts` :

- appeler des méthodes d'action comme `pi.sendMessage()` pendant le chargement de l'extension lève une `ExtensionRuntimeNotInitializedError`
...
- `triggerTurn: true` — démarre un tour lorsque inactif (`nextTurn` ignore ceci)
...
Les gestionnaires et le `execute` des outils reçoivent `ctx` avec :
...
Les unions d'événements canoniques et les types de payload se trouvent dans `types.ts`.
...
- `user_bash` (surcharge avec `{ result }`)
- `user_python` (surcharge avec `{ result }`)

### `resources_discover`

`resources_discover` existe dans les types d'extension et dans `ExtensionRunner`.
Note sur le runtime actuel : `ExtensionRunner.emitResourcesDiscover(...)` est implémenté, mais il n'y a aucun point d'appel dans `AgentSession` qui l'invoque dans la base de code actuelle.
...
`tool_call`/`tool_result` interceptent tous les outils une fois que le registre est enveloppé dans `sdk.ts`, y compris les outils intégrés et les outils d'extension/personnalisés.
...
- notifications/statut/texte d'éditeur/entrée terminal/overlays personnalisés
...
À noter également : `setWidget` est actuellement routé vers le texte de la ligne de statut via `setHookWidget(...)`.

### Mode RPC (`rpc-mode.ts`)

`ctx.ui` est adossé aux événements RPC `extension_ui_request` :

- les méthodes de dialogue (`select`, `confirm`, `input`, `editor`) font un aller-retour vers les réponses client
...
- les contrôles d'expansion des outils sont inertes

### Chemins print/headless/sous-agent
...
Le mode arrière-plan installe un objet de contexte UI non interactif. Dans l'implémentation actuelle, `ctx.hasUI` peut toujours être `true` tandis que les dialogues interactifs retournent des valeurs par défaut/un comportement sans effet.
...
3. Gardez les `details` des résultats d'outils structurés lorsque l'état doit être visible/reconstructible depuis l'historique des résultats d'outils.
...
## Rendu personnalisé de messages
...
- Les actions runtime ne sont pas disponibles pendant le chargement de l'extension.
...
- Traitez `ctx.reload()` comme terminal pour le cadre d'exécution du gestionnaire de commande courant.
...
- **Custom-tools** (`src/extensibility/custom-tools/*`) : modules centrés sur les outils ; lorsqu'ils sont chargés aux côtés des extensions, ils sont adaptés et passent toujours par les wrappers d'interception d'extension.

Si vous avez besoin d'un seul package qui gère à la fois les politiques, les outils, l'UX des commandes et le rendu, utilisez les extensions.
