...
### Comportement de dédoublonnage pré-enregistrement
...
Les déclencheurs regex invalides sont journalisés en tant qu'avertissements et ignorés ; le démarrage de la session se poursuit.

### Mise en garde sur les paramètres

`TtsrSettings.enabled` est chargé dans le gestionnaire mais n'est actuellement pas vérifié dans le contrôle d'exécution. Si des règles existent, la correspondance s'exécute quand même.
...
L'abandon n'est pas bloqué par les callbacks d'extension.
...
- `keep` : la sortie partielle de l'assistant reste dans l'état de conversation ; le rappel est ajouté après celle-ci.
...
`messageCount` s'incrémente lors du `turn_end`, donc l'intervalle est mesuré en tours complétés, pas en fragments de flux.
...
Le mode interactif utilise `session.isTtsrAbortPending` pour supprimer l'affichage de la raison d'arrêt de l'assistant abandonné en tant qu'échec visible lors de l'interruption TTSR, et affiche un `TtsrNotificationComponent` lorsque l'événement arrive.

## 7. Persistance et état de reprise (implémentation actuelle)

`SessionManager` dispose d'un support complet de schéma pour la persistance des règles injectées :
...
`TtsrManager` supporte également la restauration via `restoreInjected(ruleNames)`.
...
- `createAgentSession()` ne restaure pas `existingSession.injectedTtsrRules` dans le `ttsrManager`.
...
- la relance est différée par un minuteur (`50ms`)
...
`check()` retourne toutes les règles éligibles correspondant actuellement. Elles sont injectées en lot dans le prochain message de relance.

### Entre l'abandon et la continuation

Pendant la fenêtre du minuteur, l'état peut changer (interruption utilisateur, actions de mode, événements supplémentaires). L'appel de relance est en mode best-effort : `agent.continue().catch(() => {})` absorbe les erreurs de suivi.

## 9. Résumé des cas limites

- Regex `ttsr_trigger` invalide : ignorée avec avertissement ; les autres règles continuent.
- Noms de règles en doublon au niveau de la couche de capacité : les doublons de priorité inférieure sont masqués avant l'enregistrement.
- Noms en doublon au niveau du gestionnaire : le second enregistrement est ignoré.
- `contextMode: "keep"` : la sortie partielle en violation peut rester dans le contexte avant la relance avec rappel.
- L'intervalle repeat-after-gap dépend des incréments de compteur de tours au `turn_end` ; les fragments en cours de tour n'avancent pas les compteurs d'intervalle.
