...
La session est stockée sous forme de journal d'entrées en ajout uniquement, mais le comportement à l'exécution est basé sur un arbre :
...
- Le branchement ne **réécrit pas** l'historique ; il change uniquement l'endroit où pointe la feuille avant le prochain ajout.

Fichiers clés :

- `src/session/session-manager.ts` — modèle de données de l'arbre, parcours, déplacement des feuilles, extraction de branches/sessions
- `src/session/agent-session.ts` — flux de navigation `/tree`, résumé, émission de hooks/événements
- `src/modes/components/tree-selector.ts` — comportement interactif de l'interface arbre et filtrage
- `src/modes/controllers/selector-controller.ts` — orchestration du sélecteur pour `/tree` et `/branch`
- `src/modes/controllers/input-controller.ts` — routage des commandes (`/tree`, `/branch`, comportement double-Échap)
...
`getTree()` est une projection à l'exécution ; la persistance reste sous forme d'entrées JSONL en ajout uniquement.
...
`AgentSession.navigateTree()` est une navigation, pas un fork de fichier.
...
3. Optionnellement résumer les entrées abandonnées (résumé fourni par le hook ou résumeur intégré)
...
Important : les entrées de résumé sont attachées à la **nouvelle position de navigation**, pas à la queue de la branche abandonnée.
...
- Si le message utilisateur sélectionné est une racine (`parentId === null`) : démarrer une nouvelle session via `newSession({ parentSession: previousSessionFile })`.
...
- Construit le chemin racine→feuille via `getBranch(leafId)` ; lance une erreur si absent.
- Exclut les entrées `label` existantes du chemin copié.
- Reconstruit des entrées d'étiquettes fraîches à partir des `labelsById` résolus pour les entrées qui restent dans le chemin.
...
- `branchSummary` et `compactionSummary` deviennent des messages de contexte avec rôle utilisateur et template
- `custom`/`hookMessage` deviennent des messages de contenu avec rôle utilisateur

Ainsi, le déplacement dans l'arbre modifie le contexte en changeant le chemin actif de la feuille, pas en mutant les anciennes entrées.

## Étiquettes et comportement de l'interface arbre
...
- Prend en charge les modes de filtre : `default`, `no-tools`, `user-only`, `labeled-only`, `all`.
- Prend en charge la recherche textuelle libre sur le contenu sémantique rendu.
...
## Points de contact des extensions et hooks pour les opérations d'arbre
...
- `session_before_compact`, `session.compacting`, `session_compact` pour les entrées de compaction qui affectent ultérieurement la reconstruction du contexte de l'arbre

## Contraintes réelles et conditions limites

- `branch()` ne peut pas cibler `null` ; utilisez `resetLeaf()` pour l'état racine avant la première entrée.
- `branchWithSummary()` prend en charge une cible `null` et enregistre `fromId: "root"`.
- Sélectionner la feuille courante dans le sélecteur d'arbre est une opération sans effet.
- Le résumé nécessite un modèle actif ; s'il est absent, la navigation avec résumé échoue immédiatement.
- Si le résumé est annulé, la navigation est annulée et la feuille reste inchangée.
...
Le comportement à l'exécution actuel correspond à la sémantique d'arbre version 3 après migration.
