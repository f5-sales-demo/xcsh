---
title: Stockage de session et modèle d'entrées
description: >-
  Append-only session storage model with entry types, persistence, and migration
  between formats.
sidebar:
  order: 1
  label: Stockage et modèle d'entrées
i18n:
  sourceHash: 42fe17549e00
  translator: machine
---

# Stockage de session et modèle d'entrées

Ce document constitue la source de vérité concernant la manière dont les sessions de l'agent de codage sont représentées, persistées, migrées et reconstruites à l'exécution.
...
Ne couvre pas le comportement de rendu de l'interface `/tree` au-delà de la sémantique affectant les données de session.
...
Le contenu du fil d'Ariane est composé de deux lignes : le répertoire de travail original, puis le chemin du fichier de session. `continueRecent()` privilégie ce pointeur limité au terminal avant de rechercher le mtime le plus récent.
...
- Les entrées sont en mode append-only à l'exécution ; la navigation entre branches déplace un pointeur (`leafId`) plutôt que de modifier les entrées existantes.
...
Si le branchement part de la racine (`branchFromId === null`), `fromId` est la chaîne littérale `"root"`.
...
Appliqué quand la `version` de l'en-tête est absente ou `< 2` :
...
## Sémantique de l'arbre et de la feuille

Le modèle sous-jacent est un arbre en append-only + un pointeur de feuille mutable :
...
Les entrées `custom` et `session_init` n'injectent pas directement de contexte au modèle.
...
- La persistance est différée jusqu'à ce qu'au moins un message assistant existe.
  - Avant le premier assistant : les entrées sont conservées en mémoire ; aucune écriture fichier n'a lieu.
  - Quand un premier assistant existe : la session complète en mémoire est vidée vers le fichier.
  - Ensuite : les nouvelles entrées sont ajoutées de manière incrémentale.

Justification dans le code : éviter de persister les sessions qui n'ont jamais produit de réponse assistant.

### Opérations de durabilité

- `flush()` vide le writer et appelle `fsync()`.
- Les réécritures atomiques complètes (`#rewriteFile`) écrivent dans un fichier temporaire, flush+fsync, ferment, puis renomment par-dessus la cible.
...
- La fermeture du writer est en mode best-effort mais propage la première erreur significative.

## Contrôles de taille des données et externalisation des blobs

Avant de persister les entrées :

- Les chaînes volumineuses sont tronquées à `MAX_PERSIST_CHARS` (500 000 caractères) avec un avis :
  - `"[Session persistence truncated large content]"`
- Les champs transitoires `partialJson` et `jsonlEvents` sont supprimés.
- Si l'objet possède à la fois `content` et `lineCount`, le nombre de lignes est recalculé après la troncature.
- Les blocs d'images dans les tableaux `content` avec une longueur base64 >= 1024 sont externalisés en références blob :
  - stockés sous la forme `blob:sha256:<hash>`
  - les octets bruts sont écrits dans le magasin de blobs (`BlobStore.put`)

Au chargement, les références blob sont résolues en base64 pour les blocs d'images des entrées message/custom_message.
...
- `list(cwd, sessionDir?)` -> sessions dans la portée d'un projet
- `listAll()` -> sessions à travers toutes les portées de projets sous `~/.xcsh/agent/sessions`

L'extraction des métadonnées ne lit qu'un préfixe (`readTextPrefix(..., 4096)`) lorsque possible.

## Lié mais distinct : stockage de l'historique des prompts
...
- Index FTS5 : `history_fts` avec synchronisation maintenue par trigger
...
Utilisez les fichiers de session pour la relecture du graphe/état de conversation ; utilisez `HistoryStorage` pour l'expérience utilisateur de l'historique des prompts.
