...
description: >-
  Mécaniques de changement de session et liste des sessions récentes avec
...
Il se concentre sur le comportement de l'implémentation actuelle, y compris les chemins de secours et les mises en garde.
...
`SessionManager` stocke les sessions dans un répertoire scopé par le répertoire courant (cwd) par défaut :
...
### Deux chemins de listing avec des données différentes

Il existe deux pipelines de listing différents :
...
1. `getRecentSessions(sessionDir, limit)` (vue d'accueil/résumé)
   ...
   - Parse l'en-tête + un aperçu du premier texte utilisateur.
   - Retourne des `RecentSessionInfo` légers avec des getters paresseux pour `name` et `timeAgo`.
...
2. `SessionManager.list(...)` / `SessionManager.listAll()` (sélecteurs de reprise et correspondance par ID)
   ...
   - Exclut les sessions avec zéro entrées de type `message`.
   - Trie par `modified` en ordre décroissant.

### Comportement de secours pour les métadonnées
...
1. Lire le fil d'Ariane scopé par terminal (`~/.xcsh/agent/terminal-sessions/<terminal-id>`)
2. Valider le fil d'Ariane :
   - le terminal actuel peut être identifié
   - le cwd du fil d'Ariane correspond au cwd actuel (comparaison de chemin résolu)
   - le fichier référencé existe toujours
3. Si le fil d'Ariane est invalide/absent, se rabattre sur le fichier le plus récent par mtime dans le répertoire de session (`findMostRecentSession`)
4. Si aucun n'est trouvé, créer une nouvelle session

La dérivation de l'ID du terminal préfère le chemin TTY et se rabat sur des identifiants basés sur l'environnement (`KITTY_WINDOW_ID`, `TMUX_PANE`, `TERM_SESSION_ID`, `WT_SESSION`).

Les écritures de fil d'Ariane sont au mieux effort et non fatales.

## Résolution de la cible de reprise au démarrage (`main.ts`)

### `--resume <value>`

`createSessionManager(...)` gère les valeurs `--resume` de type chaîne de deux manières :

1. Valeur de type chemin (contient `/`, `\\`, ou se termine par `.jsonl`)
   - `SessionManager.open(sessionArg, parsed.sessionDir)` directement

2. Valeur de préfixe d'ID
   - cherche une correspondance dans `SessionManager.list(cwd, sessionDir)` par `id.startsWith(sessionArg)`
   - si aucune correspondance locale et que `sessionDir` n'est pas forcé, essaie `SessionManager.listAll()`
   - la première correspondance est utilisée (pas de prompt d'ambiguïté)

Comportement de correspondance inter-projets :

- si le cwd de la session trouvée diffère du cwd actuel, le CLI demande s'il faut forker dans le projet actuel
- oui -> `SessionManager.forkFrom(...)`
- non -> lève une erreur (`Session "..." is in another project (...)`)

Aucune correspondance -> lève une erreur (`Session "..." not found.`).
...
Utilise directement `SessionManager.continueRecent(...)` (comportement fil d'Ariane en priorité décrit ci-dessus).

## Détails internes de la sélection par sélecteur
...
2. monter `SessionSelectorComponent` dans la zone d'édition en utilisant `showSelector(...)`
3. callbacks :
...
- Échap pour annuler
- Ctrl+C pour quitter
- recherche floue sur l'id/titre/cwd/premier message/tous les messages/chemin de la session
...
Mise en garde : le texte de l'interface indique `Press Tab to view all`, mais ce composant n'a actuellement pas de gestionnaire Tab et le câblage actuel ne liste que les sessions du scope actuel.

## Exécution du changement en cours d'exécution (`AgentSession.switchSession`)

`switchSession(sessionPath)` est le chemin de changement principal en cours de processus.
...
4. se déconnecter du flux d'événements de l'agent actuel
5. abandonner la génération/le flux d'outils actifs
6. vider les tampons de messages de pilotage/suivi/tour suivant en file d'attente
...
8. `sessionManager.setSessionFile(sessionPath)`
   ...
   - si les données du fichier sont manquantes/invalides : initialise une nouvelle session à ce chemin et réécrit l'en-tête
...
14. restaurer le niveau de réflexion :
    ...
    - sinon dériver le niveau de réflexion par défaut depuis les paramètres, limiter à la capacité du modèle, le définir et ajouter une nouvelle entrée `thinking_level_change`
...
- vider l'interface de message en attente et la carte d'outils en attente
...
Ainsi, l'état visible de la conversation/todos est reconstruit à partir du nouveau fichier de session.
...
- Le modèle/réflexion sont sélectionnés pendant la création (incluant la logique de restauration/secours).
...
### Changement en session (chemin du sélecteur de type `/resume`)
...
- Aucun appel dédié de restauration de mode post-changement n'est effectué dans le flux du sélecteur ; le comportement de ré-entrée de mode n'est pas symétrique avec le `#restoreModeFromSession()` du démarrage.
...
Lors de l'ouverture/changement vers un chemin spécifique (`setSessionFile`) :

- ENOENT -> traité comme vide -> une nouvelle session est initialisée à ce chemin exact et persistée.
- en-tête malformé/invalide (ou entrées parsées effectivement illisibles) -> traité comme vide -> une nouvelle session est initialisée et persistée.

Il s'agit d'un comportement de récupération, pas d'un échec dur.

### Échecs durs

Le changement/ouverture peut toujours lever une exception en cas de véritables échecs d'E/S (erreurs de permissions, échecs de réécriture, etc.), qui se propagent aux appelants.
