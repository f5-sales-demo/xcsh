...
La compaction et les résumés de branches sont les deux mécanismes qui maintiennent les longues sessions utilisables sans perdre le contexte des travaux précédents.
...
La compaction et les résumés de branches sont des entrées de session à part entière, et non de simples messages assistant/utilisateur.
...
Lorsque le contexte est reconstruit (`buildSessionContext`) :
...
### Compaction par récupération de dépassement vs compaction par seuil

Les deux chemins automatiques sont intentionnellement différents :

- **Compaction par récupération de dépassement**
...
- Exiger au moins `20_000` tokens d'économies totales estimées.
...
S'il y a des entrées de métadonnées non-message immédiatement avant le point de coupure (`model_change`, `thinking_level_change`, labels, etc.), elles sont intégrées dans la région conservée en reculant l'index de coupure jusqu'à atteindre un message ou une frontière de compaction.
...
5. Optionnellement injecter le contexte de hook en tant que liste `<additional-context>`.
...
- résumé court pour l'UI : `compaction-short-summary.md`

Mode de résumé distant :

- Si `compaction.remoteEndpoint` est défini, la compaction envoie en POST :
  - `{ systemPrompt, prompt }`
- Attend un JSON contenant au moins `{ summary }`.

### Contexte des opérations fichier dans les résumés

La compaction suit l'activité cumulée sur les fichiers via les appels d'outils assistant :
...
- Inclut les détails de compaction précédente uniquement lorsque l'entrée précédente est générée par pi (`fromExtension !== true`).
- Dans les tours fractionnés, inclut également les opérations fichier du préfixe de tour.
- `readFiles` exclut les fichiers également modifiés.

Le texte du résumé reçoit des balises de fichier ajoutées via le modèle de prompt :
...
Après la génération du résumé (ou le résumé fourni par un hook), la session agent :
...
Opérationnellement, cela est généralement piloté par le flux `/tree` lorsque `branchSummary.enabled` est activé.
...
1. Première passe : collecter les opérations fichier cumulées de toutes les entrées résumées, y compris les détails `branch_summary` précédents générés par pi.
2. Deuxième passe : parcourir du plus récent au plus ancien, ajoutant des messages jusqu'à atteindre le budget de tokens.
3. Préférer la conservation du contexte récent.
4. Peut tout de même inclure de grandes entrées de résumé proches de la limite du budget pour la continuité.

Les entrées de compaction sont incluses en tant que messages (`compactionSummary`) dans l'entrée de résumé de branche.
...
- La compaction automatique émet des événements de session de début/fin pour les mises à jour UI/état.
