...
| Opération | Point d'entrée | Mutation de session | Création/changement de fichier de session | Artefact de sortie |
|---|---|---|---|---|
...
| `--resume` | Démarrage CLI (sélecteur) | Oui après la création de session | Ouvre un fichier de session existant sélectionné | Aucun |
| `--resume <id\|path>` | Démarrage CLI | Oui après la création de session | Ouvre une session existante ; le cas inter-projets peut créer un fork dans le projet courant | Aucun |
...
2. La commande découpe sur les espaces blancs et utilise uniquement le premier argument après `/export` comme `outputPath`.
...
- L'export intègre l'en-tête/les entrées/la feuille de session ainsi que le `systemPrompt` actuel et les descriptions d'outils depuis l'état de l'agent.
- Aucune entrée de session n'est ajoutée pendant l'export.

Mise en garde :

- L'analyse des arguments est basée sur les espaces blancs (`text.split(/\s+/)`), donc les chemins entre guillemets contenant des espaces ne sont pas préservés comme un chemin unique par ce parcours de commande.
...
- Ce parcours ne crée pas d'`AgentSession` et ne modifie aucune session en cours d'exécution.
...
`/share` est exclusivement interactif et commence toujours par exporter la session courante vers un fichier HTML temporaire.
...
- Interprétation du résultat du gestionnaire :
  ...
  - objet => `url` et/ou `message` affichés ; `url` ouvert
  - `undefined`/faleur fausse => générique `Session shared`
- Le fichier temporaire est supprimé après complétion.

Comportement de repli critique :

- Si le gestionnaire personnalisé existe mais que le chargement échoue, la commande signale une erreur et retourne.
- Si le gestionnaire personnalisé s'exécute et lève une exception, la commande signale une erreur et retourne.
- Dans les deux cas d'échec, il **ne** se rabat **pas** sur le gist GitHub.
- Le repli vers le gist n'intervient que lorsqu'aucun script de partage personnalisé n'existe.
...
4. Analyse l'URL du gist, dérive l'identifiant du gist, construit l'URL de prévisualisation `https://gistpreview.github.io/?<id>`.
...
- La commande sous-jacente `gh gist create` ne reçoit pas de signal d'abandon dans ce parcours de code ; l'annulation est au niveau de l'interface et vérifiée après le retour de la commande.
...
4. Copie le répertoire d'artefacts de l'ancien espace de noms de session vers le nouveau (au mieux ; les échecs de copie non-ENOENT sont journalisés, non fatals).
...
Comportement de correspondance d'identifiant inter-projets :

- Si le cwd de la session correspondante diffère du cwd courant, le CLI demande :
  - `Session found in different project ... Fork into current directory? [y/N]`
- Si oui : `SessionManager.forkFrom(match.path, cwd, sessionDir)` crée un nouveau fichier local forké.
- Si non/défaut non-TTY : la commande signale une erreur.
...
C'est un comportement au démarrage uniquement ; il n'existe pas de commande slash interactive `/continue`.

## Comment le changement de session modifie réellement l'état d'exécution
...
Les ponts SDK transmettent les événements de session des extensions aux callbacks `onSession` des outils personnalisés :
...
- `/fork` est bloqué pendant le streaming (l'utilisateur doit attendre/abandonner la réponse en cours d'abord).
- Le sélecteur `/resume` peut être annulé par l'utilisateur en fermant le sélecteur.
- `--resume <id>` inter-projets peut être annulé en déclinant l'invite de fork.
- `/share` dispose d'un chemin d'abandon dans l'interface (`Share cancelled`) pour le flux gist ; il ne connecte pas de sémantique de terminaison de processus pour `gh gist create` dans ce parcours de code.
...
- Les sémantiques CLI de reprise/continuation sont contournées si `--no-session` est défini, car la création du gestionnaire retourne immédiatement en mémoire.

## Mises en garde d'implémentation connues (en l'état du code actuel)

- `SelectorController.handleResumeSession()` ne vérifie pas le résultat booléen de `session.switchSession(...)` ; un changement annulé par un hook peut tout de même poursuivre le parcours de rafraîchissement/statut « Resumed session » de l'interface.
- Les échecs de `/share` avec partage personnalisé ne se dégradent pas vers le repli gist par défaut ; ils terminent la commande avec une erreur.
