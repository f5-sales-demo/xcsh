...
- Écriture de la configuration : `packages/coding-agent/src/mcp/config-writer.ts`
...
OMP peut découvrir les serveurs MCP à partir de plusieurs outils (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, et d'autres), mais pour la configuration native OMP, vous devriez généralement utiliser l'un de ces fichiers :

- Projet : `.xcsh/mcp.json`
- Utilisateur : `~/.xcsh/mcp.json`

OMP accepte également des fichiers autonomes de secours à la racine du projet :

- `mcp.json`
- `.mcp.json`

Utilisez `.xcsh/mcp.json` lorsque vous souhaitez qu'OMP soit propriétaire de la configuration. Utilisez `mcp.json` / `.mcp.json` à la racine uniquement lorsque vous souhaitez un fichier de secours portable que d'autres clients MCP peuvent également lire.
...
- `oauth?: { ... }` — paramètres client OAuth explicites utilisés lors de l'authentification/ré-authentification
...
Ceci correspond au point de terminaison hébergé du serveur MCP GitHub.
...
`sse` est toujours pris en charge pour la compatibilité, mais la spécification MCP préfère désormais le HTTP Streamable (`type: "http"`) pour les nouveaux serveurs.
...
Avant qu'OMP ne lance un serveur ou n'effectue une requête HTTP, il résout les valeurs `env` et `headers` de la manière suivante :
...
Le chargeur autonome de secours développe également `${VAR}` et `${VAR:-default}` dans les chaînes lors de la découverte.
...
Si vous souhaitez le comportement OMP le moins surprenant, préférez `.xcsh/mcp.json` et utilisez des valeurs env/headers explicites.
...
- Si vous collez une configuration de serveur distant et oubliez `"type": "http"`, OMP le traitera comme `stdio` et signalera que `command` est manquant
...
- gardez les noms de serveur uniques entre les outils dans la mesure du possible
