...
description: >-
  Configuration, validation et gestion des serveurs MCP pour l'environnement
  d'exécution de l'agent de codage.
sidebar:
  order: 1
  label: Configuration
i18n:
...
Ce guide explique comment ajouter, modifier et valider les serveurs MCP pour l'agent de codage OMP.

Source de vérité dans le code :

- Types de configuration d'exécution : `packages/coding-agent/src/mcp/types.ts`
...
OMP peut découvrir les serveurs MCP depuis plusieurs outils (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, et d'autres), mais pour la configuration native d'OMP, vous devriez généralement utiliser l'un de ces fichiers :
...
- `$schema` — URL de schéma JSON optionnelle pour l'outillage
- `mcpServers` — correspondance entre le nom du serveur et la configuration du serveur
...
Ceci suit le package officiel du serveur MCP Filesystem (`@modelcontextprotocol/server-filesystem`).
...
Ceci correspond au point de terminaison hébergé du serveur MCP GitHub de GitHub.
...
`sse` est encore pris en charge pour la compatibilité, mais la spécification MCP recommande désormais Streamable HTTP (`type: "http"`) pour les nouveaux serveurs.
...
Utilisez ceci lorsqu'OMP doit se souvenir comment réhydrater les identifiants pour un serveur.
...
Slack est l'exemple actuel le plus clair. Le serveur MCP de Slack est hébergé à `https://mcp.slack.com/mcp`, utilise Streamable HTTP et nécessite un OAuth confidentiel avec les identifiants client de votre application Slack.
...
Points de terminaison Slack pertinents issus de la documentation de Slack :
...
Cela signifie que ceci est valide et pratique pour les secrets locaux :

- `"GITHUB_PERSONAL_ACCESS_TOKEN": "GITHUB_PERSONAL_ACCESS_TOKEN"` → copie depuis l'environnement shell actuel
...
Le chargeur autonome de repli développe également `${VAR}` et `${VAR:-default}` dans les chaînes lors de la découverte.
...
Si vous souhaitez le comportement OMP le moins surprenant, préférez `.xcsh/mcp.json` et utilisez des valeurs explicites d'env/headers.
...
- `/mcp reload` pour redécouvrir et reconnecter les serveurs dans la session actuelle
...
OMP ne fusionne pas les définitions de serveur en double entre les fichiers. Les fournisseurs de découverte sont priorisés, et la définition de priorité supérieure l'emporte.

En pratique :

- préférez `.xcsh/mcp.json` ou `~/.xcsh/mcp.json` lorsque vous souhaitez un remplacement spécifique à OMP
- gardez les noms de serveurs uniques entre les outils lorsque c'est possible
- utilisez `disabledServers` dans la configuration utilisateur lorsqu'une configuration tierce réintroduit sans cesse un serveur que vous ne souhaitez pas
