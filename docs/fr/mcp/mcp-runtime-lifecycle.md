...
description: >-
  Cycle de vie du processus serveur MCP depuis l'initialisation jusqu'à
...
5. **Le câblage SDK** fusionne les outils MCP dans le registre d'outils du runtime pour la session.
6. **La session active** peut rafraîchir les outils MCP via les flux `/mcp` (`disconnectAll` + redécouverte + `session.refreshMCPTools`).
7. **L'arrêt** se produit lorsque les appelants invoquent `disconnectServer`/`disconnectAll` ; le gestionnaire efface également les enregistrements d'outils MCP pour les serveurs déconnectés.
...
Le résultat inclut à la fois les `configs` et les `sources` (métadonnées utilisées ultérieurement pour l'étiquetage du fournisseur).

### Comportement en cas d'échec au niveau de la découverte

`discoverAndLoadMCPTools()` distingue deux classes d'échec :

- **Échec critique de la découverte** (exception de `manager.discoverAndConnect`, typiquement provenant de la découverte de configuration) : retourne un ensemble d'outils vide et une erreur synthétique `{ path: ".mcp.json", error }`.
...
- `#pendingConnections: Map<string, Promise<MCPServerConnection>>` — poignée de main en cours.
- `#pendingToolLoads: Map<string, Promise<{ connection, serverTools }>>` — connectés mais outils encore en cours de chargement.
...
7. mettre en cache les définitions d'outils (`MCPToolCache.set`) en mode best-effort.
...
- ferme le transport en cas d'échec d'initialisation.
...
- les tâches encore en attente :
  - utilisent les définitions d'outils en cache si disponibles (`MCPToolCache.get`) pour créer des `DeferredMCPTool`,
  - sinon bloquent jusqu'à ce que ces tâches en attente soient résolues.

Il s'agit d'un modèle de démarrage hybride : retour rapide lorsque le cache est disponible, attente de correction lorsqu'il ne l'est pas.

### Comportement de complétion en arrière-plan

Chaque `toolsPromise` en attente possède également une continuation en arrière-plan qui finalement :

- remplace la tranche d'outils de ce serveur dans l'état du gestionnaire via `#replaceServerTools`,
- écrit dans le cache,
...
- `DeferredMCPTool` attend `waitForConnection(server)` avant l'appel ; cela permet aux outils mis en cache d'exister avant que la connexion ne soit prête.
...
Il existe également un chemin de suivi pour les connexions tardives : après avoir attendu un serveur spécifique, si le statut devient `connected`, il relance `session.refreshMCPTools(...)` afin que les outils nouvellement disponibles soient reliés en session.

## Santé, reconnexion et comportement en cas d'échec partiel

Le comportement actuel à l'exécution est intentionnellement minimal :
...
- le cache d'outils et les mises à jour en arrière-plan sont en mode best-effort (avertissements/erreurs journalisés, pas d'arrêt brutal).
...
Dans le câblage actuel, l'arrêt explicite est utilisé dans les flux de commande MCP (pour rechargement/suppression/désactivation). Il n'y a pas de hook de disposition automatique distinct du gestionnaire dans le chemin de démarrage lui-même ; les appelants sont responsables de l'invocation des méthodes de déconnexion du gestionnaire lorsqu'ils nécessitent un arrêt MCP déterministe.

## Modes d'échec et garanties

| Scénario | Comportement | Échec critique vs best-effort |
| --- | --- | --- |
| La découverte lève une exception (chemin de chargement capacité/config) | Le chargeur retourne des outils vides + erreur synthétique `.mcp.json` | Démarrage de session en mode best-effort |
| Configuration de serveur invalide | Serveur ignoré avec entrée d'erreur de validation | Best-effort par serveur |
| Délai d'expiration de connexion/échec d'initialisation | Erreur du serveur enregistrée ; les autres continuent | Best-effort par serveur |
| `tools/list` encore en attente au démarrage avec correspondance dans le cache | Outils différés retournés immédiatement | Démarrage rapide en mode best-effort |
...
| Échec tardif de chargement d'outils en arrière-plan | Journalisé après la porte de démarrage | Journalisation en mode best-effort |
| Transport interrompu à l'exécution | Pas de reconnexion automatique ; les appels futurs échouent jusqu'à reconnexion/rechargement | Récupération en mode best-effort via action manuelle |
...
- [`src/mcp/client.ts`](../../packages/coding-agent/src/mcp/client.ts) — configuration du transport, poignée de main d'initialisation, listage/appel/déconnexion.
- [`src/mcp/index.ts`](../../packages/coding-agent/src/mcp/index.ts) — exports de l'API du module MCP.
- [`src/sdk.ts`](../../packages/coding-agent/src/sdk.ts) — câblage du démarrage dans la session/le registre d'outils.
...
- [`src/session/agent-session.ts`](../../packages/coding-agent/src/session/agent-session.ts) — reliage en direct via `refreshMCPTools`.
- [`src/modes/controllers/mcp-command-controller.ts`](../../packages/coding-agent/src/modes/controllers/mcp-command-controller.ts) — flux de rechargement/reconnexion interactifs.
