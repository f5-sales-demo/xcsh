...
description: >-
  Contrat du cache de scan du système de fichiers pour une découverte rapide des
  fichiers avec une sémantique stale-while-revalidate.
sidebar:
  order: 8
...
Le cache stocke des listes complètes d'entrées de scan de répertoires (`GlobMatch[]`) indexées par portée de scan et politique de traversée, puis permet aux opérations de niveau supérieur (filtrage par glob, scoring flou, sélection de fichiers grep) de s'exécuter sur ces entrées mises en cache.
...
- permettre la récupération explicite de données obsolètes pour les résultats vides et l'invalidation explicite après des mutations de fichiers
...
- Les scans respectant gitignore et ceux désactivant l'ignore ne partagent **pas** d'entrées.
...
Politique globale (configurable par variables d'environnement) :
...
- `get_or_scan(...)`
  - si le TTL est `0` : contourner entièrement le cache, toujours un scan frais (`cache_age_ms = 0`)
  - lors d'un cache hit dans le TTL : retourner les entrées en cache + `cache_age_ms` non nul
  - lors d'un hit expiré : évincer la clé, rescanner, stocker une entrée fraîche
- l'application du nombre maximum d'entrées se fait par éviction du plus ancien selon `created_at`

## Revérification rapide des résultats vides (séparée des hits normaux)

Cache hit normal :

- un cache hit dans le TTL retourne les entrées en cache et ne fait rien d'autre.
...
- `glob` : revérifie lorsque les correspondances filtrées sont vides et que l'âge du scan dépasse le seuil
- `fuzzyFind` (`fd.rs`) : revérifie uniquement lorsque la requête est non vide et que les correspondances scorées sont vides
- `grep` : revérifie lorsque la liste de fichiers candidats sélectionnés est vide

## Valeurs par défaut des consommateurs et utilisation du cache

Le cache est opt-in sur toutes les API exposées (`cache?: boolean`, défaut `false`).
...
- L'intégration `grep` au niveau des outils désactive actuellement le cache de scan (`cache: false`) :
...
- `invalidateFsScanCache(path?: string)`
  - avec `path` : supprimer les entrées du cache dont la racine est un préfixe du chemin cible
  - sans path : vider toutes les entrées du cache de scan
...
- si la cible n'existe pas (ex. suppression), le fallback canonicalise le parent et réattache le nom de fichier lorsque possible
- cela préserve le comportement d'invalidation pour les créations/suppressions/renommages où un des côtés peut ne pas exister
...
3. **Implémenter la revérification rapide des résultats vides uniquement pour le risque de faux négatifs obsolètes**
   - utiliser `scan.cache_age_ms >= empty_recheck_ms()`
   - réessayer une fois avec `force_rescan(..., store=true, ...)`
   - garder ce chemin séparé de la logique de cache hit normale
...
5. **Connecter l'invalidation de mutation pour tout nouveau chemin d'écriture**
...
6. **Ne pas ajouter de paramètres TTL par appel**
   - le contrat actuel est une politique globale uniquement (configurée par variables d'environnement), pas de surcharge de TTL par requête
