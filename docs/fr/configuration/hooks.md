...
Ce fichier documente donc l'implémentation du sous-système de hooks en elle-même (types/loader/runner/wrapper), y compris le comportement legacy et les contraintes.
...
- `src/extensibility/hooks/runner.ts` — dispatch d'événements, recherche de commandes, signalement d'erreurs
...
`loader.ts` résout les chemins de hooks ainsi :

- chemin absolu : utilisé tel quel
- chemin `~` : développé
- chemin relatif : résolu par rapport à `cwd`

### Incompatibilité legacy importante

Les fournisseurs de découverte pour `hookCapability` modélisent encore les fichiers de hooks pré/post de type shell (par exemple `.claude/hooks/pre/*`, `.xcsh/.../hooks/pre/*`).

Le chargeur de hooks ici utilise l'import dynamique de modules et requiert une factory de hook JS/TS par défaut. Si un chemin de hook découvert n'est pas importable en tant que module, le chargement échoue et est rapporté dans `LoadHooksResult.errors`.
...
Ceci est le modèle d'interception pré/post central du sous-système de hooks.
...
En cas d'échec de l'outil, le wrapper émet `tool_result` avec `isError: true` et le texte d'erreur comme contenu, puis relance l'erreur originale.

### Ce que les hooks peuvent modifier
...
### Ce que les hooks ne peuvent pas modifier dans cette implémentation

- Les paramètres d'entrée bruts de l'outil en place (uniquement bloquer/autoriser sur `tool_call`)
- La continuation de l'exécution après des erreurs d'outil levées (le chemin d'erreur relance l'exception)
...
## Ordonnancement et comportement en cas de conflit

### Ordonnancement au niveau de la découverte

Les fournisseurs de capacités sont triés par priorité (la plus élevée en premier). La déduplication se fait par clé de capacité, le premier gagne.
...
À l'intérieur de `HookRunner`, l'ordre est déterministe selon la séquence d'enregistrement :
...
- stocké par clé
- trié par nom de clé
- assaini (`\r`, `\n`, `\t` → espaces ; espaces répétés condensés)
...
`HookRunner.emit(...)` capture les erreurs des gestionnaires pour la plupart des événements et émet `HookError` aux écouteurs (`hookPath`, `event`, `error`), puis continue.

`emitToolCall(...)` est plus strict : les erreurs des gestionnaires ne sont pas absorbées ; elles se propagent à l'appelant. Dans `HookToolWrapper`, cela bloque l'appel de l'outil (sécurité par défaut).
...
- le ré-export de `execCommand`

Et la racine du package (`src/index.ts`) ré-exporte les **types** de hooks en tant que surface de compatibilité legacy.
