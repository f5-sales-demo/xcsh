...
Ce document décrit le **code du sous-système de hooks actuel** dans `src/extensibility/hooks/*`.

## État actuel dans le runtime

Le package de hooks (`src/extensibility/hooks/`) est toujours exporté et utilisable en tant que surface d'API, mais le runtime CLI par défaut initialise désormais le chemin du **runner d'extension**. Dans le flux de démarrage actuel :

- `--hook` est traité comme un alias pour `--extension` (les chemins CLI sont fusionnés dans `additionalExtensionPaths`)
- les outils sont enveloppés par `ExtensionToolWrapper`, et non par `HookToolWrapper`
- les transformations de contexte et les émissions de cycle de vie transitent par `ExtensionRunner`

Ce fichier documente donc l'implémentation du sous-système de hooks lui-même (types/loader/runner/wrapper), y compris le comportement hérité et les contraintes.

## Fichiers clés

- `src/extensibility/hooks/types.ts` — contexte de hook, types d'événements et contrats de résultats
- `src/extensibility/hooks/loader.ts` — chargement de modules et pont de découverte de hooks
- `src/extensibility/hooks/runner.ts` — distribution d'événements, recherche de commandes et signalisation d'erreurs
- `src/extensibility/hooks/tool-wrapper.ts` — enveloppe d'interception pré/post des outils
- `src/extensibility/hooks/index.ts` — exports/ré-exports

## Ce qu'est un module de hook
...
1. Charge les hooks découverts depuis le registre de capacités (`loadCapability("hooks")`)
2. Ajoute les chemins explicitement configurés (dédupliqués par chemin absolu)
3. Appelle `loadHooks(allPaths, cwd)`

`loadHooks` importe ensuite chaque chemin et s'attend à une fonction `default`.
...
Le chargeur de hooks utilise ici l'import dynamique de modules et requiert une factory de hooks JS/TS par défaut. Si un chemin de hook découvert ne peut pas être importé en tant que module, le chargement échoue et est signalé dans `LoadHooksResult.errors`.
...
Il s'agit du modèle d'interception pré/post central du sous-système de hooks.
...
- si un gestionnaire lève une exception, le wrapper échoue prudemment et bloque l'exécution
...
En cas d'échec de l'outil, le wrapper émet `tool_result` avec `isError: true` et le contenu du texte d'erreur, puis relève l'erreur d'origine.

### Ce que les hooks peuvent muter

- le contexte LLM pour un seul appel via `context` (chaîne de remplacement des `messages`)
- le contenu/les détails de la sortie de l'outil lors d'appels réussis (`chemin tool_result`)
- le message injecté pré-agent via `before_agent_start`
- l'annulation/la compaction personnalisée/le comportement de l'arborescence via `session_before_*` et `session.compacting`

### Ce que les hooks ne peuvent pas muter dans cette implémentation

- les paramètres d'entrée bruts de l'outil en place (uniquement blocage/autorisation sur `tool_call`)
- la continuation de l'exécution après les erreurs levées par l'outil (le chemin d'erreur relève à nouveau)
...
Les fournisseurs de capacités sont triés par priorité (les plus élevées en premier). La déduplication se fait par clé de capacité, le premier gagne.

Pour `hooks`, la clé de capacité est `${type}:${tool}:${name}`. Les doublons masqués par des fournisseurs de moindre priorité sont marqués et exclus de la liste de découverte effective.
...
Dans `HookRunner`, l'ordre est déterministe selon la séquence d'enregistrement :

1. ordre du tableau des hooks
2. ordre d'enregistrement des gestionnaires par hook/événement

Comportement en cas de conflit selon le type d'événement :

- `tool_call` : le dernier résultat retourné gagne, sauf si un gestionnaire bloque ; le premier blocage court-circuite
- `tool_result` : la dernière substitution retournée gagne (pas de court-circuit)
- `context` : chaîné ; chaque gestionnaire reçoit la sortie des messages du gestionnaire précédent
...
## Interactions avec l'interface utilisateur (`HookContext.ui`)

`HookUIContext` comprend :
...
Lors de l'exécution sans interface utilisateur, le comportement par défaut sans opération du contexte est :

- `select/input/editor` retournent `undefined`
- `confirm` retourne `false`
- `notify`, `setStatus`, `setEditorText` sont sans opération
...
## Propagation des erreurs et mécanisme de repli
...
### À l'exécution des événements

`HookRunner.emit(...)` capture les erreurs des gestionnaires pour la plupart des événements et émet `HookError` vers les listeners (`hookPath`, `event`, `error`), puis continue.

`emitToolCall(...)` est plus strict : les erreurs des gestionnaires ne sont pas absorbées ; elles se propagent jusqu'à l'appelant. Dans `HookToolWrapper`, cela bloque l'appel de l'outil (sécurité par défaut).

## Exemples d'API réalistes

### Bloquer les commandes bash non sécurisées
...
### Expurger la sortie de l'outil en post-exécution
...
- la ré-exportation de `execCommand`

Et la racine du package (`src/index.ts`) ré-exporte les **types** de hooks comme surface de compatibilité héritée.
