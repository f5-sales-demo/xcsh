...
Ce guide est une checklist reproductible pour porter des modifications de pi-mono vers ce dépôt.
Utilisez-le pour toute fusion : fichier unique, branche de fonctionnalité ou synchronisation de version complète.
...
Lors du démarrage d'une nouvelle synchronisation, générez les patches depuis ce commit :
...
- Décidez quelles fonctionnalités sont incluses et lesquelles sont intentionnellement exclues.

## 1) Importer le code en toute sécurité

- Privilégiez un diff propre et ciblé plutôt qu'une copie intégrale.
- Évitez de copier les artéfacts compilés ou les fichiers générés.
...
La plupart des sources TypeScript d'exécution omettent `.js` dans les imports internes, mais certains points d'entrée de test/bench conservent `.js` pour la compatibilité ESM à l'exécution. Suivez le style existant du package local ; ne supprimez pas les extensions de manière systématique.

- Dans les sources d'exécution de `packages/coding-agent`, gardez les imports internes sans extension sauf pour l'import d'assets non-TS.
- Dans `packages/tui/test` et `packages/natives/bench`, conservez `.js` là où les fichiers environnants l'utilisent déjà.
- Conservez les vraies extensions de fichiers lorsque l'outillage l'exige (par ex., `.json`, `.css`, intégrations de texte `.md`).
...
L'amont utilise des scopes de packages différents. Remplacez-les de manière cohérente.

- Remplacez les anciens scopes par le scope local utilisé ici.
- Exemples (à adapter selon les packages que vous portez) :
...
## 4) Utiliser les API Bun lorsqu'elles améliorent Node

Nous fonctionnons sous Bun. Remplacez les API Node uniquement lorsque Bun offre une meilleure alternative.

**À REMPLACER :**

- Lancement de processus : `child_process.spawn` → Bun Shell `$` pour les commandes simples, `Bun.spawn`/`Bun.spawnSync` pour le streaming ou les travaux de longue durée
- E/S fichiers : `fs.readFileSync` → `Bun.file().text()` / `Bun.write()`
...
**NE PAS REMPLACER (ceux-ci fonctionnent correctement sous Bun) :**

- `os.homedir()` — NE PAS remplacer par `Bun.env.HOME`, `Bun.env.HOME`, ou le littéral `"~"`
- `os.tmpdir()` — NE PAS remplacer par `Bun.env.TMPDIR || "/tmp"` ou des chemins en dur
- `fs.mkdtempSync()` — NE PAS remplacer par une construction manuelle de chemin
- `path.join()`, `path.resolve()`, etc. — ceux-ci sont corrects
...
- Privilégiez Bun Shell `$` pour les commandes courtes sans streaming ; utilisez `Bun.spawn` uniquement lorsque vous avez besoin d'E/S en streaming ou de contrôle de processus.
...
- Privilégiez `Bun.sleep(ms)` plutôt que les wrappers `setTimeout`.
...
## 5) Privilégier les embeds Bun (pas de copie)

Ne copiez pas les assets d'exécution ou les fichiers vendorisés au moment du build.

- Si l'amont copie des assets dans un dossier dist, remplacez par des embeds compatibles Bun.
- Les prompts sont des fichiers `.md` statiques ; utilisez les imports texte Bun (`with { type: "text" }`) et Handlebars au lieu de chaînes de prompt en ligne.
- Utilisez `import.meta.dir` + `Bun.file` pour charger des ressources non-texte adjacentes.
...
- Si l'amont lit un fichier de secours embarqué à l'exécution, remplacez les lectures du système de fichiers par un import embed texte Bun.
  - Exemple (secours d'instructions Codex) :
...
- Ne rétrogradez pas les dépendances pour corriger des erreurs de type ; mettez à niveau à la place.
...
- Évitez les imports dynamiques et les imports de type en ligne ; utilisez uniquement les imports de niveau supérieur.
...
- Privilégiez les helpers et utilitaires existants plutôt que du nouveau code ad-hoc.
- Préservez les changements d'infrastructure Bun-first déjà effectués dans ce dépôt :
  - L'environnement d'exécution est Bun (pas de points d'entrée Node).
  ...
  - Les shebangs CLI utilisent `bun` (pas `node`, pas `tsx`).
  - Les packages utilisent directement les fichiers source (pas d'étape de build TypeScript).
...
- Remplacez les liens du dépôt pi-mono le cas échéant.
- Mettez à jour les exemples pour utiliser Bun et les scopes de packages corrects.
...
Les tests utilisent le runner Bun (pas Vitest), mais n'exécutez `bun test` que lorsque cela est explicitement demandé.

## 11) Protéger les fonctionnalités améliorées (liste de pièges à régression)

Si vous avez déjà amélioré un comportement localement, traitez-le comme **non négociable**. Avant le portage, notez les améliorations et ajoutez des vérifications explicites pour qu'elles ne se perdent pas dans la fusion.

- **Figez le comportement attendu** : ajoutez une courte note « avant/après » pour chaque amélioration (entrées, sorties, valeurs par défaut, cas limites). Cela empêche un retour silencieux.
- **Cartographiez ancien → nouveau API** : si l'amont a renommé des concepts (hooks → extensions, custom tools → tools, etc.), assurez-vous que chaque ancien point d'entrée est toujours câblé. Un flag ou export manqué équivaut à une fonctionnalité perdue.
- **Vérifiez les exports** : contrôlez les `exports` de `package.json`, les types publics et les fichiers barrel. Les portages amont oublient souvent de ré-exporter les ajouts locaux.
- **Couvrez les chemins non nominaux** : si vous avez corrigé la gestion d'erreurs, les timeouts ou la logique de secours, ajoutez un test ou au moins une checklist manuelle qui exerce ces chemins.
- **Vérifiez les valeurs par défaut et l'ordre de fusion de config** : les améliorations vivent souvent dans les valeurs par défaut. Confirmez que les nouvelles valeurs par défaut n'ont pas régressé (par ex., nouvelle priorité de configuration, fonctionnalités désactivées, listes d'outils).
- **Auditez le comportement env/shell** : si vous avez corrigé l'exécution ou le sandboxing, vérifiez que le nouveau chemin utilise toujours votre env assaini et ne réintroduit pas de surcharges d'alias/fonctions.
- **Ré-exécutez des exemples ciblés** : gardez un ensemble minimal d'exemples « connus bons » et exécutez-les après le portage (flags CLI, enregistrement d'extensions, exécution d'outils).
...
Si le diff montre que le fichier a été **remanié** (pas simplement patché) :

- Nouvelles abstractions, concepts renommés, modules fusionnés, flux de données modifié

Alors vous devez **lire la nouvelle implémentation en profondeur** avant de porter. La fusion aveugle de code remanié fait perdre des fonctionnalités car :

Note : le mode interactif a été récemment scindé en controllers/utils/types. Lors du rétroportage de modifications associées, portez les mises à jour dans les fichiers individuels que nous avons créés et assurez-vous que le câblage de `interactive-mode.ts` reste synchronisé.
...
2. **Les options d'API sont supprimées** - Lorsque des systèmes fusionnent (par ex., `hooks` + `customTools` → `extensions`), les anciennes options peuvent ne pas être câblées vers la nouvelle implémentation.

3. **Des chemins de code deviennent obsolètes** - Un concept renommé (par ex., `hookMessage` → `custom`) nécessite des mises à jour dans chaque instruction switch, garde de type et handler — pas seulement la définition.
...
4. **Recherchez les oublis** - Cherchez les anciens noms/concepts qui pourraient avoir été manqués dans les instructions switch, handlers, composants UI.

5. **Testez les limites** - Flags CLI, options SDK, gestionnaires d'événements, valeurs par défaut — c'est là que les régressions se cachent.
...
# Vérifier si toutes les valeurs enum/union ont des handlers
...
- [ ] Pas d'API Node-only dans le code nouveau/porté
- [ ] Tous les scopes de packages mis à jour
- [ ] Les scripts de `package.json` utilisent Bun
- [ ] Les prompts sont des imports texte `.md` (pas de chaînes de prompt en ligne)
- [ ] Pas de `console.*` dans coding-agent (utiliser `logger`)
- [ ] Les assets se chargent via les patterns d'embed Bun (pas de scripts de copie)
- [ ] Les tests ou vérifications s'exécutent (ou sont explicitement notés comme bloqués)
- [ ] Pas de régressions fonctionnelles (voir sections 11-12)

## 14) Format des messages de commit

Lors d'un commit de rétroportage, suivez le format du dépôt `<type>(scope): <description au passé>` et conservez la plage de commits dans le titre.
...
| `ctx.ui.setHeader()` / `ctx.ui.setFooter()` | Stub en modes non-TUI                                     | Implémenté dans le TUI, no-op ailleurs                                |
| `ctx.ui.setEditorComponent()`               | Stub en modes non-TUI                                     | Implémenté dans le TUI, no-op ailleurs                                |
| Objet d'options `InteractiveModeOptions`    | Args positionnels du constructeur (le type options est toujours exporté) | Gardez la signature du constructeur ; mettez à jour le type quand l'amont ajoute des champs |
...
| `sessionManager.getSessionName()`        | `sessionManager.getSessionName()`        | Identique (nous avons unifié pour correspondre au RPC amont) |
...
| Amont                                              | Notre fork                              | Raison                                  |
| -------------------------------------------------- | --------------------------------------- | --------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (fichiers d'outils) | Module clipboard `@f5xc-salesdemos/pi-natives` | Fusionné dans l'implémentation native N-API |
...
| Interfaces `*Operations` par outil  | Les interfaces par outil restent (`FindOperations`, `GrepOperations`) | Utilisées pour les surcharges SSH/remote                  |
...
| Amont                           | Notre fork                                  | Notes                                        |
| ------------------------------- | ------------------------------------------- | -------------------------------------------- |
| `proper-lockfile` + `auth.json` | `agent.db` (bun:sqlite)                     | Les identifiants sont stockés exclusivement dans `agent.db` |
...
| Amont                         | Notre fork                                 |
| ----------------------------- | ------------------------------------------ |
| `jiti` pour le chargement TypeScript | `import()` natif Bun                      |
| Champ manifeste `pkg.pi`      | `pkg.xcsh ?? pkg.pi` (préférer notre namespace) |

### Passer ces fonctionnalités amont

Lors du portage, **ignorez** entièrement ces fichiers/fonctionnalités :

- `footer-data-provider.ts` — nous utilisons StatusLineComponent
- `clipboard-image.ts` — le presse-papiers est dans le module N-API `@f5xc-salesdemos/pi-natives`
- Fichiers de workflow GitHub — nous avons notre propre CI
- `models.generated.ts` — auto-généré, à régénérer localement (en tant que models.json à la place)

### Fonctionnalités que nous avons ajoutées (à préserver)

Celles-ci existent dans notre fork mais pas en amont. **Ne jamais écraser :**
...
- Écriture transparente LSP pour le formatage à la sauvegarde
- Interception Bash (`checkBashInterception`)
- Suggestions de chemins floues dans l'outil de lecture
