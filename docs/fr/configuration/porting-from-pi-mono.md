...
Utilisez-le pour toute fusion : fichier unique, branche de fonctionnalité ou synchronisation complète de version.
...
Lors du démarrage d'une nouvelle synchronisation, générez les patches à partir de ce commit :
...
- Décidez quelles fonctionnalités sont dans le périmètre et lesquelles sont volontairement ignorées.

## 1) Importer le code en toute sécurité

- Préférez un diff propre et ciblé plutôt qu'une copie en masse.
- Évitez de copier les artefacts de build ou les fichiers générés.
- Si l'amont a ajouté de nouveaux fichiers, ajoutez-les explicitement et révisez leur contenu.

## 2) Respecter les conventions d'extensions d'import

La plupart des sources TypeScript d'exécution omettent `.js` dans les imports internes, mais certains points d'entrée de test/benchmark conservent `.js` pour la compatibilité d'exécution ESM. Suivez le style existant du package local ; ne supprimez pas les extensions de manière systématique.

- Dans les sources d'exécution de `packages/coding-agent`, gardez les imports internes sans extension sauf pour l'import de ressources non-TS.
...
- Exemples (ajustez en fonction des packages réels que vous portez) :
...
Nous tournons sur Bun. Remplacez les API Node uniquement lorsque Bun fournit une meilleure alternative.
...
- Hachage cryptographique : `node:crypto` → Web Crypto ou `Bun.hash`
- SQLite : `better-sqlite3` → `bun:sqlite`
- Chargement d'env : `dotenv` → Bun charge `.env` automatiquement

**NE PAS REMPLACER (ceux-ci fonctionnent très bien dans Bun) :**
...
**Style d'import :** Utilisez le préfixe `node:` uniquement avec les imports d'espace de noms (pas d'imports nommés depuis `node:fs` ou `node:path`).

**Conventions Bun supplémentaires :**

- Préférez Bun Shell `$` pour les commandes courtes sans streaming ; utilisez `Bun.spawn` uniquement lorsque vous avez besoin d'E/S en streaming ou de contrôle de processus.
...
## 5) Préférer les intégrations Bun (pas de copie)

Ne copiez pas les ressources d'exécution ou fichiers vendorisés au moment du build.

- Si l'amont copie des ressources dans un dossier dist, remplacez par des intégrations compatibles Bun.
...
- Gardez les ressources dans le dépôt et laissez le bundler les inclure.
- Éliminez les scripts de copie sauf si l'utilisateur les demande explicitement.
- Si l'amont lit un fichier de secours embarqué à l'exécution, remplacez les lectures du système de fichiers par un import texte intégré Bun.
...
- Conservez les `name`, `version`, `type`, `exports` et `bin` existants sauf si le portage nécessite des changements.
...
- Ne rétrogradez pas les dépendances pour corriger des erreurs de types ; mettez à niveau à la place.
...
- Remplacez les liens vers le dépôt pi-mono le cas échéant.
- Mettez à jour les exemples pour utiliser Bun et les scopes de package corrects.
...
Les tests utilisent le runner de Bun (pas Vitest), mais n'exécutez `bun test` que lorsque c'est explicitement demandé.

## 11) Protéger les fonctionnalités améliorées (liste de pièges de régression)
...
- **Figez le comportement attendu** : ajoutez une courte note « avant/après » pour chaque amélioration (entrées, sorties,
  valeurs par défaut, cas limites). Cela empêche un retour silencieux en arrière.
- **Cartographiez les anciennes API → nouvelles** : si l'amont a renommé des concepts (hooks → extensions, custom tools → tools, etc.),
...
- **Vérifiez les exports** : contrôlez les `exports` de `package.json`, les types publics et les fichiers barils. Les portages depuis l'amont oublient souvent
  de ré-exporter les ajouts locaux.
- **Couvrez les chemins non nominaux** : si vous avez corrigé la gestion des erreurs, les timeouts ou la logique de secours, ajoutez un test ou au
...
- **Vérifiez les valeurs par défaut et l'ordre de fusion des configurations** : les améliorations résident souvent dans les valeurs par défaut. Confirmez que les nouvelles valeurs par défaut
  n'ont pas été annulées (par ex., nouvelle priorité de configuration, fonctionnalités désactivées, listes d'outils).
- **Auditez le comportement env/shell** : si vous avez corrigé l'exécution ou le sandboxing, vérifiez que le nouveau chemin utilise toujours votre
  env assaini et ne réintroduit pas de surcharges d'alias/fonctions.
- **Relancez des exemples ciblés** : conservez un ensemble minimal d'exemples « connus bons » et exécutez-les après le portage
  (flags CLI, enregistrement d'extensions, exécution d'outils).

## 12) Détecter et gérer le code remanié

Avant de porter un fichier, vérifiez si l'amont l'a significativement restructuré :

```bash
# Comparer le fichier que vous allez porter avec ce que vous avez localement
...
Note : le mode interactif a récemment été découpé en controllers/utils/types. Lors du rétroportage de changements liés, portez les mises à jour dans les fichiers individuels que nous avons créés et assurez-vous que le câblage de `interactive-mode.ts` reste synchronisé.
...
2. **Des options d'API sont supprimées** - Lorsque des systèmes fusionnent (par ex., `hooks` + `customTools` → `extensions`), les anciennes options peuvent ne pas être connectées à la nouvelle implémentation.

3. **Des chemins de code deviennent obsolètes** - Un concept renommé (par ex., `hookMessage` → `custom`) nécessite des mises à jour dans chaque instruction switch, garde de type et gestionnaire — pas seulement la définition.
...
5. **Testez les limites** - Les flags CLI, options SDK, gestionnaires d'événements, valeurs par défaut — c'est là que les régressions se cachent.
...
# Vérifier que toutes les valeurs enum/union ont des gestionnaires
...
- [ ] Pas d'API exclusives Node dans le code nouveau/porté
...
- [ ] Les ressources se chargent via les patterns d'intégration Bun (pas de scripts de copie)
...
Lors du commit d'un rétroportage, suivez le format du dépôt `<type>(scope): <description au passé>` et conservez la plage
de commits dans le titre.
...
- Regroupez les changements par package
...
| Objet d'options `InteractiveModeOptions`    | Args positionnels de constructeur (le type options reste exporté) | Conserver la signature du constructeur ; mettre à jour le type quand l'amont ajoute des champs |
...
| Amont                               | Notre fork                                                        | Notes                                                       |
| ----------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------- |
| `createTool(cwd: string, options?)` | `createTools(session: ToolSession)` via le registre `BUILTIN_TOOLS` | Les fabriques d'outils acceptent `ToolSession` et peuvent retourner `null` |
...
| `fs/promises` de Node.js partout    | `Bun.file()`/`Bun.write()` pour les fichiers ; `node:fs/promises` pour les répertoires | Préférer les API Bun quand elles simplifient                |
...
| Champ manifeste `pkg.pi`            | `pkg.xcsh ?? pkg.pi` (préférer notre espace de noms) |
...
- `models.generated.ts` — auto-généré, régénérez localement (en tant que models.json à la place)
