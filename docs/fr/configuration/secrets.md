...
Empêche les valeurs sensibles (clés API, jetons, mots de passe) d'être envoyées aux fournisseurs de LLM. Lorsque cette fonctionnalité est activée, les secrets sont remplacés par des substituts déterministes avant de quitter le processus, et restaurés dans les arguments d'appel d'outils renvoyés par le modèle.
...
1. Au démarrage de la session, les secrets sont collectés à partir de deux sources :
   - **Variables d'environnement** correspondant aux motifs courants de secrets (`*_KEY`, `*_SECRET`, `*_TOKEN`, `*_PASSWORD`, etc.) avec des valeurs >= 8 caractères
   - **Fichiers `secrets.yml`** (voir ci-dessous)

2. Les messages sortants vers le LLM voient toutes les valeurs secrètes remplacées par des substituts comme `<<$env:S0>>`, `<<$env:S1>>`, etc.

3. Les arguments d'appel d'outils renvoyés par le modèle sont parcourus en profondeur et les substituts sont restaurés à leurs valeurs originales avant l'exécution.

Deux modes contrôlent ce qui arrive à chaque secret :

| Mode | Comportement | Réversible |
|---|---|---|
| `obfuscate` (par défaut) | Remplacé par un substitut indexé `<<$env:SN>>` | Oui (désobfusqué dans les arguments d'outils) |
...
| `content` | chaîne | Oui | La valeur du secret (plain) ou le motif regex (regex) |
...
| `flags` | chaîne | Non | Drapeaux regex (type regex uniquement) |
...
# Correspondance insensible à la casse avec drapeaux explicites
- type: regex
...
Les entrées regex effectuent toujours une recherche globale (le drapeau `g` est appliqué automatiquement). La syntaxe littérale regex `/motif/drapeaux` est supportée comme alternative aux champs séparés `content` + `flags`. Les barres obliques échappées dans le motif (`\\/`) sont gérées correctement.
...
Les variables d'environnement sont toujours collectées en premier. Les entrées définies par fichier sont ajoutées ensuite, de sorte que les entrées de fichier peuvent couvrir des secrets qui ne se trouvent pas dans les variables d'environnement (fichiers de configuration, valeurs codées en dur, etc.). Si la même valeur apparaît dans les deux, le mode de l'entrée du fichier prend la priorité.

## Fichiers clés

- `src/secrets/index.ts` -- chargement, fusion, collecte des variables d'environnement
- `src/secrets/obfuscator.ts` -- classe `SecretObfuscator`, génération de substituts, obfuscation des messages
