...
Ce document décrit comment le coding-agent résout la configuration aujourd'hui : quelles racines sont analysées, comment fonctionne la priorité, et comment la configuration résolue est consommée par les paramètres, les compétences, les hooks, les outils et les extensions.
...
- Entrées au niveau utilisateur en premier (par priorité de source)
...
Utilisez ceci lorsque la configuration projet doit être héritée des répertoires ancêtres (comportement monorepo/espace de travail imbriqué).
...
Migration héritée toujours supportée :

- Si le chemin cible est `.yml`/`.yaml`, un fichier `.json` voisin est auto-migré une seule fois (`migrateJsonToYml`).
...
2. Paramètres projet : découverts via la capacité settings (`settings.json` des fournisseurs)
...
2. Fusion avec les paramètres hérités de la BD depuis `agent.db`
...
- Structure plate héritée `theme: "..."` -> structure `theme.dark/theme.light`

---

## 5) Intégration capacités/découverte
...
- même clé => le premier élément l'emporte (élément de plus haute priorité/chargé en premier)
...
`builtin.ts` n'inclut une racine de configuration que si le répertoire existe **et n'est pas vide** (`ifNonEmptyDir`).
...
- `Settings.init()` charge le fichier global `config.yml` + les éléments de capacité `settings.json` du projet découverts.
...
- Les bascules nommées héritées existent toujours (`skills.enablePiUser`, `skills.enablePiProject`) mais elles contrôlent le fournisseur natif (`provider === "native"`).
...
- `discoverAndLoadCustomTools()` résout les chemins d'outils depuis la capacité tool + les chemins d'outils des plugins + les chemins configurés explicitement.
...
2. La priorité du fournisseur de capacités détermine la priorité inter-fournisseurs.
...
### Avertissement spécifique aux paramètres

Les éléments de capacité settings ne sont pas dédupliqués ; `Settings.#loadProjectSettings()` effectue une fusion profonde des éléments projet dans l'ordre retourné. Comme la fusion applique les valeurs des éléments ultérieurs par-dessus les valeurs antérieures, le comportement de surcharge effectif dépend de l'ordre d'émission des fournisseurs, et pas uniquement de la sémantique des clés de capacité.

---

## 9) Comportements hérités/de compatibilité toujours présents

- Migration `ConfigFile` JSON -> YAML pour les fichiers ciblés en YAML.
...
- Les noms de paramètres hérités `skills.enablePiUser` / `skills.enablePiProject` sont toujours des portes actives pour la source de compétences native.
