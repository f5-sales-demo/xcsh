...
Le mode RPC exécute l'agent de codage comme un protocole JSON délimité par des retours à la ligne via stdio.
...
- Le processus lit stdin en tant que JSONL (`readJsonl(Bun.stdin.stream())`).
...
4. Erreurs d'extensions (`{ type: "extension_error", extensionPath, event, error }`)
...
- `RpcClient` s'appuie sur cela pour la résolution des requêtes en attente.

Comportement limite important au niveau du runtime :

- Les réponses aux commandes inconnues sont émises avec `id: undefined` (même si la requête avait un `id`).
- Les exceptions de parsing/gestionnaire dans la boucle d'entrée émettent `command: "parse"` avec `id: undefined`.
...
Remplace l'état des tâches en mémoire pour la session en cours et renvoie la liste normalisée des phases :
...
modèle. Renvoyer `set_host_tools` remplace l'ensemble précédent appartenant à l'hôte.
...
## Concurrence et ordre des prompts/files d'attente

Il s'agit du comportement opérationnel le plus important.
...
- acceptation de la commande != complétion de l'exécution
- la complétion finale est observée via `agent_end`

### Pendant le streaming

`AgentSession.prompt()` nécessite `streamingBehavior` pendant un streaming actif :
...
### Valeurs par défaut de la file d'attente
...
- `set_steering_mode` / `set_follow_up_mode`
  ...
  - `"all"` : retire toute la file d'attente d'un coup
- `set_interrupt_mode`
...
Les extensions en mode RPC utilisent des trames UI requête/réponse.
...
- La génération automatique du titre de session est désactivée en mode RPC, et les requêtes
  UI `setTitle` sont également supprimées par défaut car la plupart des hôtes ne disposent pas
  d'une surface de titre de terminal significative. Définissez `PI_RPC_EMIT_TITLE=1` pour
  réactiver l'événement UI uniquement.
...
Si un dialogue a un délai d'expiration, le mode RPC résout vers une valeur par défaut lorsque le délai/l'abandon se déclenche.
...
en servant les requêtes d'exécution via le même transport.
...
Définissez `isError: true` sur `host_tool_result` pour faire remonter le contenu renvoyé comme une
erreur d'outil.

## Modèle d'erreur et récupérabilité

### Échecs au niveau des commandes

Les échecs sont `success: false` avec une `error` de type chaîne de caractères.
...
- Les JSONL malformés / exceptions de la boucle de parsing émettent une réponse d'erreur `parse` et continuent la lecture des lignes suivantes.
- Un `set_session_name` vide est rejeté (`Session name cannot be empty`).
- Les réponses UI d'extensions avec un `id` inconnu sont ignorées.
