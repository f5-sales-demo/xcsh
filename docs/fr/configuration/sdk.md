...
title: SDK
description: >-
  SDK pour construire des agents personnalisés et des intégrations par-dessus le
...
Le SDK est la surface d'intégration in-process pour `@f5xc-salesdemos/xcsh`.
Utilisez-le lorsque vous souhaitez un accès direct à l'état de l'agent, au streaming d'événements, au câblage d'outils et au contrôle de session depuis votre propre processus Bun/Node.

Si vous avez besoin d'isolation inter-langage/inter-processus, utilisez le mode RPC à la place.
...
`@f5xc-salesdemos/xcsh` exporte les API du SDK depuis la racine du paquet (et également via `@f5xc-salesdemos/xcsh/sdk`).
...
## Démarrage rapide (paramètres par défaut avec auto-découverte)
...
- **Habituellement fourni explicitement** par les intégrateurs :
...
- Utile pour les tests, les workers éphémères, les agents liés à une requête.
...
4. repli vers le résolveur de fournisseur personnalisé (si configuré)
...
`AgentSessionEvent` inclut les `AgentEvent` de base plus les événements de niveau session :
...
1. expansion optionnelle des commandes/modèles (commandes `/`, commandes personnalisées, commandes slash de fichier, modèles de prompt)
2. si le streaming est en cours :
...
- Les outils masqués (par exemple `submit_result`) sont opt-in sauf si requis par les options.
...
- `disableExtensionDiscovery` : désactiver le scan automatique des extensions
...
Ceux-ci sont optionnels pour une intégration normale à agent unique.
...
## Exemple minimal d'intégration contrôlée
