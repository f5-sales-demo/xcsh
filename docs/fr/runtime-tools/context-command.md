...
xcsh se connecte à F5 Distributed Cloud via des **contextes** -- des ensembles d'identifiants nommés qui associent une URL de tenant, un jeton API et un namespace. Si vous avez déjà utilisé `kubectl config use-context` ou `kubectx`, le flux de travail est identique : créez un contexte, basculez entre eux par nom, et utilisez `-` pour revenir en arrière.
...
Une fois activé, xcsh injecte les identifiants du tenant dans votre session. L'agent peut désormais effectuer des appels API F5 XC, et la barre de statut affiche le contexte actif.
...
Basculez par nom -- aucun sous-commande verbale nécessaire :
...
| `/context status` | Afficher le statut d'authentification actuel |
...
Les contextes peuvent embarquer des variables d'environnement supplémentaires qui sont injectées dans votre session lors de l'activation. Utile pour la configuration par tenant qui ne fait pas partie de l'ensemble d'identifiants.
...
Les noms de contextes doivent comporter de 1 à 64 caractères : lettres, chiffres, tirets, underscores.
...
Si `F5XC_API_URL` et `F5XC_API_TOKEN` sont définis dans votre environnement shell avant de lancer xcsh, ils ont la priorité sur tout contexte. Ceci est utile pour les pipelines CI/CD ou les sessions ponctuelles où vous ne souhaitez pas créer de contexte persistant.

Dans ce mode, `/context` affiche les identifiants provenant de l'environnement avec un label `(via env vars)`.

## Comportement du contexte précédent

- **Portée de session** : le contexte précédent est réinitialisé lorsque vous redémarrez xcsh. Il n'est pas persisté sur le disque.
