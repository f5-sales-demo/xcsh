...
| `/context status` | Afficher l'état d'authentification actuel |
...
Les contextes peuvent transporter des variables d'environnement supplémentaires qui sont injectées dans votre session lors de l'activation. Utile pour la configuration spécifique à un tenant qui ne fait pas partie de l'ensemble d'identifiants.
...
Si `F5XC_API_URL` et `F5XC_API_TOKEN` sont définis dans l'environnement de votre shell avant de lancer xcsh, ils prennent le dessus sur tout contexte. Cela est utile pour les pipelines CI/CD ou les sessions ponctuelles où vous ne souhaitez pas créer un contexte persistant.

En fonctionnant dans ce mode, `/context` affiche les identifiants provenant de l'environnement avec un label `(via env vars)`.

## Comportement du contexte précédent

- **Portée de la session** : le contexte précédent est réinitialisé lorsque vous redémarrez xcsh. Il n'est pas persisté sur le disque.
...
- **La réactivation est un no-op** : `/context production` alors que vous êtes déjà sur `production` ne réinitialise pas le pointeur précédent.
