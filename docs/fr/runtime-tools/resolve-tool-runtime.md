---
title: Fonctionnement interne du runtime de l'outil Resolve
description: >-
  Runtime de l'outil Resolve pour la résolution de chemins de fichiers, la
  récupération de contenu et l'accès aux ressources basé sur les URL.
sidebar:
  order: 3
  label: Outil Resolve
i18n:
...
`resolve` est un outil masqué qui finalise une action en attente de prévisualisation.
...
- `pop()` retire et retourne l'action du sommet.
- `hasPending` indique si la pile est non vide.

`resolve` consomme toujours l'action en attente **la plus haute** en premier (`pop()`), donc les outils produisant des prévisualisations multiples se résolvent dans l'ordre inverse de leur enregistrement.
...
- label (résumé lisible par l'humain)
...
- `label: string` (requis)
- `apply(reason: string): Promise<AgentToolResult<unknown>>` (requis) — invoqué lors de l'application ; `reason` est la chaîne transmise à `resolve`
...
## Disponibilité du runtime et échecs
...
Lorsque `PendingActionStore.hasPending` est vrai, le runtime de l'agent oriente la sélection d'outil vers `resolve` afin que les prévisualisations en attente soient explicitement finalisées avant que le flux normal des outils ne se poursuive.
...
- Gardez `label` concis et spécifique ; il est affiché dans la sortie du rendu de resolve.
- Assurez-vous que `apply(reason)` est suffisamment déterministe et idempotent pour une exécution en une seule fois ; `reason` est informatif et ne devrait pas modifier le comportement.
...
- Si votre outil peut mettre en file d'attente plusieurs prévisualisations, gardez à l'esprit la sémantique LIFO : la dernière action poussée est résolue en premier.
