...
- `peek()` inspecte l'action actuelle au sommet.
- `pop()` retire et retourne l'action au sommet.
- `hasPending` indique si la pile est non vide.

`resolve` consomme toujours l'action en attente **au sommet** en premier (`pop()`), de sorte que plusieurs outils produisant des prévisualisations sont résolus dans l'ordre inverse de leur enregistrement.
...
## Disponibilité à l'exécution et échecs
...
Lorsque `PendingActionStore.hasPending` est vrai, le runtime de l'agent oriente le choix d'outil vers `resolve` afin que les prévisualisations en attente soient explicitement finalisées avant que le flux normal d'outils ne se poursuive.
...
- Gardez `label` concis et spécifique ; il est affiché dans la sortie du renderer de resolve.
- Assurez-vous que `apply(reason)` est suffisamment déterministe et idempotent pour une exécution unique ; `reason` est informatif et ne devrait pas modifier le comportement.
...
- Si votre outil peut mettre en file d'attente plusieurs prévisualisations, n'oubliez pas la sémantique LIFO : la dernière action poussée est résolue en premier.
