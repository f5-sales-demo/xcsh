---
title: Composants internes du runtime de l'outil Notebook
description: >-
  Runtime de l'outil Jupyter notebook avec exécution de cellules, cycle de vie
...
# Composants internes du runtime de l'outil Notebook

Ce document décrit l'implémentation actuelle de l'outil `notebook` et sa relation avec le runtime Python adossé à un noyau.

La distinction fondamentale : **`notebook` est un éditeur JSON de notebooks, non un exécuteur de notebooks**. Il édite directement les sources de cellules `.ipynb` ; il ne démarre pas de noyau Python et ne communique pas avec lui.
...
- Résout le chemin par rapport au répertoire de travail de la session (`resolveToCwd`).
...
- Retourne un résumé textuel et des `details` structurés (`action`, `cellIndex`, `cellType`, `totalCells`, `cellSource`).

Aucun cycle de vie de noyau n'existe dans cet outil :

- pas d'acquisition de passerelle
- pas d'identifiant de session de noyau
...
Lorsque l'agent doit exécuter du code Python de type cellule (cellules séquentielles, état persistant, affichages enrichis), cela passe par l'outil **`python`**, et non par `notebook`.
...
`content` est découpé en `source: string[]` avec préservation des sauts de ligne :

- chaque ligne non finale conserve le `\n` final
- la dernière ligne n'a pas de saut de ligne forcé en fin

Cela respecte les conventions JSON des notebooks et évite toute concaténation accidentelle de lignes lors d'éditions ultérieures.
...
- `insert`
  ...
  - `cell_type` vaut `code` par défaut
...
Des échecs bloquants sont levés pour :

- fichier notebook manquant
- JSON invalide
- `cells` absent ou non tableau
- index hors limites (les plages valides diffèrent entre insertion et autre opération)
- `content` manquant pour `edit`/`insert`

Ces erreurs deviennent des réponses d'outil `Error:` en amont ; le rendu utilise le chemin du notebook et le texte d'erreur formaté.
...
L'outil `python` transmet `reset` uniquement pour la première cellule lors d'un appel multi-cellules ; les cellules suivantes s'exécutent toujours avec `reset: false`.

## Mort du noyau / redémarrage / relance

En mode session (`withKernelSession`) :

- noyau mort détecté par battement de cœur (vérification `kernel.isAlive()` toutes les 5 s) ou par échec d'exécution.
- un état mort avant l'exécution déclenche `restartKernelSession`.
- le chemin de crash à l'exécution effectue une relance unique : redémarrage du noyau, réexécution du gestionnaire.
- `restartCount > 1` dans la même session lève l'erreur `Python kernel restarted too many times in this session`.

Comportement de relance au démarrage :

- la création de noyau sur passerelle partagée effectue une relance unique en cas de `SharedGatewayCreateError` avec HTTP 5xx.

Récupération en cas d'épuisement des ressources :

- détecte les échecs de type `EMFILE`/`ENFILE`/« Too many open files »
- vide les sessions tracées
- appelle `shutdownSharedGateway()`
- relance la création de session de noyau une fois
...
`PythonKernel.#initializeKernelEnvironment(...)` exécute ensuite un script d'initialisation dans le noyau pour :
...
- les assistants de prélude qui lisent le contexte de session ou d'artefact s'appuient sur ces variables d'environnement dans l'état du processus Python.
...
- `error` -> texte de traceback poussé dans le flux de fragments + métadonnées d'erreur structurées
...
## 6) Comportement de troncature et des artefacts
...
- suit le total des lignes, des lignes de sortie et des octets
- fichier de déversement d'artefact optionnel (`artifactPath`, `artifactId`)
- lorsque le tampon en mémoire dépasse le seuil (`DEFAULT_MAX_BYTES` sauf remplacement) :
  - marque comme tronqué
  - conserve les octets de queue en mémoire (limite sûre UTF-8)
  - peut déverser le flux complet vers le collecteur d'artefacts

`dump()` retourne :

- texte de sortie visible (éventuellement tronqué par la queue)
- indicateur de troncature + compteurs
- identifiant d'artefact (pour les références `artifact://<id>`)

L'outil `python` convertit ces métadonnées en avis de troncature de résultat et avertissements TUI.

L'outil `notebook` **n'utilise pas** `OutputSink` ; il ne dispose pas de pipeline de troncature de flux/artefacts car il n'exécute pas de code.

## 7) Hypothèses de rendu et formatage
...
- vue de résultat :
  ...
  - les cellules markdown définissent l'indication de langage `markdown` ; les autres cellules n'ont pas de remplacement de langage explicite
  - la limite de prévisualisation de code réduit est `PREVIEW_LIMITS.COLLAPSED_LINES * 2`
...
- transitions d'état par cellule (`pending/running/complete/error`)
- section d'événement de statut structuré optionnelle
- arborescences de sortie JSON optionnelles
- avertissements de troncature + pointeur `artifact://<id>` optionnel

Ce comportement de rendu est sans lien avec les résultats d'édition JSON de `notebook`, sauf que les deux réutilisent des primitives TUI partagées.
...
- `python` exécute du code dans un noyau, persiste l'état selon le mode, diffuse des fragments, capture des affichages enrichis, gère les interruptions/expirations et prend en charge la troncature des sorties/artefacts.
- `notebook` effectue uniquement des mutations déterministes du JSON du notebook ; pas d'exécution, pas d'état de noyau, pas de flux de fragments, pas de sorties d'affichage, pas de pipeline d'artefacts.
...
L'implémentation actuelle ne fournit pas d'outil unique permettant à la fois de muter `.ipynb` et d'exécuter des cellules de notebook dans le contexte d'un noyau.
