...
Ce document décrit le parcours runtime hors thème, de l'entrée terminal à la sortie rendue en mode interactif. Il se concentre sur le comportement dans `packages/tui` et son intégration depuis les contrôleurs de `packages/coding-agent`.
...
- **Mode interactif `packages/coding-agent`** : construit l'arbre de composants, lie les callbacks de l'éditeur et les raccourcis clavier, réagit aux événements agent/session, et traduit l'état du domaine (streaming, exécution d'outils, tentatives de réessai, mode plan) en composants UI.
...
4. Vérifie le support du protocole clavier Kitty (`CSI ? u`), puis active les flags du protocole si supporté.
5. Sous Windows, tente l'activation de l'entrée VT via les flags de mode `kernel32`.

Comportement de `StdinBuffer` :

- Met en tampon les séquences d'échappement fragmentées (CSI/OSC/DCS/APC/SS3).
- N'émet `data` que lorsqu'une séquence est complète ou vidée par timeout.
- Détecte le collage entre crochets et émet un événement `paste` avec le texte brut collé.

Cela empêche les fragments d'échappement partiels d'être interprétés à tort comme des frappes de touches normales.
...
2. Le TUI gère le raccourci de débogage global (`shift+ctrl+d`) avant la dispatch vers les composants.
3. Si le composant ayant le focus appartient à un overlay désormais masqué/invisible, le TUI réassigne le focus au prochain overlay visible ou au focus pré-overlay sauvegardé.
4. Les événements de relâchement de touche sont filtrés sauf si le composant ayant le focus définit `wantsKeyRelease = true`.
5. Après la dispatch, le TUI planifie un rendu.
...
- annulation / sortie de mode sur `Escape`
...
Cela maintient l'analyse des touches/mécaniques de l'éditeur dans `packages/tui` et la sémantique du mode dans les contrôleurs de coding-agent.

## Boucle de rendu et stratégie de diff

`TUI.requestRender()` est dédoublé à un rendu par tick en utilisant `process.nextTick`. Plusieurs changements d'état dans le même tour sont coalescés.
...
5. Choisit entre repeinture complète ou patch différentiel :
   - premier frame
   - changement de largeur
   - rétrécissement avec `clearOnShrink` activé et sans overlays
   - éditions au-dessus du viewport précédent
6. Pour les mises à jour différentielles, corrige uniquement la plage de lignes modifiées et efface les lignes résiduelles en fin si nécessaire.
...
Vérifications de sécurité critiques dans le TUI :
...
- La position du curseur est bornée avant le déplacement.

Ces contraintes sont appliquées à l'exécution, pas simplement des conventions.
...
- Le suivi du viewport/sommet (`#previousViewportTop`, `#maxLinesRendered`) évite les calculs invalides de position relative du curseur lorsque le contenu ou la taille du terminal change.
...
- `message_update` : met à jour le contenu de streaming de l'assistant ; crée/met à jour les composants d'exécution d'outils à mesure que les appels d'outils apparaissent.
- `tool_execution_update/end` : met à jour les composants de résultats d'outils et l'état de complétion.
- `message_end` : finalise le stream de l'assistant, gère les annotations d'interruption/erreur, marque les arguments d'outils en attente comme complets lors d'un arrêt normal.
- `agent_end` : arrête les loaders, efface l'état de stream transitoire, applique le changement de modèle différé, émet une notification de complétion si en arrière-plan.

Le regroupement des outils de lecture est intentionnellement à état (`#lastReadGroup`) pour coalescer les appels consécutifs d'outils de lecture en un seul bloc visuel jusqu'à ce qu'une interruption non-lecture survienne.
...
- `statusLine` affiche les indicateurs de statut/hooks/plan persistants et pilote les mises à jour de la bordure supérieure de l'éditeur.

Comportement des loaders :

- `Loader` se met à jour toutes les 80ms via un intervalle et demande un rendu à chaque frame.
- Les gestionnaires d'échappement sont temporairement remplacés pendant l'auto-compaction et l'auto-réessai pour annuler ces opérations.
- Sur les chemins de fin/annulation, les contrôleurs restaurent les gestionnaires d'échappement précédents et arrêtent/effacent les composants de loader.

## Transitions de mode et passage en arrière-plan

### Modes d'entrée Bash/Python

Les préfixes de texte d'entrée basculent les flags de mode de bordure de l'éditeur :
...
`InteractiveMode` suit les flags du mode plan, l'état de la ligne de statut, les outils actifs et le changement de modèle. L'entrée/sortie met à jour les entrées de mode de session et l'état du statut/UI, y compris le changement de modèle différé si le streaming est actif.
...
- `Escape` pendant un loader de stream actif : restaure les messages en file d'attente dans l'éditeur et interrompt l'agent.
- `Escape` pendant l'exécution bash/python : interrompt la commande en cours.
- `Escape` pendant l'auto-compaction/réessai : invoque des méthodes d'interruption dédiées via des gestionnaires d'échappement temporaires.
- `Ctrl+C` simple pression : efface l'éditeur ; double pression dans les 500ms : arrêt.

L'annulation est conditionnelle à l'état ; la même touche peut signifier interruption, sortie de mode, déclenchement de sélecteur ou absence d'action selon l'état du runtime.
...
Chemins limités en débit/dédoublés :

- Le rendu TUI est dédoublé par tick (coalescence de `requestRender`).
- L'animation du loader est à intervalle fixe (80ms), chaque frame demandant un rendu.
- Les mises à jour d'autocomplétion de l'éditeur (dans `Editor`) utilisent des temporisateurs de dédoublement, réduisant le recalcul excessif pendant la frappe.

Le runtime mélange donc des transitions d'état pilotées par événements avec une cadence de rendu bornée pour maintenir l'interactivité réactive sans tempêtes de repeinture.
