---
title: Architecture des modules natifs
description: >-
...
3. **Couche module Rust N-API** implémente les primitives critiques en performance exportées vers JS.

Ce document constitue la base pour la documentation plus détaillée au niveau des modules.
...
`packages/natives/src/index.ts` est le barrel public. Il regroupe les exports par domaine de capacité et réexporte des wrappers typés plutôt que d'exposer directement les bindings N-API bruts.
...
- `NativeBindings` commence avec les membres partagés (`cancelWork(id: number)`)
- Les bindings spécifiques aux modules sont ajoutés par fusion de déclarations depuis le fichier `types.ts` de chaque module
...
- L'identifiant de plateforme est `"${process.platform}-${process.arch}"`.
- Les identifiants supportés sont actuellement :
  - `linux-x64`
  - `linux-arm64`
  - `darwin-x64`
  - `darwin-arm64`
...
Pour les binaires compilés (marqueurs `PI_COMPILED` ou runtime embarqué Bun), le comportement du chargeur est :
...
2. Vérifier l'emplacement hérité des binaires compilés :
   - Windows : `%LOCALAPPDATA%/xcsh` (repli vers `%USERPROFILE%/AppData/Local/xcsh`)
   - non-Windows : `~/.local/bin`
3. Se replier sur le répertoire `native/` du package et les candidats dans le répertoire de l'exécutable
...
- **Identifiant de plateforme non supporté** : lève une exception avec la liste des plateformes supportées
...
**Contrat garanti (côté API) :** le chargement de l'addon soit réussit avec un ensemble de bindings validé, soit échoue rapidement avec un message d'erreur actionnable.
...
Ces modules implémentent les symboles N-API consommés et validés par `native.ts`. Les noms côté JS sont exposés via les wrappers TS dans `packages/natives/src`.

**Contrat garanti (côté API) :** les exports des modules Rust doivent correspondre aux noms de bindings attendus par `validateNative` et les modules wrapper.

**Détail d'implémentation (susceptible de changer) :** la décomposition interne des modules Rust et les limites des modules utilitaires (`glob_util`, `task`, etc.).
...
- **Identifiant de plateforme** : Tuple runtime `platform-arch` (par exemple `darwin-arm64`).
- **Variante** : Saveur de build spécifique au CPU x64 (`modern` AVX2, `baseline` solution de repli).
...
- **Mode binaire compilé** : Mode d'exécution où le CLI est regroupé et les addons natifs sont résolus depuis des chemins extraits/en cache plutôt qu'uniquement depuis les chemins locaux au package.
- **Addon embarqué** : Métadonnées d'artefact de build et références de fichiers générées dans `embedded-addon.ts` afin que les binaires compilés puissent extraire les payloads `.node` correspondants.
