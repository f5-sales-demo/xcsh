...
| --- | --- | --- | --- |
| `Native addon missing exports ... Missing: <name>` | Binaire `.node` obsolète, export Rust incorrect ou mauvais binaire | Utiliser `PI_DEV=1` pour voir le chemin ; vérifier les exports | Recompiler ; aligner l'export ou alias `#[napi]` sur la clé JS ; supprimer les fichiers obsolètes |
...
| Le binaire compilé échoue après une mise à niveau | Cache obsolète (`~/.xcsh/natives/<ancienne-version-ou-version-incompatible>`) ou manifeste incompatible | Vérifier le répertoire versionné et les erreurs du chargeur | Supprimer le cache de cette version et relancer ; regénérer le manifeste embarqué |
