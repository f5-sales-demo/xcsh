---
title: Runtime du chargeur d'addons natifs
description: >-
  Runtime du chargeur d'addons N-API avec détection de plateforme, stratégies de
  repli et résolution de modules.
sidebar:
  order: 3
  label: Chargeur d'addons
i18n:
  sourceHash: 1bcb4f2bbe71
  translator: machine
---

# Runtime du chargeur d'addons natifs

Ce document explore en profondeur la couche de chargement/validation des addons dans `@f5xc-salesdemos/pi-natives` : comment `native.ts` décide quel fichier `.node` charger, quand l'extraction de la charge utile embarquée s'exécute, et comment les échecs au démarrage sont signalés.
...
- Construire une liste de candidats tenant compte de la plateforme et du CPU pour les noms de fichiers et répertoires d'addons.
...
- **Mode binaire compilé** (`isCompiledBinary`) : vrai si l'un des éléments suivants est vérifié :
...
- **Variante sélectionnée** : remplacement explicite, sinon détection AVX2 au runtime sur x64 (`modern` si AVX2, sinon `baseline`).
...
1. Si `PI_NATIVE_VARIANT` vaut `modern` ou `baseline`, cette valeur est prioritaire.
...
## Construction du chemin des candidats et ordre de repli
...
4. Retourner le chemin cible pour la tentative de chargement prioritaire.

En cas d'échec, l'extraction ne plante pas immédiatement ; elle ajoute une entrée d'erreur (échec de création de répertoire ou d'écriture) et le chargeur poursuit le sondage normal des candidats.
...
## Vérifications du contrat `validateNative`

`validateNative(bindings, source)` applique un contrat exclusivement fonctionnel sur `NativeBindings` au démarrage.

Mécanisme :

- Pour chaque nom d'export requis, il vérifie `typeof bindings[name] === "function"`.
- Les noms manquants sont agrégés.
- Si certains sont manquants, le chargeur lève une exception avec :
  - le chemin de l'addon source,
  - la liste des exports manquants,
  - une indication de commande de reconstruction.

C'est une porte de compatibilité stricte contre les binaires obsolètes, les builds partiels et la dérive de symboles/noms.
...
- Ancien binaire `.node` provenant d'une version/forme d'API de package précédente.
...
- la remédiation consistant à supprimer le `<versionedDir>` obsolète et relancer,
- des commandes `curl` de téléchargement direct de release pour chaque nom de fichier attendu.
