...
Cela maintient une interface de liaison agrégée unique sans fichier de types monolithique central.
...
- Chaque fichier `src/<module>/types.ts` augmente `NativeBindings`.
- `src/native.ts` importe tous les fichiers `./<module>/types` pour leurs effets de bord afin que le contrat fusionné soit en portée là où `NativeBindings` est utilisé.
...
Transition d'état : **Objet addon non fiable** → **Objet de liaison natif validé** (ou échec fatal).
...
- **Comportement d'environnement ou de politique autour des appels natifs**
  - Le wrapper clipboard ajoute la gestion OSC52/Termux/headless et traite la copie comme un effort au mieux.
...
## Différences contractuelles synchrone vs asynchrone
...
- **Exports asynchrones basés sur les Promises** pour les opérations d'E/S ou de longue durée (`grep`, `glob`, `htmlToMarkdown`, `executeShell`, clipboard, opérations sur les images).
- **Exports synchrones** pour les transformations/parseurs déterministes en mémoire (`search`, `hasMatch`, coloration syntaxique, largeur/découpe de texte, parsing de touches, requêtes de processus).
- **Exports de constructeurs** pour les objets d'exécution à état (`Shell`, `PtySession`, `PhotonImage`).
...
## Patterns de typage des objets et des enums
...
Ce sont des contrats structurels à la compilation ; la correction de la forme à l'exécution est sous la responsabilité de l'implémentation native.
...
Les appelants voient des membres d'enum nommés ; la frontière de liaison transmet des nombres.
...
1. **Vérifications du contrat TypeScript à la compilation**
   - Les wrappers appellent `native.<name>` contre le `NativeBindings` fusionné.
...
2. **Validation à l'exécution dans `validateNative`**
   - Après le chargement, `native.ts` vérifie les exports requis et lève une exception si certains sont manquants.
...
### Échecs de chargement/validation (échecs fatals)

- L'échec du chargement de l'addon ou une plateforme non supportée lève une exception lors de l'initialisation du module dans `native.ts`.
...
- Certains wrappers adoucissent intentionnellement les échecs (`copyToClipboard` fonctionne au mieux et absorbe l'échec natif).
- Les callbacks de streaming ignorent les payloads d'erreur des callbacks et ne transmettent que les événements de valeurs réussies.

### Mises en garde au niveau des types (l'exécution est plus stricte que TS)

- Les champs optionnels TS ne garantissent pas la validité sémantique ; la couche native peut toujours rejeter des valeurs mal formées.
- Le typage `const enum` n'empêche pas les valeurs numériques hors limites provenant d'appelants non typés à l'exécution.
- `validateNative` vérifie uniquement la présence et le caractère fonctionnel des exports requis, pas la compatibilité profonde des formes d'arguments/retours.
...
## Liste de contrôle pour les mainteneurs lors de modifications de liaisons
...
5. Ré-exports publics de `src/index.ts`
