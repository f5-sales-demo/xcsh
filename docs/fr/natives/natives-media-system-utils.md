...
description: >-
  ...
  gestion des images et les informations système.
sidebar:
  order: 7
  label: Utilitaires médias et système
i18n:
...
Ce document est une analyse approfondie du sous-système pour la couche de **primitives système/médias/conversion** décrite dans [`docs/natives-architecture.md`](./natives-architecture.md) : `image`, `html`, `clipboard` et profilage `work`.
...
> Remarque : il n'existe pas de `crates/pi-natives/src/work.rs` ; le profilage du travail est implémenté dans `prof.rs` et alimenté par l'instrumentation dans `task.rs`.
...
- **Frontière d'entrée JS** : `Uint8Array` d'octets d'image encodés.
...
- `quality` est utilisé uniquement pour JPEG.
...
Comportement de la conversion :

- `cleanContent` vaut `false` par défaut.
- Quand `cleanContent=true`, le prétraitement est activé avec `PreprocessingPreset::Aggressive` et des indicateurs de suppression stricte pour la navigation/les formulaires.
- `skipImages` vaut `false` par défaut.

### Presse-papiers (`clipboard`)

- **Chemin texte** :
  - TS émet d'abord OSC 52 (`\x1b]52;c;<base64>\x07`) quand stdout est un TTY.
  - Le même texte est ensuite tenté via l'API native du presse-papiers (`native.copyToClipboard`) en meilleur effort.
  - Sur Termux, TS tente `termux-clipboard-set` en premier.
- **Chemin de lecture d'image** :
  ...
  - TS retourne `null` directement sur Termux ou les sessions Linux sans serveur d'affichage (`DISPLAY`/`WAYLAND_DISPLAY` absents).

### Profilage du travail (`work`)
...
3. `resize(...)` crée un nouveau handle natif (`image.resize`), l'ancien et le nouveau handles peuvent coexister.
4. `encode(...)` matérialise les octets (`image.encode`) sans muter les dimensions de l'image.
...
2. Essai de la commande Termux quand `TERMUX_VERSION` est défini.
3. Essai de la copie texte native `arboard`.
...
2. La lecture `arboard` côté Rust s'exécute uniquement quand TS l'autorise.
3. `ContentNotAvailable` est traduit en `null`.
4. Les autres erreurs Rust rejettent.

### Cycle de vie du profilage du travail

1. Pas de démarrage explicite : le profilage est toujours actif quand les helpers de tâches s'exécutent.
...
- L'omission d'options est un comportement par défaut en meilleur effort, pas un échec.
...
## Mises en garde liées aux plateformes
