...
- `src/config/model-registry.ts` — 加载内置 + 自定义模型、提供者覆盖、运行时发现、身份验证集成
...
- `packages/ai/src/models.ts` 和 `packages/ai/src/types.ts` — 内置提供者/模型和 `Model`/`compat` 类型
...
- 如果 `models.yml` 缺失但同一位置存在 `models.json`，则会迁移到 `models.yml`。
...
`configVersion` 是由自动配置系统写入的可选整数。当存在时，xcsh 使用它来检测过时的配置并自动升级。

`provider-id` 是在选择和身份验证查找中使用的规范提供者键。
...
3. 将提供者覆盖（`baseUrl`、`headers`）应用于内置模型。
...
注册表保留每个具体的提供者模型，然后在它们之上构建一个规范层。
...
3. 针对网关/提供者变体的保守启发式标准化
4. 回退到具体模型自身的 id

当前启发式方法故意保持狭窄：

- 嵌入的上游前缀在存在时可以被剥离，例如 `anthropic/...` 或 `openai/...`
- 点分和横杠分隔的版本变体仅在映射到现有官方 id 时才进行标准化，例如 `4.6 -> 4-6`
- 没有捆绑匹配或显式覆盖时，不会合并模糊的系列或版本
...
3. 如果 `modelProviderOrder` 未设置，则使用现有的注册表/提供者顺序

已禁用或未认证的提供者会被跳过。

会话状态和转录记录继续记录实际执行该轮的具体提供者/模型。
...
如果 `ollama` 未被显式配置，注册表会添加一个隐式可发现的提供者：
...
如果 `llama.cpp` 未被显式配置，注册表会添加一个隐式可发现的提供者：
...
如果 `lm-studio` 未被显式配置，注册表会添加一个隐式可发现的提供者：
...
5. ModelRegistry 回退解析器（来自 `models.yml` 的提供者 `apiKey`，环境变量名或字面量语义）

`models.yml` 中 `apiKey` 的行为：

- 值首先被视为环境变量名。
- 如果没有对应的环境变量，则字面字符串被用作令牌。

如果 `authHeader: true` 且提供者 `apiKey` 已设置，模型会获得：

- 注入 `Authorization: Bearer <resolved-key>` 头部。
...
- `getAvailable()` 过滤为无密钥或具有可解析身份验证的模型。

因此，模型可以存在于注册表中但在身份验证可用之前不可选择。
...
像 `pi/smol` 这样的角色别名通过 `settings.modelRoles` 展开。每个角色值还可以附加思考选择器，如 `:minimal`、`:low`、`:medium` 或 `:high`。

如果一个角色指向另一个角色，目标模型仍然正常继承，而引用角色上的任何显式后缀会在该角色特定使用中获胜。

相关设置：

- `modelRoles`（记录）
- `enabledModels`（范围模式列表）
...
两个界面都保持提供者前缀的模型可见和可选。

它们现在还公开规范/合并的模型：

- `/model` 在提供者标签旁包含一个规范视图
- `--list-models` 打印一个规范部分以及具体提供者行

选择规范条目会存储规范选择器。选择提供者行会存储显式的 `provider/modelId`。

## 上下文提升（模型级回退链）

上下文提升是一种针对小上下文变体（例如 `*-spark`）的溢出恢复机制，当 API 因上下文长度错误拒绝请求时，会自动提升到更大上下文的同级模型。
...
2. 同一提供者 + API 上最小的更大上下文模型

除非凭据可解析（`ModelRegistry.getApiKey(...)`），否则候选模型会被忽略。

### OpenAI Codex websocket 切换

如果从/向 `openai-codex-responses` 切换，会话提供者状态键 `openai-codex-responses` 在模型切换前关闭。这会丢弃 websocket 传输状态，以便下一轮在提升后的模型上以干净状态开始。
...
`contextPromotionTarget` 接受以下任一值：

- `provider/model-id`（显式）
- `model-id`（在当前提供者内解析）

示例（`models.yml`），同一提供者上 Spark -> 非 Spark：
...
内置模型生成器也会在存在同提供者基础模型时自动为 `*-spark` 模型分配此项。
...
这些由 OpenAI-completions 传输逻辑消费，并与基于 URL 的自动检测结合使用。
...
一个值得注意的旧版路径仍然存在：网页搜索 Anthropic 身份验证解析仍然直接在 `src/web/search/auth.ts` 中读取 `~/.xcsh/agent/models.json`。
...
- 如果无法自动修复（环境变量未设置、写入失败），注册表会继续使用内置模型运行。
- 错误通过 `ModelRegistry.getError()` 暴露并在 UI/通知中显示。
