...
description: Provider 流式实现，包括 SSE 解析、token 计数和背压处理。
sidebar:
  order: 2
  label: 流式处理内部机制
i18n:
...
本文档说明了 `@f5xc-salesdemos/pi-ai` 中 token/工具流式传输的标准化方式，以及如何通过 `@f5xc-salesdemos/pi-agent-core` 和 `coding-agent` 会话事件进行传播。

## 端到端流程

1. `streamSimple()`（`packages/ai/src/stream.ts`）映射通用选项并分发到 provider 流式函数。
2. Provider 流式函数（`anthropic.ts`、`openai-responses.ts`、`google.ts`）将 provider 原生流事件转换为统一的 `AssistantMessageEvent` 序列。
3. 每个 provider 将事件推送到 `AssistantMessageEventStream`（`packages/ai/src/utils/event-stream.ts`），该模块对 delta 事件进行节流并暴露：
   - 用于增量更新的异步迭代
   - 用于获取最终 `AssistantMessage` 的 `result()`
4. `agentLoop`（`packages/agent/src/agent-loop.ts`）消费这些事件，修改正在进行中的助手状态，并发出携带原始 `assistantMessageEvent` 的 `message_update` 事件。
5. `AgentSession`（`packages/coding-agent/src/session/agent-session.ts`）订阅代理事件，持久化消息，驱动扩展钩子，并应用会话行为（重试、压缩、TTSR、流式编辑中止检查）。

## `@f5xc-salesdemos/pi-ai` 中的统一流式契约

所有 provider 发出相同的结构（`packages/ai/src/types.ts` 中的 `AssistantMessageEvent`）：
...
- 最终结果由终结事件（`done` 或 `error`）解析
- delta 事件被批量/节流处理（~50ms）
- 缓冲的 delta 在非 delta 事件之前和完成之前被刷新

## Delta 节流与协调行为

`AssistantMessageEventStream` 将 `text_delta`、`thinking_delta` 和 `toolcall_delta` 视为可合并事件：

- 缓冲的 delta 仅在**类型 + contentIndex** 匹配时才合并
- 合并保留最新的 `partial` 快照
- 非 delta 事件强制立即刷新
...
- 每次 delta 时通过 `parseStreamingJson()` 重新解析 `arguments`
- `toolcall_end` 再次重新解析，然后剥离 `partialJson`

## OpenAI Responses (`openai-responses`)

来源：`packages/ai/src/providers/openai-responses.ts`
...
来源：`packages/ai/src/providers/google.ts`

标准化要点：

- 迭代 `candidate.content.parts`
- 文本部分通过 `isThinkingPart(part)` 区分为思考和文本
- 块转换在开始新块之前关闭先前的块
...
- 后续有效的 delta 可以恢复结构化参数，因为每次追加都会重试解析
...
如果 provider 流抛出异常或发出失败信号，每个 provider 包装器会捕获并发出终结 `error` 事件，包含：
...
对于这些 provider 路径，数据块/SSE 帧处理由供应商 SDK 流（Anthropic SDK、OpenAI SDK、Google SDK）处理。此代码在这里不实现自定义 SSE 解码器。

当前实现中观察到的行为：

- SDK 层面的格式错误数据块/SSE 解析会表现为异常或流 `error` 事件
- Provider 包装器将其转换为统一的终结 `error` 事件
- 流函数内部没有 provider 特定的恢复/重试逻辑
- 更高层级的重试在 `AgentSession` 自动重试逻辑中处理（消息级重试，而非流数据块重放）

## 取消边界

取消是分层的：

- AI provider 请求：`options.signal` 被传入 provider 客户端流调用。
- Provider 包装器：流循环之后，已中止的信号强制进入错误路径（`"Request was aborted"`）。
- 代理循环：在处理每个 provider 事件之前检查 `signal.aborted`，并可以从最新的部分内容合成一个已中止的助手消息。
- 会话/代理控制：`AgentSession.abort()` -> `agent.abort()` -> 共享中止控制器取消。

工具执行取消与模型流取消是分开的：

- 工具运行器使用 `AbortSignal.any([agentSignal, steeringAbortSignal])`
- 转向中断可以中止剩余的工具执行，同时保留已产生的工具结果
...
- 节流降低了 UI 更新速率但不会减慢 provider 摄入速度
- 如果消费者明显滞后，排队的事件可能会增长直到完成

当前设计倾向于响应性和简单排序，而非有界缓冲区流量控制。
...
- 在终结事件（`done`/`error`）时：从 `response.result()` 解析最终消息，发出 `message_end`

`AgentSession` 随后消费这些事件用于会话级行为：

- TTSR 监视 `message_update.assistantMessageEvent` 中的 `text_delta` 和 `toolcall_delta`
- 流式编辑守卫检查 `edit` 调用中的 `toolcall_delta`/`toolcall_end`，并可提前中止
- 持久化在 `message_end` 时写入已完成的消息
- 自动重试检查助手 `stopReason === "error"` 加上 `errorMessage` 启发式规则
...
- [`../../ai/src/utils/json-parse.ts`](../../packages/ai/src/utils/json-parse.ts) — 流式工具参数的部分 JSON 解析。
...
- [`../../ai/src/providers/google.ts`](../../packages/ai/src/providers/google.ts) — Gemini 流数据块到块的转换。
