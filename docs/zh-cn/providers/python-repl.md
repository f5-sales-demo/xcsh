...
`python` 工具通过 Jupyter Kernel Gateway 支持的内核执行一个或多个 Python cell（而非每个 cell 直接生成 `python -c` 进程）。
...
```ts
{
  ...
  cwd?: string;
  reset?: boolean; // 仅在第一个 cell 之前重置内核
}
```
...
1. **外部网关**（设置了 `PI_PYTHON_GATEWAY_URL`）
   - 直接使用配置的 URL。
   - 可选使用 `PI_PYTHON_GATEWAY_TOKEN` 进行认证。
   - 不会生成或管理本地网关进程。
...
- 获取文件锁（`gateway.lock`）并保持心跳。
- 如果 PID 存活且健康检查通过，则复用 `gateway.json`。
- 在需要时清理过期的信息/PID。
- 当没有健康的网关存在时启动新网关。

`releaseSharedGateway()` 当前为空操作（内核关闭不会终止共享网关）。
...
每次执行使用通过所选网关上的 `POST /api/kernels` 创建的内核。
...
cell 在该工具调用的同一个内核实例中按顺序运行。

如果中间某个 cell 失败：

- 之前 cell 的状态保留在内存中。
- 工具返回定向错误，指示哪个 cell 失败。
- 后续 cell 不会执行。

`reset=true` 仅适用于该调用中第一个 cell 的执行。

## 环境过滤与运行时解析

在启动网关/内核运行时之前会过滤环境变量：

- 允许列表包括核心变量，如 `PATH`、`HOME`、locale 变量、`VIRTUAL_ENV`、`PYTHONPATH` 等。
- 允许前缀：`LC_`、`XDG_`、`PI_`
- 拒绝列表剥离常见 API 密钥（OpenAI/Anthropic/Gemini 等）

运行时选择顺序：

1. 活动的/定位到的 venv（`VIRTUAL_ENV`，然后 `<cwd>/.venv`，`<cwd>/venv`）
2. 托管的 venv：`~/.xcsh/python-env`
3. PATH 上的 `python` 或 `python3`

当选择了 venv 时，其 bin/Scripts 路径会被添加到 `PATH` 最前面。

Python 内部的内核环境初始化还会：

- `os.chdir(cwd)`
- 将提供的环境变量映射注入 `os.environ`
- 确保 cwd 在 `sys.path` 中
...
如果 Python 预检失败，该会话的工具创建会降级为 bash-only。
...
- 执行被标记为已取消。
- 通过 REST（`POST /interrupt`）和控制通道 `interrupt_request` 尝试内核中断。
- 结果包含 `cancelled=true`。
- 超时路径将输出注释为 `Command timed out after <n> seconds`。
...
输出通过 `OutputSink` 流式传输，可能会持久化到制品存储中。

工具结果可以包含截断元数据和 `artifact://<id>` 用于完整输出恢复。
...
- **内核可用性错误**
  - 本地模式要求在已解析的 Python 运行时中 `kernel_gateway` 和 `ipykernel` 均可导入。
  - 安装方式：
...
- **外部网关认证/可达性失败**
  ...
  - 超时/不可达 -> 验证 URL/网络和网关健康状态。

- **执行挂起然后超时**
  - 如果工作负载是合理的，增加工具 `timeout`（最大 600 秒）。
  - 对于卡住的代码，取消操作会触发内核中断，但用户代码可能仍需重构。

- **Python 代码中的 stdin/input 提示**
