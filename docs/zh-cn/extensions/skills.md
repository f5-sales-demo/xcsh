---
title: 技能
description: 用于在编码代理中注册、发现和调用专业能力的技能系统。
sidebar:
  order: 3
  label: 技能
i18n:
...
技能是基于文件的能力包，在启动时被发现并以以下方式暴露给模型：

- 系统提示词中的轻量级元数据（名称 + 描述）
- 通过 `read skill://...` 按需加载的内容
- 可选的交互式 `/skill:<name>` 命令

本文档涵盖了 `src/extensibility/skills.ts`、`src/discovery/builtin.ts`、`src/internal-urls/skill-protocol.ts` 和 `src/discovery/agents-md.ts` 中的当前运行时行为。
...
对于基于提供者的发现（native/Claude/Codex/Agents/plugin 提供者），技能被发现为 **`skills/` 下的一级目录**：
...
<root>/skills/
  ...
  └─ team/
      └─ internal/
          └─ SKILL.md  ❌ 不会被提供者加载器发现

自定义目录扫描也是非递归的，因此嵌套路径会被忽略，除非你将 `customDirectories` 指向该嵌套的父目录。
```
...
如果 `skills.enabled` 为 `false`，发现将不返回任何技能。

### 内置技能提供者和优先级

提供者排序优先级优先（高优先级胜出），同优先级按注册顺序。
...
- 能力去重已经为每个名称保留第一个技能（最高优先级提供者）
- `extensibility/skills.ts` 额外执行：
  - 通过 `realpath` 对相同文件进行去重（符号链接安全）
  - 当后续技能名称冲突时发出冲突警告
  - 保留便捷的 `discoverSkillsFromDir({ dir, source })` API 作为 `scanSkillsFromDir` 的薄封装
...
系统提示词构建（`src/system-prompt.ts`）使用已发现的技能如下：
...
任务工具子代理通过正常的会话创建接收会话的已发现/提供的技能列表；没有针对每个任务的技能固定覆盖。
...
- 剥离前置元数据
- 将技能正文作为后续自定义消息注入
- 附加元数据（`Skill: <path>`，可选的 `User: <args>`）
...
## 技能与 AGENTS.md、命令、工具、钩子的比较

### 技能与 AGENTS.md

- **技能**：命名的、可选的能力包，根据任务上下文选择或显式请求
- **AGENTS.md/上下文文件**：作为上下文文件能力加载的持久指令文件，按层级/深度规则合并

`src/discovery/agents-md.ts` 专门从 `cwd` 向上遍历祖先目录以发现独立的 `AGENTS.md` 文件（最多深度 20），排除隐藏目录段。

### 技能与斜杠命令

- **技能**：模型可读的知识/工作流内容
- **斜杠命令**：用户调用的命令入口点
- `/skill:<name>` 是一个注入技能文本的便捷封装；它不改变技能发现语义
