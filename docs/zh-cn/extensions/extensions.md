...
3. `ExtensionRunner.initialize(...)` 为当前模式连接实时操作/上下文。
...
`pi.sendUserMessage(content, { deliverAs })` 始终通过提示流程；在流式传输期间会作为 steer/follow-up 排队。
...
当前运行时说明：`ExtensionRunner.emitResourcesDiscover(...)` 已实现，但在当前代码库中没有 `AgentSession` 调用点调用它。
...
另请注意：`setWidget` 当前通过 `setHookWidget(...)` 路由到状态行文本。
...
- 即发即忘方法发送请求（`notify`、`setStatus`、用于字符串数组的 `setWidget`、`setTitle`、`setEditorText`）
...
后台模式安装了一个非交互式 UI 上下文对象。在当前实现中，`ctx.hasUI` 可能仍为 `true`，而交互式对话框返回默认值/空操作行为。
...
在 `registerTool` 定义上提供 `renderCall` / `renderResult` 以在 TUI 中实现自定义工具可视化。
...
如果你需要一个包同时管理策略、工具、命令用户体验和渲染，请使用扩展。
