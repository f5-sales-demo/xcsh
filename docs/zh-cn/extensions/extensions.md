...
4. 会话/代理/工具生命周期事件被发送到处理器。
5. 每个工具执行都通过扩展拦截进行包装（`tool_call` / `tool_result`）。
...
- `triggerTurn: true` — 空闲时启动一个回合（`nextTurn` 忽略此选项）

`pi.sendUserMessage(content, { deliverAs })` 始终通过提示流程传递；在流式传输期间，它会作为 steer/follow-up 排队。

## 2) 处理器上下文（`ExtensionContext`）

处理器和工具的 `execute` 接收的 `ctx` 包含：
...
使用命令上下文进行会话控制流程；这些方法被有意地与通用事件处理器分离。

## 事件接口（当前名称和行为）

规范的事件联合类型和载荷类型在 `types.ts` 中定义。
...
`tool_result` 是中间件风格的：处理器按扩展顺序运行，每个处理器可以看到之前的修改。
...
当前运行时备注：`ExtensionRunner.emitResourcesDiscover(...)` 已实现，但当前代码库中没有 `AgentSession` 调用点调用它。
...
一旦注册表在 `sdk.ts` 中被包装，`tool_call`/`tool_result` 会拦截所有工具，包括内置工具和扩展/自定义工具。
...
另外注意：`setWidget` 当前通过 `setHookWidget(...)` 路由到状态行文本。

### RPC 模式（`rpc-mode.ts`）

`ctx.ui` 由 RPC `extension_ui_request` 事件支持：

- 对话框方法（`select`、`confirm`、`input`、`editor`）与客户端响应进行往返通信
- 触发即忘方法发出请求（`notify`、`setStatus`、`setWidget`（针对字符串数组）、`setTitle`、`setEditorText`）
...
后台模式安装一个非交互式 UI 上下文对象。在当前实现中，`ctx.hasUI` 可能仍为 `true`，而交互式对话框返回默认值/空操作行为。
...
2. 在 `session_start`、`session_branch`、`session_tree` 事件中通过 `ctx.sessionManager.getBranch()` 重建状态。
3. 当状态应从工具结果历史中可见/可重建时，保持工具结果 `details` 的结构化。
...
在 `registerTool` 定义中提供 `renderCall` / `renderResult`，用于 TUI 中的自定义工具可视化。

## 约束和注意事项

- 运行时操作在扩展加载期间不可用。
- `tool_call` 错误会阻止执行（失败即关闭）。
- 与内置命令冲突的命令名称会被跳过并输出诊断信息。
...
如果您需要一个包同时拥有策略、工具、命令用户体验和渲染，请使用扩展。
