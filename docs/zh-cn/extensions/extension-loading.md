...
description: 扩展的 TypeScript 和 JavaScript 模块加载管线，包含解析、验证和缓存。
sidebar:
  order: 2
  label: 扩展加载
i18n:
...
`discoverAndLoadExtensions()` 首先向发现提供者请求具有 `extension-module` 能力的项目，然后仅保留 `native` 提供者的项目。
...
- 原生自动发现目前基于 `.xcsh`。
- 旧版 `.pi` 在 `package.json` 清单键（`pi.extensions`）中仍然被接受，但此处不作为原生根目录使用。
...
`disabledExtensions` 设置通过扩展 ID 格式进行过滤：
...
1. 该目录中包含 `xcsh.extensions`（或旧版 `pi.extensions`）的 `package.json` -> 使用声明的入口
2. `index.ts`
3. `index.js`
4. 否则扫描一级目录查找扩展入口：
   - 直接的 `*.ts` / `*.js`
...
含义：如果同一模块路径既被自动发现又被显式配置，则仅在第一个位置（自动发现阶段）加载一次。
...
每个扩展路径的失败被捕获为 `{ path, error }`，不会阻止其他路径的加载。
...
- 在加载期间，运行时操作方法会故意抛出 `ExtensionRuntimeNotInitializedError`；操作连接会在稍后的 `ExtensionRunner.initialize()` 中进行。

### 加载之后

当事件通过 `ExtensionRunner` 运行时，处理器异常会被捕获并作为扩展错误发出，而不是导致运行器循环崩溃。
