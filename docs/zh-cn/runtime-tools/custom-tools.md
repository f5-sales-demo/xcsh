...
自定义工具是一个 TypeScript/JavaScript 模块，导出一个工厂函数。该工厂函数接收一个宿主 API（`CustomToolAPI`）并返回一个工具或一组工具。

## 这是什么（以及不是什么）

- **自定义工具**：在对话轮次中可被模型调用（`execute` + TypeBox 模式）。
...
1. **SDK 提供的自定义工具**（`options.customTools`）
   - 通过 `CustomToolAdapter` 或扩展包装器封装为代理工具。
   - 在 SDK 引导过程中始终包含在初始活跃工具集中。

2. **通过加载器 API 进行文件系统发现的模块**（`discoverAndLoadCustomTools` / `loadCustomTools`）
   - 在 `src/extensibility/custom-tools/loader.ts` 中作为库 API 暴露。
   - 宿主代码可以调用这些 API 从配置/提供者/插件路径发现和加载工具模块。
...
- 重复的已解析路径会被去重。
- 与内置工具和已加载的自定义工具存在名称冲突的工具会被拒绝。
- 某些提供者会将 `.md` 和 `.json` 文件作为工具元数据发现，但可执行模块加载器会拒绝将它们作为可运行工具。
- 相对配置路径从 `cwd` 解析；`~` 会被展开。
...
加载器以空操作 UI 上下文启动，需要宿主代码在真实 UI 就绪时调用 `setUIContext(...)`。
...
- `ctx` 包含会话/模型状态和 `abort()` 辅助函数。
- `signal` 携带取消信号。

`CustomToolAdapter` 将此桥接到代理工具接口，并以正确的参数顺序转发调用。

## 工具如何暴露给模型

- 工具被封装为 `AgentTool` 实例（`CustomToolAdapter` 或扩展包装器）。
- 它们按名称插入到会话工具注册表中。
- 在 SDK 引导过程中，自定义工具和扩展注册的工具会被强制包含在初始活跃集中。
...
- 使用扩展包装器时，`tool_result` 处理器可以进一步重写内容/详情，甚至覆盖错误状态。
...
- `onSession` 错误会被捕获并作为警告记录；它们不会导致会话崩溃。

## 设计时需要考虑的实际约束

- 工具名称在活跃注册表中必须全局唯一。
- 优先在 `details` 中使用确定性的、符合模式的输出，以便渲染器/状态重建。
- 使用 `pi.hasUI` 保护 UI 使用。
