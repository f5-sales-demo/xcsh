...
xcsh 通过**上下文**连接到 F5 Distributed Cloud——上下文是绑定了租户 URL、API 令牌和命名空间的命名凭据集。如果您使用过 `kubectl config use-context` 或 `kubectx`，工作流程完全相同：创建上下文，按名称在它们之间切换，使用 `-` 快速切回。
...
激活后，xcsh 会将租户凭据注入您的会话中。代理现在可以发起 F5 XC API 调用，状态栏会显示当前活动的上下文。
...
| `/context delete <name> --confirm` | 删除上下文（需要 `--confirm` 确认） |
| `/context rename <old> <new>` | 重命名上下文 |
| `/context validate <name>` | 测试凭据而不切换 |
...
每个上下文都有一个默认命名空间。无需更改上下文即可切换命名空间：
...
上下文可以携带额外的环境变量，这些变量会在激活时注入您的会话。适用于不属于凭据集的租户级别配置。
...
输入 `/context ` 然后按 Tab 键。下拉菜单会显示：

1. **上下文名称**——附带租户 URL 提示，方便您区分不同租户
2. **`-`**——在您之前切换过时出现，显示将会切换到哪个上下文
3. **子命令**——`list`、`create`、`delete` 等

上下文名称排在最前面，因为切换是最常见的操作。

子命令级别的补全同样有效：`/context activate <Tab>` 补全上下文名称，`/context namespace <Tab>` 补全命名空间，`/context unset <Tab>` 补全已知的环境变量键名。
...
如果在启动 xcsh 之前在 shell 环境中设置了 `F5XC_API_URL` 和 `F5XC_API_TOKEN`，它们将优先于任何上下文。这在 CI/CD 流水线或不需要创建持久上下文的一次性会话中非常有用。

在此模式下运行时，`/context` 会显示环境变量来源的凭据，并带有 `(via env vars)` 标签。

## 上一个上下文的行为

- **会话级别作用域**：上一个上下文在重启 xcsh 时会重置，不会持久化到磁盘。
- **乒乓切换**：连续 `/context -` 两次会让您回到最初的位置。
- **跨变更安全**：如果您删除了上一个上下文，指针会被清除。如果您重命名了它，指针会跟随新名称。
- **重复激活为空操作**：当已在 `production` 上下文时执行 `/context production` 不会重置上一个上下文指针。

## 设计惯例

`/context` 的用户体验遵循：

- **kubectx**：`kubectx <name>` 用于切换，`kubectx -` 用于切回上一个，单独 `kubectx` 用于列出
- **kubectl**：`kubectl config use-context` 用于显式切换
- **Shell**：`cd -` / `OLDPWD` 用于上一个目录追踪
