...
description: Resolve 工具运行时，用于文件路径解析、内容获取和基于 URL 的资源访问。
sidebar:
  order: 3
  label: Resolve 工具
i18n:
...
本文档解释了预览/应用工作流在 coding-agent 中的建模方式，以及自定义工具如何通过 `pushPendingAction` 参与其中。
...
`resolve` 总是首先消费**最顶部**的待处理操作（`pop()`），因此多个产生预览的工具按注册的逆序进行解析。

## 内置生产者示例（`ast_edit`）

`ast_edit` 首先预览结构化替换。当预览包含替换且尚未应用时，它会推入一个待处理操作，包含：

- label（人类可读的摘要）
- `sourceToolName`（`ast_edit`）
- `apply(reason: string)` 回调，以 `dryRun: false` 重新运行 AST 编辑

`resolve(action="apply", reason="...")` 将 `reason` 传入此回调。
...
- `label: string`（必需）
- `apply(reason: string): Promise<AgentToolResult<unknown>>`（必需）——在应用时调用；`reason` 是传递给 `resolve` 的字符串
...
如果运行时没有待处理操作存储，`pushPendingAction` 会抛出：

- `Pending action store unavailable for custom tools in this runtime.`

## 工具选择行为

当 `PendingActionStore.hasPending` 为 true 时，代理运行时会将工具选择偏向 `resolve`，以确保待处理的预览在正常工具流程继续之前被明确完成。

## 开发者指南

- 仅对需要支持明确应用/丢弃的破坏性或高影响操作使用待处理操作。
- 保持 `label` 简洁且具体；它会显示在 resolve 渲染器的输出中。
- 确保 `apply(reason)` 具有确定性且足够幂等以支持一次性执行；`reason` 仅为信息性参数，不应改变行为。
- 当丢弃操作需要清理（临时状态、锁、通知）时实现 `reject(reason)`；对于无状态预览，默认消息即可满足需求时可省略。
