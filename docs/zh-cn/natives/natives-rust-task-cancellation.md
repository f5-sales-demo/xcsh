...
description: Rust 异步任务执行模型，支持协作式取消和清理语义。
sidebar:
  order: 5
  label: 任务取消
i18n:
...
本文档描述 `crates/pi-natives` 如何调度原生工作，以及取消操作如何从 JS 选项（`timeoutMs`、`AbortSignal`）流向 Rust 执行层。
...
3. `CancelToken` / `AbortToken` / `AbortReason`
   ...
   - `CancelToken::heartbeat()` 是阻塞循环的协作式取消机制。
...
- 同步 PTY 循环内部逻辑（通过 `spawn_blocking` 调用 `run_pty_sync`）
- 剪贴板/图像/HTML 转换
...
- 任务竞争（`tokio::select!`），在完成和取消之间竞争

行为：

- Future 可以将正常完成与 `ct.wait()` 进行竞争。
- 在取消路径上，异步实现通常会将取消传播到内部子系统（例如 `tokio_util::CancellationToken`），并可选地在宽限超时后强制中止。

## JS API ↔ Rust 导出映射（任务/取消相关）

| JS 接口 API | Rust 导出 (`#[napi]`) | 调度器 | 取消接入方式 |
|---|---|---|---|
...
| `pty.start(options, onChunk?)` | `PtySession::start` | `task::future(env, "pty.start", ...)` + 内部 `spawn_blocking` | `CancelToken` 在同步 PTY 循环中通过 `heartbeat()` 检查 |
...
- **启动前 / 首次取消检查前**：
  - 使用 `task::future` 的用户在 `ct.wait()` 上竞争时，一旦进入 `select!` 就可以立即解析取消。
  - 使用 `task::blocking` 的用户仅在闭包代码到达 `heartbeat()` 时才能观察到取消。如果闭包没有尽早进行心跳检查，取消将会延迟。
...
`heartbeat()` 必须在具有无界或大量工作集的循环中以可预测的节奏运行。

已观察到的模式：

- `glob::filter_entries`：在过滤/匹配之前检查每个条目。
- `fd::score_entries`：检查每个扫描的候选项。
- `grep_sync`：在繁重搜索阶段之前进行显式取消检查，加上同样接收 token 的文件系统缓存调用。
- `run_pty_sync`：每次循环 tick 检查（约 16ms 休眠节奏），取消时终止子进程。

实践规则：遍历外部大小输入的循环不应在没有心跳的情况下超过一个短的有界时间间隔。
...
3. 一些 API 有意返回结构化的取消结果而非拒绝（`ShellRunResult`/`ShellExecuteResult` 带有 `cancelled`/`timed_out` 标志和 `exit_code: None`）。

### 取消报告的区分

- **中止作为错误**：大多数使用 `heartbeat()?` 的阻塞导出。
- **中止作为类型化结果**：shell/pty 风格的命令 API，在结果结构中建模取消。
...
1. **阻塞循环中缺少心跳**
   - 症状：timeout/signal 似乎被忽略，直到循环结束。
   - 修复：在循环顶部和每个昂贵的逐项步骤之前添加 `ct.heartbeat()?`。

2. **长时间不可取消的区段**
   - 症状：在单个大型调用（解码、排序、压缩等）期间取消延迟激增。
   - 修复：将工作拆分为带有心跳边界的块；如果不可能，记录延迟情况。

3. **阻塞异步执行器**
   - 症状：当同步密集代码直接在 future 中运行时，异步 API 停滞。
   - 修复：将 CPU/同步代码块移至 `task::blocking` 或 `tokio::task::spawn_blocking`。

4. **不一致的取消语义**
   - 症状：一个 API 在取消时拒绝，另一个用标志解析，使调用者困惑。
   - 修复：按领域标准化，并保持包装器文档一致。

5. **嵌套异步任务中遗忘取消桥接**
   - 症状：外部 token 已取消，但内部读取器/子进程任务继续运行。
   - 修复：将取消桥接到内部 token/signal，并强制执行宽限超时 + 强制中止回退。
...
7. 验证没有执行器滥用：
   - 不要在异步 future 中直接执行长时间同步工作，应使用 `spawn_blocking`/阻塞任务包装器
