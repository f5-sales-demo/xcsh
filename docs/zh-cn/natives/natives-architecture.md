...
`@f5xc-salesdemos/pi-natives` 是一个三层架构栈：

1. **TypeScript 封装/API 层** 暴露稳定的 JS/TS 入口点。
2. **插件加载/验证层** 解析并验证当前运行时对应的 `.node` 二进制文件。
...
- `NativeBindings` 从共享成员开始（`cancelWork(id: number)`）
- 模块特定的绑定通过每个模块的 `types.ts` 进行声明合并来添加
- `Cancellable` 为暴露取消功能的封装器标准化了超时和中止信号选项

**保证契约（面向 API）：** 消费者从 `@f5xc-salesdemos/pi-natives` 导入并使用类型化的封装器。

**实现细节（可能变更）：** 声明合并和内部封装器布局（`src/<module>/index.ts`、`src/<module>/types.ts`）。

## 第 2 层：插件加载和验证

`packages/natives/src/native.ts` 负责运行时插件选择、可选提取和导出验证。
...
`packages/natives/package.json` 在发布文件中同时包含 `src` 和 `native`。`native/` 目录存储预构建的平台制品。

对于编译后的二进制文件（`PI_COMPILED` 或 Bun 嵌入式运行时标记），加载器行为为：

1. 检查带版本的用户缓存路径：`<getNativesDir()>/<packageVersion>/...`
2. 检查旧版编译二进制文件位置：
   - Windows：`%LOCALAPPDATA%/xcsh`（回退 `%USERPROFILE%/AppData/Local/xcsh`）
   - 非 Windows：`~/.local/bin`
3. 回退到打包的 `native/` 和可执行文件目录候选项

如果存在嵌入式插件清单（由 `scripts/embed-native.ts` 生成的 `embedded-addon.ts`），`native.ts` 可以在加载前将匹配的嵌入式二进制文件物化到带版本的缓存目录中。
...
- **缺少导出**：抛出异常并附带确切的缺少名称和重建命令
- **嵌入式提取错误**：记录目录/写入失败并将其包含在最终加载诊断中

**保证契约（面向 API）：** 插件加载要么以经过验证的绑定集成功，要么快速失败并提供可操作的错误文本。

**实现细节（可能变更）：** 确切的候选搜索顺序和编译二进制文件的回退路径排序。

## 第 3 层：Rust N-API 模块层

`crates/pi-natives/src/lib.rs` 是 Rust 入口模块，声明导出的模块所有权：
...
这些模块实现了由 `native.ts` 消费和验证的 N-API 符号。JS 级别的名称通过 `packages/natives/src` 中的 TS 封装器暴露。

**保证契约（面向 API）：** Rust 模块导出必须与 `validateNative` 和封装器模块期望的绑定名称匹配。
...
- **TS 封装/API 所有权（`packages/natives/src`）**
  - 公共 API 分组、选项类型化和稳定的 JS 易用性
...
1. 消费者从 `@f5xc-salesdemos/pi-natives` 导入。
2. 封装器模块调用单例 `native` 绑定。
3. `native.ts` 为平台/架构/变体选择候选二进制文件。
4. 对于编译发行版，执行可选的嵌入式二进制文件提取。
5. 加载插件并验证导出集。
6. 封装器将类型化结果返回给调用者。
...
- **编译二进制模式**：运行时模式，其中 CLI 被打包，原生插件从提取/缓存路径而非仅从包本地路径解析。
- **嵌入式插件**：生成到 `embedded-addon.ts` 中的构建制品元数据和文件引用，以便编译后的二进制文件可以提取匹配的 `.node` 有效载荷。
- **验证关卡**：`validateNative(...)` 检查，拒绝缺少所需导出的过时/不匹配二进制文件。
