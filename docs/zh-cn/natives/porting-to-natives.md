...
这是一份将热路径迁移到 `crates/pi-natives` 并通过 JS 绑定进行连接的实用指南。它的存在是为了避免同样的错误再次发生。

## 何时进行移植

当以下任一条件为真时进行移植：

- 热路径运行在渲染循环、频繁的 UI 更新或大批量操作中。
- JS 分配占主导（字符串频繁创建销毁、正则回溯、大数组）。
- 你已经有 JS 基准版本，可以并排对比两个版本的基准测试结果。
- 工作是 CPU 密集型或阻塞 I/O，可以在 libuv 线程池上运行。
- 工作是异步 I/O，可以在 Tokio 运行时上运行（例如 shell 执行）。

避免移植依赖于 JS 专有状态或动态导入的代码。N-API 导出应该是纯粹的，数据输入/数据输出。长时间运行的工作应通过 `task::blocking`（CPU 密集型/阻塞 I/O）或 `task::future`（异步 I/O）并支持取消操作。

## 原生导出的结构

**Rust 侧：**

- 实现位于 `crates/pi-natives/src/<module>.rs`。如果添加新模块，请在 `crates/pi-natives/src/lib.rs` 中注册。
- 使用 `#[napi]` 导出；snake_case 导出会自动转换为 camelCase。仅在真正需要别名/非默认名称时使用显式 `js_name`。对结构体使用 `#[napi(object)]`。
...
- `packages/natives/src/native.ts` 加载插件，`validateNative` 强制验证必需的导出。
- `packages/natives/src/index.ts` 为 `packages/*` 中的调用方重新导出封装器。
...
- 使用 `#[napi]` 暴露它，以保持默认的 snake_case -> camelCase 映射一致性。
...
- 在 `packages/natives/src/<module>/index.ts` 中添加调用 `native` 的封装器。
...
- 使用 `bun --cwd=packages/natives run build`，如果测试时需要加载器诊断信息，请设置 `PI_DEV=1`。
...
- 如果原生版本更慢，**保留 JS** 并让原生导出闲置。
- 如果原生版本更快，将调用点切换到原生封装器。

## 痛点及如何避免

### 1) 过期的 `pi_natives.node` 阻止新导出生效

加载器优先使用 `packages/natives/native` 中带平台标签的二进制文件（`pi_natives.<platform>-<arch>.node`）。`PI_DEV=1` 现在仅启用加载器诊断信息；它不再切换到单独的开发插件文件名。还有一个回退文件 `pi_natives.node`。编译后的二进制文件提取到 `~/.xcsh/natives/<version>/pi_natives.<platform>-<arch>.node`。如果其中任何一个过期，导出将不会更新。
...
这意味着你的二进制文件过期了，Rust 导出名称（或使用的显式别名）与 JS 名称不匹配，或者导出根本没有编译进去。修复构建和命名不匹配问题，不要削弱验证。

### 3) Rust 签名不匹配

保持简单并使用拥有所有权的类型。`String`、`Vec<String>` 和 `Uint8Array` 可以正常工作。避免在公共导出中使用引用如 `&str`。如果需要结构化数据，用 `#[napi(object)]` 结构体封装。

### 4) 基准测试错误

- 不要比较不同的输入或分配。
- 保持 JS 和原生版本使用相同的输入数组。
...
- `validateNative` 通过（没有缺少的导出）。
- `NativeBindings` 在 `packages/natives/src/<module>/types.ts` 中已扩展，封装器在 `packages/natives/src/index.ts` 中已重新导出。
...
- 如果原生版本更慢，**不要切换**。保留导出供未来使用，但 TUI 应该保持在更快的路径上。
