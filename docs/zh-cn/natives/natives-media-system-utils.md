...
本文档是 [`docs/natives-architecture.md`](./natives-architecture.md) 中描述的**系统/媒体/转换原语**层的子系统深度剖析：`image`、`html`、`clipboard` 和 `work` 性能分析。
...
> 注意：不存在 `crates/pi-natives/src/work.rs` 文件；工作性能分析在 `prof.rs` 中实现，并由 `task.rs` 中的插桩代码提供数据。
...
- **内存状态**：`PhotonImage` 存储 `Arc<DynamicImage>`。
...
- 当 `cleanContent=true` 时，启用 `PreprocessingPreset::Aggressive` 预处理以及导航/表单的强制移除标志。
...
- **文本路径**：
  ...
  - 然后以尽力而为方式通过原生剪贴板 API（`native.copyToClipboard`）尝试复制相同文本。
...
- **采集边界**：性能分析样本由 `task::blocking` 和 `task::future` 中的 `profile_region(tag)` 守卫生成。
- **存储格式**：固定大小的环形缓冲区（`MAX_SAMPLES = 10_000`），存储栈路径 + 持续时间（`μs`）+ 时间戳（`μs，自进程启动`）。
...
2. 转换使用默认选项（`cleanContent=false`、`skipImages=false`）运行，除非另行指定。
...
`copyToClipboard(text)` 故意采用尽力而为和多路径策略：

1. 如果是 TTY：尝试 OSC 52 写入（base64 有效载荷）。
...
1. TS 对不支持的运行时上下文（Termux/无头 Linux）硬性拦截，返回 `null`。
2. 仅当 TS 允许时，Rust `arboard` 读取才会执行。
...
2. 每个插桩的任务作用域在守卫销毁时记录一个样本。
3. 缓冲区容量达到上限后，样本会覆盖最旧的条目。
4. `getWorkProfile(lastSeconds)` 读取一个时间窗口并生成折叠栈/摘要/SVG 产物。

失败转换：

- SVG 生成失败是软失败（`svg: null`），折叠栈和摘要仍然返回。
- 空样本窗口返回空的折叠栈数据和 `svg: null`，而非错误。
...
- 转换错误是严格失败（拒绝）。
- 选项省略是尽力而为的默认值处理，不会失败。

### 剪贴板

- 文本复制在 TS 层是尽力而为的：操作失败会被抑制。
...
- 缓冲区截断是预期行为（环形缓冲区），而非数据丢失的 bug。

## 平台注意事项

- **剪贴板文本**：OSC 52 依赖于终端支持；原生剪贴板访问依赖于桌面环境/会话。
- **剪贴板图像读取**：在 Termux 和没有显示服务器的 Linux 上被 TS 层阻止。
