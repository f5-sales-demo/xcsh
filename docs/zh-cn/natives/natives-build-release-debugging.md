...
本文遵循 `docs/natives-architecture.md` 中的架构术语：
...
- `bun scripts/build-native.ts --dev`（`dev:native`）→ debug/dev 配置构建（输出命名相同）
- `bun scripts/embed-native.ts`（`embed:native`）→ 从已构建文件生成 `src/embedded-addon.ts`
...
- 基础命令：`cargo build`
- release 模式添加 `--release`，除非传入 `--dev`
- 交叉编译目标添加 `--target <CROSS_TARGET>`

`crates/pi-natives/Cargo.toml` 声明了 `crate-type = ["cdylib"]`，因此 Cargo 生成共享库（`.so`/`.dylib`/`.dll`），然后将其复制/重命名为 `.node` 插件文件名。
...
- 交叉编译：`<root>/<crossTarget>/<profile>` 然后 `<root>/<profile>`
- 本地构建：`<root>/<profile>`
...
找到后，使用临时文件 + 重命名的原子语义安装到 `packages/natives/native/`（Windows 回退机制会显式处理锁定 DLL 的替换失败情况）。
...
- 使用 debug 配置标志，但保持标准的平台标签输出命名

`native.ts` 中运行时加载器的候选顺序：

- release 候选
- 编译模式在包本地文件之前优先使用提取/缓存候选
...
1. 加载器计算带版本的缓存目录：`<getNativesDir()>/<packageVersion>`（实际为 `~/.xcsh/natives/<version>`）
2. 如果嵌入清单匹配当前平台+版本，加载器可能将选定的嵌入文件提取到该带版本的目录
3. 运行时候选顺序包括：
   - 带版本的缓存目录
   - 旧版编译二进制目录（Windows 上为 `%LOCALAPPDATA%/xcsh`，其他平台为 `~/.local/bin`）
   - 包/可执行文件目录
4. 第一个成功加载的插件仍须通过 `validateNative`

这就是为什么打包和运行时加载器的预期必须对齐：文件名、平台标签和导出符号必须与 `native.ts` 探测和验证的内容匹配。
...
| `getWorkProfile` | `#[napi] pub fn get_work_profile(...)`（驼峰式导出） | `crates/pi-natives/src/prof.rs` |
| `invalidateFsScanCache` | `#[napi] pub fn invalidate_fs_scan_cache(...)` | `crates/pi-natives/src/fs_cache.rs` |

如果任何必需的符号缺失，加载器将快速失败并给出重新构建的提示。
...
- 无效变体配置：
  ...
  - x64 交叉编译且无显式 `TARGET_VARIANT` → 立即报错
- Cargo 构建失败：
  - 脚本输出非零退出码和 stderr
- 未找到产物：
  - 脚本打印所有已检查的配置目录
- 安装失败：
  - 显式消息；Windows 包含锁定文件提示
...
- 不支持的平台标签：
  - 抛出错误并列出支持的平台列表
- 无候选可加载：
  - 抛出完整的候选错误列表和特定模式的修复提示
- 缺少导出：
  - 抛出确切的缺失符号名称和重新构建命令
- 嵌入提取问题：
  - mkdir/写入错误会被记录并包含在最终诊断信息中

## 故障排除矩阵

| 症状 | 可能原因 | 验证方法 | 修复方案 |
| --- | --- | --- | --- |
| `Native addon missing exports ... Missing: <name>` | 过时的 `.node` 二进制文件、Rust 导出名称不匹配或加载了错误的二进制文件 | 设置 `PI_DEV=1` 运行以查看加载路径；检查该文件的导出列表 | 重新构建 `build`；确保 Rust `#[napi]` 导出名称（或需要时的显式别名）匹配 JS 键；删除过时的缓存/带版本文件 |
| x64 机器在期望 modern 时加载了 baseline | `PI_NATIVE_VARIANT=baseline`、未检测到 AVX2 或仅存在 baseline 文件 | 检查 `PI_NATIVE_VARIANT`；检查 `native/` 中是否有 `-modern` 文件 | 构建 modern 变体（`TARGET_VARIANT=modern ... build`）并确保文件已包含在发布中 |
| 交叉编译生成不可用/标签错误的二进制文件 | `CROSS_TARGET` 与 `TARGET_PLATFORM`/`TARGET_ARCH` 不匹配，或 x64 缺少 `TARGET_VARIANT` | 确认环境变量组合和输出文件名 | 使用一致的环境变量值和显式的 x64 `TARGET_VARIANT` 重新运行 |
| 升级后编译二进制文件失败 | 过时的提取缓存（`~/.xcsh/natives/<旧版本或不匹配版本>`）或嵌入清单不匹配 | 检查带版本的 natives 目录和加载器错误列表 | 删除该包版本的带版本 natives 缓存并重新运行；在打包期间重新生成嵌入清单 |
| 加载器探测多个路径且均不可用 | 平台不匹配或包 `native/` 中缺少 release 产物 | 检查 `platformTag` 与实际文件名 | 确保构建的文件名完全匹配 `pi_natives.<platform>-<arch>(-variant).node` 约定且包中包含 `native/` |
| `embed:native` 失败并提示 "Incomplete native addons" | 嵌入前未构建所需的变体文件 | 检查错误文本中的期望列表与实际找到的列表 | 先构建所需文件（x64：modern+baseline 两者都需要；非 x64：默认文件），然后重新运行 `embed:native` |
