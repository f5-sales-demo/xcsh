...
本文档定义了位于 `@f5xc-salesdemos/pi-natives` 调用方与已加载的 N-API 插件之间的 TypeScript 侧契约。
...
这样可以维护一个聚合的绑定接口，而无需一个庞大的中央类型文件。
...
- `src/native.ts` 为了副作用导入所有 `./<module>/types` 文件，使合并后的契约在使用 `NativeBindings` 的地方处于作用域内。
...
状态转换：**不受信任的插件对象** → **已验证的原生绑定对象**（或硬失败）。

### 3) 包装器调用

- `src/<module>/index.ts` 中的模块包装器调用 `native.<export>`。
- 包装器适配默认值和回调形状（将 `(err, value)` 转换为 JS API 中的仅值回调模式）。
- `src/index.ts` 将模块包装器/类型作为公开包 API 重新导出。

状态转换：**已验证的原始绑定** → **符合人体工程学的公开 API**。

## 包装器职责

包装器刻意保持轻量；它们不会重新实现原生逻辑。

主要职责：

- **参数规范化/默认值设置**
  - `glob()` 将 `options.path` 解析为绝对路径，并设置 `hidden`、`gitignore`、`recursive` 的默认值。
  - `hasMatch()` 在原生调用前填充默认标志（`ignoreCase`、`multiline`）。
- **回调适配**
  - `grep()`、`glob()`、`executeShell()` 将 `TsFunc<T>`（`error, value`）转换为用户回调，仅接收成功值。
- **围绕原生调用的环境或策略行为**
...
`packages/natives/src/index.ts` 是标准的公开桶文件。它按能力领域分组导出：

- 搜索/文本：`grep`、`glob`、`text`、`highlight`
- 执行/进程/终端：`shell`、`pty`、`ps`、`keys`
- 系统/媒体/转换：`image`、`html`、`clipboard`、`system-info`、`work`

维护者规则：如果一个包装器没有从 `src/index.ts` 重新导出，则它不属于预期的公开包表面。

## JS API ↔ 原生导出映射（代表性示例）

Rust 侧使用 N-API 导出名称（通常来自 `#[napi]` snake_case -> camelCase 转换，偶尔使用显式别名），这些名称必须与这些绑定键匹配。
...
| Shell | `Shell` | `Shell` | 类构造函数 | 不适用 |
| PTY | `PtySession` | `PtySession` | 类构造函数 | 不适用 |
...
- **同步导出**：用于确定性的内存中转换/解析器（`search`、`hasMatch`、高亮、文本宽度/切片、按键解析、进程查询）。
- **构造函数导出**：用于有状态的运行时对象（`Shell`、`PtySession`、`PhotonImage`）。

对维护者的启示：更改现有导出的同步 ↔ 异步特性是跨包装器和调用方的破坏性 API 和契约变更。
...
这能捕获常见的陈旧二进制漂移问题：包装器/类型存在，但加载的 `.node` 缺少该导出。
...
效果：包会快速失败，而不是将失败推迟到首次调用。

### 包装器级别的行为差异

- 某些包装器有意软化失败（`copyToClipboard` 是尽力而为的，会吞掉原生失败）。
- 流式回调忽略回调错误载荷，仅转发成功的值事件。

### 类型级别的注意事项（运行时比 TS 更严格）

- TS 可选字段不保证语义有效性；原生层仍然可以拒绝格式不正确的值。
- `const enum` 类型不能阻止运行时来自无类型调用方的超出范围的数字值。
- `validateNative` 仅检查所需导出的存在性/函数性，不检查深层参数/返回形状兼容性。
- `bindings.ts` 在基础接口中包含 `cancelWork(id)`，但当前运行时验证列表不强制检查该键。
...
3. `src/native.ts` 中模块类型的导入（如果是新模块）
4. `validateNative` 所需导出的检查
