...
本文档深入介绍 `@f5xc-salesdemos/pi-natives` 中的插件加载/验证层：`native.ts` 如何决定加载哪个 `.node` 文件、嵌入式负载提取何时运行，以及启动失败如何报告。
...
- 可选地将嵌入式插件提取到版本化的用户级缓存目录。
- 按确定性顺序尝试候选项。
- 通过 `validateNative` 在暴露绑定之前拒绝过时或不兼容的插件。

不在本文档范围内：模块特定的 grep/text/highlight 行为。
...
- **选定变体**：显式覆盖优先，否则在 x64 上进行运行时 AVX2 检测（支持 AVX2 则为 `modern`，否则为 `baseline`）。
...
- 不支持的平台不会被预先拒绝。
- 加载器仍会首先尝试所有计算出的候选项。
- 如果没有任何候选项加载成功，则抛出明确的不支持平台错误，并列出支持的标签。

这在保留对接近匹配情况的有用诊断信息的同时，对真正不支持的目标仍然会硬性失败。
...
`dedupedCandidates` 在保留首次出现顺序的同时去除重复项。

### 最终运行时序列

在加载时：

1. 可选的嵌入式提取候选项（如果生成）被插入到最前面。
2. 按顺序尝试剩余的去重候选项。
...
2. 如果 `<versionedDir>/<selected filename>` 已存在，则复用（不重写）。
3. 否则读取嵌入源 `filePath` 并写入目标文件。
4. 返回目标路径作为最高优先级加载尝试。
...
`validateNative(bindings, source)` 在启动时对 `NativeBindings` 强制执行仅函数契约。
...
这是针对过时二进制文件、部分构建和符号/名称漂移的硬性兼容性门控。

### JS API ↔ 原生导出映射（验证门控）

| 在 `validateNative` 中检查的 JS 绑定名称 | 预期的原生导出名称 |
| --- | --- |
| `grep` | `grep` |
| `glob` | `glob` |
...
注意：`bindings.ts` 仅声明了基础的 `cancelWork(id)` 成员；模块 `types.ts` 文件通过声明合并添加了 `validateNative` 强制执行的额外符号。

## 失败行为和诊断

## 不支持的平台

如果所有候选项都失败且 `platformTag` 不在 `SUPPORTED_PLATFORMS` 中，加载器抛出：
...
- 选择了错误的变体产物（针对 x64）。
- 新的 Rust 导出在已加载的产物中不存在。

加载器行为：

- 记录每个候选项的缺失导出失败。
- 继续探测剩余候选项。
- 如果没有候选项通过验证，最终错误包含每个尝试过的路径及其失败消息。
