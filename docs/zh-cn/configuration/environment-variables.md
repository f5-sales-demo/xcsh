...
- `packages/utils/src/**` 和 `packages/tui/src/**`（其中直接影响 coding-agent 运行时的变量）

本文档仅记录当前活跃的行为。

## 解析模型与优先级

大多数运行时查找使用 `@f5xc-salesdemos/pi-utils`（`packages/utils/src/env.ts`）中的 `$env`。
...
除非另有说明，这些变量通过 `getEnvApiKey()`（`packages/ai/src/stream.ts`）消费。
...
| `ANTHROPIC_API_KEY`             | Anthropic API 认证 | 使用 Anthropic 且无 OAuth 令牌时                           | `ANTHROPIC_OAUTH_TOKEN` 之后的回退                                                              |
...
| `OPENAI_API_KEY`                | OpenAI 认证 | 使用 OpenAI 系列提供商且未显式传递 apiKey 参数时 | 用于 OpenAI Completions/Responses 提供商                                                      |
| `GEMINI_API_KEY`                | Google Gemini 认证 | 使用 `google` 提供商模型时                                | Gemini 提供商映射的主要密钥                                                             |
| `GOOGLE_API_KEY`                | Gemini 图像工具认证回退 | 使用 `gemini_image` 工具且无 `GEMINI_API_KEY` 时            | 用于 coding-agent 图像工具回退路径                                                       |
...
| `HF_TOKEN`                      | Hugging Face 认证 | 使用 `huggingface` 提供商时                                  | 当 `HUGGINGFACE_HUB_TOKEN` 未设置时的回退                                                      |
...
| `LITELLM_API_KEY`               | LiteLLM 认证 | 使用 `litellm` 提供商时                                      | OpenAI 兼容的 LiteLLM 代理密钥。与 `LITELLM_BASE_URL` 一起设置时，启用 `models.yml` 的自动配置 |
| `LM_STUDIO_API_KEY`             | LM Studio 认证（可选） | 使用 `lm-studio` 提供商且主机需要认证时           | 本地 LM Studio 通常无需认证运行；需要密钥时任何非空令牌均可         |
| `OLLAMA_API_KEY`                | Ollama 认证（可选） | 使用 `ollama` 提供商且主机需要认证时              | 本地 Ollama 通常无需认证运行；需要密钥时任何非空令牌均可            |
...
| `OPENROUTER_API_KEY`            | OpenRouter 认证 | 使用 OpenRouter 模型时                                       | 当首选/自动提供商为 OpenRouter 时，也被图像工具使用                                  |
| `MISTRAL_API_KEY`               | Mistral 认证 | 使用 Mistral 模型时                                          |                                                                                                     |
| `ZAI_API_KEY`                   | z.ai 认证 | 使用 z.ai 模型时                                             | 也被 z.ai 网页搜索提供商使用                                                               |
...
| `QWEN_OAUTH_TOKEN`              | 通义千问 Portal 认证 | 使用 `qwen-portal` 的 OAuth 令牌时                          | 优先于 `QWEN_PORTAL_API_KEY`                                                         |
| `QWEN_PORTAL_API_KEY`           | 通义千问 Portal 认证 | 使用 `qwen-portal` 的 API 密钥时                              | `QWEN_OAUTH_TOKEN` 之后的回退                                                                   |
| `ZENMUX_API_KEY`                | ZenMux 认证 | 使用 `zenmux` 提供商时                                       | 用于 ZenMux 的 OpenAI 和 Anthropic 兼容路由                                              |
| `VLLM_API_KEY`                  | vLLM 认证/发现启用 | 使用 `vllm` 提供商（本地 OpenAI 兼容服务器）时       | 对于无认证的本地服务器，任何非空值均可                                                 |
...
| `GH_TOKEN` | Copilot 回退；网页爬虫中的 GitHub API 认证 | 在网页爬虫中：`GITHUB_TOKEN` → `GH_TOKEN` |
| `GITHUB_TOKEN` | Copilot 回退；网页爬虫中的 GitHub API 认证 | 在网页爬虫中：在 `GH_TOKEN` 之前检查 |
...
| `ANTHROPIC_CUSTOM_HEADERS` | 头信息列表字符串 | 额外头信息；格式为 `header-a: value, header-b: value` 或换行符分隔 |
...
| 变量 | 默认值 / 行为 |
|---|---|
| `AZURE_OPENAI_API_KEY` | 除非通过选项传递 API 密钥，否则必需 |
...
基础 URL 解析：选项 `azureBaseUrl` → 环境变量 `AZURE_OPENAI_BASE_URL` → 选项/环境变量资源名称 → `model.baseUrl`。

### Google Vertex AI

| 变量 | 是否必需？ | 备注 |
|---|---|---|
| `GOOGLE_CLOUD_PROJECT` | 是（除非通过选项传递） | 回退：`GCLOUD_PROJECT` |
| `GCLOUD_PROJECT` | 回退 | 用作备选项目 ID 来源 |
| `GOOGLE_CLOUD_LOCATION` | 是（除非通过选项传递） | 提供商中无默认值 |
| `GOOGLE_APPLICATION_CREDENTIALS` | 有条件 | 如果设置，文件必须存在；否则检查 ADC 回退路径（`~/.config/gcloud/application_default_credentials.json`） |
...
| 变量 | 默认值 / 行为 |
|---|---|
| `PI_AI_ANTIGRAVITY_VERSION` | 覆盖 Gemini CLI 提供商中的 Antigravity 用户代理版本标签 |
...
| `PI_CODEX_WEBSOCKET` | `1`/`true` 启用 WebSocket 传输首选项 |
...
| 变量 | 行为 |
|---|---|
| `DEBUG_CURSOR` | 启用提供商调试日志；`2`/`verbose` 可查看详细的载荷片段 |
...
| 变量 | 行为 |
|---|---|
| `PI_CACHE_RETENTION` | 如果为 `long`，在支持的提供商上启用长期保留（`anthropic`、`openai-responses`、Bedrock 保留解析） |
...
| `OPENAI_API_KEY` / 数据库中的 Codex OAuth | Codex 搜索提供商的可用性/认证 |
...
2. `models.json` 中带有 `api: "anthropic-messages"` 的提供商条目
3. `agent.db` 中的 Anthropic OAuth 凭证（不得在 5 分钟缓冲期内过期）
4. 通用 Anthropic 环境变量回退：提供商密钥（`ANTHROPIC_FOUNDRY_API_KEY`/`ANTHROPIC_OAUTH_TOKEN`/`ANTHROPIC_API_KEY`）+ 可选的 `ANTHROPIC_BASE_URL`（Foundry 模式启用时为 `FOUNDRY_BASE_URL`）
...
| `ANTHROPIC_BASE_URL` | 第 4 级认证路径的通用回退基础 URL |
...
| 变量 | 默认值 / 行为 |
|---|---|
| `PI_PY` | Python 工具模式覆盖：`0`/`bash`=`bash-only`，`1`/`py`=`ipy-only`，`mix`/`both`=`both`；无效值被忽略 |
...
| `PI_PYTHON_IPC_TRACE` | 如果为 `1`，在内核模块中启用低级 IPC 跟踪路径 |
| `VIRTUAL_ENV` | Python 运行时解析的最高优先级虚拟环境路径 |

额外的条件行为：

- 如果 `BUN_ENV=test` 或 `NODE_ENV=test`，Python 可用性检查被视为通过，预热被跳过。
- Python 环境过滤拒绝常见 API 密钥，允许安全的基础变量以及 `LC_`、`XDG_`、`PI_` 前缀。
...
| `PI_NO_TITLE`              | 如果设置（任何非空值），禁用首条用户消息的自动会话标题生成   |
| `NULL_PROMPT`              | 如果为 `true`，系统提示构建器返回空字符串                                        |
| `PI_BLOCKED_AGENT`         | 在任务工具中阻止特定的子代理类型                                                 |
| `PI_SUBPROCESS_CMD`        | 覆盖子代理启动命令（绕过 `xcsh` / `xcsh.cmd` 解析）                       |
...
| `PI_DISABLE_LSPMUX`        | 如果为 `1`，禁用 lspmux 检测/集成并强制直接 LSP 服务器启动          |
| `LITELLM_BASE_URL`         | LiteLLM 代理基础 URL。与 `LITELLM_API_KEY` 一起设置时，首次运行触发 `models.yml` 自动生成，每次启动时进行自修复 |
...
| `PI_EDIT_VARIANT`          | 如果为 `hashline`，当编辑工具可用时强制使用 hashline 读取/搜索显示模式               |
| `PI_NO_PTY`                | 如果为 `1`，禁用 bash 工具的交互式 PTY 路径                                          |

当使用 CLI `--no-pty` 时，`PI_NO_PTY` 也会被内部设置。

---

## 6）存储和配置根路径

这些变量通过 `@f5xc-salesdemos/pi-utils/dirs` 消费，影响 coding-agent 存储数据的位置。

| 变量 | 默认值 / 行为 |
|---|---|
| `PI_CONFIG_DIR` | 主目录下的配置根目录名（默认 `.xcsh`） |
| `PI_CODING_AGENT_DIR` | 代理目录的完整覆盖（默认 `~/<PI_CONFIG_DIR 或 .xcsh>/agent`） |
| `PWD` | 用于路径辅助工具中匹配规范当前工作目录 |
...
当前实现说明：`PI_BASH_NO_LOGIN`/`CLAUDE_BASH_NO_LOGIN` 会被读取，但当前 `getShellArgs()` 在两个分支中都返回 `['-l','-c']`（目前实际上是无操作的）。

---

## 8）UI/主题/会话检测（自动检测的环境变量）

这些作为运行时信号读取；通常由终端/操作系统设置而非手动配置。
...
| `PI_CLEAR_ON_SHRINK` | 如果为 `1`，当内容缩小时清除空行 |
| `PI_DEBUG_REDRAW` | 如果为 `1`，启用重绘调试日志 |
| `PI_TUI_DEBUG` | 如果为 `1`，启用深度 TUI 调试转储路径 |
...
| 变量 | 行为 |
|---|---|
| `PI_COMMIT_TEST_FALLBACK` | 如果为 `true`（不区分大小写），强制使用提交回退生成路径 |
...
请将这些变量视为机密信息；不要记录或提交它们：

- 提供商/API 密钥和 OAuth/持有者凭证（所有 `*_API_KEY`、`*_TOKEN`、OAuth 访问/刷新令牌）
- 云凭证（`AWS_*`、`GOOGLE_APPLICATION_CREDENTIALS` 路径可能暴露服务账号材料）
- 搜索/提供商认证变量（`EXA_API_KEY`、`BRAVE_API_KEY`、`PERPLEXITY_API_KEY`、Anthropic 搜索密钥）
- Foundry mTLS 材料（`CLAUDE_CODE_CLIENT_CERT`、`CLAUDE_CODE_CLIENT_KEY`、当 `NODE_EXTRA_CA_CERTS` 指向私有 CA 包时）
