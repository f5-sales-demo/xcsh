...
本指南是将 pi-mono 中的更改移植到本仓库的可重复检查清单。
适用于任何合并：单个文件、功能分支或完整版本同步。
...
- 优先使用干净、聚焦的 diff，而非整体复制。
- 避免复制构建产物或生成的文件。
- 如果上游添加了新文件，请显式添加并审查内容。

## 2) 匹配导入扩展名约定

大多数运行时 TypeScript 源文件在内部导入中省略 `.js`，但某些测试/基准测试入口文件保留 `.js` 以兼容 ESM
运行时。遵循本地包的现有风格；不要一刀切地去除扩展名。

- 在 `packages/coding-agent` 运行时源文件中，保持内部导入无扩展名，除非导入非 TS 资源。
- 在 `packages/tui/test` 和 `packages/natives/bench` 中，在周围文件已使用 `.js` 的地方保留 `.js`。
- 当工具链要求时保留真实文件扩展名（例如 `.json`、`.css`、`.md` 文本嵌入）。
...
我们运行在 Bun 上。仅在 Bun 提供更好替代方案时替换 Node API。

**应当替换：**

- 进程生成：`child_process.spawn` → 对简单命令使用 Bun Shell `$`，对流式或长时间运行的工作使用 `Bun.spawn`/`Bun.spawnSync`
...
- `fs.mkdtempSync()` — 不要替换为手动路径构建
- `path.join()`、`path.resolve()` 等 — 这些没问题

**导入风格：** 仅使用 `node:` 前缀配合命名空间导入（不要从 `node:fs` 或 `node:path` 使用具名导入）。

**其他 Bun 约定：**

- 对简短的非流式命令优先使用 Bun Shell `$`；仅在需要流式 I/O 或进程控制时使用 `Bun.spawn`。
- 对文件使用 `Bun.file()`/`Bun.write()`，对目录使用 `node:fs/promises`。
...
- 除非移植需要更改，否则保留现有的 `name`、`version`、`type`、`exports` 和 `bin`。
...
- 保持现有格式约定。
- 除非必要，不要引入 `any`。
- 避免动态导入和内联类型导入；仅使用顶级导入。
- 永远不要在代码中构建提示词；提示词是使用 Handlebars 渲染的静态 `.md` 文件。
- 在 coding-agent 中，永远不要使用 `console.log`/`console.warn`/`console.error`；使用来自 `@f5xc-salesdemos/pi-utils` 的 `logger`。
- 使用 `Promise.withResolvers()` 替代 `new Promise((resolve, reject) => ...)`。
- **类字段或方法上不使用 `private`/`protected`/`public` 关键字。** 使用 ES `#` 私有字段进行封装；可访问成员保持裸露（无关键字）。唯一例外是构造函数参数属性（`constructor(private readonly x: T)`），TypeScript 要求必须使用关键字。移植使用 `private foo` 或 `protected bar` 的上游代码时，转换为 `#foo`（私有）或裸露的 `bar`（可访问）。
- 优先使用现有的辅助工具和实用程序，而非新的临时代码。
...
如果仓库已有与你的更改无关的失败检查，请指出。
测试使用 Bun 的运行器（非 Vitest），但仅在被明确要求时运行 `bun test`。

## 11) 保护改进的功能（回归陷阱清单）

如果你已经在本地改进了行为，将其视为**不可妥协的**。在移植之前，记录下
改进内容并添加显式检查，以免它们在合并中丢失。

- **冻结预期行为**：为每个改进添加简短的"前/后"说明（输入、输出、
  默认值、边界情况）。这可以防止静默回滚。
- **映射旧 → 新 API**：如果上游重命名了概念（hooks → extensions、custom tools → tools 等），
  确保每个旧入口点仍然正确连接。遗漏一个标志或导出就等于丢失功能。
- **验证导出**：检查 `package.json` `exports`、公共类型和桶文件。上游移植经常
  忘记重新导出本地添加的内容。
- **覆盖非正常路径**：如果你修复了错误处理、超时或回退逻辑，添加测试或至少
  一个手动检查清单来测试这些路径。
- **检查默认值和配置合并顺序**：改进通常存在于默认值中。确认新默认值
  没有回退（例如新的配置优先级、禁用的功能、工具列表）。
- **审计环境/Shell 行为**：如果你修复了执行或沙箱化，验证新路径仍然使用你
  净化过的环境，并且没有重新引入别名/函数覆盖。
- **重新运行有针对性的示例**：保留一组最小的"已知正常"示例，并在移植后运行它们
  （CLI 标志、扩展注册、工具执行）。
...
如果 diff 显示文件被**重构**（不仅仅是补丁）：

- 新的抽象、重命名的概念、合并的模块、改变的数据流

那么你必须在移植之前**彻底阅读新的实现**。盲目合并重构的代码会丢失功能，因为：

注意：交互模式最近被拆分为 controllers/utils/types。回溯相关更改时，将更新移植到我们创建的各个文件中，并确保 `interactive-mode.ts` 的连接保持同步。

1. **默认值静默改变** - 新变量 `defaultFoo = [a, b]` 可能替换了返回 `[a, b, c, d, e]` 的旧 `getAllFoo()`。

2. **API 选项被丢弃** - 当系统合并时（例如 `hooks` + `customTools` → `extensions`），旧选项可能无法连接到新实现。

3. **代码路径过时** - 重命名的概念（例如 `hookMessage` → `custom`）需要在每个 switch 语句、类型守卫和处理器中更新——不仅仅是定义。

4. **上下文/能力缩减** - 旧 API 可能暴露了 `{ logger, typebox, pi }`，而新 API 忘记包含。

### 语义移植流程

当上游重构了模块时：

1. **阅读旧实现** - 理解它做了什么，接受了什么选项，暴露了什么。

2. **阅读新实现** - 理解新的抽象以及它们如何映射到旧行为。

3. **验证功能对等** - 对于旧代码中的每个功能，确认新代码保留了它或明确移除了它。
...
- [ ] 新的/移植的代码中没有仅限 Node 的 API
- [ ] 所有包作用域已更新
- [ ] `package.json` 脚本使用 Bun
- [ ] 提示词是 `.md` 文本导入（无内联提示词字符串）
- [ ] coding-agent 中无 `console.*`（使用 `logger`）
...
提交回溯移植时，遵循仓库格式 `<type>(scope): <past-tense description>`，并在标题中保留提交范围。
...
- 包含上游 issue/PR 编号以及外部贡献者的归属
- 标题中的提交范围有助于追踪同步点

## 15) 有意的差异

我们的分支有与上游不同的架构决策。**不要移植这些上游模式：**
...
| `ctx.ui.setHeader()` / `ctx.ui.setFooter()` | 在非 TUI 模式下使用存根                                  | 在 TUI 中实现，其他地方为空操作                                       |
| `ctx.ui.setEditorComponent()`               | 在非 TUI 模式下使用存根                                  | 在 TUI 中实现，其他地方为空操作                                       |
...
| 上游                                | 我们的分支                                                        | 说明                                                      |
| ----------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------- |
| `createTool(cwd: string, options?)` | `createTools(session: ToolSession)` 通过 `BUILTIN_TOOLS` 注册表  | 工具工厂接受 `ToolSession` 并可返回 `null`                |
| 每个工具的 `*Operations` 接口       | 每个工具的接口保留（`FindOperations`、`GrepOperations`）          | 用于 SSH/远程覆盖                                         |
| 到处使用 Node.js `fs/promises`      | 文件使用 `Bun.file()`/`Bun.write()`；目录使用 `node:fs/promises` | 当 Bun API 更简洁时优先使用                               |

### 认证存储

| 上游                            | 我们的分支                                  | 说明                                         |
| ------------------------------- | ------------------------------------------- | -------------------------------------------- |
| `proper-lockfile` + `auth.json` | `agent.db` (bun:sqlite)                     | 凭据专门存储在 `agent.db` 中                 |
| 每个提供商单一凭据              | 多凭据配合轮询选择                          | 会话亲和性和退避逻辑保留                     |
...
- `models.generated.ts` — 自动生成的，在本地重新生成（使用 models.json 代替）

### 我们添加的功能（保留这些）

这些存在于我们的分支中但不在上游。**永远不要覆盖：**
