...
`@f5xc-salesdemos/xcsh` 从包根路径导出 SDK API（也可通过 `@f5xc-salesdemos/xcsh/sdk` 访问）。
...
- 无文件系统持久化。
- 适用于测试、临时工作器、请求作用域代理。
- 会话方法仍然可用，但与持久化相关的行为（文件恢复/分叉路径）自然受限。
...
- 隐藏工具（例如 `submit_result`）除非选项要求，否则需要显式选择启用。
