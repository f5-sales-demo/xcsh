...
`@f5xc-salesdemos/xcsh` 从包根目录导出 SDK API（也可通过 `@f5xc-salesdemos/xcsh/sdk` 导出）。
...
- 无文件系统持久化。
- 适用于测试、临时工作器、请求作用域代理。
- 会话方法仍然可用，但与持久化相关的行为（文件恢复/分叉路径）自然受到限制。
...
- 隐藏工具（例如 `submit_result`）除非选项要求，否则需要主动启用。
...
当您想要部分控制而无需重新创建内部发现逻辑时，请使用这些函数：
...
对于构建编排器的 SDK 消费者（类似于任务执行器流程）：

- `outputSchema`：将结构化输出期望传递到工具上下文
...
仅当您的嵌入器提供了工具/扩展应调用的 UI 功能时，才使用 `setToolUIContext(...)`。
