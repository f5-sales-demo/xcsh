---
title: Hooks
description: 编码代理生命周期中用于事件前/事件后自动化的 Hook 系统。
sidebar:
  order: 4
  label: Hooks
i18n:
...
本文档描述了 `src/extensibility/hooks/*` 中的**当前 hook 子系统代码**。

## 运行时现状

hook 包（`src/extensibility/hooks/`）仍然作为 API 接口导出且可用，但默认的 CLI 运行时现在初始化的是**扩展运行器**路径。在当前启动流程中：

- `--hook` 被视为 `--extension` 的别名（CLI 路径被合并到 `additionalExtensionPaths` 中）
- 工具由 `ExtensionToolWrapper` 包装，而非 `HookToolWrapper`
- 上下文转换和生命周期事件发送通过 `ExtensionRunner` 进行

因此本文档记录的是 hook 子系统实现本身（类型/加载器/运行器/包装器），包括遗留行为和约束。
...
`loadHooks` 随后导入每个路径并期望存在一个 `default` 函数。
...
`hookCapability` 的发现提供者仍然建模为 shell 风格的前/后 hook 文件（例如 `.claude/hooks/pre/*`、`.xcsh/.../hooks/pre/*`）。

此处的 hook 加载器使用动态模块导入，需要一个默认的 JS/TS hook 工厂函数。如果发现的 hook 路径不能作为模块导入，加载将失败并在 `LoadHooksResult.errors` 中报告。

## 事件接口

Hook 事件在 `types.ts` 中是强类型的。
...
`HookToolWrapper.execute()` 在工具执行前发出 `tool_call` 事件。

- 如果任何处理器返回 `{ block: true }`，执行将停止
- 如果处理器抛出异常，包装器将以安全关闭方式阻止执行
...
工具失败时，包装器发出 `tool_result` 事件（`isError: true` 及错误文本内容），然后重新抛出原始错误。
...
- 原地修改原始工具输入参数（`tool_call` 上只能阻止/允许）
- 在工具错误抛出后继续执行（错误路径会重新抛出）
- 包装器行为中的最终成功/错误状态（返回的 `isError` 有类型定义但 `HookToolWrapper` 不会应用它）

## 排序和冲突行为

### 发现级别排序

能力提供者按优先级排序（高优先级优先）。按能力键去重，第一个生效。

对于 `hooks`，能力键为 `${type}:${tool}:${name}`。来自低优先级提供者的被遮蔽的重复项会被标记并从有效发现列表中排除。

### 加载顺序

`discoverAndLoadHooks` 构建一个扁平的 `allPaths` 列表（按解析后的绝对路径去重），然后 `loadHooks` 按该顺序遍历。
每个已发现目录内的文件顺序取决于 `readdir` 的输出；hook 加载器不会执行额外排序。
...
- `tool_call`：除非处理器阻止，否则最后返回的结果生效；第一个阻止操作会短路
...
- `getCommand(name)` 返回跨 hook 的第一个匹配（先加载的优先）
...
`ctx.hasUI` 指示交互式 UI 是否可用。

在没有 UI 的情况下运行时，默认的空操作上下文行为是：
...
`HookRunner.emit(...)` 对大多数事件捕获处理器错误，并向监听器发出 `HookError`（`hookPath`、`event`、`error`），然后继续执行。

`emitToolCall(...)` 更加严格：处理器错误在此处不会被吞没；它们会传播给调用者。在 `HookToolWrapper` 中，这会阻止工具调用（安全失败）。
...
包根目录（`src/index.ts`）重新导出 hook **类型**作为遗留兼容性接口。
