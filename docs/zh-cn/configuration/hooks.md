...
hook 包（`src/extensibility/hooks/`）仍作为 API 接口导出并可用，但默认 CLI 运行时现已初始化**扩展运行器**路径。在当前启动流程中：

- `--hook` 被视为 `--extension` 的别名（CLI 路径合并至 `additionalExtensionPaths`）
- 工具由 `ExtensionToolWrapper` 而非 `HookToolWrapper` 包装
- 上下文转换和生命周期事件通过 `ExtensionRunner` 处理

因此，本文档记录 hook 子系统的实现本身（类型/加载器/运行器/包装器），包括历史行为和约束。

## 关键文件

- `src/extensibility/hooks/types.ts` — hook 上下文、事件类型和结果契约
- `src/extensibility/hooks/loader.ts` — 模块加载和 hook 发现桥接
- `src/extensibility/hooks/runner.ts` — 事件分发、命令查找和错误信号
...
`loadHooks` 随后导入每个路径，并期望其包含一个 `default` 函数。
...
`hookCapability` 的发现提供者仍以预/后 shell 风格的 hook 文件为模型（例如 `.claude/hooks/pre/*`、`.xcsh/.../hooks/pre/*`）。

此处的 hook 加载器使用动态模块导入，并要求提供默认的 JS/TS hook 工厂函数。如果发现的 hook 路径无法作为模块导入，加载将失败，并在 `LoadHooksResult.errors` 中报告。
...
tool_call 处理器
   ...
   执行底层工具
      ...
      │
      └─ 错误   ──> 触发 tool_result(isError=true) 后重新抛出原始错误
```
...
`HookToolWrapper.execute()` 在工具执行前触发 `tool_call`。

- 如果任意处理器返回 `{ block: true }`，执行停止
- 如果处理器抛出异常，包装器以失败-安全方式关闭并阻止执行
- 返回的 `reason` 将成为抛出的错误文本
...
工具执行失败时，包装器以 `isError: true` 和错误文本内容触发 `tool_result`，然后重新抛出原始错误。
...
- 通过 `session_before_*` 和 `session.compacting` 变更取消/自定义压缩/树行为

### 在此实现中 Hook 无法变更的内容

- 原地修改工具输入参数（`tool_call` 上仅支持阻止/允许）
- 工具错误抛出后的执行继续（错误路径会重新抛出）
- 包装器行为中的最终成功/错误状态（返回的 `isError` 有类型定义，但 `HookToolWrapper` 不应用它）

## 排序与冲突行为

### 发现层级排序

能力提供者按优先级排序（高优先级在前）。去重依据为能力键，先到先得。

对于 `hooks`，能力键为 `${type}:${tool}:${name}`。来自低优先级提供者的重复项将被标记并排除在有效发现列表之外。

### 加载顺序

`discoverAndLoadHooks` 构建一个扁平的 `allPaths` 列表，按解析后的绝对路径去重，然后 `loadHooks` 按该顺序迭代。
每个发现目录内的文件顺序取决于 `readdir` 的输出；hook 加载器不进行额外排序。
...
- `tool_call`：最后返回的结果优先，除非某个处理器阻止；第一个阻止操作立即短路
- `tool_result`：最后返回的覆盖值优先（无短路）
- `context`：链式处理；每个处理器接收前一个处理器的消息输出
- `before_agent_start`：第一个返回的消息被保留；后续消息被忽略
- `session_before_*`：追踪最新返回的结果；`cancel: true` 立即短路
- `session.compacting`：最新返回的结果优先

命令/渲染器冲突：

- `getCommand(name)` 返回跨 hook 的第一个匹配项（先加载的优先）
...
`ctx.hasUI` 表示是否有可用的交互式 UI。

在无 UI 的情况下运行时，默认的空操作上下文行为为：
...
- 无效模块或缺少默认导出 → 捕获至 `LoadHooksResult.errors`
- 其他 hook 的加载继续进行

### 事件时

`HookRunner.emit(...)` 会捕获大多数事件的处理器错误，并向监听器触发 `HookError`（包含 `hookPath`、`event`、`error`），然后继续执行。

`emitToolCall(...)` 更为严格：该处理器错误不会被吞没；它们会传播至调用方。在 `HookToolWrapper` 中，这会阻止工具调用（失败安全）。
...
包根目录（`src/index.ts`）将 hook **类型**重导出为历史兼容接口。
