...
RPC 模式将编码代理作为基于 stdio 的换行符分隔 JSON 协议运行。
...
- RPC 模式会将影响工作流的 `todo.*`、`task.*` 和 `async.*` 设置重置为内置默认值，而不是继承用户覆盖的配置。
- 进程以 JSONL 格式读取 stdin（`readJsonl(Bun.stdin.stream())`）。
- 当 stdin 关闭时，进程以代码 `0` 退出。
...
- 未知命令的响应会以 `id: undefined` 发出（即使请求中包含 `id`）。
- 输入循环中的解析/处理异常会以 `command: "parse"` 和 `id: undefined` 发出。
- `prompt` 和 `abort_and_prompt` 会立即返回成功，但如果异步提示调度失败，之后可能会发出一个带有**相同** id 的错误响应。

## 命令模式（规范定义）

`RpcCommand` 定义在 `src/modes/rpc/rpc-types.ts` 中：
...
这些工具会在下一次模型调用之前添加到活动会话的工具注册表中。重新发送 `set_host_tools` 会替换之前的宿主拥有的工具集。
...
如果在流式传输期间省略该参数，提示将失败。

### 队列默认值

来自编码代理设置模式（`packages/coding-agent/src/config/settings-schema.ts`）：
...
- `set_steering_mode` / `set_follow_up_mode`
  ...
  - `"all"`：一次性出队整个队列
- `set_interrupt_mode`
  - `"immediate"`：工具执行在工具调用之间检查引导；挂起的引导可以中止回合中剩余的工具调用
  - `"wait"`：将引导推迟到回合完成
...
- RPC 模式下自动会话标题生成被禁用，`setTitle` UI 请求默认也会被抑制，因为大多数宿主没有有意义的终端标题界面。设置 `PI_RPC_EMIT_TITLE=1` 可以重新启用该 UI 事件。
...
如果对话框有超时，RPC 模式在超时/中止触发时会解析为默认值。

## 宿主工具子协议

RPC 宿主可以通过发送 `set_host_tools` 向代理暴露自定义工具，然后通过同一传输通道服务执行请求。

### 出站请求

当代理需要宿主执行其中一个工具时，RPC 模式会发出：
...
在 `host_tool_result` 上设置 `isError: true` 可以将返回的内容作为工具错误呈现。

## 错误模型和可恢复性

### 命令级失败

失败以 `success: false` 和字符串 `error` 表示。
...
`src/modes/rpc/rpc-client.ts` 是一个便利封装，而非协议定义。
...
- **没有**为每个协议命令都提供辅助方法（例如，`set_interrupt_mode` 和 `set_session_name` 存在于协议类型中，但没有作为专用方法封装）

如果您需要完整的协议覆盖，请使用原始协议帧。
