...
- RPC 模式会将影响工作流的 `todo.*`、`task.*` 和 `async.*` 设置重置为内置默认值，而不是继承用户覆盖的设置。
...
- 如果提供了 id，正常的命令响应会回传相同的 `id`。
- `RpcClient` 依赖此机制进行待处理请求的解析。

运行时的重要边界行为：

- 未知命令的响应会以 `id: undefined` 发出（即使请求中包含 `id`）。
- 输入循环中的解析/处理异常会发出 `command: "parse"` 且 `id: undefined`。
- `prompt` 和 `abort_and_prompt` 会立即返回成功，如果异步提示调度失败，之后可能会发出带有**相同** id 的错误响应。
...
这对于希望在第一次提示之前预置计划的宿主非常有用。

### `set_host_tools` 载荷

替换当前 RPC 服务器可通过 stdio 回调的宿主拥有的工具集：
...
`AgentSession.prompt()` 在活动流式传输期间需要 `streamingBehavior`：
...
- `set_steering_mode` / `set_follow_up_mode`
  - `"one-at-a-time"`：每回合出队一条排队消息
  - `"all"`：一次性出队整个队列
- `set_interrupt_mode`
  - `"immediate"`：工具执行在工具调用之间检查引导消息；待处理的引导消息可以中止回合中剩余的工具调用
  - `"wait"`：推迟引导直到回合完成
...
- RPC 模式中禁用了自动会话标题生成，`setTitle` UI 请求默认也会被抑制，因为大多数宿主没有有意义的终端标题界面。设置 `PI_RPC_EMIT_TITLE=1` 可重新启用该 UI 事件。
...
RPC 宿主可以通过发送 `set_host_tools` 向代理暴露自定义工具，然后在同一传输通道上处理执行请求。
...
- **没有**为每个协议命令暴露辅助方法（例如，`set_interrupt_mode` 和 `set_session_name` 存在于协议类型中，但没有被封装为专用方法）

如果需要完整的接口覆盖，请使用原始协议帧。
