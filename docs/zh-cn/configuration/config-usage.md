---
title: 配置发现与解析
description: xcsh 如何从项目、用户和企业根目录中发现、解析和分层配置。
sidebar:
  order: 1
  label: 配置
i18n:
...
本文档描述了 coding-agent 当前如何解析配置：扫描哪些根目录、优先级如何运作，以及解析后的配置如何被设置、技能、钩子、工具和扩展所消费。
...
`src/config.ts` 中的通用辅助函数在源发现顺序中**不**包含 `.pi`。
...
在有序基础路径中搜索第一个存在的文件，返回第一个匹配项（仅路径或路径+元数据）。

## `findAllNearestProjectConfigDirs(subpath, cwd)`

向上遍历父目录，返回**每个源基础路径最近的现有目录**（`.xcsh`、`.claude`、`.codex`、`.gemini`），然后按源优先级排序结果。

当项目配置应从祖先目录继承时使用此函数（monorepo/嵌套工作区行为）。

---

## 3) 文件配置包装器（`src/config.ts` 中的 `ConfigFile<T>`）

`ConfigFile<T>` 是用于单个配置文件的 schema 验证加载器。
...
- 使用 AJV 根据提供的 TypeBox schema 验证解析后的数据。
- 缓存加载结果直到调用 `invalidate()`。
- 通过 `tryLoad()` 返回三态结果：
  - `ok`
  - `not-found`
  - `error`（包含 schema/解析上下文的 `ConfigError`）
...
3. 运行时覆盖：内存中的，非持久化
4. Schema 默认值：来自 `SETTINGS_SCHEMA`
...
2. 与来自 `agent.db` 的旧版数据库设置合并
3. 将合并结果写入 `config.yml`

`#migrateRawSettings` 中的字段级迁移：

- `queueMode` -> `steeringMode`
- `ask.timeout` 当旧值看起来像毫秒时（`> 1000`），从毫秒转换为秒
...
大多数非核心配置加载通过能力注册表流转（`src/capability/index.ts` + `src/discovery/index.ts`）。

## 提供者排序

提供者按数字优先级排序（数值越高越优先）。示例优先级：
...
对于 `SYSTEM.md` 和 `AGENTS.md`，原生提供者使用最近祖先项目 `.xcsh` 目录搜索（向上遍历），但仍要求 `.xcsh` 目录非空。
...
- `Settings.init()` 加载全局 `config.yml` + 已发现的项目 `settings.json` 能力条目。
- 仅 `level === "project"` 的能力条目会被合并到项目层。

## 技能子系统

- `extensibility/skills.ts` 通过 `loadCapability(skillCapability.id, { cwd })` 加载。
- 应用源开关和过滤器（`ignoredSkills`、`includeSkills`、自定义目录）。
- 旧版命名的开关仍然存在（`skills.enablePiUser`、`skills.enablePiProject`），但它们控制的是原生提供者（`provider === "native"`）。
...
- 声明式 `.md/.json` 工具文件仅包含元数据；可执行加载需要代码模块。

## 扩展子系统

- `discoverAndLoadExtensions()` 从扩展模块能力加上显式路径解析扩展模块。
- 当前实现有意仅保留 `_source.provider === "native"` 的能力条目后再进行加载。
...
2. 能力提供者优先级决定跨提供者的优先级。
3. 能力 key 去重决定冲突行为（对于有 key 的能力，第一个获胜）。
4. 子系统特定的合并逻辑可以进一步改变有效优先级（特别是设置）。

### 设置特有的注意事项

设置能力条目不会被去重；`Settings.#loadProjectSettings()` 按返回顺序对项目条目进行深度合并。由于合并会将后面条目的值覆盖前面条目的值，有效的覆盖行为取决于提供者的发射顺序，而不仅仅是能力 key 语义。
...
- 设置 key 迁移（`queueMode`、`ask.timeout`、扁平 `theme`）。
- 扩展清单兼容性：加载器同时接受 `package.json.xcsh` 和 `package.json.pi` 清单段。
- 旧版设置名称 `skills.enablePiUser` / `skills.enablePiProject` 仍然是原生技能源的活跃控制开关。

如果这些兼容性路径在代码中被移除，请立即更新本文档；目前仍有多个运行时行为依赖于它们。
