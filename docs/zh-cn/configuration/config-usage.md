...
本文档描述了 coding-agent 当前如何解析配置：扫描哪些根目录、优先级如何运作，以及已解析的配置如何被设置、技能、钩子、工具和扩展所消费。
...
在有序基础路径中搜索第一个存在的文件，返回第一个匹配结果（仅路径或路径+元数据）。

## `findAllNearestProjectConfigDirs(subpath, cwd)`

向上遍历父目录，返回**每个来源基础路径最近的现有目录**（`.xcsh`、`.claude`、`.codex`、`.gemini`），然后按来源优先级排序结果。

当项目配置需要从祖先目录继承时使用此函数（单体仓库/嵌套工作区行为）。

---

## 3) 文件配置包装器（`src/config.ts` 中的 `ConfigFile<T>`）

`ConfigFile<T>` 是用于单个配置文件的模式验证加载器。
...
- 使用 AJV 对照提供的 TypeBox 模式验证解析后的数据。
- 缓存加载结果直到调用 `invalidate()`。
- 通过 `tryLoad()` 返回三态结果：
  - `ok`
  - `not-found`
  - `error`（带有模式/解析上下文的 `ConfigError`）

仍支持遗留迁移：

- 如果目标路径为 `.yml`/`.yaml`，同级的 `.json` 文件会被自动迁移一次（`migrateJsonToYml`）。
...
大多数非核心配置加载流经能力注册表（`src/capability/index.ts` + `src/discovery/index.ts`）。

## 提供者排序

提供者按数字优先级排序（较高优先）。示例优先级：
...
- 遗留命名的开关仍然存在（`skills.enablePiUser`、`skills.enablePiProject`），但它们控制的是原生提供者（`provider === "native"`）。
...
- 声明式 `.md/.json` 工具文件仅为元数据；可执行加载期望代码模块。

## 扩展子系统

- `discoverAndLoadExtensions()` 从扩展模块能力加上显式路径解析扩展模块。
- 当前实现有意仅保留 `_source.provider === "native"` 的能力条目后再加载。
...
设置能力条目不会被去重；`Settings.#loadProjectSettings()` 按返回顺序深度合并项目条目。因为合并时后面的条目值会覆盖前面的值，所以有效的覆盖行为取决于提供者的发出顺序，而不仅仅是能力 key 语义。

---

## 9) 仍然存在的遗留/兼容性行为

- `ConfigFile` 针对 YAML 目标文件的 JSON -> YAML 迁移。
...
如果这些兼容性路径在代码中被移除，请立即更新本文档；目前有多个运行时行为仍然依赖它们。
