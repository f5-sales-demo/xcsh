---
title: 主题参考
description: TUI 主题参考，包括颜色令牌、字体设置和主题自定义。
sidebar:
  order: 3
  label: 主题
i18n:
...
本文档描述了 coding-agent 中主题系统的工作方式：schema、加载、运行时行为和故障模式。
...
1. 设置中的 `symbolPreset` 覆盖（如果设置了的话）
...
1. 内置嵌入主题（`defaults/xcsh-dark.json` 和 `defaults/xcsh-light.json` 编译到 `defaultThemes` 中）
...
`getAvailableThemes()` 返回合并后的内置 + 自定义主题名称，已排序，名称冲突时内置主题优先。
...
- `toolDiffAdded` 进行 HSV 调整（绿色偏移向蓝色）
...
存在旧版迁移：旧的扁平格式 `theme: "name"` 会根据亮度检测迁移到嵌套的 `theme.dark` 或 `theme.light`。
...
4. 在设置中选择主题（`Display -> Dark theme` 或 `Display -> Light theme`），具体取决于您想要使用的自动槽位。
...
- 256 色调色板索引（`statusLine<Segment>Bg` / `statusLine<Segment>Fg`）
  驱动 powerline 段落填充。它们独立于上述十六进制键——
  两者都必须设置。
...
- 主题 JSON 中的 `$schema` 仅供参考；运行时验证由代码中编译的 TypeBox schema 强制执行。
