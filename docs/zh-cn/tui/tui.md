...
本文档介绍了 `packages/coding-agent` 和 `packages/tui` 用于扩展 UI、自定义工具 UI 和自定义渲染器的**当前** TUI 契约。

## 该子系统的组成

运行时包含两个层次：

- **渲染引擎（`packages/tui`）**：差量终端渲染器、输入分发、焦点管理、覆盖层、光标定位。
...
| --- | --- | --- |
| 交互式 TUI | 支持 | 组件挂载到编辑器区域，获得焦点，必须调用 `done(result)` 来完成解析。 |
| 后台/无头模式 | 非交互式 | UI 上下文为空操作（`hasUI === false`）。 |
| RPC 模式 | 不支持 | `custom()` 返回 `Promise<never>`，不会挂载 TUI 组件。 |

如果您的扩展/工具需要在非交互模式下运行，请使用 `ctx.hasUI` / `pi.hasUI` 进行判断。
...
1. **任何一行都不得超过 `width`**。如果非图片行溢出，渲染器会抛出异常。
2. **测量可视宽度**，而非字符串长度：使用 `visibleWidth()`。
3. **截断/换行 ANSI 感知文本**，使用 `truncateToWidth()` / `wrapTextWithAnsi()`。
4. **清理来自外部来源的制表符/内容**，使用 `replaceTabs()`（以及 coding-agent 渲染路径中更高级别的清理工具）。
...
扩展 UI 工厂函数接收一个 `KeybindingsManager`（交互模式），以便您可以使用映射的操作而非硬编码的按键：
...
- `TUI` 中存在覆盖层 API（`showOverlay`、`OverlayHandle`），但扩展 `ctx.ui.custom` 在交互模式下的挂载当前是直接替换编辑器组件区域。
- `custom(..., options?: { overlay?: boolean })` 选项存在于扩展类型中；交互式扩展挂载当前忽略此选项。
...
- 用您的组件替换编辑器组件。
- 聚焦您的组件。
- 当 `done(result)` 被调用时：调用 `component.dispose?.()`，恢复编辑器及文本，聚焦编辑器，解析 Promise。

因此 `done(...)` 是完成流程的必要调用。

## 2) Hook/自定义工具 UI 上下文（旧版类型）

`HookUIContext.custom` 在 hook/自定义工具类型中被定义为 `(tui, theme, done)` 签名。
底层交互式实现使用 `(tui, theme, keybindings, done)` 调用工厂函数。JS 消费者可以使用额外的参数；类型层面仍然反映 3 参数的旧版签名。

自定义工具通常通过工厂作用域的 `pi.ui` 对象使用相同的 UI 入口点，然后在普通工具内容中返回选中的值：
...
- `dispose()` 在类型层面是可选的，但当您拥有定时器、子进程、监听器、套接字或覆盖层时应当实现它。
- `done(...)` 应在组件流程中恰好调用一次。
- 对于可取消的长时间运行 UI，将 `CancellableLoader` 与 `AbortSignal` 配合使用，并在 `onAbort` 中调用 `done(...)`。
...
- `packages/coding-agent/src/modes/components/tool-execution.ts` — 挂载 `renderCall`/`renderResult` 组件及部分状态选项。
