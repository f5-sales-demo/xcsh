...
- **`packages/tui` 引擎**：终端生命周期、stdin 标准化、焦点路由、渲染调度、差异化绘制、覆盖层合成、硬件光标定位。
- **`packages/coding-agent` 交互模式**：构建组件树，绑定编辑器回调和键映射，响应 agent/session 事件，将领域状态（流式传输、工具执行、重试、计划模式）转换为 UI 组件。

边界规则：TUI 引擎与消息无关。它只了解 `Component.render(width)`、`handleInput(data)`、焦点和覆盖层。Agent 语义保留在交互控制器中。
...
强制渲染（`requestRender(true)`）会在重绘之前重置之前的行缓存和光标记录。
...
1. 启用原始模式和括号粘贴模式。
2. 绑定窗口大小变化处理器。
3. 创建 `StdinBuffer`，将部分转义序列片段拆分为完整序列。
...
- 仅在序列完整或超时刷新时发出 `data` 事件。
- 检测括号粘贴并发出包含原始粘贴文本的 `paste` 事件。

这可以防止部分转义序列片段被误判为普通按键。
...
3. 如果焦点组件所属的覆盖层已被隐藏/不可见，TUI 会将焦点重新分配给下一个可见覆盖层或保存的覆盖层前焦点。
4. 除非焦点组件设置了 `wantsKeyRelease = true`，否则按键释放事件会被过滤。
5. 分发后，TUI 调度渲染。

`setFocus()` 还会切换 `Focusable.focused`，控制组件是否为硬件光标定位发出 `CURSOR_MARKER`。

## 按键处理分工：编辑器与控制器

`CustomEditor` 首先拦截高优先级组合键（escape、ctrl-c/d/z、ctrl-v、ctrl-p 变体、ctrl-t、alt-up、扩展自定义键），其余委托给基础 `Editor` 行为（文本编辑、历史记录、自动补全、光标移动）。

然后 `InputController.setupKeyHandlers()` 将编辑器回调绑定到模式操作：
...
这样按键解析/编辑器机制保留在 `packages/tui` 中，而模式语义保留在 coding-agent 控制器中。

## 渲染循环与差异比较策略

`TUI.requestRender()` 使用 `process.nextTick` 进行去抖，每个 tick 仅执行一次渲染。同一轮中的多次状态变更会被合并。

`#doRender()` 管线：

1. 将根组件树渲染为 `newLines`。
2. 合成可见覆盖层（如果有）。
3. 从可见视口行中提取并移除 `CURSOR_MARKER`。
4. 为非图片行追加段重置后缀。
5. 选择完全重绘还是差异补丁：
   - 首帧
   - 宽度变化
   - 启用 `clearOnShrink` 且没有覆盖层时的收缩
   - 在前一个视口上方的编辑
6. 对于差异更新，仅补丁变化的行范围，并在需要时清除过期的尾部行。
7. 为 IME 支持重新定位硬件光标。
...
- 非图片渲染行不得超过终端宽度；溢出会抛出异常并写入崩溃诊断信息。
- 覆盖层合成包含防御性截断和合成后宽度验证。
- 宽度变化强制完全重绘，因为换行语义发生了变化。
- 光标位置在移动前会被钳制。
...
窗口大小变化事件从 `ProcessTerminal` 事件驱动到 `TUI.requestRender()`。

影响：

- 任何宽度变化都会触发完全重绘。
- 视口/顶部跟踪（`#previousViewportTop`、`#maxLinesRendered`）避免在内容或终端大小变化时出现无效的相对光标计算。
- 覆盖层可见性可以依赖终端尺寸（`OverlayOptions.visible`）；窗口大小变化后覆盖层变为不可见时，焦点会被修正。
...
- `message_start` assistant：创建 `streamingComponent` 并挂载它。
- `message_update`：更新流式助手内容；在工具调用出现时创建/更新工具执行组件。
- `tool_execution_update/end`：更新工具结果组件和完成状态。
- `message_end`：完成助手流，处理中止/错误注解，在正常停止时标记待处理工具参数为完成。
- `agent_end`：停止加载器，清除瞬态流状态，刷新延迟的模型切换，如果在后台则发出完成通知。

读取工具分组是有状态的（`#lastReadGroup`），将连续的读取工具调用合并为一个视觉块，直到出现非读取中断。
...
- `statusLine` 渲染持久化的状态/钩子/计划指示器，并驱动编辑器顶部边框更新。
...
Escape 通过清除编辑器文本和恢复边框颜色退出非活动模式；当执行处于活动状态时，Escape 改为中止正在运行的任务。
...
当后台 `agent_end` 且没有排队工作时，控制器发送完成通知并关闭。
...
- 单击 `Ctrl+C`：清除编辑器；500ms 内双击：关闭。

取消是状态条件性的；同一按键根据运行时状态可能意味着中止、退出模式、触发选择器或无操作。
...
- 编辑器自动补全更新（在 `Editor` 内部）使用去抖定时器，减少输入时的重复计算。

因此，运行时将事件驱动的状态转换与有界的渲染节奏混合使用，以在保持交互响应性的同时避免重绘风暴。
