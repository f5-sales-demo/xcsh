...
- `/tree`
- 配置的快捷键动作 `tree`
- 在空编辑器上双击 Escape，当 `doubleEscapeAction = "tree"` 时（默认行为）
- `/branch`，当 `doubleEscapeAction = "tree"` 时（路由到树选择器而非仅用户分支选择器）

## 树 UI 模型

树从会话条目的父指针（`id` / `parentId`）渲染。

- 子节点按时间戳升序排列（较旧的在前，较新的在下方）
- 活动分支（从根到当前叶节点的路径）用圆点标记
- 标签（如果存在）在节点文本前显示为 `[label]`
- 如果存在多个根节点（孤立的/断裂的父链），它们会显示在一个虚拟分支根节点下
...
**仅包含工具调用**（无文本）的助手消息在所有过滤视图中默认隐藏，除非：

- 消息为错误/已中止状态（`stopReason` 不是 `stop`/`toolUse`），或
- 它是当前叶节点（始终保持可见）
...
- 如果父节点为 `null`（根用户消息），叶节点重置为根（`resetLeaf()`）
- 所选消息文本复制到编辑器以便编辑/重新提交
...
### 选择非用户节点（assistant/tool/summary/compaction/custom 记账等）
...
- `No summary`（不生成摘要）
- `Summarize`（生成摘要）
- `Summarize with custom prompt`（使用自定义提示词生成摘要）

流程详情：

- 在摘要提示中按 Escape 会重新打开树选择器
- 取消自定义提示词会返回摘要选择循环
- 摘要生成期间，UI 显示加载器并将 `Esc` 绑定到 `abortBranchSummary()`
...
| `/branch` | 通常是当前会话文件 -> 新会话文件 | 默认从选定的**用户**消息分支到新会话文件；如果 `doubleEscapeAction = "tree"`，`/branch` 会打开树导航 UI |
...
效果：在目标位置追加一个 `branch_summary` 条目后继续。
