...
- `/tree`
- 配置的快捷键操作 `tree`
- 在空编辑器上双击 Escape，当 `doubleEscapeAction = "tree"` 时（默认）
...
- 子节点按时间戳升序排列（较早的在前，较新的在后）
- 活动分支（从根到当前叶节点的路径）用圆点标记
- 标签（如有）在节点文本前以 `[label]` 形式渲染
- 如果存在多个根节点（孤立/断裂的父链），它们显示在一个虚拟分支根下
...
- 匹配不区分大小写
- 所有词元必须匹配（AND 语义）
- 可搜索文本包括标签、角色和特定类型内容（消息文本、分支摘要文本、自定义类型、工具命令片段等）
...
- 取消自定义提示会返回摘要选择循环
- 在生成摘要期间，UI 显示加载器并将 `Esc` 绑定到 `abortBranchSummary()`
- 如果摘要生成中止，树选择器重新打开且不应用移动
...
在树 UI 中编辑标签会调用 `appendLabelChange(targetId, label)`。

- 非空标签设置/更新解析后的标签
- 空标签清除它
- 标签以仅追加方式存储为 `label` 条目
...
| `/branch` | 通常是当前会话文件 -> 新会话文件 | 默认从选定的**用户**消息分支到新会话文件；如果 `doubleEscapeAction = "tree"`，`/branch` 改为打开树导航 UI |
...
效果：在继续之前，在目标位置追加一个 `branch_summary` 条目。
