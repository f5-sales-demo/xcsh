---
title: 压缩与分支摘要
description: 长会话中的上下文窗口压缩与分支摘要生成。
sidebar:
  order: 5
  label: 压缩
i18n:
...
压缩和分支摘要是两种保持长会话可用且不丢失先前工作上下文的机制。

- **压缩** 将当前分支上的旧历史记录重写为摘要。
- **分支摘要** 在 `/tree` 导航期间捕获被放弃的分支上下文。

两者都作为会话条目持久化存储，并在重建 LLM 输入时转换回用户上下文消息。
...
1. 活跃路径上最新的压缩条目被转换为一条 `compactionSummary` 消息。
2. 从 `firstKeptEntryId` 到压缩点之间被保留的条目被重新包含。
3. 路径上后续的条目被追加。
...
- **阈值压缩**
  ...
  - 成功后，如果 `compaction.autoContinue !== false`，注入一条合成提示：
    - `"Continue if you have next steps."`

### 压缩前修剪

在压缩检查之前，可能会运行工具结果修剪（`pruneToolOutputs`）。

默认修剪策略：

- 保护最新的 `40_000` 个工具输出 token。
- 要求至少 `20_000` 个总估计节省量。
- 永不修剪来自 `skill` 或 `read` 的工具结果。

被修剪的工具结果会被替换为：

- `[Output truncated - N tokens]`

如果修剪改变了条目，会话存储会被重写，代理消息状态会在压缩决策之前刷新。

### 边界和切割点逻辑

`prepareCompaction()` 只考虑自上次压缩条目（如果有）以来的条目。

1. 查找之前的压缩索引。
2. 计算 `boundaryStart = prevCompactionIndex + 1`。
3. 在可用时使用测量的使用率比率调整 `keepRecentTokens`。
...
如果切割点之前紧跟着非消息元数据条目（`model_change`、`thinking_level_change`、标签等），它们会被拉入保留区域，方法是向前移动切割索引直到遇到消息或压缩边界。

### 拆分轮次处理

如果切割点不在用户轮次起始处，压缩将其视为拆分轮次。
...
- 仅当先前条目是由 pi 生成（`fromExtension !== true`）时才包含先前压缩详情。
- 在拆分轮次中，也包含轮次前缀的文件操作。
...
1. 使用 `collectEntriesForBranchSummary(...)` 计算从旧叶子到公共祖先的被放弃条目。
2. 如果调用者请求了摘要（`options.summarize`），在切换叶子之前生成摘要。
3. 如果摘要存在，使用 `branchWithSummary(...)` 将其附加到导航目标。

实际上，当 `branchSummary.enabled` 启用时，这通常由 `/tree` 流程驱动。
...
1. 第一遍：从所有被摘要的条目中收集累积的文件操作，包括先前 pi 生成的 `branch_summary` 详情。
2. 第二遍：从最新到最旧遍历，添加消息直到达到 token 预算。
3. 优先保留近期上下文。
4. 在预算边界处可能仍包含大型摘要条目以保持连续性。

压缩条目在分支摘要输入中作为消息（`compactionSummary`）包含。
...
3. 如果提供了自定义指令则使用之，否则使用 `branch-summary.md`。
4. 使用 `SUMMARIZATION_SYSTEM_PROMPT` 调用摘要模型。
5. 在前面添加 `branch-summary-preamble.md`。
6. 追加文件操作标签。

结果作为 `BranchSummaryEntry` 存储，带有可选的详情（`readFiles`、`modifiedFiles`）。
...
导航后事件，暴露新/旧叶子和可选的摘要条目。

## 运行时行为和失败语义

- 手动压缩会先中止当前代理操作。
- `abortCompaction()` 取消手动和自动压缩控制器。
- 自动压缩发出开始/结束会话事件用于 UI/状态更新。
- 自动压缩可以尝试多个候选模型并重试瞬态故障。
