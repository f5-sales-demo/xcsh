...
本文档涵盖了当前时间旅行流规则（TTSR）从规则发现到流中断、重试注入、扩展通知和会话状态处理的完整运行时路径。
...
`loadCapability("rules")` 按 `rule.name` 进行去重，采用先到先得语义（优先级较高的提供者优先）。被遮蔽的重复项在 TTSR 注册之前即被移除。
...
无效的正则触发器会被记录为警告并忽略；会话启动继续进行。

### 设置注意事项

`TtsrSettings.enabled` 会被加载到管理器中，但当前在运行时门控中并未进行检查。如果规则存在，匹配仍会运行。
...
`check()` 遍历已注册的规则并返回所有通过重复策略（`#canTrigger`）的匹配规则。
...
5. 追加一条合成用户消息，其中每个规则包含一个 `<system-interrupt ...>` 块
...
`messageCount` 在 `turn_end` 时递增，因此间隔以已完成的轮次而非流式块为单位计量。
...
交互模式使用 `session.isTtsrAbortPending` 来抑制将中止的助手停止原因显示为可见错误（在 TTSR 中断期间），并在事件到达时渲染 `TtsrNotificationComponent`。

## 7. 持久化和恢复状态（当前实现）

`SessionManager` 对注入规则的持久化提供完整的 schema 支持：
...
最终效果：注入规则的抑制在当前进程的内存中生效，但通过此路径当前不会跨会话重载/恢复进行持久化/恢复。

## 8. 竞态边界和顺序保证

### 中止与重试回调

- 从 TTSR 处理器角度来看，中止是同步的（立即调用 `agent.abort()`）
- 重试通过定时器延迟（`50ms`）
- 扩展通知是异步的，在中止/重试调度之前有意不进行等待
...
在定时器窗口期间，状态可能发生变化（用户中断、模式操作、其他事件）。重试调用是尽力而为的：`agent.continue().catch(() => {})` 会吞掉后续错误。

## 9. 边界情况总结

- 无效的 `ttsr_trigger` 正则表达式：跳过并记录警告；其他规则继续运行。
- 能力层的重复规则名称：低优先级的重复项在注册前被遮蔽。
- 管理器层的重复名称：第二次注册被忽略。
- `contextMode: "keep"`：部分违规输出可能在提醒重试之前保留在上下文中。
- after-gap 重复策略依赖于 `turn_end` 时的轮次计数递增；轮次中间的流式块不会推进间隔计数器。
