---
title: 会话切换与最近会话列表
description: 会话切换机制以及带有搜索和筛选功能的最近会话列表。
sidebar:
  order: 4
  label: 切换与最近会话
i18n:
...
本文档描述了 coding-agent 如何发现最近会话、解析 `--resume` 目标、呈现会话选择器，以及切换活动运行时会话。

本文重点介绍当前的实现行为，包括回退路径和注意事项。
...
`SessionManager` 默认将会话存储在按 cwd 作用域划分的目录下：
...
1. `getRecentSessions(sessionDir, limit)`（欢迎/摘要视图）
   ...
   - 解析头部 + 最早的用户文本预览。
   - 返回轻量级 `RecentSessionInfo`，带有延迟计算的 `name` 和 `timeAgo` 获取器。
   - 按文件 `mtime` 降序排列。
...
- 显示名称优先级：`header.title` -> 第一条用户提示 -> `header.id` -> 文件名
- 名称在紧凑显示中截断为 40 个字符
- 从标题派生的名称中去除/清理控制字符和换行符

对于 `SessionInfo` 列表条目：

- `title` 为 `header.title` 或最新压缩的 `shortSummary`
- `firstMessage` 为第一条用户消息文本或 `"(no messages)"`
...
2. 验证面包屑：
   - 当前终端可被识别
   - 面包屑 cwd 与当前 cwd 匹配（解析路径比较）
   - 引用的文件仍然存在
3. 如果面包屑无效/缺失，回退到会话目录中按 mtime 最新的文件（`findMostRecentSession`）
4. 如果未找到，创建新会话

终端 ID 派生优先使用 TTY 路径，回退到基于环境变量的标识符（`KITTY_WINDOW_ID`、`TMUX_PANE`、`TERM_SESSION_ID`、`WT_SESSION`）。

面包屑写入是尽力而为的，不会导致致命错误。
...
2. ID 前缀值
   ...
   - 如果本地无匹配且 `sessionDir` 未被强制指定，尝试 `SessionManager.listAll()`
   - 使用第一个匹配项（无歧义提示）

跨项目匹配行为：

- 如果匹配的会话 cwd 与当前 cwd 不同，CLI 会提示是否 fork 到当前项目
...
`selectSession(sessions)` 创建一个带有 `SessionSelectorComponent` 的独立 TUI，并精确解析一次：
...
- Esc 取消
- Ctrl+C 退出
- 跨会话 id/标题/cwd/首条消息/所有消息/路径的模糊搜索
...
注意事项：UI 文本显示 `Press Tab to view all`，但此组件当前没有 Tab 处理程序，且当前的接线方式仅列出当前作用域的会话。
...
2. 触发 `session_before_switch` 钩子事件（`reason: "resume"`，可取消）
3. 如果被取消 -> 返回 `false`，不进行切换
4. 断开当前代理事件流
5. 中止活跃的生成/工具流程
...
- 消息/模型/思考立即就地重建。
- 触发 `session_before_switch`/`session_switch` 钩子事件。
- 刷新 UI 聊天/待办事项。
- 选择器流程中没有专用的切换后模式恢复调用；模式重新进入行为与启动时的 `#restoreModeFromSession()` 不对称。
...
切换/打开在真正的 I/O 故障（权限错误、重写失败等）时仍可能抛出异常，这些异常会传播给调用者。
