...
`SessionManager` 默认将会话存储在以当前工作目录（cwd）为范围的目录下：

- `~/.xcsh/agent/sessions/--<cwd-encoded>--/*.jsonl`

`SessionManager.list(cwd, sessionDir?)` 仅读取该目录，除非提供了显式的 `sessionDir`。
...
1. `getRecentSessions(sessionDir, limit)`（欢迎/摘要视图）
   ...
   - 解析头部 + 最早的用户文本预览。
   - 返回轻量级的 `RecentSessionInfo`，包含延迟计算的 `name` 和 `timeAgo` getter。
   - 按文件 `mtime` 降序排序。
...
1. 读取终端范围的面包屑（`~/.xcsh/agent/terminal-sessions/<terminal-id>`）
2. 验证面包屑：
   - 当前终端可被识别
...
终端 ID 的推导优先使用 TTY 路径，回退到基于环境变量的标识符（`KITTY_WINDOW_ID`、`TMUX_PANE`、`TERM_SESSION_ID`、`WT_SESSION`）。
...
2. ID 前缀值
   - 通过 `id.startsWith(sessionArg)` 在 `SessionManager.list(cwd, sessionDir)` 中查找匹配
...
`selectSession(sessions)` 创建一个独立的 TUI，使用 `SessionSelectorComponent` 并精确解析一次：
...
- Esc 取消
- Ctrl+C 退出
- 跨会话 id/标题/cwd/首条消息/全部消息/路径的模糊搜索

空列表渲染行为：

- 渲染一条消息而不是崩溃
- 在空列表上按 Enter 不执行任何操作（无回调）
- Esc/Ctrl+C 仍然有效

注意事项：UI 文本显示 `Press Tab to view all`，但该组件目前没有 Tab 处理器，且当前接线仅列出当前范围的会话。
...
7. 刷新会话写入器（`sessionManager.flush()`）以持久化待写入的数据
...
12. 用重建的上下文替换代理消息
13. 如果可用且存在于模型注册表中，从 `sessionContext.models.default` 恢复默认模型
14. 恢复思考级别：
    - 如果分支已有 `thinking_level_change`，应用保存的会话级别
    - 否则从设置中推导默认思考级别，限制到模型能力范围，设置它，并追加一个新的 `thinking_level_change` 条目
...
- 交互模式随后运行 `#restoreModeFromSession()` 以重新进入持久化的模式状态（目前为 plan/plan_paused）。
...
- 格式错误/无效头部（或实际上不可读的已解析条目）-> 视为空 -> 初始化新会话并持久化。

这是恢复行为，而非硬性失败。

### 硬性失败

切换/打开仍可能在真正的 I/O 故障（权限错误、重写失败等）时抛出异常，这些异常会传播给调用者。

### ID 前缀匹配注意事项

- ID 匹配使用 `startsWith` 并取排序列表中的第一个匹配项。
- 如果多个会话共享前缀，不会显示歧义 UI。
