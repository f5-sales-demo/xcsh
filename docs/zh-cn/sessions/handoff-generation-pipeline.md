---
title: 交接生成流水线
description: 用于创建可移植会话摘要以支持团队协作的交接生成流水线。
sidebar:
  order: 8
  label: 交接流水线
i18n:
...
本文档描述了 coding-agent 当前如何实现 `/handoff`：触发路径、生成提示词、完成捕获、会话切换和上下文重新注入。
...
相同的最小内容守卫在 `AgentSession.handoff()` 内部也存在，如果违反则抛出异常。这在 UI 层和会话层都实现了双重安全保障。
...
`expandPromptTemplates: false` 防止对此内部指令载荷进行斜杠/提示词模板展开。

### 2) 捕获完成结果

在发送提示词之前，`handoff()` 订阅会话事件并等待 `agent_end`。

在 `agent_end` 时，它通过反向扫描最近的 `assistant` 消息从代理状态中提取交接文本，然后将所有 `type === "text"` 的 `content` 块用 `\n` 连接。

重要的提取假设：

- 仅使用文本块；非文本内容被忽略。
- 假设最新的助手消息对应于交接生成。
- 不会解析 Markdown 章节或验证格式合规性。
...
生成的交接文档被包装并作为 `custom_message` 条目附加到新会话中：
...
`buildSessionContext()` 通过 `createCustomMessage(...)` 将此条目转换为运行时自定义/用户上下文消息，因此它会被包含在新会话的后续提示词中。
...
- `isGeneratingHandoff` → 当控制器存在时为 true

当使用此中止路径时，交接订阅者会以 `Error("Handoff cancelled")` 拒绝，命令控制器将其映射到取消 UI。

### 交互式 `/handoff` 路径的限制

在当前的交互式控制器连接中，`/handoff` 没有安装调用 `abortHandoff()` 的专用 Escape 处理程序（不同于压缩/分支摘要路径会临时覆盖 `editor.onEscape`）。

实际影响：

- 存在会话级取消支持，但在 `/handoff` 命令路径中没有交接特定的按键绑定钩子。
- 用户中断仍然可以通过更广泛的代理中止路径发生，但这与 `abortHandoff()` 使用的显式取消通道不同。
...
- **失败**
  - 来自 `handoff()` / 提示词流水线的任何其他抛出错误（模型/API 验证错误、运行时异常等）
  - UI 显示 `Handoff failed: ...`

额外细节：如果生成完成但未提取到文本，`handoff()` 返回 `undefined`，控制器当前报告为**已取消**，而非**失败**。

## 短会话和最小内容守卫

两个守卫防止低信号交接：

- UI 层（`handleHandoffCommand`）：对 `< 2` 个消息条目发出警告并提前返回
...
6. 如果缺失/已中止 → 返回 `undefined` 或取消错误路径
7. 如果存在：
   - 刷新旧会话
   - 创建新的空会话
   - 重置运行时队列/计数器
...
- 交接提取是启发式的："最后的助手文本块"；没有结构化验证。
- 没有硬性检查生成的 Markdown 是否遵循请求的章节格式。
- 在控制器 UX 中，缺失的提取文本被报告为取消。
- `/handoff` 交互式流程当前缺少专用的 Escape→`abortHandoff()` 绑定。
- 此路径不设置新会话的血统元数据（`parentSession`）。
