---
title: 会话树架构
description: 会话树架构，包含分支、导航和父子对话关系。
sidebar:
  order: 2
  label: 树架构
i18n:
...
本文档描述会话树导航的当前工作方式：内存中的树模型、叶节点移动规则、分支行为以及扩展/事件集成。

## 该子系统是什么

会话以追加式条目日志的形式存储，但运行时行为基于树结构：

- 每个非头部条目都有 `id` 和 `parentId`。
- 活动位置是 `SessionManager` 中的 `leafId`。
- 追加条目时总是创建当前叶节点的子节点。
- 分支**不会**重写历史；它只在下一次追加之前改变叶节点的指向位置。
...
- `#byId: Map<string, SessionEntry>` — 任意条目的快速查找
- `#leafId: string | null` — 树中的当前位置
- `#labelsById: Map<string, string>` — 按目标条目 id 解析的标签

树 API：

- `getBranch(fromId?)` 沿父链接遍历到根节点，返回根→节点路径
...
`getTree()` 是运行时投影；持久化仍然是追加式 JSONL 条目。
...
4. 计算新的叶节点目标：
   - 选择**用户**消息：叶节点移动到其父节点，消息文本返回用于编辑器预填充
   - 选择 **custom_message**：与用户消息规则相同（叶节点 = 父节点，文本预填充编辑器）
   - 选择任何其他条目：叶节点 = 选中条目的 id
5. 应用叶节点移动：
...
- 分支源必须是**用户消息**。
- 提取选中的用户文本用于编辑器预填充。
- 如果选中的用户消息是根节点（`parentId === null`）：通过 `newSession({ parentSession: previousSessionFile })` 开始新会话。
- 否则：`createBranchedSession(selectedEntry.parentId)` 分叉历史到选中的提示边界。
...
- 持久化模式：写入新的 JSONL 文件并切换管理器到该文件；返回新文件路径。
...
- 处理路径上最新的压缩：
  - 首先发射压缩摘要
  - 从 `firstKeptEntryId` 到压缩点重放保留的消息
  - 然后重放压缩后的消息
- 将 `branch_summary` 和 `custom_message` 条目作为 `AgentMessage` 对象包含。

`session/messages.ts` 然后将这些消息类型映射为模型输入：

- `branchSummary` 和 `compactionSummary` 变为用户角色的模板化上下文消息
- `custom`/`hookMessage` 变为用户角色的内容消息

因此树的移动通过改变活动叶节点路径来改变上下文，而不是通过修改旧条目。
...
- `/tree` 始终打开树选择器。
- `/branch` 打开用户消息选择器，除非 `doubleEscapeAction=tree`，此时也使用树选择器 UX。
...
- `session_before_compact`、`session.compacting`、`session_compact` 用于之后影响树上下文重建的压缩条目

## 实际约束和边界条件

- `branch()` 不能指向 `null`；使用 `resetLeaf()` 来设置首条目之前的根状态。
- `branchWithSummary()` 支持 `null` 目标并记录 `fromId: "root"`。
- 在树选择器中选择当前叶节点是空操作。
- 摘要生成需要活动模型；如果模型不存在，摘要导航会快速失败。
- 如果摘要生成被中止，导航将被取消且叶节点保持不变。
- 内存会话从 `createBranchedSession` 永远不会返回分支文件路径。
