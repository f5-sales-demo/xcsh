---
title: 会话存储与条目模型
description: 追加式会话存储模型，包含条目类型、持久化以及格式间的迁移。
sidebar:
  order: 1
  label: 存储与条目模型
i18n:
...
本文档是关于编码代理会话如何表示、持久化、迁移以及在运行时重建的权威参考。
...
面包屑内容为两行：原始 cwd，然后是会话文件路径。`continueRecent()` 在扫描最近 mtime 之前优先使用此终端级别的指针。

## 文件格式

会话文件为 JSONL 格式：每行一个 JSON 对象。

- 第 1 行始终是会话头部（`type: "session"`）。
- 其余行为 `SessionEntry` 值。
- 条目在运行时为追加模式；分支导航通过移动指针（`leafId`）而非修改现有条目。
...
- `version` 在 v1 文件中为可选；缺失即表示 v1。
- `parentSession` 是不透明的血缘标记字符串。当前代码根据流程（`fork`、`forkFrom`、`createBranchedSession` 或显式 `newSession({ parentSession })`）写入会话 id 或会话路径。将其视为元数据，而非类型化的外键。
...
- 对于 `message` 条目：将遗留的 `message.role === "hookMessage"` 重写为 `"custom"`。
- 设置头部 `version = 3`。
...
- 如果第一个已解析条目不是有效的会话头部（`type !== "session"` 或缺少字符串 `id`）-> 返回 `[]`。

`SessionManager.setSessionFile()` 行为：

- 加载器返回 `[]` 被视为空/不存在的会话，并在该路径创建一个新的初始化会话文件替代。
- 有效文件会被加载、按需迁移、解析 blob 引用，然后建立索引。
...
- `resetLeaf()` 将 `leafId` 设为 `null`；下次追加将创建一个新的根条目（`parentId: null`）。
- `branchWithSummary()` 将叶节点设置为分支目标并追加一个 `branch_summary` 条目。
...
1. 确定叶节点：
   ...
   - 否则回退到最后一个条目。
2. 从叶节点沿 `parentId` 链向根遍历，然后反转得到根->叶路径。
3. 沿路径推导运行时状态：
   - `thinkingLevel` 取自最新的 `thinking_level_change`（默认 `"off"`）
   - 模型映射取自 `model_change` 条目（`role ?? "default"`）
   - 如果没有显式模型变更，`models.default` 回退到助手消息的 provider/model
   - 从所有 `ttsr_injection` 条目收集去重后的 `injectedTtsrRules`
   - mode/modeData 取自最新的 `mode_change`（默认模式为 `"none"`）
4. 构建消息列表：
   - `message` 条目直接传递
...
- `append*` 立即更新内存状态。
- 持久化延迟到至少存在一条助手消息时才执行。
  - 在首条助手消息之前：条目保留在内存中；不进行文件追加。
  - 当首条助手消息出现时：完整的内存会话刷新到文件。
  - 之后：新条目增量追加。

代码中的设计原因：避免持久化从未产生助手响应的会话。

### 持久性操作

- `flush()` 刷新写入器并调用 `fsync()`。
- 原子全量重写（`#rewriteFile`）写入临时文件、flush+fsync、关闭，然后重命名覆盖目标文件。
...
- 移除临时字段 `partialJson` 和 `jsonlEvents`。
...
加载时，blob 引用会被解析回 base64，用于 message/custom_message 图片块。
...
- `list(cwd, sessionDir?)` -> 一个项目范围内的会话
...
`HistoryStorage`（`history-storage.ts`）是一个独立的 SQLite 子系统，用于提示词召回/搜索，而非会话回放。

- 数据库：`~/.xcsh/agent/history.db`
- 表：`history(id, prompt, created_at, cwd)`
- FTS5 索引：`history_fts`，通过触发器维护同步
- 使用内存中的上次提示词缓存来去重连续相同的提示词
- 异步插入（`setImmediate`），因此提示词捕获不会阻塞轮次执行
