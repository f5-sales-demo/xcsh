...
当您希望由 OMP 管理配置时，请使用 `.xcsh/mcp.json`。仅当您需要一个其他 MCP 客户端也可以读取的可移植备用文件时，才使用根目录的 `mcp.json` / `.mcp.json`。

## 添加 Schema 引用

在文件顶部添加此行以获得编辑器自动补全和验证功能：
...
这遵循官方文件系统 MCP 服务器包（`@modelcontextprotocol/server-filesystem`）。
...
`sse` 仍然为了兼容性而受支持，但 MCP 规范现在建议新服务器使用 Streamable HTTP（`type: "http"`）。
...
Slack 是目前最清晰的示例。Slack 的 MCP 服务器托管在 `https://mcp.slack.com/mcp`，使用 Streamable HTTP，并且需要使用您的 Slack 应用客户端凭据进行机密 OAuth 认证。
...
1. 如果值以 `!` 开头，OMP 将其作为 shell 命令运行并使用去除空白后的 stdout 输出。
2. 否则，OMP 首先检查该值是否匹配某个环境变量名。
3. 如果该环境变量未设置，OMP 将使用该字符串的字面值。
...
如果您希望 OMP 的行为最不出乎意料，请优先使用 `.xcsh/mcp.json` 并使用显式的 env/header 值。

## `disabledServers`

`disabledServers` 主要在用户配置文件（`~/.xcsh/mcp.json`）中使用，当某个服务器从其他来源被发现而您希望 OMP 忽略它，但又不想编辑那个工具的配置时非常有用。
...
- 您需要向导尚未提示的传输方式或认证选项
- 您想从另一个 MCP 客户端粘贴服务器定义
- 您想在编辑器中获得基于 Schema 的验证
...
- 省略 `type` 表示 `stdio`
- 如果您粘贴了远程服务器配置但忘记了 `"type": "http"`，OMP 会将其视为 `stdio` 并报错缺少 `command`
- `sse` 仍然有效以保持兼容性，但新的托管服务器通常应配置为 `http`

## 发现和优先级

OMP 不会合并跨文件的重复服务器定义。发现提供者按优先级排序，优先级更高的定义胜出。
...
运行 `/mcp list`。OMP 会发现许多第三方 MCP 文件，但项目级加载也可以通过 `mcp.enableProjectConfig` 设置禁用。
