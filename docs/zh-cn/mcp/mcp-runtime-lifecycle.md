---
title: MCP 运行时生命周期
description: MCP 服务器进程生命周期，涵盖从初始化到工具注册、健康监控和关闭的完整过程。
sidebar:
  order: 3
  label: 运行时生命周期
i18n:
...
本文档描述了 MCP 服务器在 coding-agent 运行时中如何被发现、连接、作为工具暴露、刷新和销毁。

## 生命周期概览

1. **SDK 启动**时调用 `discoverAndLoadMCPTools()`（除非 MCP 被禁用）。
2. **发现阶段**（`loadAllMCPConfigs`）从能力源解析 MCP 服务器配置，过滤已禁用的/项目级的/Exa 条目，并保留源元数据。
3. **管理器连接阶段**（`MCPManager.connectServers`）并行启动每个服务器的连接和 `tools/list` 请求。
4. **快速启动门控**最多等待 250ms，然后可能返回：
   - 完全加载的 `MCPTool`，
   - 每个服务器的失败信息，
   - 或仍在等待中的服务器的缓存 `DeferredMCPTool`。
5. **SDK 接入**将 MCP 工具合并到会话的运行时工具注册表中。
6. **活跃会话**可以通过 `/mcp` 流程刷新 MCP 工具（`disconnectAll` + 重新发现 + `session.refreshMCPTools`）。
7. **销毁**在调用者调用 `disconnectServer`/`disconnectAll` 时发生；管理器同时会清除已断开服务器的 MCP 工具注册。
...
`loadAllMCPConfigs()`（`src/mcp/config.ts`）通过能力发现加载规范的 MCP 服务器项，然后转换为旧版 `MCPServerConfig`。

过滤行为：

- `enableProjectConfig: false` 会移除项目级条目（`_source.level === "project"`）。
- `enabled: false` 的服务器在连接尝试前即被跳过。
- Exa 服务器默认被过滤掉，其 API 密钥被提取用于原生 Exa 工具集成。

结果包括 `configs` 和 `sources`（元数据，后续用于提供者标签）。

### 发现级别的失败行为

`discoverAndLoadMCPTools()` 区分两类失败：

- **发现硬失败**（来自 `manager.discoverAndConnect` 的异常，通常源于配置发现）：返回空工具集和一个合成错误 `{ path: ".mcp.json", error }`。
- **每服务器的运行时/连接失败**：管理器返回部分成功结果及 `errors` 映射；其他服务器继续运行。

因此，当个别 MCP 服务器失败时，不会导致整个 agent 会话启动失败。
...
- `#pendingToolLoads: Map<string, Promise<{ connection, serverTools }>>` — 已连接但工具仍在加载中。
- `#tools: CustomTool[]` — 暴露给调用者的当前 MCP 工具视图。
- `#sources: Map<string, SourceMeta>` — 即使连接尚未完成也保留的提供者/源元数据。

`getConnectionStatus(name)` 从这些映射中派生状态：

- 如果在 `#connections` 中则为 `connected`，
- 如果在待连接或待工具加载中则为 `connecting`，
- 否则为 `disconnected`。
...
7. 尽最大努力缓存工具定义（`MCPToolCache.set`）。

`connectToServer()` 的行为（`src/mcp/client.ts`）：
...
每个待处理的 `toolsPromise` 还有一个后台续延，最终会：
...
`discoverAndLoadMCPTools()` 将管理器工具转换为 `LoadedCustomTool[]`，并装饰路径（已知时格式为 `mcp:<server> via <providerName>`）。

`createAgentSession()` 然后将这些工具推入 `customTools`，工具被包装后以 `mcp_<server>_<tool>` 的命名格式添加到运行时工具注册表中。

### 工具调用

- `MCPTool` 通过已建立连接的 `MCPServerConnection` 调用工具。
- `DeferredMCPTool` 在调用前等待 `waitForConnection(server)`；这使得缓存的工具可以在连接就绪之前就存在。

两者都返回结构化工具输出，并将传输/工具错误转换为 `MCP error: ...` 工具内容（中止仍然是中止）。
...
- 一个服务器失败不会移除健康服务器的工具，
- 连接/列举失败按服务器隔离，
- 工具缓存和后台更新采用尽力而为策略（记录警告/错误，不会硬性停止）。
...
在当前的接入方式中，显式销毁用于 MCP 命令流程（重载/移除/禁用）。启动路径本身没有单独的自动管理器释放钩子；调用者在需要确定性 MCP 关闭时负责调用管理器的断开方法。
...
| 启动时 `tools/list` 仍在等待且无缓存 | 启动等待待处理任务完成 | 硬性等待以确保正确性 |
| 后台工具加载延迟失败 | 在启动门控之后记录 | 尽力而为的日志记录 |
| 运行时传输断开 | 无自动重连；后续调用失败直到重连/重载 | 通过手动操作尽力恢复 |

## 公共 API 接口

`src/mcp/index.ts` 为外部调用者重新导出加载器/管理器/客户端 API。`src/sdk.ts` 暴露 `discoverMCPServers()` 作为便捷包装器，返回相同的加载器结果结构。

## 实现文件

- [`src/mcp/loader.ts`](../../packages/coding-agent/src/mcp/loader.ts) — 加载器外观、发现错误规范化、`LoadedCustomTool` 转换。
- [`src/mcp/manager.ts`](../../packages/coding-agent/src/mcp/manager.ts) — 生命周期状态注册表、并行连接/列举流程、刷新/断开。
- [`src/mcp/client.ts`](../../packages/coding-agent/src/mcp/client.ts) — 传输设置、初始化握手、列举/调用/断开。
- [`src/mcp/index.ts`](../../packages/coding-agent/src/mcp/index.ts) — MCP 模块 API 导出。
- [`src/sdk.ts`](../../packages/coding-agent/src/sdk.ts) — 启动接入会话/工具注册表。
