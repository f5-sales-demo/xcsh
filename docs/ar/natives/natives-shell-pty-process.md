...
تتناول هذه الوثيقة **العمليات الأولية للتنفيذ/العملية/الطرفية** في `@f5xc-salesdemos/pi-natives`: `shell` و`pty` و`ps` و`keys`، باستخدام المصطلحات المعمارية من `docs/natives-architecture.md`.
