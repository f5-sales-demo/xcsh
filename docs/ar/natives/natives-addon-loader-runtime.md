...
يتعمق هذا المستند في طبقة تحميل/التحقق من الإضافات في `@f5xc-salesdemos/pi-natives`: كيف يقرر `native.ts` أي ملف `.node` سيتم تحميله، ومتى يتم تشغيل استخراج الحمولة المضمّنة، وكيف يتم الإبلاغ عن إخفاقات بدء التشغيل.
...
- بناء قائمة مرشحين لأسماء ملفات الإضافات والمجلدات بناءً على المنصة والمعالج.
- تجسيد إضافة مضمّنة اختيارياً في مجلد ذاكرة مؤقتة مُصنّف بالإصدار خاص بكل مستخدم.
- تجربة المرشحين بترتيب حتمي.
- رفض الإضافات القديمة أو غير المتوافقة عبر `validateNative` قبل كشف الارتباطات.

خارج النطاق هنا: سلوك البحث/النص/التمييز الخاص بالوحدات.
...
- **وضع الثنائي المُجمَّع** (`isCompiledBinary`): يكون صحيحاً إذا تحقق أي مما يلي:
...
- **المتغير المحدد**: التجاوز الصريح، وإلا كشف AVX2 أثناء التشغيل على x64 (`modern` إذا كان AVX2 متاحاً، وإلا `baseline`).

## دعم المنصات وتحديد الوسوم
...
- إذا لم يتم تحميل أي شيء، يُلقي خطأ صريح بعدم دعم المنصة مع قائمة الوسوم المدعومة.

هذا يحافظ على تشخيصات مفيدة لحالات الاقتراب مع الفشل الصريح للأهداف غير المدعومة حقاً.

## اختيار المتغير (`modern` / `baseline` / الافتراضي)

### سلوك x64

1. إذا كان `PI_NATIVE_VARIANT` يساوي `modern` أو `baseline`، فتلك القيمة تفوز.
2. وإلا يتم كشف دعم AVX2:
   - لينكس: مسح `/proc/cpuinfo` بحثاً عن `avx2`.
   - macOS: استعلام `sysctl` (`machdep.cpu.leaf7_features`، بديل `machdep.cpu.features`).
...
مبنية من قائمة أسماء الملفات المحددة بالمتغير ويتم البحث فيها بهذا الترتيب:
...
2. يتم تجربة المرشحين المتبقين بدون تكرار بالترتيب.
3. أول مرشح ينجح في كل من `require(...)` ويجتاز `validateNative(...)` يفوز.

## دورة حياة استخراج الإضافة المضمّنة

يُعرّف `embedded-addon.ts` شكل البيان المُولَّد:

- `platformTag`
- `version`
- `files[]` حيث يحتوي كل إدخال على `variant`، `filename`، `filePath`

القيمة الافتراضية المُسجّلة حالياً هي `embeddedAddon: null`؛ قد تستبدل القطع المُجمَّعة هذا ببيانات وصفية حقيقية.

### آلة حالة الاستخراج

يعمل الاستخراج (`maybeExtractEmbeddedAddon`) فقط عندما تتحقق جميع الشروط:

1. `isCompiledBinary === true`
2. `embeddedAddon !== null`
3. `embeddedAddon.platformTag === platformTag`
4. `embeddedAddon.version === packageVersion`
5. تم العثور على ملف مضمّن مناسب للمتغير

يعكس اختيار ملف المتغير نية المتغير أثناء التشغيل:
...
3. وإلا يتم قراءة ملف المصدر المضمّن `filePath` وكتابة الملف المستهدف.
4. إرجاع المسار المستهدف لمحاولة التحميل ذات الأولوية الأعلى.

عند الفشل، لا يتعطل الاستخراج فوراً؛ بل يُلحق إدخال خطأ (فشل إنشاء المجلد أو فشل الكتابة) ويستمر المحمّل في فحص المرشحين العاديين.
...
تفرض `validateNative(bindings, source)` عقداً قائماً على الدوال فقط على `NativeBindings` عند بدء التشغيل.

الآلية:

- لكل اسم تصدير مطلوب، يتم فحص `typeof bindings[name] === "function"`.
- يتم تجميع الأسماء المفقودة.
- إذا كان هناك أي أسماء مفقودة، يُلقي المحمّل:
  - مسار إضافة المصدر،
  - قائمة التصديرات المفقودة،
  - تلميح أمر إعادة البناء.

هذه بوابة توافق صارمة ضد الثنائيات القديمة، والبناءات الجزئية، وانحراف الرموز/الأسماء.

### تعيين واجهة JS البرمجية ↔ التصدير الأصلي (بوابة التحقق)

| اسم الارتباط في JS المفحوص في `validateNative` | اسم التصدير الأصلي المتوقع |
| --- | --- |
| `grep` | `grep` |
| `glob` | `glob` |
...
ملاحظة: يُصرّح `bindings.ts` فقط عن العضو الأساسي `cancelWork(id)`؛ ملفات `types.ts` في الوحدات تُدمج إعلانياً رموزاً إضافية يفرضها `validateNative`.

## سلوك الفشل والتشخيصات

## منصة غير مدعومة

إذا فشل جميع المرشحين و`platformTag` ليس في `SUPPORTED_PLATFORMS`، يُلقي المحمّل:

- `Unsupported platform: <tag>`
- قائمة المنصات المدعومة الكاملة
- توجيهات صريحة للإبلاغ عن المشكلة

## أعراض الثنائي القديم / عدم التطابق
...
- ثنائي `.node` قديم من إصدار/شكل API سابق للحزمة.
- تم اختيار قطعة المتغير الخاطئة (لـ x64).
- تصدير Rust جديد غير موجود في القطعة المحمّلة.

سلوك المحمّل:

- يسجل إخفاقات التصديرات المفقودة لكل مرشح.
- يستمر في فحص المرشحين المتبقين.
- إذا لم يجتاز أي مرشح التحقق، يتضمن الخطأ النهائي كل مسار تمت تجربته مع رسالة الفشل لكل منها.

## إخفاقات بدء تشغيل الثنائي المُجمَّع

في الوضع المُجمَّع تتضمن التشخيصات النهائية:

- مسارات الذاكرة المؤقتة المُصنّفة بالإصدار المتوقعة (`<versionedDir>/<filename>`)،
- معالجة لحذف `<versionedDir>` القديم وإعادة التشغيل،
- أوامر `curl` مباشرة لتنزيل الإصدار لكل اسم ملف متوقع.

## إخفاقات بدء التشغيل غير المُجمَّع

في وضع الحزمة/بيئة التشغيل العادي تتضمن التشخيصات النهائية:

- تلميح إعادة التثبيت (`bun install @f5xc-salesdemos/pi-natives`)،
...
- تعيين `PI_DEV` يُفعّل فقط تشخيصات لكل مرشح في وحدة التحكم (`Loaded native addon...` وأخطاء التحميل).
