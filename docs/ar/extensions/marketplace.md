...
description: نظام سوق الإضافات لاكتشاف وتثبيت وإدارة مجموعات الإضافات المنسقة.
sidebar:
  order: 4
  label: السوق
i18n:
...
يتيح لك نظام السوق اكتشاف الإضافات وتثبيتها وإدارتها من كتالوجات مستضافة على Git. وهو متوافق مع تنسيق سجل إضافات Claude Code.
...
أو اكتب فقط `/marketplace` بدون أي معاملات لفتح متصفح الإضافات التفاعلي.

## المفاهيم

**السوق** هو مستودع Git (أو دليل محلي) يحتوي على ملف كتالوج في المسار `.xcsh-plugin/marketplace.json`. يسرد الكتالوج الإضافات المتاحة مع مصادرها وأوصافها وبياناتها الوصفية.
...
- **المستخدم** (افتراضي) -- متاحة في جميع المشاريع، مخزنة في `~/.xcsh/plugins/installed_plugins.json`
...
| `/marketplace list` | عرض قائمة الأسواق المُعدّة |
...
| `/marketplace installed` | عرض قائمة الإضافات المثبتة من السوق |
...
عند تنفيذ `/marketplace add <source>`، يصنف النظام المصدر:
...
يقوم النظام باستنساخ المستودع (أو قراءة الدليل المحلي)، وتحديد موقع `.xcsh-plugin/marketplace.json`، والتحقق من صحته، وتخزين الكتالوج مؤقتاً محلياً.

## تنسيق الكتالوج (marketplace.json)

يوجد كتالوج السوق في المسار `.xcsh-plugin/marketplace.json` في جذر المستودع:
...
| الحقل | الوصف |
|---|---|
| `name` | اسم السوق. أحرف صغيرة وأرقام وواصلات ونقاط. يجب أن يبدأ وينتهي بحرف أبجدي رقمي. بحد أقصى 64 حرفاً. |
...
- تحتوي فقط على أحرف صغيرة وأرقام وواصلات ونقاط
- بحد أقصى 64 حرفاً

يجب ألا تتجاوز معرفات الإضافات (`name@marketplace`) 128 حرفاً إجمالاً.
