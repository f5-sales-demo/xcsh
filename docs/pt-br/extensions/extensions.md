...
Extensões podem combinar tudo a seguir em um único módulo:

- manipuladores de eventos (`pi.on(...)`)
- ferramentas invocáveis por LLM (`pi.registerTool(...)`)
...
No modo interativo, manipuladores de `input` executam antes da verificação automática de título da primeira mensagem embutida. Extensões que chamam `await pi.setSessionName(...)` a partir de `input` podem definir o nome persistido da sessão e impedir que o título autogerado padrão seja executado para aquela sessão.

Também expostos:

- `pi.logger`
- `pi.typebox`
- `pi.pi` (exports do pacote)
...
`pi.sendUserMessage(content, { deliverAs })` sempre passa pelo fluxo de prompt; durante streaming, enfileira como steer/follow-up.
...
As uniões canônicas de eventos e tipos de payload estão em `types.ts`.
...
`tool_result` segue o estilo middleware: manipuladores executam na ordem das extensões e cada um vê as modificações anteriores.
...
`tool_call`/`tool_result` interceptam todas as ferramentas uma vez que o registro é encapsulado em `sdk.ts`, incluindo ferramentas embutidas e ferramentas de extensão/personalizadas.
...
- toggle de ferramentas expandidas
...
Nota adicional: `setWidget` atualmente roteia para texto na linha de status via `setHookWidget(...)`.

### Modo RPC (`rpc-mode.ts`)

`ctx.ui` é suportado por eventos RPC `extension_ui_request`:

- métodos de diálogo (`select`, `confirm`, `input`, `editor`) fazem ida e volta com respostas do cliente
...
Quando nenhum contexto de UI é fornecido na inicialização do runner, `ctx.hasUI` é `false` e os métodos são no-op/retornam valores padrão.

### Modo interativo em background

O modo background instala um objeto de contexto de UI não interativo. Na implementação atual, `ctx.hasUI` ainda pode ser `true` enquanto diálogos interativos retornam valores padrão/comportamento no-op.
...
3. Mantenha `details` do resultado de ferramenta estruturado quando o estado deve ser visível/reconstruível a partir do histórico de resultados de ferramentas.
...
## Renderizador personalizado de mensagens
...
- Conflitos de nome de comando com comandos embutidos são ignorados com diagnósticos.
...
Se você precisa de um pacote que controle política, ferramentas, UX de comandos e renderização juntos, use extensões.
