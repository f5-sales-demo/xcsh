...
description: >-
  ...
  de instruções específicos por contexto em sessões de agente.
sidebar:
  order: 6
...
Ele reflete a implementação atual, incluindo semânticas parciais e metadados que são parseados mas não aplicados.
...
`src/discovery/index.ts` registra provedores automaticamente. Para `rules`, os provedores atuais são:
...
- frontmatter parseado via `parseFrontmatter`
...
- `description`: mantido apenas se string
...
- `name` do nome do arquivo sem extensão
...
- `alwaysApply`, `description` convertidos do frontmatter
- `ttsr_trigger`: apenas string
- `name` do nome do arquivo para regras de projeto

### Provedor Cline (`cline.ts`)

Busca de forma ascendente a partir de `cwd` pelo `.clinerules` mais próximo:
...
1. O frontmatter é parseado apenas quando o conteúdo começa com `---` e possui um `\n---` de fechamento.
2. O corpo é aparado após a extração do frontmatter.
3. Se o parsing YAML falhar:
   - um aviso é registrado,
   - o parser recorre ao parsing simples de linhas `key: value` (`^(\w+):\s*(.*)$`).
...
- Valores de fallback tornam-se strings (por exemplo `alwaysApply: true` torna-se a string `"true"`), então provedores que exigem tipos booleano/string podem descartar metadados.
...
- Provedores são ordenados por prioridade decrescente.
- Prioridade igual mantém a ordem de registro (`cursor` antes de `windsurf` em `discovery/index.ts`).
- A deduplicação é do tipo primeiro vence: o primeiro nome de regra encontrado é mantido; itens posteriores com o mesmo nome são marcados como `_shadowed` em `all` e excluídos de `items`.
...
Dentro de um provedor, a ordem dos itens vem da ordenação dos resultados do glob de `loadFilesFromDir` mais a ordem explícita de push. Isso é determinístico o suficiente para uso normal, mas não é explicitamente ordenado no código.
...
- **Bucket TTSR**: qualquer regra com `condition` (descrição não obrigatória). Tem prioridade sobre os outros buckets.
...
- Uma regra com ambos `condition` e `alwaysApply` vai apenas para TTSR (TTSR tem prioridade).
- Uma regra com ambos `alwaysApply` e `description` vai apenas para always-apply (não para rulebook).
...
- **Não é aplicado para correspondência automática neste pipeline.** Não há correspondência de glob em tempo de execução selecionando regras pelo arquivo/alvo de ferramenta atual.

### `alwaysApply`

- Parseado e preservado pelos provedores.
- Usado na exibição da UI (rótulo de gatilho `"always"` no gerenciador de estado de extensões).
...
- Se presente, a regra é roteada para o gerenciador TTSR, não para o rulebook.
...
Regras do rulebook são renderizadas em uma seção `# Rules` com:

- `Read rule://<name> when working in matching domain`
- O `name`, `description` e lista opcional de `<glob>` de cada regra

Isso é consultivo/contextual: o texto do prompt pede ao modelo para ler as regras aplicáveis, mas o código não aplica a correspondência de glob.
...
- Regras exclusivamente TTSR e regras sem descrição e sem `alwaysApply` não são endereçáveis via `rule://`.
...
1. Descrições de provedores mencionam arquivos legados (`.cursorrules`, `.windsurfrules`), mas os caminhos de código do carregador atual não leem efetivamente esses arquivos.
2. Metadados de `globs` são expostos ao prompt/UI, mas não são aplicados pela lógica de seleção de regras.
3. A seleção de regras para `rule://` inclui regras de rulebook e always-apply, mas não regras exclusivamente TTSR.
4. Avisos de descoberta (`loadCapability("rules").warnings`) são produzidos, mas `createAgentSession` não os exibe/registra atualmente neste caminho.
