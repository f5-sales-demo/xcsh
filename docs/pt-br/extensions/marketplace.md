...
O sistema de marketplace permite que você descubra, instale e gerencie plugins a partir de catálogos hospedados em Git. Ele é compatível com o formato de registro de plugins do Claude Code.
...
Um **plugin** é um diretório contendo skills, comandos, hooks, servidores MCP ou servidores LSP. Plugins são identificados por `name@marketplace` (ex.: `code-review@f5xc-salesdemos-marketplace`).
...
| `/marketplace update [name]` | Re-buscar catálogo(s); omita o nome para atualizar todos |
...
|---|---|---|
| `owner/repo` | Atalho GitHub | `anthropics/f5xc-salesdemos-marketplace` |
...
Um catálogo de marketplace reside em `.xcsh-plugin/marketplace.json` na raiz do repositório:
...
| Campo | Descrição |
|---|---|
| `name` | Nome do marketplace. Alfanumérico minúsculo, hifens e pontos. Deve começar e terminar com alfanumérico. Máximo de 64 caracteres. |
...
Nomes de marketplace e plugin devem:
