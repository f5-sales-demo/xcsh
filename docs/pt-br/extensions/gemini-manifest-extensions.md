...
Regra importante de escopo: a busca no projeto é **apenas no cwd**. Ela não percorre diretórios ancestrais.
...
A descoberta de manifesto Gemini **não** filtra nomes de diretórios prefixados com ponto. Se um diretório filho oculto existir e contiver `gemini-extension.json`, ele será considerado.
...
- O parse de JSON bem-sucedido é obrigatório.
...
- Se `native` e `gemini` emitirem a extensão com o nome `foo`, o item nativo é mantido.
...
- O carregador Gemini adiciona **usuário primeiro**, depois **projeto**.
...
Em contraste, o provedor nativo constrói a ordem de diretórios de configuração de forma diferente (`project` depois `user` em `getConfigDirs()`), então o sombreamento intra-provedor nativo é na direção oposta.
...
- Nomes duplicados dentro da fonte Gemini são resolvidos com prioridade para o usuário.
- Nomes duplicados contra provedores de maior prioridade (notadamente nativo) perdem por prioridade.
...
Esta fronteira é intencional na implementação atual e explica por que a descoberta de manifestos e o carregamento de módulos executáveis podem divergir.
