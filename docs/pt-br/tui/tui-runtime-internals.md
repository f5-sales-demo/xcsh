...
description: >-
  Detalhes internos do runtime da Terminal UI cobrindo pipeline de renderização,
  tratamento de entrada e gerenciamento de estado.
sidebar:
  order: 2
  label: Detalhes internos do runtime
i18n:
  sourceHash: cc8f7dcce46a
  translator: machine
---

# Detalhes internos do runtime da TUI

Este documento mapeia o caminho do runtime não relacionado a temas, desde a entrada do terminal até a saída renderizada no modo interativo. Ele foca no comportamento em `packages/tui` e sua integração a partir dos controladores de `packages/coding-agent`.

## Camadas do runtime e responsabilidades

- **Engine `packages/tui`**: ciclo de vida do terminal, normalização do stdin, roteamento de foco, agendamento de renderização, pintura diferencial, composição de overlays, posicionamento do cursor de hardware.
- **Modo interativo de `packages/coding-agent`**: constrói a árvore de componentes, vincula callbacks e keymaps do editor, reage a eventos de agente/sessão e traduz o estado do domínio (streaming, execução de ferramentas, retentativas, modo de plano) em componentes de UI.

Regra de fronteira: o engine da TUI é agnóstico em relação a mensagens. Ele conhece apenas `Component.render(width)`, `handleInput(data)`, foco e overlays. A semântica do agente permanece nos controladores interativos.
...
`InteractiveMode` constrói `TUI(new ProcessTerminal(), showHardwareCursor)` e cria containers persistentes:
...
Uma renderização forçada (`requestRender(true)`) reseta os caches de linhas anteriores e o controle do cursor antes de repintar.

## Ciclo de vida do terminal e normalização do stdin

`ProcessTerminal.start()`:

1. Habilita o modo raw e bracketed paste.
2. Registra handler de redimensionamento.
3. Cria um `StdinBuffer` para dividir chunks de escape parciais em sequências completas.
4. Consulta suporte ao protocolo de teclado Kitty (`CSI ? u`), então habilita flags do protocolo se suportado.
5. No Windows, tenta habilitar entrada VT via flags de modo do `kernel32`.
...
- Detecta bracketed paste e emite um evento `paste` com o texto colado em formato bruto.

Isso evita que chunks de escape parciais sejam mal interpretados como teclas normais.
...
5. Após o despacho, a TUI agenda a renderização.

`setFocus()` também alterna `Focusable.focused`, que controla se os componentes emitem `CURSOR_MARKER` para o posicionamento do cursor de hardware.

## Divisão do tratamento de teclas: editor vs controlador

`CustomEditor` intercepta combos de alta prioridade primeiro (escape, ctrl-c/d/z, ctrl-v, variantes de ctrl-p, ctrl-t, alt-up, teclas customizadas de extensão) e delega o restante para o comportamento base do `Editor` (edição de texto, histórico, autocomplete, movimentação do cursor).
...
- comandos de barra e atalhos de seletor
...
4. Adiciona sufixos de reset de segmento para linhas que não são de imagem.
...
6. Para atualizações diferenciais, aplica patch apenas no intervalo de linhas alteradas e limpa linhas finais obsoletas quando necessário.
...
Verificações de segurança críticas na TUI:

- Linhas renderizadas que não são de imagem não devem exceder a largura do terminal; overflow lança exceção e escreve diagnósticos de crash.
...
Essas restrições são impostas em tempo de execução, não são apenas convenções.
...
- O rastreamento de viewport/topo (`#previousViewportTop`, `#maxLinesRendered`) evita cálculos relativos inválidos do cursor quando o conteúdo ou o tamanho do terminal muda.
- A visibilidade do overlay pode depender das dimensões do terminal (`OverlayOptions.visible`); o foco é corrigido quando overlays se tornam não visíveis após o redimensionamento.
...
- `message_update`: atualiza o conteúdo do assistente em streaming; cria/atualiza componentes de execução de ferramentas conforme chamadas de ferramenta aparecem.
- `tool_execution_update/end`: atualiza componentes de resultado de ferramentas e estado de conclusão.
- `message_end`: finaliza o stream do assistente, trata anotações de abortado/erro, marca argumentos pendentes de ferramenta como completos na parada normal.
...
O agrupamento de ferramentas de leitura é intencionalmente stateful (`#lastReadGroup`) para coalescer chamadas consecutivas de ferramentas de leitura em um bloco visual até que ocorra uma quebra de não-leitura.

## Orquestração de status e loader

Responsabilidade da faixa de status:

- `statusContainer` mantém loaders transientes (`loadingAnimation`, `autoCompactionLoader`, `retryLoader`).
- `statusLine` renderiza indicadores persistentes de status/hooks/plano e atualiza a borda superior do editor.
...
- Nos caminhos de fim/cancelamento, os controladores restauram os handlers de escape anteriores e param/limpam os componentes de loader.

## Transições de modo e execução em segundo plano

### Modos de entrada Bash/Python

Prefixos de texto de entrada alternam flags de modo de borda do editor:

- `!` -> modo bash
- `$` (prefixo que não é de template literal) -> modo python
...
`InteractiveMode` rastreia flags de modo de plano, estado da linha de status, ferramentas ativas e troca de modelo. Entrada/saída atualiza entradas de modo de sessão e estado de status/UI, incluindo troca de modelo adiada se o streaming estiver ativo.
...
2. Para a TUI antes de suspender.
...
- Para loaders/linha de status e cancela a inscrição do handler de eventos do primeiro plano.
- Inscreve handler de eventos em segundo plano (principalmente aguarda `agent_end`).
...
- `Escape` durante loader de stream ativo: restaura mensagens enfileiradas no editor e aborta o agente.
- `Escape` durante execução bash/python: aborta o comando em execução.
- `Escape` durante auto-compactação/retentativa: invoca métodos de aborto dedicados através de handlers de escape temporários.
- `Ctrl+C` pressionado uma vez: limpa o editor; pressionado duas vezes em 500ms: encerramento.

O cancelamento é condicional ao estado; a mesma tecla pode significar abortar, sair do modo, acionar seletor ou não fazer nada, dependendo do estado do runtime.
...
- Atualizações de autocomplete do editor (dentro de `Editor`) usam timers de debounce, reduzindo recomputação excessiva durante a digitação.
