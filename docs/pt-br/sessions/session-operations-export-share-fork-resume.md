...
description: 'Operações de sessão para exportar, compartilhar, bifurcar e retomar conversas.'
sidebar:
  order: 3
  label: Operações
i18n:
...
Este documento descreve o comportamento visível ao operador para operações de exportação/compartilhamento/fork/retomada de sessão conforme atualmente implementadas.
...
| `/fork` | Comando slash interativo | Sim (identidade da sessão ativa muda) | Cria novo arquivo de sessão e troca a sessão atual para ele (apenas modo persistente) | Copia diretório de artefatos para o novo namespace de sessão quando presente |
| `/resume` | Comando slash interativo | Sim (estado ativo em memória é substituído) | Troca para arquivo de sessão existente selecionado | Nenhum |
| `--resume` | Inicialização CLI (seletor) | Sim após criação da sessão | Abre arquivo de sessão existente selecionado | Nenhum |
| `--resume <id\|path>` | Inicialização CLI | Sim após criação da sessão | Abre sessão existente; caso cross-project pode bifurcar para o projeto atual | Nenhum |
...
- A análise de argumentos é baseada em espaço em branco (`text.split(/\s+/)`), então caminhos entre aspas com espaços não são preservados como um único caminho neste fluxo de comando.

### `--export <inputSessionFile> [outputPath]` (CLI)

Fluxo em `main.ts`:

1. Tratado antecipadamente (antes da inicialização interativa/sessão).
...
- Este caminho não cria um `AgentSession` e não modifica nenhuma sessão em execução.
...
Nenhuma alteração na persistência da sessão é feita pelo dump.

## Compartilhar

`/share` é apenas interativo e sempre começa exportando a sessão atual para um arquivo HTML temporário.
...
- A UI entra em estado de carregamento `Sharing...`.
- Interpretação do resultado do handler:
  - string => tratado como URL, exibido e aberto
  - object => `url` e/ou `message` exibidos; `url` aberto
  - `undefined`/falsy => genérico `Session shared`
- O arquivo temporário é removido após a conclusão.

Comportamento crítico de fallback:

- Se o handler personalizado existe mas o carregamento falha, o comando gera erro e retorna.
- Se o handler personalizado executa e lança exceção, o comando gera erro e retorna.
- Em ambos os casos de falha, ele **não** faz fallback para GitHub gist.
...
- O carregamento tem hook `onAbort` que restaura a UI do editor e reporta `Share cancelled`.
- O comando subjacente `gh gist create` não recebe um sinal de abort neste fluxo de código; o cancelamento é no nível da UI e verificado após o comando retornar.

## Fork

`/fork` cria uma nova sessão a partir da atual e troca a identidade da sessão ativa.
...
### Fluxo no nível de sessão

`AgentSession.fork()`:

1. Emite `session_before_switch` com `reason: "fork"` (cancelável).
2. Descarrega gravações pendentes.
3. Chama `SessionManager.fork()`.
4. Copia diretório de artefatos do namespace da sessão antiga para o novo namespace (melhor esforço; falhas de cópia que não sejam ENOENT são logadas, não fatais).
...
1. Abre seletor de sessão populado via `SessionManager.list(currentCwd, currentSessionDir)`.
2. Na seleção, `SelectorController.handleResumeSession(sessionPath)` chama `session.switchSession(sessionPath)`.
3. A UI limpa/reconstrói chat e todos, então reporta `Resumed session`.

Notas:

- Este seletor lista apenas sessões no escopo do diretório de sessão atual.
- Ele não usa busca global cross-project.
...
- Se sim: `SessionManager.forkFrom(match.path, cwd, sessionDir)` cria um novo arquivo local bifurcado.
- Se não/padrão não-TTY: o comando gera erro.
...
4. Abre a sessão encontrada; se nenhuma existir, cria nova sessão.

Este é um comportamento apenas de inicialização; não existe um comando slash interativo `/continue`.

## Como a troca de sessão realmente modifica o estado em tempo de execução
...
3. Limpa mensagens de steering/follow-up/next-turn enfileiradas.
4. Descarrega gravações do gerenciador de sessão atual.
5. `sessionManager.setSessionFile(sessionPath)` e atualiza `agent.sessionId`.
6. Constrói contexto da sessão a partir das entradas carregadas.
7. Emite `session_switch` com `reason: "resume"`.
8. Substitui mensagens do agente a partir do contexto.
9. Restaura modelo (se disponível no registro atual).
10. Restaura ou inicializa nível de pensamento.
11. Reconecta a assinatura de eventos do agente.

Nenhum novo arquivo de sessão é criado pelo `switchSession()` em si.

## Emissões de eventos e pontos de cancelamento

### Hooks do ciclo de vida de switch/fork

Para `newSession`, `fork` e `switchSession`:

- Evento antes: `session_before_switch`
  - razões: `new`, `fork`, `resume`
  - cancelável retornando `{ cancel: true }`
- Evento depois: `session_switch`
  - mesmo conjunto de razões
  - inclui `previousSessionFile`

`ExtensionRunner.emit()` retorna antecipadamente no primeiro resultado de evento-antes que cancela.

### Comportamento `onSession` de ferramentas personalizadas

O bridge SDK conecta eventos de sessão de extensão aos callbacks `onSession` de ferramentas personalizadas:
...
- `/fork` é bloqueado durante streaming (o usuário deve esperar/abortar a resposta atual primeiro).
...
- `/share` tem caminho de abort na UI (`Share cancelled`) para o fluxo de gist; ele não implementa semânticas de process-kill para `gh gist create` neste fluxo de código.
...
- `/export` e `/share` falham com `Cannot export in-memory session to HTML` (propagado para UI de erro do comando).
...
## Ressalvas conhecidas da implementação (conforme código atual)
...
- Falhas de `/share` com compartilhamento personalizado não degradam para o fallback padrão de gist; elas terminam o comando com erro.
