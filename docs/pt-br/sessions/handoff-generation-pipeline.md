...
Este documento descreve como o coding-agent implementa o `/handoff` atualmente: caminho de acionamento, prompt de geração, captura de conclusão, troca de sessão e reinjeção de contexto.
...
- Internos genéricos de navegação/ramificação de árvore
...
1. `/handoff` é declarado nos metadados de comandos slash integrados (`slash-commands.ts`) com dica inline opcional: `[focus instructions]`.
...
`expandPromptTemplates: false` impede a expansão de slash/prompt-template nesta carga de instrução interna.

### 2) Captura de conclusão
...
Premissas importantes da extração:
...
1. Descarrega o escritor da sessão atual (`sessionManager.flush()`)
2. Inicia uma sessão completamente nova (`sessionManager.newSession()`)
3. Reseta o estado em memória do agente (`agent.reset()`)
4. Reconecta `agent.sessionId` ao novo id de sessão
...
`newSession()` cria um novo cabeçalho e uma lista de entradas vazia (folha resetada para `null`). No caminho de handoff, nenhum `parentSession` é passado.
...
- Tipo de entrada: `custom_message` (participa no contexto do LLM)

### 6) Reconstruir contexto ativo do agente

Após a injeção:

1. `sessionManager.buildSessionContext()` resolve a lista de mensagens para a folha atual
...
`buildSessionContext()` converte esta entrada em uma mensagem de contexto custom/user em tempo de execução via `createCustomMessage(...)`, de modo que é incluída em prompts futuros da nova sessão.
...
- Em caso de sucesso:
  ...
  - adiciona linha de chat de sucesso: `New session started with handoff context`
- Em caso de exceção:
...
- `isGeneratingHandoff` → true enquanto o controller existe

Quando este caminho de aborto é utilizado, o subscriber do handoff rejeita com `Error("Handoff cancelled")`, e o command controller mapeia para a UI de cancelamento.

### Limitação do caminho interativo `/handoff`

Na ligação atual do controller interativo, `/handoff` não instala um handler dedicado de Escape que chame `abortHandoff()` (diferentemente dos caminhos de compactação/resumo de branch que temporariamente sobrescrevem `editor.onEscape`).

Impacto prático:

- Existe suporte a cancelamento no nível da sessão, mas nenhum hook de keybinding específico para handoff no caminho do comando `/handoff`.
...
Duas proteções impedem handoffs de baixo sinal:
...
## Premissas conhecidas e limitações
...
- O fluxo interativo `/handoff` atualmente carece de um binding dedicado Escape→`abortHandoff()`.
