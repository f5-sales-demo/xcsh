...
- **Compactação** reescreve o histórico antigo em um resumo no branch atual.
- **Resumo de branch** captura o contexto de branches abandonados durante a navegação com `/tree`.
...
Compactação e resumos de branch são entradas de sessão de primeira classe, não mensagens simples de assistente/usuário.

- `CompactionEntry`
  - `type: "compaction"`
  - `summary`, `shortSummary` opcional
  - `firstKeptEntryId` (limite de compactação)
  - `tokensBefore`
  - `details`, `preserveData`, `fromExtension` opcionais
- `BranchSummaryEntry`
  - `type: "branch_summary"`
  - `fromId`, `summary`
  - `details`, `fromExtension` opcionais
...
2. Entradas mantidas de `firstKeptEntryId` até o ponto de compactação são reincluídas.
3. Entradas posteriores no caminho são adicionadas.
4. Entradas `branch_summary` são convertidas em mensagens `branchSummary`.
5. Entradas `custom_message` são convertidas em mensagens `custom`.
...
3. **Compactação automática por limite**: após um turno bem-sucedido quando o contexto excede o limite.

### Formato da compactação (visual)
        ...
        └─────┴─────┴─────┴──────┴─────┴─────┴──────┴──────┴─────┴──────┴─────┘
               └──────────┬──────┘ └──────────────────────┬───────────────────┘
                 não enviado ao LLM                  enviado ao LLM
                                                         ↑
                                              começa a partir de firstKeptEntryId
...
O que o LLM vê:
  ...
  └────────┴─────────┴─────┴─────┴──────┴──────┴─────┴──────┘
       ↑         ↑      └─────────────────┬────────────────┘
    prompt   from cmp          mensagens a partir de firstKeptEntryId
```

### Compactação de overflow-retry vs compactação por limite

Os dois caminhos automáticos são intencionalmente diferentes:

- **Compactação de overflow-retry**
  - Gatilho: erro do assistente do modelo atual é detectado como overflow de contexto.
  - A mensagem de erro do assistente com falha é removida do estado ativo do agente antes da nova tentativa.
  - A compactação automática executa com `reason: "overflow"` e `willRetry: true`.
  - Em caso de sucesso, o agente continua automaticamente (`agent.continue()`) após a compactação.

- **Compactação por limite**
  - Gatilho: `contextTokens > contextWindow - compaction.reserveTokens`.
  - Executa com `reason: "threshold"` e `willRetry: false`.
...
- Proteger os `40_000` tokens de saída de ferramentas mais recentes.
...
Se a poda altera entradas, o armazenamento da sessão é reescrito e o estado das mensagens do agente é atualizado antes das decisões de compactação.

### Lógica de limite e ponto de corte

`prepareCompaction()` considera apenas entradas desde a última entrada de compactação (se houver).

1. Encontrar o índice da compactação anterior.
2. Computar `boundaryStart = prevCompactionIndex + 1`.
3. Adaptar `keepRecentTokens` usando a taxa de uso medida quando disponível.
4. Executar `findCutPoint()` sobre a janela de limite.
...
Se houver entradas de metadados que não são mensagens imediatamente antes do ponto de corte (`model_change`, `thinking_level_change`, rótulos, etc.), elas são puxadas para a região mantida movendo o índice de corte para trás até que um limite de mensagem ou compactação seja atingido.

### Tratamento de turno dividido

Se o ponto de corte não está no início de um turno de usuário, a compactação o trata como um turno dividido.

A detecção de início de turno trata os seguintes como limites de turno de usuário:
...
- segundo passo de turno dividido: `compaction-turn-prefix.md`
- resumo curto para UI: `compaction-short-summary.md`

Modo de sumarização remota:

- Se `compaction.remoteEndpoint` estiver definido, a compactação faz POST de:
  - `{ systemPrompt, prompt }`
- Espera JSON contendo pelo menos `{ summary }`.

### Contexto de operações de arquivo nos resumos

A compactação rastreia atividade cumulativa de arquivos usando chamadas de ferramentas do assistente:
...
- Inclui detalhes de compactação anterior apenas quando a entrada anterior é gerada internamente (`fromExtension !== true`).
- Em turnos divididos, inclui operações de arquivo do prefixo do turno também.
- `readFiles` exclui arquivos também modificados.

O texto do resumo recebe tags de arquivo adicionadas via template de prompt:
...
1. Computar entradas abandonadas da folha antiga até o ancestral comum usando `collectEntriesForBranchSummary(...)`.
2. Se o chamador solicitou resumo (`options.summarize`), gerar resumo antes de trocar a folha.
3. Se o resumo existir, anexá-lo ao alvo de navegação usando `branchWithSummary(...)`.

Operacionalmente, isso é comumente acionado pelo fluxo `/tree` quando `branchSummary.enabled` está habilitado.

### Formato da troca de branch (visual)
...
`generateBranchSummary(...)` computa o orçamento como:

- `tokenBudget = model.contextWindow - branchSummary.reserveTokens`

`prepareBranchEntries(...)` então:

1. Primeiro passo: coleta operações de arquivo cumulativas de todas as entradas resumidas, incluindo detalhes de `branch_summary` anteriores gerados internamente.
2. Segundo passo: percorre do mais recente ao mais antigo, adicionando mensagens até que o orçamento de tokens seja atingido.
3. Prefere preservar contexto recente.
4. Pode ainda incluir entradas de resumo grandes próximas ao limite do orçamento para continuidade.

Entradas de compactação são incluídas como mensagens (`compactionSummary`) durante a entrada de sumarização de branch.

### Geração de resumo e persistência

A sumarização de branch:

1. Converte e serializa mensagens selecionadas.
...
6. Adiciona tags de operação de arquivo.
...
- `prompt` (substituir o prompt base de resumo)
...
Executa na navegação em árvore antes da geração padrão de resumo de branch.
...
## Comportamento em tempo de execução e semântica de falhas

- A compactação manual aborta a operação atual do agente primeiro.
- `abortCompaction()` cancela os controladores de compactação manual e automática.
- A compactação automática emite eventos de sessão de início/fim para atualizações de UI/estado.
- A compactação automática pode tentar múltiplos modelos candidatos e repetir falhas transitórias.
- Erros de overflow são excluídos do caminho genérico de retry porque são tratados pela compactação.
- Se a compactação automática falhar:
  - o caminho de overflow emite `Context overflow recovery failed: ...`
  - o caminho de limite emite `Auto-compaction failed: ...`
- A sumarização de branch pode ser cancelada via sinal de abort (ex.: Escape), retornando resultado de navegação cancelado/abortado.
