...
Essas funções personalizadas são então transformadas em mensagens de usuário voltadas para o LLM em `convertToLlm()` usando os templates estáticos:
...
O que o LLM vê:
  ...
  └────────┴─────────┴─────┴─────┴──────┴──────┴─────┴──────┘
       ↑         ↑      └─────────────────┬────────────────┘
    prompt   do cmp          mensagens a partir de firstKeptEntryId
```
...
- **Compactação por overflow-retry**
  ...
  - A mensagem de erro do assistente que falhou é removida do estado ativo do agente antes da nova tentativa.
...
Se a poda altera entradas, o armazenamento da sessão é reescrito e o estado de mensagens do agente é atualizado antes das decisões de compactação.
...
- entradas de mensagem com funções: `user`, `assistant`, `bashExecution`, `hookMessage`, `branchSummary`, `compactionSummary`
- entradas `custom_message`
- entradas `branch_summary`

Regra rígida: nunca cortar em `toolResult`.

Se houver entradas de metadados não-mensagem imediatamente antes do ponto de corte (`model_change`, `thinking_level_change`, rótulos, etc.), elas são puxadas para a região mantida movendo o índice de corte para trás até que um limite de mensagem ou compactação seja encontrado.

### Tratamento de turno dividido

Se o ponto de corte não está no início de um turno do usuário, a compactação o trata como um turno dividido.

A detecção de início de turno trata estes como limites de turno do usuário:
...
1. Converter mensagens via `convertToLlm()`.
2. Serializar com `serializeConversation()`.
3. Envolver em `<conversation>...</conversation>`.
4. Opcionalmente incluir `<previous-summary>...</previous-summary>`.
5. Opcionalmente injetar contexto de hook como lista `<additional-context>`.
6. Executar o prompt de sumarização com `SUMMARIZATION_SYSTEM_PROMPT`.
...
A compactação rastreia a atividade cumulativa de arquivos usando chamadas de ferramentas do assistente:

- `read(path)` → conjunto de leituras
...
3. Substitui as mensagens ativas do agente com o contexto reconstruído.
...
1. Calcular entradas abandonadas da folha antiga até o ancestral comum usando `collectEntriesForBranchSummary(...)`.
2. Se o chamador solicitou resumo (`options.summarize`), gerar resumo antes de trocar a folha.
3. Se o resumo existir, anexá-lo no alvo de navegação usando `branchWithSummary(...)`.

Operacionalmente, isso é comumente conduzido pelo fluxo `/tree` quando `branchSummary.enabled` está habilitado.
...
1. Primeira passagem: coletar operações cumulativas de arquivo de todas as entradas resumidas, incluindo detalhes anteriores de `branch_summary` gerados internamente.
2. Segunda passagem: percorrer do mais recente ao mais antigo, adicionando mensagens até o orçamento de tokens ser atingido.
3. Preferir preservar contexto recente.
...
5. Adiciona `branch-summary-preamble.md` no início.
...
- `prompt` (sobrescrever o prompt base de resumo)
...
## Comportamento em tempo de execução e semânticas de falha
