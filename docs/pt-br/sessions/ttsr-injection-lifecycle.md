...
Este documento cobre o caminho de execução atual do Time Traveling Stream Rules (TTSR), desde a descoberta de regras até a interrupção do stream, injeção de retry, notificações de extensão e gerenciamento de estado de sessão.
...
`loadCapability("rules")` deduplica por `rule.name` com semântica de primeiro-ganha (prioridade mais alta do provider primeiro). Duplicatas sombreadas são removidas antes do registro TTSR.
...
`TtsrSettings.enabled` é carregado no manager, mas não é verificado atualmente no controle de execução. Se existirem regras, a correspondência ainda é executada.
...
No `turn_start`, o buffer do stream é resetado:
...
- adiciona o delta ao buffer do manager
- chama `check(buffer)`
...
5. evento `ttsr_triggered` é emitido assincronamente (fire-and-forget).
6. o trabalho de retry é agendado via `setTimeout(..., 50)`.

O abort não é bloqueado por callbacks de extensão.
...
5. adiciona uma mensagem sintética do usuário contendo um bloco `<system-interrupt ...>` por regra
...
Injeções pendentes são limpas após a geração do conteúdo.

### Comportamento do `contextMode` na saída parcial

- `discard`: a mensagem parcial/abortada do assistente é removida antes do retry.
- `keep`: a saída parcial do assistente permanece no estado da conversa; o lembrete é adicionado após ela.
...
`messageCount` incrementa no `turn_end`, então o intervalo é medido em turnos completos, não em chunks do stream.
...
`SessionManager` tem suporte completo de schema para persistência de regras injetadas:

- tipo de entrada: `ttsr_injection`
- API de adição: `appendTtsrInjection(ruleNames)`
- API de consulta: `getInjectedTtsrRules()`
- reconstrução de contexto inclui `SessionContext.injectedTtsrRules`
...
- `AgentSession` não adiciona entradas `ttsr_injection` quando o TTSR é acionado.
- `createAgentSession()` não restaura `existingSession.injectedTtsrRules` de volta no `ttsrManager`.

Efeito resultante: a supressão de regras injetadas é aplicada em memória para o processo ativo, mas atualmente não é persistida/restaurada entre recarregamento/retomada de sessão por este caminho.

## 8. Limites de concorrência e garantias de ordenação

### Abort vs callback de retry

- abort é síncrono do ponto de vista do handler TTSR (`agent.abort()` chamado imediatamente)
- retry é adiado por timer (`50ms`)
- notificação de extensão é assíncrona e intencionalmente não aguardada antes do agendamento de abort/retry

### Múltiplas correspondências na mesma janela de stream

`check()` retorna todas as regras elegíveis que correspondem no momento. Elas são injetadas como um lote na próxima mensagem de retry.

### Entre abort e continue

Durante a janela do timer, o estado pode mudar (interrupção do usuário, ações de modo, eventos adicionais). A chamada de retry é feita com melhor esforço: `agent.continue().catch(() => {})` engole erros subsequentes.
...
- `contextMode: "keep"`: saída parcial com violação pode permanecer no contexto antes do retry com lembrete.
- Repeat-after-gap depende de incrementos de contagem de turnos no `turn_end`; chunks no meio do turno não avançam os contadores de intervalo.
