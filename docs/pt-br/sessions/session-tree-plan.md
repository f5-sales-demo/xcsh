...
Este documento descreve como a navegação da árvore de sessão funciona hoje: modelo de árvore em memória, regras de movimentação de folhas, comportamento de ramificação e integração de extensões/eventos.

## O que é este subsistema

A sessão é armazenada como um log de entradas somente de adição (append-only), mas o comportamento em tempo de execução é baseado em árvore:
...
- A ramificação **não** reescreve o histórico; ela apenas altera para onde a folha aponta antes da próxima adição.
...
- `src/modes/components/tree-selector.ts` — comportamento interativo da UI da árvore e filtragem
...
- `src/session/messages.ts` — conversão de entradas `branch_summary`, `compaction` e `custom_message` em mensagens de contexto para o LLM
...
- `#labelsById: Map<string, string>` — rótulos resolvidos por id da entrada alvo
...
- `getChildren(parentId)` retorna filhos diretos
- `getLabel(id)` resolve o rótulo atual de `labelsById`

`getTree()` é uma projeção em tempo de execução; a persistência permanece como entradas JSONL somente de adição.

## Semânticas de movimentação de folha
...
2. `resetLeaf()`
   - Define `leafId = null`
   - A próxima adição cria uma nova entrada raiz (`parentId = null`)

3. `branchWithSummary(branchFromId, summary, details?, fromExtension?)`
   - Aceita `branchFromId: string | null`
   - Define `leafId = branchFromId`
   - Adiciona uma entrada `branch_summary` como filha dessa folha
...
1. Validar o alvo e calcular o caminho abandonado (`collectEntriesForBranchSummary`)
2. Emitir `session_before_tree` com `TreePreparation`
3. Opcionalmente sumarizar entradas abandonadas (resumo fornecido por hook ou sumarizador interno)
4. Calcular novo alvo de folha:
   - selecionando uma mensagem de **usuário**: a folha move para seu pai, e o texto da mensagem é retornado para preenchimento do editor
...
5. Aplicar movimentação de folha:
   - com resumo: `branchWithSummary(newLeafId, ...)`
   - sem resumo e `newLeafId === null`: `resetLeaf()`
   - caso contrário: `branch(newLeafId)`
6. Reconstruir contexto do agente a partir da nova folha e emitir `session_tree`
...
Especificidades de `SessionManager.createBranchedSession(leafId)`:
...
- Modo persistente: escreve novo arquivo JSONL e alterna o gerenciador para ele; retorna o novo caminho do arquivo.
- Modo em memória: substitui entradas em memória; retorna `undefined`.

## Reconstrução de contexto e integração de resumo/custom

`buildSessionContext()` (em `session-manager.ts`) resolve o caminho ativo raiz→folha e constrói o estado de contexto LLM efetivo:
...
- `branchSummary` e `compactionSummary` se tornam mensagens de contexto com template de papel de usuário
- `custom`/`hookMessage` se tornam mensagens de conteúdo com papel de usuário

Portanto, a movimentação na árvore altera o contexto mudando o caminho ativo da folha, não mutando entradas antigas.

## Rótulos e comportamento da UI da árvore

Persistência de rótulos:

- `appendLabelChange(targetId, label?)` escreve entradas `label` na cadeia de folhas atual.
...
- Aplana a árvore para navegação, mantém destaque do caminho ativo e prioriza a exibição do branch ativo primeiro.
...
## Pontos de contato de extensões e hooks para operações de árvore

API de extensão em tempo de comando (`ExtensionCommandContext`):

- `branch(entryId)` — criar arquivo de sessão ramificada
- `navigateTree(targetId, { summarize? })` — mover dentro da árvore/arquivo atual

Eventos relacionados à navegação na árvore:
...
- `session_before_tree`
  ...
  - pode fornecer payload de resumo usado no lugar do sumarizador interno
  - recebe `signal` de aborto (caminho de cancelamento por Escape)
- `session_tree`
...
- `branch()` não pode ter `null` como alvo; use `resetLeaf()` para o estado raiz-antes-da-primeira-entrada.
...
- Se a sumarização é abortada, a navegação é cancelada e a folha permanece inalterada.
...
- v2→v3 migra o papel legado `hookMessage` para `custom`

O comportamento atual em tempo de execução segue a semântica de árvore versão 3 após a migração.
