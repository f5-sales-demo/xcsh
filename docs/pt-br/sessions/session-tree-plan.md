...
Este documento descreve como a navegação da árvore de sessão funciona atualmente: modelo de árvore em memória, regras de movimentação de folha, comportamento de ramificação e integração com extensões/eventos.
...
3. `branchWithSummary(branchFromId, summary, details?, fromExtension?)`
   ...
   - Adiciona uma entrada `branch_summary` como filha daquela folha
...
4. Computar novo alvo da folha:
   ...
   - selecionando uma **custom_message**: mesma regra que mensagem de usuário (folha = pai, texto preenche o editor)
...
Importante: entradas de resumo são anexadas na **nova posição de navegação**, não no final do branch abandonado.
...
`buildSessionContext()` (em `session-manager.ts`) resolve o caminho ativo raiz→folha e constrói o estado de contexto efetivo do LLM:
...
- Trata a compactação mais recente no caminho:
  ...
  - reproduz mensagens mantidas de `firstKeptEntryId` até o ponto de compactação
  - depois reproduz mensagens pós-compactação
...
- `branchSummary` e `compactionSummary` tornam-se mensagens de contexto com template de papel de usuário
- `custom`/`hookMessage` tornam-se mensagens de conteúdo com papel de usuário
...
- `appendLabelChange(targetId, label?)` escreve entradas `label` na cadeia de folha atual.
- `labelsById` é atualizado imediatamente (definir ou excluir).
...
- `session_before_tree`
  ...
  - pode fornecer payload de resumo usado no lugar do sumarizador integrado
...
- `session_tree`
  ...
  - `fromExtension` indica origem do resumo
...
- `branch()` não pode apontar para `null`; use `resetLeaf()` para o estado raiz-antes-da-primeira-entrada.
...
- A sumarização requer um modelo ativo; se ausente, a navegação com sumarização falha imediatamente.
...
O comportamento atual em tempo de execução é a semântica de árvore versão 3 após a migração.
