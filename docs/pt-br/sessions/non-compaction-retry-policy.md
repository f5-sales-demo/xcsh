...
Ele exclui explicitamente a recuperação de estouro de contexto via auto-compactação. O estouro é tratado pela lógica de compactação e é documentado separadamente em [`compaction.md`](./compaction.md).
...
4. Erros de estouro de contexto são rigidamente excluídos da classificação de retry (`isContextOverflow(...)` interrompe o retry antecipadamente).
5. O estouro, portanto, segue para `#checkCompaction(...)` em vez do retry padrão.

Resumindo: falhas do tipo sobrecarga/rate limit/servidor/rede usam esta política de retry; estouro de janela de contexto usa recuperação por compactação.
...
Conjunto atual de padrões retentáveis (baseados em regex):

- overloaded
- rate limit / usage limit / too many requests
- Classes de servidor similares a HTTP: 429, 500, 502, 503, 504
- service unavailable / server error / internal error
- connection error / fetch failed
- termos com `retry delay`
...
Estado da sessão utilizado pelo retry:
...
9. Remove a mensagem de erro do assistente no final do estado de execução do agente (mantida no histórico de sessão persistido).
10. Executa sleep com suporte a abort.
11. Ao acordar, agenda `agent.continue()` via `setTimeout(..., 0)`.

### O que reseta os contadores de retry

`#retryAttempt` é resetado para `0` nestes casos:
...
- caminho de retries máximos excedidos

`#retryPromise` é resolvida/limpa quando a cadeia de retry termina (sucesso, cancelamento ou máximo excedido), via `#resolveRetry()`.

## Semântica de backoff e tentativas máximas
...
- o contador de tentativas é incrementado antes da verificação do máximo
- eventos de início usam a tentativa atual (base 1)
- o evento de fim por exceder o máximo reporta `attempt: this.#retryAttempt - 1` (contagem da última tentativa de retry)
...
Entradas de substituição de atraso são usadas apenas no caminho de tratamento de limite de uso, e apenas para influenciar a decisão de troca de modelo/conta no armazenamento de autenticação. No caminho principal de retry fora da compactação, o backoff permanece com atraso exponencial local, a menos que a troca seja bem-sucedida (`delayMs = 0`).
...
- resolve a promise de retry (`#resolveRetry()`) para que os aguardadores sejam desbloqueados

Se o abort ocorre durante o sleep, o caminho de captura emite:

- `auto_retry_end { success: false, finalError: "Retry cancelled" }`
- reseta tentativa/controller
...
Em `auto_retry_start`, o EventController:
...
Em `auto_retry_end`, restaura o handler anterior de `Esc` e limpa o estado de carregamento.
...
- uma chamada de prompt não é totalmente resolvida até que qualquer cadeia de retry iniciada termine (sucesso/falha/cancelamento)
...
Definidas no schema de configurações sob o grupo retry:

- `retry.enabled`
- `retry.maxRetries`
- `retry.baseDelayMs`

Alternâncias programáticas na sessão:

- `setAutoRetryEnabled(enabled)` escreve `retry.enabled`
- `autoRetryEnabled` lê `retry.enabled`
- `isRetrying` reporta se a promise do ciclo de vida do retry está ativa
...
Ambos os comandos retornam respostas de sucesso; detalhes de progresso/falha do retry vêm de eventos de sessão transmitidos por stream, não dos payloads de resposta dos comandos.

## Emissão de eventos e exposição de falhas

Eventos de retry a nível de sessão:
...
- encaminhados ao executor de extensões como eventos de extensão
...
O retry para e não continuará automaticamente quando qualquer uma destas condições ocorrer:
...
- retries máximos excedidos
- o usuário cancela o retry (`abort_retry` ou `Esc` durante o carregamento do retry)
- abort global (`abort`) cancela o retry primeiro

Uma nova cadeia de retry ainda pode ser iniciada posteriormente em um futuro erro retentável após os contadores serem resetados.

## Ressalvas operacionais

- A classificação é por correspondência de texto via regex; erros estruturados específicos do provedor não são utilizados aqui.
- O retry remove o erro do assistente que falhou do **contexto de execução** antes de re-continuar, mas o histórico da sessão ainda mantém essa entrada de erro.
