...
O conteúdo do breadcrumb consiste em duas linhas: o cwd original, seguido pelo caminho do arquivo de sessão. `continueRecent()` prefere este ponteiro com escopo de terminal antes de buscar o mtime mais recente.
...
- As entradas são append-only em tempo de execução; a navegação entre branches move um ponteiro (`leafId`) em vez de mutar entradas existentes.
...
Armazena uma `AgentMessage` diretamente.
...
- Adiciona `id` e `parentId` a cada entrada que não seja cabeçalho.
- Reconstrói uma cadeia de parentesco linear usando a ordem do arquivo.
- Migra o campo de compactação `firstKeptEntryIndex` -> `firstKeptEntryId` quando presente.
- Define `version = 2` no cabeçalho.

### v2 -> v3

Aplicada quando `version` do cabeçalho `< 3`:
...
- A migração muta as entradas em memória primeiro, depois persiste o JSONL reescrito.
...
- Linhas não parseáveis são tratadas pelo parser JSONL tolerante (`parseJsonlLenient`).
- Se a primeira entrada parseada não for um cabeçalho de sessão válido (`type !== "session"` ou `id` de string ausente) -> retorna `[]`.

Comportamento de `SessionManager.setSessionFile()`:

- `[]` retornado pelo loader é tratado como sessão vazia/inexistente e substituído por um novo arquivo de sessão inicializado naquele caminho.
...
- `branchWithSummary()` define o leaf para o alvo do branch e adiciona uma entrada `branch_summary`.
...
2. Percorre a cadeia `parentId` da folha até a raiz e inverte para caminho raiz->folha.
3. Deriva o estado em tempo de execução ao longo do caminho:
   ...
   - fallback `models.default` do provider/modelo da mensagem do assistente se não houver mudança explícita de modelo
...
4. Construir lista de mensagens:
   ...
   - Entradas `custom_message` tornam-se `custom` AgentMessages via `createCustomMessage`
   - Entradas `branch_summary` tornam-se `branchSummary` AgentMessages via `createBranchSummaryMessage`
   - Se uma `compaction` existe no caminho:
     - emite o resumo de compactação primeiro (`createCompactionSummaryMessage`)
     - emite entradas do caminho começando em `firstKeptEntryId` até o limite de compactação
     - emite entradas após o limite de compactação

Entradas `custom` e `session_init` não injetam contexto do modelo diretamente.

## Garantias de Persistência e Modelo de Falhas

### Persistente vs em memória
...
As escritas são serializadas através de uma cadeia de promises interna (`#persistChain`) e `NdjsonFileWriter`.
...
- A persistência é adiada até que pelo menos uma mensagem do assistente exista.
  - Antes do primeiro assistente: entradas são mantidas em memória; nenhum append em arquivo ocorre.
  - Quando o primeiro assistente existe: a sessão completa em memória é descarregada para o arquivo.
  - Após isso: novas entradas são adicionadas incrementalmente.
...
- Reescritas atômicas completas (`#rewriteFile`) escrevem em arquivo temporário, flush+fsync, close, e então renomeiam sobre o alvo.
...
- O primeiro erro é registrado uma vez com contexto do arquivo de sessão.
- O fechamento do writer é best-effort, mas propaga o primeiro erro significativo.
...
- Blocos de imagem em arrays `content` com comprimento base64 >= 1024 são externalizados para referências de blob:
  ...
  - bytes brutos escritos no blob store (`BlobStore.put`)
...
A interface `SessionStorage` fornece todas as operações de sistema de arquivos usadas pelo `SessionManager`:
...
Use arquivos de sessão para grafo de conversação/replay de estado; use `HistoryStorage` para UX de histórico de prompts.
