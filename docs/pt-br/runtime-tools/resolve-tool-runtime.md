...
# Funcionamento interno do runtime da ferramenta Resolve
...
`resolve` é uma ferramenta oculta que finaliza uma ação pendente de pré-visualização.
...
`resolve` sempre consome a ação pendente **mais ao topo** primeiro (`pop()`), então múltiplas ferramentas produtoras de pré-visualização são resolvidas na ordem inversa de registro.

## Exemplo de produtor integrado (`ast_edit`)

`ast_edit` pré-visualiza substituições estruturais primeiro. Quando a pré-visualização possui substituições e ainda não foi aplicada, ele empilha uma ação pendente que contém:
...
Ferramentas personalizadas podem registrar ações pendentes compatíveis com resolve através de `CustomToolAPI.pushPendingAction(...)`.
...
## Disponibilidade em runtime e falhas

`pushPendingAction` é conectado pelo carregador de ferramentas personalizadas usando o `PendingActionStore` da sessão ativa.

Se o runtime não possui um store de ações pendentes, `pushPendingAction` lança:

- `Pending action store unavailable for custom tools in this runtime.`

## Comportamento de escolha de ferramenta

Quando `PendingActionStore.hasPending` é verdadeiro, o runtime do agente direciona a escolha de ferramenta para `resolve`, de modo que as pré-visualizações pendentes sejam explicitamente finalizadas antes que o fluxo normal de ferramentas continue.

## Orientações para desenvolvedores

- Use ações pendentes apenas para operações destrutivas ou de alto impacto que devem suportar aplicação/descarte explícito.
- Mantenha `label` conciso e específico; ele é exibido na saída do renderizador de resolve.
- Garanta que `apply(reason)` seja determinístico e idempotente o suficiente para execução única; `reason` é informacional e não deve alterar o comportamento.
- Implemente `reject(reason)` quando o descarte necessita de limpeza (estado temporário, locks, notificações); omita-o para pré-visualizações sem estado onde a mensagem padrão é suficiente.
- Se sua ferramenta pode preparar múltiplas pré-visualizações, lembre-se da semântica LIFO: a última ação empilhada é resolvida primeiro.
