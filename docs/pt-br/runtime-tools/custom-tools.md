...
1. **Ferramentas personalizadas fornecidas pelo SDK** (`options.customTools`)
   ...
   - Sempre incluídas no conjunto inicial de ferramentas ativas na inicialização do SDK.
...
- Conflitos de nomes de ferramentas são rejeitados em relação às ferramentas integradas e ferramentas personalizadas já carregadas.
...
- `pushPendingAction(action)`: registrar uma ação de preview para a ferramenta oculta `resolve` (`docs/resolve-tool-runtime.md`)

O loader inicia com um contexto de UI no-op e requer que o código host chame `setUIContext(...)` quando a UI real estiver pronta.
...
- `params` é estaticamente tipado a partir do seu esquema TypeBox via `Static<TParams>`.
- A validação de argumentos em tempo de execução acontece antes da execução no loop do agente.
- `onUpdate` emite resultados parciais para streaming na UI.
- `ctx` inclui estado da sessão/modelo e um helper `abort()`.
- `signal` carrega o cancelamento.

`CustomToolAdapter` faz a ponte para a interface de ferramenta do agente e encaminha chamadas na ordem correta de argumentos.
...
- Na inicialização do SDK, ferramentas personalizadas e registradas por extensão são incluídas forçadamente no conjunto ativo inicial.
...
- Erros do renderer são capturados e registrados em log; a UI retorna à renderização de texto padrão.
...
- Prefira saídas determinísticas e com formato de esquema em `details` para reconstrução de renderer/estado.
