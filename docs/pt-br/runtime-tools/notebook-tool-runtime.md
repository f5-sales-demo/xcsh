---
title: Internos do Runtime da Ferramenta Notebook
description: >-
...
A distinção crítica: **`notebook` é um editor JSON de notebooks, não um executor de notebooks**. Ele edita os fontes de células `.ipynb` diretamente; ele não inicia nem se comunica com um kernel Python.
...
- Resolve caminho relativo ao CWD da sessão (`resolveToCwd`).
...
- nenhuma captura de display rico (`image/png`, display JSON, MIME de status)

## Caminho de execução estilo notebook (`src/tools/python.ts` + `src/ipy/*`)

Quando o agente precisa executar código Python estilo célula (células sequenciais, estado persistente, displays ricos), isso passa pela ferramenta **`python`**, não pela `notebook`.
...
`content` é dividido em `source: string[]` com preservação de quebra de linha:

- cada linha não-final mantém o `\n` final
- a linha final não tem quebra de linha final forçada

Isso espelha as convenções JSON de notebook e evita concatenação acidental de linhas em edições posteriores.
...
- `delete`
  ...
  - retorna o `source` removido nos detalhes para pré-visualização do renderer
...
Estas se tornam respostas de ferramenta `Error:` no upstream; o renderer usa o caminho do notebook + texto de erro formatado.
...
- `session` (padrão)
  - kernels em cache no mapa `kernelSessions`
  - máximo 4 sessões; a mais antiga é removida em overflow
  - limpeza de ociosos/mortos a cada 30s, timeout após 5 minutos
  - fila por sessão serializa a execução (`session.queue`)
- `per-call`
  - cria kernel para a requisição
  - executa
  - sempre encerra o kernel no `finally`

## Comportamento de reset

A ferramenta `python` passa `reset` apenas para a primeira célula em uma chamada multi-célula; as células seguintes sempre executam com `reset: false`.

## Morte do kernel / reinício / retry
...
- caminho de crash em tempo de execução faz retry uma vez: reinicia o kernel, reexecuta o handler.
...
## 5) Streaming/chunk e manipulação de display (caminho apoiado por kernel)
...
## 6) Truncamento e comportamento de artefatos
...
- quando o buffer em memória excede o limite (`DEFAULT_MAX_BYTES` a menos que sobrescrito):
  - marca como truncado
...
A ferramenta `python` converte esses metadados em avisos de truncamento de resultado e avisos de TUI.
...
## 7) Suposições do renderer e formatação

## Renderer do notebook (`notebookToolRenderer`)
...
- visualização de resultado:
  ...
  - limite de pré-visualização recolhida é `PREVIEW_LIMITS.COLLAPSED_LINES * 2`
  - suporta modo expandido via opções de renderização compartilhadas
  - usa cache de renderização chaveado por largura + estado expandido

Suposição de renderização de erro:

- se o primeiro conteúdo de texto começa com `Error:`, o renderer formata como bloco de erro de notebook.
...
Este comportamento do renderer não tem relação com os resultados de edição JSON do `notebook`, exceto que ambos reutilizam primitivas de TUI compartilhadas.
...
- `python` executa código em um kernel, persiste estado por modo, faz streaming de chunks, captura displays ricos, lida com interrupções/timeouts e suporta truncamento de saída/artefatos.
- `notebook` realiza apenas mutações determinísticas de JSON de notebook; sem execução, sem estado de kernel, sem stream de chunks, sem saídas de display, sem pipeline de artefatos.
...
A implementação atual não fornece uma ferramenta única que tanto muta `.ipynb` quanto executa células do notebook através do contexto do kernel.
