...
`packages/natives/src/native.ts` é responsável pela seleção do addon em tempo de execução, extração opcional e validação de exportações.
...
Se um manifesto de addon incorporado estiver presente (`embedded-addon.ts` gerado por `scripts/embed-native.ts`), `native.ts` pode materializar o binário incorporado correspondente no diretório de cache versionado antes do carregamento.
...
- **Erros de extração do addon incorporado**: registra falhas de diretório/escrita e as inclui nos diagnósticos finais de carregamento
...
**Detalhe de implementação (pode mudar):** ordem exata de busca de candidatos e ordenação do caminho de fallback de binário compilado.
...
Esses módulos implementam os símbolos N-API consumidos e validados por `native.ts`. Os nomes em nível JS são expostos através dos wrappers TS em `packages/natives/src`.
...
Em nível de arquitetura, a propriedade é dividida da seguinte forma:
...
- **Propriedade do loader (`packages/natives/src/native.ts`)**
  - seleção de binário em tempo de execução
  - seleção de variante de CPU e tratamento de sobrescrita
...
4. Extração opcional do binário incorporado ocorre para distribuições compiladas.
...
- **Variante**: Flavor de build específica para CPU x64 (`modern` AVX2, `baseline` fallback).
- **Wrapper**: Função/classe TS que fornece API tipada sobre exportações nativas brutas.
- **Declaration merging**: Técnica TS utilizada por arquivos `types.ts` dos módulos para estender `NativeBindings`.
...
- **Addon incorporado**: Metadados de artefato de build e referências de arquivo gerados em `embedded-addon.ts` para que binários compilados possam extrair payloads `.node` correspondentes.
- **Gate de validação**: Verificação `validateNative(...)` que rejeita binários desatualizados/incompatíveis com exportações necessárias ausentes.
