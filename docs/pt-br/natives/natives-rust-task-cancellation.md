---
title: Execução e Cancelamento Nativos de Tarefas Rust
description: >-
  Modelo de execução de tarefas assíncronas Rust com cancelamento cooperativo e
  semânticas de limpeza.
sidebar:
  order: 5
...
# Execução e cancelamento nativos de tarefas Rust (`pi-natives`)
...
`task.rs` define três peças fundamentais:
...
3. `CancelToken` / `AbortToken` / `AbortReason`
   - `CancelToken::new(timeout_ms, signal)` combina prazo + `AbortSignal` opcional.
   ...
   - `AbortToken` permite que código externo solicite cancelamento (`abort(reason)`).
...
- O future pode competir conclusão normal contra `ct.wait()`.
- No caminho de cancelamento, implementações assíncronas tipicamente propagam o cancelamento para subsistemas internos (ex.: `tokio_util::CancellationToken`) e opcionalmente forçam abort no timeout de graça.

## Mapeamento API JS ↔ exportação Rust (relevante para task/cancel)

| API voltada para JS | Exportação Rust (`#[napi]`) | Agendador | Conexão de cancelamento |
|---|---|---|---|
...
| `shell.run(options, onChunk?)` | `Shell::run` | `task::future(env, "shell.run", ...)` | `ct.wait()` em corrida contra a tarefa de execução; conecta ao Tokio `CancellationToken` |
...
- **Antes do início / antes da primeira verificação de cancelamento**:
  - Usuários de `task::future` que competem em `ct.wait()` podem resolver o cancelamento imediatamente assim que entram no `select!`.
...
- **Durante a execução**:
  ...
  - `future`: o branch `ct.wait()` vence o `select!`, então o código cancela a maquinaria assíncrona subordinada (para shell: cancela o token Tokio, espera até 2s, então aborta a tarefa).
...
- `grep_sync`: verificação explícita de cancelamento antes da fase de busca pesada, além de chamadas ao fs-cache que também recebem o token.
- `run_pty_sync`: verifica a cada tick do loop (~16ms de cadência de sleep) e encerra o processo filho no cancelamento.

Regra prática: nenhum loop sobre entrada de tamanho externo deve exceder um intervalo curto e limitado sem um heartbeat.
...
1. Closure retorna `Err(napi::Error)` (incluindo abort de `heartbeat()`).
...
### Divisão de relatório de cancelamento

- **Abort como erro**: a maioria das exportações bloqueantes usando `heartbeat()?`.
- **Abort como resultado tipado**: APIs estilo shell/pty de comandos que modelam cancelamento em structs de resultado.
...
1. **Heartbeat ausente em loops bloqueantes**
   ...
   - Correção: adicione `ct.heartbeat()?` no topo do loop e antes de etapas caras por item.

2. **Seções longas não canceláveis**
   - Sintoma: picos de latência de cancelamento durante uma única chamada grande (decodificação, ordenação, compressão, etc.).
   - Correção: divida o trabalho em blocos com limites de heartbeat; se impossível, documente a latência.

3. **Bloqueando o executor assíncrono**
   - Sintoma: API assíncrona trava quando código pesado em sincronismo executa diretamente no future.
   - Correção: mova blocos CPU/sync para `task::blocking` ou `tokio::task::spawn_blocking`.

4. **Semânticas de cancelamento inconsistentes**
...
5. **Esquecimento da ponte de cancelamento em tarefas assíncronas aninhadas**
   - Sintoma: token externo é cancelado, mas tarefas internas de leitura/subprocesso continuam executando.
   - Correção: conecte o cancelamento ao token/signal interno e aplique timeout de graça + fallback de abort forçado.

## Checklist para novas exportações canceláveis

1. Classifique o trabalho corretamente:
   - CPU-bound ou bloqueio síncrono -> `task::blocking`
...
3. Conecte o cancelamento em todas as camadas:
   - loops bloqueantes: `ct.heartbeat()?` em intervalos estáveis
   - orquestração assíncrona: corrida com `ct.wait()` e cancelamento de sub-tarefas/tokens

4. Decida o contrato de cancelamento:
   - rejeitar promise com erro de abort, ou
   - resolver com tipagem `{ cancelled, timedOut, ... }`
...
6. Trate cancelamento antes do início e durante o voo:
   - verificação/await de cancelamento deve acontecer antes do corpo caro e durante execução longa

7. Valide que não há uso incorreto do executor:
   - nenhum trabalho síncrono longo diretamente dentro de futures assíncronos sem wrapper `spawn_blocking`/blocking task
