...
Este documento é um aprofundamento de subsistema para a camada de **primitivas de sistema/mídia/conversão** descrita em [`docs/natives-architecture.md`](./natives-architecture.md): `image`, `html`, `clipboard` e perfilamento de `work`.
...
> Nota: não existe `crates/pi-natives/src/work.rs`; o perfilamento de work é implementado em `prof.rs` e alimentado pela instrumentação em `task.rs`.
...
- **Limite de entrada JS**: `Uint8Array` com bytes de imagem codificados.
...
- **Caminho de texto**:
  ...
  - O mesmo texto é então tentado via API nativa de clipboard (`native.copyToClipboard`) como melhor esforço.
...
- **Limite de saída**: `getWorkProfile(lastSeconds)` retorna um objeto:
...
- Falha do conversor retorna promise rejeitada (`Conversion error: ...`).
...
1. O TS bloqueia rigidamente contextos de runtime não suportados (Termux/Linux headless) para `null`.
2. A leitura do `arboard` no Rust executa apenas quando o TS permite.
3. `ContentNotAvailable` é mapeado para `null`.
4. Outros erros do Rust rejeitam.

### Ciclo de vida do perfilamento de work

1. Sem início explícito: o perfilamento está sempre ativo quando os helpers de tarefa executam.
2. Cada escopo de tarefa instrumentado registra uma amostra quando o guard é descartado.
3. As amostras sobrescrevem as entradas mais antigas após a capacidade do buffer ser atingida.
...
- O truncamento do buffer é comportamento esperado (ring buffer), não bug de perda de dados.
