...
- Descoberta de `mcp.json` standalone: `packages/coding-agent/src/discovery/mcp-json.ts`
- Schema: `packages/coding-agent/src/config/mcp-schema.json`

## Locais de configuração preferidos

O OMP pode descobrir servidores MCP de múltiplas ferramentas (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, e mais), mas para configuração nativa do OMP você normalmente deve usar um destes arquivos:

- Projeto: `.xcsh/mcp.json`
- Usuário: `~/.xcsh/mcp.json`

O OMP também aceita arquivos standalone de fallback na raiz do projeto:

- `mcp.json`
- `.mcp.json`

Use `.xcsh/mcp.json` quando quiser que o OMP seja o dono da configuração. Use `mcp.json` / `.mcp.json` na raiz apenas quando quiser um arquivo de fallback portável que outros clientes MCP também possam ler.

## Adicionar uma referência ao schema
...
Nomes de servidores devem corresponder a `^[a-zA-Z0-9_.-]{1,100}$`.
...
O OMP entende dois objetos relacionados à autenticação.
...
O Slack é o exemplo mais claro atualmente. O servidor MCP do Slack é hospedado em `https://mcp.slack.com/mcp`, usa Streamable HTTP e requer OAuth confidencial com as credenciais de cliente do seu aplicativo Slack.
...
Endpoints relevantes do Slack conforme a documentação do Slack:
...
### Servidor GitHub hospedado via HTTP
...
### Servidor GitHub local via Docker
...
### Servidor Slack hospedado via OAuth
...
Esta é a parte que geralmente causa confusão.

### Em `.xcsh/mcp.json` e `~/.xcsh/mcp.json`

Antes de o OMP iniciar um servidor ou fazer uma requisição HTTP, ele resolve valores de `env` e `headers` da seguinte forma:
...
- `"GITHUB_PERSONAL_ACCESS_TOKEN": "GITHUB_PERSONAL_ACCESS_TOKEN"` → copia do ambiente do shell atual
...
O carregador de fallback standalone também expande `${VAR}` e `${VAR:-default}` dentro de strings durante a descoberta.
...
## `/mcp add` vs editar JSON diretamente

Use `/mcp add` quando quiser uma configuração guiada.

Use edição direta de JSON quando:
...
## Regras de validação aplicadas pelo OMP
...
- Se você colar a configuração de um servidor remoto e esquecer `"type": "http"`, o OMP o tratará como `stdio` e reclamará que `command` está faltando
- `sse` continua válido por compatibilidade, mas novos servidores hospedados geralmente devem ser configurados como `http`

## Descoberta e precedência

O OMP não mescla definições de servidor duplicadas entre arquivos. Os provedores de descoberta são priorizados, e a definição de maior prioridade prevalece.
...
- use `disabledServers` na configuração do usuário quando uma configuração de terceiros continua reintroduzindo um servidor que você não quer
...
Execute `/mcp list`. O OMP descobre muitos arquivos MCP de terceiros, mas o carregamento em nível de projeto também pode ser desabilitado pela configuração `mcp.enableProjectConfig`.
