...
# Configuração do MCP no OMP

Este guia explica como adicionar, editar e validar servidores MCP para o agente de codificação do OMP.

Fonte de verdade no código:

- Tipos de configuração do runtime: `packages/coding-agent/src/mcp/types.ts`
...
- Descoberta de `mcp.json` autônomo: `packages/coding-agent/src/discovery/mcp-json.ts`
...
Use `.xcsh/mcp.json` quando quiser que o OMP seja o responsável pela configuração. Use `mcp.json` / `.mcp.json` na raiz apenas quando quiser um arquivo de fallback portátil que outros clientes MCP também possam ler.
...
- `disabledServers` — lista de bloqueio a nível de usuário usada para desativar servidores descobertos por nome

Os nomes de servidores devem corresponder a `^[a-zA-Z0-9_.-]{1,100}$`.

## Campos de servidor suportados

Campos compartilhados para todos os transportes:

- `enabled?: boolean` — pula este servidor quando `false`
- `timeout?: number` — tempo limite de conexão em milissegundos
- `auth?: { ... }` — metadados de autenticação usados pelo OMP para fluxos OAuth/API-key
- `oauth?: { ... }` — configurações explícitas de cliente OAuth usadas durante autenticação/reautenticação
...
`sse` ainda é suportado para compatibilidade, mas a especificação MCP agora prefere Streamable HTTP (`type: "http"`) para novos servidores.

## Campos de autenticação

O OMP entende dois objetos relacionados a autenticação.
...
O Slack é o exemplo atual mais claro. O servidor MCP do Slack é hospedado em `https://mcp.slack.com/mcp`, usa Streamable HTTP e requer OAuth confidencial com as credenciais de cliente do seu app Slack.
...
Endpoints relevantes do Slack a partir da documentação do Slack:
...
Isso corresponde à imagem Docker local oficial do GitHub `ghcr.io/github/github-mcp-server`.
...
Antes do OMP iniciar um servidor ou fazer uma requisição HTTP, ele resolve valores de `env` e `headers` assim:

1. Se um valor começa com `!`, o OMP o executa como um comando shell e usa o stdout com espaços removidos.
...
- `"GITHUB_PERSONAL_ACCESS_TOKEN": "GITHUB_PERSONAL_ACCESS_TOKEN"` → copia do ambiente shell atual
- `"Authorization": "Bearer hardcoded-token"` → usa o valor literal
- `"Authorization": "!printf 'Bearer %s' \"$GITHUB_TOKEN\""` → constrói o header a partir de um comando
...
Se você quer o comportamento menos surpreendente do OMP, prefira `.xcsh/mcp.json` e use valores explícitos de env/header.

## `disabledServers`

`disabledServers` é principalmente útil no arquivo de configuração do usuário (`~/.xcsh/mcp.json`) quando um servidor é descoberto de outra fonte e você quer que o OMP o ignore sem editar a configuração dessa outra ferramenta.
...
Use edição direta do JSON quando:

- você precisa de uma opção de transporte ou autenticação que o assistente ainda não solicita
- você quer colar uma definição de servidor de outro cliente MCP
- você quer validação baseada em schema no seu editor
...
- `/mcp test <name>` para testar um servidor individual
...
- Se você colar uma configuração de servidor remoto e esquecer `"type": "http"`, o OMP o tratará como `stdio` e reclamará que `command` está faltando
- `sse` permanece válido para compatibilidade, mas novos servidores hospedados geralmente devem ser configurados como `http`

## Descoberta e precedência

O OMP não mescla definições duplicadas de servidores entre arquivos. Os provedores de descoberta são priorizados, e a definição de maior prioridade prevalece.

Na prática:

- prefira `.xcsh/mcp.json` ou `~/.xcsh/mcp.json` quando quiser uma substituição específica do OMP
- mantenha nomes de servidores únicos entre ferramentas quando possível
...
O JSON é válido, mas o servidor pode ainda estar inacessível. Use `/mcp test <name>` e verifique se:
...
Execute `/mcp list`. O OMP descobre muitos arquivos MCP de terceiros, mas o carregamento a nível de projeto também pode ser desabilitado pela configuração `mcp.enableProjectConfig`.
