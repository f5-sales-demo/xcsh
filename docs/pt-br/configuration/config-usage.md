...
description: >-
  Como o xcsh descobre, resolve e empilha configurações a partir de raízes de
...
Este documento descreve como o coding-agent resolve configurações atualmente: quais raízes são escaneadas, como a precedência funciona e como a configuração resolvida é consumida por settings, skills, hooks, tools e extensões.
...
## 2) Helpers centrais de descoberta (`src/config.ts`)
...
Busca o primeiro arquivo existente nas bases ordenadas, retorna a primeira correspondência (apenas caminho ou caminho+metadados).

## `findAllNearestProjectConfigDirs(subpath, cwd)`

Percorre os diretórios pai para cima e retorna o **diretório existente mais próximo por base de fonte** (`.xcsh`, `.claude`, `.codex`, `.gemini`), depois ordena os resultados por prioridade de fonte.

Use isso quando a configuração do projeto deve ser herdada de diretórios ancestrais (comportamento de monorepo/workspace aninhado).
...
- Se o caminho alvo é `.yml`/`.yaml`, um `.json` adjacente é auto-migrado uma vez (`migrateJsonToYml`).

---

## 4) Modelo de resolução de settings (`src/config/settings.ts`)

O modelo de settings em tempo de execução é empilhado:
...
4. Valores padrão do schema: de `SETTINGS_SCHEMA`
...
Na inicialização, se `config.yml` estiver ausente:
...
- `theme: "..."` legado plano -> estrutura `theme.dark/theme.light`

---

## 5) Integração com capability/discovery

A maioria dos fluxos de carregamento de configuração não centrais passa pelo registro de capabilities (`src/capability/index.ts` + `src/discovery/index.ts`).

## Ordenação de providers

Providers são ordenados por prioridade numérica (maior primeiro). Exemplo de prioridades:
...
## Semânticas de deduplicação

Capabilities definem uma `key(item)`:

- mesma key => primeiro item vence (item de maior prioridade/carregado primeiro)
- sem key (`undefined`) => sem dedup, todos os itens são mantidos
...
- extensions: `name`
- settings: sem dedup (todos os itens são preservados)
...
Para `SYSTEM.md` e `AGENTS.md`, o provider nativo usa busca de diretório `.xcsh` de projeto no ancestral mais próximo (percorrendo para cima), mas ainda requer que o diretório `.xcsh` não esteja vazio.

---

## 7) Como os principais subsistemas consomem configuração

## Subsistema de settings

- `Settings.init()` carrega o `config.yml` global + itens de capability de `settings.json` de projeto descobertos.
...
- Aplica toggles de fonte e filtros (`ignoredSkills`, `includeSkills`, diretórios personalizados).
...
- A implementação atual mantém intencionalmente apenas itens de capability com `_source.provider === "native"` antes do carregamento.
...
1. A ordenação de diretórios de fonte de `config.ts` determina a ordem dos caminhos candidatos.
2. A prioridade do provider de capability determina a precedência entre providers.
3. A deduplicação por key de capability determina o comportamento de colisão (primeiro vence para capabilities com chave).
...
Itens de capability de settings não são deduplicados; `Settings.#loadProjectSettings()` faz deep-merge dos itens de projeto na ordem retornada. Como a mesclagem aplica valores de itens posteriores sobre valores anteriores, o comportamento efetivo de override depende da ordem de emissão do provider, não apenas da semântica de key de capability.

---

## 9) Comportamentos legados/de compatibilidade ainda presentes

- Migração de JSON -> YAML do `ConfigFile` para arquivos com alvo YAML.
...
- Compatibilidade de manifesto de extensão: o carregador aceita tanto as seções de manifesto `package.json.xcsh` quanto `package.json.pi`.
