...
A maioria das consultas de runtime usa `$env` de `@f5xc-salesdemos/pi-utils` (`packages/utils/src/env.ts`).
...
Estas são consumidas via `getEnvApiKey()` (`packages/ai/src/stream.ts`), salvo indicação em contrário.

### Credenciais de provedores principais

| Variável                        | Usada para | Obrigatória quando                                            | Notas / precedência                                                                                 |
|---------------------------------|---|---------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|
...
| `GEMINI_API_KEY`                | Autenticação Google Gemini | Usando modelos do provedor `google`                           | Chave primária para mapeamento do provedor Gemini                                                   |
...
| `HUGGINGFACE_HUB_TOKEN`         | Autenticação Hugging Face | Usando provedor `huggingface`                                 | Variável de ambiente primária do token Hugging Face                                                 |
| `HF_TOKEN`                      | Autenticação Hugging Face | Usando provedor `huggingface`                                 | Fallback quando `HUGGINGFACE_HUB_TOKEN` não está definida                                           |
...
| `LITELLM_API_KEY`               | Autenticação LiteLLM | Usando provedor `litellm`                                     | Chave de proxy LiteLLM compatível com OpenAI. Quando definida com `LITELLM_BASE_URL`, habilita autoconfiguração do `models.yml` |
...
| `OPENROUTER_API_KEY`            | Autenticação OpenRouter | Usando modelos OpenRouter                                     | Também usado pela ferramenta de imagem quando o provedor preferido/automático é OpenRouter           |
...
| `ZENMUX_API_KEY`                | Autenticação ZenMux | Usando provedor `zenmux`                                      | Usado para rotas compatíveis com ZenMux OpenAI e Anthropic                                          |
| `VLLM_API_KEY`                  | Autenticação/descoberta opt-in vLLM | Usando provedor `vllm` (servidores locais compatíveis com OpenAI) | Qualquer valor não vazio funciona para servidores locais sem autenticação                            |
...
- `ANTHROPIC_CUSTOM_HEADERS` é analisado como pares `chave: valor` separados por vírgula/nova linha e mesclados nos cabeçalhos da requisição.
...
| `AWS_CONTAINER_CREDENTIALS_RELATIVE_URI` / `AWS_CONTAINER_CREDENTIALS_FULL_URI` | Habilita caminho de credenciais de tarefa ECS |
| `AWS_WEB_IDENTITY_TOKEN_FILE` + `AWS_ROLE_ARN` | Habilita caminho de autenticação por identidade web |
...
Resolução da URL base: opção `azureBaseUrl` → env `AZURE_OPENAI_BASE_URL` → opção/env nome do recurso → `model.baseUrl`.
...
### OpenAI Codex responses (controles de recurso/depuração)

| Variável | Comportamento |
|---|---|
| `PI_CODEX_DEBUG` | `1`/`true` habilita logging de depuração do provedor Codex |
...
| `PI_CODEX_WEBSOCKET_RETRY_DELAY_MS` | Substituição de backoff base de inteiro positivo (padrão 500) |
...
### Credenciais de provedores de busca
...
`packages/coding-agent/src/web/search/auth.ts` resolve as credenciais de busca web Anthropic nesta ordem:
...
4. Fallback genérico de ambiente Anthropic: chave do provedor (`ANTHROPIC_FOUNDRY_API_KEY`/`ANTHROPIC_OAUTH_TOKEN`/`ANTHROPIC_API_KEY`) + opcional `ANTHROPIC_BASE_URL` (`FOUNDRY_BASE_URL` quando o modo Foundry está habilitado)
...
| `ANTHROPIC_BASE_URL` | URL base de fallback genérica para caminho de autenticação nível 4 |
...
| `VIRTUAL_ENV` | Caminho de venv de maior prioridade para resolução de runtime Python |

Comportamento condicional extra:

- Se `BUN_ENV=test` ou `NODE_ENV=test`, as verificações de disponibilidade do Python são tratadas como OK e o aquecimento é pulado.
- A filtragem de ambiente Python nega chaves de API comuns e permite variáveis base seguras + prefixos `LC_`, `XDG_`, `PI_`.
...
| Variável                   | Padrão / comportamento                                                                       |
|----------------------------|----------------------------------------------------------------------------------------------|
| `PI_SMOL_MODEL`            | Substituição efêmera de role de modelo para `smol` (CLI `--smol` tem precedência)            |
| `PI_SLOW_MODEL`            | Substituição efêmera de role de modelo para `slow` (CLI `--slow` tem precedência)            |
| `PI_PLAN_MODEL`            | Substituição efêmera de role de modelo para `plan` (CLI `--plan` tem precedência)            |
...
| `PI_SUBPROCESS_CMD`        | Substitui o comando de spawn de subagente (bypass da resolução `xcsh` / `xcsh.cmd`)          |
...
| `PI_TIMING`                | Se `1`, habilita logs de instrumentação de timing de inicialização/ferramentas               |
| `PI_DEBUG_STARTUP`         | Habilita impressões de depuração de estágio de inicialização no stderr em múltiplos caminhos de inicialização |
| `PI_PACKAGE_DIR`           | Substitui a resolução do diretório base de ativos do pacote (consulta de caminhos de docs/exemplos/changelog) |
| `PI_DISABLE_LSPMUX`        | Se `1`, desabilita a detecção/integração do lspmux e força o spawn direto do servidor LSP    |
| `LITELLM_BASE_URL`         | URL base do proxy LiteLLM. Quando definida com `LITELLM_API_KEY`, aciona a autogeração do `models.yml` na primeira execução e autocorreção em cada inicialização |
...
| `PI_CODING_AGENT_DIR` | Substituição completa do diretório do agente (padrão `~/<PI_CONFIG_DIR ou .xcsh>/agent`) |
...
| `PI_BASH_NO_LOGIN` | Destinado a desabilitar o modo de shell de login |
...
| `EDITOR` | Comando de fallback de editor externo |
...
## 8) Detecção de UI/tema/sessão (ambiente autodetectado)
...
| `COLORFGBG` | Autodetecção de fundo claro/escuro do terminal |
...
| `HOME` | Abreviação de caminho na UI de comando MCP |
...
| `PI_DEBUG_REDRAW` | Se `1`, habilita logging de depuração de redesenho |
| `PI_TUI_DEBUG` | Se `1`, habilita caminho de dump de depuração profunda da TUI |

---

## 11) Controles de geração de commits

| Variável | Comportamento |
|---|---|
| `PI_COMMIT_TEST_FALLBACK` | Se `true` (insensível a maiúsculas/minúsculas), força o caminho de geração de fallback de commit |
| `PI_COMMIT_NO_FALLBACK` | Se `true`, desabilita o fallback quando o agente não retorna nenhuma proposta |
| `PI_COMMIT_MAP_REDUCE` | Se `false`, desabilita o caminho de análise de commit map-reduce |
...
Trate estas como segredos; não as registre em logs nem faça commit delas:

- Chaves de API de provedores e credenciais OAuth/bearer (todas as `*_API_KEY`, `*_TOKEN`, tokens de acesso/atualização OAuth)
- Credenciais de nuvem (`AWS_*`, o caminho `GOOGLE_APPLICATION_CREDENTIALS` pode expor material de conta de serviço)
