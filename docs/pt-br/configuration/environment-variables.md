...
description: >-
  Referência de variáveis de ambiente em tempo de execução para configuração e
  controle de comportamento do xcsh.
sidebar:
  order: 2
...
- `packages/ai/src/**` (resolução de provedor/autenticação usada pelo coding-agent)
...
A maioria das buscas em tempo de execução utiliza `$env` de `@f5xc-salesdemos/pi-utils` (`packages/utils/src/env.ts`).
...
Regra adicional em arquivos `.env`: chaves `XCSH_*` são espelhadas para chaves `PI_*` durante a análise.

---

## 1) Autenticação de modelo/provedor

Estas são consumidas via `getEnvApiKey()` (`packages/ai/src/stream.ts`), exceto quando indicado de outra forma.

### Credenciais principais do provedor

| Variável                        | Uso | Obrigatória quando                                                 | Notas / precedência                                                                                  |
|---------------------------------|---|---------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|
| `ANTHROPIC_OAUTH_TOKEN`         | Autenticação API Anthropic | Usando Anthropic com autenticação por token OAuth                         | Tem precedência sobre `ANTHROPIC_API_KEY` na resolução de autenticação do provedor                              |
| `ANTHROPIC_API_KEY`             | Autenticação API Anthropic | Usando Anthropic sem token OAuth                           | Fallback após `ANTHROPIC_OAUTH_TOKEN`                                                              |
...
| `LITELLM_API_KEY`               | Autenticação LiteLLM | Usando provedor `litellm`                                      | Chave de proxy LiteLLM compatível com OpenAI. Quando definida com `LITELLM_BASE_URL`, habilita auto-configuração do `models.yml` |
| `LM_STUDIO_API_KEY`             | Autenticação LM Studio (opcional) | Usando provedor `lm-studio` com hosts autenticados           | LM Studio local geralmente executa sem autenticação; qualquer token não vazio funciona quando uma chave é necessária         |
| `OLLAMA_API_KEY`                | Autenticação Ollama (opcional) | Usando provedor `ollama` com hosts autenticados              | Ollama local geralmente executa sem autenticação; qualquer token não vazio funciona quando uma chave é necessária            |
| `LLAMA_CPP_API_KEY`             | Autenticação Ollama (opcional) | Usando `llama-server` com parâmetro `--api-key`              | llama.cpp local geralmente executa sem autenticação; qualquer token não vazio funciona quando uma chave está configurada       |
...
| `ZENMUX_API_KEY`                | Autenticação ZenMux | Usando provedor `zenmux`                                       | Usado para rotas ZenMux compatíveis com OpenAI e Anthropic                                              |
| `VLLM_API_KEY`                  | Autenticação/descoberta vLLM | Usando provedor `vllm` (servidores locais compatíveis com OpenAI)       | Qualquer valor não vazio funciona para servidores locais sem autenticação                                                 |
...
| `GH_TOKEN` | Fallback Copilot; autenticação API GitHub no web scraper | No web scraper: `GITHUB_TOKEN` → `GH_TOKEN` |
| `GITHUB_TOKEN` | Fallback Copilot; autenticação API GitHub no web scraper | No web scraper: verificado antes de `GH_TOKEN` |

---

## 2) Configuração de runtime específica do provedor
...
- A resolução de chave API para o provedor `anthropic` torna-se:
  `ANTHROPIC_FOUNDRY_API_KEY` → `ANTHROPIC_OAUTH_TOKEN` → `ANTHROPIC_API_KEY`.
- `ANTHROPIC_CUSTOM_HEADERS` é analisado como pares `chave: valor` separados por vírgula/nova linha e mesclado nos cabeçalhos da requisição.
- Material TLS cliente/servidor pode ser injetado a partir de valores de ambiente:
  `NODE_EXTRA_CA_CERTS`, `CLAUDE_CODE_CLIENT_CERT`, `CLAUDE_CODE_CLIENT_KEY`.
  Cada um aceita:
  - um caminho do sistema de arquivos para conteúdo PEM, ou
...
| `CLAUDE_CODE_CLIENT_KEY` | Caminho PEM ou PEM inline | Chave privada do cliente mTLS (deve ser emparelhada com o certificado) |

### Amazon Bedrock

| Variável | Padrão / comportamento |
|---|---|
| `AWS_REGION` | Fonte primária de região |
| `AWS_DEFAULT_REGION` | Fallback se `AWS_REGION` não definida |
| `AWS_PROFILE` | Habilita caminho de autenticação por perfil nomeado |
| `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY` | Habilita caminho de autenticação por chave IAM |
| `AWS_BEARER_TOKEN_BEDROCK` | Habilita caminho de autenticação por bearer token |
| `AWS_CONTAINER_CREDENTIALS_RELATIVE_URI` / `AWS_CONTAINER_CREDENTIALS_FULL_URI` | Habilita caminho de credencial de tarefa ECS |
| `AWS_WEB_IDENTITY_TOKEN_FILE` + `AWS_ROLE_ARN` | Habilita caminho de autenticação por web identity |
...
| `AZURE_OPENAI_BASE_URL` | Substituição direta da URL base |
...
Resolução de URL base: opção `azureBaseUrl` → env `AZURE_OPENAI_BASE_URL` → opção/env resource name → `model.baseUrl`.
...
| `GCLOUD_PROJECT` | Fallback | Usado como fonte alternativa de ID do projeto |
...
| Variável | Padrão / comportamento |
|---|---|
| `KIMI_CODE_OAUTH_HOST` | Substituição primária do host OAuth |
| `KIMI_OAUTH_HOST` | Substituição de fallback do host OAuth |
...
### OpenAI Codex responses (controles de funcionalidade/depuração)

| Variável | Comportamento |
|---|---|
| `PI_CODEX_DEBUG` | `1`/`true` habilita log de depuração do provedor Codex |
...
| `PI_CODEX_WEBSOCKET_IDLE_TIMEOUT_MS` | Substituição de inteiro positivo (padrão 300000) |
| `PI_CODEX_WEBSOCKET_RETRY_BUDGET` | Substituição de inteiro não negativo (padrão 5) |
| `PI_CODEX_WEBSOCKET_RETRY_DELAY_MS` | Substituição de backoff base em inteiro positivo (padrão 500) |

### Depuração do provedor Cursor

| Variável | Comportamento |
|---|---|
| `DEBUG_CURSOR` | Habilita logs de depuração do provedor; `2`/`verbose` para trechos detalhados de payload |
| `DEBUG_CURSOR_LOG` | Caminho de arquivo opcional para saída de log de depuração JSONL |

### Opção de compatibilidade de cache de prompt
...
| `PERPLEXITY_API_KEY` | Provedor de busca Perplexity no modo chave API |
...
2. Entrada do provedor em `models.json` com `api: "anthropic-messages"`
3. Credenciais OAuth Anthropic de `agent.db` (não devem expirar dentro de um buffer de 5 minutos)
...
| `ANTHROPIC_BASE_URL` | URL base de fallback genérico para caminho de autenticação nível 4 |
...
| Variável | Padrão / comportamento |
|---|---|
| `PI_PY` | Substituição do modo de ferramenta Python: `0`/`bash`=`bash-only`, `1`/`py`=`ipy-only`, `mix`/`both`=`both`; valores inválidos são ignorados |
...
Comportamento condicional adicional:

- Se `BUN_ENV=test` ou `NODE_ENV=test`, verificações de disponibilidade do Python são tratadas como OK e o aquecimento é pulado.
- O filtro de env Python nega chaves de API comuns e permite variáveis base seguras + prefixos `LC_`, `XDG_`, `PI_`.

---

## 5) Alternâncias de comportamento do agente/runtime

| Variável                   | Padrão / comportamento                                                                           |
|----------------------------|----------------------------------------------------------------------------------------------|
| `PI_SMOL_MODEL`            | Substituição efêmera de model-role para `smol` (CLI `--smol` tem precedência)                     |
| `PI_SLOW_MODEL`            | Substituição efêmera de model-role para `slow` (CLI `--slow` tem precedência)                     |
| `PI_PLAN_MODEL`            | Substituição efêmera de model-role para `plan` (CLI `--plan` tem precedência)                     |
| `PI_NO_TITLE`              | Se definida (qualquer valor não vazio), desabilita geração automática de título de sessão na primeira mensagem do usuário   |
| `NULL_PROMPT`              | Se `true`, o construtor de system prompt retorna string vazia                                        |
| `PI_BLOCKED_AGENT`         | Bloqueia um tipo específico de subagente na ferramenta de tarefa                                                 |
| `PI_SUBPROCESS_CMD`        | Substitui o comando de spawn do subagente (bypass de resolução `xcsh` / `xcsh.cmd`)                       |
...
| `PI_TIMING`                | Se `1`, habilita logs de instrumentação de tempo de startup/ferramenta                                     |
| `PI_DEBUG_STARTUP`         | Habilita impressões de depuração de estágio de startup para stderr em múltiplos caminhos de inicialização                       |
| `PI_PACKAGE_DIR`           | Substitui resolução do diretório base de assets do pacote (busca de caminhos de docs/exemplos/changelog)            |
| `PI_DISABLE_LSPMUX`        | Se `1`, desabilita detecção/integração lspmux e força spawn direto do servidor LSP          |
| `LITELLM_BASE_URL`         | URL base do proxy LiteLLM. Quando definida com `LITELLM_API_KEY`, dispara auto-geração de `models.yml` na primeira execução e auto-correção em cada inicialização |
| `LM_STUDIO_BASE_URL`       | Substituição da URL base de descoberta implícita padrão do LM Studio (`http://127.0.0.1:1234/v1` se não definida) |
| `OLLAMA_BASE_URL`          | Substituição da URL base de descoberta implícita padrão do Ollama (`http://127.0.0.1:11434` se não definida)      |
| `LLAMA_CPP_BASE_URL`       | Substituição da URL base de descoberta implícita padrão do Llama.cpp (`http://127.0.0.1:8080` se não definida)    |
| `PI_EDIT_VARIANT`          | Se `hashline`, força modo de exibição hashline read/grep quando a ferramenta de edição está disponível               |
| `PI_NO_PTY`                | Se `1`, desabilita caminho PTY interativo para ferramenta bash                                          |

`PI_NO_PTY` também é definida internamente quando CLI `--no-pty` é usado.

---

## 6) Caminhos raiz de armazenamento e configuração

Estas são consumidas via `@f5xc-salesdemos/pi-utils/dirs` e afetam onde o coding-agent armazena dados.

| Variável | Padrão / comportamento |
|---|---|
| `PI_CONFIG_DIR` | Nome do diretório raiz de configuração sob o home (padrão `.xcsh`) |
| `PI_CODING_AGENT_DIR` | Substituição completa para o diretório do agente (padrão `~/<PI_CONFIG_DIR ou .xcsh>/agent`) |
| `PWD` | Usado ao corresponder o diretório de trabalho atual canônico em helpers de caminho |

---

## 7) Ambiente de execução de shell/ferramenta

(De `packages/utils/src/procmgr.ts` e integração da ferramenta bash do coding-agent.)

| Variável | Comportamento |
|---|---|
| `PI_BASH_NO_CI` | Suprime injeção automática de `CI=true` no env do shell gerado |
| `CLAUDE_BASH_NO_CI` | Alias legado de fallback para `PI_BASH_NO_CI` |
| `PI_BASH_NO_LOGIN` | Destinado a desabilitar modo de shell de login |
...
Nota de implementação atual: `PI_BASH_NO_LOGIN`/`CLAUDE_BASH_NO_LOGIN` são lidos, mas o `getShellArgs()` atual retorna `['-l','-c']` em ambas as ramificações (efetivamente sem efeito hoje).
...
| `TERM_PROGRAM`, `TERM_PROGRAM_VERSION`, `TERMINAL_EMULATOR` | Identidade do terminal no system prompt/contexto |
| `KDE_FULL_SESSION`, `XDG_CURRENT_DESKTOP`, `DESKTOP_SESSION`, `XDG_SESSION_DESKTOP`, `GDMSESSION`, `WINDOWMANAGER` | Detecção de desktop/gerenciador de janelas no system prompt/contexto |
| `KITTY_WINDOW_ID`, `TMUX_PANE`, `TERM_SESSION_ID`, `WT_SESSION` | IDs de breadcrumb estáveis por sessão de terminal |
| `SHELL`, `ComSpec`, `TERM_PROGRAM`, `TERM` | Diagnósticos de informações do sistema |
| `APPDATA`, `XDG_CONFIG_HOME` | Resolução de caminho de configuração lspmux |
| `HOME` | Encurtamento de caminho na UI de comando MCP |

---

## 9) Flags de loader nativo/depuração

| Variável | Comportamento |
|---|---|
| `PI_DEV` | Habilita diagnósticos detalhados de carregamento de addon nativo em `packages/natives` |

## 10) Flags de runtime TUI (pacote compartilhado, afeta UX do coding-agent)

| Variável | Comportamento |
|---|---|
| `PI_NOTIFICATIONS` | `off` / `0` / `false` suprime notificações de desktop |
| `PI_TUI_WRITE_LOG` | Se definida, registra escritas da TUI em arquivo |
| `PI_HARDWARE_CURSOR` | Se `1`, habilita modo de cursor por hardware |
| `PI_CLEAR_ON_SHRINK` | Se `1`, limpa linhas vazias quando o conteúdo encolhe |
| `PI_DEBUG_REDRAW` | Se `1`, habilita log de depuração de redesenho |
| `PI_TUI_DEBUG` | Se `1`, habilita caminho de dump de depuração profunda da TUI |
...
| Variável | Comportamento |
|---|---|
| `PI_COMMIT_TEST_FALLBACK` | Se `true` (insensível a maiúsculas), força caminho de geração de fallback de commit |
| `PI_COMMIT_NO_FALLBACK` | Se `true`, desabilita fallback quando o agente não retorna proposta |
| `PI_COMMIT_MAP_REDUCE` | Se `false`, desabilita caminho de análise de commit map-reduce |
| `DEBUG` | Se definida, stack traces de erros do agente de commit são impressos |

---

## Variáveis sensíveis à segurança

Trate estas como segredos; não as registre em log nem faça commit delas:

- Chaves de API/provedor e credenciais OAuth/bearer (todos os `*_API_KEY`, `*_TOKEN`, tokens de acesso/atualização OAuth)
