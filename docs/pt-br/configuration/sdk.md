...
Use-o quando quiser acesso direto ao estado do agente, streaming de eventos, conexão de ferramentas e controle de sessão a partir do seu próprio processo Bun/Node.

Se você precisa de isolamento entre linguagens/processos, use o modo RPC.
...
Normalmente você só precisa fornecer o que deseja controlar:
...
- **Geralmente fornecido explicitamente** em integradores:
    ...
    - `model` ou `modelPattern` (se a seleção determinística de modelo importa)
...
`AgentSession` sempre usa um `SessionManager`; o comportamento depende de qual factory você utiliza.
...
- Os métodos de sessão ainda funcionam, mas comportamentos específicos de persistência (caminhos de retomada/fork de arquivo) são naturalmente limitados.
...
1. override em tempo de execução (`setRuntimeApiKey`)
2. credenciais armazenadas em `agent.db`
3. variáveis de ambiente do provedor
4. fallback do resolver de provedor personalizado (se configurado)

## Modelo de assinatura de eventos

Assine com `session.subscribe(listener)`; retorna uma função para cancelar a assinatura.
...
- `toolNames` atua como uma lista de permissão para as integradas.
...
Estas são opcionais para incorporação normal de agente único.
...
Use `setToolUIContext(...)` somente se seu integrador fornecer capacidades de UI que ferramentas/extensões devem invocar.

## Exemplo mínimo de incorporação controlada
