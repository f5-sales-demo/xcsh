...
description: Guia prático para migrar código do monorepo pi-mono para o codebase xcsh.
sidebar:
  order: 9
...
Este guia é uma checklist repetível para portar mudanças do pi-mono para este repositório.
Use-o para qualquer merge: arquivo único, branch de feature ou sincronização completa de release.
...
- Liste os pacotes ou pastas que você planeja modificar.
- Decida quais funcionalidades estão no escopo e quais são intencionalmente ignoradas.

## 1) Traga o código de forma segura

- Prefira um diff limpo e focado em vez de uma cópia por atacado.
...
## 2) Siga as convenções de extensão de import

A maioria dos fontes TypeScript de runtime omite `.js` nos imports internos, mas alguns entrypoints de teste/benchmark mantêm `.js` para compatibilidade de runtime ESM. Siga o estilo existente do pacote local; não remova extensões indiscriminadamente.

- Em fontes de runtime de `packages/coding-agent`, mantenha imports internos sem extensão, a menos que esteja importando assets não-TS.
- Em `packages/tui/test` e `packages/natives/bench`, mantenha `.js` onde os arquivos circundantes já o utilizam.
- Mantenha extensões reais de arquivo quando exigido por ferramentas (ex.: `.json`, `.css`, embeds de texto `.md`).
- Exemplo: `import { x } from "./foo.js";` → `import { x } from "./foo";` (somente quando a convenção do pacote é sem extensão).

## 3) Substitua os escopos de import

O upstream usa escopos de pacote diferentes. Substitua-os consistentemente.

- Substitua os escopos antigos pelo escopo local usado aqui.
...
## 4) Use APIs do Bun quando melhorarem em relação ao Node

Rodamos no Bun. Substitua APIs do Node apenas quando o Bun fornecer uma alternativa melhor.

**SUBSTITUA:**

- Criação de processos: `child_process.spawn` → Bun Shell `$` para comandos simples, `Bun.spawn`/`Bun.spawnSync` para trabalho com streaming ou de longa duração
...
**NÃO substitua (funcionam bem no Bun):**
...
- `path.join()`, `path.resolve()`, etc. — estão ok

**Estilo de import:** Use o prefixo `node:` com imports de namespace apenas (sem imports nomeados de `node:fs` ou `node:path`).

**Convenções adicionais do Bun:**

- Prefira Bun Shell `$` para comandos curtos sem streaming; use `Bun.spawn` apenas quando precisar de I/O com streaming ou controle de processo.
...
```typescript
// QUEBRADO: variáveis de env podem ser undefined, "~" não é expandido
...
- Se o upstream copia assets para uma pasta dist, substitua por embeds compatíveis com o Bun.
- Prompts são arquivos `.md` estáticos; use imports de texto do Bun (`with { type: "text" }`) e Handlebars em vez de strings de prompt inline.
...
- Se o upstream lê um arquivo de fallback empacotado em runtime, substitua leituras do filesystem por um import de embed de texto do Bun.
  - Exemplo (fallback de instruções do Codex):
...
- Não introduza `any` a menos que necessário.
- Evite imports dinâmicos e imports de tipo inline; use apenas imports de nível superior.
...
- **Sem palavras-chave `private`/`protected`/`public` em campos ou métodos de classe.** Use campos privados ES `#` para encapsulamento; deixe membros acessíveis sem modificador (sem palavra-chave). A única exceção são propriedades de parâmetro do construtor (`constructor(private readonly x: T)`), onde a palavra-chave é exigida pelo TypeScript. Ao portar código upstream que usa `private foo` ou `protected bar`, converta para `#foo` (privado) ou bare `bar` (acessível).
- Prefira helpers e utilitários existentes em vez de código ad-hoc novo.
- Preserve mudanças de infraestrutura Bun-first já feitas neste repositório:
...
- Não mantenha `*_v2` ou versões paralelas.

## 9) Atualize documentação e referências

- Substitua links do repo pi-mono onde apropriado.
- Atualize exemplos para usar Bun e escopos de pacote corretos.
- Garanta que as instruções do README ainda correspondam ao comportamento atual do repo.
...
Se o repositório já tem verificações falhando não relacionadas às suas mudanças, sinalize isso.
...
Se você já melhorou o comportamento localmente, trate-os como **não negociáveis**. Antes de portar, documente
as melhorias e adicione verificações explícitas para que não se percam no merge.

- **Congele o comportamento esperado**: adicione uma nota curta de "antes/depois" para cada melhoria (entradas, saídas,
  defaults, casos extremos). Isso previne reversão silenciosa.
- **Mapeie APIs antigas → novas**: se o upstream renomeou conceitos (hooks → extensions, custom tools → tools, etc.),
  garanta que cada ponto de entrada antigo ainda funcione. Um flag ou export esquecido equivale a funcionalidade perdida.
- **Verifique exports**: confira `package.json` `exports`, tipos públicos e arquivos barrel. Portagens do upstream frequentemente
  esquecem de re-exportar adições locais.
- **Cubra caminhos não-felizes**: se você corrigiu tratamento de erros, timeouts ou lógica de fallback, adicione um teste ou pelo
  menos uma checklist manual que exercite esses caminhos.
- **Verifique defaults e ordem de merge de configuração**: melhorias frequentemente residem em defaults. Confirme que novos defaults
  não foram revertidos (ex.: nova precedência de config, features desabilitadas, listas de ferramentas).
- **Audite comportamento de env/shell**: se você corrigiu execução ou sandboxing, verifique se o novo caminho ainda usa seu
  env sanitizado e não reintroduz overrides de alias/function.
- **Re-execute amostras direcionadas**: mantenha um conjunto mínimo de exemplos "sabidamente bons" e execute-os após a portagem
  (flags de CLI, registro de extensões, execução de ferramentas).
...
```bash
# Compare o arquivo que você está prestes a portar com o que você tem localmente
git diff HEAD upstream/main -- path/to/file.ts
```

Se o diff mostra que o arquivo foi **retrabalhado** (não apenas corrigido pontualmente):
...
Nota: o modo interativo foi recentemente dividido em controllers/utils/types. Ao fazer backport de mudanças relacionadas, porte atualizações nos arquivos individuais que criamos e garanta que a ligação de `interactive-mode.ts` permaneça em sincronia.

1. **Defaults mudam silenciosamente** - Uma nova variável `defaultFoo = [a, b]` pode substituir uma antiga `getAllFoo()` que retornava `[a, b, c, d, e]`.
...
3. **Caminhos de código ficam obsoletos** - Um conceito renomeado (ex.: `hookMessage` → `custom`) precisa de atualizações em cada instrução switch, type guard e handler — não apenas na definição.

4. **Contexto/capacidades diminuem** - APIs antigas podem ter exposto `{ logger, typebox, pi }` que novas APIs esqueceram de incluir.

### Processo de portagem semântica

Quando o upstream retrabalhau um módulo:
...
3. **Verifique paridade de funcionalidades** - Para cada capacidade no código antigo, confirme que o novo código a preserva ou a remove explicitamente.

4. **Busque por remanescentes** - Procure por nomes/conceitos antigos que possam ter sido esquecidos em instruções switch, handlers, componentes de UI.

5. **Teste as fronteiras** - Flags de CLI, opções de SDK, handlers de eventos, valores padrão — é onde as regressões se escondem.
...
## 13) Checklist de auditoria rápida

Use isso como uma passada final antes de concluir:

- [ ] Extensões de import seguem a convenção local do pacote (sem remoção indiscriminada de `.js`)
- [ ] Sem APIs exclusivas do Node em código novo/portado
- [ ] Todos os escopos de pacote atualizados
- [ ] Scripts do `package.json` usam Bun
- [ ] Prompts são imports de texto `.md` (sem strings de prompt inline)
- [ ] Sem `console.*` no coding-agent (use `logger`)
- [ ] Assets carregam via padrões de embed do Bun (sem scripts de cópia)
- [ ] Testes ou verificações executam (ou explicitamente marcados como bloqueados)
- [ ] Sem regressões de funcionalidade (veja seções 11-12)

## 14) Formato de mensagem de commit

Ao fazer commit de um backport, siga o formato do repositório `<tipo>(escopo): <descrição no passado>` e mantenha o
...
| `ctx.ui.setHeader()` / `ctx.ui.setFooter()` | Stub em modos não-TUI                                     | Implementado no TUI, no-op em outros modos                            |
| `ctx.ui.setEditorComponent()`               | Stub em modos não-TUI                                     | Implementado no TUI, no-op em outros modos                            |
| Objeto de opções `InteractiveModeOptions`   | Args posicionais no construtor (tipo options ainda exportado) | Manter assinatura do construtor; atualizar o tipo quando upstream adicionar campos |
...
| `sessionManager.getSessionName()`        | `sessionManager.getSessionName()`        | Mesmo (unificamos para corresponder ao RPC do upstream) |
| `agent.sessionName` / `setSessionName()` | `agent.sessionName` / `setSessionName()` | Mesmo                                     |

### Consolidação de Arquivos

| Upstream                                           | Nosso Fork                              | Razão                                   |
| -------------------------------------------------- | --------------------------------------- | --------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (arquivos de ferramenta) | Módulo clipboard de `@f5xc-salesdemos/pi-natives` | Mesclado na implementação nativa N-API  |
...
| Interfaces `*Operations` por ferramenta | Interfaces por ferramenta permanecem (`FindOperations`, `GrepOperations`) | Usadas para overrides SSH/remote                          |
| `fs/promises` do Node.js em todo lugar | `Bun.file()`/`Bun.write()` para arquivos; `node:fs/promises` para dirs | Preferir APIs do Bun quando simplificam                   |
...
### Ignore Estas Funcionalidades do Upstream

Ao portar, **ignore** estes arquivos/funcionalidades completamente:
