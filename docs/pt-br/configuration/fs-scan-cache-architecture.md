...
Este documento define o contrato atual para o cache compartilhado de varredura do sistema de arquivos implementado em Rust (`crates/pi-natives/src/fs_cache.rs`) e consumido pelas APIs nativas de descoberta/busca expostas ao `packages/coding-agent`.

## O que é este cache

O cache armazena listas completas de entradas de varredura de diretórios (`GlobMatch[]`) indexadas por escopo de varredura e política de travessia, permitindo então que operações de nível superior (filtragem de glob, pontuação fuzzy, seleção de arquivos do grep) sejam executadas contra essas entradas em cache.

Objetivos principais:

- evitar travessias repetidas do sistema de arquivos para chamadas repetidas de descoberta/busca
- manter consistência entre `glob`, `fuzzyFind` e `grep` quando compartilham a mesma política de varredura
- permitir recuperação explícita de dados obsoletos para resultados vazios e invalidação explícita após mutações de arquivos
...
- caminho canonicalizado do diretório `root`
...
## Comportamento da coleta de varredura
...
- `node_modules` é sempre coletado no momento da varredura do cache (e opcionalmente filtrado posteriormente)
...
Política global (substituível por variáveis de ambiente):
...
- `get_or_scan(...)`
  - se o TTL é `0`: ignora o cache completamente, sempre faz varredura fresca (`cache_age_ms = 0`)
  - em cache hit dentro do TTL: retorna entradas em cache + `cache_age_ms` diferente de zero
  - em hit expirado: remove a chave, re-varre, armazena entrada fresca
- a aplicação do limite máximo de entradas é feita por evição das mais antigas por `created_at`

## Reverificação rápida de resultado vazio (separada dos hits normais)
...
- A descoberta de candidatos de menção de alto volume habilita o cache:
  - `packages/coding-agent/src/utils/file-mentions.ts`
  - perfil: `hidden=true`, `gitignore=true`, `includeNodeModules=true`, `cache=true`
- A integração do `grep` no nível de ferramenta atualmente desabilita o cache de varredura (`cache: false`):
...
- `invalidateFsScanCache(path?: string)`
  - com `path`: remove entradas de cache cuja raiz é prefixo do caminho alvo
...
- se o alvo não existe (ex.: exclusão), o fallback canonicaliza o pai e reanexa o nome do arquivo quando possível
...
Regra: se um fluxo altera o conteúdo ou localização no sistema de arquivos e ignora esses helpers, bugs de cache obsoleto são esperados.
...
1. **Use entradas de política de varredura estáveis**
   - decida primeiro a semântica de hidden/gitignore
...
3. **Implemente a reverificação rápida de resultado vazio apenas para risco de falso-negativo obsoleto**
   - use `scan.cache_age_ms >= empty_recheck_ms()`
   - tente novamente uma vez com `force_rescan(..., store=true, ...)`
   - mantenha este caminho separado da lógica normal de cache-hit

4. **Respeite o modo sem cache explicitamente**
   - quando o chamador desabilita o cache, chame `force_rescan(..., store=false, ...)`
...
5. **Conecte a invalidação por mutação para qualquer novo caminho de escrita**
   - após write/edit/delete/rename bem-sucedido, chame o helper de invalidação do coding-agent
   - para rename/move, invalide tanto os caminhos antigos quanto os novos

6. **Não adicione controles de TTL por chamada**
   - o contrato atual é apenas política global (configurada por variáveis de ambiente), sem override de TTL por requisição

## Limites conhecidos

- O escopo do cache é local ao processo em memória (`DashMap`), não é persistido entre reinicializações do processo.
...
- `.git` é sempre excluído no momento da coleta de varredura, independentemente das opções do chamador.
