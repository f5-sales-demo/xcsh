...
การค้นหาค่ารันไทม์ส่วนใหญ่ใช้ `$env` จาก `@f5xc-salesdemos/pi-utils` (`packages/utils/src/env.ts`)
...
ตัวแปรเหล่านี้ถูกใช้งานผ่าน `@f5xc-salesdemos/pi-utils/dirs` และส่งผลต่อที่ที่ coding-agent จัดเก็บข้อมูล
