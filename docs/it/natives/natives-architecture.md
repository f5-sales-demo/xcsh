...
description: >-
  Architettura degli addon nativi Rust N-API che collegano TypeScript e
  operazioni specifiche della piattaforma.
sidebar:
  order: 1
  label: Architettura
i18n:
...
2. **Livello di caricamento/validazione degli addon** risolve e valida il binario `.node` per il runtime corrente.
...
- `NativeBindings` inizia con membri condivisi (`cancelWork(id: number)`)
- i binding specifici dei moduli vengono aggiunti tramite declaration merging dal `types.ts` di ciascun modulo
...
## Livello 2: Caricamento e validazione degli addon

`packages/natives/src/native.ts` gestisce la selezione degli addon a runtime, l'estrazione opzionale e la validazione delle esportazioni.

### Modello di risoluzione dei candidati

- Il tag piattaforma è `"${process.platform}-${process.arch}"`.
- I tag supportati attualmente sono:
  - `linux-x64`
  - `linux-arm64`
  - `darwin-x64`
  - `darwin-arm64`
...
- `PI_DEV` abilita la diagnostica del loader ma non modifica i nomi file degli addon
...
`packages/natives/package.json` include sia `src` che `native` nei file pubblicati. La directory `native/` contiene gli artefatti precompilati per piattaforma.

Per i binari compilati (marcatori di runtime `PI_COMPILED` o Bun embedded), il comportamento del loader è:

1. Controlla il percorso cache utente con versione: `<getNativesDir()>/<packageVersion>/...`
2. Controlla la posizione legacy dei binari compilati:
   - Windows: `%LOCALAPPDATA%/xcsh` (fallback `%USERPROFILE%/AppData/Local/xcsh`)
   - non-Windows: `~/.local/bin`
3. Ricade su `native/` del pacchetto e candidati nella directory dell'eseguibile

Se è presente un manifest di addon embedded (`embedded-addon.ts` generato da `scripts/embed-native.ts`), `native.ts` può materializzare il binario embedded corrispondente nella directory cache con versione prima del caricamento.
...
- **Tag piattaforma non supportato**: genera un'eccezione con l'elenco delle piattaforme supportate
- **Nessun candidato caricabile**: genera un'eccezione con tutti i percorsi tentati e suggerimenti per la risoluzione
- **Esportazioni mancanti**: genera un'eccezione con i nomi esatti mancanti e il comando di rebuild
...
**Contratto garantito (lato API):** il caricamento dell'addon ha successo con un set di binding validato oppure fallisce immediatamente con un messaggio di errore actionable.

**Dettaglio implementativo (soggetto a modifiche):** ordine esatto di ricerca dei candidati e ordinamento dei percorsi di fallback per binari compilati.

## Livello 3: Livello modulo Rust N-API

`crates/pi-natives/src/lib.rs` è il modulo entry Rust che dichiara la proprietà dei moduli esportati:
...
**Contratto garantito (lato API):** le esportazioni dei moduli Rust devono corrispondere ai nomi di binding attesi da `validateNative` e dai moduli wrapper.
...
A livello architetturale, la proprietà è suddivisa come segue:

- **Proprietà del wrapper/API TS (`packages/natives/src`)**
...
- **Proprietà del loader (`packages/natives/src/native.ts`)**
  ...
  - validazione rigida delle esportazioni native richieste
- **Proprietà Rust (`crates/pi-natives/src`)**
  - implementazione algoritmica e a livello di sistema
  - comportamento nativo della piattaforma e logica critica per le prestazioni
...
- **Modalità binario compilato**: Modalità runtime in cui la CLI è impacchettata e gli addon nativi vengono risolti da percorsi estratti/cache anziché solo da percorsi locali al pacchetto.
...
- **Gate di validazione**: Controllo `validateNative(...)` che rifiuta binari obsoleti/non corrispondenti a cui mancano esportazioni richieste.
