---
title: Contratto di binding nativi (lato TypeScript)
description: >-
...
# Contratto di binding nativi (lato TypeScript)
...
- `src/native.ts` importa tutti i file `./<module>/types` per i side effect, in modo che il contratto unificato sia nello scope dove `NativeBindings` viene utilizzato.
...
Transizione di stato: **Oggetto addon non attendibile** → **Oggetto binding nativo validato** (o fallimento critico).
...
- **Normalizzazione/predefinizione degli argomenti**
...
- **Denominazione pubblica e cura delle ri-esportazioni**
...
Regola per i manutentori: se un wrapper non è ri-esportato da `src/index.ts`, non fa parte della superficie pubblica prevista del pacchetto.

## Mappatura API JS ↔ esportazione nativa (rappresentativa)

Il lato Rust utilizza nomi di esportazione N-API (tipicamente dalla conversione `#[napi]` snake_case -> camelCase, con alias espliciti occasionali) che devono corrispondere a queste chiavi di binding.

| Categoria | API JS pubblica (wrapper) | Chiave binding nativa | Tipo di ritorno | Asincrono? |
|---|---|---|---|---|
...
## Differenze di contratto tra sincrono e asincrono
...
- **Esportazioni asincrone basate su Promise** per I/O o operazioni di lunga durata (`grep`, `glob`, `htmlToMarkdown`, `executeShell`, clipboard, operazioni sulle immagini).
- **Esportazioni sincrone** per trasformazioni/parser deterministici in memoria (`search`, `hasMatch`, evidenziazione, larghezza/slicing del testo, parsing dei tasti, query sui processi).
- **Esportazioni di costruttori** per oggetti runtime con stato (`Shell`, `PtySession`, `PhotonImage`).

Implicazione per i manutentori: cambiare sincrono ↔ asincrono per un'esportazione esistente è una modifica breaking dell'API e del contratto attraverso wrapper e chiamanti.
...
Questi sono contratti strutturali a compile-time; la correttezza della forma a runtime è di responsabilità dell'implementazione nativa.
...
I chiamanti vedono i membri enum nominati; il confine di binding passa numeri.
...
1. **Controlli del contratto TypeScript a compile-time**
   - I wrapper chiamano `native.<name>` contro `NativeBindings` unificato.
...
2. **Validazione a runtime in `validateNative`**
   - Dopo il caricamento, `native.ts` verifica le esportazioni richieste e lancia un errore se ne manca qualcuna.
   - Il messaggio di errore include le chiavi mancanti e le istruzioni per la ricompilazione.

Questo intercetta il comune disallineamento da binario obsoleto: il wrapper/tipo esiste ma il `.node` caricato manca dell'esportazione.

## Comportamento in caso di errore e avvertenze

### Errori di caricamento/validazione (errori critici)
...
- Le esportazioni richieste mancanti lanciano un errore prima che i wrapper siano utilizzabili.

Effetto: il pacchetto fallisce immediatamente anziché differire l'errore alla prima chiamata.

### Differenze di comportamento a livello di wrapper

- Alcuni wrapper ammorbidiscono intenzionalmente gli errori (`copyToClipboard` è best effort e assorbe gli errori nativi).
- Le callback di streaming ignorano i payload di errore delle callback e inoltrano solo gli eventi con valori di successo.

### Avvertenze a livello di tipi (il runtime è più rigoroso di TS)

- I campi opzionali di TS non garantiscono la validità semantica; il livello nativo può comunque rifiutare valori malformati.
- La tipizzazione `const enum` non impedisce valori numerici fuori range da parte di chiamanti non tipizzati a runtime.
- `validateNative` verifica solo la presenza e la natura di funzione delle esportazioni richieste, non la compatibilità profonda della forma di argomenti/valori di ritorno.
...
## Checklist per i manutentori per le modifiche ai binding
...
3. Import di `src/native.ts` per i tipi del modulo (se è un nuovo modulo)
4. Controlli delle esportazioni richieste in `validateNative`
5. Ri-esportazioni pubbliche in `src/index.ts`

Saltare qualsiasi passaggio crea disallineamento a compile-time o errore a runtime in fase di caricamento.
