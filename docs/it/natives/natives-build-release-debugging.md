...
| --- | --- | --- | --- |
| `Native addon missing exports ... Missing: <name>` | Binario `.node` obsoleto, export Rust errato o binario sbagliato | Usare `PI_DEV=1` per vedere il percorso; controllare gli export | Ricompilare; allineare export o alias `#[napi]` alla chiave JS; eliminare i file obsoleti |
...
| Il binario compilato fallisce dopo un aggiornamento | Cache obsoleta (`~/.xcsh/natives/<versione-vecchia-o-non-corrispondente>`) o manifesto incompatibile | Controllare la directory versionata e gli errori del loader | Eliminare la cache della versione e rieseguire; rigenerare il manifesto incorporato |
