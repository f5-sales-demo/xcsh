...
- Protocollo del kernel + integrazione gateway: `src/ipy/kernel.ts`
...
Lo strumento `python` esegue una o più celle Python attraverso un kernel supportato da un Jupyter Kernel Gateway (non generando direttamente `python -c` per ogni cella).
...
Esistono due percorsi per il gateway:
...
`releaseSharedGateway()` attualmente è un no-op (lo spegnimento del kernel non arresta il gateway condiviso).
...
- Chiude il websocket
- Invoca l'hook di rilascio del gateway condiviso (no-op attualmente)
...
- `session` (predefinito)
  - Riutilizza le sessioni kernel identificate per identità di sessione + cwd.
  ...
  - I controlli heartbeat rilevano i kernel non funzionanti.
  - Il riavvio automatico è consentito una volta; crash ripetuti => errore irreversibile.

- `per-call`
  - Crea un kernel nuovo per ogni richiesta di esecuzione.
  - Spegne il kernel dopo la richiesta.
  - Nessuna persistenza di stato tra chiamate.

### Comportamento multi-cella in una singola chiamata dello strumento

Le celle vengono eseguite sequenzialmente nella stessa istanza del kernel per quella chiamata dello strumento.

Se una cella intermedia fallisce:

- Lo stato delle celle precedenti rimane in memoria.
- Lo strumento restituisce un errore mirato indicando quale cella è fallita.
...
- La lista consentita include variabili fondamentali come `PATH`, `HOME`, variabili di locale, `VIRTUAL_ENV`, `PYTHONPATH`, ecc.
- Prefissi consentiti: `LC_`, `XDG_`, `PI_`
- La lista di esclusione rimuove le chiavi API comuni (OpenAI/Anthropic/Gemini/ecc.)
...
L'inizializzazione dell'ambiente del kernel all'interno di Python inoltre:
...
`python.toolMode` (predefinito `both`) + override opzionale `PI_PY` controllano l'esposizione:
...
## Flusso di esecuzione e annullamento/timeout
...
### Annullamento dell'esecuzione del kernel
...
- `image/png` -> payload immagine
- `application/x-xcsh-status` -> eventi di stato

### Archiviazione e troncamento

L'output viene trasmesso in streaming attraverso `OutputSink` e può essere persistito nell'archiviazione degli artefatti.
...
- Renderer dello strumento (`python.ts`):
  ...
  - l'anteprima compressa mostra per default 10 righe
...
- Renderer interattivo (`python-execution.ts`):
  - utilizzato per l'esecuzione Python avviata dall'utente nella TUI
  - l'anteprima compressa mostra per default 20 righe
  - limita le singole righe molto lunghe a 4000 caratteri per sicurezza di visualizzazione
  - mostra notifiche di annullamento/errore/troncamento
...
- I controlli di salute e le operazioni CRUD del kernel vengono eseguiti verso l'endpoint esterno
- I fallimenti di autenticazione vengono evidenziati con indicazioni esplicite sul token
...
- **Strumento Python non disponibile**
  ...
  - Se il preflight fallisce, il runtime ripassa a bash-only.
...
- **L'esecuzione si blocca e poi va in timeout**
  ...
  - Per codice bloccato, l'annullamento attiva l'interrupt del kernel ma il codice utente potrebbe comunque necessitare di refactoring.
...
- **Errori nella directory di lavoro**
  - Lo strumento valida che `cwd` esista e sia una directory prima dell'esecuzione.

## Variabili d'ambiente rilevanti

- `PI_PY` — override dell'esposizione dello strumento (mappatura `bash-only`/`ipy-only`/`both` sopra descritta)
