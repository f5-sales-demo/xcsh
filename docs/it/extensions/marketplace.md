...
Un **marketplace** è un repository Git (o una directory locale) contenente un file catalogo in `.xcsh-plugin/marketplace.json`. Il catalogo elenca i plugin disponibili con le rispettive fonti, descrizioni e metadati.

Un **plugin** è una directory contenente skill, comandi, hook, server MCP o server LSP. I plugin sono identificati da `name@marketplace` (ad es. `code-review@f5xc-salesdemos-marketplace`).
...
Le installazioni con ambito progetto prevalgono sulle installazioni con ambito utente dello stesso plugin.
...
| Comando | Effetto |
|---|---|
| `/marketplace add <source>` | Aggiunge una fonte marketplace |
| `/marketplace remove <name>` | Rimuove un marketplace |
| `/marketplace update [name]` | Ricarica il/i catalogo/i; omettere il nome per aggiornare tutti |
...
## Fonti del marketplace

Quando si esegue `/marketplace add <source>`, il sistema classifica la fonte:

| Formato della fonte | Tipo | Esempio |
|---|---|---|
...
Gli ID plugin (`name@marketplace`) devono avere al massimo 128 caratteri in totale.
