...
Utilizzatelo quando desiderate accesso diretto allo stato dell'agente, allo streaming degli eventi, al collegamento degli strumenti e al controllo delle sessioni dal vostro processo Bun/Node.

Se necessitate di isolamento tra linguaggi/processi differenti, utilizzate invece la modalità RPC.
...
Esportazioni principali per gli integratori:
...
- Superficie di factory degli strumenti (`createTools`, `BUILTIN_TOOLS`, classi degli strumenti)

## Avvio rapido (valori predefiniti con auto-discovery)
...
- skills/file di contesto/template dei prompt/comandi slash/estensioni/comandi TS personalizzati
...
Tipicamente dovete fornire solo ciò che volete controllare:

- **Obbligatorio fornire**: nulla per una sessione minimale
- **Solitamente fornito esplicitamente** negli integratori:
    - `sessionManager` (se necessitate di archiviazione in memoria o in una posizione personalizzata)
    ...
    - `settings` (se necessitate di una configurazione isolata/di test)

## Comportamento del session manager (persistente vs in memoria)
...
- Persiste conversazioni/messaggi/delta di stato su file di sessione.
...
- Nessuna persistenza su filesystem.
- Utile per test, worker effimeri, agenti con ambito di richiesta.
- I metodi della sessione continuano a funzionare, ma i comportamenti specifici della persistenza (percorsi di ripresa/fork dei file) sono naturalmente limitati.

### Helper per ripresa/apertura/elenco
...
2. modello predefinito dalle impostazioni per ruolo (`default`)
...
4. fallback del resolver per provider personalizzati (se configurato)

## Modello di sottoscrizione agli eventi

Sottoscrivetevi con `session.subscribe(listener)`; restituisce una funzione di annullamento della sottoscrizione.
...
`AgentSessionEvent` include gli `AgentEvent` principali più eventi a livello di sessione:
...
1. espansione opzionale di comandi/template (comandi `/`, comandi personalizzati, comandi slash su file, template dei prompt)
2. se attualmente in streaming:
    - richiede `streamingBehavior: "steer" | "followUp"`
    - accoda invece di scartare il lavoro
3. se inattivo:
...
### Strumenti integrati e filtro

- Gli strumenti integrati provengono da `createTools(...)` e `BUILTIN_TOOLS`.
- `toolNames` funge da lista di autorizzazione per gli strumenti integrati.
...
Utilizzateli quando desiderate un controllo parziale senza ricreare la logica di discovery interna:
...
Per i consumatori dell'SDK che costruiscono orchestratori (simili al flusso di esecuzione dei task):
...
Queste sono opzionali per l'integrazione normale di un singolo agente.
...
Utilizzate `setToolUIContext(...)` solo se il vostro integratore fornisce capacità UI che gli strumenti/estensioni dovrebbero invocare.

## Esempio minimale di integrazione controllata
