...
Impedisce che valori sensibili (chiavi API, token, password) vengano inviati ai provider LLM. Quando abilitato, i segreti vengono sostituiti con placeholder deterministici prima di uscire dal processo, e ripristinati negli argomenti delle chiamate a tool restituiti dal modello.

## Abilitazione

Abilitato per impostazione predefinita. Attivabile/disattivabile tramite l'interfaccia `/settings` o direttamente in `config.yml`:
...
2. I messaggi in uscita verso l'LLM hanno tutti i valori segreti sostituiti con placeholder come `<<$env:S0>>`, `<<$env:S1>>`, ecc.

3. Gli argomenti delle chiamate a tool restituiti dal modello vengono attraversati in profondità e i placeholder vengono ripristinati ai valori originali prima dell'esecuzione.

Due modalità controllano cosa succede a ciascun segreto:

| Modalità | Comportamento | Reversibile |
|---|---|---|
| `obfuscate` (predefinita) | Sostituito con placeholder indicizzato `<<$env:SN>>` | Sì (deoffuscato negli argomenti tool) |
...
- `src/secrets/obfuscator.ts` -- classe `SecretObfuscator`, generazione placeholder, offuscamento messaggi
- `src/secrets/regex.ts` -- parsing e compilazione di letterali regex
