...
description: >-
  Pipeline di offuscamento dei segreti che redige i valori sensibili dai log di
...
Abilitato per impostazione predefinita. Attiva o disattiva tramite l'interfaccia `/settings` o direttamente in `config.yml`:
...
1. All'avvio della sessione, i segreti vengono raccolti da due fonti:
   - **Variabili d'ambiente** corrispondenti a pattern comuni di segreti (`*_KEY`, `*_SECRET`, `*_TOKEN`, `*_PASSWORD`, ecc.) con valori >= 8 caratteri
   - **File `secrets.yml`** (vedere di seguito)
...
Definisce voci di segreti personalizzate in YAML. Vengono verificate due posizioni:
...
| `content` | stringa | Sì | Il valore del segreto (plain) o il pattern regex (regex) |
...
# Sintassi letterale regex (pattern e flag in una sola stringa)
- type: regex
  content: "/bearer\\s+[a-zA-Z0-9._~+\\/=-]+/i"
```

Le voci regex effettuano sempre una scansione globale (il flag `g` viene applicato automaticamente). La sintassi letterale regex `/pattern/flags` è supportata come alternativa ai campi separati `content` + `flags`. Gli slash con escape all'interno del pattern (`\\/`) vengono gestiti correttamente.
...
Le variabili d'ambiente vengono sempre raccolte per prime. Le voci definite nei file vengono aggiunte successivamente, in modo che le voci dei file possano coprire segreti che non risiedono nelle variabili d'ambiente (file di configurazione, valori hardcoded, ecc.). Se lo stesso valore compare in entrambe, la modalità della voce del file ha la precedenza.

## File principali

- `src/secrets/index.ts` -- caricamento, fusione, raccolta delle variabili d'ambiente
