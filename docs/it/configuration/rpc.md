...
La modalità RPC esegue l'agente di codifica come un protocollo JSON delimitato da newline su stdio.
...
- La modalità RPC disabilita la generazione automatica del titolo di sessione per impostazione predefinita, per evitare una chiamata aggiuntiva al modello.
- La modalità RPC reimposta le impostazioni `todo.*`, `task.*` e `async.*` che alterano il flusso di lavoro ai valori predefiniti integrati, anziché ereditare le personalizzazioni dell'utente.
...
Non esiste un involucro oltre alla forma dell'oggetto stesso.
...
- `prompt` e `abort_and_prompt` restituiscono un successo immediato, quindi possono emettere una risposta di errore successiva con lo **stesso** id se la pianificazione asincrona del prompt fallisce.
...
Sostituisce lo stato dei todo in memoria per la sessione corrente e restituisce la lista di fasi normalizzata:
...
Questo è utile per gli host che desiderano pre-impostare un piano prima del primo prompt.
...
Questi strumenti vengono aggiunti al registro degli strumenti della sessione attiva prima della prossima chiamata al modello. Inviare nuovamente `set_host_tools` sostituisce l'insieme precedente di proprietà dell'host.
...
`message_update` include i delta dello streaming in `assistantMessageEvent` (delta di testo/thinking/toolcall).
...
- `set_interrupt_mode`
  ...
  - `"wait"`: rimanda lo steering al completamento del turno

## Sotto-Protocollo UI delle Estensioni

Le estensioni in modalità RPC utilizzano frame di richiesta/risposta UI.
...
Se un dialogo ha un timeout, la modalità RPC si risolve su un valore predefinito quando il timeout/abort si attiva.

## Sotto-Protocollo degli Strumenti Host

Gli host RPC possono esporre strumenti personalizzati all'agente inviando `set_host_tools`, quindi servendo le richieste di esecuzione sullo stesso trasporto.
...
## Modello di Errore e Recuperabilità
...
- Le condizioni di terminazione del processo sono la chiusura di stdin o uno shutdown esplicitamente attivato da un'estensione.
...
### 4) Ciclo completo UI dell'estensione
...
- Inoltra ai listener solo i tipi `AgentEvent` riconosciuti
...
- **Non** espone metodi helper per ogni comando del protocollo (ad esempio, `set_interrupt_mode` e `set_session_name` sono nei tipi del protocollo ma non hanno metodi dedicati nel wrapper)
