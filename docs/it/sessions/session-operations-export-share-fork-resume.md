...
Questo documento descrive il comportamento visibile all'operatore per le operazioni di esportazione/condivisione/fork/ripresa delle sessioni come attualmente implementate.
...
| Operazione | Percorso di accesso | Mutazione della sessione | Creazione/cambio file sessione | Artefatto in uscita |
|---|---|---|---|---|
| `/dump` | Comando slash interattivo | No | No | Testo negli appunti |
| `/export [path]` | Comando slash interattivo | No | No | File HTML |
| `--export <session.jsonl> [outputPath]` | Percorso rapido di avvio CLI | Nessuna mutazione della sessione a runtime | Nessuna sessione attiva; legge il file di destinazione | File HTML |
| `/share` | Comando slash interattivo | No | No | HTML temporaneo + URL di condivisione/gist |
| `/fork` | Comando slash interattivo | Sì (l'identità della sessione attiva cambia) | Crea un nuovo file sessione e passa la sessione corrente ad esso (solo modalità persistente) | Copia la directory degli artefatti nel nuovo namespace della sessione quando presente |
| `/resume` | Comando slash interattivo | Sì (lo stato attivo in memoria viene sostituito) | Passa a un file sessione esistente selezionato | Nessuno |
| `--resume` | Avvio CLI (selettore) | Sì dopo la creazione della sessione | Apre il file sessione esistente selezionato | Nessuno |
| `--resume <id\|path>` | Avvio CLI | Sì dopo la creazione della sessione | Apre la sessione esistente; il caso cross-progetto può eseguire il fork nel progetto corrente | Nessuno |
...
- Il parsing degli argomenti è basato sugli spazi bianchi (`text.split(/\s+/)`), quindi i percorsi tra virgolette con spazi non vengono preservati come un singolo percorso da questo percorso di comando.
...
3. `SessionManager.open(inputPath)` carica le voci, poi l'HTML viene generato e scritto.
...
### Fase 2: handler di condivisione personalizzato (se presente)
...
- Il modulo deve esportare di default una funzione `(htmlPath) => Promise<CustomShareResult | string | undefined>`.

Se presente e valido:

- L'interfaccia entra nello stato di caricamento `Sharing...`.
- Interpretazione del risultato dell'handler:
...
- Se l'handler personalizzato esiste ma il caricamento fallisce, il comando va in errore e ritorna.
- Se l'handler personalizzato viene eseguito e lancia un'eccezione, il comando va in errore e ritorna.
- In entrambi i casi di fallimento, **non** esegue il fallback al gist di GitHub.
...
Solo quando non viene trovato alcun handler di condivisione personalizzato:
...
4. Copia la directory degli artefatti dal vecchio namespace della sessione al nuovo namespace (best-effort; i fallimenti di copia diversi da ENOENT vengono registrati, non sono fatali).
5. Aggiorna `agent.sessionId`.
6. Emette `session_switch` con `reason: "fork"`.

Comportamento di `SessionManager.fork()`:

- Richiede la modalità persistente e un file sessione esistente.
- Crea un nuovo id sessione e un nuovo percorso file JSONL.
...
3. L'interfaccia cancella/ricostruisce chat e todo, poi riporta `Resumed session`.
...
Comportamento della corrispondenza id cross-progetto:

- Se il cwd della sessione trovata differisce dal cwd corrente, il CLI chiede:
...
3. Esegue il fallback al file sessione modificato più di recente.
...
Nessun nuovo file sessione viene creato da `switchSession()` stesso.
...
- `/share` ha un percorso di interruzione dell'interfaccia (`Share cancelled`) per il flusso gist; non implementa la semantica di terminazione del processo per `gh gist create` in questo percorso di codice.
...
- Il percorso del file sessione è assente.
...
- `/dump` funziona comunque perché serializza lo stato dell'agente in memoria.
- Le semantiche di resume/continue del CLI vengono bypassate se `--no-session` è impostato, perché la creazione del manager restituisce immediatamente in-memory.

## Avvertenze note dell'implementazione (al momento del codice corrente)

- `SelectorController.handleResumeSession()` non controlla il risultato booleano da `session.switchSession(...)`; uno switch cancellato da un hook può comunque procedere attraverso il percorso di ridisegno/stato dell'interfaccia "Resumed session".
