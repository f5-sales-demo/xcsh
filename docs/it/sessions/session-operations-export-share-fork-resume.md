---
title: 'Operazioni sulle sessioni: esportazione, dump, condivisione, fork, ripresa'
description: >-
  Operazioni sulle sessioni per esportazione, condivisione, fork e ripresa delle
  conversazioni.
sidebar:
  order: 3
  label: Operazioni
...
| `/fork` | Comando slash interattivo | Sì (l'identità della sessione attiva cambia) | Crea un nuovo file di sessione e passa la sessione corrente ad esso (solo in modalità persistente) | Copia la directory degli artefatti nel nuovo namespace della sessione quando presente |
| `/resume` | Comando slash interattivo | Sì (lo stato attivo in memoria viene sostituito) | Passa al file di sessione esistente selezionato | Nessuno |
| `--resume` | Avvio CLI (selettore) | Sì dopo la creazione della sessione | Apre il file di sessione esistente selezionato | Nessuno |
| `--resume <id\|path>` | Avvio CLI | Sì dopo la creazione della sessione | Apre la sessione esistente; il caso cross-project può eseguire il fork nel progetto corrente | Nessuno |
...
2. Il comando divide sugli spazi bianchi e utilizza solo il primo argomento dopo `/export` come `outputPath`.
3. `AgentSession.exportToHtml()` chiama `exportSessionToHtml(sessionManager, state, { outputPath, themeName })`.
4. In caso di successo, l'UI mostra il percorso e apre il file nel browser.

Dettagli del comportamento:

- Gli argomenti `--copy`, `clipboard` e `copy` vengono esplicitamente rifiutati con un avviso di utilizzare `/dump`.
- L'esportazione incorpora intestazione/voci/foglia della sessione più il `systemPrompt` corrente e le descrizioni degli strumenti dallo stato dell'agente.
- Nessuna voce di sessione viene aggiunta durante l'esportazione.

Avvertenza:

- L'analisi degli argomenti è basata sugli spazi bianchi (`text.split(/\s+/)`), quindi i percorsi tra virgolette con spazi non vengono preservati come un singolo percorso da questo flusso di comando.

### `--export <inputSessionFile> [outputPath]` (CLI)

Flusso in `main.ts`:

1. Gestito in anticipo (prima dell'avvio interattivo/sessione).
...
- Risultati degli strumenti e blocchi di esecuzione (eccetto le voci bash/python con `excludeFromContext`)
- Voci personalizzate/hook/menzione file/riepilogo branch/riepilogo compattazione

Il dump non produce alcuna modifica alla persistenza della sessione.
...
- Se l'esportazione fallisce (in particolare per le sessioni in memoria), la condivisione termina con un errore.
...
- L'UI entra nello stato di caricamento `Sharing...`.
...
- Se il gestore personalizzato esiste ma il caricamento fallisce, il comando segnala un errore e ritorna.
- Se il gestore personalizzato viene eseguito e genera un'eccezione, il comando segnala un errore e ritorna.
...
- Il caricamento ha un hook `onAbort` che ripristina l'UI dell'editor e riporta `Share cancelled`.
- Il comando sottostante `gh gist create` non riceve un segnale di interruzione in questo percorso di codice; la cancellazione avviene a livello UI e viene verificata dopo il ritorno del comando.
...
- Gli indicatori di stato/caricamento dell'UI vengono cancellati prima dell'operazione.
...
4. Copia la directory degli artefatti dal vecchio namespace della sessione al nuovo (best-effort; i fallimenti di copia diversi da ENOENT vengono registrati nei log, non sono fatali).
...
- Mantiene tutte le voci non-intestazione invariate nel nuovo file.
...
- L'UI riporta `Fork failed (session not persisted or cancelled)`.
...
3. L'UI cancella/ricostruisce la chat e i todo, quindi riporta `Resumed session`.
...
- `main.ts` elenca le sessioni per la cwd/sessionDir corrente e apre il selettore.
...
2. Altrimenti tratta come prefisso di id:
   - cerca nell'ambito corrente (`SessionManager.list(cwd, sessionDir)`)
   - se non trovato e nessuna `sessionDir` esplicita, cerca globalmente (`SessionManager.listAll()`)

Comportamento di corrispondenza id cross-project:

- Se la cwd della sessione trovata differisce dalla cwd corrente, il CLI chiede:
  - `Session found in different project ... Fork into current directory? [y/N]`
- In caso affermativo: `SessionManager.forkFrom(match.path, cwd, sessionDir)` crea un nuovo file locale con fork.
- In caso negativo/default non-TTY: il comando termina con errore.

## CLI `--continue`

`SessionManager.continueRecent(cwd, sessionDir)`:

1. Risolve la directory di sessione per la cwd corrente.
...
3. Cancella i messaggi di steering/follow-up/turno-successivo in coda.
...
`switchSession()` di per sé non crea alcun nuovo file di sessione.
...
Il bridge SDK collega gli eventi di sessione delle estensioni ai callback `onSession` degli strumenti personalizzati:
...
Questi callback sono osservazionali; non cancellano lo switch/fork.
...
- `--resume <id>` cross-project può essere cancellato rifiutando il prompt di fork.
- `/share` ha un percorso di interruzione UI (`Share cancelled`) per il flusso gist; non implementa semantiche di process-kill per `gh gist create` in questo percorso di codice.

## Comportamento delle sessioni non persistenti (in memoria)

Quando il session manager è creato con `SessionManager.inMemory()` (`--no-session`):

- Il percorso del file di sessione è assente.
- `/export` e `/share` falliscono con `Cannot export in-memory session to HTML` (propagato all'UI di errore del comando).
...
- Le semantiche CLI di resume/continue vengono bypassate se `--no-session` è impostato, perché la creazione del manager restituisce immediatamente la versione in memoria.

## Avvertenze di implementazione note (dal codice corrente)

- `SelectorController.handleResumeSession()` non verifica il risultato booleano di `session.switchSession(...)`; uno switch cancellato da un hook può comunque procedere attraverso il percorso di repaint/stato dell'UI "Resumed session".
- I fallimenti della condivisione personalizzata di `/share` non degradano al fallback gist predefinito; terminano il comando con un errore.
