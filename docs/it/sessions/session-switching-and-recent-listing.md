---
title: Cambio sessione e elenco sessioni recenti
description: >-
  Meccaniche di cambio sessione ed elenco delle sessioni recenti con ricerca e
  filtraggio.
sidebar:
  order: 4
...
# Cambio sessione ed elenco sessioni recenti

Questo documento descrive come coding-agent scopre le sessioni recenti, risolve i target `--resume`, presenta i selettori di sessione e cambia la sessione runtime attiva.
...
`SessionManager` archivia le sessioni in una directory con ambito cwd per impostazione predefinita:

- `~/.xcsh/agent/sessions/--<cwd-encoded>--/*.jsonl`

`SessionManager.list(cwd, sessionDir?)` legge solo quella directory a meno che non venga fornita esplicitamente una `sessionDir`.

### Due percorsi di elenco con payload differenti

Esistono due pipeline di elenco differenti:

1. `getRecentSessions(sessionDir, limit)` (vista benvenuto/riepilogo)
   - Legge solo un prefisso di 4KB (`readTextPrefix(..., 4096)`) da ogni file.
   - Analizza l'header + l'anteprima del primo testo utente.
   - Restituisce oggetti `RecentSessionInfo` leggeri con getter lazy per `name` e `timeAgo`.
...
### Comportamento di fallback per i metadati
...
- i caratteri di controllo/a capo vengono rimossi/sanitizzati dai nomi derivati dal titolo

Per le voci dell'elenco `SessionInfo`:

- `title` è `header.title` o l'ultimo `shortSummary` di compattazione
- `firstMessage` è il testo del primo messaggio utente oppure `"(no messages)"`
...
3. Se il breadcrumb è invalido/mancante, ricade sul file più recente per mtime nella directory di sessione (`findMostRecentSession`)
4. Se non ne viene trovato nessuno, crea una nuova sessione

La derivazione dell'ID del terminale preferisce il percorso TTY e ricade su identificatori basati sull'ambiente (`KITTY_WINDOW_ID`, `TMUX_PANE`, `TERM_SESSION_ID`, `WT_SESSION`).
...
- se il cwd della sessione trovata differisce dal cwd corrente, la CLI chiede se effettuare un fork nel progetto corrente
- sì -> `SessionManager.forkFrom(...)`
- no -> genera errore (`Session "..." is in another project (...)`)

Nessuna corrispondenza -> genera errore (`Session "..." not found.`).
...
2. se vuoto: stampa `No sessions found` e termina anticipatamente
3. apre il selettore TUI (`selectSession`)
4. se annullato: stampa `No session selected` e termina anticipatamente
...
2. monta `SessionSelectorComponent` nell'area dell'editor usando `showSelector(...)`
3. callback:
   - selezione -> chiude il selettore e chiama `handleResumeSession(sessionPath)`
   - annullamento -> ripristina l'editor e rirenderizza
   - uscita -> `ctx.shutdown()`
...
- Enter su lista vuota non fa nulla (nessun callback)
- Esc/Ctrl+C funzionano comunque

Avvertenza: il testo dell'UI dice `Press Tab to view all`, ma questo componente attualmente non ha un handler per Tab e il cablaggio corrente elenca solo le sessioni dell'ambito corrente.
...
3. se annullato -> ritorna `false` senza cambio
4. disconnette dallo stream di eventi dell'agente corrente
5. interrompe la generazione/flusso di tool attivo
6. svuota i buffer di messaggi accodati di steering/follow-up/next-turn
7. scarica il writer della sessione (`sessionManager.flush()`) per persistere le scritture pendenti
8. `sessionManager.setSessionFile(sessionPath)`
   - aggiorna il puntatore al file di sessione
   - scrive il breadcrumb del terminale
   - carica le voci / migra / risolve i blob / reindicizza
...
14. ripristina il livello di thinking:
    ...
    - altrimenti deriva il livello di thinking predefinito dalle impostazioni, lo limita alle capacità del modello, lo imposta e aggiunge una nuova voce `thinking_level_change`
15. riconnette i listener dell'agente e ritorna `true`
...
- svuota il contenitore di stato
- svuota l'UI dei messaggi pendenti e la mappa dei tool pendenti
- resetta il componente di streaming/riferimenti ai messaggi
- chiama `session.switchSession(...)`
- svuota il contenitore della chat e rirenderizza dal contesto della sessione (`renderInitialMessages`)
...
- La modalità interattiva quindi esegue `#restoreModeFromSession()` per rientrare nello stato di modalità persistito (attualmente plan/plan_paused).
...
- La chat/todo dell'UI vengono aggiornati.
- Non viene effettuata alcuna chiamata dedicata di ripristino modalità post-cambio nel flusso del selettore; il comportamento di rientro nella modalità non è simmetrico rispetto a `#restoreModeFromSession()` dell'avvio.

## Comportamento in caso di errore e casi limite

### Percorsi di annullamento

- Annullamento picker CLI -> ritorna `null`, il chiamante stampa `No session selected`, il processo termina anticipatamente.
- Annullamento picker interattivo -> editor ripristinato, nessun cambio di sessione.
- Annullamento hook (`session_before_switch`) -> `switchSession()` ritorna `false`.

### Percorsi con lista vuota

- CLI `--resume` (senza valore): lista vuota stampa `No sessions found` e termina.
...
Il cambio/apertura può comunque generare eccezioni per veri errori I/O (errori di permessi, errori di riscrittura, ecc.), che si propagano ai chiamanti.
...
- `SessionManager.list(...)` esclude le sessioni con zero messaggi, quindi quelle sessioni non sono ripristinabili tramite corrispondenza ID/picker dell'elenco.
