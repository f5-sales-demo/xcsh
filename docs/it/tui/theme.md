...
description: >-
  Riferimento per la tematizzazione della TUI con token di colore, impostazioni
  dei font e personalizzazione dei temi.
sidebar:
  order: 3
  label: Theming
i18n:
...
## Cosa controlla il sistema dei temi

Il sistema dei temi gestisce:
...
- colori dell'evidenziazione della sintassi utilizzati dall'highlighter nativo (`@f5xc-salesdemos/pi-natives`)
...
I file dei temi sono oggetti JSON validati rispetto allo schema a runtime in `theme.ts` (`ThemeJsonSchema`) e replicati da `src/modes/theme/theme-schema.json`.
...
### Diff degli strumenti + evidenziazione sintassi (12)
...
1. override `symbolPreset` dalle impostazioni (se definito)
2. `symbols.preset` dal JSON del tema
3. fallback `"unicode"`

Le chiavi di override non valide vengono ignorate e registrate nel log (`logger.debug`).

## Origini dei temi: integrati vs personalizzati
...
`getAvailableThemes()` restituisce i nomi integrati + personalizzati uniti, ordinati, con i temi integrati che hanno precedenza in caso di collisione di nomi.
...
4. risoluzione ricorsiva dei riferimenti a `vars`
...
- genera un errore in caso di riferimento a variabile mancante
- genera un errore in caso di riferimenti circolari
...
- parsing dell'indice dello sfondo da `COLORFGBG`
- `< 8` => slot scuro (`theme.dark`)
- `>= 8` => slot chiaro (`theme.light`)
- fallimento del parsing => slot scuro
...
- ricade sul tema integrato `dark`
- restituisce `{ success: false, error }`

### Cambio in anteprima (`previewTheme`)

- applica un tema di anteprima temporaneo al `theme` globale
- **non** modifica di per sé le impostazioni persistite
- restituisce successo/errore senza sostituzione di fallback

L'interfaccia delle Impostazioni utilizza questo meccanismo per l'anteprima dal vivo e ripristina il tema precedente in caso di annullamento.

## Watcher e ricaricamento automatico
...
- i temi integrati non sono effettivamente monitorati
- file `change`: tenta il ricaricamento (con debounce)
- file `rename`/eliminazione: ricade su `dark`, chiude il watcher
...
Gli altri token non vengono modificati.
...
## Creazione di un tema personalizzato (guida pratica)

1. Creare un file nella directory dei temi personalizzati, ad esempio `~/.xcsh/agent/themes/my-theme.json`.
2. Includere `name`, `vars` opzionali e **tutti i** token `colors` obbligatori.
...
La barra di stato ha due sistemi di colori paralleli documentati nella issue #242:
...
- Gli indici della palette a 256 colori (`statusLine<Segment>Bg` / `statusLine<Segment>Fg`)
  gestiscono il riempimento dei segmenti powerline. Sono indipendenti dalle chiavi
  esadecimali sopra indicate — entrambi devono essere impostati.
...
2. Aprire le impostazioni e visualizzare l'anteprima dei valori del tema (`previewTheme` dal vivo).
3. Per i file tema personalizzati, modificare il JSON durante l'esecuzione e verificare il ricaricamento automatico al salvataggio.
4. Verificare le superfici critiche:
   - rendering del markdown
   - blocchi degli strumenti (in attesa/successo/errore)
   - rendering dei diff (aggiunte/rimozioni/contesto)
   - leggibilità della barra di stato
   - cambiamenti dei bordi per livello di pensiero
...
- Gli errori di ricaricamento del watcher dei file mantengono il tema attualmente caricato fino a un ricaricamento riuscito o all'attivazione del percorso di fallback.
