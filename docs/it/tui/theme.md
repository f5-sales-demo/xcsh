---
title: Riferimento per la Tematizzazione
description: >-
  Riferimento per la tematizzazione TUI con token di colore, impostazioni dei
  font e personalizzazione dei temi.
sidebar:
  order: 3
...
# Riferimento per la Tematizzazione

Questo documento descrive come funziona la tematizzazione nel coding-agent attualmente: schema, caricamento, comportamento a runtime e modalità di errore.
...
- adattatori per selettore/editor/lista impostazioni (`getSelectListTheme()`, `getEditorTheme()`, `getSettingsListTheme()`)
- preset dei simboli + override dei simboli (`unicode`, `nerd`, `ascii`)
- colori per l'evidenziazione della sintassi utilizzati dall'evidenziatore nativo (`@f5xc-salesdemos/pi-natives`)
...
I file tema sono oggetti JSON validati rispetto allo schema runtime in `theme.ts` (`ThemeJsonSchema`) e rispecchiati da `src/modes/theme/theme-schema.json`.

Campi di primo livello:

- `name` (obbligatorio)
- `colors` (obbligatorio; tutti i token di colore obbligatori)
...
- indice colore 256 (`0..255`)
- stringa di riferimento a variabile (risolta tramite `vars`)
- stringa vuota (`""`) che indica il default del terminale (`\x1b[39m` fg, `\x1b[49m` bg)
...
## Sorgenti dei temi integrati vs personalizzati

Ordine di ricerca dei temi (`loadThemeJson`):

1. temi integrati incorporati (`defaults/xcsh-dark.json` e `defaults/xcsh-light.json` compilati in `defaultThemes`)
...
`getAvailableThemes()` restituisce i nomi uniti dei temi integrati + personalizzati, ordinati, con i temi integrati che hanno la precedenza in caso di collisione di nomi.

## Caricamento, validazione e risoluzione

Per i file tema personalizzati:
...
- genera un errore per riferimenti a variabili mancanti
- genera un errore per riferimenti circolari
...
- fallback al tema integrato `dark`
...
L'interfaccia delle impostazioni utilizza questa funzione per l'anteprima dal vivo e ripristina il tema precedente in caso di annullamento.
...
- i temi integrati effettivamente non vengono monitorati
- `change` del file: tenta il ricaricamento (con debounce)
- `rename`/eliminazione del file: fallback a `dark`, chiusura del watcher
...
`colorBlindMode` modifica un solo token a runtime:
...
Le impostazioni relative al tema vengono persistite da `Settings` nel file di configurazione YAML globale:
...
1. Creare un file nella directory dei temi personalizzati, ad es. `~/.xcsh/agent/themes/my-theme.json`.
...
Struttura minima. Ogni chiave in `colors` è obbligatoria — il validatore runtime
(`additionalProperties: false`) rifiuta sia le chiavi mancanti che quelle sconosciute.
Per le implementazioni di riferimento distribuite consultare
...
La barra di stato ha due sistemi di colore paralleli documentati nella issue #242:

- I colori del testo esadecimali (`statusLinePath`, `statusLineGitClean`, `statusLineGitDirty`,
  `statusLineStaged`, `statusLineDirty`, `statusLineUntracked`) guidano il
  rendering non-powerline.
- Gli indici della palette a 256 colori (`statusLine<Segment>Bg` / `statusLine<Segment>Fg`)
  guidano il riempimento dei segmenti powerline. Sono indipendenti dalle chiavi esadecimali sopra —
...
2. Aprire le impostazioni e visualizzare in anteprima i valori del tema (`previewTheme` dal vivo).
3. Per i file tema personalizzati, modificare il JSON durante l'esecuzione e confermare il ricaricamento automatico al salvataggio.
4. Verificare le superfici critiche:
   - rendering del markdown
   - blocchi strumenti (in attesa/successo/errore)
   - rendering delle diff (aggiunte/rimosse/contesto)
   - leggibilità della barra di stato
   - cambiamenti dei bordi per livello di ragionamento
   - colori dei bordi per modalità bash/python
5. Validare entrambi i preset dei simboli se il tema dipende dalla larghezza/aspetto dei glifi.
...
- L'errore di `setTheme` esegue il fallback a `dark`; l'errore di `previewTheme` non sostituisce il tema corrente.
- Gli errori di ricaricamento del watcher di file mantengono il tema attualmente caricato fino a un ricaricamento riuscito o all'attivazione del percorso di fallback.
