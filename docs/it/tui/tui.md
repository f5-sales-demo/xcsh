...
sidebar:
  order: 1
  label: Integrazione delle estensioni
i18n:
...
Il runtime è composto da due livelli:

- **Motore di rendering (`packages/tui`)**: renderer differenziale per terminale, dispatch dell'input, focus, overlay, posizionamento del cursore.
...
| Background/headless | Non interattiva | Il contesto UI è un no-op (`hasUI === false`). |
| Modalità RPC | Non supportata | `custom()` restituisce `Promise<never>` e non monta componenti TUI. |

Se la vostra estensione/strumento può funzionare in modalità non interattiva, proteggete con `ctx.hasUI` / `pi.hasUI`.

## Contratto del componente base (`@f5xc-salesdemos/pi-tui`)
...
## Vincoli di rendering (sicurezza del terminale)

L'output di `render(width)` deve essere sicuro per il terminale:

1. **Non superare mai `width` su nessuna riga**. Il renderer genera un errore se una riga non immagine eccede il limite.
2. **Misurare la larghezza visuale**, non la lunghezza della stringa: usare `visibleWidth()`.
3. **Troncare/mandare a capo il testo con consapevolezza ANSI** con `truncateToWidth()` / `wrapTextWithAnsi()`.
4. **Sanificare tabulazioni/contenuti** da fonti esterne usando `replaceTabs()` (e sanificatori di livello superiore nei percorsi di rendering di coding-agent).
...
### Rispettare i keybinding dell'applicazione configurati dall'utente

Le factory dell'interfaccia utente delle estensioni ricevono un `KeybindingsManager` (modalità interattiva) così potete rispettare le azioni mappate invece di codificare i tasti in modo fisso:
...
`HookUIContext.custom` è tipizzato come `(tui, theme, done)` nei tipi hook/strumenti personalizzati.
...
Gli strumenti personalizzati tipicamente utilizzano lo stesso punto di accesso UI tramite l'oggetto `pi.ui` nello scope della factory, quindi restituiscono il valore selezionato nel contenuto normale dello strumento:
...
## 3) Renderer personalizzati per chiamata/risultato dello strumento
...
- `done(...)` dovrebbe essere chiamato esattamente una volta nel flusso del componente.
- Per UI di lunga durata cancellabili, accoppiare `CancellableLoader` con `AbortSignal` e chiamare `done(...)` da `onAbort`.
...
- `packages/tui/src/utils.ts` — primitive per larghezza/troncamento/sanificazione.
...
- `packages/coding-agent/src/tools/context.ts` — propagazione del contesto UI dello strumento (`hasUI`, `ui`).
