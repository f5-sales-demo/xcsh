...
La distinzione critica: **`notebook` è un editor JSON di notebook, non un esecutore di notebook**. Modifica direttamente i sorgenti delle celle dei file `.ipynb`; non avvia né comunica con un kernel Python.
...
Nessun ciclo di vita del kernel esiste in questo strumento:

- nessuna acquisizione del gateway
- nessun ID sessione del kernel
- nessuna `execute_request`
...
Quando l'agente ha bisogno di eseguire codice Python in stile cella (celle sequenziali, stato persistente, display ricchi), questo passa attraverso lo strumento **`python`**, non `notebook`.

È in quel percorso che risiedono le modalità del kernel, il comportamento di riavvio/annullamento, lo streaming dei chunk e il troncamento degli artefatti di output.

## 2) Semantica di gestione delle celle del notebook (strumento `notebook`)

## Normalizzazione del sorgente

`content` viene suddiviso in `source: string[]` con preservazione dei caratteri di nuova riga:

- ogni riga non finale mantiene il `\n` finale
- la riga finale non ha un carattere di nuova riga forzato

Questo rispecchia le convenzioni JSON dei notebook ed evita concatenazioni accidentali delle righe nelle modifiche successive.
...
Vengono lanciati errori gravi per:
...
Questi diventano risposte dello strumento `Error:` a monte; il renderer utilizza il percorso del notebook + testo di errore formattato.

## 3) Semantica della sessione del kernel (dove effettivamente esiste)
...
- `session` (predefinita)
  ...
  - pulizia inattivo/morto ogni 30s, timeout dopo 5 minuti
  - la coda per sessione serializza l'esecuzione (`session.queue`)
- `per-call`
  - crea il kernel per la richiesta
  - esegue
  - arresta sempre il kernel nel blocco `finally`
...
- il percorso di crash in fase di esecuzione ritenta una volta: riavvia il kernel, riesegue l'handler.
...
- la creazione del kernel nel gateway condiviso ritenta una volta in caso di `SharedGatewayCreateError` con HTTP 5xx.
...
- gli helper di preambolo che leggono il contesto di sessione o artefatti si basano su queste variabili d'ambiente nello stato del processo Python.

## 5) Gestione dello streaming/chunk e del display (percorso supportato dal kernel)
...
- il risultato contrassegna `cancelled=true`
...
- file di riversamento artefatti opzionale (`artifactPath`, `artifactId`)
...
Lo strumento `notebook` **non** utilizza `OutputSink`; non ha alcuna pipeline di troncamento stream/artefatti perché non esegue codice.
...
- vista del risultato:
  ...
  - il limite di anteprima del codice collassato è `PREVIEW_LIMITS.COLLAPSED_LINES * 2`
  - supporta la modalità espansa tramite opzioni di rendering condivise
  - utilizza cache di rendering indicizzata per larghezza + stato di espansione
...
Questo comportamento del renderer non è correlato ai risultati dell'editing JSON del `notebook`, eccetto che entrambi riutilizzano primitive TUI condivise.
...
- `python` esegue codice in un kernel, persiste lo stato per modalità, effettua lo streaming dei chunk, cattura display ricchi, gestisce interruzioni/timeout e supporta troncamento/artefatti dell'output.
- `notebook` esegue esclusivamente mutazioni deterministiche del JSON del notebook; nessuna esecuzione, nessuno stato del kernel, nessuno stream di chunk, nessun output di display, nessuna pipeline di artefatti.

Se un flusso di lavoro necessita di entrambi:
...
L'implementazione attuale non fornisce un singolo strumento che sia muti il file `.ipynb` sia esegua le celle del notebook attraverso il contesto del kernel.
