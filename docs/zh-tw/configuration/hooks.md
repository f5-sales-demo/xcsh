---
title: Hooks
description: 編碼代理生命週期中用於前/後事件自動化的 Hook 系統。
sidebar:
  order: 4
  label: Hooks
i18n:
...
本文件描述 `src/extensibility/hooks/*` 中的**當前 hook 子系統程式碼**。

## 執行階段的當前狀態

Hook 套件（`src/extensibility/hooks/`）仍以 API 介面形式匯出並可供使用，但預設 CLI 執行階段現在會初始化**擴充套件執行器**路徑。在當前啟動流程中：

- `--hook` 被視為 `--extension` 的別名（CLI 路徑合併至 `additionalExtensionPaths`）
- 工具由 `ExtensionToolWrapper` 包裝，而非 `HookToolWrapper`
- 上下文轉換與生命週期事件發送透過 `ExtensionRunner` 處理

因此，本文件記錄 hook 子系統的實作本身（類型／載入器／執行器／包裝器），包含舊版行為與限制。

## 關鍵檔案

- `src/extensibility/hooks/types.ts` — hook 上下文、事件類型與結果契約
- `src/extensibility/hooks/loader.ts` — 模組載入與 hook 探索橋接
- `src/extensibility/hooks/runner.ts` — 事件分派、指令查找與錯誤訊號
...
- 使用 `pi.on(...)` 註冊事件處理器
- 使用 `pi.sendMessage(...)` 發送持久自訂訊息
- 使用 `pi.appendEntry(...)` 持久化非 LLM 狀態
- 使用 `pi.registerCommand(...)` 註冊斜線指令
- 使用 `pi.registerMessageRenderer(...)` 註冊自訂訊息渲染器
- 使用 `pi.exec(...)` 執行 shell 指令

## 探索與載入

`discoverAndLoadHooks(configuredPaths, cwd)` 的執行步驟：

1. 從能力登錄檔載入已探索的 hook（`loadCapability("hooks")`）
2. 附加明確設定的路徑（依絕對路徑去重）
3. 呼叫 `loadHooks(allPaths, cwd)`

`loadHooks` 隨後匯入每個路徑，並預期其具有 `default` 函式。

### 路徑解析

`loader.ts` 解析 hook 路徑的規則為：
...
此處的 hook 載入器使用動態模組匯入，並要求具有預設 JS/TS hook 工廠函式。若探索到的 hook 路徑無法作為模組匯入，載入將失敗，並記錄於 `LoadHooksResult.errors`。
...
工具失敗時，包裝器發送帶有 `isError: true` 與錯誤文字內容的 `tool_result`，然後重新拋出原始錯誤。
...
- 透過 `session_before_*` 與 `session.compacting` 取消／自訂壓縮／樹狀結構行為

### 此實作中 Hook 無法變異的內容

- 就地修改原始工具輸入參數（`tool_call` 僅支援封鎖／允許）
- 工具錯誤拋出後繼續執行（錯誤路徑會重新拋出）
- 包裝器行為中的最終成功／錯誤狀態（回傳的 `isError` 有類型定義，但 `HookToolWrapper` 不套用）

## 排序與衝突行為

### 探索層級排序

能力提供者依優先級排序（越高越優先）。去重依能力鍵，先出現者優先。

對於 `hooks`，能力鍵為 `${type}:${tool}:${name}`。來自較低優先級提供者的重複項目會被標記，並從有效探索清單中排除。

### 載入順序

`discoverAndLoadHooks` 建立一個扁平的 `allPaths` 清單，依解析後的絕對路徑去重，然後 `loadHooks` 按該順序迭代。每個已探索目錄中的檔案順序取決於 `readdir` 的輸出；hook 載入器不進行額外排序。
...
- `tool_call`：最後回傳的結果優先，除非有處理器封鎖；第一個封鎖會立即中斷
- `tool_result`：最後回傳的覆寫值優先（無中斷）
- `context`：鏈式處理；每個處理器接收前一個處理器的訊息輸出
- `before_agent_start`：保留第一個回傳的訊息；後續訊息忽略
- `session_before_*`：追蹤最新回傳的結果；`cancel: true` 立即中斷
- `session.compacting`：最新回傳的結果優先

指令／渲染器衝突：

- `getCommand(name)` 回傳所有 hook 中的第一個匹配項（最先載入者優先）
- `getMessageRenderer(customType)` 回傳第一個匹配項
- `getRegisteredCommands()` 回傳所有指令（不去重）
...
`emitToolCall(...)` 更為嚴格：此處的處理器錯誤不會被吞掉；它們會傳播至呼叫者。在 `HookToolWrapper` 中，這會封鎖工具呼叫（失敗安全）。
...
套件根目錄（`src/index.ts`）將 hook **類型**重新匯出，作為舊版相容性介面。
