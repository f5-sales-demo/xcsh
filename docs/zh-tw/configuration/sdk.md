...
當您想要從自己的 Bun/Node 進程直接存取代理狀態、事件串流、工具連接和會話控制時，請使用它。
...
`@f5xc-salesdemos/xcsh` 從套件根目錄（也透過 `@f5xc-salesdemos/xcsh/sdk`）匯出 SDK API。
...
`createAgentSession()` 遵循「提供即覆寫，省略即探索」原則。
...
- 無檔案系統持久化。
- 適用於測試、臨時工作者、請求範圍的代理。
- 會話方法仍然可用，但持久化特定的行為（檔案恢復/分叉路徑）自然受到限制。
...
使用 `session.subscribe(listener)` 訂閱；它返回一個取消訂閱函式。
...
`AgentSessionEvent` 包含核心 `AgentEvent` 加上會話層級事件：
...
1. 選用的命令/範本展開（`/` 命令、自訂命令、檔案斜線命令、提示範本）
2. 如果目前正在串流：
...
- 隱藏工具（例如 `submit_result`）除非選項要求，否則為選擇性加入。
