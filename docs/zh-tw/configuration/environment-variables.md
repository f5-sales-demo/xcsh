---
title: 環境變數
description: xcsh 設定與行為控制的執行時期環境變數參考。
sidebar:
  order: 2
  label: 環境變數
i18n:
...
- `packages/utils/src/**` 與 `packages/tui/src/**` 中直接影響 coding-agent 執行時期的變數

本文件僅記錄目前有效的行為。

## 解析模型與優先順序

大多數執行時期查詢使用來自 `@f5xc-salesdemos/pi-utils`（`packages/utils/src/env.ts`）的 `$env`。
...
`.env` 檔案中的額外規則：在解析期間，`XCSH_*` 鍵會被鏡像為 `PI_*` 鍵。

---

## 1）模型/提供者驗證

除非另有說明，這些透過 `getEnvApiKey()`（`packages/ai/src/stream.ts`）消費。
...
| `ANTHROPIC_API_KEY`             | Anthropic API 驗證 | 使用 Anthropic 但未使用 OAuth 權杖                           | `ANTHROPIC_OAUTH_TOKEN` 之後的備用方案                                                              |
...
| `OPENAI_API_KEY`                | OpenAI 驗證 | 使用 OpenAI 系列提供者且未明確傳入 apiKey 引數 | 由 OpenAI Completions/Responses 提供者使用                                                      |
| `GEMINI_API_KEY`                | Google Gemini 驗證 | 使用 `google` 提供者模型                                | Gemini 提供者對應的主要金鑰                                                             |
| `GOOGLE_API_KEY`                | Gemini 圖片工具驗證備用 | 在沒有 `GEMINI_API_KEY` 的情況下使用 `gemini_image` 工具            | 由 coding-agent 圖片工具備用路徑使用                                                       |
| `GROQ_API_KEY`                  | Groq 驗證 | 使用 Groq 模型                                             |                                                                                                     |
| `CEREBRAS_API_KEY`              | Cerebras 驗證 | 使用 Cerebras 模型                                         |                                                                                                     |
| `TOGETHER_API_KEY`              | Together 驗證 | 使用 `together` 提供者                                     |                                                                                                     |
| `HUGGINGFACE_HUB_TOKEN`         | Hugging Face 驗證 | 使用 `huggingface` 提供者                                  | 主要的 Hugging Face 權杖環境變數                                                                  |
...
| `LITELLM_API_KEY`               | LiteLLM 驗證 | 使用 `litellm` 提供者                                      | OpenAI 相容的 LiteLLM 代理金鑰。當與 `LITELLM_BASE_URL` 一起設定時，啟用 `models.yml` 的自動設定 |
| `LM_STUDIO_API_KEY`             | LM Studio 驗證（選用） | 使用 `lm-studio` 提供者搭配需要驗證的主機           | 本機 LM Studio 通常無需驗證運行；需要金鑰時，任何非空權杖都可使用         |
| `OLLAMA_API_KEY`                | Ollama 驗證（選用） | 使用 `ollama` 提供者搭配需要驗證的主機              | 本機 Ollama 通常無需驗證運行；需要金鑰時，任何非空權杖都可使用            |
| `LLAMA_CPP_API_KEY`             | Ollama 驗證（選用） | 使用 `llama-server` 搭配 `--api-key` 參數              | 本機 llama.cpp 通常無需驗證運行；設定金鑰時，任何非空權杖都可使用       |
| `XIAOMI_API_KEY`                | 小米 MiMo 驗證 | 使用 `xiaomi` 提供者                                       |                                                                                                     |
| `MOONSHOT_API_KEY`              | Moonshot 驗證 | 使用 `moonshot` 提供者                                     |                                                                                                     |
| `XAI_API_KEY`                   | xAI 驗證 | 使用 xAI 模型                                              |                                                                                                     |
| `OPENROUTER_API_KEY`            | OpenRouter 驗證 | 使用 OpenRouter 模型                                       | 當偏好/自動提供者為 OpenRouter 時，也由圖片工具使用                                  |
...
| `VLLM_API_KEY`                  | vLLM 驗證/探索啟用 | 使用 `vllm` 提供者（本機 OpenAI 相容伺服器）       | 對於無驗證的本機伺服器，任何非空值都可使用                                                 |
| `CURSOR_ACCESS_TOKEN`           | Cursor 提供者驗證 | 使用 Cursor 提供者                                         |                                                                                                     |
| `AI_GATEWAY_API_KEY`            | Vercel AI Gateway 驗證 | 使用 `vercel-ai-gateway` 提供者                            |                                                                                                     |
| `CLOUDFLARE_AI_GATEWAY_API_KEY` | Cloudflare AI Gateway 驗證 | 使用 `cloudflare-ai-gateway` 提供者                        | Base URL 必須設定為 `https://gateway.ai.cloudflare.com/v1/<account>/<gateway>/anthropic` |
...
| `GITHUB_TOKEN` | Copilot 備用方案；網頁爬蟲中的 GitHub API 驗證 | 在網頁爬蟲中：優先於 `GH_TOKEN` 檢查 |
...
當啟用 `CLAUDE_CODE_USE_FOUNDRY` 時，Anthropic 請求切換至 Foundry 模式：

- Base URL 從 `FOUNDRY_BASE_URL` 解析（若未設定，備用方案仍為模型/預設 base URL）。
- 提供者 `anthropic` 的 API 金鑰解析變為：
  `ANTHROPIC_FOUNDRY_API_KEY` → `ANTHROPIC_OAUTH_TOKEN` → `ANTHROPIC_API_KEY`。
- `ANTHROPIC_CUSTOM_HEADERS` 以逗號/換行分隔的 `key: value` 對解析，並合併至請求標頭。
...
| `FOUNDRY_BASE_URL` | URL 字串 | Foundry 模式下的 Anthropic 端點 base URL |
| `ANTHROPIC_FOUNDRY_API_KEY` | 權杖字串 | 用於 `Authorization: Bearer <token>` |
| `ANTHROPIC_CUSTOM_HEADERS` | 標頭清單字串 | 額外標頭；格式 `header-a: value, header-b: value` 或換行分隔 |
| `NODE_EXTRA_CA_CERTS` | PEM 路徑或內嵌 PEM | 用於伺服器憑證驗證的額外 CA 鏈 |
...
| `AWS_BEARER_TOKEN_BEDROCK` | 啟用不記名權杖驗證路徑 |
...
| `AWS_BEDROCK_SKIP_AUTH` | 若為 `1`，注入虛擬憑證（代理/無驗證場景） |
...
| 變數 | 預設值 / 行為 |
|---|---|
| `AZURE_OPENAI_API_KEY` | 除非以選項傳入 API 金鑰，否則必需 |
| `AZURE_OPENAI_API_VERSION` | 預設 `v1` |
| `AZURE_OPENAI_BASE_URL` | 直接 base URL 覆寫 |
| `AZURE_OPENAI_RESOURCE_NAME` | 用於建構 base URL：`https://<resource>.openai.azure.com/openai/v1` |
| `AZURE_OPENAI_DEPLOYMENT_NAME_MAP` | 選用的對應字串：`modelId=deploymentName,model2=deployment2` |

Base URL 解析順序：選項 `azureBaseUrl` → 環境變數 `AZURE_OPENAI_BASE_URL` → 選項/環境變數資源名稱 → `model.baseUrl`。

### Google Vertex AI

| 變數 | 是否必需？ | 備註 |
|---|---|---|
| `GOOGLE_CLOUD_PROJECT` | 是（除非以選項傳入） | 備用方案：`GCLOUD_PROJECT` |
| `GCLOUD_PROJECT` | 備用方案 | 用作替代的專案 ID 來源 |
| `GOOGLE_CLOUD_LOCATION` | 是（除非以選項傳入） | 提供者中無預設值 |
| `GOOGLE_APPLICATION_CREDENTIALS` | 條件性 | 若設定，檔案必須存在；否則檢查 ADC 備用路徑（`~/.config/gcloud/application_default_credentials.json`） |
...
| `KIMI_CODE_BASE_URL` | 覆寫 Kimi 使用端點 base URL（`usage/kimi.ts`） |
...
| 變數 | 預設值 / 行為 |
|---|---|
| `PI_AI_ANTIGRAVITY_VERSION` | 在 Gemini CLI 提供者中覆寫 Antigravity user-agent 版本標籤 |
...
| 變數 | 行為 |
|---|---|
| `DEBUG_CURSOR` | 啟用提供者除錯日誌；`2`/`verbose` 可取得詳細的酬載片段 |
...
| 變數 | 行為 |
|---|---|
| `PI_CACHE_RETENTION` | 若為 `long`，在支援的提供者中啟用長保留期限（`anthropic`、`openai-responses`、Bedrock 保留解析） |
...
| `ZAI_API_KEY` | z.ai 搜尋提供者（也檢查 `agent.db` 中儲存的 OAuth） |
...
3. 來自 `agent.db` 的 Anthropic OAuth 憑證（必須在 5 分鐘緩衝時間內不過期）
4. 通用 Anthropic 環境變數備用方案：提供者金鑰（`ANTHROPIC_FOUNDRY_API_KEY`/`ANTHROPIC_OAUTH_TOKEN`/`ANTHROPIC_API_KEY`）+ 選用的 `ANTHROPIC_BASE_URL`（Foundry 模式啟用時為 `FOUNDRY_BASE_URL`）
...
| `ANTHROPIC_BASE_URL` | 第 4 層驗證路徑的通用備用 base URL |

### Perplexity OAuth 流程行為旗標

| 變數 | 行為 |
|---|---|
| `PI_AUTH_NO_BORROW` | 若設定，在 Perplexity 登入流程中停用 macOS 原生應用程式權杖借用路徑 |
...
| `PI_PYTHON_GATEWAY_URL` | 若設定，使用外部核心閘道而非本機共享閘道 |
| `PI_PYTHON_GATEWAY_TOKEN` | 外部閘道的選用驗證權杖（`Authorization: token <value>`） |
| `PI_PYTHON_IPC_TRACE` | 若為 `1`，在核心模組中啟用低階 IPC 追蹤路徑 |
| `VIRTUAL_ENV` | Python 執行時期解析的最高優先順序虛擬環境路徑 |

額外條件行為：

- 若 `BUN_ENV=test` 或 `NODE_ENV=test`，Python 可用性檢查視為通過，且跳過預熱。
- Python 環境過濾會拒絕常見的 API 金鑰，並允許安全的基礎變數及 `LC_`、`XDG_`、`PI_` 前綴。
...
| `PI_NO_TITLE`              | 若設定（任何非空值），停用第一條使用者訊息時的自動工作階段標題生成   |
| `NULL_PROMPT`              | 若為 `true`，系統提示建構器回傳空字串                                        |
| `PI_BLOCKED_AGENT`         | 在任務工具中封鎖特定的子代理類型                                                 |
| `PI_SUBPROCESS_CMD`        | 覆寫子代理產生命令（`xcsh` / `xcsh.cmd` 解析繞過）                       |
| `PI_TASK_MAX_OUTPUT_BYTES` | 每個子代理的最大擷取輸出位元組數（預設 `500000`）                                    |
| `PI_TASK_MAX_OUTPUT_LINES` | 每個子代理的最大擷取輸出行數（預設 `5000`）                                      |
| `PI_TIMING`                | 若為 `1`，啟用啟動/工具計時測量日誌                                     |
| `PI_DEBUG_STARTUP`         | 在多個啟動路徑中啟用啟動階段除錯輸出至 stderr                       |
| `PI_PACKAGE_DIR`           | 覆寫套件資源基礎目錄解析（文件/範例/變更日誌路徑查詢）            |
| `PI_DISABLE_LSPMUX`        | 若為 `1`，停用 lspmux 偵測/整合並強制直接產生 LSP 伺服器          |
| `LITELLM_BASE_URL`         | LiteLLM 代理 base URL。與 `LITELLM_API_KEY` 一起設定時，在首次執行時觸發自動生成 `models.yml`，並在每次啟動時自我修復 |
| `LM_STUDIO_BASE_URL`       | 預設隱含的 LM Studio 探索 base URL 覆寫（若未設定則為 `http://127.0.0.1:1234/v1`） |
| `OLLAMA_BASE_URL`          | 預設隱含的 Ollama 探索 base URL 覆寫（若未設定則為 `http://127.0.0.1:11434`）      |
| `LLAMA_CPP_BASE_URL`       | 預設隱含的 Llama.cpp 探索 base URL 覆寫（若未設定則為 `http://127.0.0.1:8080`）    |
...
| 變數 | 行為 |
|---|---|
| `PI_BASH_NO_CI` | 抑制自動將 `CI=true` 注入已產生的 shell 環境 |
| `CLAUDE_BASH_NO_CI` | `PI_BASH_NO_CI` 的舊版別名備用方案 |
| `PI_BASH_NO_LOGIN` | 旨在停用登入 shell 模式 |
...
目前實作備註：`PI_BASH_NO_LOGIN`/`CLAUDE_BASH_NO_LOGIN` 會被讀取，但目前 `getShellArgs()` 在兩個分支中都回傳 `['-l','-c']`（實際上今天是無操作的）。

---

## 8）UI/主題/工作階段偵測（自動偵測的環境變數）

這些作為執行時期信號被讀取；通常由終端機/作業系統設定，而非手動設定。

| 變數 | 用途 |
|---|---|
| `COLORTERM`、`TERM`、`WT_SESSION` | 色彩能力偵測（主題色彩模式） |
| `COLORFGBG` | 終端機背景明暗自動偵測 |
| `TERM_PROGRAM`、`TERM_PROGRAM_VERSION`、`TERMINAL_EMULATOR` | 系統提示/上下文中的終端機身份 |
| `KDE_FULL_SESSION`、`XDG_CURRENT_DESKTOP`、`DESKTOP_SESSION`、`XDG_SESSION_DESKTOP`、`GDMSESSION`、`WINDOWMANAGER` | 系統提示/上下文中的桌面/視窗管理器偵測 |
| `KITTY_WINDOW_ID`、`TMUX_PANE`、`TERM_SESSION_ID`、`WT_SESSION` | 穩定的每終端機工作階段追蹤 ID |
...
## 10）TUI 執行時期旗標（共享套件，影響 coding-agent 使用者體驗）

| 變數 | 行為 |
|---|---|
| `PI_NOTIFICATIONS` | `off` / `0` / `false` 抑制桌面通知 |
| `PI_TUI_WRITE_LOG` | 若設定，將 TUI 寫入記錄到檔案 |
...
- 提供者/API 金鑰和 OAuth/不記名憑證（所有 `*_API_KEY`、`*_TOKEN`、OAuth 存取/重新整理權杖）
...
Python 執行時期也會在產生核心子程序之前明確移除許多常見的金鑰變數（`packages/coding-agent/src/ipy/runtime.ts`）。
