...
RPC 模式將程式碼代理以換行分隔的 JSON 協定透過 stdio 執行。

- **stdin**：命令（`RpcCommand`）與擴充功能 UI 回應
...
- `@file` CLI 參數在 RPC 模式中會被拒絕。
- RPC 模式預設會停用自動工作階段標題產生，以避免額外的模型呼叫。
- RPC 模式會將影響工作流程的 `todo.*`、`task.*` 和 `async.*` 設定重設為內建預設值，而非繼承使用者的覆寫設定。
- 行程以 JSONL 格式讀取 stdin（`readJsonl(Bun.stdin.stream())`）。
- 當 stdin 關閉時，行程以代碼 `0` 退出。
...
- 未知命令的回應會以 `id: undefined` 發出（即使請求帶有 `id`）。
- 輸入迴圈中的解析/處理例外會發出 `command: "parse"` 且 `id: undefined`。
- `prompt` 和 `abort_and_prompt` 會立即回傳成功，但如果非同步提示排程失敗，之後可能會發出帶有**相同** id 的錯誤回應。

## 命令結構描述（標準）

`RpcCommand` 定義於 `src/modes/rpc/rpc-types.ts`：
...
替換當前工作階段的記憶體內待辦事項狀態，並回傳標準化的階段列表：
...
這對於希望在第一個提示之前預先載入計畫的主機端很有用。

### `set_host_tools` 酬載

替換 RPC 伺服器可透過 stdio 回呼的目前主機端擁有的工具集：
...
這些工具會在下一次模型呼叫之前加入到作用中的工作階段工具登錄中。重新發送 `set_host_tools` 會替換先前主機端擁有的工具集。

## 事件串流結構描述

RPC 模式會從 `AgentSession.subscribe(...)` 轉發 `AgentSessionEvent` 物件。
...
來自程式碼代理設定結構描述（`packages/coding-agent/src/config/settings-schema.ts`）：
...
- `set_steering_mode` / `set_follow_up_mode`
  - `"one-at-a-time"`：每回合從佇列中取出一則訊息
  - `"all"`：一次取出整個佇列
- `set_interrupt_mode`
  - `"immediate"`：工具執行時在工具呼叫之間檢查引導；待處理的引導可以中止回合中剩餘的工具呼叫
  - `"wait"`：延遲引導直到回合完成
...
- RPC 模式中停用自動工作階段標題產生，且 `setTitle` UI 請求預設也會被抑制，因為大多數主機端沒有有意義的終端標題介面。設定 `PI_RPC_EMIT_TITLE=1` 可重新啟用此 UI 事件。
...
RPC 主機端可以透過發送 `set_host_tools` 向代理公開自訂工具，然後透過相同的傳輸服務執行請求。

### 輸出請求

當代理希望主機端執行其中一個工具時，RPC 模式會發出：
...
在 `host_tool_result` 上設定 `isError: true` 可將回傳內容作為工具錯誤呈現。

## 錯誤模型與可恢復性

### 命令層級失敗

失敗為 `success: false` 並帶有字串 `error`。
