---
title: 原生媒體與系統工具
description: 用於螢幕截圖、影像處理和系統資訊的原生媒體處理工具。
sidebar:
  order: 7
  label: 媒體與系統工具
i18n:
...
本文件是 [`docs/natives-architecture.md`](./natives-architecture.md) 中描述的**系統/媒體/轉換基礎元件**層的子系統深入探討：`image`、`html`、`clipboard` 和 `work` 效能分析。
...
> 注意：不存在 `crates/pi-natives/src/work.rs`；工作效能分析實作於 `prof.rs` 中，並由 `task.rs` 中的檢測程式碼提供資料。
...
- 當 `cleanContent=true` 時，會啟用 `PreprocessingPreset::Aggressive` 預處理以及導覽/表單的強制移除標誌。
- `skipImages` 預設為 `false`。

### 剪貼簿 (`clipboard`)

- **文字路徑**：
  - TS 在 stdout 為 TTY 時先發送 OSC 52（`\x1b]52;c;<base64>\x07`）。
  - 接著以盡力而為的方式透過原生剪貼簿 API（`native.copyToClipboard`）嘗試相同文字。
...
4. `encode(...)` 具現化位元組（`image.encode`），不會改變影像尺寸。
...
2. 除非指定，否則轉換使用預設選項（`cleanContent=false`、`skipImages=false`）執行。
3. 回傳 Markdown 字串或拒絕。
...
`copyToClipboard(text)` 有意設計為盡力而為的多路徑方式：
...
`readImageFromClipboard()` 在各階段有不同的嚴格程度：

1. TS 對不支援的執行時環境（Termux/無頭 Linux）硬性閘控為 `null`。
...
1. 無需明確啟動：當任務輔助函式執行時，效能分析始終開啟。
2. 每個被檢測的任務作用域在守衛釋放時記錄一個樣本。
3. 樣本在達到緩衝區容量後覆寫最舊的條目。
4. `getWorkProfile(lastSeconds)` 讀取時間窗口並衍生折疊/摘要/SVG 產出物。

失敗轉換：

- SVG 生成失敗為軟性失敗（`svg: null`），而折疊和摘要仍會回傳。
- 空的樣本窗口回傳空的折疊資料和 `svg: null`，而非錯誤。
...
- 文字複製在 TS 層為盡力而為：操作失敗會被抑制。
- 影像讀取區分「無影像」（`null`）與操作失敗（拒絕）。
- Termux/無頭 Linux 被視為影像讀取的不支援環境（`null`）。

### 工作效能分析

- 函式呼叫本身的擷取是嚴格的，但產出物生成為部分盡力而為（`svg` 可為 null）。
- 緩衝區截斷是預期行為（環形緩衝區），而非資料遺失錯誤。

## 平台注意事項

- **剪貼簿文字**：OSC 52 取決於終端機支援；原生剪貼簿存取取決於桌面環境/工作階段。
- **剪貼簿影像讀取**：在 Termux 和沒有顯示伺服器的 Linux 上於 TS 中被阻擋。
