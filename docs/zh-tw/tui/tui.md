...
description: 針對擴充功能、自訂工具及自訂渲染器的 TUI 整合契約。
sidebar:
  order: 1
  label: 擴充功能整合
i18n:
...
本文件涵蓋 `packages/coding-agent` 和 `packages/tui` 用於擴充功能 UI、自訂工具 UI 及自訂渲染器的**現行** TUI 契約。

## 此子系統是什麼

執行時期有兩個層級：

- **渲染引擎（`packages/tui`）**：差異式終端機渲染器、輸入分派、焦點、覆蓋層、游標定位。
- **整合層（`packages/coding-agent`）**：掛載擴充功能/自訂工具元件、連接快捷鍵/主題，並還原編輯器狀態。

## 各模式的執行時期行為

| 模式 | `ctx.ui.custom(...)` 可用性 | 備註 |
| --- | --- | --- |
| 互動式 TUI | 支援 | 元件掛載於編輯器區域、獲得焦點，且必須呼叫 `done(result)` 才能解析。 |
| 背景/無頭模式 | 非互動式 | UI 上下文為空操作（`hasUI === false`）。 |
| RPC 模式 | 不支援 | `custom()` 回傳 `Promise<never>` 且不會掛載 TUI 元件。 |

如果您的擴充功能/工具可在非互動模式下執行，請使用 `ctx.hasUI` / `pi.hasUI` 進行判斷。
...
游標行為使用 `CURSOR_MARKER`（而非 `getCursorPosition`）。獲得焦點的元件在渲染文字中發出標記；`TUI` 提取標記並定位硬體游標。
...
3. **使用 ANSI 感知方式截斷/換行文字**，透過 `truncateToWidth()` / `wrapTextWithAnsi()`。
4. **清理 tab 字元/內容**，針對外部來源使用 `replaceTabs()`（以及 coding-agent 渲染路徑中的高階清理器）。
...
使用 `matchesKey(data, "...")` 來匹配導航鍵和組合鍵。

### 遵循使用者設定的應用程式快捷鍵

擴充功能 UI 工廠接收一個 `KeybindingsManager`（互動模式），因此您可以遵循映射的操作而非硬編碼按鍵：
...
- `TUI.setFocus(component)` 將輸入路由至該元件。
- 覆蓋層 API 存在於 `TUI` 中（`showOverlay`、`OverlayHandle`），但在互動模式下，擴充功能 `ctx.ui.custom` 掛載目前是直接替換編輯器元件區域。
- `custom(..., options?: { overlay?: boolean })` 選項存在於擴充功能型別中；互動式擴充功能掛載目前會忽略此選項。
...
- 以您的元件替換編輯器元件。
- 將焦點設定至您的元件。
- 當 `done(result)` 被呼叫時：呼叫 `component.dispose?.()`、還原編輯器及文字、將焦點設定至編輯器、解析 Promise。

因此 `done(...)` 是完成流程的必要步驟。

## 2) Hook/自訂工具 UI 上下文（舊版型別）

`HookUIContext.custom` 在 hook/自訂工具型別中被定義為 `(tui, theme, done)`。
底層的互動式實作以 `(tui, theme, keybindings, done)` 呼叫工廠函式。JS 使用者可以使用額外的參數；型別層面的相容性仍反映 3 個參數的舊版簽章。
...
- `dispose()` 在型別層面是選擇性的，但當您擁有計時器、子行程、監看器、socket 或覆蓋層時應該實作。
- `done(...)` 應在您的元件流程中僅呼叫一次。
- 對於可取消的長時間執行 UI，將 `CancellableLoader` 與 `AbortSignal` 搭配使用，並從 `onAbort` 呼叫 `done(...)`。
...
- `packages/tui/src/keys.ts` / `keybindings.ts` — 按鍵解析與可設定的操作映射。
