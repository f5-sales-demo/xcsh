...
- 執行環境設定型別：`packages/coding-agent/src/mcp/types.ts`
- 設定寫入器：`packages/coding-agent/src/mcp/config-writer.ts`
- 載入器與驗證：`packages/coding-agent/src/mcp/config.ts`
- 獨立 `mcp.json` 探索：`packages/coding-agent/src/discovery/mcp-json.ts`
- Schema：`packages/coding-agent/src/config/mcp-schema.json`

## 建議的設定檔位置

OMP 可以從多種工具探索 MCP 伺服器（`.claude/`、`.cursor/`、`.vscode/`、`opencode.json` 等等），但對於 OMP 原生設定，您通常應使用以下其中一個檔案：

- 專案層級：`.xcsh/mcp.json`
- 使用者層級：`~/.xcsh/mcp.json`

OMP 也接受專案根目錄中的備援獨立檔案：

- `mcp.json`
- `.mcp.json`

當您希望由 OMP 擁有設定時，請使用 `.xcsh/mcp.json`。僅當您需要一個其他 MCP 客戶端也可能讀取的可攜式備援檔案時，才使用根目錄的 `mcp.json` / `.mcp.json`。

## 新增 schema 參考

在檔案頂部新增此行以啟用編輯器的自動完成與驗證：
...
- `enabled?: boolean` — 當為 `false` 時跳過此伺服器
- `timeout?: number` — 連線逾時時間（毫秒）
- `auth?: { ... }` — OMP 用於 OAuth/API 金鑰流程的驗證中繼資料
- `oauth?: { ... }` — 在驗證/重新驗證期間使用的明確 OAuth 客戶端設定
...
這與 GitHub 託管的 GitHub MCP 伺服器端點相符。
...
`sse` 仍為相容性而受到支援，但 MCP 規範現在建議新伺服器使用 Streamable HTTP（`type: "http"`）。
...
Slack 是目前最明確的範例。Slack 的 MCP 伺服器託管在 `https://mcp.slack.com/mcp`，使用 Streamable HTTP，並需要使用您 Slack 應用程式的客戶端憑證進行機密 OAuth。
...
這與 GitHub 官方本機 Docker 映像 `ghcr.io/github/github-mcp-server` 相符。
...
在 OMP 啟動伺服器或發出 HTTP 請求之前，它會如下解析 `env` 和 `headers` 的值：

1. 如果值以 `!` 開頭，OMP 會將其作為 shell 命令執行，並使用修剪後的 stdout。
2. 否則 OMP 會先檢查該值是否與環境變數名稱相符。
3. 如果該環境變數未設定，OMP 會將字串作為字面值使用。
...
這意味著以下寫法對於本機秘密是有效且方便的：

- `"GITHUB_PERSONAL_ACCESS_TOKEN": "GITHUB_PERSONAL_ACCESS_TOKEN"` → 從當前 shell 環境複製
- `"Authorization": "Bearer hardcoded-token"` → 使用字面值
- `"Authorization": "!printf 'Bearer %s' \"$GITHUB_TOKEN\""` → 透過命令建構標頭

### 在根目錄的 `mcp.json` 和 `.mcp.json` 中

獨立備援載入器也會在探索期間展開字串中的 `${VAR}` 和 `${VAR:-default}`。
...
如果您想要最不令人意外的 OMP 行為，建議使用 `.xcsh/mcp.json` 並使用明確的 env/header 值。

## `disabledServers`

`disabledServers` 主要在使用者設定檔（`~/.xcsh/mcp.json`）中使用，當伺服器從其他來源被探索到，而您希望 OMP 忽略它但又不想編輯該工具的設定時很有用。
...
當您想要引導式設定時，請使用 `/mcp add`。

在以下情況下使用直接編輯 JSON：

- 您需要精靈尚未提示的傳輸方式或驗證選項
- 您想從另一個 MCP 客戶端貼上伺服器定義
- 您想在編輯器中獲得基於 schema 的驗證

編輯後，請使用：

- `/mcp reload` 在當前工作階段中重新探索並重新連線伺服器
...
- 省略 `type` 表示 `stdio`
- 如果您貼上遠端伺服器設定卻忘記加上 `"type": "http"`，OMP 會將其視為 `stdio` 並抱怨缺少 `command`
- `sse` 仍然有效以保持相容性，但新的託管伺服器通常應設定為 `http`

## 探索與優先順序

OMP 不會合併跨檔案的重複伺服器定義。探索提供者有優先順序排列，較高優先順序的定義勝出。

實務上：

- 當您想要 OMP 專用的覆寫時，建議使用 `.xcsh/mcp.json` 或 `~/.xcsh/mcp.json`
- 盡可能在不同工具間保持伺服器名稱唯一
- 當第三方設定持續重新引入您不想要的伺服器時，在使用者設定中使用 `disabledServers`
...
JSON 是有效的，但伺服器可能仍然無法連線。請使用 `/mcp test <name>` 並檢查：
...
### 伺服器存在於其他工具的設定中但未出現在 OMP 中

執行 `/mcp list`。OMP 會探索許多第三方 MCP 檔案，但專案層級的載入也可以透過 `mcp.enableProjectConfig` 設定停用。
