---
title: 交接生成流水線
description: 交接生成流水線，用於建立可攜式會話摘要以支援團隊協作。
sidebar:
  order: 8
  label: 交接流水線
i18n:
...
本文件描述 coding-agent 目前如何實作 `/handoff`：觸發路徑、生成提示詞、完成捕獲、會話切換，以及上下文重新注入。
...
2. 在互動式輸入處理（`InputController`）中，符合 `/handoff` 或 `/handoff ...` 的提交文字會在正常提示詞提交之前被攔截。
3. 編輯器被清除並呼叫 `handleHandoffCommand(customInstructions?)`。
4. `CommandController.handleHandoffCommand` 使用目前的條目執行前置檢查：
   - 計算 `type === "message"` 的條目數量。
   - 若 `< 2`，則警告：`Nothing to hand off (no messages yet)` 並返回。

相同的最低內容檢查也存在於 `AgentSession.handoff()` 中，若違反則拋出錯誤。這在 UI 層和會話層都做了重複的安全檢查。
...
- 構建一個固定的行內提示詞，要求產生結構化的交接文件（`Goal`、`Constraints & Preferences`、`Progress`、`Key Decisions`、`Critical Context`、`Next Steps`）
...
`expandPromptTemplates: false` 防止此內部指令負載被進行斜線/提示詞範本展開。

### 2) 捕獲完成結果

在發送提示詞之前，`handoff()` 訂閱會話事件並等待 `agent_end`。

收到 `agent_end` 時，它從代理狀態中提取交接文字，方法是向後掃描找到最近的 `assistant` 訊息，然後將所有 `type === "text"` 的 `content` 區塊用 `\n` 串接。
...
它始終在 `finally` 中清除 `#handoffAbortController`。

### 4) 新會話建立

若文字已捕獲且未被中止：

1. 刷新目前會話寫入器（`sessionManager.flush()`）
2. 啟動全新會話（`sessionManager.newSession()`）
3. 重置記憶體中的代理狀態（`agent.reset()`）
4. 重新綁定 `agent.sessionId` 到新會話 id
5. 清除佇列中的上下文陣列（`#steeringMessages`、`#followUpMessages`、`#pendingNextTurnMessages`）
6. 重置待辦提醒計數器

`newSession()` 建立一個新的標頭和空的條目列表（葉節點重置為 `null`）。在交接路徑中，不會傳遞 `parentSession`。

### 5) 交接上下文注入

生成的交接文件被包裝後附加到新會話，作為 `custom_message` 條目：
...
1. `sessionManager.buildSessionContext()` 解析目前葉節點的訊息列表
...
在生成期間，正常的訊息持久化保持活躍。助手的交接回應在 `message_end` 時作為常規 `message` 條目被持久化。

結果：原始會話包含可見的已生成交接內容，作為歷史對話記錄的一部分。

### 新會話

會話重置後，交接以 `custom_message` 持久化，`customType: "handoff"`。

`buildSessionContext()` 透過 `createCustomMessage(...)` 將此條目轉換為執行時期的自訂/使用者上下文訊息，因此它會被包含在新會話的後續提示詞中。
...
- 成功時：
  - `rebuildChatFromMessages()`（載入新會話上下文，包括注入的交接內容）
  - 使狀態列和編輯器頂部邊框失效
  - 重新載入待辦事項
  - 附加成功聊天訊息：`New session started with handoff context`
- 發生例外時：
...
在目前的互動式控制器接線中，`/handoff` 沒有安裝專用的 Escape 處理器來呼叫 `abortHandoff()`（不像壓縮/分支摘要路徑會暫時覆蓋 `editor.onEscape`）。

實際影響：

- 存在會話層級的取消支援，但在 `/handoff` 命令路徑中沒有交接專用的快捷鍵綁定。
- 使用者中斷仍可能透過更廣泛的代理中止路徑發生，但這與 `abortHandoff()` 使用的明確取消通道不同。
...
- **失敗**
  - `handoff()` / 提示詞管線拋出的任何其他錯誤（模型/API 驗證錯誤、執行時期例外等）
  - UI 顯示 `Handoff failed: ...`

額外細微差異：若生成完成但未提取到文字，`handoff()` 返回 `undefined`，控制器目前報告為**已取消**，而非**失敗**。
...
4. 內部交接提示詞提交（在聊天中作為正常助手生成可見）
5. 收到 `agent_end` 時，提取最後助手文字
...
- 交接提取是啟發式的：「最後助手文字區塊」；無結構性驗證。
- 無硬性檢查確認生成的 markdown 遵循所要求的區段格式。
