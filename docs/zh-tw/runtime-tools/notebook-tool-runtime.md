...
關鍵區別：**`notebook` 是一個 JSON notebook 編輯器，而非 notebook 執行器**。它直接編輯 `.ipynb` 儲存格來源；它不會啟動或與 Python 核心通訊。
...
- 相對於工作階段當前工作目錄解析路徑（`resolveToCwd`）。
...
- 無 `execute_request`
- 無來自核心通道的串流區塊
- 無豐富顯示擷取（`image/png`、JSON 顯示、status MIME）

## 類 Notebook 執行路徑 (`src/tools/python.ts` + `src/ipy/*`)

當代理需要執行儲存格風格的 Python 程式碼（循序儲存格、持久狀態、豐富顯示）時，會透過 **`python` 工具** 進行，而非 `notebook`。

該路徑是核心模式、重啟/取消行為、區塊串流以及輸出成品截斷實際存在的地方。

## 2) Notebook 儲存格處理語意（`notebook` 工具）

## 來源正規化

`content` 被分割為 `source: string[]` 並保留換行符號：

- 每個非最終行保留尾端 `\n`
- 最終行不強制加入尾端換行符號

這符合 notebook JSON 慣例，避免後續編輯時意外的行串接。
...
- `delete`
  - 移除 `cells[cell_index]`
  - 在 details 中回傳已移除的 `source` 供渲染器預覽
...
- 無效的 JSON
- `cells` 遺失或非陣列
- 索引超出範圍（插入和非插入操作有不同的有效範圍）
- `edit`/`insert` 缺少 `content`

這些在上游成為 `Error:` 工具回應；渲染器使用 notebook 路徑 + 格式化的錯誤文字。

## 3) 核心工作階段語意（實際存在的位置）

核心語意實作於 `executePython` / `PythonKernel`，適用於 `python` 工具。
...
- `session`（預設）
  - 核心快取於 `kernelSessions` 映射中
  - 最多 4 個工作階段；溢出時驅逐最舊的
  - 每 30 秒進行閒置/死亡清理，5 分鐘後逾時
  - 每個工作階段的佇列序列化執行（`session.queue`）
- `per-call`
  - 為請求建立核心
  - 執行
  - 始終在 `finally` 中關閉核心
...
`python` 工具僅在多儲存格呼叫中的第一個儲存格傳遞 `reset`；後續儲存格始終以 `reset: false` 執行。

## 核心死亡 / 重啟 / 重試

在工作階段模式（`withKernelSession`）中：

- 死亡核心透過心跳偵測（每 5 秒 `kernel.isAlive()` 檢查）或執行失敗偵測。
- 執行前的死亡狀態觸發 `restartKernelSession`。
- 執行時崩潰路徑重試一次：重啟核心，重新執行處理器。
- 同一工作階段中 `restartCount > 1` 會拋出 `Python kernel restarted too many times in this session`。

啟動重試行為：

- 共享閘道器核心建立在遇到 HTTP 5xx 的 `SharedGatewayCreateError` 時重試一次。
...
核心啟動時從執行器接收可選的 env 映射：

- `PI_SESSION_FILE`（工作階段狀態檔案路徑）
- `ARTIFACTS`（成品目錄）

`PythonKernel.#initializeKernelEnvironment(...)` 接著在核心內部執行初始化腳本以：

- `os.chdir(cwd)`
- 將 env 項目注入 `os.environ`
- 如果遺失，將 cwd 加到 `sys.path` 前方

影響：

- 讀取工作階段或成品上下文的前置輔助程式依賴 Python 程序狀態中的這些環境變數。

## 5) 串流/區塊及顯示處理（核心支援路徑）

核心客戶端按每次執行處理 Jupyter 協定訊息：

- `stream` -> 文字區塊傳送至 `onChunk`
- `execute_result` / `display_data` ->
  - 依 MIME 優先順序選擇顯示文字：`text/markdown` > `text/plain` > 轉換後的 `text/html`
  - 結構化輸出另外擷取：
    - `application/json` -> `{ type: "json" }`
    - `image/png` -> `{ type: "image" }`
    - `application/x-xcsh-status` -> `{ type: "status" }`（不發出文字）
- `error` -> 回溯文字推送至區塊串流 + 結構化錯誤中繼資料
- `input_request` -> 發出 stdin 警告文字，送出空白 `input_reply`，標記已請求 stdin
- 完成等待 `execute_reply` 和核心 `status=idle` 兩者

取消/逾時：

- 中止訊號觸發 `interrupt()`（REST `/interrupt` + control-channel `interrupt_request`）
- 結果標記 `cancelled=true`
- 逾時路徑在輸出中加註 `Command timed out after <n> seconds`
...
- 清理每個區塊（`sanitizeText`）
- 追蹤總行數/輸出行數和位元組數
- 可選的成品溢出檔案（`artifactPath`、`artifactId`）
- 當記憶體中的緩衝區超過閾值（除非覆寫，否則為 `DEFAULT_MAX_BYTES`）時：
  - 標記為已截斷
  - 在記憶體中保留尾端位元組（UTF-8 安全邊界）
  - 可將完整串流溢出至成品接收器
...
- 可見輸出文字（可能經過尾端截斷）
- 截斷旗標 + 計數
- 成品 ID（供 `artifact://<id>` 引用使用）

`python` 工具將此中繼資料轉換為結果截斷通知和 TUI 警告。

`notebook` 工具**不使用** `OutputSink`；它沒有串流/成品截斷管線，因為它不執行程式碼。
...
- 如果第一段文字內容以 `Error:` 開頭，渲染器格式化為 notebook 錯誤區塊。
...
此渲染器行為與 `notebook` JSON 編輯結果無關，只是兩者都重用共享的 TUI 基本元件。

## 8) 與純 Python 工具行為的差異

如果「純 Python 工具」指的是 `python` 執行路徑：

- `python` 在核心中執行程式碼，按模式持久化狀態，串流區塊，擷取豐富顯示，處理中斷/逾時，並支援輸出截斷/成品。
- `notebook` 僅執行確定性的 notebook JSON 變更；無執行、無核心狀態、無區塊串流、無顯示輸出、無成品管線。
...
目前的實作不提供一個同時變更 `.ipynb` 並透過核心上下文執行 notebook 儲存格的單一工具。
