...
description: 用於探索、安裝和管理精選外掛集合的 Marketplace 外掛系統。
sidebar:
  order: 4
  label: Marketplace
i18n:
...
Marketplace 系統讓您可以從 Git 託管的目錄中探索、安裝和管理外掛。它與 Claude Code 外掛註冊格式相容。
...
當您執行 `/marketplace add <source>` 時，系統會分類來源：
...
系統會克隆儲存庫（或讀取本地目錄），找到 `.xcsh-plugin/marketplace.json`，驗證它，並在本地快取目錄。
...
| 欄位 | 必填 | 描述 |
|---|---|---|
| `name` | 是 | 外掛名稱（規則與 marketplace 名稱相同） |
| `source` | 是 | 外掛的位置（見下方） |
| `description` | 否 | 簡短描述 |
| `version` | 否 | 版本字串 |
| `author` | 否 | `{ name, email? }` |
