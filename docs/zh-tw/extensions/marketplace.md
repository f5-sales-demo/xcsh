...
或者直接輸入 `/marketplace`（不帶任何參數）來開啟互動式外掛瀏覽器。

## 概念

**marketplace** 是一個 Git 儲存庫（或本地目錄），其中包含位於 `.xcsh-plugin/marketplace.json` 的目錄檔案。該目錄列出了可用的外掛及其來源、描述和中繼資料。

**外掛**是一個包含技能、命令、鉤子、MCP 伺服器或 LSP 伺服器的目錄。外掛以 `name@marketplace` 格式識別（例如 `code-review@f5xc-salesdemos-marketplace`）。

**作用域**：外掛可以在兩個作用域中安裝：

- **user**（預設）——在所有專案中可用，儲存在 `~/.xcsh/plugins/installed_plugins.json`
- **project**——僅在目前專案中可用，儲存在 `.xcsh/installed_plugins.json`

專案作用域的安裝會覆蓋同一外掛的使用者作用域安裝。
...
系統會複製儲存庫（或讀取本地目錄）、找到 `.xcsh-plugin/marketplace.json`、驗證它，並在本地快取目錄。
...
| 欄位 | 描述 |
|---|---|
| `name` | Marketplace 名稱。小寫英數字、連字號和點號。必須以英數字開頭和結尾。最多 64 個字元。 |
...
| 欄位 | 必要 | 描述 |
|---|---|---|
| `name` | 是 | 外掛名稱（與 marketplace 名稱相同規則） |
| `source` | 是 | 外掛的位置（見下方） |
| `description` | 否 | 簡短描述 |
| `version` | 否 | 版本字串 |
| `author` | 否 | `{ name, email? }` |
...
~/.xcsh/
  ...
  plugins/
    installed_plugins.json     # 使用者作用域已安裝的外掛
    cache/
      marketplaces/            # 已快取的 marketplace 目錄
      plugins/                 # 已快取的外掛目錄

<project>/.xcsh/
  installed_plugins.json       # 專案作用域已安裝的外掛
```
