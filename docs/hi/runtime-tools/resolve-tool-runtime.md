...
`resolve` एक छिपा हुआ टूल है जो एक लंबित प्रीव्यू एक्शन को अंतिम रूप देता है।

- `action: "apply"` लंबित एक्शन पर `apply(reason)` निष्पादित करता है और परिवर्तनों को सहेजता है।
- `action: "discard"` यदि प्रदान किया गया है तो `reject(reason)` को आमंत्रित करता है; अन्यथा एक डिफ़ॉल्ट "Discarded" संदेश के साथ एक्शन को हटा देता है।

यदि कोई लंबित एक्शन मौजूद नहीं है, तो `resolve` इस त्रुटि के साथ विफल होता है:

- `No pending action to resolve. Nothing to apply or discard.`

## लंबित एक्शन एक स्टैक (LIFO) हैं
...
`resolve` हमेशा पहले **सबसे ऊपरी** लंबित एक्शन का उपभोग करता है (`pop()`), इसलिए कई प्रीव्यू-उत्पादक टूल पंजीकरण के विपरीत क्रम में हल होते हैं।

## बिल्ट-इन प्रोड्यूसर उदाहरण (`ast_edit`)

`ast_edit` पहले संरचनात्मक प्रतिस्थापनों का प्रीव्यू करता है। जब प्रीव्यू में प्रतिस्थापन होते हैं और अभी तक लागू नहीं किए गए होते हैं, तो यह एक लंबित एक्शन पुश करता है जिसमें शामिल होता है:

- label (मानव-पठनीय सारांश)
- `sourceToolName` (`ast_edit`)
- `apply(reason: string)` कॉलबैक जो `dryRun: false` के साथ AST संपादन को पुनः चलाता है
...
- `reject?(reason: string): Promise<AgentToolResult<unknown> | undefined>` (वैकल्पिक) — discard पर आमंत्रित होता है; यदि प्रदान किया गया है तो रिटर्न वैल्यू डिफ़ॉल्ट "Discarded" संदेश को प्रतिस्थापित करता है
...
`pushPendingAction` कस्टम टूल लोडर द्वारा सक्रिय सत्र `PendingActionStore` का उपयोग करके जोड़ा जाता है।
...
जब `PendingActionStore.hasPending` true होता है, तो एजेंट रनटाइम टूल चॉइस को `resolve` की ओर पक्षपाती करता है ताकि सामान्य टूल प्रवाह जारी रहने से पहले लंबित प्रीव्यू स्पष्ट रूप से अंतिम रूप दिए जाएं।

## डेवलपर मार्गदर्शन

- लंबित एक्शन का उपयोग केवल विनाशकारी या उच्च-प्रभाव वाले ऑपरेशनों के लिए करें जिन्हें स्पष्ट apply/discard का समर्थन करना चाहिए।
- `label` को संक्षिप्त और विशिष्ट रखें; यह resolve रेंडरर आउटपुट में दिखाया जाता है।
- सुनिश्चित करें कि `apply(reason)` एक-बार निष्पादन के लिए पर्याप्त रूप से नियतात्मक और आइडेम्पोटेंट है; `reason` सूचनात्मक है और व्यवहार को नहीं बदलना चाहिए।
- `reject(reason)` तब लागू करें जब discard को क्लीनअप की आवश्यकता हो (अस्थायी स्थिति, लॉक, सूचनाएँ); स्टेटलेस प्रीव्यू के लिए इसे छोड़ दें जहाँ डिफ़ॉल्ट संदेश पर्याप्त है।
- यदि आपका टूल कई प्रीव्यू स्टेज कर सकता है, तो LIFO सिमेंटिक्स याद रखें: नवीनतम पुश किया गया एक्शन पहले हल होता है।
