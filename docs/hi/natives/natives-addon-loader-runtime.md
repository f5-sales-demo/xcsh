...
यह दस्तावेज़ `@f5xc-salesdemos/pi-natives` में addon loading/validation परत की गहन जानकारी प्रदान करता है: `native.ts` कैसे तय करता है कि कौन सी `.node` फ़ाइल लोड करनी है, एम्बेडेड पेलोड एक्सट्रैक्शन कब चलता है, और स्टार्टअप विफलताओं की रिपोर्ट कैसे की जाती है।
...
Loader/runtime के उत्तरदायित्व जानबूझकर सीमित रखे गए हैं:

- Addon फ़ाइलनामों और निर्देशिकाओं के लिए प्लेटफ़ॉर्म/CPU-जागरूक उम्मीदवार सूची बनाना।
- वैकल्पिक रूप से एक एम्बेडेड addon को संस्करणित प्रति-उपयोगकर्ता कैश निर्देशिका में मूर्त रूप देना।
- निर्धारित क्रम में उम्मीदवारों का प्रयास करना।
...
मॉड्यूल आरंभीकरण (`export const native = loadNative();`) पर, `native.ts` स्थिर संदर्भ की गणना करता है:
...
- **मुख्य निर्देशिकाएँ**:
  ...
  - `execDir`: `process.execPath` वाली निर्देशिका।
  - `versionedDir`: `<getNativesDir()>/<packageVersion>`।
  - `userDataDir` फ़ॉलबैक:
    - Windows: `%LOCALAPPDATA%/xcsh` (या `%USERPROFILE%/AppData/Local/xcsh`)।
    - गैर-Windows: `~/.local/bin`।
- **संकलित-बाइनरी मोड** (`isCompiledBinary`): सत्य यदि इनमें से कोई भी:
  - `PI_COMPILED` env var सेट है, या
  - `import.meta.url` में Bun-एम्बेडेड मार्कर हैं (`$bunfs`, `~BUN`, `%7EBUN`)।
- **वैरिएंट ओवरराइड**: `PI_NATIVE_VARIANT` (केवल `modern`/`baseline`; अमान्य मान अनदेखे किए जाते हैं)।
- **चयनित वैरिएंट**: स्पष्ट ओवरराइड, अन्यथा x64 पर रनटाइम AVX2 पहचान (AVX2 होने पर `modern`, अन्यथा `baseline`)।

## प्लेटफ़ॉर्म समर्थन और टैग रिज़ॉल्यूशन

`SUPPORTED_PLATFORMS` इनमें निर्धारित है:
...
- असमर्थित प्लेटफ़ॉर्म पहले से अस्वीकार नहीं किए जाते।
- Loader फिर भी पहले सभी गणना किए गए उम्मीदवारों का प्रयास करता है।
- यदि कुछ भी लोड नहीं होता, तो यह समर्थित टैग सूचीबद्ध करते हुए एक स्पष्ट असमर्थित-प्लेटफ़ॉर्म त्रुटि फेंकता है।

यह वास्तव में असमर्थित लक्ष्यों के लिए कठोर विफलता बनाए रखते हुए निकट-मिस मामलों के लिए उपयोगी निदान संरक्षित करता है।
...
3. परिणाम:
   - AVX2 उपलब्ध -> `modern`
   - AVX2 अनुपलब्ध/अनपता -> `baseline`

### गैर-x64 व्यवहार

- कोई वैरिएंट उपयोग नहीं किया जाता; loader डिफ़ॉल्ट फ़ाइलनाम (`pi_natives.<platform>-<arch>.node`) पर रहता है।
...
अंतिम त्रुटि संदेशों में प्रयुक्त `addonLabel` या तो `<tag>` या `<tag> (<variant>)` है।
...
वैरिएंट-रिज़ॉल्व्ड फ़ाइलनाम सूची से निर्मित और इस क्रम में खोजे जाते हैं:

- **गैर-संकलित रनटाइम**:
  1. `<nativeDir>/<filename>`
  2. `<execDir>/<filename>`

- **संकलित रनटाइम** (`PI_COMPILED` या Bun एम्बेडेड मार्कर):
  1. `<versionedDir>/<filename>`
  2. `<userDataDir>/<filename>`
  3. `<nativeDir>/<filename>`
  4. `<execDir>/<filename>`

`dedupedCandidates` पहली उपस्थिति क्रम बनाए रखते हुए डुप्लिकेट हटाता है।
...
2. शेष डीडुप्लिकेटेड उम्मीदवारों का क्रम में प्रयास किया जाता है।
3. पहला उम्मीदवार जो `require(...)` और `validateNative(...)` दोनों पास करता है, विजयी होता है।
...
- `platformTag`
- `version`
- `files[]` जहाँ प्रत्येक प्रविष्टि में `variant`, `filename`, `filePath` है

वर्तमान चेक-इन डिफ़ॉल्ट `embeddedAddon: null` है; संकलित आर्टिफ़ैक्ट इसे वास्तविक मेटाडेटा से बदल सकते हैं।
...
वैरिएंट फ़ाइल चयन रनटाइम वैरिएंट इरादे को दर्पण करता है:
...
1. `<versionedDir>` का अस्तित्व सुनिश्चित करें (`mkdirSync(..., { recursive: true })`)।
2. यदि `<versionedDir>/<selected filename>` पहले से मौजूद है, तो उसे पुनः उपयोग करें (कोई पुनर्लेखन नहीं)।
3. अन्यथा एम्बेडेड स्रोत `filePath` पढ़ें और लक्ष्य फ़ाइल लिखें।
4. उच्चतम-प्राथमिकता लोड प्रयास के लिए लक्ष्य पथ लौटाएँ।

विफलता पर, एक्सट्रैक्शन तुरंत क्रैश नहीं करता; यह एक त्रुटि प्रविष्टि जोड़ता है (निर्देशिका निर्माण या लेखन विफलता) और loader सामान्य उम्मीदवार जाँच पर आगे बढ़ता है।
...
- प्रत्येक आवश्यक एक्सपोर्ट नाम के लिए, यह जाँचता है `typeof bindings[name] === "function"`।
- गायब नाम एकत्रित किए जाते हैं।
- यदि कोई गायब है, तो loader फेंकता है:
  - स्रोत addon पथ,
  - गायब एक्सपोर्ट सूची,
...
नोट: `bindings.ts` केवल आधार `cancelWork(id)` सदस्य घोषित करता है; मॉड्यूल `types.ts` फ़ाइलें अतिरिक्त सिंबल डिक्लेरेशन-मर्ज करती हैं जिन्हें `validateNative` लागू करता है।

## विफलता व्यवहार और निदान
...
- प्रति-उम्मीदवार गायब-एक्सपोर्ट विफलताएँ रिकॉर्ड करता है।
- शेष उम्मीदवारों की जाँच जारी रखता है।
- यदि कोई उम्मीदवार सत्यापित नहीं होता, तो अंतिम त्रुटि में प्रत्येक प्रयास किया गया पथ और उसका विफलता संदेश शामिल होता है।

## संकलित-बाइनरी स्टार्टअप विफलताएँ

संकलित मोड में अंतिम निदान में शामिल हैं:

- अपेक्षित संस्करणित कैश लक्ष्य पथ (`<versionedDir>/<filename>`),
- पुराने `<versionedDir>` को हटाने और पुनः चलाने का उपचार,
- प्रत्येक अपेक्षित फ़ाइलनाम के लिए प्रत्यक्ष रिलीज़ डाउनलोड `curl` कमांड।

## गैर-संकलित स्टार्टअप विफलताएँ

सामान्य पैकेज/रनटाइम मोड में अंतिम निदान में शामिल हैं:

- पुनर्स्थापना संकेत (`bun install @f5xc-salesdemos/pi-natives`),
...
- `PI_DEV` सेट करने से केवल प्रति-उम्मीदवार कंसोल निदान सक्षम होते हैं (`Loaded native addon...` और लोड त्रुटियाँ)।
