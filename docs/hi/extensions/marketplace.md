...
description: >-
  क्यूरेटेड प्लगइन संग्रह को खोजने, इंस्टॉल करने और प्रबंधित करने के लिए
...
या बस `/marketplace` बिना किसी तर्क के टाइप करें और इंटरैक्टिव प्लगइन ब्राउज़र खोलें।

## अवधारणाएँ

एक **मार्केटप्लेस** एक Git रिपॉज़िटरी (या स्थानीय डायरेक्टरी) होती है जिसमें `.xcsh-plugin/marketplace.json` पर एक कैटलॉग फ़ाइल होती है। कैटलॉग उपलब्ध प्लगइन को उनके स्रोतों, विवरणों और मेटाडेटा के साथ सूचीबद्ध करता है।

एक **प्लगइन** एक ऐसी डायरेक्टरी होती है जिसमें स्किल्स, कमांड, हुक, MCP सर्वर, या LSP सर्वर होते हैं। प्लगइन की पहचान `name@marketplace` (जैसे `code-review@f5xc-salesdemos-marketplace`) से होती है।
...
| कमांड | प्रभाव |
|---|---|
| `/marketplace add <source>` | मार्केटप्लेस स्रोत जोड़ें |
| `/marketplace remove <name>` | मार्केटप्लेस हटाएँ |
...
| `/marketplace install [--force] [--scope user\|project] name@marketplace` | प्लगइन इंस्टॉल करें |
| `/marketplace uninstall [--scope user\|project] name@marketplace` | प्लगइन अनइंस्टॉल करें |
...
| `https://...*.json` | सीधा कैटलॉग URL | `https://example.com/marketplace.json` |
| `https://...*.git` या `git@...` | Git रिपॉज़िटरी | `https://github.com/org/repo.git` |
| `./path` या `~/path` या `/path` | स्थानीय डायरेक्टरी | `./my-marketplace` |

सिस्टम रिपॉज़िटरी क्लोन करता है (या स्थानीय डायरेक्टरी पढ़ता है), `.xcsh-plugin/marketplace.json` का पता लगाता है, उसे मान्य करता है, और कैटलॉग को स्थानीय रूप से कैश करता है।

## कैटलॉग फ़ॉर्मेट (marketplace.json)

एक मार्केटप्लेस कैटलॉग रिपॉज़िटरी रूट में `.xcsh-plugin/marketplace.json` पर स्थित होती है:
...
| `plugins` | प्लगइन प्रविष्टियों की सरणी |

### प्लगइन प्रविष्टि फ़ील्ड

| फ़ील्ड | आवश्यक | विवरण |
|---|---|---|
| `name` | हाँ | प्लगइन नाम (मार्केटप्लेस नाम के समान नियम) |
...
| `homepage` | नहीं | URL |
| `category` | नहीं | कैटेगरी स्ट्रिंग (जैसे `development`, `productivity`, `security`) |
...
~/.xcsh/
  ...
  plugins/
    ...
    cache/
      marketplaces/            # कैश की गई मार्केटप्लेस कैटलॉग
...
मार्केटप्लेस और प्लगइन नाम इस प्रकार होने चाहिए:

- लोअरकेस अक्षर या अंक से शुरू और समाप्त हों
- केवल लोअरकेस अक्षर, अंक, हाइफ़न और डॉट हों
- अधिकतम 64 अक्षर हों
