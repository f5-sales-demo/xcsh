...
title: थीमिंग संदर्भ
description: >-
  TUI थीमिंग संदर्भ जिसमें कलर टोकन, फ़ॉन्ट सेटिंग्स, और थीम कस्टमाइज़ेशन शामिल
  है।
sidebar:
  order: 3
  label: थीमिंग
...
यह दस्तावेज़ बताता है कि coding-agent में थीमिंग आज कैसे काम करती है: स्कीमा, लोडिंग, रनटाइम व्यवहार, और विफलता मोड।
...
- मार्कडाउन स्टाइलिंग एडाप्टर (`getMarkdownTheme()`)
- सिलेक्टर/एडिटर/सेटिंग्स लिस्ट एडाप्टर (`getSelectListTheme()`, `getEditorTheme()`, `getSettingsListTheme()`)
...
- वेरिएबल रेफरेंस स्ट्रिंग (`vars` के माध्यम से रिज़ॉल्व होती है)
- खाली स्ट्रिंग (`""`) जिसका अर्थ टर्मिनल डिफ़ॉल्ट है (`\x1b[39m` fg, `\x1b[49m` bg)
...
### टूल डिफ + सिंटैक्स हाइलाइटिंग (12)
...
- `symbols.preset` थीम-स्तरीय डिफ़ॉल्ट सिंबल सेट सेट करता है।
...
`getAvailableThemes()` मर्ज किए गए बिल्ट-इन + कस्टम नाम लौटाता है, क्रमबद्ध, नाम टकराव पर बिल्ट-इन को प्राथमिकता के साथ।

## लोडिंग, मान्यता, और रिज़ॉल्यूशन
...
4. `vars` रेफरेंस को पुनरावर्ती रूप से रिज़ॉल्व करें
5. टर्मिनल क्षमता मोड द्वारा रिज़ॉल्व किए गए वैल्यू को ANSI में रूपांतरित करें

मान्यता व्यवहार:

- गायब आवश्यक कलर टोकन: स्पष्ट समूहीकृत त्रुटि संदेश
- खराब टोकन प्रकार/मान: JSON पथ के साथ मान्यता त्रुटियाँ
- अज्ञात थीम फ़ाइल: `Theme not found: <name>`

वेरिएबल रेफरेंस व्यवहार:

- नेस्टेड रेफरेंस का समर्थन करता है
- गायब वेरिएबल रेफरेंस पर त्रुटि फेंकता है
- सर्कुलर रेफरेंस पर त्रुटि फेंकता है
...
- `TERM` `dumb`, `linux` में, या खाली => 256color
- अन्यथा => truecolor
...
- स्वयं से स्थायी सेटिंग्स **नहीं** बदलता
...
- `toolDiffAdded` HSV-एडजस्ट किया जाता है (हरा नीले की ओर शिफ्ट होता है)
- एडजस्टमेंट केवल तब लागू होता है जब रिज़ॉल्व किया गया मान हेक्स स्ट्रिंग हो

अन्य टोकन अपरिवर्तित रहते हैं।

## थीम सेटिंग्स कहाँ स्थायी होती हैं

थीम-संबंधित सेटिंग्स `Settings` द्वारा वैश्विक कॉन्फ़िग YAML में स्थायी की जाती हैं:
...
लीगेसी माइग्रेशन मौजूद है: पुराना फ्लैट `theme: "name"` ल्यूमिनेंस डिटेक्शन के आधार पर नेस्टेड `theme.dark` या `theme.light` में माइग्रेट किया जाता है।
...
न्यूनतम ढाँचा। `colors` में प्रत्येक कुंजी आवश्यक है — रनटाइम वैलिडेटर
(`additionalProperties: false`) गायब कुंजियों और अज्ञात कुंजियों दोनों को अस्वीकार करता है।
शिप किए गए संदर्भ कार्यान्वयन के लिए देखें
...
स्टेटस लाइन में दो समानांतर कलर सिस्टम हैं जो इश्यू #242 में प्रलेखित हैं:
...
- 256-कलर पैलेट इंडिसेस (`statusLine<Segment>Bg` / `statusLine<Segment>Fg`)
  पावरलाइन सेगमेंट फ़िल को संचालित करते हैं। ये ऊपर की हेक्स कुंजियों से स्वतंत्र हैं —
  दोनों को सेट किया जाना चाहिए।
...
4. महत्वपूर्ण सतहों का परीक्षण करें:
   - मार्कडाउन रेंडरिंग
   - टूल ब्लॉक (पेंडिंग/सफलता/त्रुटि)
   - डिफ रेंडरिंग (जोड़ा गया/हटाया गया/संदर्भ)
   - स्टेटस लाइन पठनीयता
   - थिंकिंग लेवल बॉर्डर परिवर्तन
   - bash/python मोड बॉर्डर रंग
5. यदि आपकी थीम ग्लिफ़ चौड़ाई/दिखावट पर निर्भर करती है तो दोनों सिंबल प्रीसेट को मान्य करें।

## वास्तविक बाधाएँ और चेतावनियाँ

- कस्टम थीम के लिए सभी `colors` टोकन आवश्यक हैं।
- `export` और `symbols` वैकल्पिक हैं।
- थीम JSON में `$schema` सूचनात्मक है; रनटाइम मान्यता कोड में संकलित TypeBox स्कीमा द्वारा लागू की जाती है।
- `setTheme` विफलता `dark` पर फ़ॉलबैक करती है; `previewTheme` विफलता वर्तमान थीम को प्रतिस्थापित नहीं करती।
- फ़ाइल वॉचर रीलोड त्रुटियाँ सफल रीलोड या फ़ॉलबैक पथ ट्रिगर होने तक वर्तमान लोड की गई थीम को बनाए रखती हैं।
