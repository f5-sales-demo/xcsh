...
यह दस्तावेज़ वर्णन करता है कि coding-agent में थीमिंग आज कैसे काम करती है: स्कीमा, लोडिंग, रनटाइम व्यवहार, और विफलता मोड।

## थीम सिस्टम क्या नियंत्रित करता है

थीम सिस्टम निम्नलिखित को संचालित करता है:

- TUI में उपयोग किए जाने वाले foreground/background रंग टोकन
- मार्कडाउन स्टाइलिंग एडेप्टर (`getMarkdownTheme()`)
- सेलेक्टर/एडिटर/सेटिंग्स सूची एडेप्टर (`getSelectListTheme()`, `getEditorTheme()`, `getSettingsListTheme()`)
...
थीम फ़ाइलें JSON ऑब्जेक्ट हैं जो `theme.ts` (`ThemeJsonSchema`) में रनटाइम स्कीमा के विरुद्ध सत्यापित होती हैं और `src/modes/theme/theme-schema.json` द्वारा प्रतिबिंबित होती हैं।

शीर्ष-स्तरीय फ़ील्ड:

- `name` (आवश्यक)
- `colors` (आवश्यक; सभी रंग टोकन आवश्यक)
- `vars` (वैकल्पिक; पुन: प्रयोज्य रंग चर)
- `export` (वैकल्पिक; HTML निर्यात रंग)
- `symbols` (वैकल्पिक)
...
HTML निर्यात थीमिंग हेल्पर्स के लिए उपयोग किया जाता है:

- `export.pageBg`
- `export.cardBg`
- `export.infoBg`

यदि छोड़ दिया जाए, तो निर्यात कोड हल किए गए थीम रंगों से डिफ़ॉल्ट प्राप्त करता है।
...
`getAvailableThemes()` मर्ज किए गए बिल्ट-इन + कस्टम नाम लौटाता है, क्रमबद्ध, नाम टकराव पर बिल्ट-इन को प्राथमिकता दी जाती है।
...
5. हल किए गए मानों को टर्मिनल क्षमता मोड द्वारा ANSI में परिवर्तित करें

सत्यापन व्यवहार:

- गायब आवश्यक रंग टोकन: स्पष्ट समूहीकृत त्रुटि संदेश
...
- गायब चर संदर्भ पर त्रुटि फेंकता है
- परिपत्र संदर्भों पर त्रुटि फेंकता है

## टर्मिनल रंग मोड व्यवहार

रंग मोड पहचान (`detectColorMode`):
...
`main.ts` सेटिंग्स के साथ थीम को इनिशियलाइज़ करता है:
...
ऑटो थीम स्लॉट चयन `COLORFGBG` बैकग्राउंड पहचान का उपयोग करता है:
...
- `toolDiffAdded` HSV-समायोजित किया जाता है (हरा नीले की ओर शिफ्ट)
- समायोजन केवल तब लागू होता है जब हल किया गया मान एक hex स्ट्रिंग हो

अन्य टोकन अपरिवर्तित रहते हैं।

## थीम सेटिंग्स कहाँ स्थायी होती हैं

थीम-संबंधित सेटिंग्स `Settings` द्वारा ग्लोबल कॉन्फ़िग YAML में स्थायी की जाती हैं:

- पथ: `<agentDir>/config.yml`
- डिफ़ॉल्ट एजेंट dir: `~/.xcsh/agent`
...
लेगेसी माइग्रेशन मौजूद है: पुरानी फ्लैट `theme: "name"` को ल्यूमिनेंस पहचान के आधार पर नेस्टेड `theme.dark` या `theme.light` में माइग्रेट किया जाता है।
...
न्यूनतम कंकाल। `colors` में प्रत्येक कुंजी आवश्यक है — रनटाइम सत्यापनकर्ता
...
स्टेटस लाइन में दो समानांतर रंग प्रणालियाँ हैं जो इश्यू #242 में प्रलेखित हैं:
...
- 256-रंग पैलेट इंडेक्स (`statusLine<Segment>Bg` / `statusLine<Segment>Fg`)
  पावरलाइन सेगमेंट फिल को संचालित करते हैं। ये ऊपर दी गई hex कुंजियों से स्वतंत्र हैं —
...
2. सेटिंग्स खोलें और थीम मानों का प्रीव्यू करें (लाइव `previewTheme`)।
...
4. महत्वपूर्ण सतहों का परीक्षण करें:
   ...
   - डिफ रेंडरिंग (added/removed/context)
   - स्टेटस लाइन पठनीयता
   - थिंकिंग स्तर बॉर्डर परिवर्तन
   - bash/python मोड बॉर्डर रंग
5. यदि आपकी थीम ग्लिफ़ चौड़ाई/दिखावट पर निर्भर करती है तो दोनों सिंबल प्रीसेट को सत्यापित करें।
...
- फ़ाइल वॉचर रीलोड त्रुटियाँ वर्तमान लोड की गई थीम को तब तक बनाए रखती हैं जब तक कोई सफल रीलोड या फ़ॉलबैक पथ ट्रिगर नहीं होता।
