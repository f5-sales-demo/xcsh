...
- RPC मोड उपयोगकर्ता ओवरराइड को इनहेरिट करने के बजाय वर्कफ़्लो-बदलने वाली `todo.*`, `task.*`, और `async.*` सेटिंग्स को उनके बिल्ट-इन डिफ़ॉल्ट पर रीसेट करता है।
...
ऑब्जेक्ट के आकार के अतिरिक्त कोई एन्वेलप नहीं है।
...
- यदि प्रदान किया गया हो, तो सामान्य कमांड प्रतिक्रियाएँ उसी `id` को वापस भेजती हैं।
...
- `prompt` और `abort_and_prompt` तत्काल सफलता लौटाते हैं, फिर यदि एसिंक प्रॉम्प्ट शेड्यूलिंग विफल होती है तो **उसी** id के साथ बाद में त्रुटि प्रतिक्रिया उत्सर्जित कर सकते हैं।
...
वर्तमान सत्र के लिए इन-मेमोरी टूडू स्थिति को बदलता है और सामान्यीकृत चरण सूची लौटाता है:
...
होस्ट-स्वामित्व वाले टूल के वर्तमान सेट को बदलता है जिसे RPC सर्वर stdio पर वापस कॉल कर सकता है:
...
ये टूल अगली मॉडल कॉल से पहले सक्रिय सत्र टूल रजिस्ट्री में जोड़े जाते हैं। `set_host_tools` को पुनः भेजने पर पिछला होस्ट-स्वामित्व वाला सेट बदल जाता है।

## इवेंट स्ट्रीम स्कीमा

RPC मोड `AgentSession.subscribe(...)` से `AgentSessionEvent` ऑब्जेक्ट को फॉरवर्ड करता है।
...
`message_update` में `assistantMessageEvent` (टेक्स्ट/थिंकिंग/टूलकॉल डेल्टा) में स्ट्रीमिंग डेल्टा शामिल होते हैं।

## प्रॉम्प्ट/कतार समवर्तिता और क्रम

यह सबसे महत्वपूर्ण संचालन व्यवहार है।

### तत्काल पावती बनाम पूर्णता

`prompt` और `abort_and_prompt` **तत्काल पावती दिए जाते हैं**:
...
यदि स्ट्रीमिंग के दौरान छोड़ दिया जाए, तो प्रॉम्प्ट विफल हो जाता है।
...
- `set_steering_mode` / `set_follow_up_mode`
  - `"one-at-a-time"`: प्रति टर्न एक कतारबद्ध संदेश डिक्यू करें
  - `"all"`: पूरी कतार एक साथ डिक्यू करें
- `set_interrupt_mode`
  - `"immediate"`: टूल निष्पादन टूल कॉल के बीच स्टीयरिंग की जाँच करता है; लंबित स्टीयरिंग टर्न में शेष टूल कॉल को रद्द कर सकती है
...
`RpcExtensionUIRequest` (`type: "extension_ui_request"`) विधियाँ:
...
- RPC मोड में स्वचालित सत्र शीर्षक जनरेशन अक्षम है, और `setTitle` UI
  ...
  एक सार्थक टर्मिनल-शीर्षक सतह नहीं होती। केवल UI इवेंट को पुनः सक्षम करने
  के लिए `PI_RPC_EMIT_TITLE=1` सेट करें।
...
यदि किसी डायलॉग में टाइमआउट है, तो RPC मोड टाइमआउट/एबॉर्ट सक्रिय होने पर डिफ़ॉल्ट मान पर हल हो जाता है।

## होस्ट टूल उप-प्रोटोकॉल

RPC होस्ट `set_host_tools` भेजकर एजेंट को कस्टम टूल एक्सपोज कर सकते हैं, फिर उसी ट्रांसपोर्ट पर निष्पादन अनुरोधों को सर्व कर सकते हैं।
...
यदि टूल निष्पादन बाद में रद्द किया जाता है, तो RPC मोड उत्सर्जित करता है:
...
पूर्णता इस प्रकार उपयोग करती है:
...
लौटाई गई सामग्री को टूल त्रुटि के रूप में सतह पर लाने के लिए `host_tool_result` पर `isError: true` सेट करें।

## त्रुटि मॉडल और पुनर्प्राप्ति
...
- अज्ञात `id` वाली एक्सटेंशन UI प्रतिक्रियाएँ अनदेखा की जाती हैं।
...
### 2) स्पष्ट कतार नीति के साथ स्ट्रीमिंग के दौरान प्रॉम्प्ट
...
### 3) कतार व्यवहार का निरीक्षण और ट्यूनिंग
...
- `bun <cliPath> --mode rpc` को स्पॉन करता है
...
- `setCustomTools()` और `host_tool_call` / `host_tool_cancel` के स्वचालित हैंडलिंग के माध्यम से होस्ट-स्वामित्व वाले कस्टम टूल का समर्थन करता है
- हर प्रोटोकॉल कमांड के लिए हेल्पर विधियाँ एक्सपोज **नहीं** करता (उदाहरण के लिए, `set_interrupt_mode` और `set_session_name` प्रोटोकॉल प्रकारों में हैं लेकिन समर्पित विधियों के रूप में रैप नहीं किए गए हैं)
