---
title: सत्र भंडारण और प्रविष्टि मॉडल
description: >-
  Append-only सत्र भंडारण मॉडल जिसमें प्रविष्टि प्रकार, स्थायित्व, और प्रारूपों
  के बीच माइग्रेशन शामिल है।
sidebar:
  order: 1
  label: भंडारण और प्रविष्टि मॉडल
i18n:
  sourceHash: 42fe17549e00
  translator: machine
---

# सत्र भंडारण और प्रविष्टि मॉडल

यह दस्तावेज़ इस बात का प्रामाणिक स्रोत है कि coding-agent सत्रों को कैसे प्रदर्शित, स्थायी, माइग्रेट और रनटाइम पर पुनर्निर्मित किया जाता है।
...
- प्रविष्टि वर्गीकरण और ट्री सिमेंटिक्स (`id`/`parentId` + लीफ पॉइंटर)
- पुरानी या दोषपूर्ण फ़ाइलों को लोड करते समय माइग्रेशन/संगतता व्यवहार
- संदर्भ पुनर्निर्माण (`buildSessionContext`)
- स्थायित्व गारंटी, विफलता व्यवहार, ट्रंकेशन/ब्लॉब एक्सटर्नलाइज़ेशन
- भंडारण अमूर्तन (`FileSessionStorage`, `MemorySessionStorage`) और संबंधित उपयोगिताएँ

इसमें शामिल नहीं है `/tree` UI रेंडरिंग व्यवहार, सिवाय उन सिमेंटिक्स के जो सत्र डेटा को प्रभावित करते हैं।
...
`<cwd-encoded>` कार्य निर्देशिका से प्राप्त किया जाता है, जिसमें अग्रणी स्लैश हटाकर `/`, `\\`, और `:` को `-` से बदल दिया जाता है।
...
ब्रेडक्रम्ब सामग्री दो पंक्तियों की होती है: मूल cwd, फिर सत्र फ़ाइल पथ। `continueRecent()` सबसे हाल के mtime को स्कैन करने से पहले इस टर्मिनल-स्कोप्ड पॉइंटर को प्राथमिकता देता है।

## फ़ाइल प्रारूप

सत्र फ़ाइलें JSONL हैं: प्रति पंक्ति एक JSON ऑब्जेक्ट।

- पंक्ति 1 हमेशा सत्र हेडर होती है (`type: "session"`)।
- शेष पंक्तियाँ `SessionEntry` मान हैं।
- प्रविष्टियाँ रनटाइम पर केवल-जोड़ (append-only) होती हैं; शाखा नेविगेशन मौजूदा प्रविष्टियों को बदलने के बजाय एक पॉइंटर (`leafId`) को स्थानांतरित करता है।
...
- `parentSession` एक अपारदर्शी वंशावली स्ट्रिंग है। वर्तमान कोड प्रवाह के आधार पर या तो एक सत्र id या एक सत्र पथ लिखता है (`fork`, `forkFrom`, `createBranchedSession`, या स्पष्ट `newSession({ parentSession })`)। इसे मेटाडेटा के रूप में मानें, टाइप्ड फॉरेन की के रूप में नहीं।
...
`parentId` मूल प्रविष्टि के लिए `null` हो सकता है (पहला जोड़, या `resetLeaf()` के बाद)।

## प्रविष्टि वर्गीकरण

`SessionEntry` निम्नलिखित का संघ (union) है:
...
`role` वैकल्पिक है; संदर्भ पुनर्निर्माण में अनुपस्थित होने पर `default` माना जाता है।
...
यदि मूल (root) से शाखा बनाई जाती है (`branchFromId === null`), तो `fromId` शाब्दिक स्ट्रिंग `"root"` होती है।
...
`label: undefined` `targetId` के लिए एक लेबल हटाता है।
...
हेडर `version` अनुपस्थित होने या `< 2` होने पर लागू होता है:
...
- उपस्थित होने पर कंपैक्शन फ़ील्ड `firstKeptEntryIndex` -> `firstKeptEntryId` माइग्रेट करता है।
- हेडर `version = 2` सेट करता है।

### v2 -> v3

हेडर `version < 3` होने पर लागू होता है:

- `message` प्रविष्टियों के लिए: लीगेसी `message.role === "hookMessage"` को `"custom"` में पुनर्लिखित करता है।
...
- लोडर से `[]` को खाली/अस्तित्वहीन सत्र के रूप में माना जाता है और उस पथ पर एक नई आरंभीकृत सत्र फ़ाइल से बदल दिया जाता है।
- वैध फ़ाइलें लोड की जाती हैं, आवश्यकता होने पर माइग्रेट की जाती हैं, ब्लॉब रेफ हल किए जाते हैं, फिर अनुक्रमित की जाती हैं।

## ट्री और लीफ सिमेंटिक्स

अंतर्निहित मॉडल केवल-जोड़ (append-only) ट्री + परिवर्तनीय लीफ पॉइंटर है:

- प्रत्येक append विधि ठीक एक नई प्रविष्टि बनाती है जिसका `parentId` वर्तमान `leafId` होता है।
...
- `resetLeaf()` `leafId = null` सेट करता है; अगला append एक नई मूल प्रविष्टि बनाता है (`parentId: null`)।
...
1. लीफ निर्धारित करें:
   ...
   - अन्यथा अंतिम प्रविष्टि पर फॉलबैक करें।
2. लीफ से मूल तक `parentId` श्रृंखला पर चलें और मूल->लीफ पथ के लिए उलट दें।
3. पथ के पार रनटाइम स्थिति प्राप्त करें:
   ...
   - सभी `ttsr_injection` प्रविष्टियों से डी-डुप्लिकेटेड `injectedTtsrRules`
...
लेखन एक आंतरिक प्रॉमिस श्रृंखला (`#persistChain`) और `NdjsonFileWriter` के माध्यम से क्रमबद्ध किए जाते हैं।

- `append*` इन-मेमोरी स्थिति को तुरंत अपडेट करता है।
- स्थायित्व तब तक स्थगित रहता है जब तक कम से कम एक सहायक संदेश मौजूद न हो।
  - पहले सहायक से पहले: प्रविष्टियाँ मेमोरी में रखी जाती हैं; कोई फ़ाइल append नहीं होता।
  - जब पहला सहायक मौजूद होता है: पूरा इन-मेमोरी सत्र फ़ाइल में फ्लश किया जाता है।
  - उसके बाद: नई प्रविष्टियाँ क्रमिक रूप से append होती हैं।

कोड में तर्क: ऐसे सत्रों को स्थायी करने से बचना जिन्होंने कभी सहायक प्रतिक्रिया उत्पन्न नहीं की।

### स्थायित्व संचालन

- `flush()` राइटर को फ्लश करता है और `fsync()` कॉल करता है।
- परमाणु पूर्ण पुनर्लेखन (`#rewriteFile`) अस्थायी फ़ाइल में लिखता है, flush+fsync करता है, बंद करता है, फिर लक्ष्य पर rename करता है।
- माइग्रेशन, `setSessionName`, `rewriteEntries`, स्थानांतरण संचालन, और टूल-कॉल arg पुनर्लेखन के लिए उपयोग किया जाता है।
...
## डेटा आकार नियंत्रण और ब्लॉब एक्सटर्नलाइज़ेशन

प्रविष्टियों को स्थायी करने से पहले:

- बड़ी स्ट्रिंग्स को `MAX_PERSIST_CHARS` (500,000 वर्ण) तक ट्रंकेट किया जाता है सूचना के साथ:
  - `"[Session persistence truncated large content]"`
- क्षणिक फ़ील्ड `partialJson` और `jsonlEvents` हटा दिए जाते हैं।
- यदि ऑब्जेक्ट में `content` और `lineCount` दोनों हैं, तो ट्रंकेशन के बाद पंक्ति गणना पुनर्गणना की जाती है।
- `content` सरणियों में base64 लंबाई >= 1024 वाले इमेज ब्लॉक को ब्लॉब रेफ में एक्सटर्नलाइज़ किया जाता है:
  - `blob:sha256:<hash>` के रूप में संग्रहीत
  - कच्चे बाइट्स ब्लॉब स्टोर (`BlobStore.put`) में लिखे जाते हैं

लोड करते समय, message/custom_message इमेज ब्लॉक के लिए ब्लॉब रेफ वापस base64 में हल किए जाते हैं।

## भंडारण अमूर्तन

`SessionStorage` इंटरफ़ेस `SessionManager` द्वारा उपयोग किए जाने वाले सभी फ़ाइलसिस्टम संचालन प्रदान करता है:

- सिंक्रोनस: `ensureDirSync`, `existsSync`, `writeTextSync`, `statSync`, `listFilesSync`
- एसिंक्रोनस: `exists`, `readText`, `readTextPrefix`, `writeText`, `rename`, `unlink`, `openWriter`

कार्यान्वयन:

- `FileSessionStorage`: वास्तविक फ़ाइलसिस्टम (Bun + node fs)
- `MemorySessionStorage`: परीक्षणों/गैर-स्थायी सत्रों के लिए मैप-आधारित इन-मेमोरी कार्यान्वयन

`SessionStorageWriter` `writeLine`, `flush`, `fsync`, `close`, `getError` को उजागर करता है।
...
## संबंधित लेकिन अलग: प्रॉम्प्ट इतिहास भंडारण

`HistoryStorage` (`history-storage.ts`) प्रॉम्प्ट रीकॉल/खोज के लिए एक अलग SQLite उपप्रणाली है, सत्र रीप्ले के लिए नहीं।

- DB: `~/.xcsh/agent/history.db`
- तालिका: `history(id, prompt, created_at, cwd)`
- FTS5 अनुक्रमणिका: ट्रिगर-अनुरक्षित सिंक के साथ `history_fts`
- इन-मेमोरी अंतिम-प्रॉम्प्ट कैश का उपयोग करके लगातार समान प्रॉम्प्ट्स को डी-डुप्लिकेट करता है
- एसिंक्रोनस सम्मिलन (`setImmediate`) ताकि प्रॉम्प्ट कैप्चर टर्न निष्पादन को अवरुद्ध न करे

वार्तालाप ग्राफ/स्थिति रीप्ले के लिए सत्र फ़ाइलों का उपयोग करें; प्रॉम्प्ट इतिहास UX के लिए `HistoryStorage` का उपयोग करें।
