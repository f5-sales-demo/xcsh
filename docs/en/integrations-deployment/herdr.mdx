...
<p id="fidelity-lc-b03b75c1ad3cb4a9" data-fidelity="lc-b03b75c1ad3cb4a9"><strong>Media and graphics protocol negotiation.</strong> When a released Herdr version exposes HERDRKITTYGRAPHICS=1 alongside HERDRENV=1, xcsh enables the Kitty image protocol for TTY output even though Herdr reports TERM=xterm-256color.
Current implementation points include HERDR_KITTY_GRAPHICS=1, HERDR_ENV=1, xcsh.</p>

<p id="fidelity-lc-1e355a06dd861533" data-fidelity="lc-1e355a06dd861533"><strong>Launching conversation-owned workspaces.</strong> Launch a conversation-owned workspace from outside Herdr. The launcher executes the following sequence. Current implementation points include 0600, HerdrBindingV1, xcsh.</p>

<p id="fidelity-lc-b1f433b68395d4c9" data-fidelity="lc-b1f433b68395d4c9"><strong>Terminal management actions.</strong> Within a conversation, users invoke /terminal and agents invoke herdrterminal. Current implementation points include /terminal, herdr_terminal, list.</p>

<p id="fidelity-lc-fc126b63197fd31b" data-fidelity="lc-fc126b63197fd31b"><strong>Security and isolation.</strong> Ownership isolation is enforced through a randomly generated xcshowner metadata token attached to the workspace and each managed pane. Current implementation points include
xcsh_owner, xcsh.</p>

<p id="fidelity-lc-f4497b80fc1b75ee" data-fidelity="lc-f4497b80fc1b75ee"><strong>Socket protocol and resilience.</strong> The socket client communicates using Herdr protocol 18 over newline-delimited JSON across HERDRSOCKETPATH. Current implementation points include HERDR_SOCKET_PATH, ping.</p>
