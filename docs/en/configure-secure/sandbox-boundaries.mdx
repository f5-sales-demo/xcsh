...
Start with `--no-tools` or an explicit `--tools` allow-list when a run needs only conversation. Disable MCP and extensions separately because they can register additional capabilities. Review custom tool and hook code with the same care as any locally executed module.
...
## Separate local and remote authority
