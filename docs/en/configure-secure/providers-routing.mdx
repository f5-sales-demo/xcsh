...
Generated LiteLLM configuration shows seven current internal models: GPT-6 Luna, GPT-5.6 Terra,
GPT-6 Sol, GPT-6 Astra, Claude Haiku 4.5, Claude Sonnet 5, and Claude Opus 5.5. Older GPT-5.6
Luna/Sol and Claude Opus 5 selectors remain resolvable for historical sessions but are hidden from
ordinary browsing.
...
The four current internal OpenAI models use a 272,000-token effective context window by default.
Enable **LiteLLM Maximum Context** under **Providers** to use their verified 1,050,000-token window
with up to 128,000 output tokens. This setting is independent from **OpenAI Codex Maximum Context**,
which applies only to ChatGPT subscription routing. Claude Haiku 4.5 retains its 200,000-token context
and 64,000-token output limits; Sonnet 5 and Opus 5.5 retain 1,000,000-token context and 128,000-token
output limits.
