...
Active-work hints must display the configured key, not hard-code Escape. Cooperative actions receive
...
| `/plan` | Explicit enable/pause choices use session-qualified Cancel-first review. Typed prompts preserve active planning and recover pending saves before queueing. Approval begins on Stay, then uses one comprehensive Cancel-first planning-session → execution-session review with fresh plan/destination validation, veto-before-mutation, same-identity persistence retry, and backed prompt outcome. | Focused lifecycle regressions and directly judged 16-variant actual-terminal enable/pause/typed-prompt/progress/approval matrix pass; external-editor/failure states and complete-candidate gate pending |
| `/model` | Provider/model browsing uses stable tabs, editable global search, exact identity, explicit conversation/default/role scope and reasoning steps. All mutations share a Cancel-first target/change/consequence review, fresh catalog resolution, persistence-before-success and exact no-op handling; `/models` is the same path. | Focused race/rollback/session tests and a directly judged 16-variant actual-terminal matrix with reopened session/default/role state pass; complete-candidate gate pending |
| `/fast` | Argument-free invocation offers Cancel/Enable/Disable. Typed `on`, `off`, and retained `toggle` compatibility use the same cancel-first session-scoped review; `status` is read-only. Session history is flushed before success; cancellation, status, and unchanged requests do not write. | Focused lifecycle regressions and directly judged 16-variant actual-terminal choice/review/cancel/enable/disable/status/no-op matrix pass; complete-candidate gate pending |
...
| `/browser` | Explicit Cancel/headless/visible choices and typed aliases share a Cancel-first saved-default/effective-state review. Status/no-op are read-only; save precedes optional reset, partial reset failure is truthful, and retry repeats only unresolved work. | Focused lifecycle regressions and directly judged 16-variant actual-terminal settings persistence matrix pass; complete-candidate gate pending |
| `/chrome` | Status probes the selected loopback endpoint. Relaunch review identifies endpoint, executable, allowed profiles, authentication uncertainty and restart impact; every effect revalidates the target, failures remain unresolved, and changed recovery state requires renewed review. | Focused race/privacy regressions and directly judged 16-variant disposable-Chrome attach/failure/recovery matrix pass; complete-candidate gate pending |
| `/copy` | Typed and selector copies share Cancel-first review, fresh source resolution and delivery feedback. The selector uses shared framed search, identity-qualified targets, labelled copy/open actions, stable history loading and overflow-only details paging; preserve parent context on cancellation. | Adapter, selector migration, focused regressions, deterministic matrix, and 16-variant actual-terminal visual/readback/persistence matrix pass; complete-candidate gate pending |
...
| `/tools` | The active-model tool inventory now uses bounded responsive Markdown and paging for complete long descriptions. Availability, enabled state, provenance, and configuration requirements must remain explicit as remaining inventory surfaces converge. | Shared-report regressions and a directly judged 16-variant actual-terminal matrix pass; provenance convergence and complete-candidate gate pending |
...
| `/login` | Configured providers and the full catalog use framed identity-qualified browse/search with retained parent context. Typed and menu setup share masked Cancel-first credential/configuration review, distinguish saved configuration, authentication, validation and catalog readiness, and persist only after successful review. | Focused lifecycle/rollback tests and a directly judged 16-variant actual-terminal loopback-provider matrix pass; other connection families and complete-candidate gate pending |
...
| `/compact` | Default and typed instructions share a session-qualified Cancel-first review with stale-state renewal. Progress keeps Escape navigation-only and uses Ctrl+C for acknowledged interruption; completed model output survives persistence failure for save-only retry. Too-small/already-compacted requests are no-write outcomes. | Controller regressions plus a directly judged 16-variant actual-terminal/local-provider matrix pass; complete-candidate gate pending |
...
| `/background` | Transfer already-running work without review: identify stable session/work, state non-cancellation, stop UI activity, and detach standard streams before POSIX suspension. `bg` continues headlessly, `fg` waits, session reopen returns, and `/jobs` inspects async jobs. Duplicate completion tracking is rejected and queued work is allowed to settle. | Focused lifecycle tests and a directly judged 16-variant actual-zsh/local-provider persistence matrix pass; complete-candidate gate pending |
| `/debug` | Diagnostic selector: concise choices and safe export boundaries; shared cancel default applies. | P2 / partial |
| `/memory` | Read-only view uses a pageable shared report with enabled/source context and distinct populated, empty, and unavailable states. Clear/reset disclose all-project rows versus current-project files in a cancel-first stale-checked review; enqueue/rebuild review project/database identity and report only a persisted request, never worker completion. | Focused race/rollback/storage regressions and directly judged 16-variant actual-terminal view/clear/reset/enqueue/rebuild matrix pass; complete-candidate gate pending |
...
| `/exit` | Clean sessions exit immediately. Outstanding responses, compaction, handoff, shell/Python work, queued prompts, and async jobs open one session-qualified Cancel-first review; confirmation interrupts supported work, waits for all settlement, flushes state, clears transient UI, and then returns the shell. | Focused lifecycle tests and a directly judged 16-variant actual-terminal/persistence matrix pass; complete-candidate gate pending |
| `/plugin` | `/plugin`, `/plugins`, `/marketplace`, typed forms and registered subcommands share one framed single-column dashboard and reviewed-action lifecycle. Browse/search/details/refresh are read-only; scoped install/reinstall/upgrade/setup/enable/disable/remove and marketplace writes use exact-version Cancel-first review, immediate catalog/target revalidation, atomic pre-commit catalog validation, duplicate/stale guards, explicit partial/offline outcomes and retry-only-unresolved. `--force` remains replacement semantics, never consent. | Focused lifecycle/race/parser/persistence tests and a directly judged 16-variant actual-terminal install→disable→upgrade→uninstall/reopen matrix pass; complete-candidate gate pending |
...
| `/route` | Strict status/mode/profile paths distinguish saved user policy, project/runtime overrides, effective policy, active model/tier, shadow state and manual pins. Mode and exact-role profile mutations use stale-checked Cancel-first reviews, bounded non-cancellable preparation/progress, no-write equality, unresolved-save retry, and unavailable-catalog recovery. | Focused persistence/race tests plus a directly judged 16-variant actual-terminal matrix pass; complete-candidate gate pending |
...
The next full visual passes are settings/extensions, then session/tree/agents, then connection
wizards. Review consequential publication, deletion, and resource-operation feedback alongside
those passes. Keep simple text reports simple; enrich them only where inspection, recovery, or
decision-making benefits from interaction.
