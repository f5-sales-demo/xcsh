...
An upstream commit is evidence of intent, not a patch that is safe to apply unchanged. Freeze the
reviewed upstream commit, identify the xcsh modules that implement the same contract, port the
behavior with a regression test, and record deliberate differences. A candidate remains excluded
until its prerequisites and product fit are reviewed; omission from one batch does not imply
acceptance or rejection in a later audit.
