...
Select context and namespace before any live read or write, then issue a read-only command for an
expected resource and inspect the returned tenant scope. Context files contain connection metadata
and may reference credentials; keep them out of repositories and transcripts. Changing the active
scope affects later commands only. After a temporary test, restore the prior context and namespace
