...
Before installing this cask on a Mac that received an earlier xcsh package, uninstall both old Homebrew entries (`brew uninstall --formula xcsh` and `brew uninstall --cask xcsh`). Then have MDM forget the `com.f5.xcsh` receipt and remove only the package-owned `/usr/local/bin/xcsh` and `/Library/Application Support/xcsh` payload. Do not remove `~/.xcsh`. The signed and stapled `.pkg` release assets remain available only for direct MDM deployment.
</Aside>
</TabItem>
