...
head:
  - tag: style
    content: |
      .landing-hero { display: grid; gap: clamp(2rem, 5vw, 5rem); grid-template-columns: minmax(0, 1fr) minmax(18rem, 35rem); align-items: center; margin-block: 2rem 4rem; }
      .landing-hero__content, .landing-hero__image, .landing-hero .expressive-code, .landing-command { min-width: 0; }
      h1#_top { max-width: 13ch; text-wrap: balance; }
      .landing-hero__lede { max-width: 42rem; color: var(--sl-color-gray-2); font-size: var(--sl-text-lg); }
      .landing-hero__install { max-width: 46rem; margin-top: 2rem; padding: 1.25rem; border: 1px solid var(--sl-color-gray-5); border-radius: .75rem; background: var(--sl-color-bg-nav); }
      .landing-hero__install h2 { margin: 0; font-size: var(--sl-text-lg); }
      .landing-hero__install p { margin-block: .5rem 0; }
      .landing-hero__install pre { max-width: 100%; overflow-x: auto; }
      ...
      :is(.landing-hero, .landing-grid, .landing-disclosure) :is(a, button, summary):focus-visible { outline: 2px solid var(--sl-color-accent-high); outline-offset: 3px; }
      @media (max-width: 50rem) { .landing-hero { grid-template-columns: minmax(0, 1fr); margin-block: 1rem 3rem; } .landing-hero__install { padding: 1rem; } .landing-grid { grid-template-columns: minmax(0, 1fr); } }
---
...
<section class="landing-hero" aria-labelledby="_top">
  <div class="landing-hero__content">
    ...
    <section class="landing-hero__install" aria-labelledby="install-xcsh">
...
## Install xcsh
      ...
      <p>It installs the latest prebuilt release into <code>~/.local/bin</code>. Need a managed or platform-specific channel? See <a href="#installation-options">installation options</a>, then <a href="#verification">verify the executable</a>.</p>
    </section>
  </div>
...
<div class="landing-grid landing-capabilities">
  <Card title="Agentic terminal work" icon="terminal">
    Run interactive or print-mode prompts, inspect files, use tools, and keep durable session history when the task needs it. [Learn the agent loop](./use-xcsh/agent-loop/).
  </Card>
  <Card title="Reviewable F5 Distributed Cloud workflows" icon="document">
    Discover schemas, validate manifests, and inspect proposed differences before an authorized resource change. [Use resource workflows](./f5-distributed-cloud/).
  </Card>
  <Card title="Extensible automation" icon="puzzle">
    Add headless workflows, custom tools, skills, plugins, or MCP servers when built-in commands are not enough. [Automate and extend](./automate-extend/).
  </Card>
</div>
...
Every installation channel converges on the same check: confirm which executable your shell will run.
...
For a POSIX shell, resolve the path, expose duplicates, and print the active version:
...
In PowerShell, inspect the resolved command and version:
...
Remove or reorder an older executable before switching channels. Once the executable is unambiguous, a provider credential is the next dependency.

<span id="authentication" aria-hidden="true"></span>

## Authenticate a provider

Keep model credentials in the provider-supported environment or subscription login, never in a prompt or committed settings file. Follow [model authentication](./getting-started/model-authentication/) and confirm that the selected provider can enumerate models:
...
Model discovery proves the route before a task uses it. With that result, run one bounded prompt.

## Quickstart

<span id="run-the-quickstart" aria-hidden="true"></span>

Start with a prompt that disables tools and session persistence:
...
Continue with the [quickstart](./getting-started/quickstart/), or choose a [context and namespace](./f5-distributed-cloud/contexts-namespaces/) before an authorized F5 Distributed Cloud change.

The working installation can now take you to the documentation organized around the task at hand.
...
<span data-fidelity-insertion="Explore by goal"></span>
