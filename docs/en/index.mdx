...
head:
  - tag: style
    content: |
      ...
      .landing-hero { display: grid; gap: clamp(2rem, 4vw, 4rem); grid-template-columns: minmax(0, 1fr) minmax(18rem, 35rem); align-items: start; margin-block: .75rem 4rem; }
      ...
      .landing-hero__install { max-width: 46rem; margin-top: 1.5rem; padding: 1.25rem; border: 1px solid var(--sl-color-gray-5); border-radius: .75rem; background: var(--sl-color-bg-nav); }
      .landing-hero__install h2 { margin: 0; font-size: var(--sl-text-lg); }
      .landing-hero__install p { margin-block: .5rem 0; }
      .landing-hero__install pre { max-width: 100%; overflow-x: hidden; }
      .landing-hero__install .expressive-code .ec-line .code { padding-inline-end: 3.5rem; overflow-wrap: anywhere; }
      .landing-hero__install .expressive-code .copy button { opacity: 1; }
      ...
      @media (max-width: 50rem) { .landing-hero { grid-template-columns: minmax(0, 1fr); gap: 2rem; margin-block: .5rem 3rem; } .landing-hero__install { padding: 1rem; } .landing-grid { grid-template-columns: minmax(0, 1fr); } }
---
...
A matching response confirms the installation, provider, and selected model are working together. Next, learn the [agent loop](./use-xcsh/agent-loop/) or choose a [context and namespace](./f5-distributed-cloud/contexts-namespaces/) before an authorized F5 Distributed Cloud change.
