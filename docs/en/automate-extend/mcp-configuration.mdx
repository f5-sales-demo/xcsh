...
credentials, then run `xcsh mcp list`. Add required environment references only after discovery
succeeds. Starting xcsh may launch the configured child process; listing configuration alone must
not turn an invalid entry into an active server. After testing, remove the entry, reload MCP, and
confirm the server and its tools disappear.
