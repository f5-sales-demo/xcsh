...
Este documento explica cómo las definiciones de servidores MCP se convierten en herramientas `mcp_*` invocables en coding-agent, y qué deben esperar los operadores cuando las configuraciones son inválidas, están duplicadas, deshabilitadas o protegidas por autenticación.
...
`src/mcp/types.ts` define la estructura de autoría utilizada por los escritores de configuración MCP y el runtime:

- `stdio` (predeterminado cuando falta `type`): requiere `command`, opcionales `args`, `env`, `cwd`
...
`validateServerConfig()` (`src/mcp/config.ts`) aplica las validaciones básicas de transporte:
...
`config-writer.ts` aplica esta validación para operaciones de adición/actualización y también valida los nombres de servidor:
...
- Si se omite `type`, se asume stdio. Si la intención era HTTP/SSE pero se omitió `type`, `command` se vuelve obligatorio.
- `sse` sigue siendo aceptado pero se trata internamente como transporte HTTP (`createHttpTransport`).
- La validación es estructural, no de accesibilidad: una URL sintácticamente válida puede fallar igualmente al momento de conectar.
...
1. carga los proveedores en orden de prioridad
...
`convertToLegacyConfig()` (`src/mcp/config.ts`) mapea `MCPServer` canónico a `MCPServerConfig` de runtime.
...
`mcp-json.ts` también realiza verificaciones de tipo en tiempo de ejecución para el JSON del usuario y registra advertencias para valores inválidos de `enabled`/`timeout` en lugar de hacer fallar todo el archivo.
...
- un valor que comienza con `!` => ejecuta un comando shell, usa la salida estándar recortada (en caché)
...
Advertencia operativa: esto significa que una clave de secreto/comando/variable de entorno mal escrita puede eliminar silenciosamente esa entrada de encabezado/variable de entorno, produciendo errores 401/403 o fallos en el inicio del servidor.
...
Esto evita muchas colisiones, pero no todas. Diferentes nombres sin procesar pueden sanitizarse al mismo identificador (por ejemplo, `my-server` y `my.server` se sanitizan de forma similar), y la inserción en el registro es "última escritura gana".

### Mapeo de esquemas

`convertSchema()` mantiene el JSON Schema de MCP mayormente tal cual, pero parchea los esquemas de objeto que carecen de `properties` con `{}` para compatibilidad con proveedores.
...
- mapea fallos de transporte/runtime lanzados a `MCP error: ...`
...
- **Modo interactivo/TUI**: `/mcp` proporciona una experiencia dentro de la aplicación (asistente, flujo OAuth, texto de estado de conexión, vinculación en tiempo de ejecución inmediata).
- **Integración SDK/sin interfaz**: `discoverAndLoadMCPTools()` (`src/mcp/loader.ts`) devuelve las herramientas cargadas + errores por servidor; sin experiencia de comando `/mcp`.
...
El JSON de origen con errores en el descubrimiento generalmente se maneja como advertencias/logs; las rutas de config-writer lanzan errores explícitos.
...
3. Use `type` explícito para evitar valores predeterminados stdio accidentales.
