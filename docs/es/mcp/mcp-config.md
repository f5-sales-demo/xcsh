...
description: >-
  Configuración, validación y gestión de servidores MCP para el runtime del
...
- Tipos de configuración del runtime: `packages/coding-agent/src/mcp/types.ts`
...
- `mcpServers` — mapa del nombre del servidor a la configuración del servidor
...
- `auth?: { ... }` — metadatos de autenticación utilizados por OMP para flujos de OAuth/clave API
...
`sse` todavía es soportado por compatibilidad, pero la especificación MCP ahora prefiere Streamable HTTP (`type: "http"`) para servidores nuevos.
...
Use esto cuando OMP deba recordar cómo rehidratar credenciales para un servidor.
...
Use esto cuando el servidor MCP requiera configuración explícita del cliente OAuth.
...
Esto coincide con la imagen Docker local oficial de GitHub `ghcr.io/github/github-mcp-server`.
...
1. Si un valor comienza con `!`, OMP lo ejecuta como un comando de shell y usa la salida estándar recortada.
2. De lo contrario, OMP primero verifica si el valor coincide con el nombre de una variable de entorno.
3. Si esa variable de entorno no está establecida, OMP usa la cadena de forma literal.
...
Eso significa que lo siguiente es válido y conveniente para secretos locales:
...
El cargador de respaldo independiente también expande `${VAR}` y `${VAR:-default}` dentro de cadenas durante el descubrimiento.
...
Si desea el comportamiento menos sorprendente de OMP, prefiera `.xcsh/mcp.json` y use valores explícitos de env/header.
...
- Si pega la configuración de un servidor remoto y olvida `"type": "http"`, OMP lo tratará como `stdio` y se quejará de que falta `command`
...
OMP no fusiona definiciones de servidor duplicadas entre archivos. Los proveedores de descubrimiento tienen prioridad, y la definición con mayor prioridad gana.
...
- use `disabledServers` en la configuración de usuario cuando una configuración de terceros sigue reintroduciendo un servidor que no desea
...
- el token OAuth o de API es válido
