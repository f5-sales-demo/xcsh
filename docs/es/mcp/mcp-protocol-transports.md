...
description: >-
  Implementación del protocolo MCP con capas de transporte stdio, SSE y HTTP con
  streaming.
sidebar:
  order: 2
...
No cubre la experiencia de autoría de extensiones ni la interfaz de usuario de comandos.
...
Las implementaciones de transporte son propietarias del enmarcado y los detalles de E/S:

- `StdioTransport`: JSON delimitado por líneas sobre stdio de subproceso
- `HttpTransport`: JSON-RPC sobre HTTP POST, con respuestas/escucha SSE opcionales

### Advertencia actual importante
...
`client.ts:createTransport()` elige el transporte desde la configuración:
...
- La solicitud saliente es un `POST` HTTP con cuerpo JSON e `id` generado.
- Ruta de respuesta no-SSE: parsea una respuesta JSON-RPC y retorna `result`/lanza en `error`.
- Ruta de respuesta SSE (`Content-Type: text/event-stream`): transmite eventos, retorna el primer mensaje cuyo `id` coincide con el ID de solicitud esperado y tiene `result` o `error`.
- Los mensajes SSE con `method` y sin `id` se tratan como notificaciones.

Si el flujo SSE termina antes de la respuesta coincidente, la solicitud falla con `No response received for request ID ...`.

## Notificaciones

El cliente emite notificaciones JSON-RPC mediante `transport.notify(...)`.

- Stdio: escribe el marco de notificación en stdin (`jsonrpc`, `method`, `params` opcional) más nueva línea.
- HTTP: envía cuerpo POST sin `id`; el éxito acepta `2xx` o `202 Accepted`.

Las notificaciones iniciadas por el servidor solo se exponen a través del `onNotification` del transporte; no existe un suscriptor global predeterminado en manager/client.
...
- `connect()`:
  - genera subproceso con comando/args/env/cwd configurados
  - marca como conectado
  - inicia el bucle de lectura de stdout (`readJsonl`)
  - inicia el bucle de stderr (lectura/descarte; actualmente silencioso)
- `close()`:
...
Si el bucle de lectura termina inesperadamente, `finally` activa `#handleClose()` que realiza el mismo rechazo de solicitudes pendientes y callback de cierre.
...
Si el parser del flujo subyacente lanza una excepción, se invoca `onError` (cuando aún está conectado), luego la conexión se cierra.

## Comportamiento de desconexión/fallo

Cuando el proceso termina o el flujo se cierra:

- todas las solicitudes en vuelo se rechazan con `Transport closed`
- no hay reinicio ni reconexión automática
- las capas superiores deben reconectarse creando un nuevo transporte

## Notas sobre contrapresión/streaming

- Las escrituras salientes usan `stdin.write()` + `flush()` sin esperar la semántica de drenaje.
- No hay gestión explícita de cola ni marca de agua alta en el transporte.
- El procesamiento entrante está dirigido por el flujo (`for await` sobre `readJsonl`), un mensaje parseado a la vez.

## Internos del transporte HTTP/SSE

## Ciclo de vida y semántica de conexión

El transporte HTTP tiene un estado de conexión lógico, pero la ruta de solicitud es sin estado por cada llamada HTTP:

- `connect()` establece `connected=true` (sin handshake de socket/sesión)
- seguimiento opcional de sesión del servidor mediante el encabezado `Mcp-Session-Id`
- `close()` opcionalmente envía `DELETE` con `Mcp-Session-Id`, aborta el listener SSE, emite `onClose`

Por lo tanto, `connected` significa "transporte utilizable", no "flujo persistente establecido".

## Comportamiento del encabezado de sesión

- En la respuesta POST, si el encabezado `Mcp-Session-Id` está presente, el transporte lo almacena.
...
- abort del llamador: se relanza el AbortError original cuando la señal externa ya está abortada
...
- si están presentes, las sugerencias de autenticación de `WWW-Authenticate` y `Mcp-Auth-Server` se agregan

En objeto de error JSON-RPC:

- lanza `MCP error <code>: <message>`

Un cuerpo JSON malformado (fallo de `response.json()`) se propaga como excepción de parseo.
...
1. **Respuesta SSE por solicitud** (`#parseSSEResponse`)
   ...
   - consume el flujo hasta encontrar el id de respuesta coincidente
   - puede procesar notificaciones intercaladas durante el mismo flujo
...
Los errores de parseo JSON de SSE emergen de `readSseJson` y rechazan la solicitud/listener.

- Los errores de parseo SSE de solicitud rechazan la solicitud activa.
- Los errores del listener en segundo plano activan `onError` (excepto AbortError).
- No hay reconexión automática para el listener en segundo plano.

## Utilidad `json-rpc.ts` vs abstracción de transporte

`src/mcp/json-rpc.ts` proporciona helpers `callMCP()` y `parseSSE()` para llamadas MCP HTTP directas (usadas por la integración Exa), no la abstracción `MCPTransport` usada por `MCPClient`/`MCPManager`.

Diferencias notables respecto a `HttpTransport`:

- parsea primero todo el texto de la respuesta, luego extrae la primera línea `data:` (`parseSSE`), con fallback a JSON
- sin gestión de timeout de solicitud, sin API de abort, sin manejo de session-id, sin ciclo de vida de transporte
- retorna el objeto de envolvente JSON-RPC sin procesar
...
- reconectan después de la salida del proceso stdio
- reconectan listeners SSE
- reenvían solicitudes en vuelo después de una desconexión

Fallan rápidamente y propagan errores.

## Nivel de manager/client

`MCPManager` maneja la orquestación de descubrimiento/conexión inicial y puede reconectarse solo ejecutando los flujos de conexión nuevamente (rutas `connectToServer`/`discoverAndConnect`). No repara automáticamente un transporte ya conectado ante callbacks de fallo en tiempo de ejecución.

`MCPManager` sí tiene comportamiento de fallback en el inicio para servidores lentos (herramientas diferidas desde caché), pero eso es fallback de disponibilidad de herramientas, no reintento de transporte.

## Resumen de escenarios de fallo

- **Línea de mensaje stdio malformada**: descartada; el flujo continúa.
- **Flujo/proceso stdio termina**: el transporte se cierra; las solicitudes pendientes se rechazan como `Transport closed`.
...
- **Abort del llamador**: AbortError/razón propagado desde la señal del llamador.

## Regla práctica de límites

Si la preocupación es la forma del mensaje, la correlación de ids o el orden de los métodos MCP, pertenece a la lógica de protocolo/cliente.

Si la preocupación es el enmarcado (JSONL vs HTTP/SSE), el parseo de flujos, el ciclo de vida de fetch/spawn, los relojes de timeout o el cierre de conexión, pertenece a la implementación de transporte.
