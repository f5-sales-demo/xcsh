...
Este documento describe cómo el agente de codificación carga actualmente los modelos, aplica sobrescrituras, resuelve credenciales y elige modelos en tiempo de ejecución.

## Qué controla el comportamiento de los modelos

Archivos de implementación principales:

- `src/config/model-registry.ts` — carga modelos integrados + personalizados, sobrescrituras de proveedores, descubrimiento en tiempo de ejecución, integración de autenticación
...
- Las rutas de configuración explícitas `.json` / `.jsonc` siguen siendo compatibles cuando se pasan programáticamente a `ModelRegistry`.

## Estructura de `models.yml`

```yaml
configVersion: 1  # opcional — escrito por auto-config, usado para detección de migración
providers:
  <provider-id>:
    # configuración a nivel de proveedor
equivalence:
  overrides:
...
`configVersion` es un entero opcional escrito por el sistema de auto-configuración. Cuando está presente, xcsh lo usa para detectar configuraciones desactualizadas y actualizarlas automáticamente.

`provider-id` es la clave canónica del proveedor utilizada en la selección y búsqueda de autenticación.

`equivalence` es opcional y configura la agrupación canónica de modelos sobre los modelos concretos del proveedor:

- `overrides` mapea un selector concreto exacto (`provider/modelId`) a un id canónico oficial del proveedor upstream
...
### Proveedor solo con sobrescrituras (`models` ausente o vacío)
...
## Orden de fusión y sobrescritura

Pipeline de ModelRegistry (al actualizar):

1. Cargar proveedores/modelos integrados desde `@f5xc-salesdemos/pi-ai`.
2. Cargar configuración personalizada de `models.yml`.
3. Aplicar sobrescrituras de proveedor (`baseUrl`, `headers`) a los modelos integrados.
4. Aplicar `modelOverrides` (por proveedor + id de modelo).
5. Fusionar `models` personalizados:
   - mismo `proveedor + id` reemplaza el existente
   - de lo contrario, se añade al final
6. Aplicar modelos descubiertos en tiempo de ejecución (actualmente Ollama y LM Studio), luego reaplicar las sobrescrituras de modelos.

## Equivalencia canónica de modelos y agrupación

El registro mantiene cada modelo concreto de proveedor y luego construye una capa canónica sobre ellos.

Los ids canónicos son solo ids oficiales del upstream, por ejemplo:
...
1. sobrescritura exacta del usuario desde `equivalence.overrides`
2. coincidencias de id oficial incluidas en los metadatos del modelo integrado
...
- los prefijos upstream incrustados pueden eliminarse cuando están presentes, por ejemplo `anthropic/...` u `openai/...`
- las variantes de versión con puntos y guiones solo se normalizan cuando se mapean a un id oficial existente, por ejemplo `4.6 -> 4-6`
- las familias o versiones ambiguas no se fusionan sin una coincidencia incluida o una sobrescritura explícita
...
El estado de sesión y las transcripciones continúan registrando el proveedor/modelo concreto que realmente ejecutó el turno.

Valores predeterminados del proveedor vs sobrescrituras por modelo:

- Los `headers` del proveedor son la base.
- Los `headers` del modelo sobrescriben las claves de encabezado del proveedor.
- `modelOverrides` puede sobrescribir metadatos del modelo (`name`, `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`, `headers`, `compat`, `contextPromotionTarget`).
...
El descubrimiento en tiempo de ejecución llama a `GET /api/tags` en Ollama y sintetiza entradas de modelos con valores predeterminados locales.
...
El descubrimiento en tiempo de ejecución llama a `GET models` en llama.cpp y sintetiza entradas de modelos con valores predeterminados locales.
...
El descubrimiento en tiempo de ejecución obtiene modelos (`GET /models`) y sintetiza entradas de modelos con valores predeterminados locales.
...
- registro de manejadores de flujo personalizados para nuevos IDs de API
...
1. Sobrescritura en tiempo de ejecución (CLI `--api-key`)
...
5. Resolutor de respaldo de ModelRegistry (`apiKey` del proveedor desde `models.yml`, semántica de nombre-de-env-o-literal)
...
- id canónico de modelo exacto
- id de modelo exacto (proveedor inferido)
- coincidencia difusa/por subcadena
- patrones de alcance con comodines en `--models` (p. ej. `openai/*`, `*sonnet*`)
...
1. `provider/modelId` exacto omite la agrupación
2. id canónico exacto se resuelve a través del índice canónico
3. id concreto desnudo exacto sigue funcionando
4. la coincidencia difusa y con comodines se ejecuta después de las rutas exactas
...
2. primer modelo con alcance (si no se está reanudando)
3. proveedor/modelo predeterminado guardado
4. proveedores predeterminados conocidos (p. ej. OpenAI/Anthropic/etc.) entre los modelos disponibles
...
Si un rol apunta a otro rol, el modelo objetivo sigue heredando normalmente y cualquier sufijo explícito en el rol que referencia tiene prioridad para ese uso específico del rol.
...
- `provider/modelId` para fijar una variante concreta de proveedor
- un id canónico como `gpt-5.3-codex` para permitir la agrupación de proveedores
...
- los comodines y coincidencias difusas siguen operando sobre modelos concretos

## `/model` y `--list-models`

Ambas superficies mantienen visibles y seleccionables los modelos con prefijo de proveedor.

Ahora también exponen modelos canónicos/agrupados:

- `/model` incluye una vista canónica junto a las pestañas de proveedor
- `--list-models` imprime una sección canónica más las filas de proveedores concretos
...
La promoción de contexto es un mecanismo de recuperación por desbordamiento para variantes de contexto pequeño (por ejemplo `*-spark`) que promueve automáticamente a un modelo hermano de mayor contexto cuando la API rechaza una solicitud con un error de longitud de contexto.
...
1. Si `contextPromotion.enabled` es true, resolver un objetivo de promoción (ver más abajo).
2. Si se encuentra un objetivo, cambiar a él y reintentar la solicitud — no se necesita compactación.
3. Si no hay objetivo disponible, proceder a la auto-compactación en el modelo actual.

### Selección del objetivo

La selección se basa en el modelo, no en el rol:

1. `currentModel.contextPromotionTarget` (si está configurado)
2. modelo de mayor contexto más pequeño en el mismo proveedor + API
...
- se registra como un `model_change` temporal en el historial de sesión
- no reescribe el mapeo de roles guardado

### Configuración de cadenas de respaldo explícitas
...
Estos son consumidos por la lógica de transporte de OpenAI-completions y combinados con la auto-detección basada en URL.
...
### Sobrescribir ruta de proveedor integrado + metadatos del modelo
...
### Auto-generación en primera ejecución
...
También se genera un `config.yml` predeterminado con configuraciones razonables de proveedor de imágenes.
...
| Configuración saludable | Sin acción |

Todas las reparaciones crean respaldos `.bak` antes de sobrescribir. Todas las operaciones son idempotentes.

### Comando CLI

```bash
xcsh setup litellm              # Generar o reparar configuración de LiteLLM
xcsh setup litellm --check      # Validar sin escribir
xcsh setup litellm --check --json  # Salida de validación legible por máquina
```
...
Si alguna variable no está configurada, la auto-configuración se omite silenciosamente.
...
La mayoría de la configuración de modelos ahora fluye a través de `models.yml` mediante `ModelRegistry`.

Queda una ruta heredada notable: la resolución de autenticación de Anthropic para búsqueda web todavía lee `~/.xcsh/agent/models.json` directamente en `src/web/search/auth.ts`.
...
- Si `LITELLM_BASE_URL` y `LITELLM_API_KEY` están configurados, la verificación de salud al inicio intenta auto-reparar (respaldar archivo corrupto, regenerar desde variables de entorno). Si la reparación tiene éxito, el registro recarga la configuración corregida.
- Si la auto-reparación no es posible (variables de entorno no configuradas, fallo de escritura), el registro sigue operando con los modelos integrados.
- El error se expone mediante `ModelRegistry.getError()` y se muestra en la UI/notificaciones.
