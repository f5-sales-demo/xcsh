...
Este documento describe cómo el coding-agent actualmente carga modelos, aplica sobreescrituras, resuelve credenciales y elige modelos en tiempo de ejecución.
...
- `src/config/model-resolver.ts` — analiza patrones de modelo y selecciona modelos initial/smol/slow
...
6. Aplicar modelos descubiertos en tiempo de ejecución (actualmente Ollama y LM Studio), luego reaplicar sobreescrituras de modelo.

## Equivalencia canónica de modelos y coalescencia

El registro mantiene cada modelo concreto del proveedor y luego construye una capa canónica sobre ellos.

Los ids canónicos son solo ids oficiales del proveedor upstream, por ejemplo:
...
2. coincidencias de id oficial agrupadas desde metadatos de modelos integrados
3. normalización heurística conservadora para variantes de gateway/proveedor
4. respaldo al id propio del modelo concreto

Las heurísticas actuales son intencionalmente estrictas:

- los prefijos upstream incorporados pueden eliminarse cuando están presentes, por ejemplo `anthropic/...` u `openai/...`
...
- las familias o versiones ambiguas no se fusionan sin una coincidencia agrupada o una sobreescritura explícita
...
- `compat` se combina en profundidad para bloques de enrutamiento anidados (`openRouterRouting`, `vercelGatewayRouting`, `extraBody`).
...
### Registro de proveedores por extensión
...
5. Resolvedor de respaldo de ModelRegistry (`apiKey` del proveedor desde `models.yml`, semántica de nombre-de-env-o-literal)
...
- `getAvailable()` filtra a modelos que no requieren clave o tienen autenticación resoluble.
...
- patrones de alcance glob en `--models` (por ejemplo, `openai/*`, `*sonnet*`)
...
2. id canónico exacto resuelve a través del índice canónico
...
4. proveedores predeterminados conocidos (por ejemplo, OpenAI/Anthropic/etc.) entre los modelos disponibles
...
Si un rol apunta a otro rol, el modelo destino sigue heredando normalmente y cualquier sufijo explícito en el rol que hace referencia prevalece para ese uso específico del rol.

Configuraciones relacionadas:

- `modelRoles` (registro)
- `enabledModels` (lista de patrones con alcance)
- `modelProviderOrder` (precedencia global de proveedor canónico)
...
- los globs y coincidencias difusas siguen operando sobre modelos concretos

## `/model` y `--list-models`

Ambas interfaces mantienen visibles y seleccionables los modelos con prefijo de proveedor.

Ahora también exponen modelos canónicos/coalescidos:

- `/model` incluye una vista canónica junto a las pestañas de proveedor
- `--list-models` imprime una sección canónica además de las filas concretas de proveedor
...
La promoción de contexto es un mecanismo de recuperación por desbordamiento para variantes de contexto pequeño (por ejemplo `*-spark`) que promueve automáticamente a un modelo hermano con contexto más grande cuando la API rechaza una solicitud con un error de longitud de contexto.
...
1. Si `contextPromotion.enabled` es verdadero, resolver un objetivo de promoción (ver abajo).
...
2. modelo con contexto más grande y más pequeño en el mismo proveedor + API
...
La promoción utiliza cambio temporal (`setModelTemporary`):
...
### Sobreescribir ruta de proveedor integrado + metadatos del modelo
...
| Variable | Propósito |
|----------|---------|
| `LITELLM_BASE_URL` | URL del proxy LiteLLM (por ejemplo, `https://your-proxy.example.com`). Debe comenzar con `http://` o `https://`. |
...
Si cualquiera de las variables no está configurada, la auto-configuración se omite silenciosamente.
...
Una ruta heredada notable permanece: la resolución de autenticación de Anthropic para búsqueda web aún lee `~/.xcsh/agent/models.json` directamente en `src/web/search/auth.ts`.

Si depende de esa ruta específica, tenga en cuenta la compatibilidad con JSON hasta que ese módulo sea migrado.

## Modo de fallo

Si `models.yml` falla en las verificaciones de esquema o validación:
