...
Este documento describe cómo el agente de codificación carga actualmente los modelos, aplica sobreescrituras, resuelve credenciales y elige modelos en tiempo de ejecución.

## Qué controla el comportamiento de los modelos

Archivos de implementación principales:

- `src/config/model-registry.ts` — carga modelos integrados y personalizados, sobreescrituras de proveedores, descubrimiento en tiempo de ejecución, integración de autenticación
- `src/config/model-resolver.ts` — analiza patrones de modelos y selecciona modelos initial/smol/slow
- `src/config/settings-schema.ts` — configuración relacionada con modelos (`modelRoles`, preferencias de transporte de proveedor)
...
- Si `models.yml` no existe y existe `models.json` en la misma ubicación, se migra a `models.yml`.
- Las rutas de configuración explícitas `.json` / `.jsonc` aún se admiten cuando se pasan de forma programática a `ModelRegistry`.
...
`configVersion` es un entero opcional escrito por el sistema de auto-configuración. Cuando está presente, xcsh lo usa para detectar configuraciones obsoletas y actualizarlas automáticamente.
...
### Proveedor solo de sobreescritura (`models` faltante o vacío)
...
6. Aplicar modelos descubiertos en tiempo de ejecución (actualmente Ollama y LM Studio), luego volver a aplicar las sobreescrituras de modelo.
...
Los ids canónicos son solo ids oficiales upstream, por ejemplo:
...
1. sobreescritura explícita del usuario desde `equivalence.overrides`
...
- los prefijos upstream embebidos pueden eliminarse cuando están presentes, por ejemplo `anthropic/...` o `openai/...`
- las variantes de versión con puntos y guiones pueden normalizarse solo cuando se mapean a un id oficial existente, por ejemplo `4.6 -> 4-6`
...
3. orden del registro/proveedor existente si `modelProviderOrder` no está configurado

Los proveedores deshabilitados o no autenticados se omiten.

El estado de sesión y las transcripciones continúan registrando el proveedor/modelo concreto que ejecutó realmente el turno.

Valores predeterminados del proveedor vs sobreescrituras por modelo:

- Los `headers` del proveedor son la línea base.
- Los `headers` del modelo sobreescriben las claves de encabezado del proveedor.
...
### Descubrimiento explícito de proveedor
...
5. Resolvedor de reserva de ModelRegistry (proveedor `apiKey` de `models.yml`, semántica de nombre-de-entorno-o-literal)

Comportamiento de `apiKey` en `models.yml`:

- El valor se trata primero como un nombre de variable de entorno.
- Si no existe ninguna variable de entorno, la cadena literal se usa como token.

Si `authHeader: true` y el proveedor `apiKey` está configurado, los modelos obtienen:
...
- `getAvailable()` filtra a los modelos que son sin clave o tienen autenticación resoluble.
...
3. el id concreto exacto sin prefijo aún funciona
...
1. proveedor+modelo explícito de CLI
2. primer modelo con alcance (si no se está reanudando)
3. proveedor/modelo predeterminado guardado
4. valores predeterminados de proveedor conocidos (p. ej. OpenAI/Anthropic/etc.) entre los modelos disponibles
5. primer modelo disponible

### Alias de roles y configuración
...
Si un rol apunta a otro rol, el modelo destino sigue heredando normalmente y cualquier sufijo explícito en el rol que hace referencia tiene prioridad para ese uso específico del rol.

Configuración relacionada:

- `modelRoles` (registro)
- `enabledModels` (lista de patrones con alcance)
- `modelProviderOrder` (precedencia canónica-proveedor global)
...
- `provider/modelId` para anclar una variante concreta del proveedor
...
La promoción de contexto es un mecanismo de recuperación por desbordamiento para variantes de contexto pequeño (por ejemplo `*-spark`) que promueve automáticamente a un hermano de contexto mayor cuando la API rechaza una solicitud con un error de longitud de contexto.
...
1. Si `contextPromotion.enabled` es verdadero, resolver un destino de promoción (ver abajo).
2. Si se encuentra un destino, cambiar a él y reintentar la solicitud — no se necesita compactación.
3. Si no hay destino disponible, continuar con la auto-compactación en el modelo actual.

### Selección del destino

La selección está dirigida por el modelo, no por el rol:

1. `currentModel.contextPromotionTarget` (si está configurado)
2. modelo de contexto mayor más pequeño en el mismo proveedor + API
...
Si se cambia desde/hacia `openai-codex-responses`, la clave de estado del proveedor de sesión `openai-codex-responses` se cierra antes del cambio de modelo. Esto descarta el estado de transporte websocket para que el siguiente turno comience limpio en el modelo promovido.

### Comportamiento de persistencia

La promoción usa cambio temporal (`setModelTemporary`):

- registrado como un `model_change` temporal en el historial de sesión
- no reescribe el mapeo de rol guardado
...
### Endpoint compatible con OpenAI local (sin autenticación)
...
### Sobreescribir ruta de proveedor integrada + metadatos del modelo
...
### Auto-generación en el primer uso
...
También se genera un `config.yml` predeterminado con configuraciones de proveedor de imágenes razonables.

### Auto-reparación en el inicio

En cada inicio, `startupHealthCheck()` en el registro de modelos ejecuta las siguientes verificaciones:

| Condición | Acción |
|-----------|--------|
| `models.yml` faltante | Auto-generar desde variables de entorno |
...
| `configVersion` faltante u obsoleto | Respaldar a `.bak`, regenerar con la versión actual |
...
Las configuraciones generadas incluyen un campo `configVersion`. Cuando el formato generado cambie en versiones futuras, xcsh detecta las configuraciones obsoletas y las actualiza automáticamente (con respaldo).

## Advertencia para consumidores heredados

La mayor parte de la configuración de modelos ahora fluye a través de `models.yml` mediante `ModelRegistry`.

Permanece una ruta heredada notable: la resolución de autenticación de Anthropic para búsqueda web aún lee `~/.xcsh/agent/models.json` directamente en `src/web/search/auth.ts`.
...
- Si `LITELLM_BASE_URL` y `LITELLM_API_KEY` están configurados, la verificación de salud en el inicio intenta la auto-reparación (respaldar el archivo corrupto, regenerar desde las variables de entorno). Si la reparación tiene éxito, el registro recarga la configuración corregida.
...
- El error se expone mediante `ModelRegistry.getError()` y se muestra en la interfaz de usuario/notificaciones.
