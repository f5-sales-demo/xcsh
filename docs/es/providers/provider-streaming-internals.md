...
description: >-
  Implementación del streaming de proveedores con análisis de SSE, conteo de
  tokens y manejo de contrapresión.
sidebar:
  order: 2
...
2. Las funciones de stream del proveedor (`anthropic.ts`, `openai-responses.ts`, `google.ts`) traducen los eventos nativos del stream del proveedor a la secuencia unificada de `AssistantMessageEvent`.
...
## Contrato de stream unificado en `@f5xc-salesdemos/pi-ai`

Todos los proveedores emiten la misma estructura (`AssistantMessageEvent` en `packages/ai/src/types.ts`):

- `start`
- tripletas del ciclo de vida de bloques de contenido:
...
- el resultado final se resuelve por evento terminal (`done` o `error`)
- los deltas se agrupan/regulan (~50ms)
- los deltas en buffer se vacían antes de los eventos no-delta y antes de la finalización
...
- los deltas en buffer se combinan solo cuando **tipo + contentIndex** coinciden
...
## Detalles de normalización de proveedores
...
- `content_block_stop` emite el correspondiente `*_end`
...
- `response.completed` mapea estado a razón de parada y uso

Streaming de argumentos de llamada a herramienta:

- mismo patrón de acumulación de `partialJson` que Anthropic
- los proveedores que envían solo `response.function_call_arguments.done` aún populan los args finales
...
- las partes de texto se dividen en pensamiento vs texto por `isThinkingPart(part)`
- las transiciones de bloque cierran el bloque anterior antes de iniciar uno nuevo
- `part.functionCall` se trata como una llamada a herramienta completa (start/delta/end emitidos inmediatamente)
- la razón de finalización se mapea por `mapStopReason()` desde `google-shared.ts`

Streaming de argumentos de llamada a herramienta:

- los args de llamada a función llegan como objeto estructurado, no como texto JSON incremental
...
## Acumulación y recuperación de JSON parcial de llamadas a herramienta
...
2. respaldo al analizador `partial-json` para fragmentos incompletos
...
- deltas válidos posteriores pueden recuperar argumentos estructurados porque el análisis se reintenta en cada adición
- `toolcall_end` final realiza un último intento de análisis antes de la emisión
...
Si el stream del proveedor lanza una excepción o señala fallo, cada wrapper del proveedor captura y emite un evento terminal `error` con:

- `stopReason = "aborted"` cuando la señal de aborto está activa
- de lo contrario `stopReason = "error"`
- `errorMessage = formatErrorMessageWithRetryAfter(error)`

## Comportamiento ante fragmentos malformados / fallos de análisis SSE

Para estas rutas de proveedor, el enmarcado de fragmentos/SSE es manejado por los streams del SDK del vendedor (Anthropic SDK, OpenAI SDK, Google SDK). Este código no implementa un decodificador SSE personalizado aquí.

Comportamiento observado en la implementación actual:

- el análisis de fragmentos/SSE malformados a nivel de SDK se manifiesta como una excepción o evento `error` del stream
- el wrapper del proveedor lo convierte en un evento terminal `error` unificado
- no hay reanudación/reintento específico del proveedor dentro de la propia función de stream
- los reintentos de nivel superior se manejan en la lógica de auto-reintento de `AgentSession` (reintento a nivel de mensaje, no reproducción de fragmentos de stream)

## Límites de cancelación

La cancelación está organizada en capas:

- Solicitud al proveedor de IA: `options.signal` se pasa a la llamada de stream del cliente del proveedor.
- Wrapper del proveedor: después del bucle de stream, la señal de aborto fuerza la ruta de error (`"Request was aborted"`).
...
No hay un mecanismo de contrapresión duro entre el stream del SDK del proveedor y los consumidores posteriores:
...
El diseño actual favorece la capacidad de respuesta y el ordenamiento simple sobre el control de flujo con buffer acotado.
...
- TTSR observa `message_update.assistantMessageEvent` para `text_delta` y `toolcall_delta`
...
- la persistencia escribe los mensajes finalizados en `message_end`
- el auto-reintento examina `stopReason === "error"` del asistente más heurísticas de `errorMessage`
...
- regulación + reglas de combinación de deltas
- modelo de propagación de eventos agente/sesión

Específicas del proveedor (no completamente abstraídas):

- taxonomías de eventos ascendentes y lógica de mapeo
- tablas de traducción de razones de parada
- convenciones de ID de llamada a herramienta
- semántica y firmas de bloques de razonamiento/pensamiento
- semántica de tokens de uso y disponibilidad temporal
- restricciones de conversión de mensajes por API

## Archivos de implementación

- [`../../ai/src/stream.ts`](../../packages/ai/src/stream.ts) — despacho al proveedor, mapeo de opciones, fontanería de clave API/sesión.
- [`../../ai/src/utils/event-stream.ts`](../../packages/ai/src/utils/event-stream.ts) — cola de stream genérica + regulación de deltas del asistente.
...
- [`../../ai/src/providers/openai-responses.ts`](../../packages/ai/src/providers/openai-responses.ts) — traducción de eventos de OpenAI Responses y mapeo de estado.
- [`../../ai/src/providers/google.ts`](../../packages/ai/src/providers/google.ts) — traducción de fragmentos de stream a bloques de Gemini.
