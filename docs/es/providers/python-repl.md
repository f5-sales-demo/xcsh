---
title: Herramienta Python y Runtime IPython
description: >-
...
Este documento describe la pila de ejecución Python actual en `packages/coding-agent`.
...
- Superficie de herramienta: `src/tools/python.ts`
- Orquestación de kernel por sesión/llamada: `src/ipy/executor.ts`
- Protocolo de kernel + integración con gateway: `src/ipy/kernel.ts`
...
```ts
{
  ...
  cwd?: string;
  reset?: boolean; // reiniciar kernel solo antes de la primera celda
}
```

La herramienta es `concurrency = "exclusive"` para una sesión, por lo que las llamadas no se superponen.
...
Cada ejecución utiliza un kernel creado vía `POST /api/kernels` en el gateway seleccionado.
...
6. Cargar módulos de extensión desde:
   ...
   - proyecto: `<cwd>/.xcsh/modules/*.py` (sobrescribe módulos de usuario con el mismo nombre)

Apagado del kernel:

- Elimina el kernel remoto vía `DELETE /api/kernels/:id`
- Cierra el websocket
...
- `session` (por defecto)
  - Reutiliza sesiones de kernel indexadas por identidad de sesión + cwd.
  - La ejecución se serializa por sesión mediante una cola.
  - Las sesiones inactivas se desalojan después de 5 minutos.
  - Máximo 4 sesiones; la más antigua se desaloja por desbordamiento.
  - Las verificaciones de heartbeat detectan kernels muertos.
  - Se permite un reinicio automático; fallos repetidos => error irrecuperable.
...
### Comportamiento multi-celda en una única llamada a la herramienta
...
- La herramienta devuelve un error dirigido indicando qué celda falló.
...
- La lista de permitidos incluye variables fundamentales como `PATH`, `HOME`, variables de localización, `VIRTUAL_ENV`, `PYTHONPATH`, etc.
- Prefijos permitidos: `LC_`, `XDG_`, `PI_`
- La lista de denegados elimina claves de API comunes (OpenAI/Anthropic/Gemini/etc.)

Orden de selección de runtime:
...
`python.toolMode` (por defecto `both`) + la anulación opcional `PI_PY` controlan la exposición:
...
- Se intenta interrumpir el kernel vía REST (`POST /interrupt`) y `interrupt_request` del canal de control.
...
Los resultados de la herramienta pueden incluir metadatos de truncamiento y `artifact://<id>` para recuperación de la salida completa.
...
- Las verificaciones de salud y CRUD de kernel se ejecutan contra el endpoint externo
...
- **Prompts de stdin/input en código Python**
  - `input()` no está soportado interactivamente en esta ruta de runtime; pasar datos de forma programática.
...
- `PI_PY` — anulación de exposición de herramienta (mapeo `bash-only`/`ipy-only`/`both` descrito arriba)
