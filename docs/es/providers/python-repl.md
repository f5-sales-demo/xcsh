---
title: Herramienta Python y entorno de ejecución IPython
description: >-
  Entorno de ejecución de la herramienta Python REPL con gestión del kernel
  IPython, ejecución y captura de salida.
sidebar:
  order: 3
...
# Herramienta Python y entorno de ejecución IPython
...
- Coordinador del gateway local compartido: `src/ipy/gateway-coordinator.ts`
...
- Filtrado de entorno de ejecución y resolución de Python: `src/ipy/runtime.ts`

## Qué es la herramienta Python

La herramienta `python` ejecuta una o más celdas de Python a través de un kernel respaldado por Jupyter Kernel Gateway (no mediante la ejecución directa de `python -c` por cada celda).
...
```ts
{
  ...
  cwd?: string;
  reset?: boolean; // reiniciar el kernel solo antes de la primera celda
}
```
...
`releaseSharedGateway()` actualmente es un no-op (el apagado del kernel no destruye el gateway compartido).

`shutdownSharedGateway()` termina explícitamente el proceso compartido y limpia los metadatos del gateway.

### Restricción importante

`python.sharedGateway=false` se rechaza al iniciar el kernel:
...
- Cierra el websocket
- Llama al hook de liberación del gateway compartido (no-op actualmente)
...
- `session` (por defecto)
  ...
  - Como máximo 4 sesiones; la más antigua se desaloja cuando hay desbordamiento.
  - Las verificaciones de heartbeat detectan kernels muertos.
  - Se permite un reinicio automático; un fallo repetido => fallo definitivo.
...
### Comportamiento multi-celda en una sola llamada a la herramienta

Las celdas se ejecutan secuencialmente en la misma instancia de kernel para esa llamada.
...
## Filtrado de entorno y resolución del runtime
...
- La lista de denegados elimina claves API comunes (OpenAI/Anthropic/Gemini/etc.)
...
`python.toolMode` (por defecto `both`) + la variable opcional `PI_PY` controlan la exposición:
...
- Se intenta la interrupción del kernel mediante REST (`POST /interrupt`) y `interrupt_request` del canal de control.
...
- La ejecución se trata como fallo en la capa del ejecutor
...
- Renderizador de la herramienta (`python.ts`):
  - muestra bloques de código-celda con estado por celda
...
- Renderizador interactivo (`python-execution.ts`):
  - utilizado para la ejecución de Python activada por el usuario en TUI
...
- Las verificaciones de salud y las operaciones CRUD del kernel se ejecutan contra el endpoint externo
- Los fallos de autenticación se muestran con orientación explícita sobre el token

## Resolución de problemas operacionales (modos de fallo actuales)
...
- **Fallos de autenticación/accesibilidad del gateway externo**
  ...
  - timeout/inaccesible -> verificar URL/red y salud del gateway.

- **La ejecución se cuelga y luego expira por timeout**
  - Aumentar el `timeout` de la herramienta (máximo 600s) si la carga de trabajo es legítima.
  - Para código atascado, la cancelación activa la interrupción del kernel pero el código del usuario puede necesitar refactorización.

- **Prompts de stdin/input en código Python**
  - `input()` no está soportado interactivamente en esta ruta de ejecución; pasar datos programáticamente.
...
- `PI_PY` — sobreescritura de exposición de la herramienta (mapeo `bash-only`/`ipy-only`/`both` descrito arriba)
- `PI_PYTHON_GATEWAY_URL` — usar gateway externo
- `PI_PYTHON_GATEWAY_TOKEN` — token de autenticación opcional para gateway externo
- `PI_PYTHON_SKIP_CHECK=1` — omitir verificaciones previas/warm de Python
