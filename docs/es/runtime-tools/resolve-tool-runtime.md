...
# Componentes internos del runtime de la herramienta Resolve
...
- `push(action)` agrega una nueva acción pendiente en la parte superior.
...
`resolve` siempre consume la acción pendiente **más reciente** primero (`pop()`), por lo que múltiples herramientas que producen vistas previas se resuelven en orden inverso al de registro.

## Ejemplo de productor incorporado (`ast_edit`)

`ast_edit` primero muestra una vista previa de los reemplazos estructurales. Cuando la vista previa tiene reemplazos y aún no se ha aplicado, agrega una acción pendiente que contiene:

- label (resumen legible para humanos)
...
## Disponibilidad en runtime y fallos

`pushPendingAction` está conectado por el cargador de herramientas personalizadas utilizando el `PendingActionStore` de la sesión activa.

Si el runtime no tiene un almacén de acciones pendientes, `pushPendingAction` lanza:

- `Pending action store unavailable for custom tools in this runtime.`

## Comportamiento de selección de herramienta

Cuando `PendingActionStore.hasPending` es true, el runtime del agente sesga la selección de herramienta hacia `resolve` para que las vistas previas pendientes se finalicen explícitamente antes de que continúe el flujo normal de herramientas.
...
- Si su herramienta puede preparar múltiples vistas previas, recuerde la semántica LIFO: la última acción agregada se resuelve primero.
