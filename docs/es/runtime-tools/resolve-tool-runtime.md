...
- `action: "discard"` invoca `reject(reason)` si se proporcionó; de lo contrario, descarta la acción con un mensaje predeterminado de "Discarded".
...
`resolve` siempre consume primero la acción pendiente **más reciente** (`pop()`), por lo que múltiples herramientas que producen vistas previas se resuelven en orden inverso al de registro.

## Ejemplo de productor integrado (`ast_edit`)

`ast_edit` primero muestra una vista previa de los reemplazos estructurales. Cuando la vista previa tiene reemplazos y aún no se ha aplicado, inserta una acción pendiente que contiene:
...
Cuando `PendingActionStore.hasPending` es verdadero, el runtime del agente sesga la selección de herramienta hacia `resolve` para que las vistas previas pendientes se finalicen explícitamente antes de que continúe el flujo normal de herramientas.

## Orientación para desarrolladores

- Use acciones pendientes solo para operaciones destructivas o de alto impacto que deban soportar aplicar/descartar explícitamente.
