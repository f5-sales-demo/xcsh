---
title: Entorno de ejecución de la herramienta Bash
description: >-
  Entorno de ejecución de la herramienta Bash con gestión de procesos shell,
...
Existen dos superficies de ejecución bash diferentes en coding-agent:
...
- los valores extraídos son solo valores de respaldo.

### Advertencia

Los comentarios de `bash-normalize.ts` mencionan la eliminación de `2>&1`, pero la implementación actual no lo elimina. El comportamiento en tiempo de ejecución sigue siendo correcto (stdout/stderr ya están fusionados), pero el comportamiento de normalización es más limitado de lo que sugieren los comentarios.
...
`InterceptionResult` incluye `suggestedTool`, pero `BashTool` actualmente solo muestra el texto del mensaje (no hay campo estructurado de herramienta sugerida en `details`).

## 3) Validación de CWD y ajuste de tiempo de espera
...
El tiempo de espera se ajusta al rango `[1, 3600]` segundos y se convierte a milisegundos.
...
- la falla en la asignación de artefactos no es fatal (la ejecución continúa sin archivo de desbordamiento de artefacto),
...
`BashTool` elige la ejecución PTY solo cuando todas las condiciones son verdaderas:
...
Esto significa que el modo de impresión y los contextos RPC/herramientas sin UI siempre usan no-PTY.

## Motor de ejecución no interactivo (`executeBash`)

## Modelo de reutilización de sesión shell
...
- clave opcional de sesión del agente.
...
## Configuración del shell y comportamiento de snapshots
...
- el snapshot captura alias/funciones/opciones del rc del usuario,
- la creación del snapshot es de mejor esfuerzo,
- la falla recurre a no usar snapshot.
...
`Shell.run()` transmite fragmentos al callback. El ejecutor canaliza cada fragmento hacia `OutputSink` y un callback `onChunk` opcional.
...
No se lanza excepción dentro del ejecutor por tiempo de espera/cancelación; devuelve un `BashResult` estructurado y permite al llamador mapear la semántica de error.

## Ruta PTY interactiva (`runInteractiveBashPty`)

Cuando PTY está habilitado, la herramienta ejecuta `runInteractiveBashPty()` que abre un componente de consola superpuesto y controla una `PtySession` nativa.

Aspectos destacados del comportamiento:

- la terminal virtual xterm-headless renderiza el viewport en la superposición,
...
- el redimensionamiento de la terminal se propaga al PTY (`session.resize(cols, rows)`).
...
La salida PTY se normaliza (`CRLF`/`CR` a `LF`, `sanitizeText`) y se escribe en `OutputSink`, incluyendo soporte de desbordamiento a artefactos.
...
## Manejo de salida: transmisión, truncamiento, desbordamiento a artefactos
...
- cuando el umbral de memoria desborda, recorta el búfer en memoria a la cola (seguro en límites UTF-8),
...
- `artifactId` si el archivo de artefacto estuvo activo.

### Advertencia sobre salida larga

El truncamiento en tiempo de ejecución se basa en umbral de bytes en `OutputSink` (50KB por defecto). No impone un límite estricto de 2000 líneas en esta ruta de código.
...
## Modelado de resultados, metadatos y mapeo de errores

Después de la ejecución:

1. Manejo de `cancelled`:
   - si la señal de aborto está abortada -> lanza `ToolAbortError` (semántica de aborto),
   - de lo contrario -> lanza `ToolError` (tratado como falla de herramienta).
...
`BashRenderContext` tiene `isFullOutput`, pero el constructor de contexto de renderizado actual no lo establece para resultados de la herramienta bash. La vista expandida aún usa el texto que ya está en el contenido del resultado (salida cola/truncada) a menos que otro llamador proporcione el contenido completo del artefacto.
...
- muestra advertencias de truncamiento + artefactos cuando los metadatos están presentes,
- marca los estados de cancelado/error/salida por separado.
...
| Llamada a herramienta en modo impresión | `BashTool.execute`                              | No (sin contexto UI)                                                  | Sin superposición TUI; la salida aparece en el flujo de eventos/texto final del asistente | Mismo mapeo de errores de herramienta               |
| Llamada a herramienta RPC (herramientas del agente) | `BashTool.execute`                       | Generalmente sin UI -> no-PTY                                         | Eventos/resultados de herramienta estructurados                               | Mismo mapeo de errores de herramienta               |
...
- La caché de sesiones shell no tiene desalojo explícito en este módulo; el tiempo de vida tiene alcance de proceso.
- Las superficies de tiempo de espera de PTY y no-PTY difieren:
  - PTY expone un campo de resultado `timedOut` explícito,
  - no-PTY mapea el tiempo de espera en un resumen de `cancelled + anotación`.

## Archivos de implementación

- [`src/tools/bash.ts`](../../packages/coding-agent/src/tools/bash.ts) — punto de entrada de la herramienta, normalización/intercepción, selección PTY/no-PTY, mapeo de resultados/errores, renderizador de la herramienta bash.
- [`src/tools/bash-normalize.ts`](../../packages/coding-agent/src/tools/bash-normalize.ts) — normalización de comandos y filtrado head/tail post-ejecución.
...
- [`src/exec/bash-executor.ts`](../../packages/coding-agent/src/exec/bash-executor.ts) — ejecutor no-PTY, reutilización de sesión shell, cableado de cancelación, integración con output sink.
...
- [`src/session/streaming-output.ts`](../../packages/coding-agent/src/session/streaming-output.ts) — truncamiento/desbordamiento a artefactos de `OutputSink` y metadatos de resumen.
- [`src/tools/output-utils.ts`](../../packages/coding-agent/src/tools/output-utils.ts) — ayudantes de asignación de artefactos y búfer de cola para transmisión.
...
- [`src/modes/controllers/command-controller.ts`](../../packages/coding-agent/src/modes/controllers/command-controller.ts) — cableado para flujo de transmisión/actualización de completado del comando `!` interactivo en UI.
