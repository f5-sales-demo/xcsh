...
- **Skill**: paquete estático de orientación/contexto, no código de herramienta ejecutable.
...
1. **Herramientas personalizadas proporcionadas por el SDK** (`options.customTools`)
   - Envueltas en herramientas del agente mediante `CustomToolAdapter` o wrappers de extensiones.
   - Siempre incluidas en el conjunto inicial de herramientas activas durante el bootstrap del SDK.

2. **Módulos descubiertos en el sistema de archivos mediante la API del loader** (`discoverAndLoadCustomTools` / `loadCustomTools`)
...
2. Manifiestos de plugins instalados (`~/.xcsh/plugins/node_modules/*` mediante el loader de plugins)
3. Rutas configuradas explícitas pasadas al loader

### Comportamiento importante

- Las rutas resueltas duplicadas se deduplicar.
- Los conflictos de nombres de herramientas se rechazan contra las herramientas incorporadas y las herramientas personalizadas ya cargadas.
...
El loader comienza con un contexto de UI no-op y requiere que el código del host llame a `setUIContext(...)` cuando la UI real esté lista.
...
- `onUpdate` emite resultados parciales para el streaming de UI.
- `ctx` incluye el estado de sesión/modelo y un helper `abort()`.
- `signal` transporta la cancelación.

`CustomToolAdapter` hace de puente hacia la interfaz de herramientas del agente y reenvía las llamadas en el orden de argumentos correcto.

## Cómo se exponen las herramientas al modelo

- Las herramientas se envuelven en instancias de `AgentTool` (`CustomToolAdapter` o wrappers de extensiones).
...
- Con los wrappers de extensiones, los handlers de `tool_result` pueden reescribir adicionalmente el contenido/detalles e incluso sobrescribir el estado de error.
...
- Proteja el uso de UI con `pi.hasUI`.
