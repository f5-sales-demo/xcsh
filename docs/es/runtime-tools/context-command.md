...
O use el asistente guiado si prefiere indicaciones paso a paso:
...
Cada contexto tiene un namespace por defecto. Cámbielo sin cambiar el contexto:
...
Los contextos pueden llevar variables de entorno adicionales que se inyectan en su sesión al activarse. Útil para configuración por tenant que no forma parte del conjunto de credenciales.
...
1. **Nombres de contexto** -- con indicaciones de URL del tenant, para que pueda distinguir entre tenants
...
Si `F5XC_API_URL` y `F5XC_API_TOKEN` están definidas en el entorno de su shell antes de iniciar xcsh, tienen prioridad sobre cualquier contexto. Esto es útil para pipelines de CI/CD o sesiones puntuales donde no desea crear un contexto persistente.
...
- **Alcance de sesión**: el contexto anterior se reinicia cuando reinicia xcsh. No se persiste en disco.
...
- **La reactivación es una operación nula**: `/context production` cuando ya está en `production` no reinicia el puntero del contexto anterior.
