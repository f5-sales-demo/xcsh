...
# Aspectos internos del runtime de la herramienta notebook
...
La distinción crítica: **`notebook` es un editor de notebooks JSON, no un ejecutor de notebooks**. Edita las fuentes de celdas `.ipynb` directamente; no inicia ni se comunica con un kernel de Python.
...
- sin `execute_request`
- sin fragmentos de stream desde canales del kernel
...
Esa ruta es donde residen los modos del kernel, el comportamiento de reinicio/cancelación, el streaming de fragmentos y el truncamiento de artefactos de salida.
...
- cada línea no final mantiene el `\n` final
- la línea final no tiene salto de línea final forzado

Esto refleja las convenciones del JSON de notebooks y evita la concatenación accidental de líneas en ediciones posteriores.
...
- `delete`
  ...
  - devuelve el `source` eliminado en los detalles para la vista previa del renderer
...
- JSON inválido
- `cells` faltante/no es array
- índice fuera de rango (inserción y no-inserción tienen diferentes rangos válidos)
...
## 3) Semántica de sesión del kernel (donde realmente existe)
...
- `session` (por defecto)
  ...
  - máximo 4 sesiones; la más antigua es desalojada en desbordamiento
...
- estado muerto pre-ejecución activa `restartKernelSession`.
- la ruta de crash en tiempo de ejecución reintenta una vez: reinicia el kernel, re-ejecuta el handler.
- `restartCount > 1` en la misma sesión lanza `Python kernel restarted too many times in this session`.

Comportamiento de reintento en arranque:

- la creación de kernel con gateway compartido reintenta una vez en `SharedGatewayCreateError` con HTTP 5xx.
...
El arranque del kernel recibe un mapa de env opcional desde el executor:
...
- los helpers de preludio que leen contexto de sesión o artefactos dependen de estas variables de entorno en el estado del proceso Python.
...
- `input_request` -> emite texto de advertencia de stdin, envía `input_reply` vacío, marca stdin solicitado
- la completación espera tanto `execute_reply` como `status=idle` del kernel

Cancelación/timeout:

- la señal de abort activa `interrupt()` (REST `/interrupt` + `interrupt_request` del canal de control)
...
- vista de resultado:
  ...
  - las celdas de markdown establecen hint de lenguaje `markdown`; otras celdas no tienen sobreescritura explícita de lenguaje
  - el límite de vista previa colapsada es `PREVIEW_LIMITS.COLLAPSED_LINES * 2`
  - soporta modo expandido vía opciones de renderizado compartidas
  - usa caché de renderizado con clave por ancho + estado expandido

Suposición de renderizado de errores:

- si el primer contenido de texto comienza con `Error:`, el renderer lo formatea como bloque de error de notebook.
...
Este comportamiento del renderer no está relacionado con los resultados de edición JSON de `notebook` excepto que ambos reutilizan primitivas compartidas de TUI.
...
- `python` ejecuta código en un kernel, persiste estado por modo, transmite fragmentos, captura displays enriquecidos, maneja interrupciones/timeouts, y soporta truncamiento de salida/artefactos.
- `notebook` realiza mutaciones determinísticas de JSON de notebook únicamente; sin ejecución, sin estado del kernel, sin stream de fragmentos, sin salidas de display, sin pipeline de artefactos.
