---
title: 'Internos nativos de Shell, PTY, Proceso y Teclas'
description: >-
...
# Internos nativos de Shell, PTY, Proceso y Teclas
...
- **Capa de módulo Rust N-API** (`crates/pi-natives/src/*`): ejecución de shell/PTY de procesos, recorrido/terminación del árbol de procesos y análisis de secuencias de teclas.
...
Ambos transmiten la salida a través de un callback threadsafe y retornan `{ exitCode?, cancelled, timedOut }`.
...
Enriquecimiento de rutas solo para Windows (`shell/windows.rs`): las rutas descubiertas de Git-for-Windows (`cmd`, `bin`, `usr/bin`) se añaden si están presentes y no están ya incluidas.
...
- **Idle/No inicializado**: `session: None`.
- **En ejecución**: la primera llamada a `run()` crea la sesión de forma lazy, almacena el token `current_abort`, ejecuta el comando.
...
- El lector decodifica UTF-8 de forma incremental; las secuencias de bytes inválidas emiten chunks de reemplazo `U+FFFD`.
- Después de la finalización del proceso, el drenaje de salida tiene guardas de inactividad/máximo (`250ms` de inactividad, `2s` máximo) para evitar bloqueos en trabajos en segundo plano que mantienen descriptores abiertos.
...
- Si ocurre la cancelación, los trabajos en segundo plano se terminan (`TERM`, luego `KILL` con retraso) utilizando los metadatos de jobs de brush.

Comportamiento de `Shell.abort()`:

- aborta solo el comando en ejecución actual para esa instancia de `Shell`,
- retorna éxito sin efecto cuando no hay nada en ejecución.

### Comportamiento ante fallos

Los errores comunes expuestos incluyen:

- fallos de inicialización de sesión (`Failed to initialize shell`),
- errores de cwd (`Failed to set cwd`),
- fallos de set/pop de env,
...
- **Idle**: `core: None`.
- **Reservado**: `start()` instala el canal de control de forma síncrona (`core: Some`) antes de que comience el trabajo asíncrono, por lo que `write/resize/kill` se vuelven válidos inmediatamente.
...
- **Finalizado**: `core` siempre se restablece a `None` después de que la tarea de start se complete (éxito o error).

Guarda de concurrencia:

- iniciar mientras ya se está ejecutando retorna `PTY session already running`.

### Patrones de spawn/attach/write/read/terminate

- PTY abierto mediante `portable_pty::native_pty_system().openpty(...)`.
- El comando actualmente se ejecuta como `sh -lc <command>` con `cwd` y sobreescrituras de env opcionales.
...
- un hilo dedicado de lectura lee el stream del master,
- decodificación incremental de UTF-8 con reemplazo `U+FFFD` en bytes inválidos,
- los chunks se reenvían a través del callback threadsafe de N-API.

### Semántica de cancelación y timeout

- `timeoutMs` y `AbortSignal` alimentan un `CancelToken`.
- el bucle llama a `ct.heartbeat()` periódicamente; el aborto dispara la terminación del hijo.
- la clasificación de timeout se basa en cadenas (subcadena `"Timeout"` en el error del heartbeat).
...
- `write/resize/kill` retornan `PTY session is not running`.
...
- una búsqueda sin resultados típicamente produce `[]` de `listDescendants` y `0` de `killTree`.
...
- mapeos directos de byte único (`enter`, `tab`, `ctrl+<letra>`, ASCII imprimible),
...
- Las funciones de coincidencia retornan `false` ante fallo de análisis o no coincidencia.
- No hay superficie de errores lanzados para entrada de teclas malformada.

## Mapeo de API wrapper JS ↔ Exportación Rust

### Shell + PTY + Proceso

| API wrapper TS | Exportación Rust N-API | Notas |
|---|---|---|
...
- **Sesión persistente de Shell**: si una ejecución es cancelada/tiempo agotado/error/flujo de control no-keepalive, Rust descarta explícitamente el estado interno de la sesión. Las ejecuciones normales exitosas mantienen la sesión para reutilización.
- **Sesión PTY**: `core` siempre se limpia después de que `start()` finaliza, incluyendo rutas de fallo.
- **No se expone un contrato explícito de kill dirigido por finalizador JS** por parte de los wrappers; la limpieza está principalmente vinculada a las rutas de completación/cancelación de la ejecución. Los llamadores deben usar `timeoutMs`, `AbortSignal`, `shell.abort()` o `pty.kill()` para un teardown determinístico.
