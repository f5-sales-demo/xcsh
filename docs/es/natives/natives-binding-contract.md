...
Este documento define el contrato del lado TypeScript que se sitúa entre los consumidores de `@f5xc-salesdemos/pi-natives` y el addon N-API cargado.

Se centra en tres piezas:

1. forma del contrato (`NativeBindings` + ampliación de módulo),
...
- Cada `src/<module>/types.ts` amplía `NativeBindings`.
- `src/native.ts` importa todos los archivos `./<module>/types` por sus efectos secundarios para que el contrato fusionado esté en el ámbito donde se utiliza `NativeBindings`.
...
Transición de estado: **Objeto addon no confiable** → **Objeto de binding nativo validado** (o fallo severo).

### 3) Invocación del wrapper

- Los wrappers de módulo en `src/<module>/index.ts` llaman a `native.<export>`.
- Los wrappers adaptan valores por defecto y la forma del callback (`(err, value)` a patrones de callback solo con valor en las APIs de JS).
- `src/index.ts` re-exporta los wrappers/tipos de los módulos como la API pública del paquete.
...
- **Comportamiento de entorno o política alrededor de las llamadas nativas**
  - El wrapper de clipboard añade manejo de OSC52/Termux/headless y trata la copia como mejor esfuerzo.
- **Nombrado público y curación de re-exportaciones**
...
- Sistema/media/conversión: `image`, `html`, `clipboard`, `system-info`, `work`

Regla para mantenedores: si un wrapper no se re-exporta desde `src/index.ts`, no forma parte de la superficie pública prevista del paquete.

## Mapeo API JS ↔ exportación nativa (representativo)

El lado Rust usa nombres de exportación N-API (típicamente de la conversión `#[napi]` snake_case -> camelCase, con alias explícitos ocasionales) que deben coincidir con estas claves de binding.
...
| Clipboard | `copyToClipboard(text)` | `copyToClipboard` | `Promise<void>` (comportamiento del wrapper de mejor esfuerzo) | Sí |
...
## Diferencias del contrato síncrono vs asíncrono
...
- **Exportaciones asíncronas basadas en Promise** para I/O o trabajo de larga duración (`grep`, `glob`, `htmlToMarkdown`, `executeShell`, clipboard, operaciones de imagen).
- **Exportaciones síncronas** para transformaciones/parsers determinísticos en memoria (`search`, `hasMatch`, resaltado de sintaxis, ancho/recorte de texto, parseo de teclas, consultas de procesos).
- **Exportaciones de constructores** para objetos de runtime con estado (`Shell`, `PtySession`, `PhotonImage`).

Implicación para mantenedores: cambiar síncrono ↔ asíncrono en una exportación existente es un cambio de API y contrato incompatible que afecta a wrappers y consumidores.
...
Los consumidores ven miembros de enum con nombre; el límite del binding pasa números.
...
1. **Verificaciones del contrato TypeScript en tiempo de compilación**
   - Los wrappers llaman a `native.<name>` contra el `NativeBindings` fusionado.
   - Claves de binding faltantes/renombradas rompen la verificación de tipos de TS en los wrappers.
...
### Fallos de carga/validación (fallos severos)

- El fallo en la carga del addon o una plataforma no soportada lanza una excepción durante la inicialización del módulo en `native.ts`.
...
- Algunos wrappers suavizan intencionalmente los fallos (`copyToClipboard` es de mejor esfuerzo y absorbe los fallos nativos).
- Los callbacks de streaming ignoran los payloads de error del callback y solo reenvían eventos de valores exitosos.
...
- El tipado `const enum` no previene valores numéricos fuera de rango por parte de consumidores no tipados en tiempo de ejecución.
- `validateNative` solo verifica la presencia/naturaleza de función de las exportaciones requeridas, no la compatibilidad profunda de forma de argumentos/retornos.
- `bindings.ts` incluye `cancelWork(id)` en la interfaz base, pero la lista de validación en tiempo de ejecución actual no impone esa clave.
...
1. `src/<module>/types.ts` (ampliación + tipos del contrato)
...
5. Re-exportaciones públicas en `src/index.ts`

Omitir cualquier paso crea deriva en tiempo de compilación o fallo en tiempo de ejecución durante la carga.
