...
| --- | --- | --- | --- |
| `Native addon missing exports ... Missing: <name>` | Binario `.node` obsoleto, export de Rust incorrecto o binario equivocado | Usar `PI_DEV=1` para ver la ruta cargada; revisar los exports | Recompilar; hacer coincidir el export o alias `#[napi]` con la clave JS; eliminar archivos obsoletos |
...
| El binario compilado falla después de una actualización | Caché obsoleta (`~/.xcsh/natives/<versión-antigua-o-no-coincidente>`) o manifiesto incompatible | Revisar el directorio versionado y los errores del cargador | Eliminar la caché de la versión y re-ejecutar; regenerar el manifiesto embebido |
