...
description: >-
  Guía de compilación, publicación y depuración para el addon nativo de Rust en
...
- **carga del addon en tiempo de ejecución + validación de compuerta** (`src/native.ts`)
...
## Descripción general del pipeline de compilación
...
### 3) Descubrimiento e instalación de artefactos

Después de que Cargo completa, `build-native.ts` escanea directorios de salida candidatos en orden:
...
Cuando lo encuentra, lo instala atómicamente en `packages/natives/native/` con semántica de archivo temporal + renombrado (el respaldo de Windows maneja explícitamente los fallos de reemplazo de DLL bloqueadas).
...
- `PI_NATIVE_VARIANT` (comportamiento del cargador, solo x64): forzar selección de `modern` o `baseline` en tiempo de ejecución
- `PI_COMPILED` (comportamiento del cargador): habilitar comportamiento de candidatos/extracción de binario compilado
...
- `TARGET_PLATFORM`: sobrescribir el nombre de etiqueta de plataforma de salida
- `TARGET_ARCH`: sobrescribir el nombre de arquitectura de salida
...
- `RUSTFLAGS`:
  ...
  - si ya está definido, el script no lo sobrescribe

## Transiciones de estado/ciclo de vida de compilación

### Ciclo de vida de compilación (`build-native.ts`)

1. **Inicialización**: analizar argumentos/entorno (`--dev`, sobrescrituras de destino, flags de compilación cruzada)
2. **Resolución de variante**:
   - no-x64 → sin variante
   - x64 + `TARGET_VARIANT` → variante explícita
   - compilación cruzada x64 sin `TARGET_VARIANT` → error fatal
   - compilación local x64 sin sobrescritura → detectar AVX2 del host
3. **Compilación**: ejecutar Cargo con perfil/destino resuelto
...
6. **Completado**: addon de salida listo para candidatos del cargador

Los fallos con salida ocurren en cualquier etapa con texto de error explícito (variante inválida, fallo en cargo build, biblioteca de salida faltante, fallo de instalación/renombrado).

### Ciclo de vida de embebido (`embed-native.ts`)

1. **Inicialización**: calcular etiqueta de plataforma desde `TARGET_PLATFORM`/`TARGET_ARCH` o valores del host
...
5. **Listo para extracción en tiempo de ejecución** para modo compilado

`--reset` omite la validación y escribe un stub de manifiesto nulo (`embeddedAddon = null`).

## Flujo de trabajo de desarrollo vs comportamiento enviado/compilado
...
3. El cargador en `native.ts` resuelve candidatos del `native/` local del paquete (y respaldo del directorio del ejecutable)
4. `validateNative` impone compatibilidad de exportaciones antes de que los wrappers usen el binding

## Flujo de trabajo de binario enviado/compilado

En modo compilado (`PI_COMPILED` o marcadores embebidos de Bun):

1. El cargador calcula un directorio de caché versionado: `<getNativesDir()>/<packageVersion>` (operativamente `~/.xcsh/natives/<version>`)
...
3. El orden de candidatos en tiempo de ejecución incluye:
   ...
   - directorio heredado de binario compilado (`%LOCALAPPDATA%/xcsh` en Windows, `~/.local/bin` en otros)
...
## Mapeo de API JS ↔ exportaciones Rust (subconjunto de la compuerta de validación)

`native.ts` requiere que estas exportaciones visibles desde JS existan en el addon cargado. Se mapean a exportaciones N-API de Rust en `crates/pi-natives/src`:

| Nombre JS requerido por `validateNative` | Declaración de exportación Rust | Archivo fuente Rust |
| --- | --- | --- |
...
| `getWorkProfile` | `#[napi] pub fn get_work_profile(...)` (exportación en camelCase) | `crates/pi-natives/src/prof.rs` |
...
- Fallo en la compilación de Cargo:
  - el script muestra la salida no-cero y stderr
...
- Exportaciones faltantes:
  - lanza error con nombres exactos de símbolos faltantes y comando de recompilación
...
| --- | --- | --- | --- |
| `Native addon missing exports ... Missing: <name>` | Binario `.node` obsoleto, desajuste en nombre de exportación Rust, o binario incorrecto cargado | Ejecutar con `PI_DEV=1` para ver la ruta cargada; inspeccionar la lista de exportaciones de ese archivo | Recompilar `build`; asegurar que el nombre de exportación Rust `#[napi]` (o alias explícito cuando sea necesario) coincida con la clave JS; eliminar archivos obsoletos en caché/versionados |
| Máquina x64 carga baseline cuando se esperaba modern | `PI_NATIVE_VARIANT=baseline`, AVX2 no detectado, o solo el archivo baseline está presente | Verificar `PI_NATIVE_VARIANT`; inspeccionar `native/` buscando archivo `-modern` | Compilar variante modern (`TARGET_VARIANT=modern ... build`) y asegurar que el archivo se incluya |
| Compilación cruzada produce binario inutilizable/mal etiquetado | Desajuste entre `CROSS_TARGET` y `TARGET_PLATFORM`/`TARGET_ARCH`, o `TARGET_VARIANT` faltante para x64 | Confirmar la tupla de entorno y el nombre de archivo de salida | Re-ejecutar con valores de entorno consistentes y `TARGET_VARIANT` explícito para x64 |
| Binario compilado falla después de actualización | Caché extraída obsoleta (`~/.xcsh/natives/<versión-antigua-o-no-coincidente>`) o desajuste en manifiesto embebido | Inspeccionar directorio de natives versionado y lista de errores del cargador | Eliminar caché de natives versionada para la versión del paquete y re-ejecutar; regenerar manifiesto embebido durante el empaquetado |
| El cargador sondea muchas rutas y ninguna funciona | Desajuste de plataforma o artefacto de release faltante en `native/` del paquete | Verificar `platformTag` vs nombre(s) de archivo reales | Asegurar que el nombre de archivo compilado coincida exactamente con la convención `pi_natives.<platform>-<arch>(-variant).node` y que el paquete incluya `native/` |
| `embed:native` falla con "Incomplete native addons" | Archivos de variante requeridos no compilados antes del embebido | Verificar lista de esperados vs encontrados en el texto del error | Compilar los archivos requeridos primero (x64: tanto modern+baseline; no-x64: por defecto), luego re-ejecutar `embed:native` |

## Comandos operacionales

```bash
# Artefacto de release para el host actual
bun --cwd=packages/natives run build

# Compilación de artefacto con perfil debug
...
# Generar manifiesto de addon embebido desde archivos nativos compilados
bun --cwd=packages/natives run embed:native

# Restablecer manifiesto embebido a stub nulo
