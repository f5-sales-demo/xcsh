...
Migre cuando cualquiera de estas condiciones sea verdadera:
...
Evite migraciones que dependan de estado exclusivo de JS o importaciones dinámicas. Las exportaciones N-API deben ser puras, datos de entrada/datos de salida. El trabajo de larga duración debe pasar por `task::blocking` (limitado por CPU/I/O bloqueante) o `task::future` (I/O asíncrono) con cancelación.
...
- Exporte con `#[napi]`; las exportaciones en snake_case se convierten a camelCase automáticamente. Use `js_name` explícito solo para alias reales/nombres no predeterminados. Use `#[napi(object)]` para structs.
...
- `packages/natives/src/<module>/types.ts` define los tipos TS y extiende `NativeBindings` mediante fusión de declaraciones.
...
- `packages/natives/src/native.ts` carga el addon y `validateNative` aplica las exportaciones requeridas.
...
- Expóngalo con `#[napi]` para que el mapeo predeterminado de snake_case -> camelCase se mantenga consistente.
...
- Añada los tipos y la extensión de `NativeBindings` en `packages/natives/src/<module>/types.ts`.
...
- Incluya una línea base en JS y la versión nativa en la misma ejecución.
- Use `Bun.nanoseconds()` y un número fijo de iteraciones.
...
Si está ejecutando un binario compilado, elimine el directorio de addon en caché:
...
significa que su binario está obsoleto, el nombre de la exportación en Rust (o el alias explícito cuando se usa) no coincide con el nombre en JS, o la exportación nunca se compiló. Corrija la compilación y el desajuste de nombres, no debilite la validación.

### 3) Desajuste en la firma de Rust
...
- `NativeBindings` está extendida en `packages/natives/src/<module>/types.ts` y el wrapper está re-exportado en `packages/natives/src/index.ts`.
- `Object.keys(require(...))` incluye su nueva exportación.
- Los números del benchmark están registrados en el PR/notas.
- El punto de llamada se actualiza **solo si** la versión nativa es más rápida o igual.

## Regla general

- Si la versión nativa es más lenta, **no cambie**. Mantenga la exportación para trabajo futuro, pero la TUI debe quedarse en la ruta más rápida.
