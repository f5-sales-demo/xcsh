...
Este documento es una inmersión profunda en el subsistema de la capa de **primitivas de sistema/medios/conversión** descrita en [`docs/natives-architecture.md`](./natives-architecture.md): `image`, `html`, `clipboard` y perfilado `work`.
...
- **Límite de entrada JS**: `Uint8Array` con bytes de imagen codificados.
...
- Cuando `cleanContent=true`, se habilita el preprocesamiento con `PreprocessingPreset::Aggressive` y banderas de eliminación estricta para navegación/formularios.
...
- **Ruta de lectura de imagen**:
  - Rust lee la imagen en bruto desde `arboard`.
...
- El fallo en la detección de formato/decodificación rechaza la promesa de parse.
- El fallo en la codificación rechaza la promesa de encode.
...
- El fallo del convertidor devuelve una promesa rechazada (`Conversion error: ...`).
...
2. Intenta comando Termux cuando `TERMUX_VERSION` está configurado.
...
1. Sin inicio explícito: el perfilado está siempre activo cuando los helpers de tareas se ejecutan.
2. Cada ámbito de tarea instrumentado registra una muestra al liberar la guarda.
3. Las muestras sobrescriben las entradas más antiguas después de alcanzar la capacidad del buffer.
4. `getWorkProfile(lastSeconds)` lee una ventana de tiempo y genera artefactos plegados/resumen/svg.

Transiciones de fallo:

- El fallo en la generación de SVG es un fallo suave (`svg: null`), mientras que folded y summary aún se devuelven.
...
- La omisión de opciones es un establecimiento de valores predeterminados de mejor esfuerzo, no un fallo.
...
- El truncamiento del buffer es un comportamiento esperado (buffer circular), no un error de pérdida de datos.
