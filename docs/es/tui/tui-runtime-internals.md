---
title: Internos del entorno de ejecución TUI
description: >-
  Internos del entorno de ejecución de la interfaz de terminal que cubren el
  pipeline de renderizado, el manejo de entrada y la gestión de estado.
sidebar:
  order: 2
  label: Internos del entorno de ejecución
i18n:
  sourceHash: cc8f7dcce46a
  translator: machine
---

# Internos del entorno de ejecución TUI

Este documento describe la ruta del entorno de ejecución no relacionada con el tema, desde la entrada del terminal hasta la salida renderizada en modo interactivo. Se centra en el comportamiento en `packages/tui` y su integración desde los controladores de `packages/coding-agent`.

## Capas del entorno de ejecución y propiedad

- **Motor de `packages/tui`**: ciclo de vida del terminal, normalización de stdin, enrutamiento de foco, programación de renderizado, pintado diferencial, composición de overlays, posicionamiento del cursor de hardware.
...
`init()` conecta el árbol en ese orden, establece el foco en el editor, registra manejadores de entrada a través de `InputController`, inicia TUI y solicita un renderizado forzado.

Un renderizado forzado (`requestRender(true)`) restablece las cachés de líneas anteriores y el seguimiento del cursor antes de repintar.

## Ciclo de vida del terminal y normalización de stdin

`ProcessTerminal.start()`:

1. Habilita el modo raw y el pegado con delimitadores (bracketed paste).
2. Adjunta un manejador de redimensionamiento.
3. Crea un `StdinBuffer` para dividir fragmentos parciales de secuencias de escape en secuencias completas.
...
- Detecta el pegado con delimitadores y emite un evento `paste` con el texto pegado en crudo.

Esto evita que fragmentos parciales de secuencias de escape se interpreten erróneamente como pulsaciones de teclas normales.
...
3. Si el componente enfocado pertenece a un overlay que ahora está oculto/invisible, TUI reasigna el foco al siguiente overlay visible o al foco guardado previo al overlay.
...
`setFocus()` también alterna `Focusable.focused`, lo cual controla si los componentes emiten `CURSOR_MARKER` para el posicionamiento del cursor de hardware.

## División del manejo de teclas: editor vs controlador

`CustomEditor` intercepta primero las combinaciones de alta prioridad (escape, ctrl-c/d/z, ctrl-v, variantes de ctrl-p, ctrl-t, alt-arriba, teclas personalizadas de extensiones) y delega el resto al comportamiento base de `Editor` (edición de texto, historial, autocompletado, movimiento del cursor).
...
- cierre en doble `Ctrl+C` o `Ctrl+D` con editor vacío
- suspender/reanudar en `Ctrl+Z`
- comandos con barra diagonal y atajos de selector
- alternar seguimiento/desencolar y alternar expansión

Esto mantiene el análisis de teclas y la mecánica del editor en `packages/tui` y la semántica de modo en los controladores de coding-agent.
...
4. Añade sufijos de restablecimiento de segmento para líneas que no son imágenes.
...
6. Para actualizaciones diferenciales, parchea solo el rango de líneas cambiado y limpia líneas residuales finales cuando sea necesario.
7. Reposiciona el cursor de hardware para soporte de IME.

Las escrituras de renderizado usan modo de salida sincronizada (`CSI ? 2026 h/l`) para reducir el parpadeo/tearing.
...
- La composición de overlays incluye truncamiento defensivo y verificación de ancho posterior a la composición.
...
Los eventos de redimensionamiento se transmiten por eventos desde `ProcessTerminal` a `TUI.requestRender()`.

Efectos:

- Cualquier cambio de ancho dispara un redibujado completo.
- El seguimiento del viewport/parte superior (`#previousViewportTop`, `#maxLinesRendered`) evita cálculos de cursor relativos inválidos cuando el contenido o el tamaño del terminal cambian.
- La visibilidad del overlay puede depender de las dimensiones del terminal (`OverlayOptions.visible`); el foco se corrige cuando los overlays dejan de ser visibles después del redimensionamiento.
...
- `message_end`: finaliza el stream del asistente, maneja anotaciones de abortado/error, marca los argumentos pendientes de herramientas como completos en parada normal.
- `agent_end`: detiene los loaders, limpia el estado transitorio de stream, aplica el cambio de modelo diferido, emite notificación de completitud si está en segundo plano.
...
## Orquestación de estado y loader
...
- `Loader` se actualiza cada 80ms mediante intervalo y solicita renderizado en cada frame.
...
- En las rutas de fin/cancelación, los controladores restauran los manejadores de escape anteriores y detienen/limpian los componentes loader.

## Transiciones de modo y ejecución en segundo plano
...
- `!` -> modo bash
- `$` (prefijo que no es literal de plantilla) -> modo python
...
1. Registra un manejador `SIGCONT` de un solo uso para reiniciar TUI y forzar renderizado.
...
En `agent_end` en segundo plano sin trabajo en cola, el controlador envía notificación de completitud y se cierra.

## Rutas de cancelación

Entradas principales de cancelación:

- `Escape` durante loader de stream activo: restaura mensajes en cola al editor y aborta el agente.
- `Escape` durante ejecución bash/python: aborta el comando en ejecución.
...
- `Ctrl+C` una sola pulsación: limpiar editor; doble pulsación dentro de 500ms: cierre.

La cancelación es condicional al estado; la misma tecla puede significar abortar, salir de modo, disparar selector o no hacer nada dependiendo del estado del entorno de ejecución.

## Comportamiento dirigido por eventos vs limitado por frecuencia
...
- Las actualizaciones de autocompletado del editor (dentro de `Editor`) usan temporizadores de debounce, reduciendo el recálculo excesivo durante la escritura.

El entorno de ejecución por lo tanto mezcla transiciones de estado dirigidas por eventos con una cadencia de renderizado acotada para mantener la interactividad receptiva sin tormentas de repintado.
