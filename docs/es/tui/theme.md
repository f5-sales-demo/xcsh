...
description: >-
  Referencia de temas de la TUI con tokens de color, configuración de fuentes y
...
- preajuste de símbolos + anulaciones de símbolos (`unicode`, `nerd`, `ascii`)
...
- `name` (obligatorio)
- `colors` (obligatorio; todos los tokens de color son obligatorios)
...
- `symbols` (opcional)
  ...
  - `overrides` (opcional: anulaciones clave/valor para `SymbolKey`)
...
## Tokens de color obligatorios (actuales)

Todos los tokens a continuación son obligatorios en `colors`.
...
Utilizada para los helpers de temas de exportación HTML:

- `export.pageBg`
- `export.cardBg`
- `export.infoBg`

Si se omite, el código de exportación deriva valores predeterminados a partir de los colores del tema resueltos.

### Sección `symbols` (opcional)

- `symbols.preset` establece un conjunto de símbolos predeterminado a nivel de tema.
- `symbols.overrides` puede anular valores individuales de `SymbolKey`.

Precedencia en tiempo de ejecución:

1. anulación de `symbolPreset` en la configuración (si está establecida)
2. `symbols.preset` del JSON del tema
3. respaldo `"unicode"`

Las claves de anulación inválidas se ignoran y se registran (`logger.debug`).
...
- anulado por `PI_CODING_AGENT_DIR` (`$PI_CODING_AGENT_DIR/themes`)
...
- tokens de color obligatorios faltantes: mensaje de error agrupado explícito
- tipos/valores de token incorrectos: errores de validación con ruta JSON
...
- `""` -> reseteo de fg/bg predeterminado
...
La selección automática de ranura de tema usa la detección de fondo `COLORFGBG`:

- parsear índice de fondo desde `COLORFGBG`
- `< 8` => ranura oscura (`theme.dark`)
- `>= 8` => ranura clara (`theme.light`)
- fallo de parseo => ranura oscura
...
La interfaz de configuración utiliza esto para vista previa en vivo y restaura el tema anterior al cancelar.
...
- `change` del archivo: intenta recarga (con debounce)
...
Existe migración heredada: el antiguo `theme: "name"` plano se migra a `theme.dark` o `theme.light` anidado basándose en la detección de luminancia.
...
2. Incluir `name`, `vars` opcional, y **todos los tokens de** `colors` **obligatorios**.
3. Opcionalmente incluir `symbols` y `export`.
4. Seleccionar el tema en Configuración (`Display -> Dark theme` o `Display -> Light theme`) dependiendo de qué ranura automática desee.

Esqueleto mínimo. Cada clave en `colors` es obligatoria — el validador en tiempo de ejecución
...
1. Iniciar el modo interactivo (el observador se habilita desde el inicio).
2. Abrir la configuración y previsualizar los valores del tema (`previewTheme` en vivo).
...
4. Ejercitar las superficies críticas:
   ...
   - cambios de borde según nivel de pensamiento
   - colores de borde de modo bash/python
5. Validar ambos preajustes de símbolos si su tema depende del ancho/apariencia de los glifos.

## Restricciones reales y advertencias

- Todos los tokens de `colors` son obligatorios para temas personalizados.
...
- Los errores de recarga del observador de archivos mantienen el tema cargado actualmente hasta que se produzca una recarga exitosa o se active la ruta de respaldo.
