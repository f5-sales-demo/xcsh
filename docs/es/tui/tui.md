...
- **Motor de renderizado (`packages/tui`)**: renderizador diferencial de terminal, despacho de entrada, enfoque, superposiciones, posicionamiento del cursor.
- **Capa de integración (`packages/coding-agent`)**: monta componentes de extensión/herramientas personalizadas, conecta los atajos de teclado/tema y restaura el estado del editor.

## Comportamiento del entorno de ejecución por modo

| Modo | Disponibilidad de `ctx.ui.custom(...)` | Notas |
| --- | --- | --- |
| TUI interactivo | Soportado | El componente se monta en el área del editor, recibe el enfoque y debe llamar a `done(result)` para resolverse. |
| Segundo plano/sin interfaz | No interactivo | El contexto de UI es no-op (`hasUI === false`). |
| Modo RPC | No soportado | `custom()` devuelve `Promise<never>` y no monta componentes TUI. |

Si su extensión/herramienta puede ejecutarse en modo no interactivo, verifique con `ctx.hasUI` / `pi.hasUI`.
...
El comportamiento del cursor utiliza `CURSOR_MARKER` (no `getCursorPosition`). Los componentes enfocados emiten el marcador en el texto renderizado; `TUI` lo extrae y posiciona el cursor de hardware.
...
3. **Trunque/ajuste texto con reconocimiento de ANSI** con `truncateToWidth()` / `wrapTextWithAnsi()`.
4. **Sanee tabulaciones/contenido** de fuentes externas usando `replaceTabs()` (y sanitizadores de nivel superior en las rutas de renderizado de coding-agent).
...
## Manejo de entrada y atajos de teclado

### Coincidencia de teclas en bruto

Use `matchesKey(data, "...")` para teclas de navegación y combinaciones.

### Respete los atajos de teclado configurados por el usuario

Las fábricas de UI de extensiones reciben un `KeybindingsManager` (modo interactivo) para que pueda respetar las acciones mapeadas en lugar de codificar teclas directamente:
...
## Enfoque, superposiciones y cursor
...
## 2) Contexto de UI para hooks/herramientas personalizadas (tipado heredado)

`HookUIContext.custom` está tipado como `(tui, theme, done)` en los tipos de hooks/herramientas personalizadas.
La implementación interactiva subyacente llama a las fábricas con `(tui, theme, keybindings, done)`. Los consumidores JS pueden usar el argumento adicional; la compatibilidad a nivel de tipos aún refleja la firma heredada de 3 argumentos.
...
- `packages/tui/src/tui.ts` — `Component`, `Focusable`, marcador de cursor, enfoque, superposición, despacho de entrada.
- `packages/tui/src/utils.ts` — primitivas de ancho/truncado/saneamiento.
...
- `packages/coding-agent/src/extensibility/hooks/types.ts` — contrato de UI para hooks (firma heredada de custom).
