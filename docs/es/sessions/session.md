...
Este documento es la fuente de verdad sobre cómo las sesiones del agente de programación se representan, persisten, migran y reconstruyen en tiempo de ejecución.
...
El contenido de la referencia consta de dos líneas: el directorio de trabajo original y luego la ruta del archivo de sesión. `continueRecent()` prefiere este puntero con alcance de terminal antes de buscar por el mtime más reciente.
...
- Las líneas restantes son valores de `SessionEntry`.
- Las entradas son de solo adición en tiempo de ejecución; la navegación entre ramas mueve un puntero (`leafId`) en lugar de mutar las entradas existentes.
...
- `parentSession` es una cadena opaca de linaje. El código actual escribe un id de sesión o una ruta de sesión dependiendo del flujo (`fork`, `forkFrom`, `createBranchedSession`, o `newSession({ parentSession })` explícito). Trátelo como metadatos, no como una clave foránea tipada.
...
`parentId` puede ser `null` para una entrada raíz (primer adición, o después de `resetLeaf()`).
...
- Reconstruye una cadena lineal de padres usando el orden del archivo.
...
Se aplica cuando el `version` del encabezado es `< 3`:

- Para entradas `message`: reescribe el legado `message.role === "hookMessage"` a `"custom"`.
- Establece `version = 3` en el encabezado.

### Activación y Persistencia de la Migración

- Las migraciones se ejecutan durante la carga de sesión (`setSessionFile`).
- Si se ejecutó alguna migración, el archivo completo se reescribe en disco inmediatamente.
- La migración muta primero las entradas en memoria y luego persiste el JSONL reescrito.

## Comportamiento de Carga y Compatibilidad

Comportamiento de `loadEntriesFromFile(path)`:

- Archivo no encontrado (`ENOENT`) -> devuelve `[]`.
- Las líneas no parseables son manejadas por el parser JSONL tolerante (`parseJsonlLenient`).
- Si la primera entrada parseada no es un encabezado de sesión válido (`type !== "session"` o falta `id` como cadena) -> devuelve `[]`.
...
- Los archivos válidos se cargan, se migran si es necesario, se resuelven las referencias de blobs y luego se indexan.

## Semántica del Árbol y la Hoja
...
- `branchWithSummary()` establece la hoja en el objetivo de la rama y añade una entrada `branch_summary`.

`getEntries()` devuelve todas las entradas que no son encabezado en orden de inserción. Las entradas existentes no se eliminan en operación normal; las reescrituras preservan el historial lógico mientras actualizan la representación (migraciones, movimiento, ayudantes de reescritura dirigida).
...
1. Determinar la hoja:
   ...
   - en caso contrario, recurrir a la última entrada.
...
4. Construir la lista de mensajes:
   ...
   - Las entradas `custom_message` se convierten en `AgentMessages` de tipo `custom` mediante `createCustomMessage`
   - Las entradas `branch_summary` se convierten en `AgentMessages` de tipo `branchSummary` mediante `createBranchSummaryMessage`
...
Las escrituras se serializan a través de una cadena interna de promesas (`#persistChain`) y `NdjsonFileWriter`.
...
- La persistencia se difiere hasta que exista al menos un mensaje del asistente.
  - Antes del primer asistente: las entradas se retienen en memoria; no se realiza ninguna adición al archivo.
  - Cuando existe el primer asistente: la sesión completa en memoria se vuelca al archivo.
  - Posteriormente: las nuevas entradas se añaden incrementalmente.

Razón en el código: evitar persistir sesiones que nunca produjeron una respuesta del asistente.
...
- Los errores de persistencia se bloquean (`#persistError`) y se relanzarán en operaciones posteriores.
...
- Los bloques de imagen en arrays `content` con longitud base64 >= 1024 se externalizan como referencias de blob:
  - almacenados como `blob:sha256:<hash>`
  - bytes crudos escritos en el almacén de blobs (`BlobStore.put`)
...
La interfaz `SessionStorage` proporciona todas las operaciones del sistema de archivos utilizadas por `SessionManager`:
...
- `getRecentSessions(sessionDir, limit)` -> metadatos ligeros para la interfaz/selector de sesiones
- `findMostRecentSession(sessionDir)` -> la más reciente por mtime
- `list(cwd, sessionDir?)` -> sesiones en el alcance de un proyecto
- `listAll()` -> sesiones de todos los alcances de proyecto bajo `~/.xcsh/agent/sessions`

La extracción de metadatos lee solo un prefijo (`readTextPrefix(..., 4096)`) cuando es posible.

## Relacionado pero Distinto: Almacenamiento del Historial de Prompts
...
- Inserción asíncrona (`setImmediate`) para que la captura del prompt no bloquee la ejecución del turno

Use archivos de sesión para la reproducción del grafo/estado de conversación; use `HistoryStorage` para la experiencia de usuario del historial de prompts.
