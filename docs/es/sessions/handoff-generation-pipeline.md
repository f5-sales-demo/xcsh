...
- Cómo se captura la salida de handoff desde la salida del asistente
...
- Internos genéricos de navegación/ramificación del árbol
...
4. `CommandController.handleHandoffCommand` realiza una verificación previa usando las entradas actuales:
   ...
   - Si son `< 2`, advierte: `Nothing to hand off (no messages yet)` y retorna.
...
### 1) Iniciar la generación de handoff
...
- Construye un prompt fijo e inline que solicita un documento de handoff estructurado (`Goal`, `Constraints & Preferences`, `Progress`, `Key Decisions`, `Critical Context`, `Next Steps`)
...
Al recibir `agent_end`, extrae el texto de handoff del estado del agente escaneando hacia atrás en busca del mensaje `assistant` más reciente, luego concatenando todos los bloques `content` donde `type === "text"` con `\n`.

Suposiciones importantes de la extracción:

- Solo se usan bloques de texto; el contenido que no es texto se ignora.
- Se asume que el mensaje más reciente del asistente corresponde a la generación del handoff.
- No analiza secciones de markdown ni valida el cumplimiento del formato.
...
`newSession()` crea un encabezado nuevo y una lista de entradas vacía (la hoja se reinicia a `null`). En la ruta de handoff, no se pasa `parentSession`.
...
2. `agent.replaceMessages(sessionContext.messages)` hace que el mensaje de handoff inyectado sea contexto activo
...
Durante la generación, la persistencia normal de mensajes permanece activa. La respuesta de handoff del asistente se persiste como una entrada regular `message` en `message_end`.
...
`buildSessionContext()` convierte esta entrada en un mensaje de contexto personalizado/de usuario en tiempo de ejecución mediante `createCustomMessage(...)`, de modo que se incluye en futuros prompts de la nueva sesión.
...
Cuando se usa esta ruta de aborto, el suscriptor de handoff rechaza con `Error("Handoff cancelled")`, y el controlador de comandos lo mapea a la UI de cancelación.

### Limitación de la ruta interactiva de `/handoff`

En el cableado actual del controlador interactivo, `/handoff` no instala un manejador dedicado de Escape que llame a `abortHandoff()` (a diferencia de las rutas de compactación/resumen de rama que temporalmente sobrescriben `editor.onEscape`).
...
- **Fallido**
  - cualquier otro error lanzado desde `handoff()` / pipeline de prompt (errores de validación del modelo/API, excepciones en tiempo de ejecución, etc.)
...
Dos protecciones previenen handoffs de baja señal:

- Capa de UI (`handleHandoffCommand`): advierte y retorna anticipadamente para `< 2` entradas de mensaje
...
7. Si está presente:
   ...
   - reiniciar colas/contadores del runtime
   - agregar `custom_message(handoff)`
   - reconstruir y reemplazar los mensajes activos del agente
8. El controlador reconstruye la UI del chat y anuncia el éxito
9. `#handoffAbortController` limpiado (`isGeneratingHandoff = false`)

## Suposiciones y limitaciones conocidas

- La extracción de handoff es heurística: "últimos bloques de texto del asistente"; sin validación estructural.
- No hay verificación estricta de que el markdown generado siga el formato de secciones solicitado.
- El texto extraído ausente se reporta como cancelación en la UX del controlador.
