...
En la creación de la sesión, `createAgentSession()` carga todas las reglas descubiertas y construye un `TtsrManager`:
...
En `turn_start`, el búfer del flujo se reinicia:
...
- se añade el delta al búfer del manager
...
## 4. Programación de reintentos, modo de contexto e inyección de recordatorios
...
5. se añade un mensaje de usuario sintético que contiene un bloque `<system-interrupt ...>` por regla
...
El modo interactivo utiliza `session.isTtsrAbortPending` para suprimir la visualización de la razón de detención del asistente cancelado como un fallo visible durante la interrupción TTSR, y renderiza un `TtsrNotificationComponent` cuando llega el evento.
...
## 8. Límites de condiciones de carrera y garantías de orden

### Cancelación vs callback de reintento

- la cancelación es sincrónica desde la perspectiva del manejador TTSR (`agent.abort()` se llama inmediatamente)
- el reintento se difiere por temporizador (`50ms`)
...
## 9. Resumen de casos extremos

- Expresión regular `ttsr_trigger` inválida: se omite con advertencia; las demás reglas continúan.
- Nombres de regla duplicados en la capa de capacidad: los duplicados de menor prioridad se sombrean antes del registro.
...
- El repeat-after-gap depende de los incrementos del conteo de turnos en `turn_end`; los fragmentos a mitad de turno no avanzan los contadores de intervalo.
