...
La compactación y los resúmenes de rama son los dos mecanismos que mantienen las sesiones largas utilizables sin perder el contexto del trabajo previo.
...
- `CompactionEntry`
  ...
  - `tokensBefore`
  - opcionales `details`, `preserveData`, `fromExtension`
- `BranchSummaryEntry`
  - `type: "branch_summary"`
  - `fromId`, `summary`
  - opcionales `details`, `fromExtension`
...
### Compactación por desbordamiento-reintento vs compactación por umbral

Las dos rutas automáticas son intencionalmente diferentes:

- **Compactación por desbordamiento-reintento**
  - Disparador: el error del asistente del modelo actual se detecta como desbordamiento de contexto.
  ...
  - En caso de éxito, el agente auto-continúa (`agent.continue()`) después de la compactación.
...
- Requerir al menos `20_000` de ahorro total estimado.
...
3. Adaptar `keepRecentTokens` usando la ratio de uso medida cuando está disponible.
...
Si hay entradas de metadatos que no son mensajes inmediatamente antes del punto de corte (`model_change`, `thinking_level_change`, etiquetas, etc.), se incorporan a la región conservada moviendo el índice de corte hacia atrás hasta que se alcanza un mensaje o un límite de compactación.

### Manejo de turno dividido
...
1. Convertir mensajes mediante `convertToLlm()`.
...
6. Ejecutar el prompt de sumarización con `SUMMARIZATION_SYSTEM_PROMPT`.
...
Modo de sumarización remota:

- Si `compaction.remoteEndpoint` está configurado, la compactación envía un POST con:
  - `{ systemPrompt, prompt }`
...
- Incluye detalles de compactación previa solo cuando la entrada previa es generada por pi (`fromExtension !== true`).
...
El texto del resumen recibe etiquetas de archivos añadidas mediante la plantilla del prompt:
...
## Pipeline de sumarización de rama

La sumarización de rama está vinculada a la navegación del árbol, no al desbordamiento de tokens.
...
3. Si existe un resumen, adjuntarlo en el objetivo de navegación usando `branchWithSummary(...)`.
...
2. Segunda pasada: recorrer desde la más reciente a la más antigua, añadiendo mensajes hasta alcanzar el presupuesto de tokens.
3. Preferir preservar el contexto reciente.
4. Puede incluir entradas de resumen grandes cerca del límite del presupuesto para mantener la continuidad.

Las entradas de compactación se incluyen como mensajes (`compactionSummary`) durante la entrada de sumarización de rama.

### Generación de resumen y persistencia

La sumarización de rama:

1. Convierte y serializa los mensajes seleccionados.
2. Los envuelve en `<conversation>`.
3. Usa instrucciones personalizadas si se proporcionan, de lo contrario `branch-summary.md`.
4. Llama al modelo de sumarización con `SUMMARIZATION_SYSTEM_PROMPT`.
...
- `prompt` (sobrescribir el prompt base de resumen)
- `context` (líneas de contexto extra inyectadas en `<additional-context>`)
- `preserveData` (almacenado en la entrada de compactación)

### `session_compact`

Notificación posterior a la compactación con `compactionEntry` guardada y la bandera `fromExtension`.

### `session_before_tree`

Se ejecuta en la navegación del árbol antes de la generación por defecto del resumen de rama.

Puede:

- cancelar la navegación
- proporcionar un `{ summary: { summary, details } }` personalizado usado cuando el usuario solicitó sumarización
...
- `abortCompaction()` cancela los controladores de compactación tanto manual como automática.
...
- La sumarización de rama puede cancelarse mediante señal de aborto (por ejemplo, Escape), devolviendo un resultado de navegación cancelado/abortado.
...
Estos valores son consumidos en tiempo de ejecución por `AgentSession` y los módulos de compactación/sumarización de rama.
