---
title: Política de reintento automático fuera de compactación
description: >-
...
## Límite de alcance vs compactación
...
En resumen: los fallos de tipo sobrecarga/límite de tasa/servidor/red utilizan esta política de reintento; el desbordamiento de ventana de contexto utiliza la recuperación por compactación.
...
- Clases HTTP de servidor: 429, 500, 502, 503, 504
...
- `#retryPromise: Promise<void> | undefined` (rastrea el ciclo de vida del reintento en progreso)
...
9. Eliminar el mensaje de error del asistente final del estado de ejecución del agente (se mantiene en el historial de sesión persistido).
...
## Semántica de backoff y máximo de intentos
...
- los eventos de inicio usan el intento actual (basado en 1)
- el evento de fin por máximo excedido reporta `attempt: this.#retryAttempt - 1` (último conteo de reintentos intentados)

Secuencia de backoff con la configuración por defecto:

- intento 1: 2000 ms
- intento 2: 4000 ms
- intento 3: 8000 ms

Las entradas de anulación de retardo solo se usan en la ruta de manejo de límite de uso, y solo para influir en la decisión de cambio de modelo/cuenta del almacenamiento de autenticación. En la ruta principal de reintento fuera de compactación, el backoff permanece como retardo exponencial local a menos que el cambio tenga éxito (`delayMs = 0`).
...
- resuelve la promesa de reintento (`#resolveRetry()`) para desbloquear a los esperadores
...
`abort()` llama a `abortRetry()` antes de cancelar el flujo activo del agente. Esto garantiza que el backoff del reintento se cancele cuando el usuario emite una cancelación general.

### Interacción con la TUI

En `auto_retry_start`, EventController:

- cambia el manejador de `Esc` a `session.abortRetry()`
- renderiza texto de carga: `Retrying (attempt/maxAttempts) in Ns… (esc to cancel)`

En `auto_retry_end`, restaura el manejador previo de `Esc` y limpia el estado del indicador de carga.
...
Ambos comandos devuelven respuestas de éxito; los detalles de progreso/fallo del reintento provienen de los eventos de sesión transmitidos por streaming, no de las cargas útiles de respuesta de los comandos.
...
- reenviados al runner de extensiones como eventos de extensión
- en modo RPC, reenviados directamente como objetos JSON de evento (`session.subscribe(event => output(event))`)
- en la TUI, consumidos por `EventController` para la UI de carga/error

Exposición de fallos finales:
...
- La clasificación es coincidencia de texto por regex; los errores estructurados específicos del proveedor no se utilizan aquí.
- El reintento elimina el error del asistente fallido del **contexto de ejecución** antes de re-continuar, pero el historial de sesión aún conserva esa entrada de error.
