---
title: Arquitectura de árbol de sesión
description: >-
  Arquitectura de árbol de sesión con ramificación, navegación y relaciones de
...
# Arquitectura de árbol de sesión (actual)
...
La sesión se almacena como un registro de entradas de solo anexado (append-only), pero el comportamiento en tiempo de ejecución está basado en árboles:
...
- La ramificación **no** reescribe el historial; solo cambia hacia dónde apunta la hoja antes del siguiente anexado.
...
- `src/modes/components/tree-selector.ts` — comportamiento interactivo de la interfaz del árbol y filtrado
...
`getTree()` es una proyección en tiempo de ejecución; la persistencia sigue siendo entradas JSONL de solo anexado.

## Semántica del movimiento de hojas
...
4. Calcular el nuevo objetivo de hoja:
   - seleccionando un mensaje de **usuario**: la hoja se mueve a su padre, y el texto del mensaje se devuelve para prerrellenar el editor
   - seleccionando un **custom_message**: misma regla que el mensaje de usuario (hoja = padre, texto prerellena el editor)
   - seleccionando cualquier otra entrada: hoja = id de la entrada seleccionada
...
- `/branch` crea un nuevo archivo de rama de sesión (o reemplazo en memoria para el modo no persistente).
...
- El texto del usuario seleccionado se extrae para prerrellenar el editor.
...
Detalles de `SessionManager.createBranchedSession(leafId)`:

- Construye la ruta raíz→hoja mediante `getBranch(leafId)`; lanza error si falta.
- Excluye las entradas `label` existentes de la ruta copiada.
- Reconstruye entradas de etiqueta frescas desde `labelsById` resuelto para las entradas que permanecen en la ruta.
...
## Reconstrucción de contexto e integración de resúmenes/mensajes personalizados
...
- Rastrea el estado más reciente de thinking/model/mode/ttsr en la ruta.
- Maneja la compactación más reciente en la ruta:
...
`session/messages.ts` luego mapea estos tipos de mensajes para la entrada del modelo:
...
## Etiquetas y comportamiento de la interfaz del árbol

Persistencia de etiquetas:

- `appendLabelChange(targetId, label?)` escribe entradas `label` en la cadena de hojas actual.
...
- `Shift+L` abre la edición en línea de etiquetas y escribe mediante `appendLabelChange`.

Enrutamiento de comandos:

- `/tree` siempre abre el selector de árbol.
- `/branch` abre el selector de mensajes de usuario a menos que `doubleEscapeAction=tree`, en cuyo caso también usa la experiencia de usuario del selector de árbol.

## Puntos de integración de extensiones y hooks para operaciones del árbol
...
- `session_before_tree`
  ...
  - puede proporcionar un payload de resumen usado en lugar del resumidor integrado
...
- `branch()` no puede apuntar a `null`; use `resetLeaf()` para el estado raíz-antes-de-primera-entrada.
- `branchWithSummary()` soporta objetivo `null` y registra `fromId: "root"`.
- Seleccionar la hoja actual en el selector de árbol es una operación nula (no-op).
...
## Compatibilidad legacy aún presente

Las migraciones de sesión aún se ejecutan al cargar:

- v1→v2 añade `id`/`parentId` y convierte el ancla de índice de compactación a ancla por id
- v2→v3 migra el rol legacy `hookMessage` a `custom`

El comportamiento actual en tiempo de ejecución usa la semántica de árbol versión 3 después de la migración.
