...
Se centra en el comportamiento de la implementación actual, incluyendo rutas de fallback y advertencias.
...
### Ámbito del directorio

`SessionManager` almacena sesiones bajo un directorio con ámbito de cwd por defecto:
...
### Dos rutas de listado con cargas útiles diferentes
...
1. `getRecentSessions(sessionDir, limit)` (vista de bienvenida/resumen)
   ...
   - Analiza la cabecera + vista previa del texto más temprano del usuario.
   - Devuelve `RecentSessionInfo` ligeros con getters perezosos de `name` y `timeAgo`.
...
### Comportamiento de fallback de metadatos

Para resúmenes recientes (`RecentSessionInfo`):

- preferencia de nombre para mostrar: `header.title` -> primer prompt del usuario -> `header.id` -> nombre de archivo
- el nombre se trunca a 40 caracteres para visualizaciones compactas
- los caracteres de control/saltos de línea se eliminan/sanean de los nombres derivados del título
...
1. Leer el breadcrumb con ámbito de terminal (`~/.xcsh/agent/terminal-sessions/<terminal-id>`)
...
3. Si el breadcrumb es inválido/faltante, recurrir al archivo más reciente por mtime en el directorio de sesiones (`findMostRecentSession`)
...
`createSessionManager(...)` maneja `--resume` con valor string en dos modos:

1. Valor tipo ruta (contiene `/`, `\\`, o termina en `.jsonl`)
...
- si el cwd de la sesión coincidente difiere del cwd actual, la CLI pregunta si bifurcar en el proyecto actual
...
2. si está vacío: imprimir `No sessions found` y salir anticipadamente
3. abrir selector TUI (`selectSession`)
4. si se cancela: imprimir `No session selected` y salir anticipadamente
...
## Componentes internos de selección basada en selector
...
Advertencia: el texto de la UI dice `Press Tab to view all`, pero este componente actualmente no tiene un manejador de Tab y el cableado actual solo lista sesiones del ámbito actual.

## Ejecución del cambio en tiempo de ejecución (`AgentSession.switchSession`)

`switchSession(sessionPath)` es la ruta principal de cambio en proceso.
...
3. si se cancela -> devolver `false` sin cambio
4. desconectar del flujo de eventos del agente actual
5. abortar generación/flujo de herramienta activo
6. limpiar búferes de mensajes de dirección/seguimiento/siguiente turno en cola
...
14. restaurar nivel de pensamiento:
    ...
    - en caso contrario, derivar nivel de pensamiento por defecto desde la configuración, limitar a la capacidad del modelo, establecerlo y agregar una nueva entrada `thinking_level_change`
15. reconectar listeners del agente y devolver `true`

## Reconstrucción del estado de la UI después del cambio interactivo

`SelectorController.handleResumeSession` realiza un reinicio de la UI alrededor de `switchSession`:
...
- reiniciar componente de streaming/referencias de mensajes
...
Así, el estado visible de conversación/todos se reconstruye desde el nuevo archivo de sesión.

## Reanudación en inicio vs cambio dentro de sesión
...
- El modelo/pensamiento se seleccionan durante la creación (incluyendo lógica de restauración/fallback).
- El modo interactivo luego ejecuta `#restoreModeFromSession()` para reentrar al estado de modo persistido (actualmente plan/plan_paused).

### Cambio dentro de sesión (ruta del selector estilo `/resume`)

- Usa `AgentSession.switchSession(...)` en una `AgentSession` ya en ejecución.
- Los mensajes/modelo/pensamiento se reconstruyen inmediatamente en su lugar.
- Se emiten eventos de hook `session_before_switch`/`session_switch`.
...
- Cancelación del selector CLI -> devuelve `null`, el llamador imprime `No session selected`, el proceso sale anticipadamente.
- Cancelación del selector interactivo -> se restaura el editor, sin cambio de sesión.
- Cancelación por hook (`session_before_switch`) -> `switchSession()` devuelve `false`.

### Rutas de lista vacía
...
Este es un comportamiento de recuperación, no un fallo grave.
