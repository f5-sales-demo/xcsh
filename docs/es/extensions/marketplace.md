...
O simplemente escriba `/marketplace` sin argumentos para abrir el explorador interactivo de plugins.
...
Un **plugin** es un directorio que contiene skills, comandos, hooks, servidores MCP o servidores LSP. Los plugins se identifican por `name@marketplace` (por ejemplo, `code-review@f5xc-salesdemos-marketplace`).
...
Las instalaciones con ámbito de proyecto prevalecen sobre las instalaciones con ámbito de usuario del mismo plugin.
...
| Comando | Efecto |
|---|---|
| `/marketplace` | Abrir explorador interactivo de plugins (instalar) |
...
| `/marketplace update [name]` | Re-obtener catálogo(s); omitir el nombre para actualizar todos |
...
## Formato de catálogo (marketplace.json)
...
| Campo | Descripción |
|---|---|
| `name` | Nombre del marketplace. Alfanumérico en minúsculas, guiones y puntos. Debe comenzar y terminar con alfanumérico. Máximo 64 caracteres. |
...
| `source` | sí | Dónde encontrar el plugin (ver abajo) |
| `description` | no | Descripción breve |
| `version` | no | Cadena de versión |
| `author` | no | `{ name, email? }` |
| `homepage` | no | URL |
| `category` | no | Cadena de categoría (por ejemplo, `development`, `productivity`, `security`) |
...
- Comenzar y terminar con una letra minúscula o dígito
