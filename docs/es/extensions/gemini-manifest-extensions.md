...
Regla de alcance importante: la búsqueda de proyecto es **solo cwd**. No recorre directorios padre.
...
2. conservar solo los directorios hijos directos (`entry.isDirectory()`)
3. para cada hijo `<name>`, intentar leer exactamente:
...
No se aplica verificación de tipo de cadena aquí.
...
Debido a que la deduplicación es "el primero visto gana", el orden local de elementos del proveedor importa.
...
En contraste, el proveedor native construye el orden de directorios de configuración de manera diferente (`project` luego `user` en `getConfigDirs()`), por lo que el ensombrecimiento intra-proveedor de native es en dirección opuesta.
...
- La raíz de proyecto está fijada a `<cwd>/.gemini/extensions` (sin recorrido de ancestros).
- Los nombres duplicados dentro de la fuente Gemini se resuelven con prioridad de usuario.
...
El descubrimiento de `gemini-extension.json` actualmente alimenta metadatos de capacidad (elementos `Extension`). **No** carga directamente módulos de extensión ejecutables TS/JS.
