...
description: >-
  Almacén de blobs direccionable por contenido y registro de artefactos para
  medios de sesión, capturas de pantalla y salidas de herramientas.
sidebar:
  order: 7
...
Este documento describe cómo coding-agent almacena cargas útiles grandes/binarias fuera del JSONL de sesión, cómo se persisten las salidas truncadas de herramientas, y cómo las URLs internas (`artifact://`, `agent://`) resuelven hacia los datos almacenados.
...
- **Blobs direccionados por contenido** (`blob:sha256:<hash>`): almacenamiento global orientado a binarios, utilizado para externalizar cargas útiles grandes de imágenes en base64 desde las entradas de sesión persistidas.
- **Artefactos con ámbito de sesión** (archivos bajo `<archivoSesión-sin-.jsonl>/`): archivos de texto por sesión utilizados para salidas completas de herramientas y salidas de subagentes.

Están intencionalmente separados:
...
- el mismo contenido binario en diferentes sesiones resuelve al mismo hash/ruta,
...
`ArtifactManager` deriva el directorio de artefactos desde la ruta del archivo de sesión:
...
- archivos de salida de herramientas truncada: `<numericId>.<toolType>.log` (para `artifact://`)
...
`BlobStore.put()` calcula SHA-256 sobre los bytes binarios crudos y devuelve:
...
## IDs de artefactos: entero monótono local a la sesión
...
`AgentOutputManager` asigna IDs para las salidas de subagentes como `<index>-<requestedId>` (opcionalmente anidados bajo un prefijo padre, por ejemplo `0-Parent.1-Child`). Escanea los archivos `.md` existentes durante la inicialización para continuar desde el siguiente índice en caso de reanudación.

## Flujo de datos de persistencia

## 1) Ruta de reescritura en la persistencia de entradas de sesión
...
1. **Truncamiento de cadenas grandes**: las cadenas sobredimensionadas se cortan y se les añade el sufijo `"[Session persistence truncated large content]"`.
...
3. **Externalización de imágenes a blobs**:
   - solo aplica a bloques de imagen en arrays `content`,
...
## 3) Ruta de volcado/truncamiento de salida de herramientas

`OutputSink` impulsa la salida en streaming en bash/python/ssh y ejecutores relacionados.

Comportamiento:

1. Cada fragmento se sanitiza y se añade al búfer de cola en memoria.
2. Cuando los bytes en memoria superan el umbral de volcado (`DEFAULT_MAX_BYTES`, 50KB), el sink marca la salida como truncada.
3. Si hay una ruta de artefacto disponible, el sink abre un escritor de archivos y escribe:
   - el contenido almacenado en búfer existente una vez,
   - todos los fragmentos subsiguientes.
4. El búfer en memoria siempre se recorta a la ventana de cola para visualización.
...
Si la creación del escritor de archivos del sink falla (error de E/S, ruta faltante, etc.), el sink recurre silenciosamente al truncamiento solo en memoria; la salida completa no se persiste.
...
- requiere un directorio activo de artefactos de sesión,
- el ID debe ser numérico,
- resuelve haciendo coincidir el prefijo del nombre de archivo `<id>.`,
...
- en forma simple devuelve texto markdown,
- las formas `/path` o `?q=` realizan extracción JSON,
- la extracción por ruta y por consulta no pueden combinarse,
- si se solicita extracción, el contenido del archivo debe poder parsearse como JSON.
...
Integración con la herramienta de lectura:

- `read` soporta paginación con offset/limit para lecturas de URLs internas sin extracción,
- rechaza `offset/limit` cuando se utiliza extracción con `agent://`.
...
`SessionManager.fork()` crea un nuevo archivo de sesión con un nuevo ID de sesión y un enlace `parentSession`, luego devuelve las rutas de archivos antiguo/nuevo. La copia de artefactos es manejada por `AgentSession.fork()`:

- intenta copiar recursivamente el directorio de artefactos antiguo al nuevo directorio de artefactos,
...
- los blobs son globales y direccionados por contenido, por lo que no se requiere copiar el directorio de blobs.

## Mover a un nuevo cwd

`SessionManager.moveTo()` renombra tanto el archivo de sesión como el directorio de artefactos al nuevo directorio de sesión predeterminado, con lógica de reversión si un paso posterior falla. Esto preserva la identidad de los artefactos mientras se reubica el ámbito de la sesión.
...
Los dos sistemas se intersectan solo indirectamente (ambos reducen la inflación del JSONL de sesión) pero tienen diferentes caminos de identidad, tiempo de vida y recuperación.

## Archivos de implementación

- [`src/session/blob-store.ts`](../../packages/coding-agent/src/session/blob-store.ts) — formato de referencia de blob, hashing, put/get, helpers de externalización/resolución.
...
- [`src/session/streaming-output.ts`](../../packages/coding-agent/src/session/streaming-output.ts) — comportamiento de truncamiento/volcado a archivo de `OutputSink` y metadatos de resumen.
- [`src/session/session-manager.ts`](../../packages/coding-agent/src/session/session-manager.ts) — transformaciones de persistencia, rehidratación de blobs durante la carga, interacciones de bifurcación/movimiento de sesión.
...
- [`src/tools/output-utils.ts`](../../packages/coding-agent/src/tools/output-utils.ts) — inicialización del administrador de artefactos de herramientas y asignación de rutas de artefactos por herramienta.
...
- [`src/sdk.ts`](../../packages/coding-agent/src/sdk.ts) — cableado del enrutador de URLs internas y resolutor de directorio de artefactos.
- [`src/task/output-manager.ts`](../../packages/coding-agent/src/task/output-manager.ts) — asignación de IDs de salida de agente con ámbito de sesión para `agent://`.
