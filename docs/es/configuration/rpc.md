...
El modo RPC ejecuta el agente de programación como un protocolo JSON delimitado por saltos de línea sobre stdio.

- **stdin**: comandos (`RpcCommand`) y respuestas de UI de extensiones
- **stdout**: respuestas de comandos (`RpcResponse`), eventos de sesión/agente, solicitudes de UI de extensiones
...
- Los argumentos CLI `@file` son rechazados en modo RPC.
...
- El modo RPC restablece las configuraciones `todo.*`, `task.*` y `async.*` que alteran el flujo de trabajo a sus valores predeterminados incorporados en lugar de heredar las personalizaciones del usuario.
...
No existe un envoltorio más allá de la forma del objeto en sí.
...
- Si se proporciona, las respuestas normales de comandos replican el mismo `id`.
- `RpcClient` depende de esto para la resolución de solicitudes pendientes.

Comportamiento importante observado en tiempo de ejecución:
...
- `prompt` y `abort_and_prompt` devuelven éxito inmediato, y pueden emitir una respuesta de error posterior con el **mismo** id si la programación asíncrona del prompt falla.
...
Todos los resultados de comandos usan `RpcResponse`:

- Éxito: `{ id?, type: "response", command: <command>, success: true, data?: ... }`
- Fallo: `{ id?, type: "response", command: string, success: false, error: string }`

Los datos de carga útil son específicos de cada comando y están definidos en `rpc-types.ts`.

### Carga útil de `get_state`
...
### Carga útil de `set_todos`

Reemplaza el estado de tareas en memoria para la sesión actual y devuelve la lista de fases normalizada:
...
Esto es útil para hosts que desean precargar un plan antes del primer prompt.

### Carga útil de `set_host_tools`
...
`message_update` incluye deltas de streaming en `assistantMessageEvent` (deltas de texto/pensamiento/llamada a herramienta).

## Concurrencia y Ordenamiento de Prompts/Cola
...
- la completación final se observa a través de `agent_end`

### Durante el streaming

`AgentSession.prompt()` requiere `streamingBehavior` durante el streaming activo:

- `"steer"` => mensaje de dirección encolado (ruta de interrupción)
- `"followUp"` => mensaje de seguimiento encolado (ruta post-turno)

Si se omite durante el streaming, el prompt falla.

### Valores predeterminados de cola

Del esquema de configuración del agente de programación (`packages/coding-agent/src/config/settings-schema.ts`):
...
- `set_steering_mode` / `set_follow_up_mode`
  - `"one-at-a-time"`: desencola un mensaje encolado por turno
  - `"all"`: desencola toda la cola de una vez
- `set_interrupt_mode`
  - `"immediate"`: la ejecución de herramientas verifica la dirección entre llamadas a herramientas; la dirección pendiente puede abortar las llamadas a herramientas restantes en el turno
  - `"wait"`: posterga la dirección hasta la completación del turno

## Sub-Protocolo de UI de Extensiones

Las extensiones en modo RPC usan tramas de solicitud/respuesta de UI.
...
Si un diálogo tiene un tiempo de espera, el modo RPC resuelve a un valor predeterminado cuando el tiempo de espera/aborto se dispara.

## Sub-Protocolo de Herramientas del Host

Los hosts RPC pueden exponer herramientas personalizadas al agente enviando `set_host_tools`, y luego sirviendo solicitudes de ejecución sobre el mismo transporte.

### Solicitud saliente

Cuando el agente quiere que el host ejecute una de esas herramientas, el modo RPC emite:
...
Si la ejecución de la herramienta se aborta posteriormente, el modo RPC emite:
...
Establezca `isError: true` en `host_tool_result` para presentar el contenido devuelto como un error de herramienta.

## Modelo de Errores y Recuperabilidad

### Fallos a nivel de comando

Los fallos son `success: false` con `error` de tipo string.
...
- JSONL malformado / excepciones del bucle de análisis emiten una respuesta de error `parse` y continúan leyendo las líneas subsiguientes.
- Un `set_session_name` vacío es rechazado (`Session name cannot be empty`).
- Las respuestas de UI de extensiones con `id` desconocido son ignoradas.
- Las condiciones de terminación del proceso son el cierre de stdin o un apagado explícito disparado por una extensión.
...
`src/modes/rpc/rpc-client.ts` es un envoltorio de conveniencia, no la definición del protocolo.

Características actuales del helper:

- Genera un proceso `bun <cliPath> --mode rpc`
...
- **No** expone métodos auxiliares para todos los comandos del protocolo (por ejemplo, `set_interrupt_mode` y `set_session_name` están en los tipos del protocolo pero no están envueltos como métodos dedicados)

Use tramas de protocolo sin procesar si necesita cobertura completa de la superficie.
