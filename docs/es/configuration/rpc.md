...
description: >-
  Referencia del protocolo JSON-RPC para comunicación entre procesos de los
  componentes de xcsh.
sidebar:
  order: 5
  label: Protocolo RPC
...
No hay envoltorio más allá de la forma del objeto en sí.
...
- Si se proporciona, las respuestas normales de comandos devuelven el mismo `id`.
- `RpcClient` depende de esto para la resolución de solicitudes pendientes.

Comportamiento importante de borde en tiempo de ejecución:
...
`message_update` incluye deltas de streaming en `assistantMessageEvent` (deltas de texto/pensamiento/llamadas a herramientas).
...
`prompt` y `abort_and_prompt` son **confirmados inmediatamente**:
...
- `set_interrupt_mode`
  ...
  - `"wait"`: postergar la dirección hasta la completación del turno
...
Si un diálogo tiene un tiempo de espera, el modo RPC se resuelve a un valor predeterminado cuando se activa el tiempo de espera/aborto.

## Sub-Protocolo de Herramientas del Host

Los hosts RPC pueden exponer herramientas personalizadas al agente enviando `set_host_tools`, y luego atendiendo solicitudes de ejecución sobre el mismo transporte.
...
Si la ejecución de la herramienta es abortada posteriormente, el modo RPC emite:
...
- Las condiciones de terminación del proceso son el cierre de stdin o un apagado explícito activado por una extensión.
...
### 3) Inspeccionar y ajustar comportamiento de cola
