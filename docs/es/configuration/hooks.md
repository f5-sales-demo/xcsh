...
El paquete de hooks (`src/extensibility/hooks/`) todavía se exporta y es utilizable como superficie de API, pero el runtime CLI predeterminado ahora inicializa la ruta del **extension runner**. En el flujo de inicio actual:
...
- las transformaciones de contexto y las emisiones de ciclo de vida pasan a través de `ExtensionRunner`

Por lo tanto, este archivo documenta la implementación del subsistema de hooks en sí (tipos/cargador/ejecutor/wrapper), incluyendo el comportamiento heredado y las restricciones.
...
Un módulo de hook debe exportar por defecto una factoría:
...
`discoverAndLoadHooks(configuredPaths, cwd)` hace lo siguiente:

1. Cargar hooks descubiertos desde el registro de capacidades (`loadCapability("hooks")`)
2. Agregar las rutas configuradas explícitamente (deduplicadas por ruta absoluta)
...
### Desajuste importante del sistema heredado
...
El cargador de hooks aquí utiliza importación dinámica de módulos y requiere una factoría JS/TS con exportación por defecto. Si una ruta de hook descubierta no es importable como módulo, la carga falla y se reporta en `LoadHooksResult.errors`.
...
- `session_start`
- `session_before_switch` → puede devolver `{ cancel?: boolean }`
- `session_switch`
- `session_before_branch` → puede devolver `{ cancel?: boolean; skipConversationRestore?: boolean }`
- `session_branch`
- `session_before_compact` → puede devolver `{ cancel?: boolean; compaction?: CompactionResult }`
- `session.compacting` → puede devolver `{ context?: string[]; prompt?: string; preserveData?: Record<string, unknown> }`
- `session_compact`
- `session_before_tree` → puede devolver `{ cancel?: boolean; summary?: { summary: string; details?: unknown } }`
- `session_tree`
- `session_shutdown`

### Eventos de agente/contexto

- `context` → puede devolver `{ messages?: Message[] }`
- `before_agent_start` → puede devolver `{ message?: { customType; content; display; details } }`
- `agent_start`
- `agent_end`
- `turn_start`
- `turn_end`
...
- `tool_call` (pre-ejecución) → puede devolver `{ block?: boolean; reason?: string }`
- `tool_result` (post-ejecución) → puede devolver `{ content?; details?; isError? }`
...
manejadores de tool_call
   │
   ├─ ¿alguno con { block: true }? ── sí ──> lanzar error (herramienta bloqueada)
   │
   └─ no
      │
      ▼
...
- si algún manejador devuelve `{ block: true }`, la ejecución se detiene
- si el manejador lanza una excepción, el wrapper falla de forma segura y bloquea la ejecución
- el `reason` devuelto se convierte en el texto del error lanzado
...
Si el manejador devuelve sobrescrituras:
...
- Continuación de ejecución después de errores lanzados por herramientas (la ruta de error relanza)
- Estado final de éxito/error en el comportamiento del wrapper (el `isError` devuelto está tipado pero no es aplicado por `HookToolWrapper`)
...
Comportamiento de conflicto por tipo de evento:

- `tool_call`: el último resultado devuelto gana a menos que un manejador bloquee; el primer bloqueo cortocircuita
- `tool_result`: la última sobrescritura devuelta gana (sin cortocircuito)
- `context`: encadenado; cada manejador recibe la salida de mensajes del manejador anterior
- `before_agent_start`: se conserva el primer mensaje devuelto; los mensajes posteriores se ignoran
- `session_before_*`: se rastrea el último resultado devuelto; `cancel: true` cortocircuita inmediatamente
- `session.compacting`: el último resultado devuelto gana

Conflictos de comandos/renderizadores:

- `getCommand(name)` devuelve la primera coincidencia entre hooks (el primero cargado gana)
- `getMessageRenderer(customType)` devuelve la primera coincidencia
- `getRegisteredCommands()` devuelve todos los comandos (sin deduplicación)
...
Cuando se ejecuta sin UI, el comportamiento predeterminado del contexto no-op es:

- `select/input/editor` devuelven `undefined`
...
- unido y truncado en ancho para visualización
...
`HookRunner.emit(...)` captura errores de manejadores para la mayoría de eventos y emite `HookError` a los listeners (`hookPath`, `event`, `error`), luego continúa.

`emitToolCall(...)` es más estricto: los errores de manejadores no se suprimen allí; se propagan al llamador. En `HookToolWrapper`, esto bloquea la llamada a la herramienta (a prueba de fallos).
...
### Redactar salida de herramientas en post-ejecución
...
Y la raíz del paquete (`src/index.ts`) re-exporta los **tipos** de hooks como superficie de compatibilidad heredada.
