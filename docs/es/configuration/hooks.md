...
title: Hooks
description: >-
  Sistema de hooks para automatización pre/post eventos en el ciclo de vida del
...
El paquete de hooks (`src/extensibility/hooks/`) aún se exporta y es utilizable como superficie de API, pero el runtime CLI predeterminado ahora inicializa la ruta del **extension runner**. En el flujo de inicio actual:

- `--hook` se trata como un alias para `--extension` (las rutas CLI se fusionan en `additionalExtensionPaths`)
...
Por lo tanto, este archivo documenta la implementación del subsistema de hooks en sí mismo (tipos/cargador/ejecutor/wrapper), incluyendo el comportamiento heredado y las restricciones.
...
1. Carga los hooks descubiertos del registro de capacidades (`loadCapability("hooks")`)
...
Los proveedores de descubrimiento para `hookCapability` aún modelan archivos de hook estilo shell pre/post (por ejemplo `.claude/hooks/pre/*`, `.xcsh/.../hooks/pre/*`).

El cargador de hooks aquí usa importación dinámica de módulos y requiere una factoría de hook JS/TS por defecto. Si una ruta de hook descubierta no es importable como módulo, la carga falla y se reporta en `LoadHooksResult.errors`.
...
- si un manejador lanza una excepción, el wrapper falla de forma segura y bloquea la ejecución
...
- Parámetros de entrada de herramientas en su lugar (solo bloquear/permitir en `tool_call`)
...
## Orden y comportamiento de conflictos

### Orden a nivel de descubrimiento

Los proveedores de capacidades están ordenados por prioridad (mayor primero). La deduplicación es por clave de capacidad, el primero gana.
...
Dentro de `HookRunner`, el orden es determinístico por secuencia de registro:
...
- módulo inválido o exportación default faltante → capturado en `LoadHooksResult.errors`
...
`emitToolCall(...)` es más estricto: los errores de manejadores no se absorben ahí; se propagan al llamador. En `HookToolWrapper`, esto bloquea la llamada a la herramienta (a prueba de fallos).
...
- todos los tipos de hook
- re-exportación de `execCommand`

Y la raíz del paquete (`src/index.ts`) re-exporta los **tipos** de hook como una superficie de compatibilidad heredada.
