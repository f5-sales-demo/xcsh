---
title: RPC 프로토콜 참조
description: xcsh 컴포넌트 간 프로세스 간 통신을 위한 JSON-RPC 프로토콜 참조.
sidebar:
  order: 5
  label: RPC 프로토콜
i18n:
...
RPC 모드는 코딩 에이전트를 stdio를 통한 줄바꿈 구분 JSON 프로토콜로 실행합니다.

- **stdin**: 명령어(`RpcCommand`) 및 확장 UI 응답
- **stdout**: 명령어 응답(`RpcResponse`), 세션/에이전트 이벤트, 확장 UI 요청
...
- RPC 모드는 사용자 오버라이드를 상속하는 대신 워크플로우를 변경하는 `todo.*`, `task.*`, `async.*` 설정을 기본 내장값으로 초기화합니다.
...
모든 명령어는 선택적 `id?: string`을 허용합니다.

- 제공된 경우, 정상 명령어 응답은 동일한 `id`를 에코합니다.
- `RpcClient`는 이를 대기 중인 요청 해결에 활용합니다.

런타임의 중요한 엣지 동작:

- 알 수 없는 명령어 응답은 `id: undefined`로 전송됩니다(요청에 `id`가 있었더라도).
- 입력 루프에서의 파싱/핸들러 예외는 `command: "parse"`와 `id: undefined`로 전송됩니다.
- `prompt`와 `abort_and_prompt`는 즉시 성공을 반환하고, 비동기 프롬프트 스케줄링이 실패하면 **동일한** id로 나중에 오류 응답을 전송할 수 있습니다.
...
현재 세션의 인메모리 할 일 상태를 교체하고 정규화된 단계 목록을 반환합니다:
...
이는 첫 번째 프롬프트 이전에 계획을 사전 설정하려는 호스트에 유용합니다.
...
이 도구들은 다음 모델 호출 전에 활성 세션 도구 레지스트리에 추가됩니다. `set_host_tools`를 다시 전송하면 이전 호스트 소유 세트가 교체됩니다.

## 이벤트 스트림 스키마

RPC 모드는 `AgentSession.subscribe(...)`에서 `AgentSessionEvent` 객체를 전달합니다.
...
`prompt`와 `abort_and_prompt`는 **즉시 확인(ack)**됩니다:
...
`AgentSession.prompt()`는 활성 스트리밍 중에 `streamingBehavior`가 필요합니다:

- `"steer"` => 큐에 추가된 조향 메시지 (인터럽트 경로)
- `"followUp"` => 큐에 추가된 후속 메시지 (턴 종료 후 경로)

스트리밍 중에 생략되면 프롬프트가 실패합니다.
...
- `set_steering_mode` / `set_follow_up_mode`
  - `"one-at-a-time"`: 턴당 큐에서 메시지를 하나씩 꺼냄
  - `"all"`: 전체 큐를 한 번에 꺼냄
- `set_interrupt_mode`
  - `"immediate"`: 도구 실행 시 도구 호출 사이에서 조향을 확인하며, 대기 중인 조향이 턴의 나머지 도구 호출을 중단할 수 있음
  - `"wait"`: 턴 완료까지 조향을 지연
...
- RPC 모드에서는 자동 세션 제목 생성이 비활성화되며, 대부분의 호스트에는 의미 있는 터미널 제목 표시 영역이 없기 때문에 `setTitle` UI 요청도 기본적으로 억제됩니다. UI 이벤트만 다시 활성화하려면 `PI_RPC_EMIT_TITLE=1`을 설정하세요.
...
대화 상자에 타임아웃이 있는 경우, RPC 모드는 타임아웃/중단 발생 시 기본값으로 해결합니다.

## 호스트 도구 하위 프로토콜

RPC 호스트는 `set_host_tools`를 전송한 후 동일한 전송 채널을 통해 실행 요청을 처리하여 에이전트에 커스텀 도구를 노출할 수 있습니다.

### 아웃바운드 요청

에이전트가 호스트에 해당 도구 중 하나의 실행을 요청할 때 RPC 모드는 다음을 전송합니다:
...
- 대부분의 명령어 실패는 복구 가능하며, 프로세스는 활성 상태를 유지합니다.
- 잘못된 JSONL / 파싱 루프 예외는 `parse` 오류 응답을 전송하고 후속 라인을 계속 읽습니다.
- 빈 `set_session_name`은 거부됩니다(`Session name cannot be empty`).
- 알 수 없는 `id`의 확장 UI 응답은 무시됩니다.
...
- 모든 프로토콜 명령어에 대한 헬퍼 메서드를 노출하지는 **않습니다** (예를 들어, `set_interrupt_mode`와 `set_session_name`은 프로토콜 타입에는 있지만 전용 메서드로 래핑되어 있지 않습니다)

완전한 표면 커버리지가 필요한 경우 원시 프로토콜 프레임을 사용하세요.
