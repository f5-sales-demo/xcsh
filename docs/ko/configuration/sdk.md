...
자체 Bun/Node 프로세스에서 에이전트 상태, 이벤트 스트리밍, 도구 연결 및 세션 제어에 직접 접근하고자 할 때 사용하세요.

크로스 언어/프로세스 격리가 필요한 경우에는 대신 RPC 모드를 사용하세요.
...
## `createAgentSession()`이 기본적으로 디스커버리하는 항목
...
- **필수 제공**: 최소 세션을 위해서는 아무것도 필요 없음
- **임베더에서 일반적으로 명시적으로 제공하는 항목**:
...
1. 기존 세션에서 모델 복원 (복원 가능 + 키가 사용 가능한 경우)
...
`session.subscribe(listener)`로 구독하면 구독 해제 함수를 반환합니다.
...
- 숨겨진 도구(예: `submit_result`)는 옵션에서 필요로 하지 않는 한 옵트인입니다.
...
활성 도구 변경을 반영하여 시스템 프롬프트가 재구축됩니다.

## 디스커버리 헬퍼

내부 디스커버리 로직을 재생성하지 않고 부분적인 제어를 원할 때 다음을 사용하세요:
...
- `parentTaskPrefix`: 중첩된 태스크 출력을 위한 아티팩트 네이밍 접두사

일반적인 단일 에이전트 임베딩에서는 선택 사항입니다.
