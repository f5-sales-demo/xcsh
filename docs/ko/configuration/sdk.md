---
title: SDK
description: xcsh 코딩 에이전트 런타임 위에 커스텀 에이전트 및 통합을 구축하기 위한 SDK.
sidebar:
  order: 6
  label: SDK
i18n:
...
자체 Bun/Node 프로세스에서 에이전트 상태, 이벤트 스트리밍, 도구 연결, 세션 제어에 직접 접근하고자 할 때 사용하세요.

언어 간/프로세스 격리가 필요한 경우에는 RPC 모드를 대신 사용하세요.
...
- `ModelRegistry`
- `discoverAuthStorage`
- 탐색 헬퍼 (`discoverExtensions`, `discoverSkills`, `discoverContextFiles`, `discoverPromptTemplates`, `discoverSlashCommands`, `discoverCustomTSCommands`, `discoverMCPServers`)
...
## `createAgentSession()`이 기본적으로 탐색하는 것

`createAgentSession()`은 "제공하면 재정의, 생략하면 탐색" 원칙을 따릅니다.

생략된 경우 다음을 해석합니다:

- `cwd`: `getProjectDir()`
- `agentDir`: `~/.xcsh/agent` (`getAgentDir()`를 통해)
...
일반적으로 제어하고 싶은 것만 제공하면 됩니다:

- **필수 제공**: 최소 세션에는 아무것도 필요 없음
- **임베더에서 보통 명시적으로 제공하는 것**:
    - `sessionManager` (인메모리 또는 커스텀 위치가 필요한 경우)
    - `authStorage` + `modelRegistry` (자격 증명/모델 생명주기를 직접 관리하는 경우)
    - `model` 또는 `modelPattern` (결정적 모델 선택이 중요한 경우)
    - `settings` (격리된/테스트 설정이 필요한 경우)

## 세션 관리자 동작 (영구 vs 인메모리)

`AgentSession`은 항상 `SessionManager`를 사용합니다; 동작은 사용하는 팩토리에 따라 달라집니다.
...
`createAgentSession()`은 모델 선택 및 API 키 해석에 `ModelRegistry` + `AuthStorage`를 사용합니다.
...
복원이 실패하면 `modelFallbackMessage`가 폴백을 설명합니다.
...
3. 프로바이더 환경 변수
4. 커스텀 프로바이더 리졸버 폴백 (설정된 경우)

## 이벤트 구독 모델

`session.subscribe(listener)`로 구독합니다; 구독 해제 함수를 반환합니다.
...
`AgentSessionEvent`는 핵심 `AgentEvent`와 세션 수준 이벤트를 포함합니다:
...
- 숨겨진 도구(예: `submit_result`)는 옵션에 의해 요구되지 않는 한 옵트인 방식입니다.
...
내부 탐색 로직을 재생성하지 않고 부분적 제어를 원할 때 사용합니다:
...
오케스트레이터를 구축하는 SDK 소비자를 위한 옵션 (태스크 실행기 흐름과 유사):

- `outputSchema`: 구조화된 출력 기대값을 도구 컨텍스트에 전달
- `requireSubmitResultTool`: `submit_result` 도구 포함을 강제
- `taskDepth`: 중첩된 태스크 세션을 위한 재귀 깊이 컨텍스트
- `parentTaskPrefix`: 중첩된 태스크 출력을 위한 아티팩트 명명 접두사
...
`setToolUIContext(...)`는 임베더가 도구/확장이 호출해야 하는 UI 기능을 제공하는 경우에만 사용하세요.
