...
자체 Bun/Node 프로세스에서 에이전트 상태, 이벤트 스트리밍, 도구 연결, 세션 제어에 직접 접근하고 싶을 때 사용하세요.

교차 언어/프로세스 격리가 필요한 경우 대신 RPC 모드를 사용하세요.
...
## `createAgentSession()`이 기본적으로 탐색하는 항목
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.xcsh/agent` (`getAgentDir()`을 통해)
...
- 세션 메서드는 여전히 작동하지만, 영구 저장 관련 동작(파일 재개/포크 경로)은 자연적으로 제한됩니다.
...
`createAgentSession()`은 모델 선택과 API 키 해석에 `ModelRegistry` + `AuthStorage`를 사용합니다.
...
복원에 실패하면 `modelFallbackMessage`가 폴백 사유를 설명합니다.
...
2. 현재 스트리밍 중인 경우:
    - `streamingBehavior: "steer" | "followUp"`이 필요합니다
    - 작업을 버리지 않고 큐에 추가합니다
3. 유휴 상태인 경우:
    - 모델 + API 키 유효성 검사
    - 사용자 메시지 추가
...
- `customTools`와 확장으로 등록된 도구는 여전히 포함됩니다.
- 숨겨진 도구(예: `submit_result`)는 옵션에서 필요하지 않는 한 옵트인 방식입니다.
...
내부 탐색 로직을 재생성하지 않고 부분적으로 제어하고 싶을 때 사용하세요:
...
임베더가 도구/확장이 호출해야 하는 UI 기능을 제공하는 경우에만 `setToolUIContext(...)`를 사용하세요.
