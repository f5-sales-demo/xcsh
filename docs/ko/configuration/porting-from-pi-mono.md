---
title: 'pi-mono에서 포팅하기: 실용 병합 가이드'
...
# pi-mono에서 포팅하기: 실용 병합 가이드

이 가이드는 pi-mono에서 이 저장소로 변경 사항을 포팅할 때 반복적으로 사용할 수 있는 체크리스트입니다.
단일 파일, 기능 브랜치, 또는 전체 릴리스 동기화 등 모든 병합에 사용하세요.
...
새로운 동기화를 시작할 때, 이 커밋 이후의 패치를 생성하세요:
...
- 작업할 패키지 또는 폴더를 나열합니다.
- 범위에 포함할 기능과 의도적으로 건너뛸 기능을 결정합니다.

## 1) 안전하게 코드 가져오기

- 전체 복사보다는 깔끔하고 집중된 diff를 선호합니다.
- 빌드 아티팩트나 생성된 파일은 복사하지 마세요.
- 업스트림에서 새 파일을 추가한 경우, 명시적으로 추가하고 내용을 검토하세요.

## 2) import 확장자 규칙 맞추기

대부분의 런타임 TypeScript 소스는 내부 import에서 `.js`를 생략하지만, 일부 test/bench 진입점은 ESM
런타임 호환성을 위해 `.js`를 유지합니다. 로컬 패키지의 기존 스타일을 따르세요. 확장자를 일괄적으로 제거하지 마세요.

- `packages/coding-agent` 런타임 소스에서는 비-TS 에셋을 import하는 경우가 아니라면 내부 import에서 확장자를 생략합니다.
- `packages/tui/test`와 `packages/natives/bench`에서는 주변 파일이 이미 `.js`를 사용하고 있다면 유지합니다.
- 도구에서 필요한 경우(예: `.json`, `.css`, `.md` 텍스트 임베드) 실제 파일 확장자를 유지합니다.
- 예시: `import { x } from "./foo.js";` → `import { x } from "./foo";` (패키지 규칙이 확장자 없음인 경우에만).
...
Bun에서 실행합니다. Bun이 더 나은 대안을 제공하는 경우에만 Node API를 교체합니다.

**교체할 것:**

- 프로세스 생성: `child_process.spawn` → 간단한 명령에는 Bun Shell `$`, 스트리밍이나 장시간 실행 작업에는 `Bun.spawn`/`Bun.spawnSync`
...
- Env 로딩: `dotenv` → Bun이 `.env`를 자동으로 로드함

**교체하지 말 것 (Bun에서도 정상 작동):**

- `os.homedir()` — `Bun.env.HOME`, `Bun.env.HOME`, 또는 리터럴 `"~"`로 교체하지 마세요
- `os.tmpdir()` — `Bun.env.TMPDIR || "/tmp"` 또는 하드코딩된 경로로 교체하지 마세요
- `fs.mkdtempSync()` — 수동 경로 조합으로 교체하지 마세요
- `path.join()`, `path.resolve()` 등 — 이들은 문제없습니다

**Import 스타일:** `node:` 접두사는 네임스페이스 import에서만 사용합니다 (`node:fs`나 `node:path`에서 named import 하지 마세요).

**추가 Bun 규칙:**

- 짧은 비-스트리밍 명령에는 Bun Shell `$`를 선호합니다. 스트리밍 I/O나 프로세스 제어가 필요한 경우에만 `Bun.spawn`을 사용합니다.
...
- 업스트림이 에셋을 dist 폴더로 복사하는 경우, Bun 친화적인 임베드로 교체합니다.
- 프롬프트는 정적 `.md` 파일입니다. 인라인 프롬프트 문자열 대신 Bun 텍스트 import(`with { type: "text" }`)와 Handlebars를 사용합니다.
- 인접한 비-텍스트 리소스를 로드하려면 `import.meta.dir` + `Bun.file`을 사용합니다.
- 에셋을 저장소 내에 유지하고 번들러가 포함하도록 합니다.
...
- 업스트림이 런타임에 번들된 폴백 파일을 읽는 경우, 파일시스템 읽기를 Bun 텍스트 임베드 import로 교체합니다.
  - 예시 (Codex 지시사항 폴백):
    ...
    - `readFileSync(FALLBACK_PROMPT_PATH, "utf8")` 대신 `return FALLBACK_INSTRUCTIONS;` 사용

## 6) `package.json` 신중하게 포팅

`package.json`을 계약으로 취급하세요. 의도적으로 병합합니다.

- 포트에서 변경이 필요하지 않는 한 기존 `name`, `version`, `type`, `exports`, `bin`을 유지합니다.
- npm/node 스크립트를 Bun 동등 항목으로 교체합니다 (예: `bun check`, `bun test`).
...
- 기존 포맷팅 규칙을 유지합니다.
- 필요한 경우가 아니라면 `any`를 도입하지 마세요.
- 동적 import와 인라인 타입 import를 피하고, 최상위 import만 사용합니다.
- 코드에서 프롬프트를 빌드하지 마세요. 프롬프트는 Handlebars로 렌더링되는 정적 `.md` 파일입니다.
- coding-agent에서는 절대 `console.log`/`console.warn`/`console.error`를 사용하지 마세요. `@f5xc-salesdemos/pi-utils`의 `logger`를 사용합니다.
- `new Promise((resolve, reject) => ...)` 대신 `Promise.withResolvers()`를 사용합니다.
- **클래스 필드나 메서드에 `private`/`protected`/`public` 키워드를 사용하지 마세요.** 캡슐화에는 ES `#` 프라이빗 필드를 사용하고, 접근 가능한 멤버는 키워드 없이 둡니다. 유일한 예외는 생성자 매개변수 속성(`constructor(private readonly x: T)`)으로, TypeScript에서 키워드가 필수입니다. 업스트림 코드에서 `private foo`나 `protected bar`를 사용하는 경우, `#foo` (프라이빗) 또는 그냥 `bar` (접근 가능)로 변환합니다.
- 새로운 임시 코드보다 기존 헬퍼와 유틸리티를 선호합니다.
- 이 저장소에서 이미 수행된 Bun 우선 인프라 변경 사항을 보존합니다:
  - 런타임은 Bun입니다 (Node 진입점 없음).
  - 패키지 관리자는 Bun입니다 (npm 잠금 파일 없음).
  - 무거운 Node API (`child_process`, `readline`)는 Bun 동등 항목으로 교체됩니다.
  - 가벼운 Node API (`os.homedir`, `os.tmpdir`, `fs.mkdtempSync`, `path.*`)는 유지됩니다.
  - CLI 셔뱅은 `bun`을 사용합니다 (`node`나 `tsx`가 아님).
  - 패키지는 소스 파일을 직접 사용합니다 (TypeScript 빌드 단계 없음).
  - CI 워크플로우는 설치/검사/테스트에 Bun을 실행합니다.

## 8) 이전 호환성 레이어 제거

요청되지 않는 한 업스트림 호환성 심을 제거합니다.
...
변경 사항과 관련 없는 검사 실패가 이미 저장소에 있는 경우, 이를 명시적으로 알리세요.
...
개선 사항을 기록하고 명시적 검사를 추가하여 병합 과정에서 누락되지 않도록 합니다.

- **예상 동작 고정**: 각 개선 사항에 대해 짧은 "이전/이후" 메모를 추가합니다 (입력, 출력,
  기본값, 엣지 케이스). 이는 무음 롤백을 방지합니다.
- **이전 → 새 API 매핑**: 업스트림이 개념을 이름 변경한 경우 (hooks → extensions, custom tools → tools 등),
  모든 이전 진입점이 여전히 연결되는지 확인합니다. 하나의 누락된 플래그나 export가 기능 손실을 의미합니다.
- **Export 검증**: `package.json` `exports`, 공개 타입, 배럴 파일을 확인합니다. 업스트림 포트는 종종
  로컬 추가 사항을 재-export하는 것을 잊습니다.
- **비-정상 경로 커버**: 오류 처리, 타임아웃 또는 폴백 로직을 수정한 경우, 테스트나 최소한
  해당 경로를 실행하는 수동 체크리스트를 추가합니다.
- **기본값 및 설정 병합 순서 확인**: 개선 사항은 종종 기본값에 존재합니다. 새 기본값이
  되돌려지지 않았는지 확인합니다 (예: 새 설정 우선순위, 비활성화된 기능, 도구 목록).
- **환경/셸 동작 감사**: 실행이나 샌드박싱을 수정한 경우, 새 경로가 여전히 정제된
  env를 사용하고 별칭/함수 재정의를 재도입하지 않는지 검증합니다.
- **대상 샘플 재실행**: 최소한의 "정상 작동 확인" 예시를 유지하고 포트 후 실행합니다
  (CLI 플래그, 확장 등록, 도구 실행).

## 12) 재작성된 코드 감지 및 처리

파일을 포팅하기 전에, 업스트림이 상당히 리팩토링했는지 확인합니다:
...
diff에서 파일이 (단순 패치가 아니라) **재작성**된 것으로 나타나는 경우:

- 새로운 추상화, 이름 변경된 개념, 병합된 모듈, 변경된 데이터 흐름

포팅 전에 **새 구현을 철저히 읽어야** 합니다. 재작성된 코드를 맹목적으로 병합하면 다음과 같은 이유로 기능이 손실됩니다:

참고: 인터랙티브 모드는 최근 controllers/utils/types로 분리되었습니다. 관련 변경 사항을 백포팅할 때, 우리가 만든 개별 파일에 업데이트를 포팅하고 `interactive-mode.ts` 연결이 동기화 상태를 유지하는지 확인하세요.

1. **기본값이 무음으로 변경됨** - 새 변수 `defaultFoo = [a, b]`가 `[a, b, c, d, e]`를 반환하던 이전 `getAllFoo()`를 교체할 수 있습니다.

2. **API 옵션이 누락됨** - 시스템이 병합될 때 (예: `hooks` + `customTools` → `extensions`), 이전 옵션이 새 구현으로 연결되지 않을 수 있습니다.
...
2. **새 구현 읽기** - 새 추상화와 이전 동작에 어떻게 매핑되는지 이해합니다.

3. **기능 동등성 검증** - 이전 코드의 각 기능에 대해, 새 코드가 이를 보존하거나 명시적으로 제거하는지 확인합니다.

4. **누락 항목 검색** - switch 문, 핸들러, UI 컴포넌트에서 누락되었을 수 있는 이전 이름/개념을 검색합니다.

5. **경계 테스트** - CLI 플래그, SDK 옵션, 이벤트 핸들러, 기본값—이곳이 회귀가 숨는 곳입니다.
...
- [ ] 에셋이 Bun 임베드 패턴으로 로드됨 (복사 스크립트 없음)
- [ ] 테스트 또는 검사가 실행됨 (또는 차단된 것으로 명시적 기재)
- [ ] 기능 회귀 없음 (섹션 11-12 참조)

## 14) 커밋 메시지 형식

백포트를 커밋할 때, 저장소 형식 `<type>(scope): <과거형 설명>`을 따르고 커밋
범위를 제목에 유지합니다.
...
- 외부 기여에 대해 업스트림 이슈/PR 번호와 기여자 표기 포함
- 제목의 커밋 범위는 동기화 지점 추적에 도움됨

## 15) 의도적 차이점

우리 포크에는 업스트림과 다른 아키텍처 결정이 있습니다. **다음 업스트림 패턴을 포팅하지 마세요:**

### UI 아키텍처

| 업스트림                                    | 우리 포크                                                  | 이유                                                                |
| ------------------------------------------- | --------------------------------------------------------- | --------------------------------------------------------------------- |
| `FooterDataProvider` 클래스                  | `StatusLineComponent`                                     | 더 단순하고 통합된 상태 표시줄                                       |
...
| 업스트림                                 | 우리 포크                                 | 비고                                     |
| ---------------------------------------- | ---------------------------------------- | ----------------------------------------- |
| `sessionManager.appendSessionInfo(name)` | `sessionManager.setSessionName(name)`    | 전체적으로 `sessionName` 사용           |
...
| `node:test` 어서션    | `expect()` 매처           |

### 도구 아키텍처

| 업스트림                            | 우리 포크                                                          | 비고                                                     |
| ----------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------- |
| `createTool(cwd: string, options?)` | `createTools(session: ToolSession)` (`BUILTIN_TOOLS` 레지스트리를 통해)  | 도구 팩토리는 `ToolSession`을 받고 `null`을 반환할 수 있음 |
| 도구별 `*Operations` 인터페이스   | 도구별 인터페이스 유지 (`FindOperations`, `GrepOperations`)   | SSH/원격 재정의에 사용                             |
| 모든 곳에서 Node.js `fs/promises`    | 파일에는 `Bun.file()`/`Bun.write()`; 디렉토리에는 `node:fs/promises` | 단순화할 수 있는 경우 Bun API 선호                        |
...
| 프로바이더당 단일 자격 증명  | 라운드 로빈 선택으로 다중 자격 증명 | 세션 어피니티와 백오프 로직 보존 |
...
- `models.generated.ts` — 자동 생성되므로 로컬에서 재생성 (models.json으로)

### 우리가 추가한 기능 (보존할 것)

이들은 우리 포크에는 있지만 업스트림에는 없습니다. **절대 덮어쓰지 마세요:**

- 인터랙티브 모드의 `StatusLineComponent`
- 세션 어피니티를 포함한 다중 자격 증명 인증
