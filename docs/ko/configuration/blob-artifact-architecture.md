...
description: '세션 미디어, 스크린샷, 도구 출력을 위한 콘텐츠 주소 기반 blob 저장소 및 아티팩트 레지스트리.'
sidebar:
  order: 7
...
이 문서는 coding-agent가 세션 JSONL 외부에 대용량/바이너리 페이로드를 저장하는 방법, 잘린 도구 출력이 영속화되는 방식, 그리고 내부 URL(`artifact://`, `agent://`)이 저장된 데이터로 다시 해석되는 방법을 설명합니다.

## 두 개의 저장소 시스템이 존재하는 이유

런타임은 서로 다른 데이터 형태를 위해 두 가지 다른 영속화 메커니즘을 사용합니다:

- **콘텐츠 주소 기반 blob** (`blob:sha256:<hash>`): 영속화된 세션 항목에서 대용량 이미지 base64 페이로드를 외부화하는 데 사용되는 전역, 바이너리 지향 저장소.
- **세션 범위 아티팩트** (`<sessionFile-without-.jsonl>/` 아래의 파일): 전체 도구 출력과 하위 에이전트 출력에 사용되는 세션별 텍스트 파일.

이들은 의도적으로 분리되어 있습니다:

- blob 저장소는 콘텐츠 해시를 통한 중복 제거와 안정적인 참조를 최적화하고,
- 아티팩트 저장소는 추가 전용 세션 도구와 로컬 ID를 통한 사람/도구의 검색을 최적화합니다.

## 저장소 경계와 디스크 레이아웃

## Blob 저장소 경계 (전역)

`SessionManager`는 `BlobStore(getBlobsDir())`를 생성하므로, blob 파일은 공유 전역 blob 디렉토리에 위치합니다 (세션 폴더 내부가 아님).
...
- 여러 세션에 걸쳐 동일한 바이너리 콘텐츠는 같은 해시/경로로 해석됩니다,
- 쓰기는 콘텐츠 수준에서 멱등성을 가집니다,
- blob은 개별 세션 파일보다 오래 존속할 수 있습니다.

## 아티팩트 경계 (세션 로컬)

`ArtifactManager`는 세션 파일 경로에서 아티팩트 디렉토리를 파생합니다:
...
`AgentOutputManager`는 하위 에이전트 출력을 위한 ID를 `<index>-<requestedId>` 형식으로 할당합니다 (선택적으로 부모 접두사 아래에 중첩, 예: `0-Parent.1-Child`). 초기화 시 기존 `.md` 파일을 스캔하여 재개 시 다음 인덱스부터 계속합니다.

## 영속화 데이터 흐름

## 1) 세션 항목 영속화 재작성 경로

세션 항목이 작성되기 전에 (`#rewriteFile` / 증분 영속화), `SessionManager`는 (`truncateForPersistence`를 통해) `prepareEntryForPersistence()`를 호출합니다.

주요 동작:

1. **대용량 문자열 잘림**: 초과 크기 문자열은 잘리고 `"[Session persistence truncated large content]"` 접미사가 붙습니다.
2. **일시적 필드 제거**: `partialJson`과 `jsonlEvents`는 영속화 항목에서 제거됩니다.
3. **이미지를 blob으로 외부화**:
...
이를 통해 세션 JSONL을 간결하게 유지하면서 복구 가능성을 보존합니다.

## 2) 세션 로드 재수화 경로

세션을 열 때 (`setSessionFile`), 마이그레이션 후 `SessionManager`는 `resolveBlobRefsInEntries()`를 실행합니다.

`blob:sha256:<hash>`를 가진 각 message/custom-message 이미지 블록에 대해:
...
- `resolveImageData()`가 경고를 로그하고,
- 원래 참조 문자열을 변경 없이 반환하고,
- 로드가 계속됩니다 (하드 크래시 없음).

## 3) 도구 출력 스필/잘림 경로

`OutputSink`는 bash/python/ssh 및 관련 실행기에서 스트리밍 출력을 처리합니다.

동작:

1. 모든 청크가 정제되어 인메모리 테일 버퍼에 추가됩니다.
2. 인메모리 바이트가 스필 임계값을 초과하면 (`DEFAULT_MAX_BYTES`, 50KB), 싱크가 출력을 잘림으로 표시합니다.
3. 아티팩트 경로가 사용 가능한 경우, 싱크가 파일 라이터를 열고 다음을 기록합니다:
   - 기존 버퍼링된 콘텐츠를 한 번,
   - 모든 후속 청크를.
4. 인메모리 버퍼는 항상 표시를 위해 테일 윈도우로 잘립니다.
5. `dump()`는 파일 싱크가 성공적으로 생성된 경우에만 `artifactId`를 포함한 요약을 반환합니다.

실질적 효과:

- UI/도구 반환은 잘린 테일을 보여주고,
- 전체 출력은 아티팩트 파일에 보존되며 `artifact://<id>`로 참조됩니다.

파일 싱크 생성이 실패하면 (I/O 오류, 경로 누락 등), 싱크는 조용히 인메모리 잘림만으로 폴백합니다; 전체 출력은 영속화되지 않습니다.

## URL 접근 모델

## `blob:` 참조

`blob:sha256:<hash>`는 세션 항목 페이로드 내부의 영속화 참조이며, 라우터가 처리하는 내부 URL 스킴이 아닙니다. 해석은 세션 로드 시 `SessionManager`에 의해 수행됩니다.
...
- 일반 형태는 마크다운 텍스트를 반환합니다,
- `/path` 또는 `?q=` 형태는 JSON 추출을 수행합니다,
- 경로와 쿼리 추출은 결합할 수 없습니다,
- 추출이 요청된 경우, 파일 콘텐츠는 JSON으로 파싱 가능해야 합니다.
...
- `agent://` 추출 사용 시 `offset/limit`을 거부합니다.

## 재개, 포크, 이동 의미론

## 재개

- `ArtifactManager`는 첫 할당 시 기존 `{id}.*.log` 파일을 스캔하고 번호를 이어갑니다.
- `AgentOutputManager`는 기존 `.md` 출력 ID를 스캔하고 번호를 이어갑니다.
...
- 이전 디렉토리 누락은 허용됩니다,
- ENOENT가 아닌 복사 오류는 경고로 로그되며 포크는 여전히 완료됩니다.

포크 후 ID 영향:

- 복사가 성공한 경우, 새 세션의 아티팩트 카운터는 최대 복사된 ID 이후부터 계속됩니다,
- 복사가 실패/건너뛰어진 경우, 새 세션 아티팩트 ID는 `0`부터 시작합니다.

포크 후 Blob 영향:

- blob은 전역이고 콘텐츠 주소 기반이므로 blob 디렉토리 복사가 필요하지 않습니다.

## 새 cwd로 이동

`SessionManager.moveTo()`는 세션 파일과 아티팩트 디렉토리를 새 기본 세션 디렉토리로 이름을 변경하며, 이후 단계가 실패할 경우 롤백 로직을 포함합니다. 이는 세션 범위를 재배치하면서 아티팩트 신원을 보존합니다.

## 실패 처리 및 폴백 경로

| 경우 | 동작 |
| --- | --- |
| 재수화 중 Blob 파일 누락 | 경고 후 인메모리에 `blob:sha256:` 참조 문자열 유지 |
| `BlobStore.get`을 통한 Blob 읽기 ENOENT | `null` 반환 |
| 아티팩트 디렉토리 누락 (`ArtifactManager.listFiles`) | 빈 목록 반환 (새로 할당 시작 가능) |
| 아티팩트 디렉토리 누락 (`artifact://` / `agent://`) | 명시적으로 `No artifacts directory found` throw |
| 아티팩트 ID 찾을 수 없음 | 사용 가능한 ID 목록과 함께 throw |
| OutputSink 아티팩트 라이터 초기화 실패 | 테일 전용 잘림으로 계속 (전체 출력 아티팩트 없음) |
| 세션 파일 없음 (일부 태스크 경로) | Task 도구가 하위 에이전트 출력을 위한 임시 아티팩트 디렉토리로 폴백 |

## 바이너리 blob 외부화 vs 텍스트 출력 아티팩트

- **Blob 외부화**는 영속화된 세션 항목 콘텐츠 내부의 바이너리 이미지 페이로드를 위한 것입니다; JSONL의 인라인 base64를 안정적인 콘텐츠 참조로 대체합니다.
- **아티팩트**는 실행 출력과 하위 에이전트 출력을 위한 일반 텍스트 파일입니다; 내부 URL을 통해 세션 로컬 ID로 주소 지정이 가능합니다.

두 시스템은 간접적으로만 교차하며 (둘 다 세션 JSONL 비대화를 줄임), 서로 다른 신원, 수명 주기, 검색 경로를 가집니다.
...
- [`src/session/agent-session.ts`](../../packages/coding-agent/src/session/agent-session.ts) — 대화형 포크 시 아티팩트 디렉토리 복사.
- [`src/tools/output-utils.ts`](../../packages/coding-agent/src/tools/output-utils.ts) — 도구 아티팩트 관리자 부트스트랩 및 도구별 아티팩트 경로 할당.
