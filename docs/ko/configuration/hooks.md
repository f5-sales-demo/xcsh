...
이 문서는 `src/extensibility/hooks/*`에 있는 **현재 hook 하위 시스템 코드**를 설명합니다.

## 런타임에서의 현재 상태

hook 패키지(`src/extensibility/hooks/`)는 여전히 API 표면으로 내보내지고 사용 가능하지만, 기본 CLI 런타임은 이제 **확장 러너(extension runner)** 경로를 초기화합니다. 현재 시작 흐름에서:
...
- 컨텍스트 변환 및 라이프사이클 이벤트 발생은 `ExtensionRunner`를 통해 이루어집니다

따라서 이 문서는 레거시 동작과 제약 사항을 포함한 hook 하위 시스템 구현 자체(타입/로더/러너/래퍼)를 문서화합니다.

## 주요 파일

- `src/extensibility/hooks/types.ts` — hook 컨텍스트, 이벤트 타입 및 결과 계약
- `src/extensibility/hooks/loader.ts` — 모듈 로딩 및 hook 검색 브리지
...
hook 모듈은 팩토리를 기본 내보내기(default-export)해야 합니다:
...
1. 기능 레지스트리에서 검색된 hook을 로드 (`loadCapability("hooks")`)
2. 명시적으로 구성된 경로를 추가 (절대 경로로 중복 제거)
...
`hookCapability`에 대한 검색 프로바이더는 여전히 사전/사후 셸 스타일 hook 파일(예: `.claude/hooks/pre/*`, `.xcsh/.../hooks/pre/*`)을 모델링합니다.

여기서 hook 로더는 동적 모듈 import를 사용하며 기본 JS/TS hook 팩토리를 필요로 합니다. 검색된 hook 경로가 모듈로 import할 수 없는 경우 로드가 실패하며 `LoadHooksResult.errors`에 보고됩니다.

## 이벤트 표면

hook 이벤트는 `types.ts`에서 강하게 타입이 지정됩니다.
...
이것은 hook 하위 시스템의 핵심 사전/사후 인터셉션 모델입니다.
...
도구 실패 시 래퍼는 `isError: true`와 오류 텍스트 콘텐츠로 `tool_result`를 발생시킨 후 원래 오류를 다시 던집니다.
...
- 원시 도구 입력 매개변수의 직접 수정 (`tool_call`에서 차단/허용만 가능)
- 던져진 도구 오류 이후의 실행 계속 (오류 경로는 재throw)
- 래퍼 동작에서의 최종 성공/오류 상태 (반환된 `isError`는 타입이 지정되어 있지만 `HookToolWrapper`에서 적용되지 않음)

## 순서 및 충돌 동작

### 검색 수준 순서

기능 프로바이더는 우선순위순으로 정렬됩니다 (높은 것이 먼저). 중복 제거는 기능 키 기준으로, 첫 번째가 우선합니다.

`hooks`의 경우 기능 키는 `${type}:${tool}:${name}`입니다. 낮은 우선순위 프로바이더의 중복된 항목은 표시되어 유효 검색 목록에서 제외됩니다.

### 로드 순서

`discoverAndLoadHooks`는 해석된 절대 경로로 중복 제거된 평탄한 `allPaths` 목록을 구성한 후, `loadHooks`가 해당 순서로 반복합니다.
각 검색된 디렉토리 내의 파일 순서는 `readdir` 출력에 따라 다릅니다; hook 로더는 추가 정렬을 수행하지 않습니다.
...
- `tool_call`: 핸들러가 차단하지 않는 한 마지막으로 반환된 결과가 우선; 첫 번째 차단이 단락 회로(short-circuit)
- `tool_result`: 마지막으로 반환된 재정의가 우선 (단락 회로 없음)
- `context`: 체이닝됨; 각 핸들러는 이전 핸들러의 메시지 출력을 수신
- `before_agent_start`: 첫 번째로 반환된 메시지가 유지됨; 이후 메시지는 무시
- `session_before_*`: 가장 최근에 반환된 결과가 추적됨; `cancel: true`는 즉시 단락 회로
- `session.compacting`: 가장 최근에 반환된 결과가 우선
...
`HookRunner.emit(...)`는 대부분의 이벤트에서 핸들러 오류를 잡아 리스너에게 `HookError`를 발생시킨 후 (`hookPath`, `event`, `error`) 계속 진행합니다.

`emitToolCall(...)`은 더 엄격합니다: 핸들러 오류가 그곳에서 삼켜지지 않고 호출자에게 전파됩니다. `HookToolWrapper`에서는 이것이 도구 호출을 차단합니다 (안전 장치).
...
그리고 패키지 루트(`src/index.ts`)는 레거시 호환성 표면으로 hook **타입**을 재내보냅니다.
