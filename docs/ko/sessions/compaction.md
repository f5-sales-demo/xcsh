...
- **압축**은 현재 브랜치에서 이전 히스토리를 요약으로 다시 작성합니다.
- **브랜치 요약**은 `/tree` 탐색 중 폐기된 브랜치의 컨텍스트를 캡처합니다.

둘 다 세션 항목으로 영속화되며 LLM 입력을 재구성할 때 사용자 컨텍스트 메시지로 다시 변환됩니다.
...
압축과 브랜치 요약은 일반 assistant/user 메시지가 아닌 일급 세션 항목입니다.
...
2. **자동 오버플로우 복구**: 컨텍스트 오버플로우로 감지되는 어시스턴트 오류 발생 후.
...
프루닝이 항목을 변경하면, 압축 결정 전에 세션 저장소가 다시 작성되고 에이전트 메시지 상태가 갱신됩니다.
...
3. 측정된 사용 비율이 있을 때 `keepRecentTokens`를 조정합니다.
4. 경계 윈도우에서 `findCutPoint()`를 실행합니다.

유효한 절단 지점은 다음을 포함합니다:

- 다음 역할의 메시지 항목: `user`, `assistant`, `bashExecution`, `hookMessage`, `branchSummary`, `compactionSummary`
- `custom_message` 항목
- `branch_summary` 항목

엄격한 규칙: `toolResult`에서는 절대 절단하지 않습니다.

절단 지점 바로 앞에 비메시지 메타데이터 항목(`model_change`, `thinking_level_change`, 레이블 등)이 있으면, 메시지 또는 압축 경계에 도달할 때까지 절단 인덱스를 뒤로 이동하여 유지 영역으로 포함시킵니다.

### 분할 턴 처리

절단 지점이 사용자 턴 시작 위치가 아닌 경우, 압축은 이를 분할 턴으로 처리합니다.
...
`compact(...)`는 직렬화된 대화 텍스트에서 요약을 구성합니다:
...
- `compaction.remoteEndpoint`가 설정되면, 압축은 다음을 POST합니다:
  - `{ systemPrompt, prompt }`
...
3. 요약이 있으면, `branchWithSummary(...)`를 사용하여 탐색 대상에 첨부합니다.

운영적으로 이것은 `branchSummary.enabled`가 활성화된 경우 `/tree` 플로우에 의해 주로 구동됩니다.
...
`prepareBranchEntries(...)`는 그 다음:

1. 첫 번째 패스: 이전 pi 생성 `branch_summary` 세부 정보를 포함하여 모든 요약 대상 항목에서 누적 파일 작업을 수집합니다.
2. 두 번째 패스: 최신 → 오래된 순으로 순회하며, 토큰 예산에 도달할 때까지 메시지를 추가합니다.
3. 최근 컨텍스트 보존을 우선합니다.
4. 연속성을 위해 예산 경계 근처의 대규모 요약 항목은 여전히 포함될 수 있습니다.

브랜치 요약 입력 시 압축 항목은 메시지(`compactionSummary`)로 포함됩니다.
...
3. 제공된 경우 커스텀 지침을 사용하고, 그렇지 않으면 `branch-summary.md`를 사용합니다.
...
저장된 `compactionEntry`와 `fromExtension` 플래그가 포함된 압축 후 알림.
...
- 탐색 취소
- 사용자가 요약을 요청했을 때 사용되는 커스텀 `{ summary: { summary, details } }` 제공
