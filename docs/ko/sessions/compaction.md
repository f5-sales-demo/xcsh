...
# 압축(Compaction)과 브랜치 요약

압축과 브랜치 요약은 이전 작업 컨텍스트를 잃지 않으면서 긴 세션을 사용 가능한 상태로 유지하는 두 가지 메커니즘입니다.

- **압축(Compaction)**은 현재 브랜치에서 이전 히스토리를 요약으로 재작성합니다.
- **브랜치 요약(Branch summary)**은 `/tree` 탐색 중 폐기된 브랜치의 컨텍스트를 캡처합니다.

둘 다 세션 항목으로 영속화되며, LLM 입력을 재구성할 때 사용자 컨텍스트 메시지로 변환됩니다.
...
1. 활성 경로에서 가장 최근의 압축이 하나의 `compactionSummary` 메시지로 변환됩니다.
2. `firstKeptEntryId`부터 압축 지점까지의 유지된 항목들이 다시 포함됩니다.
3. 경로상의 이후 항목들이 추가됩니다.
4. `branch_summary` 항목들이 `branchSummary` 메시지로 변환됩니다.
5. `custom_message` 항목들이 `custom` 메시지로 변환됩니다.

이러한 커스텀 역할들은 `convertToLlm()`에서 정적 템플릿을 사용하여 LLM 대상 사용자 메시지로 변환됩니다:
...
2. **자동 오버플로 복구**: 컨텍스트 오버플로와 일치하는 어시스턴트 오류 발생 후.
3. **자동 임계값 압축**: 컨텍스트가 임계값을 초과할 때 성공적인 턴 이후.
...
프루닝이 항목을 변경하면, 압축 결정 전에 세션 저장소가 재작성되고 에이전트 메시지 상태가 새로고침됩니다.

### 경계 및 절단 지점 로직

`prepareCompaction()`은 마지막 압축 항목 이후의 항목만 고려합니다(있는 경우).

1. 이전 압축 인덱스를 찾습니다.
2. `boundaryStart = prevCompactionIndex + 1`을 계산합니다.
3. 사용 가능한 측정된 사용 비율을 사용하여 `keepRecentTokens`를 조정합니다.
4. 경계 윈도우에서 `findCutPoint()`를 실행합니다.

유효한 절단 지점에는 다음이 포함됩니다:

- 다음 역할을 가진 메시지 항목: `user`, `assistant`, `bashExecution`, `hookMessage`, `branchSummary`, `compactionSummary`
- `custom_message` 항목
- `branch_summary` 항목

엄격한 규칙: `toolResult`에서는 절대 절단하지 않습니다.

절단 지점 바로 앞에 비메시지 메타데이터 항목(`model_change`, `thinking_level_change`, 레이블 등)이 있는 경우, 메시지 또는 압축 경계에 도달할 때까지 절단 인덱스를 뒤로 이동하여 유지 영역에 포함시킵니다.

### 분할 턴 처리

절단 지점이 사용자 턴 시작점에 있지 않으면, 압축은 이를 분할 턴으로 처리합니다.
...
`compact(...)`는 직렬화된 대화 텍스트에서 요약을 구축합니다:
...
6. `SUMMARIZATION_SYSTEM_PROMPT`와 함께 요약 프롬프트를 실행합니다.
...
- 이전 항목이 pi에서 생성된 경우(`fromExtension !== true`)에만 이전 압축 세부 정보를 포함합니다.
...
요약 생성(또는 훅에서 제공한 요약) 후, 에이전트 세션은:
...
브랜치 요약은 토큰 오버플로가 아닌 트리 탐색에 연결됩니다.

### 트리거

`navigateTree(...)` 중:

1. `collectEntriesForBranchSummary(...)`를 사용하여 이전 리프에서 공통 조상까지의 폐기된 항목을 계산합니다.
2. 호출자가 요약을 요청한 경우(`options.summarize`), 리프 전환 전에 요약을 생성합니다.
3. 요약이 존재하면, `branchWithSummary(...)`를 사용하여 탐색 대상에 첨부합니다.

이는 일반적으로 `branchSummary.enabled`가 활성화된 상태에서 `/tree` 플로우에 의해 구동됩니다.
...
1. 첫 번째 패스: pi에서 생성된 이전 `branch_summary` 세부 정보를 포함하여 모든 요약 대상 항목에서 누적 파일 작업을 수집합니다.
2. 두 번째 패스: 최신 → 최구순으로 이동하며, 토큰 예산에 도달할 때까지 메시지를 추가합니다.
3. 최근 컨텍스트 보존을 우선시합니다.
4. 연속성을 위해 예산 경계 근처의 대형 요약 항목을 포함할 수 있습니다.
...
3. 제공된 경우 커스텀 지시사항을, 그렇지 않으면 `branch-summary.md`를 사용합니다.
4. `SUMMARIZATION_SYSTEM_PROMPT`와 함께 요약 모델을 호출합니다.
...
저장된 `compactionEntry`와 `fromExtension` 플래그를 포함하는 압축 후 알림.
...
- 오버플로 오류는 압축에 의해 처리되므로 일반 재시도 경로에서 제외됩니다.
- 자동 압축이 실패하는 경우:
  - 오버플로 경로는 `Context overflow recovery failed: ...`를 발생시킵니다.
  - 임계값 경로는 `Auto-compaction failed: ...`를 발생시킵니다.
- 브랜치 요약은 중단 신호(예: Escape)를 통해 취소할 수 있으며, 취소됨/중단됨 탐색 결과를 반환합니다.
