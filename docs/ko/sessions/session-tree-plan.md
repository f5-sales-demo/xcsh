---
title: 세션 트리 아키텍처
description: '분기, 탐색, 부모-자식 대화 관계를 갖춘 세션 트리 아키텍처.'
sidebar:
  order: 2
  label: 트리 아키텍처
i18n:
...
이 문서는 현재 세션 트리 탐색이 어떻게 작동하는지 설명합니다: 인메모리 트리 모델, 리프 이동 규칙, 분기 동작, 확장/이벤트 통합.

## 이 서브시스템의 정의

세션은 추가 전용(append-only) 항목 로그로 저장되지만, 런타임 동작은 트리 기반입니다:

- 모든 비헤더 항목은 `id`와 `parentId`를 가집니다.
- 활성 위치는 `SessionManager`의 `leafId`입니다.
- 항목을 추가하면 항상 현재 리프의 자식으로 생성됩니다.
- 분기는 히스토리를 다시 작성하지 **않습니다**; 다음 추가 전에 리프가 가리키는 위치만 변경합니다.

주요 파일:

- `src/session/session-manager.ts` — 트리 데이터 모델, 순회, 리프 이동, 브랜치/세션 추출
- `src/session/agent-session.ts` — `/tree` 탐색 흐름, 요약, 훅/이벤트 발생
...
- `#labelsById: Map<string, string>` — 대상 항목 id별 해석된 라벨

트리 API:

- `getBranch(fromId?)`는 부모 링크를 루트까지 따라가 루트→노드 경로를 반환합니다
...
2. `resetLeaf()`
   - `leafId = null`로 설정합니다
   - 다음 추가 시 새 루트 항목을 생성합니다 (`parentId = null`)

3. `branchWithSummary(branchFromId, summary, details?, fromExtension?)`
   - `branchFromId: string | null`을 받습니다
   - `leafId = branchFromId`로 설정합니다
   - 해당 리프의 자식으로 `branch_summary` 항목을 추가합니다
   - `branchFromId`가 `null`일 때, `fromId`는 `"root"`로 저장됩니다

## `/tree` 탐색 동작 (동일 세션 파일)

`AgentSession.navigateTree()`는 파일 포크가 아닌 탐색입니다.

흐름:

1. 대상을 검증하고 폐기 경로를 계산합니다 (`collectEntriesForBranchSummary`)
2. `TreePreparation`과 함께 `session_before_tree`를 발생시킵니다
3. 선택적으로 폐기된 항목들을 요약합니다 (훅이 제공한 요약 또는 내장 요약기)
4. 새 리프 대상을 계산합니다:
   - **user** 메시지를 선택한 경우: 리프가 부모로 이동하고, 메시지 텍스트가 에디터 사전 입력으로 반환됩니다
   - **custom_message**를 선택한 경우: user 메시지와 동일한 규칙 (리프 = 부모, 텍스트가 에디터에 사전 입력)
   - 다른 항목을 선택한 경우: 리프 = 선택한 항목 id
5. 리프 이동을 적용합니다:
   - 요약이 있는 경우: `branchWithSummary(newLeafId, ...)`
   - 요약이 없고 `newLeafId === null`인 경우: `resetLeaf()`
   - 그 외: `branch(newLeafId)`
6. 새 리프에서 에이전트 컨텍스트를 재구성하고 `session_tree`를 발생시킵니다

중요: 요약 항목은 폐기된 브랜치 끝이 아닌 **새 탐색 위치**에 첨부됩니다.
...
- `/branch`는 새 세션 브랜치 파일을 생성합니다 (또는 비영속 모드에서는 인메모리 교체).

사용자 대상 `/branch` 흐름 (`SelectorController.showUserMessageSelector` → `AgentSession.branch`):

- 브랜치 소스는 **user 메시지**여야 합니다.
- 선택한 사용자 텍스트가 에디터 사전 입력용으로 추출됩니다.
- 선택한 user 메시지가 루트인 경우 (`parentId === null`): `newSession({ parentSession: previousSessionFile })`을 통해 새 세션을 시작합니다.
- 그 외: `createBranchedSession(selectedEntry.parentId)`를 사용하여 선택한 프롬프트 경계까지의 히스토리를 포크합니다.

`SessionManager.createBranchedSession(leafId)` 세부사항:

- `getBranch(leafId)`를 통해 루트→리프 경로를 구성합니다; 없으면 예외를 발생시킵니다.
- 복사 경로에서 기존 `label` 항목을 제외합니다.
- 경로에 남아있는 항목에 대해 해석된 `labelsById`에서 새 라벨 항목을 재구성합니다.
- 영속 모드: 새 JSONL 파일을 작성하고 매니저를 해당 파일로 전환합니다; 새 파일 경로를 반환합니다.
- 인메모리 모드: 인메모리 항목을 교체합니다; `undefined`를 반환합니다.

## 컨텍스트 재구성 및 요약/커스텀 통합

`buildSessionContext()` (`session-manager.ts` 내)는 활성 루트→리프 경로를 해석하고 유효한 LLM 컨텍스트 상태를 구성합니다:

- 경로상의 최신 thinking/model/mode/ttsr 상태를 추적합니다.
- 경로상의 최신 컴팩션을 처리합니다:
  - 먼저 컴팩션 요약을 발생시킵니다
  - `firstKeptEntryId`에서 컴팩션 지점까지 유지된 메시지를 재생합니다
  - 그 다음 컴팩션 이후 메시지를 재생합니다
- `branch_summary` 및 `custom_message` 항목을 `AgentMessage` 객체로 포함합니다.
...
따라서 트리 이동은 기존 항목을 변경하는 것이 아니라 활성 리프 경로를 변경함으로써 컨텍스트를 변경합니다.

## 라벨과 트리 UI 동작

라벨 영속성:

- `appendLabelChange(targetId, label?)`는 현재 리프 체인에 `label` 항목을 작성합니다.
- `labelsById`는 즉시 업데이트됩니다 (설정 또는 삭제).
- `getTree()`는 반환된 각 노드에 현재 라벨을 해석하여 적용합니다.

트리 셀렉터 동작 (`tree-selector.ts`):

- 탐색을 위해 트리를 평탄화하고, 활성 경로 강조를 유지하며, 활성 브랜치를 우선 표시합니다.
- 필터 모드를 지원합니다: `default`, `no-tools`, `user-only`, `labeled-only`, `all`.
- 렌더링된 의미 콘텐츠에 대한 자유 텍스트 검색을 지원합니다.
- `Shift+L`로 인라인 라벨 편집을 열고 `appendLabelChange`를 통해 작성합니다.

명령 라우팅:

- `/tree`는 항상 트리 셀렉터를 엽니다.
- `/branch`는 `doubleEscapeAction=tree`가 아닌 한 user 메시지 셀렉터를 열며, 해당 경우에는 트리 셀렉터 UX도 사용합니다.
...
- `branch()`는 `null`을 대상으로 할 수 없습니다; 첫 번째 항목 이전의 루트 상태를 위해서는 `resetLeaf()`를 사용하세요.
...
마이그레이션 후 현재 런타임 동작은 버전 3 트리 의미론을 따릅니다.
