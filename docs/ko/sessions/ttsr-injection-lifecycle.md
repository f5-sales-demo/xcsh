...
이 문서는 규칙 탐색부터 스트림 중단, 재시도 주입, 확장 알림, 세션 상태 처리까지의 현재 Time Traveling Stream Rules (TTSR) 런타임 경로를 다룹니다.
...
`loadCapability("rules")`는 `rule.name` 기준으로 선착순 의미론(higher provider priority first)에 따라 중복을 제거합니다. 가려진 중복 항목은 TTSR 등록 전에 제거됩니다.
...
- 동일한 `rule.name`을 가진 규칙이 이미 이 매니저에 등록된 경우
- 정규식 컴파일에 실패한 경우 (`new RegExp(rule.ttsrTrigger)`가 예외를 던지는 경우)

유효하지 않은 정규식 트리거는 경고로 로깅되고 무시됩니다; 세션 시작은 계속됩니다.

### 설정 관련 주의사항

`TtsrSettings.enabled`는 매니저에 로드되지만 현재 런타임 게이팅에서는 확인되지 않습니다. 규칙이 존재하면 매칭이 여전히 실행됩니다.
...
`check()`는 등록된 규칙을 순회하며 반복 정책(`#canTrigger`)을 통과하는 모든 일치 규칙을 반환합니다.
...
2. 일치된 규칙이 `#pendingTtsrInjections`에 큐잉됩니다.
3. `#ttsrAbortPending = true`.
4. `agent.abort()`가 즉시 호출됩니다.
5. `ttsr_triggered` 이벤트가 비동기적으로 발생합니다 (fire-and-forget).
6. 재시도 작업이 `setTimeout(..., 50)`을 통해 예약됩니다.

중단은 확장 콜백을 기다리지 않습니다.

## 4. 재시도 예약, 컨텍스트 모드, 리마인더 주입
...
`messageCount`는 `turn_end`에서 증가하므로, 간격은 스트림 청크가 아닌 완료된 턴으로 측정됩니다.

## 6. 이벤트 발생 및 확장/훅 인터페이스
...
대화형 모드는 `session.isTtsrAbortPending`을 사용하여 TTSR 중단 중에 중단된 어시스턴트 중지 사유가 표시되는 실패로 나타나는 것을 억제하며, 이벤트가 도착하면 `TtsrNotificationComponent`를 렌더링합니다.

## 7. 지속성 및 재개 상태 (현재 구현)

`SessionManager`는 주입된 규칙 지속성에 대한 전체 스키마를 지원합니다:
...
- `AgentSession`은 TTSR가 트리거될 때 `ttsr_injection` 항목을 추가하지 않습니다.
- `createAgentSession()`은 `existingSession.injectedTtsrRules`를 `ttsrManager`로 다시 복원하지 않습니다.

결과: 주입된 규칙 억제는 라이브 프로세스의 인메모리에서 적용되지만, 이 경로를 통한 세션 리로드/재개 시에는 현재 지속/복원되지 않습니다.
...
- 확장 알림은 비동기적이며 중단/재시도 예약 전에 의도적으로 대기하지 않습니다

### 같은 스트림 윈도우에서의 다중 매치

`check()`는 현재 일치하는 모든 적격 규칙을 반환합니다. 이들은 다음 재시도 메시지에서 일괄 주입됩니다.

### 중단과 계속 사이

타이머 윈도우 동안 상태가 변경될 수 있습니다 (사용자 중단, 모드 액션, 추가 이벤트). 재시도 호출은 최선 노력 방식입니다: `agent.continue().catch(() => {})`가 후속 오류를 무시합니다.

## 9. 엣지 케이스 요약

- 유효하지 않은 `ttsr_trigger` 정규식: 경고와 함께 건너뛰어짐; 다른 규칙은 계속됩니다.
- 기능 계층에서의 중복 규칙 이름: 낮은 우선순위의 중복은 등록 전에 가려집니다.
- 매니저 계층에서의 중복 이름: 두 번째 등록은 무시됩니다.
- `contextMode: "keep"`: 부분적으로 위반하는 출력이 리마인더 재시도 전에 컨텍스트에 남을 수 있습니다.
- 간격 후 재트리거는 `turn_end`에서의 턴 카운트 증가에 의존합니다; 턴 중간 청크는 간격 카운터를 진행시키지 않습니다.
