...
4. `agentLoop` (`packages/agent/src/agent-loop.ts`)는 해당 이벤트를 소비하고, 진행 중인 어시스턴트 상태를 변경하며, 원시 `assistantMessageEvent`를 포함하는 `message_update` 이벤트를 발행합니다.
5. `AgentSession` (`packages/coding-agent/src/session/agent-session.ts`)은 에이전트 이벤트를 구독하고, 메시지를 영속화하며, 확장 훅을 구동하고, 세션 동작(재시도, 압축, TTSR, 스트리밍 편집 중단 검사)을 적용합니다.
...
- 최종 결과는 종단 이벤트(`done` 또는 `error`)에 의해 해결됨
- 델타는 배치/스로틀링됨(~50ms)
...
이는 TUI/이벤트 소비자를 위해 고빈도 프로바이더 스트림을 평활화하지만, 프로바이더 백프레셔는 아닙니다: 프로바이더는 여전히 최대 속도로 생산하며, 로컬 스트림이 버퍼링합니다.
...
- `message_start`는 사용량(입력/출력/캐시 토큰)을 초기화
- `content_block_start`는 텍스트/사고/도구호출 시작에 매핑
- `content_block_delta` 매핑:
  - `text_delta` → `text_delta`
  - `thinking_delta` → `thinking_delta`
  - `input_json_delta` → `toolcall_delta`
  - `signature_delta`는 `thinkingSignature`만 업데이트 (이벤트 없음)
- `content_block_stop`은 해당하는 `*_end`를 발행
- `message_delta.stop_reason`은 `mapStopReason()`을 통해 매핑

도구 호출 인자 스트리밍:

- 각 도구 블록은 내부 `partialJson`을 보유
- 모든 JSON 델타는 `partialJson`에 추가됨
- `arguments`는 매 델타마다 `parseStreamingJson()`을 통해 재파싱됨
- `toolcall_end`는 한 번 더 재파싱 후 `partialJson`을 제거
...
- `response.output_item.added`는 추론/텍스트/함수호출 블록을 시작
- 추론 요약 이벤트(`response.reasoning_summary_text.delta`)는 `thinking_delta`가 됨
- 출력/거절 델타는 `text_delta`가 됨
- `response.function_call_arguments.delta`는 `toolcall_delta`가 됨
- `response.output_item.done`은 `thinking_end` / `text_end` / `toolcall_end`를 발행
- `response.completed`는 상태를 중지 사유 및 사용량에 매핑

도구 호출 인자 스트리밍:

- Anthropic과 동일한 `partialJson` 축적 패턴
- `response.function_call_arguments.done`만 보내는 프로바이더도 최종 인자를 채움
...
- `candidate.content.parts`를 순회
- 텍스트 파트는 `isThinkingPart(part)`에 의해 사고 vs 텍스트로 분리
- 블록 전환 시 새 블록을 시작하기 전에 이전 블록을 닫음
- `part.functionCall`은 완전한 도구 호출로 처리됨 (start/delta/end가 즉시 발행)
- 종료 사유는 `google-shared.ts`의 `mapStopReason()`에 의해 매핑

도구 호출 인자 스트리밍:

- 함수 호출 인자는 점진적 JSON 텍스트가 아닌 구조화된 객체로 도착
- 구현은 `JSON.stringify(arguments)`를 포함하는 하나의 합성 `toolcall_delta`를 발행
- 이 경로에서는 Google용 부분 JSON 파서가 필요 없음

## 부분 도구 호출 JSON 축적 및 복구

Anthropic/OpenAI Responses의 공유 동작은 `parseStreamingJson()` (`packages/ai/src/utils/json-parse.ts`)를 사용합니다:
...
- 잘못되거나 잘린 인자 델타가 스트림 처리를 즉시 크래시시키지 않음
- 진행 중인 `arguments`는 일시적으로 `{}`일 수 있음
- 이후 유효한 델타가 구조화된 인자를 복구할 수 있음 (매 추가마다 파싱이 재시도되므로)
- 최종 `toolcall_end`는 발행 전에 한 번 더 파싱을 시도

## 중지 사유 vs 전송/런타임 오류

프로바이더 중지 사유는 정규화된 `stopReason`에 매핑됩니다:

- Anthropic: `end_turn`→`stop`, `max_tokens`→`length`, `tool_use`→`toolUse`, 안전/거절 케이스→`error`
- OpenAI Responses: `completed`→`stop`, `incomplete`→`length`, `failed/cancelled`→`error`
- Google: `STOP`→`stop`, `MAX_TOKENS`→`length`, 안전/금지/잘못된-함수-호출 클래스→`error`

오류 의미론은 두 단계로 분리됩니다:

1. **모델 완료 의미론** (프로바이더가 보고하는 종료 사유/상태)
2. **전송/런타임 실패** (네트워크/클라이언트/파서/중단 예외)

프로바이더 스트림이 예외를 던지거나 실패를 신호하면, 각 프로바이더 래퍼가 이를 캐치하고 다음과 함께 종단 `error` 이벤트를 발행합니다:
...
- SDK 수준의 잘못된 청크/SSE 파싱은 예외 또는 스트림 `error` 이벤트로 표면화됨
- 프로바이더 래퍼가 이를 통합 종단 `error` 이벤트로 변환
- 스트림 함수 자체 내부에 프로바이더별 재개/재시도 없음
- 상위 수준 재시도는 `AgentSession` 자동 재시도 로직에서 처리됨 (스트림 청크 리플레이가 아닌 메시지 수준 재시도)
...
- 프로바이더 래퍼: 스트림 루프 이후, 중단된 신호는 오류 경로를 강제함 (`"Request was aborted"`).
- 에이전트 루프: 각 프로바이더 이벤트를 처리하기 전에 `signal.aborted`를 확인하며, 최신 부분 데이터로부터 중단된 어시스턴트 메시지를 합성할 수 있음.
...
- 조향 인터럽트는 이미 생성된 도구 결과를 보존하면서 나머지 도구 실행을 중단할 수 있음

## 백프레셔 경계

프로바이더 SDK 스트림과 다운스트림 소비자 간에 하드 백프레셔 메커니즘은 없습니다:

- `EventStream`은 최대 크기 제한 없는 인메모리 큐를 사용
- 스로틀링은 UI 업데이트 속도를 줄이지만 프로바이더 수신 속도를 늦추지 않음
- 소비자가 상당히 지연되면, 대기 중인 이벤트가 완료될 때까지 증가할 수 있음

현재 설계는 바운딩된 버퍼 흐름 제어보다 응답성과 단순한 순서 보장을 우선시합니다.

## 스트림 이벤트가 에이전트/세션 이벤트로 표면화되는 방식

`agentLoop.streamAssistantResponse()`는 `AssistantMessageEvent`를 `AgentEvent`로 브릿지합니다:
...
- 종단(`done`/`error`) 시: `response.result()`에서 최종 메시지를 해결하고, `message_end`를 발행

`AgentSession`은 이후 세션 수준 동작을 위해 해당 이벤트를 소비합니다:

- TTSR은 `text_delta`와 `toolcall_delta`에 대해 `message_update.assistantMessageEvent`를 감시
- 스트리밍 편집 가드는 `edit` 호출 시 `toolcall_delta`/`toolcall_end`를 검사하고 조기 중단할 수 있음
- 영속화는 `message_end`에서 확정된 메시지를 기록
- 자동 재시도는 어시스턴트 `stopReason === "error"`와 `errorMessage` 휴리스틱을 검사
...
- [`../../ai/src/providers/anthropic.ts`](../../packages/ai/src/providers/anthropic.ts) — Anthropic 이벤트 변환 및 도구 JSON 델타 축적.
...
- [`../../agent/src/agent-loop.ts`](../../packages/agent/src/agent-loop.ts) — 프로바이더 스트림 소비 및 `message_update` 브릿지.
- [`../src/session/agent-session.ts`](../../packages/coding-agent/src/session/agent-session.ts) — 스트리밍 업데이트, 중단, 재시도, 영속화의 세션 수준 처리.
