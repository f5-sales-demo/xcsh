...
description: 'IPython 커널 관리, 실행 및 출력 캡처 기능을 갖춘 Python REPL 도구 런타임.'
sidebar:
  order: 3
...
도구 동작, 커널/게이트웨이 생명주기, 환경 처리, 실행 의미론, 출력 렌더링 및 운영 장애 모드를 다룹니다.
...
- 사용자 트리거 Python 실행을 위한 대화형 모드 렌더러: `src/modes/components/python-execution.ts`
- 런타임/환경 필터링 및 Python 해석: `src/ipy/runtime.ts`

## Python 도구란

`python` 도구는 Jupyter Kernel Gateway 기반 커널을 통해 하나 이상의 Python 셀을 실행합니다(셀마다 `python -c`를 직접 스폰하지 않습니다).
...
```ts
{
  ...
  cwd?: string;
  reset?: boolean; // 첫 번째 셀 실행 전에만 커널 리셋
}
```
...
1. **외부 게이트웨이** (`PI_PYTHON_GATEWAY_URL` 설정 시)
   - 구성된 URL을 직접 사용합니다.
   - `PI_PYTHON_GATEWAY_TOKEN`으로 선택적 인증을 수행합니다.
   - 로컬 게이트웨이 프로세스를 스폰하거나 관리하지 않습니다.

2. **로컬 공유 게이트웨이** (기본 경로)
   - `~/.xcsh/agent/python-gateway` 하위에서 조정되는 단일 공유 프로세스를 사용합니다.
...
- PID가 살아있고 상태 확인이 통과하면 `gateway.json`을 재사용합니다.
...
6. 확장 모듈 로드:
   ...
   - 프로젝트: `<cwd>/.xcsh/modules/*.py` (동일 이름의 사용자 모듈을 오버라이드)

커널 종료:

- `DELETE /api/kernels/:id`를 통해 원격 커널 삭제
- 웹소켓 닫기
- 공유 게이트웨이 해제 후크 호출 (현재 no-op)
...
- `session` (기본값)
  - 세션 ID + cwd를 키로 커널 세션을 재사용합니다.
  - 실행은 큐를 통해 세션당 직렬화됩니다.
  - 유휴 세션은 5분 후 제거됩니다.
  - 최대 4개 세션; 초과 시 가장 오래된 세션이 제거됩니다.
  - 하트비트 확인으로 죽은 커널을 감지합니다.
  - 자동 재시작은 1회 허용; 반복 크래시 시 하드 실패.
...
셀은 해당 도구 호출의 동일한 커널 인스턴스에서 순차적으로 실행됩니다.

중간 셀이 실패하면:

- 이전 셀 상태는 메모리에 남아 있습니다.
- 도구는 어떤 셀이 실패했는지 나타내는 특정 오류를 반환합니다.
- 이후 셀은 실행되지 않습니다.
...
게이트웨이/커널 런타임을 실행하기 전에 환경이 필터링됩니다:

- 허용 목록에는 `PATH`, `HOME`, 로캘 변수, `VIRTUAL_ENV`, `PYTHONPATH` 등 핵심 변수가 포함됩니다.
- 허용 접두사: `LC_`, `XDG_`, `PI_`
- 차단 목록은 일반적인 API 키(OpenAI/Anthropic/Gemini 등)를 제거합니다.

런타임 선택 순서:

1. 활성/탐지된 venv (`VIRTUAL_ENV`, 그다음 `<cwd>/.venv`, `<cwd>/venv`)
2. `~/.xcsh/python-env`의 관리형 venv
3. PATH의 `python` 또는 `python3`

venv가 선택되면 해당 bin/Scripts 경로가 `PATH` 앞에 추가됩니다.

Python 내부 커널 환경 초기화도 다음을 수행합니다:
...
`python.toolMode` (기본값 `both`) + 선택적 `PI_PY` 오버라이드로 노출을 제어합니다:
...
- 실행이 취소됨으로 표시됩니다.
- REST(`POST /interrupt`) 및 제어 채널 `interrupt_request`를 통해 커널 인터럽트를 시도합니다.
- 결과에 `cancelled=true`가 포함됩니다.
- 타임아웃 경로는 출력에 `Command timed out after <n> seconds`로 주석을 추가합니다.

### stdin 동작

대화형 stdin은 지원되지 않습니다.
...
출력은 `OutputSink`를 통해 스트리밍되며 아티팩트 스토리지에 저장될 수 있습니다.
...
- 도구 렌더러 (`python.ts`):
  ...
  - 접힌 미리보기는 기본 10줄입니다
  - 전체 출력과 더 풍부한 상태 세부 정보를 위한 확장 모드를 지원합니다
- 대화형 렌더러 (`python-execution.ts`):
...
- **커널 가용성 오류**
  - 로컬 모드에서는 해석된 Python 런타임에서 `kernel_gateway`와 `ipykernel` 모두 가져올 수 있어야 합니다.
  - 다음으로 설치하세요:
...
- **실행이 멈춘 후 타임아웃**
  ...
  - 멈춘 코드의 경우, 취소가 커널 인터럽트를 트리거하지만 사용자 코드는 여전히 리팩토링이 필요할 수 있습니다.

- **Python 코드에서 stdin/input 프롬프트**
  - 이 런타임 경로에서는 `input()`이 대화형으로 지원되지 않습니다; 데이터를 프로그래밍 방식으로 전달하세요.
