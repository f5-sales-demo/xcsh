...
description: 'IPython 커널 관리, 실행 및 출력 캡처를 지원하는 Python REPL 도구 런타임.'
sidebar:
  order: 3
...
`python` 도구는 하나 이상의 Python 셀을 Jupyter Kernel Gateway 기반 커널을 통해 실행합니다 (셀마다 직접 `python -c`를 스폰하는 방식이 아닙니다).
...
- PID가 활성 상태이고 헬스 체크를 통과하면 `gateway.json`을 재사용합니다.
...
- `session` (기본값)
  ...
  - 유휴 세션은 5분 후 퇴거됩니다.
  - 최대 4개 세션; 오버플로 시 가장 오래된 세션이 퇴거됩니다.
  - 하트비트 체크로 죽은 커널을 감지합니다.
  - 자동 재시작은 1회 허용; 반복 크래시 시 하드 실패가 발생합니다.
...
- 이전 셀 상태는 메모리에 유지됩니다.
- 도구는 어떤 셀이 실패했는지 나타내는 대상 오류를 반환합니다.
- 이후 셀은 실행되지 않습니다.
...
- 허용 목록에는 `PATH`, `HOME`, 로케일 변수, `VIRTUAL_ENV`, `PYTHONPATH` 등의 핵심 변수가 포함됩니다.
- 허용 접두사: `LC_`, `XDG_`, `PI_`
- 거부 목록은 일반적인 API 키를 제거합니다 (OpenAI/Anthropic/Gemini 등)

런타임 선택 순서:

1. 활성/위치 확인된 venv (`VIRTUAL_ENV`, 이후 `<cwd>/.venv`, `<cwd>/venv`)
2. `~/.xcsh/python-env`의 관리형 venv
3. PATH의 `python` 또는 `python3`

venv가 선택되면 해당 bin/Scripts 경로가 `PATH` 앞에 추가됩니다.

Python 내부 커널 환경 초기화도 수행됩니다:
...
Python 사전 검사가 실패하면 해당 세션에서 도구 생성이 bash-only로 저하됩니다.
...
출력은 `OutputSink`를 통해 스트리밍되며 아티팩트 스토리지에 영속화될 수 있습니다.
...
- 인터랙티브 렌더러 (`python-execution.ts`):
  ...
  - 접힌 미리보기는 기본 20줄입니다
  - 디스플레이 안전을 위해 매우 긴 개별 줄을 4000자로 제한합니다
  - 취소/오류/잘림 알림을 표시합니다
...
- **`python.sharedGateway=false`가 시작 실패를 유발**
  - 이는 현재 구현에서 예상된 동작입니다.
...
- **실행이 멈춘 후 타임아웃**
  - 워크로드가 정당한 경우 도구 `timeout`을 늘리세요 (최대 600초).
  - 멈춘 코드의 경우 취소가 커널 인터럽트를 트리거하지만 사용자 코드를 리팩토링해야 할 수 있습니다.

- **Python 코드에서 stdin/입력 프롬프트**
  - `input()`은 이 런타임 경로에서 인터랙티브하게 지원되지 않습니다; 데이터를 프로그래밍 방식으로 전달하세요.

- **리소스 소진 (`EMFILE` / 열린 파일 수 초과)**
  - 세션 관리자가 공유 게이트웨이 복구를 트리거합니다 (세션 해제 + 공유 게이트웨이 재시작).

- **작업 디렉토리 오류**
  - 도구는 실행 전에 `cwd`가 존재하고 디렉토리인지 검증합니다.
