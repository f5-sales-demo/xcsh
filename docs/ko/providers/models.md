---
title: 모델 및 프로바이더 설정
description: 'models.yml을 통한 모델 레지스트리 및 프로바이더 설정으로 라우팅, 폴백, 가격 정보를 포함합니다.'
sidebar:
  order: 1
  label: 모델 및 프로바이더
i18n:
...
- `src/config/model-registry.ts` — 내장 + 사용자 정의 모델 로드, 프로바이더 오버라이드, 런타임 디스커버리, 인증 통합
...
- `models.yml`이 없고 동일 위치에 `models.json`이 있으면 `models.yml`로 마이그레이션됩니다.
- `ModelRegistry`에 프로그래밍 방식으로 전달할 때 명시적 `.json` / `.jsonc` 설정 경로도 여전히 지원됩니다.

## `models.yml` 구조

```yaml
configVersion: 1  # 선택 사항 — auto-config에 의해 작성되며, 마이그레이션 감지에 사용됨
providers:
  <provider-id>:
    # 프로바이더 수준 설정
equivalence:
...
`configVersion`은 auto-config 시스템에 의해 작성되는 선택적 정수입니다. 이 값이 있으면 xcsh는 이를 사용하여 오래된 설정을 감지하고 자동으로 업그레이드합니다.

`provider-id`는 선택 및 인증 조회 전반에서 사용되는 정규 프로바이더 키입니다.

`equivalence`는 선택 사항이며, 구체적인 프로바이더 모델 위에 정규 모델 그룹화를 설정합니다:

- `overrides`는 정확한 구체적 셀렉터(`provider/modelId`)를 공식 업스트림 정규 ID에 매핑합니다
- `exclude`는 구체적 셀렉터를 정규 그룹화에서 제외합니다
...
### 완전한 사용자 정의 프로바이더 (`models`가 비어있지 않은 경우)
...
- `discovery`는 프로바이더 수준의 `api`가 필요합니다.

### 모델 값 검사

- `id` 필수
- `contextWindow`와 `maxTokens`는 제공된 경우 양수여야 합니다
...
2. `models.yml` 사용자 정의 설정을 로드합니다.
3. 내장 모델에 프로바이더 오버라이드(`baseUrl`, `headers`)를 적용합니다.
4. `modelOverrides`를 적용합니다 (프로바이더 + 모델 ID별).
5. 사용자 정의 `models`를 병합합니다:
   - 동일한 `provider + id`는 기존 항목을 대체합니다
   - 그렇지 않으면 추가합니다
6. 런타임에서 디스커버리된 모델(현재 Ollama 및 LM Studio)을 적용한 후, 모델 오버라이드를 다시 적용합니다.

## 정규 모델 동등성 및 통합

레지스트리는 모든 구체적 프로바이더 모델을 유지한 다음, 그 위에 정규 레이어를 구축합니다.

정규 ID는 공식 업스트림 ID만 해당됩니다. 예를 들어:
...
2. 내장 모델 메타데이터에서 번들된 공식 ID 매칭
3. 게이트웨이/프로바이더 변형에 대한 보수적 휴리스틱 정규화
4. 구체적 모델 자체 ID로 폴백

현재 휴리스틱은 의도적으로 좁게 설정되어 있습니다:

- 내장된 업스트림 접두사가 있는 경우 제거할 수 있습니다. 예를 들어 `anthropic/...` 또는 `openai/...`
- 점과 대시로 구분된 버전 변형은 기존 공식 ID에 매핑되는 경우에만 정규화할 수 있습니다. 예를 들어 `4.6 -> 4-6`
- 모호한 패밀리나 버전은 번들된 매칭이나 명시적 오버라이드 없이는 병합되지 않습니다

### 정규 해석 동작

여러 구체적 변형이 동일한 정규 ID를 공유하는 경우, 해석은 다음을 사용합니다:

1. 가용성 및 인증
2. `config.yml`의 `modelProviderOrder`
3. `modelProviderOrder`가 설정되지 않은 경우 기존 레지스트리/프로바이더 순서

비활성화되었거나 인증되지 않은 프로바이더는 건너뜁니다.

세션 상태와 트랜스크립트는 실제로 턴을 실행한 구체적 프로바이더/모델을 계속 기록합니다.

프로바이더 기본값 vs 모델별 오버라이드:

- 프로바이더 `headers`는 기본값입니다.
...
`ollama`가 명시적으로 설정되지 않은 경우, 레지스트리는 암시적 디스커버리 가능 프로바이더를 추가합니다:
...
`llama.cpp`가 명시적으로 설정되지 않은 경우, 레지스트리는 암시적 디스커버리 가능 프로바이더를 추가합니다:
...
`lm-studio`가 명시적으로 설정되지 않은 경우, 레지스트리는 암시적 디스커버리 가능 프로바이더를 추가합니다:
...
확장은 런타임에 프로바이더를 등록할 수 있으며(`pi.registerProvider(...)`), 다음을 포함합니다:

- 프로바이더에 대한 모델 교체/추가
- 새 API ID에 대한 사용자 정의 스트림 핸들러 등록
- 사용자 정의 OAuth 프로바이더 등록

## 인증 및 API 키 해석 순서

프로바이더에 대한 키를 요청할 때 유효한 순서는 다음과 같습니다:
...
5. ModelRegistry 폴백 리졸버 (`models.yml`의 프로바이더 `apiKey`, 환경변수명 또는 리터럴 의미론)

`models.yml` `apiKey` 동작:

- 값은 먼저 환경 변수 이름으로 처리됩니다.
- 해당 환경 변수가 없으면 리터럴 문자열이 토큰으로 사용됩니다.

`authHeader: true`이고 프로바이더 `apiKey`가 설정된 경우, 모델에 다음이 주입됩니다:

- `Authorization: Bearer <해석된-키>` 헤더.
...
- `getAll()`은 로드된 모델 레지스트리(내장 + 병합된 사용자 정의 + 디스커버리된)를 반환합니다.
- `getAvailable()`은 키 없거나 해석 가능한 인증이 있는 모델로 필터링합니다.

따라서 모델이 레지스트리에 존재하더라도 인증이 사용 가능해질 때까지 선택할 수 없을 수 있습니다.
...
`pi/smol`과 같은 역할 별칭은 `settings.modelRoles`를 통해 확장됩니다. 각 역할 값은 `:minimal`, `:low`, `:medium`, 또는 `:high`와 같은 사고 셀렉터를 추가할 수도 있습니다.

역할이 다른 역할을 가리키는 경우, 대상 모델은 여전히 정상적으로 상속되며 참조하는 역할의 명시적 접미사가 해당 역할 특정 사용에서 우선합니다.
...
- `providers.openaiWebsockets` (`auto|off|on` OpenAI Codex 전송에 대한 웹소켓 기본 설정)

`modelRoles`는 다음 중 하나를 저장할 수 있습니다:

- 구체적 프로바이더 변형을 고정하는 `provider/modelId`
...
두 인터페이스 모두 프로바이더 접두사가 붙은 모델을 표시하고 선택할 수 있습니다.
...
정규 항목을 선택하면 정규 셀렉터가 저장됩니다. 프로바이더 행을 선택하면 명시적 `provider/modelId`가 저장됩니다.

## 컨텍스트 프로모션 (모델 수준 폴백 체인)

컨텍스트 프로모션은 API가 컨텍스트 길이 오류로 요청을 거부할 때 자동으로 더 큰 컨텍스트의 형제 모델로 프로모션하는, 소규모 컨텍스트 변형(예: `*-spark`)을 위한 오버플로우 복구 메커니즘입니다.

### 트리거 및 순서

턴이 컨텍스트 오버플로우 오류(예: `context_length_exceeded`)로 실패하면, `AgentSession`은 컴팩션으로 폴백하기 **전에** 프로모션을 시도합니다:

1. `contextPromotion.enabled`가 true이면, 프로모션 대상을 해석합니다 (아래 참조).
2. 대상이 발견되면, 해당 모델로 전환하고 요청을 재시도합니다 — 컴팩션이 필요하지 않습니다.
3. 사용 가능한 대상이 없으면, 현재 모델에서 자동 컴팩션으로 넘어갑니다.
...
2. 동일 프로바이더 + API에서 가장 작은 큰 컨텍스트 모델

자격 증명이 해석되지 않는 후보(`ModelRegistry.getApiKey(...)`)는 무시됩니다.

### OpenAI Codex 웹소켓 핸드오프

`openai-codex-responses`에서/로 전환할 때, 세션 프로바이더 상태 키 `openai-codex-responses`는 모델 전환 전에 닫힙니다. 이렇게 하면 웹소켓 전송 상태가 삭제되어 다음 턴이 프로모션된 모델에서 새로 시작됩니다.
...
동일 프로바이더에서 Spark -> 비-Spark에 대한 예시 (`models.yml`):
...
내장 모델 생성기도 동일 프로바이더의 기본 모델이 존재하는 경우 `*-spark` 모델에 자동으로 이를 할당합니다.
...
이들은 OpenAI-completions 전송 로직에 의해 소비되며 URL 기반 자동 감지와 결합됩니다.
...
`LITELLM_BASE_URL`과 `LITELLM_API_KEY` 환경 변수가 모두 설정되면, xcsh는 LiteLLM 프록시에 대한 `models.yml` 설정을 자동으로 관리합니다.
...
합리적인 이미지 프로바이더 설정이 포함된 기본 `config.yml`도 함께 생성됩니다.

### 시작 시 자가 복구

매 시작 시, 모델 레지스트리의 `startupHealthCheck()`가 다음 검사를 실행합니다:

| 조건 | 동작 |
|------|------|
| `models.yml` 없음 | 환경 변수에서 자동 생성 |
| `models.yml` 손상 또는 파싱 불가 | `.bak`으로 백업 후 재생성 |
| `baseUrl`이 `LITELLM_BASE_URL`과 불일치 | `.bak`으로 백업 후 새 URL로 재생성 |
| `configVersion` 없음 또는 오래됨 | `.bak`으로 백업 후 현재 버전으로 재생성 |
| 설정이 정상임 | 동작 없음 |

모든 복구 작업은 덮어쓰기 전에 `.bak` 백업을 생성합니다. 모든 작업은 멱등적입니다.

### CLI 명령

```bash
xcsh setup litellm              # LiteLLM 설정 생성 또는 수정
...
| `LITELLM_API_KEY` | 프록시의 API 키. 생성된 설정에서 이름으로 참조되며, 런타임에 해석됩니다. |

어느 한 변수라도 설정되지 않으면, 자동 설정은 조용히 건너뜁니다.
...
- `LITELLM_BASE_URL`과 `LITELLM_API_KEY`가 설정되어 있으면, 시작 상태 검사가 자동 복구를 시도합니다 (손상된 파일 백업, 환경 변수에서 재생성). 복구에 성공하면 레지스트리는 수정된 설정을 다시 로드합니다.
- 자동 복구가 불가능한 경우 (환경 변수 미설정, 쓰기 실패), 레지스트리는 내장 모델로 계속 동작합니다.
