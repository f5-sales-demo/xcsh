...
이 문서는 coding-agent가 현재 모델을 로드하고, 오버라이드를 적용하며, 자격 증명을 해석하고, 런타임에 모델을 선택하는 방식을 설명합니다.
...
- `src/config/settings-schema.ts` — 모델 관련 설정 (`modelRoles`, 프로바이더 전송 프리퍼런스)
...
- `models.yml`이 없고 동일한 위치에 `models.json`이 존재하면 `models.yml`로 마이그레이션됩니다.
- 프로그래밍 방식으로 `ModelRegistry`에 전달할 때 명시적 `.json` / `.jsonc` 설정 경로도 여전히 지원됩니다.
...
`provider-id`는 선택 및 인증 조회 전반에 사용되는 표준 프로바이더 키입니다.

`equivalence`는 선택 사항이며 구체적인 프로바이더 모델 위에 표준 모델 그룹화를 설정합니다:

- `overrides`는 정확한 구체적 셀렉터(`provider/modelId`)를 공식 업스트림 표준 ID에 매핑합니다
- `exclude`는 구체적 셀렉터를 표준 그룹화에서 제외합니다
...
### 오버라이드 전용 프로바이더 (`models`가 없거나 비어있음)
...
- `id` 필수
- `contextWindow`과 `maxTokens`는 제공될 경우 양수여야 합니다
...
5. 커스텀 `models`를 병합합니다:
   - 동일한 `provider + id`는 기존 항목을 교체합니다
   - 그 외의 경우 추가합니다
6. 런타임 디스커버리 모델(현재 Ollama 및 LM Studio)을 적용한 후 모델 오버라이드를 다시 적용합니다.

## 표준 모델 동등성 및 통합

레지스트리는 모든 구체적인 프로바이더 모델을 유지한 다음 그 위에 표준 레이어를 구축합니다.

표준 ID는 공식 업스트림 ID만 해당합니다. 예를 들어:
...
- 포함된 업스트림 접두사가 존재할 경우 제거될 수 있습니다. 예: `anthropic/...` 또는 `openai/...`
- 점 및 대시 버전 변형은 기존 공식 ID에 매핑될 때만 정규화될 수 있습니다. 예: `4.6 -> 4-6`
- 모호한 패밀리나 버전은 번들 매칭이나 명시적 오버라이드 없이는 병합되지 않습니다

### 표준 해석 동작

여러 구체적 변형이 동일한 표준 ID를 공유할 때 해석은 다음을 사용합니다:
...
런타임 디스커버리는 모델을 가져와(`GET /models`) 로컬 기본값으로 모델 항목을 합성합니다.
...
확장은 런타임에 프로바이더를 등록할 수 있습니다(`pi.registerProvider(...)`). 여기에는 다음이 포함됩니다:
...
- `getAll()`은 로드된 모델 레지스트리(내장 + 병합된 커스텀 + 디스커버리)를 반환합니다.
...
- 정확한 모델 ID (프로바이더 추론)
- 퍼지/부분 문자열 매칭
- `--models`의 글롭 범위 패턴 (예: `openai/*`, `*sonnet*`)
...
2. 정확한 표준 ID는 표준 인덱스를 통해 해석됩니다
3. 정확한 단독 구체적 ID도 여전히 작동합니다
4. 퍼지 및 글롭 매칭은 정확한 경로 이후에 실행됩니다
...
`pi/smol`과 같은 역할 별칭은 `settings.modelRoles`를 통해 확장됩니다. 각 역할 값에는 `:minimal`, `:low`, `:medium`, 또는 `:high`와 같은 사고 셀렉터를 추가할 수도 있습니다.

역할이 다른 역할을 가리키는 경우, 대상 모델은 정상적으로 상속되며 참조하는 역할에 대한 명시적 접미사가 해당 역할 특정 사용에 우선합니다.

관련 설정:

- `modelRoles` (레코드)
- `enabledModels` (범위 지정 패턴 목록)
- `modelProviderOrder` (전역 표준-프로바이더 우선순위)
- `providers.kimiApiFormat` (`openai` 또는 `anthropic` 요청 형식)
- `providers.openaiWebsockets` (`auto|off|on` OpenAI Codex 전송을 위한 웹소켓 프리퍼런스)

`modelRoles`는 다음 중 하나를 저장할 수 있습니다:

- 구체적 프로바이더 변형을 고정하기 위한 `provider/modelId`
- 프로바이더 통합을 허용하기 위한 `gpt-5.3-codex`와 같은 표준 ID

`enabledModels` 및 CLI `--models`의 경우:

- 정확한 표준 ID는 해당 표준 그룹의 모든 구체적 변형으로 확장됩니다
- 명시적 `provider/modelId` 항목은 정확하게 유지됩니다
- 글롭 및 퍼지 매칭은 여전히 구체적 모델에서 작동합니다

## `/model` 및 `--list-models`

두 인터페이스 모두 프로바이더 접두사가 붙은 모델을 보이고 선택 가능하게 유지합니다.

또한 이제 표준/통합 모델도 노출합니다:

- `/model`은 프로바이더 탭과 함께 표준 뷰를 포함합니다
- `--list-models`는 표준 섹션과 구체적 프로바이더 행을 출력합니다

표준 항목을 선택하면 표준 셀렉터가 저장됩니다. 프로바이더 행을 선택하면 명시적 `provider/modelId`가 저장됩니다.

## 컨텍스트 프로모션 (모델 수준 폴백 체인)

컨텍스트 프로모션은 API가 컨텍스트 길이 오류로 요청을 거부할 때 자동으로 더 큰 컨텍스트의 동류 모델로 승격하는 소규모 컨텍스트 변형(예: `*-spark`)을 위한 오버플로 복구 메커니즘입니다.

### 트리거 및 순서

턴이 컨텍스트 오버플로 오류(예: `context_length_exceeded`)로 실패하면 `AgentSession`은 컴팩션으로 폴백하기 **전에** 프로모션을 시도합니다:

1. `contextPromotion.enabled`가 true이면 프로모션 대상을 해석합니다(아래 참조).
2. 대상이 발견되면 해당 모델로 전환하고 요청을 재시도합니다 — 컴팩션이 필요하지 않습니다.
3. 대상이 없으면 현재 모델에서 자동 컴팩션으로 넘어갑니다.

### 대상 선택

선택은 역할 기반이 아니라 모델 기반입니다:

1. `currentModel.contextPromotionTarget` (설정된 경우)
2. 동일 프로바이더 + API에서 가장 작은 더 큰 컨텍스트 모델

자격 증명이 해석되지 않는(`ModelRegistry.getApiKey(...)`) 후보는 무시됩니다.

### OpenAI Codex 웹소켓 핸드오프

`openai-codex-responses`에서/로 전환할 때 세션 프로바이더 상태 키 `openai-codex-responses`는 모델 전환 전에 닫힙니다. 이렇게 하면 웹소켓 전송 상태가 삭제되어 다음 턴이 승격된 모델에서 깨끗하게 시작됩니다.
...
동일 프로바이더에서 Spark -> non-Spark로의 예시 (`models.yml`):
...
내장 모델 생성기도 동일 프로바이더 기본 모델이 존재할 때 `*-spark` 모델에 이를 자동으로 할당합니다.
...
적절한 이미지 프로바이더 설정과 함께 기본 `config.yml`도 생성됩니다.
...
| `configVersion` 누락 또는 구버전 | `.bak`으로 백업 후 현재 버전으로 재생성 |
| 설정이 정상 | 조치 없음 |

모든 복구는 덮어쓰기 전에 `.bak` 백업을 생성합니다. 모든 작업은 멱등적입니다.
...
두 변수 중 하나라도 설정되지 않으면 자동 설정이 조용히 건너뛰어집니다.
...
주목할 만한 레거시 경로가 하나 남아 있습니다: 웹 검색 Anthropic 인증 해석이 `src/web/search/auth.ts`에서 여전히 `~/.xcsh/agent/models.json`을 직접 읽습니다.
...
- `LITELLM_BASE_URL`과 `LITELLM_API_KEY`가 설정되어 있으면 시작 상태 검사가 자동 복구를 시도합니다(손상된 파일을 백업하고 환경 변수에서 재생성). 복구가 성공하면 레지스트리가 수정된 설정을 다시 로드합니다.
