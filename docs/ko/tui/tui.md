...
이 문서는 확장 기능 UI, 커스텀 도구 UI, 커스텀 렌더러를 위해 `packages/coding-agent`와 `packages/tui`에서 사용하는 **현재** TUI 계약을 다룹니다.

## 이 서브시스템의 정의

런타임은 두 개의 레이어로 구성됩니다:

- **렌더링 엔진 (`packages/tui`)**: 차분(differential) 터미널 렌더러, 입력 디스패치, 포커스, 오버레이, 커서 배치.
- **통합 레이어 (`packages/coding-agent`)**: 확장 기능/커스텀 도구 컴포넌트를 마운트하고, 키바인딩/테마를 연결하며, 에디터 상태를 복원합니다.

## 모드별 런타임 동작

| 모드 | `ctx.ui.custom(...)` 가용 여부 | 비고 |
| --- | --- | --- |
| 인터랙티브 TUI | 지원됨 | 컴포넌트가 에디터 영역에 마운트되고 포커스되며, 해결을 위해 `done(result)`을 호출해야 합니다. |
| 백그라운드/헤드리스 | 인터랙티브 아님 | UI 컨텍스트가 no-op입니다 (`hasUI === false`). |
| RPC 모드 | 지원되지 않음 | `custom()`은 `Promise<never>`를 반환하며 TUI 컴포넌트를 마운트하지 않습니다. |

확장 기능/도구가 비인터랙티브 모드에서 실행 가능한 경우, `ctx.hasUI` / `pi.hasUI`로 가드하세요.

## 핵심 컴포넌트 계약 (`@f5xc-salesdemos/pi-tui`)

`packages/tui/src/tui.ts`에서 다음을 정의합니다:
...
커서 동작은 `CURSOR_MARKER`를 사용합니다 (`getCursorPosition`이 아님). 포커스된 컴포넌트는 렌더링된 텍스트에 마커를 삽입하고, `TUI`가 이를 추출하여 하드웨어 커서를 배치합니다.

## 렌더링 제약 조건 (터미널 안전성)

`render(width)` 출력은 터미널에 안전해야 합니다:

1. **어떤 줄에서도 `width`를 초과하지 마세요**. 렌더러는 이미지가 아닌 줄이 오버플로우하면 에러를 발생시킵니다.
2. **문자열 길이가 아닌 시각적 너비를 측정하세요**: `visibleWidth()`를 사용하세요.
3. **ANSI 인식 텍스트를 잘라내기/줄바꿈하세요**: `truncateToWidth()` / `wrapTextWithAnsi()`를 사용하세요.
4. **외부 소스의 탭/콘텐츠를 정제하세요**: `replaceTabs()`를 사용하세요 (그리고 coding-agent 렌더 경로의 상위 수준 정제기도 사용하세요).
...
확장 기능 UI 팩토리는 `KeybindingsManager`(인터랙티브 모드)를 수신하므로 키를 하드코딩하는 대신 매핑된 액션을 준수할 수 있습니다:
...
컴포넌트가 다음을 설정하지 않으면 키 릴리스 이벤트는 필터링됩니다:
...
- 오버레이 API는 `TUI`에 존재하지만 (`showOverlay`, `OverlayHandle`), 인터랙티브 모드에서의 확장 기능 `ctx.ui.custom` 마운트는 현재 에디터 컴포넌트 영역을 직접 교체합니다.
- `custom(..., options?: { overlay?: boolean })` 옵션은 확장 기능 타입에 존재하지만, 인터랙티브 확장 기능 마운트에서는 현재 이 옵션을 무시합니다.

## 마운트 포인트 및 반환 계약

## 1) 확장 기능 UI (`ExtensionUIContext`)
...
- 사용자의 컴포넌트에 포커스합니다.
- `done(result)` 호출 시: `component.dispose?.()`를 호출하고, 에디터 + 텍스트를 복원하고, 에디터에 포커스하고, 프로미스를 해결합니다.

따라서 `done(...)`은 완료를 위해 필수입니다.

## 2) 훅/커스텀 도구 UI 컨텍스트 (레거시 타이핑)

`HookUIContext.custom`은 훅/커스텀 도구 타입에서 `(tui, theme, done)`으로 타이핑됩니다.
내부 인터랙티브 구현은 `(tui, theme, keybindings, done)`으로 팩토리를 호출합니다. JS 소비자는 추가 인수를 사용할 수 있지만, 타입 수준의 호환성은 여전히 3개 인수의 레거시 시그니처를 반영합니다.

커스텀 도구는 일반적으로 팩토리 범위의 `pi.ui` 객체를 통해 동일한 UI 진입점을 사용한 후, 일반 도구 콘텐츠에서 선택된 값을 반환합니다:
...
- `dispose()`는 타입 수준에서 선택 사항이지만, 타이머, 서브프로세스, 워처, 소켓 또는 오버레이를 소유할 때 구현해야 합니다.
- `done(...)`은 컴포넌트 흐름에서 정확히 한 번 호출되어야 합니다.
...
- `packages/coding-agent/src/modes/controllers/extension-ui-controller.ts` — 확장 기능/훅/커스텀 도구 UI의 인터랙티브 마운트/언마운트.
- `packages/coding-agent/src/extensibility/extensions/types.ts` — 확장 기능 UI 및 렌더러 계약.
- `packages/coding-agent/src/extensibility/hooks/types.ts` — 훅 UI 계약 (레거시 커스텀 시그니처).
