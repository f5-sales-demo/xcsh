...
이 문서는 현재 coding-agent에서 테마가 어떻게 작동하는지 설명합니다: 스키마, 로딩, 런타임 동작, 그리고 실패 모드.
...
- `vars` (선택 사항; 재사용 가능한 색상 변수)
- `export` (선택 사항; HTML 내보내기 색상)
- `symbols` (선택 사항)
  - `preset` (선택 사항: `unicode | nerd | ascii`)
  - `overrides` (선택 사항: `SymbolKey`에 대한 키/값 오버라이드)
...
- 빈 문자열 (`""`)은 터미널 기본값을 의미 (`\x1b[39m` fg, `\x1b[49m` bg)

## 필수 색상 토큰 (현재)

아래 모든 토큰은 `colors`에 필수입니다.
...
생략하면, 내보내기 코드가 해석된 테마 색상에서 기본값을 도출합니다.
...
1. 내장 임베디드 테마 (`defaults/xcsh-dark.json` 및 `defaults/xcsh-light.json`이 `defaultThemes`에 컴파일됨)
2. 커스텀 테마 파일: `<customThemesDir>/<name>.json`

커스텀 테마 디렉토리는 `getCustomThemesDir()`에서 제공됩니다:
...
- `TERM`이 `dumb`, `linux` 또는 비어 있음 => 256color
- 그 외 => truecolor
...
- 전역 `theme`에 임시 미리보기 테마 적용
- 자체적으로 영구 설정을 변경하지 **않음**
- 폴백 교체 없이 성공/오류 반환

설정 UI는 이를 라이브 미리보기에 사용하며 취소 시 이전 테마를 복원합니다.
...
- 내장 테마는 사실상 감시되지 않음
- 파일 `change`: 리로드 시도 (디바운스됨)
- 파일 `rename`/삭제: `dark`로 폴백, 워처 닫기
...
- `toolDiffAdded`가 HSV 조정됨 (녹색이 파란색 방향으로 이동)
- 해석된 값이 16진수 문자열인 경우에만 조정이 적용됨
...
- 실제 기본 파일: `~/.xcsh/agent/config.yml`
...
레거시 마이그레이션이 존재합니다: 이전의 플랫 `theme: "name"` 형식이 밝기 감지를 기반으로 중첩된 `theme.dark` 또는 `theme.light`로 마이그레이션됩니다.

## 커스텀 테마 만들기 (실용적)

1. 커스텀 테마 디렉토리에 파일을 만듭니다. 예: `~/.xcsh/agent/themes/my-theme.json`.
...
최소 골격. `colors`의 모든 키가 필수입니다 — 런타임 검증기
...
- 256색 팔레트 인덱스 (`statusLine<Segment>Bg` / `statusLine<Segment>Fg`)는
  powerline 세그먼트 채우기를 구동합니다. 이들은 위의 16진수 키와 독립적입니다 —
  둘 다 설정해야 합니다.
...
1. 대화형 모드를 시작합니다 (시작 시 워처 활성화됨).
2. 설정을 열고 테마 값을 미리봅니다 (실시간 `previewTheme`).
...
- 테마 JSON의 `$schema`는 정보 제공용입니다; 런타임 검증은 코드 내 컴파일된 TypeBox 스키마에 의해 수행됩니다.
- `setTheme` 실패 시 `dark`로 폴백됩니다; `previewTheme` 실패 시 현재 테마가 교체되지 않습니다.
