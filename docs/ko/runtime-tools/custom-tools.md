...
- **커스텀 도구**: 턴 중에 모델이 호출할 수 있는 도구 (`execute` + TypeBox 스키마).
- **확장(Extension)**: 도구를 등록하고 이벤트를 가로채거나 수정할 수 있는 라이프사이클/이벤트 프레임워크.
- **훅(Hook)**: 외부 pre/post 명령 스크립트.
...
2. **로더 API를 통한 파일시스템 검색 모듈** (`discoverAndLoadCustomTools` / `loadCustomTools`)
   ...
   - 호스트 코드가 이를 호출하여 config/provider/plugin 경로에서 도구 모듈을 검색하고 로드할 수 있습니다.
...
- `.md` 및 `.json` 파일은 일부 제공자에 의해 도구 메타데이터로 검색되지만, 실행 가능한 모듈 로더는 이를 실행 가능한 도구로 거부합니다.
...
- `params`는 TypeBox 스키마에서 `Static<TParams>`를 통해 정적으로 타이핑됩니다.
- 런타임 인자 유효성 검사는 에이전트 루프에서 실행 전에 수행됩니다.
- `onUpdate`는 UI 스트리밍을 위한 부분 결과를 내보냅니다.
- `ctx`는 세션/모델 상태와 `abort()` 헬퍼를 포함합니다.
- `signal`은 취소를 전달합니다.

`CustomToolAdapter`는 이를 에이전트 도구 인터페이스에 연결하고 올바른 인자 순서로 호출을 전달합니다.
...
- CLI `--tools`는 현재 내장 도구 이름만 유효성을 검사합니다. 커스텀 도구 포함은 검색/등록 경로 및 SDK 옵션을 통해 처리됩니다.
...
- 렌더러 오류는 포착되고 기록됩니다. UI는 기본 텍스트 렌더링으로 폴백합니다.
...
브랜치/세션 컨텍스트가 변경될 때 `ctx.sessionManager`를 사용하여 히스토리에서 상태를 재구성하십시오.
...
- 에이전트 런타임은 실패를 `isError: true`와 오류 텍스트 내용이 포함된 도구 결과 메시지로 변환합니다.
- 확장 래퍼에서 `tool_result` 핸들러는 내용/세부 정보를 추가로 재작성하고 오류 상태를 재정의할 수도 있습니다.

### 취소

- 에이전트 중단은 `AbortSignal`을 통해 `execute`에 전파됩니다.
- 협력적 취소를 위해 `signal`을 서브프로세스 작업(`pi.exec(..., { signal })`)에 전달하십시오.
- `ctx.abort()`를 통해 도구가 현재 에이전트 작업의 중단을 요청할 수 있습니다.

### onSession 오류

- `onSession` 오류는 포착되어 경고로 기록됩니다. 세션을 크래시시키지 않습니다.

## 설계 시 고려해야 할 실제 제약 사항

- 도구 이름은 활성 레지스트리에서 전역적으로 고유해야 합니다.
- 렌더러/상태 재구성을 위해 `details`에서 결정적이고 스키마 형태의 출력을 선호하십시오.
- `pi.hasUI`로 UI 사용을 보호하십시오.
- 도구 디렉터리의 `.md`/`.json`은 메타데이터로 취급하고, 실행 가능한 모듈이 아닌 것으로 간주하십시오.
