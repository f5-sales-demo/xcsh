...
- **커스텀 도구**: 턴 동안 모델에 의해 호출 가능 (`execute` + TypeBox 스키마).
- **확장(Extension)**: 도구를 등록하고 이벤트를 가로채거나 수정할 수 있는 라이프사이클/이벤트 프레임워크.
- **훅(Hook)**: 외부 사전/사후 명령 스크립트.
- **스킬(Skill)**: 정적 가이드/컨텍스트 패키지이며, 실행 가능한 도구 코드가 아님.

모델이 코드를 직접 호출해야 하는 경우 커스텀 도구를 사용하세요.
...
1. **SDK 제공 커스텀 도구** (`options.customTools`)
   ...
   - SDK 부트스트랩에서 항상 초기 활성 도구 세트에 포함됩니다.

2. **로더 API를 통한 파일시스템 탐색 모듈** (`discoverAndLoadCustomTools` / `loadCustomTools`)
   - `src/extensibility/custom-tools/loader.ts`에서 라이브러리 API로 노출됩니다.
   - 호스트 코드가 이를 호출하여 config/provider/plugin 경로에서 도구 모듈을 탐색하고 로드할 수 있습니다.
...
1. 기능 제공자 (`toolCapability`), 다음을 포함:
...
- 중복된 해석 경로는 중복 제거됩니다.
- 도구 이름 충돌은 내장 도구 및 이미 로드된 커스텀 도구와 비교하여 거부됩니다.
- `.md` 및 `.json` 파일은 일부 제공자에 의해 도구 메타데이터로 탐색되지만, 실행 모듈 로더는 이를 실행 가능한 도구로 거부합니다.
...
- `hasUI`: 비대화형 흐름에서는 `false`
- `logger`: 공유 파일 로거
- `typebox`: 주입된 `@sinclair/typebox`
- `pi`: 주입된 `@f5xc-salesdemos/xcsh` 내보내기
- `pushPendingAction(action)`: 숨겨진 `resolve` 도구를 위한 미리보기 액션 등록 (`docs/resolve-tool-runtime.md`)
...
- 런타임 인수 검증은 에이전트 루프에서 실행 전에 수행됩니다.
- `onUpdate`는 UI 스트리밍을 위한 부분 결과를 내보냅니다.
- `ctx`는 세션/모델 상태와 `abort()` 헬퍼를 포함합니다.
- `signal`은 취소를 전달합니다.

`CustomToolAdapter`는 이를 에이전트 도구 인터페이스로 브리지하고 올바른 인수 순서로 호출을 전달합니다.

## 도구가 모델에 노출되는 방식

- 도구는 `AgentTool` 인스턴스(`CustomToolAdapter` 또는 확장 래퍼)로 래핑됩니다.
- 이름별로 세션 도구 레지스트리에 삽입됩니다.
- SDK 부트스트랩에서 커스텀 및 확장 등록 도구는 초기 활성 세트에 강제 포함됩니다.
- CLI `--tools`는 현재 내장 도구 이름만 검증하며, 커스텀 도구 포함은 탐색/등록 경로와 SDK 옵션을 통해 처리됩니다.
...
- 훅이 존재하면 도구 출력이 `Box` 컨테이너 내에서 렌더링됩니다.
- `renderResult`는 `{ expanded, isPartial, spinnerFrame? }`를 받습니다.
- 렌더러 오류는 캐치되어 로그에 기록되며, UI는 기본 텍스트 렌더링으로 폴백됩니다.
...
브랜치/세션 컨텍스트가 변경될 때 `ctx.sessionManager`를 사용하여 히스토리에서 상태를 재구성하세요.

## 실패 및 취소 의미론

### 동기/비동기 실패

- `execute`에서 throw하거나 (또는 거부된 프로미스) 도구 실패로 처리됩니다.
- 에이전트 런타임은 실패를 `isError: true`와 오류 텍스트 콘텐츠가 포함된 도구 결과 메시지로 변환합니다.
...
- `onSession` 오류는 캐치되어 경고로 로그에 기록됩니다. 세션을 크래시시키지 않습니다.
...
- 도구 디렉토리의 `.md`/`.json`은 실행 가능한 모듈이 아닌 메타데이터로 취급하세요.
