...
xcsh는 **컨텍스트**를 통해 F5 Distributed Cloud에 연결합니다 -- 컨텍스트는 테넌트 URL, API 토큰, 네임스페이스를 바인딩하는 명명된 자격 증명 세트입니다. `kubectl config use-context`나 `kubectx`를 사용해 본 적이 있다면 워크플로우는 동일합니다: 컨텍스트를 생성하고, 이름으로 전환하며, `-`를 사용하여 이전 컨텍스트로 되돌아갑니다.
...
또는 단계별 프롬프트를 선호한다면 가이드 마법사를 사용하세요:
...
이름으로 전환 -- 하위 명령어 동사가 필요 없습니다:

```
/context staging
```

이전 컨텍스트로 되돌아가기 (`cd -` 스타일):

```
/context -
```

`/context -`를 두 번 호출하면 원래 있던 곳으로 돌아갑니다.
...
| 명령어 | 기능 |
|---|---|
| `/context` | 모든 컨텍스트 목록 표시 |
| `/context <name>` | 지정한 컨텍스트로 전환 |
| `/context -` | 이전 컨텍스트로 전환 |
| `/context show` | 활성 컨텍스트 세부 정보 표시 (토큰 마스킹됨) |
...
| `/context delete <name> --confirm` | 컨텍스트 삭제 (`--confirm` 필수) |
| `/context rename <old> <new>` | 컨텍스트 이름 변경 |
| `/context validate <name>` | 전환 없이 자격 증명 테스트 |
| `/context export [name] [--include-token]` | JSON으로 내보내기 (토큰은 기본적으로 마스킹됨) |
...
탭 완성 기능이 활성 테넌트의 네임스페이스 이름을 제안합니다.
...
1. **컨텍스트 이름** -- 테넌트 URL 힌트와 함께 표시되어 테넌트를 구분할 수 있습니다
2. **`-`** -- 이전에 전환한 적이 있을 때 나타나며, 어떤 컨텍스트로 전환될지 보여줍니다
3. **하위 명령어** -- `list`, `create`, `delete` 등

전환이 가장 일반적인 작업이므로 컨텍스트 이름이 먼저 표시됩니다.

하위 명령어 수준의 완성도 작동합니다: `/context activate <Tab>`은 컨텍스트 이름을 완성하고, `/context namespace <Tab>`은 네임스페이스를 완성하며, `/context unset <Tab>`은 알려진 환경 변수 키를 완성합니다.

## 이름 규칙

컨텍스트 이름은 1-64자여야 하며: 문자, 숫자, 하이픈, 밑줄을 사용할 수 있습니다.

하위 명령어와 충돌하는 이름은 거부됩니다:
...
전체 예약어 목록: `list`, `show`, `status`, `create`, `delete`, `rename`, `namespace`, `env`, `set`, `unset`, `add`, `remove`, `clear`, `activate`, `validate`, `export`, `import`, `wizard`, `help`. 비교는 대소문자를 구분하지 않습니다.

## 환경 변수 오버라이드

xcsh를 실행하기 전에 셸 환경에 `F5XC_API_URL`과 `F5XC_API_TOKEN`이 설정되어 있으면, 모든 컨텍스트보다 우선합니다. 이는 CI/CD 파이프라인이나 영구 컨텍스트를 생성하고 싶지 않은 일회성 세션에 유용합니다.

이 모드에서 실행 중일 때, `/context`는 환경에서 가져온 자격 증명을 `(via env vars)` 레이블과 함께 표시합니다.

## 이전 컨텍스트 동작

- **세션 범위**: 이전 컨텍스트는 xcsh를 재시작하면 초기화됩니다. 디스크에 저장되지 않습니다.
- **핑퐁**: `/context -`를 두 번 실행하면 원래 있던 곳으로 돌아갑니다.
- **변경에 안전**: 이전 컨텍스트를 삭제하면 포인터가 지워집니다. 이름을 변경하면 포인터가 새 이름을 따라갑니다.
...
- **kubectx**: 전환에 `kubectx <name>`, 이전으로 `kubectx -`, 목록 표시에 `kubectx` 단독 사용
- **kubectl**: 명시적 형태로 `kubectl config use-context`
- **셸**: 이전 디렉터리 추적에 `cd -` / `OLDPWD`
