...
xcsh는 **컨텍스트**를 통해 F5 Distributed Cloud에 연결합니다 -- 컨텍스트는 테넌트 URL, API 토큰, 네임스페이스를 바인딩하는 이름이 지정된 자격 증명 세트입니다. `kubectl config use-context`나 `kubectx`를 사용해 본 적이 있다면 워크플로우는 동일합니다: 컨텍스트를 생성하고, 이름으로 전환하며, `-`를 사용하여 이전 컨텍스트로 되돌아갑니다.
...
이전 컨텍스트로 다시 전환 (`cd -` 스타일):

```
/context -
```

`/context -`를 두 번 호출하면 처음 시작한 위치로 돌아갑니다.
...
탭 완성 시 활성 테넌트의 네임스페이스 이름이 제안됩니다.
...
1. **컨텍스트 이름** -- 테넌트 URL 힌트가 포함되어 테넌트를 구별할 수 있습니다
2. **`-`** -- 이전에 전환한 적이 있을 때 나타나며, 어떤 컨텍스트로 전환되는지 표시합니다
...
전체 예약어 목록: `list`, `show`, `status`, `create`, `delete`, `rename`, `namespace`, `env`, `set`, `unset`, `add`, `remove`, `clear`, `activate`, `validate`, `export`, `import`, `wizard`, `help`. 비교 시 대소문자를 구분하지 않습니다.

## 환경 변수 오버라이드

xcsh를 실행하기 전에 셸 환경에 `F5XC_API_URL`과 `F5XC_API_TOKEN`이 설정되어 있으면 모든 컨텍스트보다 우선합니다. 이는 영구적인 컨텍스트를 생성하지 않으려는 CI/CD 파이프라인이나 일회성 세션에 유용합니다.

이 모드로 실행할 때 `/context`는 환경에서 가져온 자격 증명에 `(via env vars)` 레이블을 표시합니다.

## 이전 컨텍스트 동작

- **세션 범위**: 이전 컨텍스트는 xcsh를 재시작하면 초기화됩니다. 디스크에 저장되지 않습니다.
- **핑퐁**: `/context -`를 두 번 실행하면 처음 시작한 위치로 돌아갑니다.
- **변경에 안전**: 이전 컨텍스트를 삭제하면 포인터가 제거됩니다. 이름을 변경하면 포인터가 새 이름을 따릅니다.
- **재활성화는 무동작**: 이미 `production`에 있을 때 `/context production`을 실행하면 이전 포인터가 재설정되지 않습니다.

## 설계 규칙

`/context` UX는 다음을 따릅니다:

- **kubectx**: 전환에 `kubectx <name>`, 이전으로 `kubectx -`, 목록에 `kubectx` 단독 사용
- **kubectl**: 명시적 형태로 `kubectl config use-context`
- **Shell**: 이전 디렉토리 추적에 `cd -` / `OLDPWD`
