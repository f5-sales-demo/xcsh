...
이 문서는 coding-agent가 지원되는 설정 형식에서 규칙을 검색하고, 단일 `Rule` 형태로 정규화하며, 우선순위 충돌을 해결하고, 결과를 다음과 같이 분리하는 방법을 설명합니다:

- **룰북 규칙** (시스템 프롬프트 + `rule://` URL을 통해 모델에 제공)
- **TTSR 규칙** (시간 여행 스트림 인터럽션 규칙)

이 문서는 부분적 의미론 및 파싱되지만 강제되지 않는 메타데이터를 포함한 현재 구현을 반영합니다.
...
결과적으로 우선순위와 중복 제거는 **이름 기반으로만** 수행됩니다. 동일한 `name`을 가진 서로 다른 두 파일은 동일한 논리적 규칙으로 간주됩니다.
...
- 프론트매터는 `parseFrontmatter`를 통해 파싱
- `content` = 본문 (프론트매터 제거됨)
- `globs`, `alwaysApply`, `description`, `ttsr_trigger` 직접 매핑

중요한 주의사항: 이 프로바이더에서 `globs`는 요소 필터링 없이 `string[] | undefined`로 캐스팅됩니다.
...
- `alwaysApply`, `description` 프론트매터에서 캐스팅
- `ttsr_trigger`: 문자열만
...
모든 프로바이더는 다음 의미론을 가진 `parseFrontmatter` (`utils/frontmatter.ts`)를 사용합니다:

1. 프론트매터는 콘텐츠가 `---`로 시작하고 `\n---`로 닫히는 경우에만 파싱됩니다.
2. 프론트매터 추출 후 본문은 트리밍됩니다.
3. YAML 파싱이 실패하면:
   - 경고가 로깅되고,
   - 파서가 단순 `key: value` 라인 파싱(`^(\w+):\s*(.*)$`)으로 폴백합니다.

모호성에 따른 결과:

- 폴백 파서는 배열, 중첩 객체, 인용 규칙 또는 하이픈이 포함된 키를 지원하지 않습니다.
- 폴백 값은 문자열이 됩니다(예: `alwaysApply: true`가 문자열 `"true"`가 됨). 따라서 불리언/문자열 타입을 요구하는 프로바이더는 메타데이터를 누락할 수 있습니다.
- `ttsr_trigger`는 폴백에서 동작합니다(언더스코어 키). `thinking-level`과 같은 키는 동작하지 않습니다.
- 유효한 프론트매터가 없는 파일도 빈 메타데이터와 전체 콘텐츠 본문을 가진 규칙으로 로드됩니다.

## 4. 프로바이더 우선순위 및 중복 제거

`loadCapability("rules")` (`capability/index.ts`)는 프로바이더 출력을 병합한 다음 `rule.name`으로 중복을 제거합니다.

### 우선순위 모델

- 프로바이더는 우선순위 내림차순으로 정렬됩니다.
- 동일 우선순위는 등록 순서를 유지합니다 (`discovery/index.ts`에서 `cursor`가 `windsurf`보다 먼저).
- 중복 제거는 선착순입니다: 먼저 발견된 규칙 이름이 유지되고, 이후 동일 이름 항목은 `all`에서 `_shadowed`로 표시되며 `items`에서 제외됩니다.

현재 유효한 규칙 프로바이더 순서는 다음과 같습니다:
...
프로바이더 내에서 항목 순서는 `loadFilesFromDir` glob 결과 순서와 명시적 push 순서에서 옵니다. 이는 일반적인 사용에서는 충분히 결정적이지만 코드에서 명시적으로 정렬되지는 않습니다.

주목할 만한 소스 순서 차이:

- `native`는 프로젝트 디렉토리를 먼저 추가한 다음 사용자 설정 디렉토리를 추가합니다.
- `cursor`는 사용자 결과를 먼저 추가한 다음 프로젝트 결과를 추가합니다.
- `windsurf`는 사용자 `global_rules`를 먼저 추가한 다음 프로젝트 규칙을 추가합니다.
- `cline`은 가장 가까운 `.clinerules` 소스만 로드합니다.

## 5. 룰북, 항상 적용, TTSR 버킷으로의 분리
...
- **TTSR 버킷**: `condition`이 있는 모든 규칙 (설명은 필요 없음). 다른 버킷보다 우선합니다.
- **항상 적용 버킷**: `alwaysApply === true`이고 TTSR이 아닌 경우. 전체 콘텐츠가 시스템 프롬프트에 주입됩니다. `rule://`을 통해 해석 가능합니다.
- **룰북 버킷**: 설명이 있어야 하고, TTSR이 아니어야 하며, `alwaysApply`가 아니어야 합니다. 시스템 프롬프트에 이름+설명으로 나열되며, 콘텐츠는 `rule://`을 통해 요청 시 읽힙니다.
- `condition`과 `alwaysApply`가 모두 있는 규칙은 TTSR에만 포함됩니다 (TTSR이 우선).
- `alwaysApply`와 `description`이 모두 있는 규칙은 항상 적용에만 포함됩니다 (룰북 아님).
...
- 설명이 없으면 규칙은 `rule://`을 통해 사용할 수 없으며 시스템 프롬프트 규칙에 나열되지 않습니다.
...
`buildSystemPromptInternal`은 `rules` (룰북)와 `alwaysApplyRules` 모두를 수신합니다.

항상 적용 규칙이 먼저 렌더링되어 원시 콘텐츠가 프롬프트에 직접 주입됩니다.
...
이는 권고/맥락적 성격입니다: 프롬프트 텍스트는 모델에게 적용 가능한 규칙을 읽도록 요청하지만, 코드가 glob 적용 가능성을 강제하지는 않습니다.
...
- TTSR 전용 규칙과 설명도 없고 `alwaysApply`도 아닌 규칙은 `rule://`을 통해 접근할 수 없습니다.
- 해석은 정확한 이름 매칭으로 수행됩니다.
...
3. `rule://`에 대한 규칙 선택에는 룰북 및 항상 적용 규칙이 포함되지만, TTSR 전용 규칙은 포함되지 않습니다.
4. 검색 경고(`loadCapability("rules").warnings`)는 생성되지만 `createAgentSession`은 현재 이 경로에서 이를 표시/로깅하지 않습니다.
