---
title: 스킬
description: '코딩 에이전트에서 특수 기능을 등록, 검색, 호출하기 위한 스킬 시스템.'
sidebar:
  order: 3
  label: 스킬
i18n:
...
스킬은 시작 시 검색되는 파일 기반 기능 팩으로, 모델에 다음과 같이 노출됩니다:

- 시스템 프롬프트의 경량 메타데이터 (이름 + 설명)
- `read skill://...`을 통한 온디맨드 콘텐츠
- 선택적 대화형 `/skill:<name>` 명령

이 문서는 `src/extensibility/skills.ts`, `src/discovery/builtin.ts`, `src/internal-urls/skill-protocol.ts`, `src/discovery/agents-md.ts`에서의 현재 런타임 동작을 다룹니다.
...
## 필수 레이아웃 및 SKILL.md 기대 사항

### 디렉토리 레이아웃

프로바이더 기반 검색(native/Claude/Codex/Agents/plugin 프로바이더)의 경우, 스킬은 **`skills/` 하위 한 단계**에서 검색됩니다:

- `<skills-root>/<skill-name>/SKILL.md`

`<skills-root>/group/<skill>/SKILL.md`와 같은 중첩 패턴은 프로바이더 로더에 의해 검색되지 않습니다.

`skills.customDirectories`의 경우, 스캔은 동일한 비재귀 레이아웃(`*/SKILL.md`)을 사용합니다.
...
커스텀 디렉토리 스캔도 비재귀이므로, 해당 중첩 상위 디렉토리를 `customDirectories`에 지정하지 않는 한 중첩 경로는 무시됩니다.
```
...
- `description`은 다음의 경우 필수입니다:
  - 네이티브 `.xcsh` 프로바이더 스킬 검색 (`requireDescription: true`)
...
2. `scanSkillsFromDir(..., { requireDescription: true })`를 통한 **커스텀 디렉토리** (한 단계 디렉토리 열거)
...
- 스킬 이름에 대한 glob 필터:
  - `ignoredSkills` (제외)
  - `includeSkills` (포함 허용 목록; 비어있으면 모두 포함)
...
codex/claude/native 이외의 프로바이더(예: `agents`, `claude-plugins`)의 경우, 활성화는 현재 다음으로 폴백됩니다: **어떤** 내장 소스 토글이든 활성화되어 있으면 활성화됨.

### 충돌 및 중복 처리

- 기능 중복 제거는 이미 이름당 첫 번째 스킬을 유지합니다 (최고 우선순위 프로바이더)
- `extensibility/skills.ts`는 추가로:
  - `realpath`로 동일한 파일을 중복 제거합니다 (심볼릭 링크 안전)
  - 이후 스킬 이름이 충돌할 때 충돌 경고를 발생시킵니다
  - `scanSkillsFromDir` 위에 얇은 어댑터로 편의 API `discoverSkillsFromDir({ dir, source })`를 유지합니다
- 커스텀 디렉토리 스킬은 프로바이더 스킬 이후에 병합되며 동일한 충돌 동작을 따릅니다
...
Task 도구 서브에이전트는 일반 세션 생성을 통해 세션의 검색된/제공된 스킬 목록을 받습니다. 태스크별 스킬 고정 오버라이드는 없습니다.

### 대화형 `/skill:<name>` 명령

`skills.enableSkillCommands`가 true이면, 대화형 모드는 검색된 스킬당 하나의 슬래시 명령을 등록합니다.
...
`src/discovery/agents-md.ts`는 `cwd`에서 상위 디렉토리를 순회하며 독립형 `AGENTS.md` 파일을 검색합니다 (최대 깊이 20), 숨겨진 디렉토리 세그먼트를 제외합니다.

### 스킬 vs 슬래시 명령

- **스킬**: 모델이 읽을 수 있는 지식/워크플로우 콘텐츠
- **슬래시 명령**: 사용자가 호출하는 명령 진입점
- `/skill:<name>`은 스킬 텍스트를 주입하는 편의 래퍼입니다. 스킬 검색 의미론을 변경하지 않습니다

### 스킬 vs 커스텀 도구

- **스킬**: 프롬프트 컨텍스트와 `read`를 통해 로드되는 문서/워크플로우 콘텐츠
- **커스텀 도구**: 스키마와 런타임 부수 효과를 가진 모델이 호출할 수 있는 실행 가능한 도구 API
...
## 검색 로직과 연관된 실용적 작성 가이드

- 각 스킬을 자체 디렉토리에 배치하세요: `<skills-root>/<skill-name>/SKILL.md`
- 항상 명시적 `name`과 `description` 프런트매터를 포함하세요
- 참조된 에셋을 동일한 스킬 디렉토리 하위에 두고 `skill://<name>/...`으로 접근하세요
- 중첩 분류 체계(`team/domain/skill`)의 경우, `skills.customDirectories`를 중첩 상위 디렉토리에 지정하세요. 스캔 자체는 비재귀로 유지됩니다
- 소스 간 중복 스킬 이름을 피하세요. 프로바이더 우선순위에 따라 첫 번째 일치가 우선합니다
