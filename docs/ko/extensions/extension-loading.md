...
description: '해석, 유효성 검증 및 캐싱을 포함한 확장 기능용 TypeScript 및 JavaScript 모듈 로딩 파이프라인.'
sidebar:
  order: 2
  label: 확장 기능 로딩
i18n:
...
확장 기능 로딩은 모듈 엔트리 파일 목록을 구성하고, Bun으로 각 모듈을 임포트하며, 팩토리를 실행하여 다음을 반환합니다:
...
- `src/extensibility/extensions/loader.ts` — 경로 검색 + 임포트/실행
...
메인 세션 시작 경로(`sdk.ts`)에서의 구성된 경로 소스:

1. CLI 제공 경로 (`--extension/-e`, `--hook`도 확장 경로로 취급됨)
2. 설정 `extensions` 배열 (전역 + 프로젝트 설정 병합)

전역 설정 파일:

- `~/.xcsh/agent/config.yml` (또는 `PI_CODING_AGENT_DIR`을 통한 사용자 정의 에이전트 디렉토리)
...
- CLI 경로 구성(`main.ts`)은 현재 `--no-extensions`가 설정되면 CLI 확장 경로를 지우므로, 해당 모드에서는 명시적 `-e/--hook`이 전달되지 않습니다.
...
`derivedName`은 엔트리 경로(`getExtensionNameFromPath`)를 기반으로 합니다. 예를 들어:
...
1. 해당 디렉토리의 `package.json`에 `xcsh.extensions` (또는 레거시 `pi.extensions`)가 있는 경우 -> 선언된 엔트리 사용
2. `index.ts`
3. `index.js`
4. 그 외에는 한 단계 깊이로 확장 엔트리를 스캔:
   - 직접적인 `*.ts` / `*.js`
   - 하위 디렉토리의 `index.ts` / `index.js`
   - 하위 디렉토리의 `package.json`에 `xcsh.extensions` / `pi.extensions`

규칙 및 제약:

- 하위 디렉토리 한 단계를 넘어서는 재귀적 검색 없음
- 선언된 `extensions` 매니페스트 엔트리는 해당 패키지 디렉토리 기준 상대 경로로 해석
- 선언된 엔트리는 파일이 존재하고 접근이 허용되는 경우에만 포함
- `*/index.{ts,js}` 쌍에서는 TypeScript가 JavaScript보다 우선
- 심볼릭 링크는 유효한 파일/디렉토리로 취급

### 소스에 따라 무시 동작이 다름

- 네이티브 자동 검색(검색 헬퍼의 `discoverExtensionModulePaths`)은 `gitignore: true` 및 `hidden: false`로 네이티브 glob을 사용합니다.
- `loader.ts`의 명시적 구성 디렉토리 스캔은 `readdir` 규칙을 사용하며 gitignore 필터링을 적용하지 **않습니다**.

---

## 로드 순서 및 우선순위

`discoverAndLoadExtensions()`는 하나의 정렬된 목록을 구성한 다음 `loadExtensions()`를 호출합니다.

순서:

1. 네이티브 자동 검색된 모듈
2. 명시적 구성 경로 (제공된 순서대로)
...
의미: 동일한 모듈 경로가 자동 검색과 명시적 구성 모두에 포함된 경우, 첫 번째 위치(자동 검색 단계)에서 한 번만 로드됩니다.
...
- 확장 기능은 **샌드박스되지 않습니다** (동일 프로세스/런타임).
- 하나의 `EventBus`와 하나의 `ExtensionRuntime` 인스턴스를 공유합니다.
- 로드 중에는 런타임 액션 메서드가 의도적으로 `ExtensionRuntimeNotInitializedError`를 던집니다. 액션 와이어링은 이후 `ExtensionRunner.initialize()`에서 수행됩니다.

### 로딩 이후

이벤트가 `ExtensionRunner`를 통해 실행될 때, 핸들러 예외는 러너 루프를 크래시시키는 대신 확장 오류로 캐치되어 전파됩니다.
