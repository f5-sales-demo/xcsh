...
description: '큐레이션된 플러그인 컬렉션을 검색, 설치 및 관리하기 위한 마켓플레이스 플러그인 시스템.'
sidebar:
  order: 4
  label: 마켓플레이스
i18n:
...
또는 인수 없이 `/marketplace`만 입력하면 대화형 플러그인 브라우저가 열립니다.

## 개념

**마켓플레이스**는 `.xcsh-plugin/marketplace.json`에 카탈로그 파일을 포함하는 Git 저장소(또는 로컬 디렉토리)입니다. 카탈로그에는 사용 가능한 플러그인의 소스, 설명 및 메타데이터가 나열되어 있습니다.
...
| `/marketplace upgrade [--scope user\|project] [name@marketplace]` | 하나 또는 전체 플러그인 업그레이드 |
...
`/marketplace add <source>`를 실행하면 시스템이 소스를 분류합니다:

| 소스 형식 | 유형 | 예시 |
|---|---|---|
| `owner/repo` | GitHub 약식 | `anthropics/f5xc-salesdemos-marketplace` |
...
시스템은 저장소를 클론하거나 로컬 디렉토리를 읽고, `.xcsh-plugin/marketplace.json`을 찾아 유효성을 검사한 후 카탈로그를 로컬에 캐시합니다.
...
| 필드 | 설명 |
|---|---|
| `name` | 마켓플레이스 이름. 소문자 영숫자, 하이픈, 점 사용 가능. 영숫자로 시작하고 끝나야 함. 최대 64자. |
...
| 필드 | 필수 | 설명 |
|---|---|---|
| `name` | 예 | 플러그인 이름 (마켓플레이스 이름과 동일한 규칙) |
...
마켓플레이스 및 플러그인 이름은 다음 규칙을 따라야 합니다:

- 소문자 또는 숫자로 시작하고 끝나야 합니다
- 소문자, 숫자, 하이픈, 점만 포함할 수 있습니다
- 최대 64자까지 가능합니다

플러그인 ID (`name@marketplace`)는 총 128자를 초과할 수 없습니다.
