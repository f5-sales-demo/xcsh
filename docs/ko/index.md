...
title: xcsh 문서
description: >-
  TypeScript 코딩 에이전트와 Rust 네이티브 레이어를 갖춘 AI 기반 개발 CLI로, 장기 세션, MCP 지원, 플랫폼 패키징을
  제공합니다.
sidebar:
  order: 0
  label: 개요
...
xcsh는 TypeScript 코딩 에이전트와 Rust 네이티브 레이어(`pi-natives`)를 갖춘
AI 기반 개발 CLI입니다. 오픈 소스
[`badlogic/pi-mono`](https://github.com/badlogic/pi-mono) 계열을 확장하여
강화된 런타임, 트리 탐색 및 압축 기능이 포함된 장기 세션,
...
- **런타임 및 도구** — bash / 노트북 / resolve 도구 런타임과
  슬래시 명령어 인터페이스입니다.
- **세션** — 추가 전용 엔트리 로그, 트리 탐색, 압축, 그리고
  자율 메모리 시스템입니다.
- **네이티브 (Rust)** — shell / PTY / 미디어 / 검색을 구동하는
...
- **TUI** — 테마 설정, `/tree` 명령어, 그리고 확장 및
  커스텀 도구를 위한 통합 훅입니다.

## 이 문서 세트의 구성 방식

사이드바의 각 최상위 그룹은 에이전트의 하위 시스템에 대응합니다. 그룹 내에서
페이지는 "개요"에서 "내부 구조" 순서로 진행되므로, 현재 작업에
충분한 맥락을 얻었을 때 읽기를 멈출 수 있습니다.
