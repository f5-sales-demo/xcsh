...
이 문서는 coding-agent가 MCP JSON-RPC 메시징을 어떻게 구현하는지, 그리고 프로토콜 관심사가 전송 관심사와 어떻게 분리되는지를 설명합니다.
...
- stdio 및 HTTP/SSE 전송의 요청 상관관계 및 생명주기
- 타임아웃 및 취소 동작
- 오류 전파 및 잘못된 페이로드 처리
...
전송 구현체는 프레이밍과 I/O 세부사항을 담당합니다:

- `StdioTransport`: 서브프로세스 stdio를 통한 개행 구분 JSON
- `HttpTransport`: HTTP POST를 통한 JSON-RPC, 선택적 SSE 응답/리스닝 포함
...
`client.ts:createTransport()`가 설정에서 전송 방식을 선택합니다:

- `type` 생략 또는 `"stdio"` -> `createStdioTransport`
- `"http"` 또는 `"sse"` -> `createHttpTransport`

`"sse"`는 HTTP 전송 변형(동일 클래스)으로 취급되며, 별도의 전송 구현체가 아닙니다.
...
- `#pendingRequests: Map<id, {resolve,reject}>`에 진행 중인 요청이 저장됩니다.
...
- Stdio: 알림 프레임 (`jsonrpc`, `method`, 선택적 `params`)에 개행을 추가하여 stdin에 씁니다.
- HTTP: `id` 없이 POST 본문을 전송합니다; 성공 시 `2xx` 또는 `202 Accepted`를 수락합니다.

서버 발신 알림은 전송의 `onNotification`을 통해서만 노출됩니다; 매니저/클라이언트에 기본 전역 구독자는 없습니다.

## Stdio 전송 내부 구조

## 생명주기 및 상태 전이

- 초기 상태: `connected=false`, `process=null`, 대기 맵 비어 있음
- `connect()`:
  - 설정된 명령어/인자/환경변수/작업디렉터리로 서브프로세스 스폰
  - 연결됨으로 표시
  - stdout 읽기 루프 시작 (`readJsonl`)
  - stderr 루프 시작 (읽기/폐기; 현재 무음)
- `close()`:
  - 연결 해제로 표시
  - 모든 대기 중인 요청 reject (`Transport closed`)
  - 서브프로세스 종료
  - 읽기 루프 종료 대기
  - `onClose` 발행

읽기 루프가 예기치 않게 종료되면, `finally`가 `#handleClose()`를 트리거하여 동일한 대기 요청 reject와 close 콜백을 수행합니다.
...
취소는 로컬에서만 이루어집니다: 전송은 서버에 프로토콜 수준의 취소 알림을 보내지 않습니다.

## 잘못된 페이로드 처리

읽기 루프에서:

- 파싱된 각 JSONL 줄은 `try/catch` 내의 `#handleMessage`로 전달됩니다
- 잘못된/유효하지 않은 메시지 처리 예외는 삭제됩니다 (`Skip malformed lines` 주석)
- 루프는 계속되므로, 하나의 잘못된 메시지가 연결을 끊지 않습니다

기본 스트림 파서가 throw하면, `onError`가 호출되고 (여전히 연결된 경우), 연결이 닫힙니다.
...
- 자동 재시작이나 재연결 없음
- 상위 레이어에서 새 전송을 생성하여 재연결해야 합니다

## 배압/스트리밍 참고사항

- 아웃바운드 쓰기는 drain 시맨틱을 기다리지 않고 `stdin.write()` + `flush()`를 사용합니다.
- 전송에 명시적 큐나 high-watermark 관리가 없습니다.
...
1. **요청별 SSE 응답** (`#parseSSEResponse`)
   ...
   - 일치하는 응답 id를 찾을 때까지 스트림을 소비합니다
   - 동일 스트림에서 인터리빙된 알림을 처리할 수 있습니다

2. **백그라운드 SSE 리스너** (`startSSEListener()`)
   - 서버 발신 알림을 위한 선택적 GET 리스너
   - 현재 MCP 매니저/클라이언트에 의해 자동 시작되지 않습니다
   - GET이 `405`를 반환하면 리스너가 자동으로 비활성화됩니다 (서버가 이 모드를 지원하지 않음)

## 잘못된 페이로드 및 연결 해제 처리

SSE JSON 파싱 오류는 `readSseJson`에서 버블업되어 요청/리스너를 reject합니다.

- 요청 SSE 파싱 오류는 활성 요청을 reject합니다.
- 백그라운드 리스너 오류는 `onError`를 트리거합니다 (AbortError 제외).
- 백그라운드 리스너에 대한 자동 재연결은 없습니다.

## `json-rpc.ts` 유틸리티 vs 전송 추상화

`src/mcp/json-rpc.ts`는 직접 HTTP MCP 호출을 위한 `callMCP()`와 `parseSSE()` 헬퍼를 제공합니다 (Exa 통합에 사용됨). 이는 `MCPClient`/`MCPManager`가 사용하는 `MCPTransport` 추상화가 아닙니다.

`HttpTransport`와의 주요 차이점:

- 전체 응답 텍스트를 먼저 파싱한 다음, 첫 번째 `data:` 줄을 추출합니다 (`parseSSE`), JSON 폴백 포함
- 요청 타임아웃 관리, abort API, session-id 처리, 전송 생명주기 없음
...
`MCPManager`는 느린 서버에 대한 시작 시 폴백 동작 (캐시에서 지연된 도구)을 가지고 있지만, 이는 도구 가용성 폴백이지 전송 재시도가 아닙니다.

## 실패 시나리오 요약

- **잘못된 stdio 메시지 줄**: 삭제됨; 스트림 계속됨.
- **Stdio 스트림/프로세스 종료**: 전송 닫힘; 대기 중인 요청이 `Transport closed`로 reject됨.
...
- **타임아웃**: 전송별 타임아웃 오류.
- **호출자 abort**: 호출자 signal에서 AbortError/사유가 전파됨.

## 실용적 경계 규칙

관심사가 메시지 형태, id 상관관계, 또는 MCP 메서드 순서에 관한 것이라면, 프로토콜/클라이언트 로직에 속합니다.

관심사가 프레이밍 (JSONL vs HTTP/SSE), 스트림 파싱, fetch/spawn 생명주기, 타임아웃 클록, 또는 연결 해제에 관한 것이라면, 전송 구현체에 속합니다.
