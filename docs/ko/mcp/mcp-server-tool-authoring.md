...
description: 코딩 에이전트를 위한 커스텀 MCP 서버 빌드 및 도구 등록 가이드.
sidebar:
  order: 4
  label: 서버 및 도구 작성
i18n:
...
이 문서는 MCP 서버 정의가 코딩 에이전트에서 호출 가능한 `mcp_*` 도구로 변환되는 과정과, 설정이 유효하지 않거나 중복되거나 비활성화되거나 인증이 필요한 경우 운영자가 예상해야 할 사항을 설명합니다.
...
`src/mcp/types.ts`는 MCP 설정 작성자와 런타임에서 사용하는 작성 형태를 정의합니다:

- `stdio` (`type` 누락 시 기본값): `command` 필수, `args`, `env`, `cwd` 선택
- `http`: `url` 필수, `headers` 선택
- `sse`: `url` 필수, `headers` 선택 (호환성을 위해 유지)
- 공통 필드: `enabled`, `timeout`, `auth`

`validateServerConfig()` (`src/mcp/config.ts`)는 전송 계층의 기본 사항을 검증합니다:

- `command`와 `url`이 모두 설정된 경우 거부
- stdio의 경우 `command` 필수
- http/sse의 경우 `url` 필수
- 알 수 없는 `type` 거부
...
- `type` 생략 시 stdio로 처리됩니다. HTTP/SSE를 의도했지만 `type`을 생략하면 `command`가 필수가 됩니다.
- `sse`는 여전히 허용되지만 내부적으로는 HTTP 전송(`createHttpTransport`)으로 처리됩니다.
- 유효성 검사는 구조적이며 연결 가능성을 확인하지 않습니다: 구문적으로 유효한 URL도 연결 시점에 실패할 수 있습니다.

## 2) 디스커버리, 정규화, 우선순위

### 능력(capability) 기반 디스커버리

`loadAllMCPConfigs()` (`src/mcp/config.ts`)는 `loadCapability(mcpCapability.id)`를 통해 표준 `MCPServer` 항목을 로드합니다.

능력 계층(`src/capability/index.ts`)은 다음을 수행합니다:

1. 우선순위 순서로 프로바이더를 로드
2. `server.name` 기준으로 중복 제거 (선착순 = 최고 우선순위)
3. 중복 제거된 항목을 검증

결과: 여러 소스에 걸쳐 중복된 서버 이름은 병합되지 않습니다. 하나의 정의만 사용되며, 낮은 우선순위의 중복 항목은 가려집니다.
...
- 명시적 제어를 위해 `.xcsh/mcp.json` (프로젝트) 또는 `~/.xcsh/mcp.json` (사용자)을 권장합니다.
- 폴백 호환성이 필요한 경우 루트 `mcp.json` / `.mcp.json`을 사용하세요.
- 여러 소스에서 같은 서버 이름을 재사용하면 병합이 아닌 우선순위 가림(shadowing)이 발생합니다.

### 정규화 동작

`convertToLegacyConfig()` (`src/mcp/config.ts`)는 표준 `MCPServer`를 런타임 `MCPServerConfig`로 매핑합니다.

주요 동작:

- 전송 계층은 `server.transport ?? (command ? "stdio" : url ? "http" : "stdio")`로 추론
- 비활성화된 서버(`enabled === false`)는 연결 전에 제거
- 선택적 필드는 존재하면 보존
...
- 해석되지 않은 값은 리터럴 `${VAR}` 문자열로 유지

`mcp-json.ts`는 사용자 JSON에 대해 런타임 타입 검사도 수행하며, 유효하지 않은 `enabled`/`timeout` 값에 대해 전체 파일을 실패시키지 않고 경고를 로그합니다.

## 3) 인증 및 런타임 값 해석

`MCPManager.prepareConfig()`/`#resolveAuthConfig()` (`src/mcp/manager.ts`)는 연결 직전의 최종 처리 단계입니다.
...
- `http`/`sse`: `Authorization: Bearer <access_token>` 헤더 주입
- `stdio`: `OAUTH_ACCESS_TOKEN` 환경 변수 주입

자격 증명 조회에 실패하면, 매니저는 경고를 로그하고 해석되지 않은 인증 상태로 계속 진행합니다.

### 헤더/환경 변수 값 해석

연결 전에 매니저는 `resolveConfigValue()` (`src/config/resolve-config-value.ts`)를 통해 각 헤더/환경 변수 값을 해석합니다:

- `!`로 시작하는 값 => 셸 명령 실행, 트리밍된 stdout 사용 (캐시됨)
- 그 외의 경우, 먼저 환경 변수 이름으로 처리(`process.env[name]`), 리터럴 값으로 폴백
- 해석되지 않은 명령/환경 변수 값은 최종 헤더/환경 변수 맵에서 제외

운영 주의사항: 이는 잘못 입력된 시크릿 명령/환경 변수 키가 해당 헤더/환경 변수 항목을 조용히 제거하여 다운스트림에서 401/403 또는 서버 시작 실패를 유발할 수 있음을 의미합니다.

## 4) 도구 브리지: MCP -> 에이전트 호출 가능 도구
...
이는 많은 충돌을 방지하지만 전부는 아닙니다. 서로 다른 원본 이름이 동일한 식별자로 정제될 수 있으며(예: `my-server`와 `my.server`는 유사하게 정제됨), 레지스트리 삽입은 마지막 쓰기가 우선합니다.
...
- 서버가 보고한 `isError`를 `Error: ...` 텍스트 결과로 매핑
- 발생한 전송/런타임 실패를 `MCP error: ...`로 매핑
- AbortError를 `ToolAbortError`로 변환하여 중단 시맨틱 보존
...
`refreshMCPTools()`는 모든 `mcp_` 레지스트리 항목을 교체하고 최신 MCP 도구 세트를 즉시 재활성화하므로, 세션을 재시작하지 않아도 변경 사항이 적용됩니다.

### 모드 차이점

- **인터랙티브/TUI 모드**: `/mcp`가 앱 내 UX를 제공합니다 (마법사, OAuth 흐름, 연결 상태 텍스트, 즉각적인 런타임 재바인딩).
- **SDK/헤드리스 통합**: `discoverAndLoadMCPTools()` (`src/mcp/loader.ts`)가 로드된 도구 + 서버별 오류를 반환합니다; `/mcp` 명령 UX는 없습니다.

## 6) 사용자에게 표시되는 오류 표면

사용자/운영자가 볼 수 있는 일반적인 오류 문자열:
...
디스커버리에서의 잘못된 소스 JSON은 일반적으로 경고/로그로 처리되며, config-writer 경로는 명시적 오류를 발생시킵니다.
...
2. 생성된 `mcp_*` 도구 이름에서 정제된 이름 충돌을 방지하기 위해 영숫자/밑줄 이름을 권장합니다.
3. 의도치 않은 stdio 기본값을 방지하기 위해 명시적 `type`을 사용하세요.
4. `enabled: false`는 완전 비활성으로 처리합니다: 서버가 런타임 연결 세트에서 제외됩니다.
