---
title: '네이티브 Shell, PTY, Process 및 Key 내부 구조'
description: '네이티브 레이어에서의 Shell 실행, PTY 관리, 프로세스 수명 주기 및 키 이벤트 처리.'
sidebar:
  order: 4
...
# 네이티브 Shell, PTY, Process 및 Key 내부 구조

이 문서는 `@f5xc-salesdemos/pi-natives`의 **실행/프로세스/터미널 프리미티브**인 `shell`, `pty`, `ps`, `keys`를 다루며, `docs/natives-architecture.md`의 아키텍처 용어를 사용합니다.
...
- **TS 래퍼/API 레이어** (`packages/natives/src/*`): 타입이 지정된 진입점, 취소 표면(`timeoutMs`, `AbortSignal`), JS 사용 편의성.
- **Rust N-API 모듈 레이어** (`crates/pi-natives/src/*`): shell/PTY 프로세스 실행, 프로세스 트리 순회/종료, 키 시퀀스 파싱.
...
1. **원샷(One-shot)** — `executeShell(options, onChunk?)` 사용.
2. **영속 세션** — `new Shell(options?)` 생성 후 `shell.run(...)` 반복 호출.

둘 다 스레드 안전 콜백을 통해 출력을 스트리밍하며 `{ exitCode?, cancelled, timedOut }`를 반환합니다.
...
- `ShellRunOptions.env`는 명령어 범위(`EnvironmentScope::Command`)이며 각 실행 후 제거됩니다.
...
영속 셸(`Shell.run`)은 다음 상태 머신을 사용합니다:

- **유휴/미초기화**: `session: None`.
- **실행 중**: 첫 번째 `run()`이 세션을 지연 생성하고, `current_abort` 토큰을 저장하며, 명령어를 실행합니다.
- **완료 + 유지**: 실행 제어 흐름이 `Normal`이면, `current_abort`가 해제되고 세션이 재사용됩니다.
- **완료 + 해제**: 제어 흐름이 루프/스크립트/셸 종료 관련(`BreakLoop`, `ContinueLoop`, `ReturnFromFunctionOrScript`, `ExitShell`)인 경우, 세션이 해제됩니다(`session: None`).
- **취소됨/시간 초과**: 실행 태스크가 취소되고, 유예 대기(2초) 후 강제 중단; 세션이 해제됩니다.
- **오류**: 세션이 해제됩니다.

원샷 셸(`executeShell`)은 항상 호출마다 새 세션을 생성하고 해제합니다.

### 스트리밍/출력 동작

- Stdout/stderr는 공유 파이프로 라우팅되어 동시에 읽힙니다.
- 리더는 UTF-8을 점진적으로 디코딩하며, 유효하지 않은 바이트 시퀀스는 `U+FFFD` 대체 청크로 출력됩니다.
- 프로세스 완료 후, 출력 드레인에는 백그라운드 작업이 디스크립터를 열어둔 상태로 행이 걸리는 것을 방지하기 위한 유휴/최대 가드(`250ms` 유휴, `2s` 최대)가 있습니다.

### 취소, 시간 초과, 백그라운드 작업

- `CancelToken`은 `timeoutMs` 및 선택적 `AbortSignal`로 구성됩니다.
- 취소/시간 초과 시, 셸 취소 토큰이 트리거된 다음 태스크에 강제 중단 전 2초의 유예 기간이 주어집니다.
- 취소가 발생하면 백그라운드 작업은 brush 작업 메타데이터를 사용하여 종료됩니다(`TERM`, 이후 지연된 `KILL`).

`Shell.abort()` 동작:

- 해당 `Shell` 인스턴스의 현재 실행 중인 명령어만 중단합니다,
- 실행 중인 것이 없을 때는 무응답 성공입니다.
...
- abort 시그널 -> `exitCode: undefined`, `cancelled: true`.

## PTY 서브시스템 (`pty`)

### API 모델

`new PtySession()`이 제공하는 메서드:
...
- **유휴**: `core: None`.
- **예약됨**: `start()`가 비동기 작업 시작 전에 제어 채널을 동기적으로 설치하므로(`core: Some`), `write/resize/kill`이 즉시 유효해집니다.
...
- 명령어는 현재 선택적 `cwd` 및 환경 오버라이드와 함께 `sh -lc <command>`로 실행됩니다.
- `write()`는 원시 바이트를 PTY stdin으로 전송합니다.
- `resize()`는 크기를 클램핑하고(`cols 20..400`, `rows 5..200`) 마스터 리사이즈를 호출합니다.
- `kill()`은 실행을 취소됨으로 표시하고 자식 프로세스를 종료합니다.

출력 경로:

- 전용 리더 스레드가 마스터 스트림을 읽습니다,
- 유효하지 않은 바이트에 대해 `U+FFFD` 대체를 사용한 점진적 UTF-8 디코드,
- 청크는 N-API 스레드 안전 콜백을 통해 전달됩니다.

### 취소 및 시간 초과 의미론

- `timeoutMs`와 `AbortSignal`이 `CancelToken`에 입력됩니다.
- 루프는 주기적으로 `ct.heartbeat()`를 호출하며, 중단 시 자식을 종료합니다.
...
- 자손이 재귀적으로 수집됩니다.
- 종료 순서는 하향식(가장 깊은 자손 우선)으로 고아 재부모화를 줄입니다.
- 루트 pid가 마지막에 종료됩니다.
- 반환 값은 성공적인 종료 횟수입니다.
...
- 누락되거나 접근할 수 없는 프로세스 트리 분기는 건너뜁니다,
- pid별 종료 실패는 실패(오류가 아님)로 카운트됩니다,
...
- 키 ID 정규화(`ctrl+c`, `shift+tab`, `pageUp`, `f5` 등).
...
- 기본 레이아웃 폴백은 리매핑된 레이아웃이 ASCII 문자/기호에 대해 잘못된 매치를 생성하지 않도록 의도적으로 제한됩니다.

### 실패 동작

- 인식되지 않거나 유효하지 않은 시퀀스는 파싱 함수에서 `null`을 생성합니다.
- 매치 함수는 파싱 실패 또는 불일치 시 `false`를 반환합니다.
- 잘못된 형식의 키 입력에 대한 예외 발생 표면이 없습니다.
...
| `matchesKey(data, keyId, kittyProtocolActive)` | `matchesKey` (`matches_key`) | 상위 수준 키 매처 |

## 방치된 세션 정리 및 최종화 참고사항

- **Shell 영속 세션**: 실행이 취소/시간 초과/오류/비유지 제어 흐름인 경우, Rust가 내부 세션 상태를 명시적으로 해제합니다. 성공적인 정상 실행은 재사용을 위해 세션을 유지합니다.
- **PTY 세션**: `core`는 실패 경로를 포함하여 `start()` 완료 후 항상 해제됩니다.
- 래퍼에 의해 **명시적인 JS 파이널라이저 기반 종료 계약이 노출되지 않습니다**; 정리는 주로 실행 완료/취소 경로에 연결됩니다. 호출자는 결정적 해제를 위해 `timeoutMs`, `AbortSignal`, `shell.abort()`, 또는 `pty.kill()`을 사용해야 합니다.
