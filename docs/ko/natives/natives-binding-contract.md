...
- `TsFunc<T>` N-API threadsafe 콜백에서 사용하는 콜백 형태
...
이를 통해 하나의 통합 바인딩 인터페이스를 모놀리식 중앙 타입 파일 없이 유지할 수 있습니다.

## 선언 병합 수명 주기 및 상태 전이

### 1) 컴파일 타임 타입 조립

- `bindings.ts`가 기본 `NativeBindings` 심볼을 제공합니다.
- 모든 `src/<module>/types.ts`가 `NativeBindings`를 보강합니다.
- `src/native.ts`가 모든 `./<module>/types` 파일을 사이드 이펙트용으로 임포트하여 `NativeBindings`가 사용되는 곳에서 병합된 계약이 스코프에 포함됩니다.

상태 전이: **기본 계약** → **병합된 계약**.

### 2) 런타임 애드온 로드 및 검증 게이트

- `src/native.ts`가 후보 `.node` 바이너리를 로드합니다.
- 로드된 객체는 `NativeBindings`로 취급되어 즉시 `validateNative(...)`를 통과합니다.
- `validateNative`는 `typeof bindings[name] === "function"`으로 필수 내보내기 키를 검증합니다.

상태 전이: **신뢰할 수 없는 애드온 객체** → **검증된 네이티브 바인딩 객체** (또는 하드 실패).

### 3) 래퍼 호출

- `src/<module>/index.ts`의 모듈 래퍼가 `native.<export>`를 호출합니다.
- 래퍼는 기본값과 콜백 형태를 적응합니다 (`(err, value)`를 JS API의 값 전용 콜백 패턴으로 변환).
- `src/index.ts`가 모듈 래퍼/타입을 공개 패키지 API로 재내보내기합니다.

상태 전이: **검증된 원시 바인딩** → **사용하기 편리한 공개 API**.

## 래퍼 책임

래퍼는 의도적으로 얇게 구성되어 있으며, 네이티브 로직을 다시 구현하지 않습니다.
...
- **콜백 적응**
  - `grep()`, `glob()`, `executeShell()`은 `TsFunc<T>` (`error, value`)를 성공한 값만 수신하는 사용자 콜백으로 변환합니다.
- **네이티브 호출 관련 환경 또는 정책 동작**
  - 클립보드 래퍼는 OSC52/Termux/헤드리스 처리를 추가하고 복사를 최선 노력(best effort)으로 취급합니다.
- **공개 명명 및 재내보내기 큐레이션**
  - `searchContent()`는 네이티브 내보내기 `search`에 매핑됩니다.

## 공개 내보내기 표면 구성

`packages/natives/src/index.ts`가 정식 공개 배럴입니다. 기능 도메인별로 내보내기를 그룹화합니다:

- 검색/텍스트: `grep`, `glob`, `text`, `highlight`
- 실행/프로세스/터미널: `shell`, `pty`, `ps`, `keys`
- 시스템/미디어/변환: `image`, `html`, `clipboard`, `system-info`, `work`

유지보수자 규칙: 래퍼가 `src/index.ts`에서 재내보내기되지 않으면, 의도된 공개 패키지 표면의 일부가 아닙니다.

## JS API ↔ 네이티브 내보내기 매핑 (대표적)

Rust 측은 N-API 내보내기 이름(일반적으로 `#[napi]` snake_case -> camelCase 변환, 때때로 명시적 별칭 포함)을 사용하며, 이는 이러한 바인딩 키와 일치해야 합니다.
...
계약은 동기와 비동기 API를 혼합합니다. 래퍼는 하나의 모델을 강제하기보다 네이티브 호출 스타일을 보존합니다:

- **Promise 기반 비동기 내보내기**: I/O 또는 장시간 실행 작업용 (`grep`, `glob`, `htmlToMarkdown`, `executeShell`, 클립보드, 이미지 작업).
- **동기 내보내기**: 결정론적 인메모리 변환/파서용 (`search`, `hasMatch`, 하이라이팅, 텍스트 너비/슬라이싱, 키 파싱, 프로세스 조회).
- **생성자 내보내기**: 상태를 가진 런타임 객체용 (`Shell`, `PtySession`, `PhotonImage`).

유지보수자를 위한 함의: 기존 내보내기의 동기 ↔ 비동기를 변경하는 것은 래퍼와 호출자 전반에 걸친 파괴적 API 및 계약 변경입니다.

## 객체 및 열거형 타이핑 패턴

### 객체 패턴 (`#[napi(object)]` 스타일 JS 객체)

TypeScript는 객체 형태의 네이티브 값을 인터페이스로 모델링합니다. 예를 들어:

- `GrepResult`, `SearchResult`, `GlobResult`
- `SystemInfo`, `WorkProfile`
- `ClipboardImage`, `ParsedKittyResult`

이는 컴파일 타임의 구조적 계약이며, 런타임 형태 정확성은 네이티브 구현이 담당합니다.

### 열거형 패턴

숫자형 네이티브 열거형은 TypeScript에서 `const enum` 값으로 표현됩니다:

- `FileType` (`1=file`, `2=dir`, `3=symlink`)
- `ImageFormat` (`0=PNG`, `1=JPEG`, `2=WEBP`, `3=GIF`)
- `SamplingFilter`, `Ellipsis`, `KeyEventType`

호출자는 명명된 열거형 멤버를 보며, 바인딩 경계에서는 숫자가 전달됩니다.

## 불일치 감지 방법

불일치 감지는 두 계층에서 발생합니다:

1. **컴파일 타임 TypeScript 계약 검사**
   - 래퍼가 병합된 `NativeBindings`에 대해 `native.<name>`을 호출합니다.
   - 누락/이름 변경된 바인딩 키는 래퍼에서 TS 타입 검사를 실패시킵니다.

2. **`validateNative`의 런타임 검증**
   - 로드 후 `native.ts`가 필수 내보내기를 검사하고 누락된 것이 있으면 throw합니다.
   - 오류 메시지에 누락된 키와 재빌드 지침이 포함됩니다.

이는 일반적인 오래된 바이너리 불일치를 포착합니다: 래퍼/타입은 존재하지만 로드된 `.node`에 해당 내보내기가 없는 경우입니다.

## 실패 동작 및 주의사항

### 로드/검증 실패 (하드 실패)

- 애드온 로드 실패 또는 지원되지 않는 플랫폼은 `native.ts`에서 모듈 초기화 중에 throw합니다.
- 필수 내보내기 누락은 래퍼를 사용할 수 있기 전에 throw합니다.

효과: 패키지가 첫 번째 호출로 실패를 지연시키지 않고 빠르게 실패합니다.

### 래퍼 수준 동작 차이

- 일부 래퍼는 의도적으로 실패를 완화합니다 (`copyToClipboard`는 최선 노력이며 네이티브 실패를 삼킵니다).
- 스트리밍 콜백은 콜백 오류 페이로드를 무시하고 성공한 값 이벤트만 전달합니다.

### 타입 수준 주의사항 (런타임이 TS보다 엄격)

- TS 선택적 필드는 의미적 유효성을 보장하지 않으며, 네이티브 계층은 여전히 잘못된 형태의 값을 거부할 수 있습니다.
- `const enum` 타이핑은 런타임에서 타입이 지정되지 않은 호출자의 범위 밖 숫자 값을 방지하지 못합니다.
- `validateNative`는 필수 내보내기의 존재/함수 여부만 검사하며, 깊은 인수/반환 형태 호환성은 검사하지 않습니다.
- `bindings.ts`는 기본 인터페이스에 `cancelWork(id)`를 포함하지만, 현재 런타임 검증 목록은 해당 키를 강제하지 않습니다.

## 바인딩 변경을 위한 유지보수자 체크리스트

내보내기를 추가/변경할 때 다음을 모두 업데이트하십시오:
...
어떤 단계라도 건너뛰면 컴파일 타임 불일치 또는 런타임 로드 시 실패가 발생합니다.
