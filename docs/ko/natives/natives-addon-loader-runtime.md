...
description: '플랫폼 감지, 폴백 전략, 모듈 해석 기능을 갖춘 N-API 애드온 로더 런타임.'
sidebar:
  order: 3
  label: 애드온 로더
i18n:
...
이 문서는 `@f5xc-salesdemos/pi-natives`의 애드온 로딩/검증 레이어를 심층적으로 다룹니다: `native.ts`가 어떤 `.node` 파일을 로드할지 결정하는 방법, 임베디드 페이로드 추출이 실행되는 시점, 그리고 시작 실패가 보고되는 방식을 설명합니다.
...
- 플랫폼/CPU 인식 기반의 애드온 파일명 및 디렉터리 후보 목록을 구성합니다.
- 선택적으로 임베디드 애드온을 버전별 사용자 캐시 디렉터리에 물리화합니다.
- 결정론적 순서로 후보를 시도합니다.
- 바인딩을 노출하기 전에 `validateNative`를 통해 오래되거나 호환되지 않는 애드온을 거부합니다.

이 범위에 포함되지 않는 것: 모듈별 grep/text/highlight 동작.
...
- **핵심 디렉터리**:
  ...
  - `userDataDir` 폴백:
    ...
    - 비Windows: `~/.local/bin`.
- **컴파일 바이너리 모드** (`isCompiledBinary`): 다음 중 하나라도 해당하면 true:
  - `PI_COMPILED` 환경 변수가 설정된 경우, 또는
...
- 로더는 먼저 계산된 모든 후보를 시도합니다.
- 아무것도 로드되지 않으면 지원되는 태그 목록과 함께 명시적인 미지원 플랫폼 오류를 발생시킵니다.

이는 정말로 지원되지 않는 대상에 대해서는 여전히 강하게 실패하면서도, 근접한 경우에 유용한 진단 정보를 보존합니다.
...
2. 그렇지 않으면 AVX2 지원을 감지합니다:
   - Linux: `/proc/cpuinfo`에서 `avx2`를 스캔합니다.
...
`tag = <platform>-<arch>`가 주어질 때:

- 비x64 또는 변형 없음: `pi_natives.<tag>.node`
- x64 + `modern`: 다음 순서로 시도
  1. `pi_natives.<tag>-modern.node`
  2. `pi_natives.<tag>-baseline.node` (의도적 폴백)
- x64 + `baseline`: `pi_natives.<tag>-baseline.node`만 시도

최종 오류 메시지에 사용되는 `addonLabel`은 `<tag>` 또는 `<tag> (<variant>)` 형태입니다.
...
변형 해석된 파일명 목록을 기반으로 구성되며 다음 순서로 검색됩니다:
...
`dedupedCandidates`는 첫 번째 발생 순서를 유지하면서 중복을 제거합니다.
...
- `platformTag`
- `version`
- `files[]` - 각 항목은 `variant`, `filename`, `filePath`를 가짐
...
- 비x64: `default`를 선호하고, 그 다음 첫 번째 사용 가능한 파일.
- x64 + `modern`: `modern`을 선호하고, `baseline`으로 폴백.
- x64 + `baseline`: `baseline`을 필수로 요구.

물리화 동작:

1. `<versionedDir>`이 존재하는지 확인합니다 (`mkdirSync(..., { recursive: true })`).
2. `<versionedDir>/<selected filename>`이 이미 존재하면 재사용합니다 (재작성 없음).
3. 그렇지 않으면 임베디드 소스 `filePath`를 읽고 대상 파일을 작성합니다.
4. 최우선 로드 시도를 위해 대상 경로를 반환합니다.

실패 시 추출은 즉시 크래시하지 않으며, 오류 항목(디렉터리 생성 또는 쓰기 실패)을 추가하고 로더는 일반 후보 탐색으로 진행합니다.
...
`validateNative(bindings, source)`는 시작 시 `NativeBindings`에 대한 함수 전용 계약을 강제합니다.

메커니즘:

- 각 필수 내보내기 이름에 대해 `typeof bindings[name] === "function"`을 확인합니다.
- 누락된 이름이 집계됩니다.
- 하나라도 누락되면 로더가 다음을 포함하는 오류를 발생시킵니다:
  - 소스 애드온 경로,
  - 누락된 내보내기 목록,
  - 재빌드 명령어 힌트.

이는 오래된 바이너리, 부분 빌드, 심볼/이름 드리프트에 대한 강력한 호환성 게이트입니다.

### JS API ↔ 네이티브 내보내기 매핑 (검증 게이트)

| `validateNative`에서 확인하는 JS 바인딩 이름 | 예상 네이티브 내보내기 이름 |
| --- | --- |
| `grep` | `grep` |
| `glob` | `glob` |
...
참고: `bindings.ts`는 기본 `cancelWork(id)` 멤버만 선언하며, 모듈 `types.ts` 파일이 `validateNative`가 강제하는 추가 심볼을 선언 병합합니다.

## 실패 동작 및 진단

## 미지원 플랫폼

모든 후보가 실패하고 `platformTag`가 `SUPPORTED_PLATFORMS`에 없으면, 로더는 다음을 포함하는 오류를 발생시킵니다:
...
- x64에서 잘못된 변형 아티팩트가 선택됨.
- 로드된 아티팩트에 새로운 Rust 내보내기가 존재하지 않음.

로더 동작:

- 후보별 누락 내보내기 실패를 기록합니다.
- 나머지 후보를 계속 탐색합니다.
- 검증을 통과하는 후보가 없으면, 최종 오류에 각 실패 메시지와 함께 시도된 모든 경로가 포함됩니다.
...
- 오래된 `<versionedDir>`을 삭제하고 재실행하는 조치 방법,
- 각 예상 파일명에 대한 직접 릴리스 다운로드 `curl` 명령어.
...
- 로컬 재빌드 명령어 (`bun --cwd=packages/natives run build`),
- 선택적 x64 변형 빌드 힌트 (`TARGET_VARIANT=baseline|modern ...`).

## 런타임 동작

- 로더는 항상 릴리스 후보 체인을 사용합니다.
- `PI_DEV` 설정은 후보별 콘솔 진단만 활성화합니다 (`Loaded native addon...` 및 로드 오류).
