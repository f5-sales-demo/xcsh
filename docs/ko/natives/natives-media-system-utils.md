...
이 문서는 [`docs/natives-architecture.md`](./natives-architecture.md)에 설명된 **시스템/미디어/변환 프리미티브** 레이어의 하위 시스템 심층 분석입니다: `image`, `html`, `clipboard`, 그리고 `work` 프로파일링.
...
- **Rust 디코드 경계**: 바이트가 `Vec<u8>`로 복사되고, `ImageReader::with_guessed_format()`으로 형식을 추측한 후 `DynamicImage`로 디코딩됩니다.
- **인메모리 상태**: `PhotonImage`는 `Arc<DynamicImage>`를 저장합니다.
...
- 지원하지 않는 형식 ID는 실패합니다 (`Invalid image format: <id>`).
...
- `cleanContent=true`일 때, `PreprocessingPreset::Aggressive`와 네비게이션/폼의 강제 제거 플래그로 전처리가 활성화됩니다.
...
- **텍스트 경로**:
  ...
  - 같은 텍스트가 네이티브 클립보드 API (`native.copyToClipboard`)를 통해 최선 노력(best-effort)으로 시도됩니다.
  - Termux에서는 TS가 먼저 `termux-clipboard-set`을 시도합니다.
- **이미지 읽기 경로**:
  - Rust가 `arboard`에서 원시 이미지를 읽습니다.
  - Rust가 PNG 바이트로 재인코딩합니다 (`image` 크레이트), `{ data: Uint8Array, mimeType: "image/png" }`를 반환합니다.
  - TS는 Termux 또는 디스플레이 서버가 없는 Linux 세션 (`DISPLAY`/`WAYLAND_DISPLAY` 미설정)에서 조기에 `null`을 반환합니다.
...
- **출력 경계**: `getWorkProfile(lastSeconds)`는 객체를 반환합니다:
  - `folded`: 폴디드 스택 텍스트 (flamegraph 입력)
  - `summary`: 마크다운 테이블 요약
...
1. `PhotonImage.parse(bytes)`는 블로킹 디코드 작업 (`image.decode`)을 스케줄링합니다.
2. 성공 시, JS에 네이티브 `PhotonImage` 핸들이 존재합니다.
3. `resize(...)`는 새로운 네이티브 핸들 (`image.resize`)을 생성하며, 기존 핸들과 새 핸들이 공존할 수 있습니다.
...
1. `htmlToMarkdown(html, options)`는 블로킹 변환 작업을 스케줄링합니다.
2. 변환은 지정하지 않는 한 기본 옵션 (`cleanContent=false`, `skipImages=false`)으로 실행됩니다.
...
`copyToClipboard(text)`는 의도적으로 최선 노력(best-effort) 및 다중 경로 방식입니다:

1. TTY인 경우: OSC 52 쓰기를 시도합니다 (base64 페이로드).
2. `TERMUX_VERSION`이 설정되어 있으면 Termux 명령을 시도합니다.
3. 네이티브 `arboard` 텍스트 복사를 시도합니다.
4. TS 레이어에서 오류를 무시합니다.

`readImageFromClipboard()`의 엄격도는 단계별로 다릅니다:

1. TS가 지원하지 않는 런타임 컨텍스트 (Termux/헤드리스 Linux)를 `null`로 강제 차단합니다.
...
1. 명시적 시작 없음: 작업 헬퍼가 실행될 때 프로파일링이 항상 활성화됩니다.
2. 계측된 모든 작업 스코프는 가드 드롭 시 하나의 샘플을 기록합니다.
3. 버퍼 용량에 도달하면 샘플이 가장 오래된 항목을 덮어씁니다.
4. `getWorkProfile(lastSeconds)`는 시간 창을 읽고 folded/summary/svg 아티팩트를 생성합니다.

실패 전이:

- SVG 생성 실패는 소프트 실패입니다 (`svg: null`), folded와 summary는 여전히 반환됩니다.
...
- 지원하지 않는 디코드 입력 또는 손상된 바이트: 엄격한 실패 (프로미스 거부).
- 지원하지 않는 인코드 형식 ID: 엄격한 실패.
- TS 래퍼에 최선 노력 폴백 경로 없음.
...
- 텍스트 복사는 TS 레이어에서 최선 노력(best-effort): 작동 실패가 억제됩니다.
- 이미지 읽기는 "이미지 없음" (`null`)과 작동 실패 (거부)를 구분합니다.
- Termux/헤드리스 Linux는 이미지 읽기에 대해 미지원 컨텍스트로 처리됩니다 (`null`).

### 작업 프로파일링

- 검색은 함수 호출 자체에 대해 엄격하지만, 아티팩트 생성은 부분적으로 최선 노력입니다 (`svg` null 가능).
- 버퍼 잘림은 예상된 동작 (링 버퍼)이며, 데이터 손실 버그가 아닙니다.

## 플랫폼 주의사항

- **클립보드 텍스트**: OSC 52는 터미널 지원에 의존하며, 네이티브 클립보드 접근은 데스크톱 환경/세션에 의존합니다.
- **클립보드 이미지 읽기**: Termux 및 디스플레이 서버가 없는 Linux에서 TS에서 차단됩니다.
