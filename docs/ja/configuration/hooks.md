...
フックパッケージ（`src/extensibility/hooks/`）は引き続きAPIサーフェスとしてエクスポートおよび利用可能ですが、デフォルトのCLIランタイムは現在**エクステンションランナー**パスを初期化します。現在の起動フローでは：
...
そのため、このファイルではフックサブシステムの実装自体（型/ローダー/ランナー/ラッパー）を、レガシーの動作と制約を含めてドキュメント化しています。

## 主要ファイル

- `src/extensibility/hooks/types.ts` — フックコンテキスト、イベント型、結果コントラクト
- `src/extensibility/hooks/loader.ts` — モジュールローディングとフックディスカバリブリッジ
...
1. ケイパビリティレジストリからディスカバーされたフックを読み込む（`loadCapability("hooks")`）
...
`hookCapability` のディスカバリプロバイダーは、依然としてシェルスタイルの前後フックファイル（例：`.claude/hooks/pre/*`、`.xcsh/.../hooks/pre/*`）をモデル化しています。
...
tool_call ハンドラ
   │
   ├─ { block: true } あり？ ── はい ──> throw（ツールブロック）
   │
   └─ いいえ
      │
      ▼
   基盤ツールを実行
      │
      ├─ 成功 ──> tool_result ハンドラが { content, details } をオーバーライド可能
      │
...
- ハンドラがスローした場合、ラッパーはフェイルクローズドで実行をブロックします
- 返された `reason` がスローされるエラーテキストになります

### 2) ツール実行

ブロックされていない場合、基盤ツールは通常通り実行されます。

### 3) 実行後：`tool_result`

成功後、ラッパーは以下の内容で `tool_result` をエミットします：
...
ツール失敗時、ラッパーは `isError: true` とエラーテキストコンテンツで `tool_result` をエミットし、その後元のエラーを再スローします。
...
- `session_before_*` と `session.compacting` を通じたキャンセル/カスタムコンパクション/ツリー動作
...
`hooks` の場合、ケイパビリティキーは `${type}:${tool}:${name}` です。低優先度プロバイダーからのシャドウされた重複はマークされ、有効なディスカバーリストから除外されます。

### ロード順序

`discoverAndLoadHooks` は解決済み絶対パスで重複排除されたフラットな `allPaths` リストを構築し、`loadHooks` がその順序で反復処理します。
各ディスカバーされたディレクトリ内のファイル順序は `readdir` の出力に依存します。フックローダーは追加のソートを行いません。

### ランタイムハンドラの順序

`HookRunner` 内部では、順序は登録シーケンスにより決定的です：
...
- `tool_call`：ハンドラがブロックしない限り、最後に返された結果が優先。最初のブロックでショートサーキット
- `tool_result`：最後に返されたオーバーライドが優先（ショートサーキットなし）
- `context`：チェーン処理。各ハンドラは前のハンドラのメッセージ出力を受け取る
- `before_agent_start`：最初に返されたメッセージが保持される。後のメッセージは無視
- `session_before_*`：最後に返された結果が追跡される。`cancel: true` で即座にショートサーキット
...
UIなしで実行する場合、デフォルトのノーオプコンテキスト動作は：

- `select/input/editor` は `undefined` を返す
- `confirm` は `false` を返す
- `notify`、`setStatus`、`setEditorText` はノーオプ
- `getEditorText` は `""` を返す
...
- 無効なモジュールまたはデフォルトエクスポートの欠落 → `LoadHooksResult.errors` にキャプチャされる
- 他のフックのローディングは継続される

### イベント時

`HookRunner.emit(...)` はほとんどのイベントでハンドラエラーをキャッチし、リスナーに `HookError`（`hookPath`、`event`、`error`）をエミットしてから処理を継続します。

`emitToolCall(...)` はより厳格です：ハンドラエラーはそこでは飲み込まれず、呼び出し元に伝播します。`HookToolWrapper` では、これによりツール呼び出しがブロックされます（フェイルセーフ）。
...
### コマンドセーフなコンテキストメソッドによるスラッシュコマンドの登録
...
パッケージルート（`src/index.ts`）はレガシー互換サーフェスとしてフック**型**を再エクスポートします。
