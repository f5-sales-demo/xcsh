...
SDKは `@f5xc-salesdemos/xcsh` のインプロセス統合インターフェースです。
自身のBun/Nodeプロセスからエージェントの状態、イベントストリーミング、ツール接続、セッション制御に直接アクセスしたい場合に使用します。

クロス言語/プロセス分離が必要な場合は、代わりにRPCモードを使用してください。
...
`@f5xc-salesdemos/xcsh` はパッケージルートから（また `@f5xc-salesdemos/xcsh/sdk` 経由でも）SDK APIをエクスポートします。
...
## `createAgentSession()` がデフォルトでディスカバリーする内容

`createAgentSession()` は「オーバーライドするには提供し、ディスカバリーするには省略する」という方針に従います。
...
- `sessionManager`: `SessionManager.create(cwd)`（ファイルベース）
- skills/context files/prompt templates/slash commands/extensions/custom TSコマンド
...
通常、制御したいものだけを提供する必要があります：

- **必須提供**: 最小限のセッションには不要
- **埋め込み時に通常明示的に提供するもの**:
    - `sessionManager`（インメモリまたはカスタムロケーションが必要な場合）
    - `authStorage` + `modelRegistry`（認証情報/モデルのライフサイクルを自分で管理する場合）
    - `model` または `modelPattern`（決定論的なモデル選択が重要な場合）
    - `settings`（分離された/テスト用の設定が必要な場合）

## セッションマネージャーの動作（永続化 vs インメモリ）

`AgentSession` は常に `SessionManager` を使用します。動作はどのファクトリーを使用するかによって異なります。
...
- resume/open/list/forkワークフローをサポートします。
...
- ファイルシステムへの永続化なし。
- テスト、エフェメラルワーカー、リクエストスコープのエージェントに便利です。
- セッションメソッドは引き続き動作しますが、永続化固有の動作（ファイルresume/forkパス）は自然に制限されます。

### Resume/open/listヘルパー
...
1. 既存のセッションからモデルを復元（復元可能でキーが利用可能な場合）
...
- `toolNames` はビルトインの許可リストとして機能します。
- `customTools` とエクステンション登録ツールは引き続き含まれます。
- 隠しツール（例：`submit_result`）はオプションで必要とされない限りオプトインです。
...
- `additionalExtensionPaths`: 追加のエクステンションファイルを読み込む
- `disableExtensionDiscovery`: 自動エクステンションスキャンを無効化
- `preloadedExtensions`: 既に読み込まれたエクステンションセットを再利用
...
- `outputSchema`: 構造化出力の期待をツールコンテキストに渡す
...
`setToolUIContext(...)` は、ツール/エクステンションが呼び出すべきUI機能を埋め込み側が提供する場合にのみ使用してください。
