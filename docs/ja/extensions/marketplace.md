...
description: 厳選されたプラグインコレクションの検出、インストール、管理を行うマーケットプレイスプラグインシステム。
sidebar:
  order: 4
  label: マーケットプレイス
i18n:
...
マーケットプレイスシステムでは、Gitホスト型カタログからプラグインを検出、インストール、管理できます。Claude Codeプラグインレジストリ形式と互換性があります。
...
または引数なしで `/marketplace` と入力するだけで、インタラクティブなプラグインブラウザが開きます。

## コンセプト

**マーケットプレイス**とは、`.xcsh-plugin/marketplace.json` にカタログファイルを含むGitリポジトリ（またはローカルディレクトリ）です。カタログには、利用可能なプラグインのソース、説明、メタデータが一覧化されています。
...
プロジェクトスコープのインストールは、同じプラグインのユーザースコープのインストールをシャドウします。
...
| `/marketplace update [name]` | カタログを再取得。名前を省略するとすべてを更新 |
...
| ソース形式 | タイプ | 例 |
|---|---|---|
| `owner/repo` | GitHub短縮形 | `anthropics/f5xc-salesdemos-marketplace` |
| `https://...*.json` | ダイレクトカタログURL | `https://example.com/marketplace.json` |
| `https://...*.git` または `git@...` | Gitリポジトリ | `https://github.com/org/repo.git` |
| `./path` または `~/path` または `/path` | ローカルディレクトリ | `./my-marketplace` |

システムはリポジトリをクローン（またはローカルディレクトリを読み取り）し、`.xcsh-plugin/marketplace.json` を検出して検証し、カタログをローカルにキャッシュします。
...
| フィールド | 説明 |
|---|---|
| `name` | マーケットプレイス名。小文字英数字、ハイフン、ドットが使用可能。英数字で開始・終了する必要あり。最大64文字。 |
...
| フィールド | 必須 | 説明 |
|---|---|---|
| `name` | はい | プラグイン名（マーケットプレイス名と同じ命名規則） |
...
| `tags` | いいえ | 文字列タグの配列 |
| `strict` | いいえ | ブール値 |
| `commands` | いいえ | 提供されるスラッシュコマンド |
| `agents` | いいえ | 提供されるエージェント |
| `hooks` | いいえ | フック定義 |
...
~/.xcsh/
  config/
    marketplaces.json          # 追加済みマーケットプレイスのレジストリ
  plugins/
...
マーケットプレイス名およびプラグイン名は以下の条件を満たす必要があります：
