...
description: キュレーションされたプラグインコレクションの発見、インストール、管理のためのマーケットプレイスプラグインシステム。
sidebar:
  order: 4
  label: マーケットプレイス
i18n:
...
マーケットプレイスシステムを使用すると、Gitホストのカタログからプラグインを発見、インストール、管理できます。Claude Codeプラグインレジストリ形式と互換性があります。
...
**マーケットプレイス**とは、`.xcsh-plugin/marketplace.json` にカタログファイルを含むGitリポジトリ（またはローカルディレクトリ）です。カタログには、利用可能なプラグインとそのソース、説明、メタデータがリストされています。
...
システムはリポジトリをクローン（またはローカルディレクトリを読み取り）し、`.xcsh-plugin/marketplace.json` を検出、検証して、カタログをローカルにキャッシュします。
...
| フィールド | 説明 |
|---|---|
| `name` | マーケットプレイス名。小文字の英数字、ハイフン、ドットが使用可能。英数字で始まり英数字で終わる必要あり。最大64文字。 |
...
マーケットプレイスおよびプラグインの名前は以下の条件を満たす必要があります：

- 小文字の英字または数字で始まり、小文字の英字または数字で終わること
- 小文字の英字、数字、ハイフン、ドットのみを含むこと
- 最大64文字であること

プラグインID（`name@marketplace`）は合計で最大128文字までです。
