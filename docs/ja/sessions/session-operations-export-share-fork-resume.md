...
このドキュメントでは、現在実装されているセッションのエクスポート/共有/フォーク/再開操作について、オペレーターが確認できる動作を説明します。
...
| 操作 | エントリパス | セッションの変更 | セッションファイルの作成/切替 | 出力アーティファクト |
|---|---|---|---|---|
| `/dump` | インタラクティブスラッシュコマンド | なし | なし | クリップボードテキスト |
| `/export [path]` | インタラクティブスラッシュコマンド | なし | なし | HTMLファイル |
| `--export <session.jsonl> [outputPath]` | CLI起動時のファストパス | ランタイムセッションの変更なし | アクティブセッションなし; 対象ファイルを読み取り | HTMLファイル |
...
| `/resume` | インタラクティブスラッシュコマンド | あり（アクティブなインメモリ状態が置換） | 選択された既存のセッションファイルに切替 | なし |
| `--resume` | CLI起動時（ピッカー） | セッション作成後にあり | 選択された既存のセッションファイルを開く | なし |
| `--resume <id\|path>` | CLI起動時 | セッション作成後にあり | 既存セッションを開く; クロスプロジェクトの場合は現在のプロジェクトにフォーク可能 | なし |
| `--continue` | CLI起動時 | セッション作成後にあり | ターミナルブレッドクラムまたは最新セッションを開く; 存在しない場合は新規作成 | なし |
...
4. 成功時、UIにパスを表示し、ブラウザでファイルを開く。

動作の詳細:

- `--copy`、`clipboard`、`copy` 引数は明示的に拒否され、`/dump` を使用するよう警告が表示される。
- エクスポートにはセッションヘッダー/エントリ/リーフに加え、エージェント状態からの現在の `systemPrompt` とツール説明が埋め込まれる。
- エクスポート中にセッションエントリが追加されることはない。

注意点:

- 引数解析はホワイトスペースベース（`text.split(/\s+/)`）であるため、スペースを含むクォートされたパスはこのコマンドパスでは単一パスとして保持されない。

### `--export <inputSessionFile> [outputPath]`（CLI）

`main.ts` でのフロー:

1. インタラクティブ/セッション起動前の早期段階で処理。
2. `exportFromFile(inputPath, outputPath?)` を呼び出し。
3. `SessionManager.open(inputPath)` がエントリを読み込み、HTMLが生成・書き出される。
4. プロセスが `Exported to: ...` を出力して終了。

動作の詳細:

- 入力ファイルが見つからない場合は `File not found: <path>` として表示。
...
3. それ以外の場合、ネイティブの `copyToClipboard` 経由でクリップボードにコピー。
...
- カスタム/フック/ファイル言及/ブランチサマリー/コンパクションサマリーエントリ
...
- ハンドラー結果の解釈:
  - string => URLとして扱い、表示して開く
  - object => `url` および/または `message` を表示; `url` を開く
...
- カスタムハンドラーが実行されてスローした場合、コマンドはエラーで返る。
- 両方の失敗ケースで、GitHub gistへのフォールバックは**行われない**。
- Gistフォールバックはカスタム共有スクリプトが存在しない場合にのみ発生。
...
4. Gist URLを解析し、gist idを導出し、プレビューURL `https://gistpreview.github.io/?<id>` を構築。
5. プレビューとgist URLの両方を表示; プレビューを開く。

共有でのキャンセル/中断セマンティクス:

- ローダーにはエディタUIを復元し `Share cancelled` と報告する `onAbort` フックがある。
- このコードパスでは、基盤となる `gh gist create` コマンドにアボートシグナルは渡されない; キャンセルはUIレベルであり、コマンド返却後にチェックされる。

## フォーク

`/fork` は現在のセッションから新しいセッションを作成し、アクティブセッションのIDを切り替える。

### 前提条件と即時ガード

- エージェントがストリーミング中の場合、`/fork` は警告とともに拒否される。
- 操作前にUIのステータス/ローディングインジケーターがクリアされる。
...
4. 旧セッション名前空間から新しい名前空間にアーティファクトディレクトリをコピー（ベストエフォート; ENOENT以外のコピー失敗はログ記録されるが致命的ではない）。
...
- 以下でヘッダーを書き換え:
  ...
  - 新しいタイムスタンプ
  - `cwd` は変更なし
  - `parentSession` を前のセッションIDに設定
- 新しいファイルではヘッダー以外のすべてのエントリを変更なしで保持。
...
- このピッカーは現在のセッションディレクトリスコープ内のセッションのみをリスト表示。
...
- マッチしたセッションのcwdが現在のcwdと異なる場合、CLIが確認:
  - `Session found in different project ... Fork into current directory? [y/N]`
- yesの場合: `SessionManager.forkFrom(match.path, cwd, sessionDir)` が新しいローカルフォークファイルを作成。
- no/非TTYデフォルトの場合: コマンドがエラーで終了。
...
これは起動時のみの動作であり、インタラクティブな `/continue` スラッシュコマンドは存在しない。

## セッション切替がランタイム状態を実際にどのように変更するか

`AgentSession.switchSession(sessionPath)` は、再開のような操作で使用されるランタイム遷移を行う:

1. `reason: "resume"` と `targetSessionFile` で `session_before_switch` を発行（キャンセル可能）。
2. エージェントイベントサブスクリプションを切断し、進行中の作業を中断。
3. キューに入っているステアリング/フォローアップ/次ターンメッセージをクリア。
4. 現在のセッションマネージャーの書き込みをフラッシュ。
5. `sessionManager.setSessionFile(sessionPath)` を実行し `agent.sessionId` を更新。
...
SDKブリッジは拡張機能のセッションイベントをカスタムツールの `onSession` コールバックに橋渡し:
...
これらのコールバックは観察用であり、切替/フォークをキャンセルしない。

### このドキュメントに関連するその他のキャンセルサーフェス

- `/fork` はストリーミング中はブロックされる（ユーザーは現在のレスポンスを待つか中断する必要がある）。
- `/resume` セレクターはユーザーがセレクターを閉じることでキャンセル可能。
- クロスプロジェクトの `--resume <id>` はフォークプロンプトを拒否することでキャンセル可能。
- `/share` にはgistフローのUI中断パス（`Share cancelled`）がある; このコードパスでは `gh gist create` のプロセスキルセマンティクスは配線されていない。
...
- セッションファイルパスが存在しない。
- `/export` と `/share` は `Cannot export in-memory session to HTML` で失敗（コマンドエラーUIに伝播）。
- `/fork` は `SessionManager.fork()` が永続性を要求するため失敗。
- `/dump` はインメモリのエージェント状態をシリアライズするため引き続き動作。
- マネージャー作成が即座にインメモリを返すため、`--no-session` が設定されている場合、CLIのresume/continueセマンティクスはバイパスされる。

## 既知の実装上の注意点（現在のコード時点）

- `SelectorController.handleResumeSession()` は `session.switchSession(...)` のブーリアン結果をチェックしない; フックでキャンセルされた切替でも、UIの「Resumed session」再描画/ステータスパスが進行する可能性がある。
- `/share` のカスタム共有失敗はデフォルトのgistフォールバックに降格しない; エラーでコマンドを終了する。
- `/export` の引数トークン化は単純であり、スペースを含むクォートされたパスを保持しない。
