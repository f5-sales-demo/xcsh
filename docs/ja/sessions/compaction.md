...
コンパクションとブランチサマリーは、以前の作業コンテキストを失うことなく長期セッションを使いやすく保つための2つのメカニズムです。
...
コンパクションとブランチサマリーはファーストクラスのセッションエントリであり、単純なassistant/userメッセージではありません。
...
3. **自動閾値コンパクション**: コンテキストが閾値を超えた成功ターンの後。
...
### オーバーフローリトライ vs 閾値コンパクション

2つの自動パスは意図的に異なっています：

- **オーバーフローリトライコンパクション**
  - トリガー: 現在のモデルのアシスタントエラーがコンテキストオーバーフローとして検出された場合。
  - 失敗したアシスタントエラーメッセージはリトライ前にアクティブなエージェント状態から削除されます。
  - 自動コンパクションが `reason: "overflow"` と `willRetry: true` で実行されます。
  - 成功時、エージェントはコンパクション後に自動的に続行します（`agent.continue()`）。

- **閾値コンパクション**
  - トリガー: `contextTokens > contextWindow - compaction.reserveTokens`。
  - `reason: "threshold"` と `willRetry: false` で実行されます。
...
- 最新の `40_000` ツール出力トークンを保護します。
- 推定節約量が合計で少なくとも `20_000` 必要です。
- `skill` または `read` からのツール結果は決してプルーニングしません。

プルーニングされたツール結果は以下に置き換えられます：

- `[Output truncated - N tokens]`

プルーニングによってエントリが変更された場合、セッションストレージが書き換えられ、コンパクション判定の前にエージェントメッセージの状態がリフレッシュされます。

### 境界とカットポイントロジック

`prepareCompaction()` は最後のコンパクションエントリ以降のエントリのみを考慮します（存在する場合）。

1. 前回のコンパクションインデックスを見つけます。
2. `boundaryStart = prevCompactionIndex + 1` を計算します。
3. 利用可能な場合、測定された使用率を使って `keepRecentTokens` を調整します。
...
厳格なルール: `toolResult` でのカットは決して行いません。

カットポイントの直前にメッセージ以外のメタデータエントリ（`model_change`、`thinking_level_change`、ラベルなど）がある場合、メッセージまたはコンパクション境界に到達するまでカットインデックスを後方に移動して保持領域に引き込みます。

### スプリットターン処理

カットポイントがユーザーターンの開始位置にない場合、コンパクションはそれをスプリットターンとして扱います。

ターン開始の検出では、以下をユーザーターン境界として扱います：
...
- スプリットターンの2パス目: `compaction-turn-prefix.md`
- UIの短いサマリー: `compaction-short-summary.md`
...
コンパクションはアシスタントのツール呼び出しを使用して累積的なファイルアクティビティを追跡します：
...
サマリーテキストにはプロンプトテンプレートを介してファイルタグが追加されます：
...
3. サマリーが存在する場合、`branchWithSummary(...)` を使用してナビゲーションターゲットにアタッチします。

これは一般的に `branchSummary.enabled` が有効な場合の `/tree` フローによって駆動されます。
...
1. 第1パス: pi生成の `branch_summary` 詳細を含む、すべての要約対象エントリから累積的なファイル操作を収集します。
2. 第2パス: 最新→最古の順で、トークンバジェットに達するまでメッセージを追加します。
3. 最近のコンテキストの保持を優先します。
4. 連続性のために、バジェット境界付近の大きなサマリーエントリを含める場合があります。

ブランチ要約の入力中、コンパクションエントリはメッセージ（`compactionSummary`）として含まれます。

### サマリー生成と永続化

ブランチ要約：

1. 選択されたメッセージを変換してシリアライズします。
2. `<conversation>` でラップします。
3. カスタム指示が提供された場合はそれを使用し、そうでなければ `branch-summary.md` を使用します。
4. `SUMMARIZATION_SYSTEM_PROMPT` で要約モデルを呼び出します。
5. `branch-summary-preamble.md` を先頭に追加します。
6. ファイル操作タグを追加します。

結果はオプションの詳細（`readFiles`、`modifiedFiles`）を含む `BranchSummaryEntry` として保存されます。
...
デフォルトのブランチサマリー生成前にツリーナビゲーションで実行されます。
...
- 手動コンパクションは現在のエージェント操作を最初に中止します。
- `abortCompaction()` は手動および自動コンパクションのコントローラーの両方をキャンセルします。
- 自動コンパクションはUI/状態更新のための開始/終了セッションイベントを発行します。
- 自動コンパクションは複数のモデル候補を試行し、一時的な障害をリトライできます。
- オーバーフローエラーはコンパクションによって処理されるため、汎用リトライパスからは除外されます。
- 自動コンパクションが失敗した場合：
  - オーバーフローパスは `Context overflow recovery failed: ...` を発行します
  - 閾値パスは `Auto-compaction failed: ...` を発行します
- ブランチ要約はアボートシグナル（例：Escape）でキャンセルでき、キャンセル/中止されたナビゲーション結果を返します。
...
これらの値は `AgentSession` およびコンパクション/ブランチ要約モジュールによって実行時に消費されます。
