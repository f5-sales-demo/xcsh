...
ツールの動作、カーネル/ゲートウェイのライフサイクル、環境処理、実行セマンティクス、出力レンダリング、および運用上の障害モードについて取り上げます。
...
`python` ツールは、1つ以上の Python セルを Jupyter Kernel Gateway をバックエンドとするカーネルを通じて実行します（セルごとに `python -c` を直接スポーンするのではありません）。
...
- PID が生存しヘルスチェックに合格した場合、`gateway.json` を再利用します。
...
- プロセスごとの非共有ローカルゲートウェイモードはありません。

## カーネルのライフサイクル

各実行は、選択されたゲートウェイ上で `POST /api/kernels` を介して作成されたカーネルを使用します。
...
- `DELETE /api/kernels/:id` によるリモートカーネルの削除
- WebSocket のクローズ
- 共有ゲートウェイリリースフックの呼び出し (現在は no-op)
...
- `session` (デフォルト)
  ...
  - アイドルセッションは 5 分後にエビクションされます。
  - 最大 4 セッション。オーバーフロー時に最も古いものがエビクションされます。
  - ハートビートチェックにより停止したカーネルを検出します。
  - 自動再起動は 1 回まで許可。繰り返しクラッシュするとハード障害になります。
...
- それ以前のセルの状態はメモリに残ります。
- ツールは、どのセルが失敗したかを示す対象を絞ったエラーを返します。
- 後続のセルは実行されません。

`reset=true` は、その呼び出しの最初のセル実行にのみ適用されます。
...
- REST (`POST /interrupt`) およびコントロールチャネル `interrupt_request` を介してカーネル割り込みが試行されます。
- 結果に `cancelled=true` が含まれます。
- タイムアウトパスでは出力に `Command timed out after <n> seconds` と注記されます。
...
ツールの結果には切り詰めメタデータと、完全な出力を復元するための `artifact://<id>` が含まれる場合があります。
...
- インタラクティブレンダラー (`python-execution.ts`):
  - TUI でユーザーがトリガーした Python 実行に使用
  - 折りたたみプレビューのデフォルトは 20 行
  - 表示の安全性のため、非常に長い個別の行を 4000 文字にクランプ
  - キャンセル/エラー/切り詰め通知を表示
...
- **Python コードでの stdin/入力プロンプト**
  - `input()` はこのランタイムパスではインタラクティブにサポートされていません。データをプログラム的に渡してください。

- **リソース枯渇 (`EMFILE` / オープンファイルが多すぎる)**
  - セッションマネージャーが共有ゲートウェイのリカバリ (セッションのティアダウン + 共有ゲートウェイの再起動) をトリガーします。
