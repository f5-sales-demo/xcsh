...
Wenn aktiviert, extrahiert der Agent automatisch dauerhaftes Wissen aus vergangenen Sitzungen und fügt eine kompakte Zusammenfassung in jede neue Sitzung ein. Im Laufe der Zeit baut er einen projektbezogenen Wissensspeicher auf — technische Entscheidungen, wiederkehrende Arbeitsabläufe, Fallstricke — der ohne manuellen Aufwand weitergegeben wird.
...
Beim Sitzungsstart wird, falls eine Speicherzusammenfassung für das aktuelle Projekt existiert, diese als **Memory Guidance**-Block in den System-Prompt eingefügt. Der Agent wird angewiesen:
...
- Den Pfad des Speicher-Artefakts zu zitieren, wenn der Speicher den Plan ändert, und ihn mit aktuellen Repository-Belegen zu kombinieren, bevor er handelt.
- Den Repository-Zustand und die Benutzeranweisung zu bevorzugen, wenn sie mit dem Speicher in Konflikt stehen; widersprüchlichen Speicher als veraltet zu behandeln.
...
| URL | Inhalt |
|---|---|
| `memory://root` | Kompakte Zusammenfassung, die beim Start eingefügt wird |
...
| Unterbefehl | Wirkung |
|---|---|
| `view` | Zeigt die aktuelle Speicher-Injektions-Nutzlast an |
...
**Phase 1 — Extraktion pro Sitzung:** Für jede vergangene Sitzung, die sich seit der letzten Verarbeitung geändert hat, liest ein Modell die Sitzungshistorie und extrahiert dauerhaftes Signal: technische Entscheidungen, Einschränkungen, behobene Fehler, wiederkehrende Arbeitsabläufe. Sitzungen, die zu aktuell, zu alt oder derzeit aktiv sind, werden übersprungen. Jede Extraktion erzeugt einen rohen Speicherblock und eine kurze Zusammenfassung für diese Sitzung.

**Phase 2 — Konsolidierung:** Nach der Extraktion liest ein zweiter Modelldurchlauf alle sitzungsbezogenen Extraktionen und erzeugt drei auf die Festplatte geschriebene Ausgaben:

- `MEMORY.md` — ein kuratiertes Langzeitspeicher-Dokument
- `memory_summary.md` — der kompakte Text, der beim Sitzungsstart eingefügt wird
- `skills/` — wiederverwendbare prozedurale Playbooks, jedes in einem eigenen Unterverzeichnis
...
Alle Ausgaben werden vor dem Schreiben auf die Festplatte auf Geheimnisse überprüft.
...
| `read_path.md` | Speicherhinweise, die in Live-Sitzungen eingefügt werden | `{{memory_summary}}` |

### Modellauswahl

Der Speicher nutzt das Modell-Rollensystem.
...
| `memories.maxRolloutAgeDays` | `30` | Sitzungen, die älter sind, werden nicht verarbeitet |
| `memories.minRolloutIdleHours` | `12` | Sitzungen, die kürzer zurückliegen, werden übersprungen |
| `memories.maxRolloutsPerStartup` | `64` | Obergrenze der pro Start verarbeiteten Sitzungen |
| `memories.summaryInjectionTokenLimit` | `5000` | Maximale Token der in den System-Prompt eingefügten Zusammenfassung |

Zusätzliche Feinabstimmungsparameter (Parallelität, Sperrdauer, Token-Budgets) sind in der Konfiguration für fortgeschrittene Nutzung verfügbar.
