...
Primäre Anleitung zur Erstellung von Laufzeit-Extensions in `packages/coding-agent`.
...
Für Entdeckungspfade und Dateisystem-Laderegeln siehe `docs/extension-loading.md`.
...
Extensions können alle folgenden Funktionalitäten in einem Modul kombinieren:
...
4. Session/Agent/Tool-Lebenszyklusereignisse werden an Handler emittiert.
...
- Der Aufruf von Aktionsmethoden wie `pi.sendMessage()` während des Extension-Ladens löst einen `ExtensionRuntimeNotInitializedError` aus
...
Im interaktiven Modus werden `input`-Handler vor der eingebauten Prüfung auf automatische Titelgenerierung der ersten Nachricht ausgeführt. Extensions, die `await pi.setSessionName(...)` aus `input` aufrufen, können den persistierten Sessionsnamen setzen und verhindern, dass der standardmäßig automatisch generierte Titel für diese Session ausgeführt wird.
...
- `deliverAs: "followUp"` — wird in die Warteschlange eingereiht und nach dem aktuellen Lauf ausgeführt
...
`pi.sendUserMessage(content, { deliverAs })` durchläuft immer den Prompt-Flow; während des Streamings wird es als Steer/Follow-up in die Warteschlange eingereiht.

## 2) Handler-Kontext (`ExtensionContext`)

Handler und die `execute`-Funktion von Tools erhalten `ctx` mit:
...
Verwenden Sie den Befehlskontext für Session-Steuerungsabläufe; diese Methoden sind absichtlich von allgemeinen Event-Handlern getrennt.
...
Aktuelle Laufzeitnotiz: `ExtensionRunner.emitResourcesDiscover(...)` ist implementiert, aber es gibt keine `AgentSession`-Aufrufstellen, die es im aktuellen Codebase aufrufen.

## Details zur Tool-Erstellung
...
`tool_call`/`tool_result` fangen alle Tools ab, sobald die Registry in `sdk.ts` umschlossen ist, einschließlich eingebauter und Extension-/benutzerdefinierter Tools.
...
Außerdem zu beachten: `setWidget` leitet derzeit über `setHookWidget(...)` an den Statuszeilen-Text weiter.

### RPC-Modus (`rpc-mode.ts`)

`ctx.ui` wird durch RPC-`extension_ui_request`-Events unterstützt:

- Dialog-Methoden (`select`, `confirm`, `input`, `editor`) werden per Roundtrip an Client-Antworten gesendet
...
- Tool-Aufklapp-Steuerungen sind inaktiv
...
3. Tool-Ergebnis-`details` strukturiert halten, wenn der Zustand aus der Tool-Ergebnishistorie sichtbar/rekonstruierbar sein soll.

Beispiel für ein Rekonstruktionsmuster:
...
Wird beim interaktiven Rendering verwendet, wenn benutzerdefinierte Nachrichten angezeigt werden.
...
- Laufzeitaktionen sind während des Extension-Ladens nicht verfügbar.
- `tool_call`-Fehler blockieren die Ausführung (Fail-Closed).
- Befehlsnamenskonflikte mit eingebauten Befehlen werden mit Diagnosemeldungen übersprungen.
...
- **Custom-Tools** (`src/extensibility/custom-tools/*`): Tool-fokussierte Module; wenn sie zusammen mit Extensions geladen werden, werden sie adaptiert und durchlaufen weiterhin Extension-Interception-Wrapper.

Wenn Sie ein Paket benötigen, das Richtlinien, Tools, Befehls-UX und Rendering gemeinsam verwaltet, verwenden Sie Extensions.
