...
`resolve` ist ein verstecktes Tool, das eine ausstehende Vorschau-Aktion abschließt.

- `action: "apply"` führt `apply(reason)` auf der ausstehenden Aktion aus und persistiert die Änderungen.
- `action: "discard"` ruft `reject(reason)` auf, sofern vorhanden; andernfalls wird die Aktion mit einer Standard-Nachricht "Discarded" verworfen.

Falls keine ausstehende Aktion existiert, schlägt `resolve` fehl mit:
...
`resolve` verarbeitet immer zuerst die **oberste** ausstehende Aktion (`pop()`), sodass mehrere vorschauerzeugende Tools in umgekehrter Reihenfolge ihrer Registrierung aufgelöst werden.

## Eingebautes Producer-Beispiel (`ast_edit`)
...
- `apply(reason: string): Promise<AgentToolResult<unknown>>` (erforderlich) — wird beim Anwenden aufgerufen; `reason` ist die Zeichenkette, die an `resolve` übergeben wird
- `reject?(reason: string): Promise<AgentToolResult<unknown> | undefined>` (optional) — wird beim Verwerfen aufgerufen; der Rückgabewert ersetzt die Standard-Nachricht "Discarded", sofern vorhanden
- `details?: unknown` (optional)
- `sourceToolName?: string` (optional, Standard ist `"custom_tool"`)
...
Falls die Laufzeitumgebung keinen Pending-Action-Store besitzt, wirft `pushPendingAction` folgenden Fehler:

- `Pending action store unavailable for custom tools in this runtime.`

## Tool-Auswahl-Verhalten

Wenn `PendingActionStore.hasPending` wahr ist, bevorzugt die Agent-Laufzeitumgebung bei der Tool-Auswahl `resolve`, sodass ausstehende Vorschauen explizit abgeschlossen werden, bevor der normale Tool-Ablauf fortgesetzt wird.

## Hinweise für Entwickler

- Verwenden Sie ausstehende Aktionen nur für destruktive oder stark wirkende Operationen, die eine explizite Anwenden/Verwerfen-Unterstützung erfordern.
- Halten Sie `label` prägnant und spezifisch; es wird in der Ausgabe des Resolve-Renderers angezeigt.
- Stellen Sie sicher, dass `apply(reason)` deterministisch und ausreichend idempotent für eine einmalige Ausführung ist; `reason` ist informativ und sollte das Verhalten nicht verändern.
