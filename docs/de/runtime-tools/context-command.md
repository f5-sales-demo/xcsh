...
description: >-
  Verbinden Sie xcsh mit F5 Distributed Cloud Mandanten -- erstellen, wechseln
  und verwalten Sie Authentifizierungskontexte.
sidebar:
  order: 1
...
xcsh verbindet sich mit F5 Distributed Cloud über **Kontexte** -- benannte Anmeldedatensätze, die eine Mandanten-URL, ein API-Token und einen Namespace binden. Wenn Sie `kubectl config use-context` oder `kubectx` verwendet haben, ist der Arbeitsablauf identisch: Erstellen Sie einen Kontext, wechseln Sie zwischen ihnen per Name, und verwenden Sie `-`, um zurückzuwechseln.

## Erste Schritte

### 1. Erstellen Sie Ihren ersten Kontext

Sie benötigen drei Dinge aus Ihrer F5 XC Konsole: die Mandanten-URL, ein API-Token und optional einen Namespace.
...
Nach der Aktivierung injiziert xcsh die Mandanten-Anmeldedaten in Ihre Sitzung. Der Agent kann nun F5 XC API-Aufrufe durchführen, und die Statuszeile zeigt den aktiven Kontext an.
...
Wechseln Sie zum vorherigen Kontext zurück (im `cd -`-Stil):
...
Tab-Vervollständigung bietet Namespace-Namen vom aktiven Mandanten an.

## Umgebungsvariablen in Kontexten

Kontexte können zusätzliche Umgebungsvariablen tragen, die bei der Aktivierung in Ihre Sitzung injiziert werden. Nützlich für mandantenspezifische Konfiguration, die nicht Teil der Anmeldedaten ist.
...
1. **Kontextnamen** -- mit Mandanten-URL-Hinweisen, damit Sie Mandanten unterscheiden können
2. **`-`** -- erscheint, wenn Sie zuvor gewechselt haben, und zeigt, zu welchem Kontext Sie wechseln würden
...
Die vollständige reservierte Menge: `list`, `show`, `status`, `create`, `delete`, `rename`, `namespace`, `env`, `set`, `unset`, `add`, `remove`, `clear`, `activate`, `validate`, `export`, `import`, `wizard`, `help`. Der Vergleich erfolgt ohne Berücksichtigung der Groß-/Kleinschreibung.
...
- **Sitzungsbezogen**: Der vorherige Kontext wird beim Neustart von xcsh zurückgesetzt. Er wird nicht auf der Festplatte gespeichert.
...
Die `/context`-Benutzererfahrung folgt:
