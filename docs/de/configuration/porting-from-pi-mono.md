...
- Entscheiden Sie, welche Features im Umfang enthalten sind und welche bewusst übersprungen werden.
...
- Beispiele (passen Sie diese an die tatsächlichen Pakete an, die Sie portieren):
...
- Prozess-Spawning: `child_process.spawn` → Bun Shell `$` für einfache Befehle, `Bun.spawn`/`Bun.spawnSync` für Streaming oder lang laufende Aufgaben
- Datei-I/O: `fs.readFileSync` → `Bun.file().text()` / `Bun.write()`
- HTTP-Clients: `node-fetch`, `axios` → natives `fetch`
- Crypto-Hashing: `node:crypto` → Web Crypto oder `Bun.hash`
...
- Wenn Upstream Assets in einen dist-Ordner kopiert, ersetzen Sie dies durch Bun-kompatible Einbettungen.
- Prompts sind statische `.md`-Dateien; verwenden Sie Bun-Textimporte (`with { type: "text" }`) und Handlebars anstelle von Inline-Prompt-Strings.
...
- Wenn Upstream zur Laufzeit eine gebündelte Fallback-Datei liest, ersetzen Sie Dateisystem-Lesezugriffe durch einen Bun-Texteinbettungs-Import.
...
Behandeln Sie `package.json` als Vertrag. Führen Sie bewusst zusammen.
...
- Stufen Sie Abhängigkeiten nicht herunter, um Typ-Fehler zu beheben; aktualisieren Sie stattdessen.
...
- Erstellen Sie niemals Prompts im Code; Prompts sind statische `.md`-Dateien, die mit Handlebars gerendert werden.
...
- **Keine `private`/`protected`/`public`-Schlüsselwörter bei Klassenfeldern oder -methoden.** Verwenden Sie ES `#`-private Felder für Kapselung; lassen Sie zugängliche Member ohne Schlüsselwort (bare). Die einzige Ausnahme sind Konstruktor-Parameter-Properties (`constructor(private readonly x: T)`), bei denen das Schlüsselwort von TypeScript benötigt wird. Wenn Sie Upstream-Code portieren, der `private foo` oder `protected bar` verwendet, konvertieren Sie zu `#foo` (privat) oder bare `bar` (zugänglich).
...
Entfernen Sie Upstream-Kompatibilitäts-Shims, sofern nicht anders gewünscht.

- Löschen Sie alte APIs, die ersetzt wurden.
- Aktualisieren Sie alle Aufrufstellen direkt auf die neue API.
- Behalten Sie keine `*_v2` oder parallelen Versionen.
...
## 11) Verbesserte Features schützen (Regressions-Falle)

Wenn Sie das Verhalten lokal bereits verbessert haben, behandeln Sie diese als **nicht verhandelbar**. Schreiben Sie vor der Portierung
die Verbesserungen auf und fügen Sie explizite Prüfungen hinzu, damit sie beim Merge nicht verloren gehen.
...
- **Alte → neue APIs zuordnen**: Wenn Upstream Konzepte umbenannt hat (Hooks → Extensions, Custom Tools → Tools, etc.),
  stellen Sie sicher, dass jeder alte Einstiegspunkt weiterhin durchgeleitet wird. Ein vergessenes Flag oder Export bedeutet verlorene Funktionalität.
- **Exports überprüfen**: Prüfen Sie `package.json` `exports`, öffentliche Typen und Barrel-Dateien. Upstream-Portierungen vergessen
  oft, lokale Ergänzungen erneut zu exportieren.
...
- **Standardwerte und Config-Merge-Reihenfolge prüfen**: Verbesserungen leben oft in Standardwerten. Bestätigen Sie, dass neue Standardwerte
  nicht zurückgesetzt wurden (z.B. neue Konfigurationspriorität, deaktivierte Features, Tool-Listen).
- **Env/Shell-Verhalten auditieren**: Wenn Sie Ausführung oder Sandboxing behoben haben, überprüfen Sie, dass der neue Pfad weiterhin Ihre
  bereinigte Umgebung verwendet und keine Alias-/Funktions-Overrides wieder einführt.
- **Gezielte Beispiele erneut ausführen**: Halten Sie einen minimalen Satz von "bekannt guten" Beispielen bereit und führen Sie sie nach der Portierung
  aus (CLI-Flags, Extension-Registrierung, Tool-Ausführung).

## 12) Umgearbeiteten Code erkennen und behandeln

Bevor Sie eine Datei portieren, prüfen Sie, ob Upstream sie signifikant refaktorisiert hat:
...
Wenn der Diff zeigt, dass die Datei **umgearbeitet** wurde (nicht nur gepatcht):

- Neue Abstraktionen, umbenannte Konzepte, zusammengeführte Module, geänderter Datenfluss

Dann müssen Sie **die neue Implementierung gründlich lesen**, bevor Sie portieren. Blindes Mergen von umgearbeitetem Code verliert Funktionalität, weil:

Hinweis: Der interaktive Modus wurde kürzlich in Controllers/Utils/Types aufgeteilt. Beim Zurückportieren verwandter Änderungen portieren Sie Updates in die einzelnen Dateien, die wir erstellt haben, und stellen Sie sicher, dass die `interactive-mode.ts`-Verdrahtung synchron bleibt.
...
2. **API-Optionen gehen verloren** - Wenn Systeme zusammengeführt werden (z.B. `hooks` + `customTools` → `extensions`), werden alte Optionen möglicherweise nicht in die neue Implementierung durchgeleitet.

3. **Code-Pfade werden veraltet** - Ein umbenanntes Konzept (z.B. `hookMessage` → `custom`) erfordert Updates in jeder Switch-Anweisung, Type-Guard und jedem Handler – nicht nur in der Definition.
...
Wenn Upstream ein Modul umgearbeitet hat:

1. **Die alte Implementierung lesen** - Verstehen Sie, was sie tat, welche Optionen sie akzeptierte, was sie exponierte.

2. **Die neue Implementierung lesen** - Verstehen Sie die neuen Abstraktionen und wie sie auf altes Verhalten abbilden.
...
4. **Nach Überbleibseln suchen** - Suchen Sie nach alten Namen/Konzepten, die möglicherweise in Switch-Anweisungen, Handlern, UI-Komponenten übersehen wurden.
...
Verwenden Sie diese als letzten Durchgang, bevor Sie abschließen:
...
- [ ] Prompts sind `.md`-Textimporte (keine Inline-Prompt-Strings)
- [ ] Kein `console.*` im coding-agent (verwenden Sie `logger`)
- [ ] Assets laden über Bun-Einbettungsmuster (keine Kopier-Skripte)
- [ ] Tests oder Prüfungen laufen (oder sind explizit als blockiert vermerkt)
- [ ] Keine Funktionalitäts-Regressionen (siehe Abschnitte 11-12)

## 14) Commit-Nachricht-Format

Beim Committen eines Backports folgen Sie dem Repository-Format `<type>(scope): <Beschreibung in Vergangenheitsform>` und behalten Sie den Commit-Bereich im Titel.
...
- Upstream-Issue-/PR-Nummern und Contributor-Attribution für externe Beiträge einschließen
...
| `InteractiveModeOptions` Options-Objekt     | Positionale Konstruktor-Argumente (Options-Typ wird weiterhin exportiert) | Konstruktor-Signatur beibehalten; Typ aktualisieren, wenn Upstream Felder hinzufügt |
...
| Upstream                                 | Unser Fork                               | Hinweise                                  |
| ---------------------------------------- | ---------------------------------------- | ----------------------------------------- |
| `sessionManager.appendSessionInfo(name)` | `sessionManager.setSessionName(name)`    | Wir verwenden durchgehend `sessionName`   |
| `sessionManager.getSessionName()`        | `sessionManager.getSessionName()`        | Gleich (wir haben vereinheitlicht, um Upstreams RPC zu entsprechen) |
...
| Upstream                            | Unser Fork                                                        | Hinweise                                                  |
| ----------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------- |
| `createTool(cwd: string, options?)` | `createTools(session: ToolSession)` über `BUILTIN_TOOLS`-Registry | Tool-Factories akzeptieren `ToolSession` und können `null` zurückgeben |
| Per-Tool `*Operations`-Interfaces   | Per-Tool-Interfaces bleiben bestehen (`FindOperations`, `GrepOperations`) | Verwendet für SSH/Remote-Overrides                        |
...
| Upstream                        | Unser Fork                                  | Hinweise                                     |
| ------------------------------- | ------------------------------------------- | -------------------------------------------- |
| `proper-lockfile` + `auth.json` | `agent.db` (bun:sqlite)                     | Anmeldedaten werden ausschließlich in `agent.db` gespeichert |
| Einzelne Anmeldedaten pro Provider | Multi-Anmeldedaten mit Round-Robin-Auswahl | Session-Affinität und Backoff-Logik beibehalten |
...
| `pkg.pi` Manifest-Feld       | `pkg.xcsh ?? pkg.pi` (unseren Namespace bevorzugen) |
...
- `clipboard-image.ts` — Clipboard befindet sich im `@f5xc-salesdemos/pi-natives` N-API-Modul
...
- Bash-Abfangung (`checkBashInterception`)
