...
description: >-
  MCP-Serverkonfiguration, -Validierung und -Verwaltung für die
...
Diese Anleitung erklärt, wie Sie MCP-Server für den OMP-Coding-Agent hinzufügen, bearbeiten und validieren.
...
OMP kann MCP-Server aus mehreren Tools erkennen (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json` und mehr), aber für die OMP-native Konfiguration sollten Sie üblicherweise eine dieser Dateien verwenden:
...
Verwenden Sie `.xcsh/mcp.json`, wenn OMP die Konfiguration verwalten soll. Verwenden Sie die Stamm-Dateien `mcp.json` / `.mcp.json` nur, wenn Sie eine portable Fallback-Datei wünschen, die auch andere MCP-Clients lesen können.
...
OMP schreibt dies jetzt automatisch, wenn `/mcp add`, `/mcp enable`, `/mcp disable`, `/mcp reauth` oder andere konfigurationsschreibende Abläufe eine von OMP verwaltete MCP-Datei erstellen oder aktualisieren.
...
- `disabledServers` — Benutzer-Sperrliste, um erkannte Server namentlich zu deaktivieren
...
Dies folgt dem offiziellen Filesystem-MCP-Serverpaket (`@modelcontextprotocol/server-filesystem`).
...
Dies entspricht dem gehosteten GitHub-MCP-Server-Endpunkt von GitHub.
...
Slack ist das derzeit klarste Beispiel. Slacks MCP-Server wird unter `https://mcp.slack.com/mcp` gehostet, verwendet Streamable HTTP und erfordert vertrauliches OAuth mit den Client-Anmeldedaten Ihrer Slack-App.
...
Dies entspricht dem offiziellen lokalen Docker-Image von GitHub `ghcr.io/github/github-mcp-server`.
...
Dies ist der Teil, der üblicherweise für Verwirrung sorgt.

### In `.xcsh/mcp.json` und `~/.xcsh/mcp.json`

Bevor OMP einen Server startet oder eine HTTP-Anfrage stellt, löst es `env`- und `headers`-Werte wie folgt auf:
...
### In Stamm-Dateien `mcp.json` und `.mcp.json`

Der eigenständige Fallback-Loader expandiert auch `${VAR}` und `${VAR:-default}` innerhalb von Strings während der Erkennung.
...
Wenn Sie das vorhersehbarste OMP-Verhalten wünschen, bevorzugen Sie `.xcsh/mcp.json` und verwenden Sie explizite env/header-Werte.

## `disabledServers`

`disabledServers` ist hauptsächlich in der Benutzerkonfigurationsdatei (`~/.xcsh/mcp.json`) nützlich, wenn ein Server aus einer anderen Quelle erkannt wird und Sie möchten, dass OMP ihn ignoriert, ohne die Konfiguration des anderen Tools zu bearbeiten.
...
- Sie eine Serverdefinition von einem anderen MCP-Client einfügen möchten
- Sie schemabasierte Validierung in Ihrem Editor wünschen

Nach der Bearbeitung verwenden Sie:

- `/mcp reload`, um Server in der aktuellen Sitzung neu zu erkennen und neu zu verbinden
...
- ein Server kann nicht gleichzeitig `command` und `url` gesetzt haben
- unbekannte `type`-Werte werden abgelehnt

Praktische Auswirkungen:

- Das Weglassen von `type` bedeutet `stdio`
- Wenn Sie eine Remote-Server-Konfiguration einfügen und `"type": "http"` vergessen, behandelt OMP es als `stdio` und beschwert sich, dass `command` fehlt
- `sse` bleibt aus Kompatibilitätsgründen gültig, aber neue gehostete Server sollten üblicherweise als `http` konfiguriert werden

## Erkennung und Priorität

OMP führt keine doppelten Serverdefinitionen aus verschiedenen Dateien zusammen. Erkennungs-Provider werden priorisiert, und die Definition mit höherer Priorität gewinnt.

In der Praxis:

- bevorzugen Sie `.xcsh/mcp.json` oder `~/.xcsh/mcp.json`, wenn Sie eine OMP-spezifische Überschreibung wünschen
- halten Sie Servernamen nach Möglichkeit über alle Tools hinweg eindeutig
...
Das JSON ist gültig, aber der Server kann trotzdem nicht erreichbar sein. Verwenden Sie `/mcp test <name>` und prüfen Sie, ob:
...
- Filesystem-Serverpaket: <https://www.npmjs.com/package/@modelcontextprotocol/server-filesystem>
