...
| --- | --- | --- | --- |
| `Native addon missing exports ... Missing: <name>` | Veraltete `.node`-Datei, falscher Rust-Exportname oder falsche Binärdatei | Mit `PI_DEV=1` den geladenen Pfad anzeigen; Exportliste prüfen | Neu bauen; Rust-`#[napi]`-Export/Alias an den JS-Schlüssel angleichen; alte Cache-/Versionsdateien löschen |
...
| Kompilierte Binärdatei schlägt nach Upgrade fehl | Alter Cache (`~/.xcsh/natives/<alte-oder-nicht-übereinstimmende-version>`) oder Manifest-Diskrepanz | Versionsverzeichnis und Loader-Fehlerliste prüfen | Cache der Paketversion löschen und erneut ausführen; eingebettetes Manifest neu erzeugen |
