...
Dieses Dokument beschreibt, wie der Coding-Agent derzeit Modelle lädt, Überschreibungen anwendet, Anmeldeinformationen auflöst und Modelle zur Laufzeit auswählt.

## Was das Modellverhalten steuert

Primäre Implementierungsdateien:

- `src/config/model-registry.ts` — lädt eingebaute + benutzerdefinierte Modelle, Anbieter-Überschreibungen, Laufzeiterkennung, Auth-Integration
...
- Wenn `models.yml` fehlt und `models.json` am gleichen Speicherort existiert, wird sie zu `models.yml` migriert.
- Explizite `.json`- / `.jsonc`-Konfigurationspfade werden weiterhin unterstützt, wenn sie programmgesteuert an `ModelRegistry` übergeben werden.
...
`provider-id` ist der kanonische Anbieterschlüssel, der übergreifend bei Auswahl und Auth-Suche verwendet wird.
...
Aktuelle Heuristiken sind bewusst eng gefasst:

- Eingebettete Upstream-Präfixe können entfernt werden, wenn vorhanden, zum Beispiel `anthropic/...` oder `openai/...`
- Punkt- und Bindestrich-Versionsvarianten werden nur normalisiert, wenn sie auf eine existierende offizielle ID abbilden, zum Beispiel `4.6 -> 4-6`
...
1. Verfügbarkeit und Authentifizierung
2. `config.yml` `modelProviderOrder`
3. Bestehende Registry-/Anbieterreihenfolge, wenn `modelProviderOrder` nicht gesetzt ist
...
Die Laufzeit-Discovery ruft `GET /api/tags` bei Ollama auf und synthetisiert Modelleinträge mit lokalen Standardwerten.
...
Die Laufzeit-Discovery ruft `GET models` bei llama.cpp auf und synthetisiert Modelleinträge mit lokalen Standardwerten.
...
Sie können die Discovery selbst konfigurieren:
...
### Erweiterungs-Anbieterregistrierung
...
Beim Anfordern eines Schlüssels für einen Anbieter gilt folgende effektive Reihenfolge:

1. Laufzeit-Überschreibung (CLI `--api-key`)
2. Gespeicherte API-Schlüssel-Anmeldeinformation in `agent.db`
3. Gespeicherte OAuth-Anmeldeinformation in `agent.db` (mit Aktualisierung)
4. Umgebungsvariablen-Zuordnung (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.)
5. ModelRegistry-Fallback-Resolver (Anbieter-`apiKey` aus `models.yml`, Umgebungsvariablenname-oder-Literal-Semantik)
...
Wenn `authHeader: true` und der Anbieter-`apiKey` gesetzt ist, erhalten Modelle:

- `Authorization: Bearer <aufgelöster-schlüssel>` Header injiziert.

Schlüssellose Anbieter:

- Anbieter, die mit `auth: none` markiert sind, werden als verfügbar ohne Anmeldeinformationen behandelt.
- `getApiKey*` gibt `kNoAuth` für diese zurück.
...
- `getAvailable()` filtert auf Modelle, die schlüssellos sind oder auflösbare Authentifizierung haben.

Ein Modell kann also in der Registry existieren, aber erst auswählbar sein, wenn die Authentifizierung verfügbar ist.

## Laufzeit-Modellauflösung

### CLI und Musterparsung
...
4. Bekannte Anbieter-Standards (z.B. OpenAI/Anthropic/etc.) unter verfügbaren Modellen
...
Wenn eine Rolle auf eine andere Rolle verweist, erbt das Zielmodell trotzdem normal und jedes explizite Suffix der verweisenden Rolle gewinnt für diese rollenspezifische Verwendung.
...
Kontextpromotion ist ein Überlauf-Wiederherstellungsmechanismus für kleine Kontext-Varianten (zum Beispiel `*-spark`), der automatisch zu einem Geschwistermodell mit größerem Kontext hochstuft, wenn die API eine Anfrage mit einem Kontextlängenfehler ablehnt.

### Auslöser und Reihenfolge

Wenn ein Turn mit einem Kontextüberlauf-Fehler fehlschlägt (z.B. `context_length_exceeded`), versucht `AgentSession` eine Promotion **bevor** auf Kompaktierung zurückgegriffen wird:

1. Wenn `contextPromotion.enabled` wahr ist, ein Promotionsziel auflösen (siehe unten).
2. Wenn ein Ziel gefunden wird, darauf wechseln und die Anfrage erneut versuchen — keine Kompaktierung erforderlich.
3. Wenn kein Ziel verfügbar ist, zur automatischen Kompaktierung auf dem aktuellen Modell übergehen.
...
2. Kleinstes Modell mit größerem Kontext auf demselben Anbieter + API

Kandidaten werden ignoriert, es sei denn, die Anmeldeinformationen lassen sich auflösen (`ModelRegistry.getApiKey(...)`).

### OpenAI Codex WebSocket-Übergabe

Beim Wechsel von/zu `openai-codex-responses` wird der Sitzungsanbieterstatus-Schlüssel `openai-codex-responses` vor dem Modellwechsel geschlossen. Dies verwirft den WebSocket-Transportstatus, sodass der nächste Turn sauber auf dem promotierten Modell startet.

### Persistenzverhalten

Promotion verwendet temporäres Umschalten (`setModelTemporary`):

- Wird als temporärer `model_change` in der Sitzungshistorie aufgezeichnet
...
Beispiel (`models.yml`) für Spark -> Nicht-Spark auf demselben Anbieter:
...
Der eingebaute Modellgenerator weist dies auch automatisch für `*-spark`-Modelle zu, wenn ein Basismodell desselben Anbieters existiert.
...
Diese werden von der OpenAI-Completions-Transportlogik konsumiert und mit URL-basierter automatischer Erkennung kombiniert.
...
## LiteLLM-Proxy-Autokonfiguration
...
Bei jedem Start führt `startupHealthCheck()` in der Modell-Registry folgende Prüfungen durch:

| Bedingung | Aktion |
|-----------|--------|
| `models.yml` fehlt | Automatische Generierung aus Umgebungsvariablen |
| `models.yml` beschädigt oder nicht parsebar | Sicherung als `.bak`, Neugenerierung |
| `baseUrl` stimmt nicht mit `LITELLM_BASE_URL` überein | Sicherung als `.bak`, Neugenerierung mit neuer URL |
| `configVersion` fehlt oder veraltet | Sicherung als `.bak`, Neugenerierung mit aktueller Version |
| Konfiguration ist gesund | Keine Aktion |

Alle Reparaturen erstellen `.bak`-Sicherungen vor dem Überschreiben. Alle Operationen sind idempotent.
...
Wenn eine der beiden Variablen nicht gesetzt ist, wird die Autokonfiguration stillschweigend übersprungen.

### Konfigurationsversionierung

Generierte Konfigurationen enthalten ein `configVersion`-Feld. Wenn sich das generierte Format in zukünftigen Versionen ändert, erkennt xcsh veraltete Konfigurationen und aktualisiert sie automatisch (mit Sicherung).

## Hinweis zu Legacy-Konsumenten

Die meiste Modellkonfiguration fließt jetzt über `models.yml` via `ModelRegistry`.

Ein bemerkenswerter Legacy-Pfad bleibt: Die Anthropic-Auth-Auflösung für die Websuche liest weiterhin direkt `~/.xcsh/agent/models.json` in `src/web/search/auth.ts`.

Wenn Sie auf diesen spezifischen Pfad angewiesen sind, beachten Sie die JSON-Kompatibilität, bis dieses Modul migriert ist.

## Fehlerverhalten

Wenn `models.yml` die Schema- oder Validierungsprüfungen nicht besteht:

- Wenn `LITELLM_BASE_URL` und `LITELLM_API_KEY` gesetzt sind, versucht die Start-Integritätsprüfung eine automatische Reparatur (beschädigte Datei sichern, aus Umgebungsvariablen neu generieren). Wenn die Reparatur erfolgreich ist, lädt die Registry die korrigierte Konfiguration neu.
- Wenn eine automatische Reparatur nicht möglich ist (Umgebungsvariablen nicht gesetzt, Schreibfehler), arbeitet die Registry weiterhin mit eingebauten Modellen.
