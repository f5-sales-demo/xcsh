...
- **Rendering-Engine (`packages/tui`)**: differenzieller Terminal-Renderer, Eingabedispatch, Fokus, Overlays, Cursorplatzierung.
- **Integrationsschicht (`packages/coding-agent`)**: Bindet Erweiterungs-/benutzerdefinierte-Tool-Komponenten ein, verdrahtet Tastenkombinationen/Theme und stellt den Editorzustand wieder her.

## Laufzeitverhalten nach Modus

| Modus | `ctx.ui.custom(...)`-Verfügbarkeit | Hinweise |
| --- | --- | --- |
| Interaktives TUI | Unterstützt | Die Komponente wird im Editorbereich eingebunden, fokussiert und muss `done(result)` aufrufen, um aufzulösen. |
| Hintergrund/headless | Nicht interaktiv | UI-Kontext ist No-Op (`hasUI === false`). |
| RPC-Modus | Nicht unterstützt | `custom()` gibt `Promise<never>` zurück und bindet keine TUI-Komponenten ein. |

Wenn Ihre Erweiterung/Ihr Tool im nicht-interaktiven Modus ausgeführt werden kann, prüfen Sie mit `ctx.hasUI` / `pi.hasUI`.
...
## Rendering-Einschränkungen (Terminalsicherheit)

Ihre `render(width)`-Ausgabe muss terminalsicher sein:

1. **Überschreiten Sie niemals `width` in einer Zeile**. Der Renderer wirft einen Fehler, wenn eine Nicht-Bild-Zeile überläuft.
2. **Messen Sie die visuelle Breite**, nicht die Stringlänge: Verwenden Sie `visibleWidth()`.
3. **Kürzen/umbrechen Sie ANSI-bewussten Text** mit `truncateToWidth()` / `wrapTextWithAnsi()`.
4. **Bereinigen Sie Tabs/Inhalte** aus externen Quellen mit `replaceTabs()` (und übergeordneten Bereinigungsfunktionen in den Coding-Agent-Renderpfaden).
...
### Benutzer-konfigurierte App-Tastenkombinationen respektieren

Erweiterungs-UI-Factories erhalten einen `KeybindingsManager` (interaktiver Modus), damit Sie gemappte Aktionen statt fest codierter Tasten verwenden können:
...
Verwenden Sie dann `isKeyRelease()` / `isKeyRepeat()` bei Bedarf.

## Fokus, Overlays und Cursor

- `TUI.setFocus(component)` leitet die Eingabe an diese Komponente weiter.
- Overlay-APIs existieren in `TUI` (`showOverlay`, `OverlayHandle`), aber die Einbindung von Erweiterungs-`ctx.ui.custom` im interaktiven Modus ersetzt derzeit direkt den Editor-Komponentenbereich.
...
`done(...)` ist daher für die Fertigstellung zwingend erforderlich.

## 2) Hook-/benutzerdefinierte-Tool-UI-Kontext (Legacy-Typisierung)

`HookUIContext.custom` ist als `(tui, theme, done)` in den Hook-/benutzerdefinierten-Tool-Typen typisiert.
Die zugrunde liegende interaktive Implementierung ruft Factories mit `(tui, theme, keybindings, done)` auf. JS-Consumer können das zusätzliche Argument verwenden; die Typkompatibilität spiegelt noch die 3-Argument-Legacy-Signatur wider.
...
Benutzerdefinierte Tools und Erweiterungs-Tools können Komponenten zurückgeben von:
...
- `done(...)` sollte genau einmal aus Ihrem Komponentenablauf aufgerufen werden.
...
- `packages/tui/src/tui.ts` — `Component`, `Focusable`, Cursor-Marker, Fokus, Overlay, Eingabedispatch.
...
- `packages/coding-agent/src/modes/controllers/extension-ui-controller.ts` — Interaktive Einbindung/Ausbindung für Erweiterungs-/Hook-/benutzerdefinierte-Tool-UI.
