...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".xcsh\agent"
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("xcsh-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed xcsh via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'xcsh' to get started!"
}
...
function Install-Binary {
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'xcsh' to get started!"
    } else {
        Write-Host "Run 'xcsh' to get started!"
    }
}
