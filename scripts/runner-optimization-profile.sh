...
usage() {
  echo "usage: $0 <experiment> <cold|warm> <pair-id> <0|2|4|6|8|10-file-workers> <output-dir> <all|native|rust|typescript>" >&2
  exit 2
}
...
case "$experiment" in
image-control | image-candidate | d16-serial | d16-parallel-2 | d16-parallel-4 | d16-parallel-6 | d16-parallel-8 | d16-parallel-10 | d16-hardware | f32-hardware | d16-burst | f32-burst | dag-control | dag-candidate) ;;
*) usage ;;
esac
case "$cache_state" in cold | warm) ;; *) usage ;; esac
case "$file_workers" in 0 | 2 | 4 | 6 | 8 | 10) ;; *) usage ;; esac
case "$phase_set" in all | native | rust | typescript) ;; *) usage ;; esac
[[ "$pair_id" =~ ^[1-5](-slot-[1-4])?$ ]] || usage
case "$experiment:$file_workers" in
d16-parallel-2:2 | d16-parallel-4:4 | d16-parallel-6:6 | d16-parallel-8:8 | d16-parallel-10:10) ;;
*:0)
  [[ "$experiment" != d16-parallel-2 && "$experiment" != d16-parallel-4 && "$experiment" != d16-parallel-6 && "$experiment" != d16-parallel-8 && "$experiment" != d16-parallel-10 ]]
  ;;
*) usage ;;
esac
