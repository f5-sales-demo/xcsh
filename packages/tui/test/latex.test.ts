...
(1,13,34),\ (1,34,89)
	...
	it("treats legacy TeX font switches as safe no-ops", () => {
		const switches = ["rm", "bf", "it", "sf", "tt", "cal", "sl"];
		for (const fontSwitch of switches) {
			assert.strictEqual(renderLatex(`F_{\\${fontSwitch} intrinsic}(\\lambda)`), "F_intrinsic(λ)");
		}
	});
