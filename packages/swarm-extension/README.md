...
Multi-agent orchestration for xcsh. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full xcsh subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
xcsh-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup xcsh-swarm path/to/swarm.yaml \
...
Register the extension in your config (`~/.xcsh/config.json` or `.xcsh/config.json`):
...
Each agent is a full xcsh session. It can:
...
Any model configured in xcsh works. Set a swarm default and optionally override per agent:
...
src/swarm/
  ...
  executor.ts         Spawns agents via xcsh's runSubprocess
