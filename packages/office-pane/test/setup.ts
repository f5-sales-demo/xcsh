...
// DOM (e.g. a Fluent <textarea> role=textbox) leaks into the happy-dom document
// that all test FILES share in a single `bun test` process. cleanup() only
// tracks RTL `render()` trees; raw createRoot renders (see host-adapter.test.ts)
// must unmount themselves.
afterEach(() => {
	cleanup();
});
