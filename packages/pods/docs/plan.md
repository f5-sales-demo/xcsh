...
### Pods
```bash
pi pods setup dc1 "ssh root@192.0.2.37" --mount "mount -t nfs..."  # Setup pod (requires HF_TOKEN, PI_API_KEY env vars)
...
```bash
pi pods setup dc1 "ssh root@192.0.2.37" --mount "mount -t nfs..."
```
