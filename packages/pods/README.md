...
pi pods setup dc1 "ssh root@192.0.2.37" \
...
export OPENAI_BASE_URL='http://192.0.2.37:8001/v1'
...
pi pods setup gpt-pod "ssh root@192.0.2.37" --models-path /workspace --vllm gpt-oss
...
pi pods setup dc1 "ssh dana@example.com" \
  --mount "sudo mount -t nfs -o nconnect=16 nfs.fin-02.datacrunch.io:/your-pseudo /mnt/hf-models"
...
```bash
# With network volume
pi pods setup runpod "ssh dana@example.com" --models-path /runpod-volume

# Or use workspace (persists with pod but not shareable)
pi pods setup runpod "ssh dana@example.com" --models-path /workspace
```
