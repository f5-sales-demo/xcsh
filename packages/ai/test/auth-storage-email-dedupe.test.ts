...
describe("AuthStorage openai-codex email dedupe", () => {
	...
	it("reactivates only #3210-disabled Codex rows and deduplicates active identities idempotently", async () => {
		if (!store || !dbPath) throw new Error("test setup failed");
		...
		store = await AuthCredentialStore.open(dbPath);
		const activeCredentials = store.listAuthCredentials("openai-codex");
		expect(
			activeCredentials
				.flatMap(row => (row.credential.type === "oauth" && row.credential.email ? [row.credential.email] : []))
				.sort(),
		).toEqual(["restore@example.com", "shared@example.com"]);
		const restored = activeCredentials.find(
			row => row.credential.type === "oauth" && row.credential.email === "restore@example.com",
		);
		expect(restored?.credential.type).toBe("oauth");
		if (restored?.credential.type !== "oauth") throw new Error("expected restored oauth credential");
		expect(restored.credential.expires).toBe(0);
		expect(readDisabledCauses(dbPath, "openai-codex")).toEqual(["oauth refresh failed: invalid_grant"]);
		expect(countCredentialRows(dbPath, "openai-codex")).toBe(3);
