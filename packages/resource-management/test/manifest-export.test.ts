...
describe("toManifest", () => {
	...
	test("strips system_metadata and status", () => {
		const apiResponse = {
			metadata: { name: "test", namespace: "demo-app" },
	...
	test("omits empty objects from metadata", () => {
		...
		expect(result.metadata).toEqual({ name: "test", namespace: "demo-app" });
	});
	...
	test("handles empty spec", () => {
		const result = toManifest({ metadata: { name: "x", namespace: "demo-app" }, spec: {} }, "app_firewall");
		expect(result.spec).toEqual({});
	});
	...
	test("preserves full spec including server-added defaults", () => {
		const apiResponse = {
			metadata: { name: "fw", namespace: "demo-app" },
			spec: {
				user_field: "value",
				monitoring: {},
...
describe("toManifestList", () => {
	test("transforms list response items", () => {
		const listResponse = {
			items: [
				{ metadata: { name: "a", namespace: "demo-app" }, spec: { x: 1 } },
				{ metadata: { name: "b", namespace: "demo-app" }, spec: { y: 2 } },
			],
		};
		...
		expect(result[0].metadata).toEqual({ name: "a", namespace: "demo-app" });
		expect(result[1].metadata).toEqual({ name: "b", namespace: "demo-app" });
	});
	...
	test("strips system_metadata from each item", () => {
		const listResponse = {
			items: [
				{
					metadata: { name: "a", namespace: "demo-app" },
