...
const { session: newSession } = await createAgentSession({
	sessionManager: SessionManager.create(process.cwd(), SessionManager.getDefaultSessionDir(process.cwd())),
});
console.log("New session file:", newSession.sessionFile);

// Continue most recent session (or create new if none)
const { session: continued, modelFallbackMessage } = await createAgentSession({
	sessionManager: await SessionManager.continueRecent(process.cwd(), SessionManager.getDefaultSessionDir(process.cwd())),
});
if (modelFallbackMessage) console.log("Note:", modelFallbackMessage);
console.log("Continued session:", continued.sessionFile);

// List and open specific session
const sessions = await SessionManager.list(SessionManager.getDefaultSessionDir(process.cwd()));
