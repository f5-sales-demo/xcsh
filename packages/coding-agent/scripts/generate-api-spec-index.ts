...
const authoritativeSpecPath = path.join(specsDir, "openapi.json");
if (!fs.existsSync(authoritativeSpecPath)) {
	throw new Error(`Authoritative openapi.json not found at: ${authoritativeSpecPath}`);
}
const authoritativeSpec = JSON.parse(fs.readFileSync(authoritativeSpecPath, "utf-8")) as OpenApiDocumentLike & {
	info?: Record<string, unknown>;
};
const rawNetworkAllowlist = authoritativeSpec.info?.["x-f5xc-network-allowlist"];
...
if (rawNetworkAllowlist !== undefined) {
	if (!rawNetworkAllowlist || typeof rawNetworkAllowlist !== "object" || Array.isArray(rawNetworkAllowlist)) {
		throw new Error("info.x-f5xc-network-allowlist must be an object");
	}
	const extension = rawNetworkAllowlist as Record<string, unknown>;
	if (
		typeof extension.source_url !== "string" ||
		!/^https:\/\//.test(extension.source_url) ||
		typeof extension.sha256 !== "string" ||
		!/^[0-9a-f]{64}$/.test(extension.sha256) ||
		!extension.manifest ||
		typeof extension.manifest !== "object" ||
		Array.isArray(extension.manifest)
	) {
		throw new Error("info.x-f5xc-network-allowlist has invalid provenance or manifest data");
	}
	networkAllowlist = {
		sourceUrl: extension.source_url,
		sha256: extension.sha256,
		releaseVersion: rawIndex.version,
		manifest: extension.manifest as Record<string, unknown>,
	};
}
...
const catalogCategories = (
	(catalog?.categories ?? []) as Array<
		CatalogCategoryLike & { displayName: string; operations: Array<Record<string, unknown>> }
	>
).sort((left, right) => left.name.localeCompare(right.name));
const pathToCatalogCategories = buildPathToCatalogCategories(catalogCategories);
...
for (const entry of rawIndex.specifications) {
	...
	const resources = (entry["x-f5xc-primary-resources"] ?? []).map(r => {
		...
		for (const ap of r.api_paths ?? []) {
			for (const cat of pathToCatalogCategories.get(normalizeApiPath(ap)) ?? []) {
				catalogCats.add(cat);
			}
		}
		const sortedCatalogCats = [...catalogCats].sort((left, right) => left.localeCompare(right));
		if (sortedCatalogCats.length > 0) fields.push(`catalogCategories: ${JSON.stringify(sortedCatalogCats)}`);
		const canonicalCrudOperations = deriveCanonicalCrudOperations(specJson, r.api_paths ?? []);
		assertCanonicalCrudMapped(
			`${entry.domain}/${r.name}`,
		...
		if (canonicalCrudOperations.length > 0) {
			fields.push(`canonicalCrudOperations: ${JSON.stringify(canonicalCrudOperations)}`);
		}
		return `\t\t\t{ ${fields.join(", ")} },`;
	});
...
const output = [
	...
	"",
	`import type { ApiSpecDomainEnrichments, ApiSpecIndex, ApiSpecNetworkAllowlist, ApiSpecValidationResourceEntry } from "./api-spec-types";`,
	"",
	`export const API_SPEC_VERSION = ${JSON.stringify(rawIndex.version)};`,
	"",
	`export const API_NETWORK_ALLOWLIST: ApiSpecNetworkAllowlist | undefined = ${networkAllowlist ? serializeGeneratedValue(networkAllowlist) : "undefined"};`,
	"",
...
let catalogOutput: string | null = null;
if (catalog) {
...
if (catalog) assertCatalogIntegrity(catalogCategories, [authoritativeSpec]);
...
if (catalogOutput) {
	...
	console.log(
		`Generated ${path.relative(process.cwd(), catalogOutputPath)} (${catalogCategories.length} categories, ${catalogSize} MB)`,
	);
}
