...
const toolReferencePath = path.resolve(here, "../src/internal-urls/extension-tools.generated.ts");

const REQUIRED_PROMPT_HINTS = ["role", "grounding", "referenceLinks", "toolUse"] as const;
const REQUIRED_INTERACTION_MODES = [
	"educational",
	"presentation",
	"configuration",
	"screenshot",
...
interface GeneratedCapabilityManifest {
	version: string;
	contractVersion: string;
	multiPortDiscovery?: boolean;
	protocol: string;
	tools: Array<{
		name: string;
		summary: string;
	...
	features: Record<string, unknown> & {
		chat: {
			promptHints: {
				...
				toolUse: string;
				modes: Record<(typeof REQUIRED_INTERACTION_MODES)[number], string>;
			};
		};
	};
}
...
function asRecord(value: unknown, path: string): Record<string, unknown> {
	if (value === null || typeof value !== "object" || Array.isArray(value)) {
		throw new Error(`Invalid extension capability manifest: ${path} must be an object`);
	}
	return value as Record<string, unknown>;
}

function requireNonemptyString(record: Record<string, unknown>, key: string, path: string): void {
	const value = record[key];
	if (typeof value !== "string" || value.trim() === "") {
		throw new Error(`Invalid extension capability manifest: ${path}.${key} must be a non-empty string`);
	}
}

/** Validate the contract fields xcsh compiles into behavioral prompts. */
export function validateCapabilityManifest(value: unknown): GeneratedCapabilityManifest {
	const manifest = asRecord(value, "manifest");
	const features = asRecord(manifest.features, "features");
	const chat = asRecord(features.chat, "features.chat");
	const promptHints = asRecord(chat.promptHints, "features.chat.promptHints");
	for (const hint of REQUIRED_PROMPT_HINTS) requireNonemptyString(promptHints, hint, "features.chat.promptHints");
	const modes = asRecord(promptHints.modes, "features.chat.promptHints.modes");
	for (const mode of REQUIRED_INTERACTION_MODES) {
		requireNonemptyString(modes, mode, "features.chat.promptHints.modes");
	}
	if (!Array.isArray(manifest.tools)) {
		throw new Error("Invalid extension capability manifest: tools must be an array");
	}
	return value as GeneratedCapabilityManifest;
}
...
if (import.meta.main) {
	...
	const manifest = validateCapabilityManifest(JSON.parse(fs.readFileSync(vendoredPath, "utf8")));
	...
	const toolReferenceOutput = [
		"// Auto-generated by scripts/generate-extension-capabilities.ts - DO NOT EDIT",
		"// Source: src/browser/capabilities.json (the Chrome extension's published contract).",
		"",
		`export const EXTENSION_TOOL_REFERENCE = ${JSON.stringify(renderToolReference(manifest))};`,
		"",
	].join("\n");

	await Bun.write(generatedPath, output);
	await Bun.write(toolReferencePath, toolReferenceOutput);
	console.log(
		`Generated ${path.relative(process.cwd(), generatedPath)} and ${path.relative(process.cwd(), toolReferencePath)} (${toolNames.length} tools, contract ${manifest.contractVersion})`,
	);
}
