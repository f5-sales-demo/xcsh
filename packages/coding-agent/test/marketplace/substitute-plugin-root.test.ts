...
import { substitutePluginRoot } from "@f5-sales-demo/xcsh/discovery/substitute-plugin-root";
...
const XCSH_VAR = "$" + "{XCSH_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces XCSH_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${XCSH_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${XCSH_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
