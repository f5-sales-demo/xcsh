...
describe("SandboxPolicy (pure)", () => {
	...
	it("longest-prefix wins: deny a subdir inside a broadly-allowed root", () => {
		const p = policy(
			[
				["/home/user/.xcsh", true], // broad allow (e.g. a misconfigured widening)
				["/home/user/.xcsh/agent/memories", false], // deeper deny
			],
			[["/work/custA", true]],
		);
		expect(p.isAllowed("/home/user/.xcsh/plugins/x", "read")).toBe(true);
		expect(p.isAllowed("/home/user/.xcsh/agent/memories/other/MEMORY.md", "read")).toBe(false);
	});
