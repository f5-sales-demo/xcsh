...
import { clearCache } from "@f5-sales-demo/xcsh/capability/fs";
import type { Skill } from "@f5-sales-demo/xcsh/capability/skill";
import type { LoadContext, LoadResult } from "@f5-sales-demo/xcsh/capability/types";
import { scanSkillsFromDir } from "@f5-sales-demo/xcsh/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	test("finds skills in ancestor .xcsh/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".xcsh", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".xcsh", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".xcsh", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".xcsh", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".xcsh", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".xcsh", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".xcsh", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".xcsh", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".xcsh", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".xcsh", "skills"), "root-skill", "At repo root");
