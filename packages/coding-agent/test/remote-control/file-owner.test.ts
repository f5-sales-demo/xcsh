...
test.each([true, false])(
	"the shared SDK owner preserves file history, attached=%s",
	async attached => {
							...
							? [
									{
										...
										name: "edit",
										arguments: {
											edits: [{ path: "fixture.txt", old_text: "FILE-ONE", new_text: "FILE-TWO" }],
										},
									},
								]
		...
		try {
			...
			if (remote) {
				...
				await session.prompt("Write and update fixture.txt");
			...
			if (attached) {
				expect(
					events
						.filter(
							event => event.method === "item/completed" && (event.params.item as any).type === "fileChange",
						)
			...
			const stored = session.sessionManager
				.getBranch()
			...
			expect(
				session.messages
					...
					.every(message => !("toolExecution" in message)),
			).toBe(true);
			const reopened = await SessionManager.open(session.sessionManager.getSessionFile()!);
			try {
				expect(projectHistory(session.sessionId, reopened.getBranch())).toEqual(history);
			} finally {
