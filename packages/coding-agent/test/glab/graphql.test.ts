...
import { buildSearchQuery, executeGraphQL } from "../../src/tools/glab/graphql";
import type { GraphQLIssueNode } from "../../src/tools/glab/types";
...
describe("executeGraphQL", () => {
	function makePi(stdout: string, code = 0): GlabExecApi {
		return {
			cwd: "/tmp",
			exec: async () => ({ stdout, stderr: "", code, killed: false }) as GlabExecResult,
		};
	}
	...
	it("returns parsed nodes on success", async () => {
		...
		const result = await executeGraphQL(pi, "group/project", "test query", 10);
	...
	it("returns empty array when project not found", async () => {
		...
		const pi = makePi(JSON.stringify(mockResponse));
		const result = await executeGraphQL(pi, "group/project", "test", 10);
	...
	it("throws on GraphQL errors array", async () => {
		...
		const pi = makePi(JSON.stringify(mockResponse));
		await expect(executeGraphQL(pi, "group/project", "test", 10)).rejects.toThrow("Field 'issues' doesn't exist");
	});

	it("throws on glab CLI failure", async () => {
		const pi = makePi("", 1);
		await expect(executeGraphQL(pi, "group/project", "test", 10)).rejects.toThrow();
	});
});
