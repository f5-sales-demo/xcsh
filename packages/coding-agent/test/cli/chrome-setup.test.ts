...
describe("writeNativeHostManifest", () => {
	it("writes the macOS native-host manifest with allowed_origins for each extension id", () => {
		...
		expect(r.manifestPath).toBe(
			"/Users/user/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.f5xc.xcsh.chrome_host.json",
		);
	...
	it("uses the linux native-host dir", () => {
		...
		expect(r.manifestPath).toBe("/home/user/.config/google-chrome/NativeMessagingHosts/com.f5xc.xcsh.chrome_host.json");
	});
});
