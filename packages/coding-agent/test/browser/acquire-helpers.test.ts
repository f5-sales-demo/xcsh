...
describe("acquire helpers", () => {
	it("dedicatedProfileDir is under ~/.xcsh", () => {
		expect(dedicatedProfileDir("/home/user")).toBe("/home/user/.xcsh/chrome-profile");
	});
	...
	it("buildLaunchArgs adds --user-data-dir when a profileDir is given", () => {
		const args = buildLaunchArgs({ debugPort: 9333, profileDir: "/home/user/.xcsh/chrome-profile" });
		expect(args).toContain("--user-data-dir=/home/user/.xcsh/chrome-profile");
	...
	it("isProfileLockError detects singleton-lock failures", () => {
		...
		expect(
			isProfileLockError(
				"The browser is already running for /Users/user/Library/Application Support/Google/Chrome. Use a different `userDataDir` or stop the running browser first.",
			),
		).toBe(true);
	...
	it("defaultProfileDir resolves the macOS default profile", () => {
		expect(defaultProfileDir({ platform: "darwin", home: "/Users/user" })).toBe(
			"/Users/user/Library/Application Support/Google/Chrome",
		);
	});

	it("defaultProfileDir resolves the Linux default profile", () => {
		expect(defaultProfileDir({ platform: "linux", home: "/home/user" })).toBe("/home/user/.config/google-chrome");
	});

	it("defaultProfileDir resolves the Windows default profile from LOCALAPPDATA", () => {
		expect(defaultProfileDir({ platform: "win32", env: { LOCALAPPDATA: "C:\\Users\\user\\AppData\\Local" } })).toBe(
