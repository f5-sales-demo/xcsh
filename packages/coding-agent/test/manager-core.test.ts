...
describe("parseControlMsg", () => {
	test("valid provision/release/status", () => {
		expect(parseControlMsg({ type: "provision", sessionId: "tab-7", tenant: "example-corp" })).toEqual({
			type: "provision",
			sessionId: "tab-7",
			tenant: "example-corp",
		});
	...
	test("rejects junk / missing fields / bad tenant", () => {
		...
		expect(parseControlMsg({ type: "provision", tenant: "example-corp" })).toBeNull(); // no sessionId
...
describe("selectSpawnPort never probes a port it already handed out (#2463)", () => {
	...
	function regWith(entries: Array<[string, number]>): Registry {
		...
		for (const [sessionId, port] of entries) {
			reg.set(sessionId, { sessionId, tenant: "example-corp", port, pid: 1000 + port, lastSeen: 0 });
		}
		return reg;
	}
