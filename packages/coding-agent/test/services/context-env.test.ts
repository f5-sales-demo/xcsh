...
describe("createContextEnv", () => {
	...
	describe("resolvePath()", () => {
		it("substitutes explicit params first", () => {
			...
			const result = ctx.resolvePath("/api/{namespace}/resources", { namespace: "demo-app" });
	...
	describe("resolvePayloadVars()", () => {
		it("expands $F5XC_NAMESPACE in payload JSON", () => {
			...
			expect(ctx.resolvePayloadVars(payload)).toBe('{"metadata":{"namespace":"demo-app","name":"test"}}');
		});
