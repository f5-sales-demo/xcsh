...
describe("CI installs Zig without a deprecated JavaScript action", () => {
	it("executes and verifies the Zig installer inside every native matrix job", async () => {
		...
		expect(smokeJob).toBe("");
		expect(nativeJob).toContain("ubuntu-24.04");
		expect(nativeJob).toContain("macos-15-intel");
	...
	it("uses the checksum-pinned composite installer only in jobs that need Zig", async () => {
