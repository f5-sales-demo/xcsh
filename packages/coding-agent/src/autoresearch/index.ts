...
const EXPERIMENT_TOOLS = new Set(["init_experiment", "run_experiment", "log_experiment"]);
const addExperimentTools = (tools: string[]) => [...new Set([...tools, ...EXPERIMENT_TOOLS])];
const removeExperimentTools = (tools: string[]) => tools.filter(name => !EXPERIMENT_TOOLS.has(name));
export const createAutoresearchExtension: ExtensionFactory = api => {
	const runtimeStore = S.createRuntimeStore();
	...
	const rehydrate = async (ctx: ExtensionContext): Promise<void> => {
		...
		const workDir = H.resolveWorkDir(ctx.cwd);
		const reconstructed = S.reconstructStateFromJsonl(workDir);
		const control = S.reconstructControlState(ctx.sessionManager.getBranch());
		runtime.state = S.cloneExperimentState(reconstructed.state);
		runtime.state.maxExperiments = H.readMaxExperiments(ctx.cwd);
		...
		applyPendingRunToRuntime(runtime, await H.readPendingRunSummary(workDir, loggedRunNumbers));
	...
	const blocked = (reason: string) => ({ block: true as const, reason });
	api.on("tool_call", (event, ctx) => {
		...
		for (const rawPath of rawPaths) {
			...
			if (!relativePath.ok) return blocked(relativePath.reason);
			const validationError = validateEditableAutoresearchPath(relativePath.relativePath, runtime);
			if (validationError)
				return blocked(`Autoresearch blocked edits to ${relativePath.relativePath}: ${validationError}`);
		}
	});
	api.registerCommand("autoresearch", {
		...
		async handler(args, ctx): Promise<void> {
			...
			const runtime = getRuntime(ctx);
			const workDirError = H.validateWorkDir(ctx.cwd);
			if (workDirError) {
			...
			if ((trimmed === "" && runtime.autoresearchMode) || trimmed === "off" || trimmed === "clear") {
				if (trimmed === "clear") {
					...
					runtime.state = S.createExperimentState();
					runtime.state.maxExperiments = H.readMaxExperiments(ctx.cwd);
					runtime.goal = null;
			...
			const shouldResumeExistingNotes =
				fs.existsSync(autoresearchMdPath) &&
				(fs.existsSync(path.join(workDir, "autoresearch.jsonl")) ||
					fs.existsSync(path.join(workDir, ".autoresearch")) ||
					(S.reconstructControlState(ctx.sessionManager.getBranch()).lastMode !== "clear" &&
						trimmed.length === 0));
	...
	api.on("before_agent_start", async (event, ctx) => {
		...
		const ideasPath = path.join(workDir, "autoresearch.ideas.md");
		const pendingRun = await refreshPendingRun(runtime, workDir);
		const currentSegmentResults = S.currentResults(runtime.state.results, runtime.state.currentSegment);
		const baselineMetric = S.findBaselineMetric(runtime.state.results, runtime.state.currentSegment);
		const bestKept = S.findBestKeptResult(runtime.state);
		...
		const recentResults = currentSegmentResults.slice(-3).map(result => {
			...
			return {
				...
				metric_display: H.formatNum(result.metric, runtime.state.metricUnit),
		...
		return {
			systemPrompt: prompt.render(promptTemplate, {
				...
				baseline_metric_display: H.formatNum(baselineMetric, runtime.state.metricUnit),
				has_best_result: Boolean(bestKept),
				best_metric_display: bestKept
					? H.formatNum(bestKept.result.metric, runtime.state.metricUnit)
					: H.formatNum(baselineMetric, runtime.state.metricUnit),
				...
				pending_run_metric_display: hasPendingMetric
					? H.formatNum(pendingRun!.parsedPrimary!, runtime.state.metricUnit)
					: null,
			}),
		};
	});
...
function resolveAutoresearchRelativePath(
	...
	return { ok: true, relativePath: relativePath.length === 0 ? "." : H.normalizeAutoresearchPath(relativePath) };
}
function validateEditableAutoresearchPath(relativePath: string, runtime: AutoresearchRuntime): string | null {
	if (H.isAutoresearchLocalStatePath(relativePath))
	...
	if (H.isAutoresearchCommittableFile(relativePath)) return null;
...
const collectLoggedRunNumbers = (results: ExperimentResult[]): Set<number> =>
	new Set(results.map(r => r.runNumber).filter((n): n is number => n !== null));
...
function summaryToChecks(summary: PendingRunSummary | null): AutoresearchRuntime["lastRunChecks"] {
	...
	return { pass: summary.checksPass, output: "", duration: summary.checksDurationSeconds ?? 0 };
}
...
async function refreshPendingRun(runtime: AutoresearchRuntime, workDir: string): Promise<PendingRunSummary | null> {
	const pendingRun =
		...
		(await H.readPendingRunSummary(workDir, collectLoggedRunNumbers(runtime.state.results)));
