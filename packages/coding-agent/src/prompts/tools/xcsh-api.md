...
|forward_proxy|`"no_forward_proxy":{}` OR `"active_forward_proxy_policies":{"active_forward_proxy_policies":[{"name":"<n>","namespace":"<ns>"}]}`|prereq: create `forward_proxy_policys` with `spec:{"drp_http_connect":{},"allow_all":{}}` — drp_http_connect **REQUIRED**. When asked to create an SMSv2 site *with* a forward proxy policy, the target resource is `securemesh_site_v2s` — POST the site with the policy reference in `active_forward_proxy_policies`, do NOT create a `forward_proxy_policys` resource|
