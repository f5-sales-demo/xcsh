...
| Steering, interruption and explicit task cancellation | API and race coverage pass; spoken-output interruption passed manually. On `12d945ac4-r2`, remote steering retained a live Terra terminal turn ID, suppressed its original file and created one exact replacement; a separate interrupted turn persisted as `interrupted` without its delayed file | Repeat steer and explicit task cancellation from the iPhone |
...
| Voice closure preserves agent work | Unit lifecycle checks and normal manual closure pass. On `12d945ac4-r2`, a real WebRTC v3 call closed while delayed Astra work remained active; the work subsequently completed with an exact 31-byte no-newline fixture | Confirm the same behavior from the iPhone |
...
limited to the explicit iPhone presentation/control/lifecycle observations, the
four exact voice-created fixtures, expired/revoked subscription recovery and
re-pairing, then final documentation reconciliation, PR/CI, squash merge and
cleanup. The accepted Plan transcript-continuity checkpoint is not repeated.
