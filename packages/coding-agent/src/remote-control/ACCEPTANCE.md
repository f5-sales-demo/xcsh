...
| Questions and approvals in both interfaces | Broker, grouped ask, plan ownership and execution preparation tests pass; generated input requests now reach voice without consuming answers; dedicated session and transport tests pass | iPhone grouped/plan presentation, custom UI, dedicated approvals, remaining concurrent control/retry coverage and manual decision acceptance |
