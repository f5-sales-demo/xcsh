...
| Preserve session identity, name, cwd, history and work model | Alpha/Beta retain Luna/Astra; four-model lifecycle SDK tests preserve identity/history/model; stale lifecycle/configuration callbacks reject or retire; disposal settles work before storage closes | Final packaged CLI model restoration, lifecycle and phone acceptance |
...
| TDD and repository checks | Recorded red-to-green work; configuration checkpoint package suite 7644 pass, 561 skips, zero failures; cancellation/configuration lifecycle tests, workspace TypeScript and CLI bundle analysis | Final affected suites and required CI; existing full-history PII findings remain unresolved |
