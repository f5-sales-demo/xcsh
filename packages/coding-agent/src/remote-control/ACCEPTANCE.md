...
| Discover every live top-level terminal, exclude subagents | Two dedicated terminals routed correctly; 128 owner sockets discovered; a last-page selection routes once in integration tests | Verify creation, internal-agent exclusion, stale heartbeats and final live discovery |
| Preserve session identity, name, cwd, history and work model | Alpha/Beta resumed with original identities and Luna/Astra models; AgentSession new/fork/resume/reload/branch tests refresh adapter identity and history; local host registration changes before a switch returns | Repair implicit model-resume mismatch; verify rename/exit and transitions across host restart and on the phone |
...
| TDD and repository checks | Recorded red-to-green work; lifecycle suite 7485 pass, 561 skips, zero failures; 259 focused tests, workspace TypeScript and staged privacy checks pass | Final affected suites and required CI; existing full-history PII findings remain unresolved |
