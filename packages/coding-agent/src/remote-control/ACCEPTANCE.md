...
| Canonical complete history and pagination | Pinned turn/item page schemas, defaults, filtering, summary/full/unloaded views, scoped bidirectional cursors and fresh reads pass; voice provenance and canonical voice notifications exist | Durable selected-branch history through compaction, mixed `thread/timeline/list`, tool items, and stable turn/item identities across steering and restart |
| Typed prompts and streamed turns | Alpha/Beta manual typed routing; adapter tests | Complete stream phases/tool events and schema-level history consistency |
...
| TDD and repository checks | Recorded red-to-green work; pagination suite 7433 pass, 561 skip, zero failures; TypeScript and scoped privacy checks pass | Final affected suites and required CI; existing full-history PII findings remain unresolved |
