...
| Canonical complete history and pagination | Persisted branch and mixed timeline; command identity/outcomes survive reload; foreground/background output streams with current previews | Cancelled background persistence, older command provenance, file items, remaining visible message types, active attachment/fork lifecycle, experimental handling and phone history acceptance |
