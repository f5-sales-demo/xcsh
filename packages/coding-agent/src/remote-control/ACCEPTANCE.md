...
| Canonical complete history and pagination | Persisted branch and mixed timeline; stable command IDs, actual outcomes and late background completion survive reload | Command output deltas, cancelled background persistence, older command provenance, file items, remaining visible message types, active attachment/fork lifecycle, experimental handling and phone history acceptance |
...
| Initial context, speech boundaries and transcripts | Recorded timeline/replay state; turn-scoped mode, default/empty instructions and retained-context recovery match the pinned snapshot; real tool/pruning/resume/identity tests pass | Final live mode/context/history acceptance; response-item promotion and remaining recovery matrix |
| Delegation and speakable output | Reference and native both created/read fixture; v3 streaming, completed v1 and response-item output pass 32 four-model adapter variants; explicit speech and prefixed items match pinned Rust budget fixtures | Fresh live streaming/item-mode capture, remaining cancellation and canonical timeline promotions |
...
| TDD and repository checks | Recorded red-to-green work; timeline-reducer package suite 7855 pass, 561 skips, zero failures; 512 remote and 234 utility tests pass; agent-core suite, pinned Rust fixtures and TypeScript/bundle checks pass | Final affected suites and required CI; 218 existing HEAD PII-shaped findings across 13 files remain unresolved |
