...
| Preserve session identity, name, cwd, history and work model | Alpha/Beta retained their exact IDs, names, cwd, message counts and Luna/Astra models through a controlled handoff from the first phone binary to the clean `62def29d6` build; phone rename now follows pinned schemas, broadcasts to discovery clients and persists through reload/resume; four-model lifecycle SDK tests preserve identity/history/model; stale lifecycle/configuration callbacks reject or retire; disposal settles work before storage closes; compiled extension-model resume passes | Final phone acceptance on the clean build |
...
| Host restart and surviving terminal reconnection | Local socket restart keeps a real AgentSession/tool running; EOF repair restores registration; host relay errors reconnect safely; compiled SIGKILL recovery reclaims the owned stale socket, reattaches both terminals, completes active work and deduplicates retry; the clean live host replacement reattached Alpha/Beta unchanged and returned to a connected relay | Final phone acceptance against the release artifact |
...
| Sol, Luna, Terra and Astra fidelity | Parameterized adapter tests; the clean live runtime exposes four distinct Luna/Astra/Sol/Terra sessions; Sol and Terra each completed a real write/read tool task with exact file bytes; isolated model-registry SDK tests preserve all four identities on resume | Final phone voice task in each clean-build session; no model replacement by voice component |
| No Codex installation or subprocess dependency | Current compiled host and two real TUIs pass ten isolated Ubuntu checks without Codex, standalone Bun or source dependencies; real file tools, remote rename, clean and SIGKILL host recovery, active-work continuation, replay and compiled resume verified | Preserve this check on the final release artifact; live subscription/voice evidence remains separate |
...
| TDD and repository checks | Recorded red-to-green work; latest guarded package suite 8264 pass, 561 skips and zero failures before the final host recovery changes; 884 current remote tests, workspace types/lint, bundle and ten-check compiled artifact pass; pinned Rust fixtures and source hashes recorded | Final affected package suite and required CI; 218 existing HEAD PII-shaped findings across 13 files remain unresolved |
