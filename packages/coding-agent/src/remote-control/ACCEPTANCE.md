...
| Preserve session identity, name, cwd, history and work model | Alpha/Beta preserved Luna/Astra; AgentSession transition tests refresh identity/history; local restart retains a renamed active owner; disposal unregisters before storage closes | Repair implicit model-resume mismatch; verify final packaged lifecycle and phone behavior |
...
| TDD and repository checks | Recorded red-to-green work; recovery suite 7490 pass, 561 skips, zero failures; 252 focused remote tests, workspace TypeScript and staged privacy checks pass | Final affected suites and required CI; existing full-history PII findings remain unresolved |
