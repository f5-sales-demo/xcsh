...
| Preserve session identity, name, cwd, history and work model | Alpha/Beta preserved Luna/Astra; transition/restart tests preserve identity/history; four-model SDK tests retain selected models across new/resume/branch/handoff; disposal unregisters before storage closes | Final packaged CLI model restoration, lifecycle and phone acceptance |
...
| Questions and approvals in both interfaces | Shared broker, pinned schemas, active-tool mapping, client routing, queued widgets, host-restart denial, retirement/expiry/snapshot recovery and host bounds pass | Grouped/multiselect/custom UI, explicit plan and dedicated approvals, remaining control races and manual iPhone decision acceptance |
...
| Initial context, speech boundaries and transcripts | Reference/native recordings; timeline pages preserve speech boundaries, replay identity and selected-branch voice state | Full context update/recovery, response-item promotion and final live canonical history acceptance |
...
| TDD and repository checks | Recorded red-to-green work; interaction-retirement package suite 7548 pass, 561 skips, zero failures; final 324 focused tests, workspace TypeScript and staged privacy checks pass | Final affected suites and required CI; existing full-history PII findings remain unresolved |
