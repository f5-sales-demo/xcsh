...
| Preserve session identity, name, cwd, history and work model | Alpha/Beta retained their exact IDs, names, cwd, message counts and Luna/Astra models through a controlled handoff from the first phone binary to the clean `62def29d6` build; four-model lifecycle SDK tests preserve identity/history/model; stale lifecycle/configuration callbacks reject or retire; disposal settles work before storage closes; compiled extension-model resume passes | Final phone acceptance on the clean build |
