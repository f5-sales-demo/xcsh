...
export class StatusLineComponent implements Component {
   ...
   private buildStatusLine(): string {
      ...
      if (state.model?.reasoning) {
         ...
         if (level !== "off") {
            // Shorten thinking level labels
            const shortLevel =
               level === "minimal" ? "min" : level === "medium" ? "mid" : level === "high" ? "hi" : level === "xhigh" ? "max" : level;
            modelContent += ` · ${shortLevel}`;
         }
      }
      ...
      if (totalTokens) {
         spendParts.push(theme.fg("statusLineOutput", `${ICONS.tokens} ${formatTokens(totalTokens)}`));
      }

      const usingSubscription = state.model ? this.session.modelRegistry.isUsingOAuth(state.model) : false;
      if (totalCost || usingSubscription) {
         const costDisplay = `$${totalCost.toFixed(2)}${usingSubscription ? " (sub)" : ""}`;
         spendParts.push(theme.fg("statusLineCost", costDisplay));
      }

      const spendContent = spendParts.join(" · ");
