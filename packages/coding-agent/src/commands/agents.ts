 ...
 */
import { Args, Command, Flags, renderCommandHelp } from "@f5-sales-demo/pi-utils/cli";
...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.xcsh/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.xcsh/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  xcsh agents unpack",
		"# Export bundled agents into project config\n  xcsh agents unpack --project",
		"# Overwrite existing local agent files\n  xcsh agents unpack --project --force",
		"# Export into a custom directory\n  xcsh agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("xcsh", "agents", Agents);
			return;
		}
