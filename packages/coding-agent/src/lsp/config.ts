...
import { $which, isRecord, logger } from "@f5-sales-demo/pi-utils";
...
function getConfigPaths(cwd: string): string[] {
	...
	// Project config directories (.xcsh/, .pi/)
	...
	// User config directories (~/.xcsh/agent/, ~/.pi/agent/, ~/.xcsh/)
 ...
 * 2. Project config dirs: .xcsh/lsp.*, .pi/lsp.*, .xcsh/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.xcsh/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.xcsh/lsp.* (+ hidden variants)
