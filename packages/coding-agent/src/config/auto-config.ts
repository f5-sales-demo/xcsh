...
/**
 * Default model role value written into config.yml for the F5 LiteLLM proxy.
 * `claude-opus-4-8` is the id the gateway serves (via the anthropic passthrough);
 * its catalog entry carries the `context-1m-2025-08-07` beta for 1M context.
 */
export const DEFAULT_MODEL_ROLE_VALUE = "anthropic/claude-opus-4-8";
...
export function generateConfigYml(): string {
	return [
		"# Auto-generated by xcsh for LiteLLM proxy",
		"modelRoles:",
		`  default: ${DEFAULT_MODEL_ROLE_VALUE}`,
		"",
		"providers:",
		"  image: openai",
		"  webSearch: anthropic",
...
/**
 * Ensure config.yml has a usable modelRoles.default entry.
 * Two failure modes are healed:
 *  1. No `modelRoles:` section at all (older configs) — prepend the default.
 *  2. A `default:` that points at a provider that can never resolve at runtime
 *     (e.g. `bench-instant/bench-instant` leaked from a benchmark run) — rewrite
 *     it to the F5 proxy default. Left unhealed, this triggers the catalog-wide
 *     fallback that surfaced as the AWS-SSO / invalid-model errors.
 */
export function healConfigYmlModelRoles(configPath: string): void {
	try {
		...
		// modelRoles present — repair a default that can never resolve at runtime.
