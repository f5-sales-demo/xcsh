...
class CiCapacityContractTests(unittest.TestCase):
    ...
    def test_manual_benchmark_is_frozen_and_bounded(self) -> None:
        ...
        self.assertIn('harness_commit=$(git -C "${{ github.action_path }}/../../.." rev-parse HEAD)', action)
        ...
        self.assertNotIn("FROZEN_SOURCE_SHA", legacy_setup)
        self.assertIn('[[ "$expected_source_sha" =~ ^[0-9a-f]{40}$ ]] || usage', legacy_setup)
        self.assertIn("checked-out source SHA $source_commit does not match expected source SHA $expected_source_sha", legacy_setup)
        self.assertIn('"$PROFILE_PHASE_SET" "$PROFILE_SOURCE_SHA"', action)
        self.assertIn("qualification harness SHA $harness_commit does not match expected source SHA $PROFILE_SOURCE_SHA", action)
        ...
        self.assertIn("--concurrent is not supported", runner)
        self.assertIn("workers-${{ inputs.file-workers }}", action)
    ...
    def test_dag_qualification_uses_one_image_and_real_job_dependencies(self) -> None:
        benchmark = (WORKFLOWS / "compute-benchmark.yml").read_text(encoding="utf-8")
        self.assertIn(
            "image-candidate|d16-serial|d16-parallel|d16-hardware|dag-control|dag-candidate)",
            benchmark,
        )
