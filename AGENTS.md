...
| Binary lookup   | `$which("git")` from `@f5-sales-demo/pi-utils` | `spawnSync(["which", "git"])`   |
...
- **Tabs → spaces**: Always pass displayed text through `replaceTabs()` before rendering. Tabs produce variable-width gaps in terminals and cause visual holes in the TUI. Import from `@f5-sales-demo/pi-tui` or `../tools/render-utils`.
...
- **Internal changes (from issues)**: `Fixed foo bar ([#123](https://github.com/f5-sales-demo/xcsh/issues/123))`
- **External contributions**: `Added feature X ([#456](https://github.com/f5-sales-demo/xcsh/pull/456) by [@username](https://github.com/username))`
