...
impl builtins::Command for ReadCommand {
	...
	async fn execute<SE: brush_core::ShellExtensions>(
		...
		let read_result =
			self.read_line(input_stream, context.stderr(), timeout, context.cancel_token())?;
...
struct InputReader {
	...
	/// Cancellation shared with the enclosing execution.
...
impl InputReader {
	...
	fn read_event(&mut self) -> Result<InputEvent, brush_core::Error> {
		...
		{
			return Ok(InputEvent::CtrlC);
		}

		// Check timeout before attempting read.
		if self.deadline.is_some() || self.cancel_token.is_some() {
			let remaining = self
				.deadline
			...
			// Poll in bounded slices so cancellation is observed without consuming input.
			loop {
				match brush_core::sys::poll::poll_for_input(&self.input, remaining) {
					...
					Err(e) if e.kind() == std::io::ErrorKind::Unsupported => break,
...
impl ReadCommand {
	...
	fn read_line(
		...
		let mut reader = InputReader::new(input_file, timeout, term_mode, cancel_token);
...
mod tests {
	...
	fn test_build_array_fields_none_input() {
		...
		assert_eq!(
			result,
			[] as [(std::option::Option<std::string::String>, std::string::String); 0]
		);
	}
