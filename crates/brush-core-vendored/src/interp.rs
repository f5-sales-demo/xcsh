...
async fn spawn_ao_list_as_job<'a, SE: extensions::ShellExtensions>(
...
async fn try_spawn_pipeline_as_job<SE: extensions::ShellExtensions>(
	...
	// An asynchronous list without explicit standard input reads from /dev/null.
	if let Ok(null) = openfiles::null() {
		child_params.set_fd(openfiles::OpenFiles::STDIN_FD, null);
	}

	let spawn_results = spawn_pipeline_processes(pipeline, &mut subshell, &child_params).await?;
...
fn spawn_ao_list_in_task<SE: extensions::ShellExtensions>(
	ao_list: &ast::AndOrList,
	shell: &Shell<SE>,
	params: &ExecutionParameters,
) -> jobs::Job {
...
async fn wait_for_pipeline_processes_and_update_status(
	...
	while let Some(child) = process_spawn_results.pop_front() {
		...
		let wait_result = if !stopped_children.is_empty() {
			...
			child.wait(params.cancel_token()).await?
		};
...
impl Execute for ast::CoprocessCommand {
	async fn execute(
		...
		let child_cancel = child_params.cancel_token();
		...
		let join_handle = tokio::spawn(async move {
			...
			let spawn_result = body
				...
				.await?;
			match spawn_result.wait(child_cancel).await? {
				...
				ast::IoFileRedirectTarget::Filename(f) => {
					...
					let opened_file = shell
						.open_file(
							...
							params,
							match kind {
								ast::IoFileRedirectKind::Read => crate::containment::FenceAccess::Read,
								_ => crate::containment::FenceAccess::Write,
							},
						)
						.map_err(|err| {
...
fn setup_redirect_output_and_error_to(
	...
	let stdout_file = shell
		.open_file(&file_options, &abs_file_path, params, crate::containment::FenceAccess::Write)
		.map_err(|err| {
			error::ErrorKind::RedirectionFailure(
