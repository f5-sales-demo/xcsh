...
**Path Mappings** (in `/Users/user/workspaces/pi-mono/tsconfig.json`):
```json
"paths": {
...
- Automated version sync script: `/Users/user/workspaces/pi-mono/scripts/sync-versions.js`
...
**Add to `/Users/user/workspaces/pi-mono/tsconfig.json` paths:**
```json
"paths": {
  "@mariozechner/pi-tui": ["./packages/tui/src/index.ts"],
  "@mariozechner/pi-agent": ["./packages/agent/src/index.ts"],
...
**Update build script in root `/Users/user/workspaces/pi-mono/package.json`:**
```json
...
If your package depends on other monorepo packages, add synchronization logic to `/Users/user/workspaces/pi-mono/scripts/sync-versions.js`.
...
After thoroughly analyzing the existing agent package (`/Users/user/workspaces/pi-mono/packages/agent`), here are the comprehensive requirements for a unified AI API based on current usage patterns:
