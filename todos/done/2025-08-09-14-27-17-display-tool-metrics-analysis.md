...
The token usage is defined as an event type in `/Users/user/workspaces/pi-mono/packages/agent/src/agent.ts` (lines 16-23):
...
The agent defines a specific event structure for tool calls in `/Users/user/workspaces/pi-mono/packages/agent/src/agent.ts`:
...
- **Session Manager**: All AgentEvents (including tool calls) are stored in `/Users/user/workspaces/pi-mono/packages/agent/src/session-manager.ts`. Each event is logged to a JSONL file with timestamps.
...
### 1. Console Renderer (`/Users/user/workspaces/pi-mono/packages/agent/src/renderers/console-renderer.ts`)
...
### 2. JSON Renderer (`/Users/user/workspaces/pi-mono/packages/agent/src/renderers/json-renderer.ts`)
...
### 3. TUI Renderer (`/Users/user/workspaces/pi-mono/packages/agent/src/renderers/tui-renderer.ts`)
...
Based on the `AgentEvent` type definition in `/Users/user/workspaces/pi-mono/packages/agent/src/agent.ts`:
